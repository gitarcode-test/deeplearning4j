...
/**
 * A downloadable resource represents a resource that is downloadable and cached in a specified
 * directory. This will usually be in the user's home directory under a hidden dot directory.
 */
public interface DownloadableResource {
  ...
  /**
   * The local path to the file. This will usually be {@link #localCacheDirectory()} + {@link
   * #fileName()}
   *
   * @return
   */
  ...
  default void delete() throws IOException {
    if (localPath().isDirectory()) FileUtils.deleteDirectory(localPath());
    else FileUtils.forceDelete(localPath());
  }
}
