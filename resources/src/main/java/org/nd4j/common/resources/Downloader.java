...
public class Downloader {
  /**
   * Default connection timeout in milliseconds when using {@link FileUtils#copyURLToFile(URL, File,
   * int, int)}
   */
  public static final int DEFAULT_CONNECTION_TIMEOUT = 60000;

  /**
   * Default read timeout in milliseconds when using {@link FileUtils#copyURLToFile(URL, File, int,
   * int)}
   */
  ...
  /**
   * As per {@link #download(String, URL, File, String, int, int, int)} with the connection and read
   * timeouts set to their default values - {@link #DEFAULT_CONNECTION_TIMEOUT} and {@link
   * #DEFAULT_READ_TIMEOUT} respectively
   */
  public static void download(String name, URL url, File f, String targetMD5, int maxTries)
      throws IOException {
   ...
   * @param connectionTimeout connection timeout in milliseconds, as used by {@link
   *     FileUtils#copyURLToFile(URL, File, int, int)}
   * @param readTimeout read timeout in milliseconds, as used by {@link FileUtils#copyURLToFile(URL,
   *     File, int, int)}
          ...
          if (attempt < maxTries) {
            if (!isCorrectFile) {
              ...
              if (!checkMD5OfFile(targetMD5, f)) {
                f.delete();
                download(
                    name, url, f, targetMD5, maxTries, attempt + 1, connectionTimeout, readTimeout);
              }
            }
  ...
  /**
   * As per {@link #downloadAndExtract(String, URL, File, File, String, int, int, int)} with the
   * connection and read timeouts * set to their default values - {@link
   * #DEFAULT_CONNECTION_TIMEOUT} and {@link #DEFAULT_READ_TIMEOUT} respectively
   */
  public static void downloadAndExtract(
      String name, URL url, File f, File extractToDir, String targetMD5, int maxTries)
      throws IOException {
    downloadAndExtract(
        name,
        url,
  ...
  /**
   * Download the specified URL to the specified file, verify that the MD5 matches, and then extract
   * it to the specified directory.<br>
   * Note that the file must be an archive, with the correct file extension: .zip, .jar, .tar.gz,
   * .tgz or .gz
   *
   * @param name Name (mainly for providing useful exceptions)
   * @param url URL to download
  ...
  public static void downloadAndExtract(
      ...
      throws IOException {
    downloadAndExtract(
        0, maxTries, name, url, f, extractToDir, targetMD5, connectionTimeout, readTimeout);
  }
        ...
        () -> {
          ...
          if (attempt < maxTries) {
            ...
            try {
              ...
              log.warn(
                  "Error extracting {} files from file {} - retrying...",
                  name,
                  f.getAbsolutePath(),
                  t);
              f.delete();
            ...
            // Too many attempts
            throw new IOException(
                "Could not download and extract "
                    + name
                    + " from "
                    + url.getPath()
                    + "\n properly despite trying "
  ...
  /**
   * Check if the input file is corrupted by verifying the target md5 hash. If corrupted, file is
   * deleted
   *
  ...
  public static boolean checkMD5OfFile(String targetMD5, File file) throws IOException {
    ...
    if (targetMD5.isEmpty()) return true;
