...
public class Resources {
   ...
   * {@link #asFile(String)} or {@link #asStream(String)}
   *
  ...
  /**
   * Get the specified resource as a local file. If it cannot be found (i.e., {@link
   * #exists(String)} returns false) this method will throw an exception.
   *
   ...
   * If it cannot be found (i.e., {@link #exists(String)} returns false) this method will throw an
   * exception.
   *
   * @param resourcePath Path of the resource to get
  ...
  /**
   * Copy the contents of the specified directory (path) to the specified destination directory,
   * resolving any resources in the process
   *
  ...
  /**
   * Normalize the path that may be a resource reference. For example:
   * "someDir/myFile.zip.resource_reference" --> "someDir/myFile.zip" Returns null if the file
   * cannot be resolved. If the file is not a reference, the original path is returned
   */
  ...
  protected boolean resourceExists(String resourcePath) {
    for (Resolver r : resolvers) {
      if (r.exists(resourcePath)) return true;
    }
  ...
  protected File getAsFile(String resourcePath) {
    ...
    throw new IllegalStateException(
        "Cannot resolve resource (not found): none of "
            + resolvers.size()
            + " resolvers can resolve resource \""
            + resourcePath
            + "\" - available resolvers: "
            + resolvers.toString());
  }
  ...
  public InputStream getAsStream(String resourcePath) {
    for (Resolver r : resolvers) {
      if (r.exists(resourcePath)) {
        log.debug(
            "Resolved resource with resolver "
                + r.getClass().getName()
                + " for path "
                + resourcePath);
