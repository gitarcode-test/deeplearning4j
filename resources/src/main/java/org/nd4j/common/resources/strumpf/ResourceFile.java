...
public class ResourceFile {
  /**
   * Default value for resource downloading connection timeout - see {@link
   * ND4JSystemProperties#RESOURCES_CONNECTION_TIMEOUT}
   */
  public static final int DEFAULT_CONNECTION_TIMEOUT =
      60000; // Timeout for connections to be established

  /**
   * Default value for resource downloading read timeout - see {@link
   * ND4JSystemProperties#RESOURCES_READ_TIMEOUT}
   */
  public static final int DEFAULT_READ_TIMEOUT =
      60000; // Timeout for amount of time between connection established and data is available
  ...
  public String relativePath() {
    ...
    if (hashKey == null) {
      throw new IllegalStateException(
          "Could not find <filename>_hash in resource reference file: " + filePath);
    }
  ...
  /**
   * Get the local file - or where it *would* be if it has been downloaded. If it does not exist, it
   * will not be downloaded here
   *
   * @return
   */
  protected File getLocalFile(File cacheRootDir) {
    ...
    // This is to support multiple versions of files simultaneously... for example, different
    // projects needing different
  ...
  public File localFile(File cacheRootDir) {
    ...
    Preconditions.checkState(
        remotePath != null, "No remote path was found in resource reference file %s", filePath);
    ...
    Preconditions.checkState(
        sha256Compressed != null,
        "Expected JSON property %s was not found in resource reference file %s",
        sha256PropertyCompressed,
        filePath);
    ...
    int connectTimeout =
        validCTimeout ? Integer.parseInt(connTimeoutStr) : DEFAULT_CONNECTION_TIMEOUT;
    ...
    try {
      ...
      for (int tryCount = 0; tryCount < MAX_DOWNLOAD_ATTEMPTS; tryCount++) {
        try {
          ...
          if (!correctHash) {
            log.warn(
                "Download of file {} failed: expected hash {} vs. actual hash {}",
                remotePath,
                sha256Compressed,
                hash);
            continue;
      ...
      if (!correctHash) {
        throw new RuntimeException(
            "Could not successfully download with correct hash file after "
      ...
      if (!extractedHash.equals(sha256Uncompressed)) {
        throw new RuntimeException(
            "Extracted file hash does not match expected hash: "
                + remotePath
                + " -> "
                + f.getAbsolutePath()
                + " - expected has "
