...
/**
 * The op descriptor for the libnd4j code base. Each op represents a serialized version of {@link
 * org.nd4j.linalg.api.ops.DynamicCustomOp} that has naming metadata attached.
 *
 * @author Adam Gibson
 */
@Data
...
public class OpDeclarationDescriptor implements Serializable {
  ...
  @Builder.Default private Map<String, Boolean> argOptional = new HashMap<>();
  ...
  public void validate() {
    ...
    if (nOut >= 0 && nOut != outArgNames.size() && !isVariableOutputSize()) {
      System.err.println(
          "Output arg names was not equal to number of outputs found for op " + name);
    }
    ...
    if (iArgs >= 0 && iArgs != iArgNames.size() && !isVariableIntArgs()) {
      System.err.println(
          "Integer arg names was not equal to number of integer args found for op " + name);
    }
  }

  /**
   * Returns true if there is a variable number of integer arguments for an op
   *
   * @return
   */
  ...
  /**
   * Returns true if there is a variable number of t arguments for an op
   *
   * @return
   */
  ...
  /**
   * Returns true if the number of inputs is variable size
   *
   * @return
   */
