...
/**
 * Parses the libnd4j code base based on a relative path default of ../deeplearning4j/libnd4j or a
 * passed in file path. It generates a descriptor for each op. The file properties can be found at
 * {@link OpDeclarationDescriptor}
 *
 * @author Adam Gibson
 */
public class ParseOpFile {
  ...
  public static void main(String... args) throws Exception {
    String libnd4jPath =
        args.length > 0 ? args[0] : Libnd4jArgDescriptorSource.DEFAULT_LIBND4J_DIRECTORY;
    String outputFilePath =
        args.length > 1 ? args[1] : ArgDescriptorParserUtils.DEFAULT_OUTPUT_FILE;
    ...
    System.out.println(
        "Parsing  libnd4j code base at "
            + libnd4jRootDir.getAbsolutePath()
            + " and writing to "
            + outputFilePath);
    Libnd4jArgDescriptorSource libnd4jArgDescriptorSource =
        Libnd4jArgDescriptorSource.builder().libnd4jPath(libnd4jPath).weight(99999.0).build();

    JavaSourceArgDescriptorSource javaSourceArgDescriptorSource =
        JavaSourceArgDescriptorSource.builder().nd4jApiRootDir(nd4jApiRootDir).weight(1.0).build();
    ...
    for (ArgDescriptorSource argDescriptorSource :
        new ArgDescriptorSource[] {libnd4jArgDescriptorSource, javaSourceArgDescriptorSource}) {
    ...
    for (Map.Entry<String, List<ArgDescriptorProposal>> proposal : proposals.entrySet()) {
      ...
      Map<String, List<ArgDescriptorProposal>> collect =
          proposal.getValue().stream()
      ...
      Map<Pair<Integer, OpNamespace.ArgDescriptor.ArgType>, OpNamespace.ArgDescriptor>
          rankedProposals = ArgDescriptorParserUtils.standardizeNames(collect, proposal.getKey());
      OpNamespace.OpDescriptor.Builder opDescriptorBuilder =
          OpNamespace.OpDescriptor.newBuilder()
              .setOpDeclarationType(opTypes.get(proposal.getKey()))
              .setName(proposal.getKey());
      rankedProposals.entrySet().stream()
    ...
    OpNamespace.OpDescriptorList.Builder sortedListBuilder =
        OpNamespace.OpDescriptorList.newBuilder();
    ...
    for (int i = 0; i < listBuilder.getOpListCount(); i++) {
      ...
      Map<OpNamespace.ArgDescriptor.ArgType, List<OpNamespace.ArgDescriptor>> sortedByType =
          opList.getArgDescriptorList().stream()
              .collect(Collectors.groupingBy(input -> input.getArgType()));
      ...
      sortedByType
          ...
          .forEach(
              entry -> {
                Collections.sort(
                    entry.getValue(), Comparator.comparing(inputArg -> inputArg.getArgIndex()));
                for (int j = 0; j < entry.getValue().size(); j++) {
                  ...
                  if (currDescriptor.getArgIndex() != j
                      && !namesEncountered.contains(currDescriptor.getName())) {
                    throw new IllegalStateException(
                        "Op name "
                            + opList.getName()
                            + " has incontiguous indices for type "
                            + entry.getKey()
                            + " with descriptor being "
                            + currDescriptor);
                  } else if (currDescriptor.getArgIndex() != j
                      && namesEncountered.contains(currDescriptor.getName())) {
                  ...
                  OpNamespace.ArgDescriptor.Builder newDescriptor =
    ...
    // sort alphabetically
    Collections.sort(
        sortedDescriptors, Comparator.comparing(opDescriptor -> opDescriptor.getName()));
