...
public class JavaSourceArgDescriptorSource implements ArgDescriptorSource {
   ...
   *
   * <p>void addIArgument(int... arg);
   *
   * <p>void addIArgument(long... arg);
   *
   * <p>void addBArgument(boolean... arg);
   *
   * <p>void addDArgument(DataType... arg);
   */
  public static final String ADD_T_ARGUMENT_INVOCATION = "addTArgument";

  public static final String ADD_I_ARGUMENT_INVOCATION = "addIArgument";
  public static final String ADD_B_ARGUMENT_INVOCATION = "addBArgument";
  public static final String ADD_D_ARGUMENT_INVOCATION = "addDArgument";
  public static final String ADD_INPUT_ARGUMENT = "addInputArgument";
  public static final String ADD_OUTPUT_ARGUMENT = "addOutputArgument";
  @Getter private Map<String, OpNamespace.OpDescriptor.OpDeclarationType> opTypes;
  ...
  public Map<String, List<ArgDescriptorProposal>> doReflectionsExtraction() {
    ...
    Set<Class<? extends DifferentialFunction>> subTypesOf =
        reflections.getSubTypesOf(DifferentialFunction.class);
  ...
  private void processClazz(
      ...
      Set<String> opNamesForDifferentialFunction,
      Class<?> clazz) {
    try {
      ...
      else opTypes.put(name, OpNamespace.OpDescriptor.OpDeclarationType.CUSTOM_OP_IMPL);

      String fileName = clazz.getName().replace(".", File.separator);
      StringBuilder fileBuilder = new StringBuilder();
      fileBuilder.append(fileName);
      fileBuilder.append(".java");
      CounterMap<Pair<String, OpNamespace.ArgDescriptor.ArgType>, Integer> paramIndicesCount =
          new CounterMap<>();

      // Our sample is in the root of this directory, so no package name.
      CompilationUnit cu =
          sourceRoot.parse(clazz.getPackage().getName(), clazz.getSimpleName() + ".java");
                      ...
                      .forEach(
                          argument -> {
                            if (argument.isNameExpr())
                              paramIndicesCount.incrementCount(
                                  Pair.of(
                          ...
                          argument -> {
                            if (argument.isNameExpr())
                              ...
                              if (argument
                                ...
                                paramIndicesCount.incrementCount(
                                    Pair.of(
                                        argument.toString().replace(".ordinal()", ""),
                                        OpNamespace.ArgDescriptor.ArgType.INT64),
                                    indexed.get(),
                                    100.0);
                              }
      ...
      List<ResolvedConstructorDeclaration> collect =
        ...
        for (int i = 0; i < parameterDeclaration.getNumberOfParams(); i++) {
          ...
          if (isValidParam(param)) {
            ...
            switch (argType) {
              case INPUT_TENSOR:
                paramIndicesCount.incrementCount(
                    Pair.of(param.getName(), argType), inputIdx, 100.0);
                inputIdx++;
                break;
              case INT64:
              case INT32:
                paramIndicesCount.incrementCount(
                    Pair.of(param.getName(), OpNamespace.ArgDescriptor.ArgType.INT64),
                    intIdx,
                    100.0);
                intIdx++;
                break;
              case DATA_TYPE:
                paramIndicesCount.incrementCount(
                    Pair.of(param.getName(), OpNamespace.ArgDescriptor.ArgType.DATA_TYPE),
                    dTypeIndex,
                    100.0);
                dTypeIndex++;
                break;
              ...
              case FLOAT:
                paramIndicesCount.incrementCount(
                    Pair.of(param.getName(), OpNamespace.ArgDescriptor.ArgType.FLOAT),
                    floatIdx,
                    100.0);
                paramIndicesCount.incrementCount(
                    Pair.of(param.getName(), OpNamespace.ArgDescriptor.ArgType.DOUBLE),
                    floatIdx,
                    100.0);
                floatIdx++;
                break;
              ...
              case OUTPUT_TENSOR:
                paramIndicesCount.incrementCount(
                    Pair.of(param.getName(), argType), outputIdx, 100.0);
                outputIdx++;
                break;
              case UNRECOGNIZED:
                continue;
      ...
      intIdx = 0;
      boolIdx = 0;
      Set<List<Pair<String, String>>> typesAndParams =
          parameters.stream()
              .map(
                  collectedParam ->
      ...
      typesAndParams.forEach(
          listOfTypesAndNames -> {
            listOfTypesAndNames.forEach(
                parameter -> {
                  if (typeNameOrArrayOfTypeNameMatches(
                      parameter.getFirst(), SDVariable.class.getName(), INDArray.class.getName())) {
                    constructorNamesEncountered.add(parameter.getValue());
                    if (outputNames.contains(parameter.getValue())) {
                      Counter<Integer> counter =
                          paramIndicesCount.getCounter(
                              Pair.of(
                                  parameter.getSecond(),
                                  OpNamespace.ArgDescriptor.ArgType.OUTPUT_TENSOR));
                      if (counter != null)
                        finalArgDescriptorProposals.add(
                  ...
                  } else if (typeNameOrArrayOfTypeNameMatches(
                          parameter.getFirst(),
                          int.class.getName(),
                          long.class.getName(),
                          Integer.class.getName(),
                          Long.class.getName())
                      || paramIsEnum(parameter.getFirst())) {
                    constructorNamesEncountered.add(parameter.getValue());

                    Counter<Integer> counter =
                        paramIndicesCount.getCounter(
                            Pair.of(
                                parameter.getSecond(), OpNamespace.ArgDescriptor.ArgType.INT64));
                    if (counter != null)
                      finalArgDescriptorProposals.add(
                          ArgDescriptorProposal.builder()
                              .sourceOfProposal("java")
                    ...
                    Counter<Integer> counter =
                        paramIndicesCount.getCounter(
                            Pair.of(
                                parameter.getSecond(), OpNamespace.ArgDescriptor.ArgType.FLOAT));
                    if (counter != null)
                      finalArgDescriptorProposals.add(
                          ArgDescriptorProposal.builder()
                              .sourceOfProposal("java")
                      ...
                      parameter.getFirst(), boolean.class.getName(), Boolean.class.getName())) {
                    constructorNamesEncountered.add(parameter.getValue());
                    Counter<Integer> counter =
                        paramIndicesCount.getCounter(
                            Pair.of(parameter.getSecond(), OpNamespace.ArgDescriptor.ArgType.BOOL));
                    if (counter != null)
                      finalArgDescriptorProposals.add(
                          ArgDescriptorProposal.builder()
                              .sourceOfProposal("java")
      ...
      for (ResolvedFieldDeclaration field : fields) {
        if (!constructorNamesEncountered.contains(field.getName())
            && typeNameOrArrayOfTypeNameMatches(
                field.getType().describe(), SDVariable.class.getName(), INDArray.class.getName())) {
          if (outputNames.contains(field.getName())) {
            argDescriptorProposals.add(
                ArgDescriptorProposal.builder()
                    .sourceOfProposal("java")
                    .proposalWeight(99.0)
                    .descriptor(
          ...
          floatIdx++;
            ...
            && typeNameOrArrayOfTypeNameMatches(
                field.getType().describe(), Boolean.class.getName(), boolean.class.getName())) {
          argDescriptorProposals.add(
              ArgDescriptorProposal.builder()
                  .sourceOfProposal("java")
                  .proposalWeight(99.0)
      ...
      if (funcInstance instanceof BaseReduceOp
          || funcInstance instanceof BaseReduceBoolOp
          || funcInstance instanceof BaseReduceSameOp) {
      ...
      if (funcInstance instanceof BaseTransformBoolOp) {
        ...
        if (baseTransformBoolOp.getOpType() == Op.Type.PAIRWISE_BOOL) {
          if (numProposalsWithType(
                  OpNamespace.ArgDescriptor.ArgType.INPUT_TENSOR, argDescriptorProposals)
              < 2) {
            argDescriptorProposals.add(
                ArgDescriptorProposal.builder()
                    .sourceOfProposal("java")
      ...
      // hard coded case, impossible to parse from as the code exists today, and it doesn't exist
      // anywhere in the libnd4j code base
      ...
      if (name.equals("while")
          || name.equals("enter")
          || name.equals("exit")
          || name.equals("next_iteration")
          || name.equals("loop_cond")
          || name.equals("switch")
          || name.equals("While")) {
        argDescriptorProposals.add(
            ArgDescriptorProposal.builder()
            ...
            ArgDescriptorProposal.builder()
                ...
                .descriptor(
                    OpNamespace.ArgDescriptor.newBuilder()
                        .setArgIndex(1)
                        .setArgType(OpNamespace.ArgDescriptor.ArgType.BOOL)
                        .setName("halfPixelCenters")
                        .build())
                .build());
      }

      if (funcInstance instanceof BaseTransformSameOp
          || funcInstance instanceof BaseTransformOp
          || funcInstance instanceof BaseDynamicTransformOp) {
        argDescriptorProposals.add(
            ArgDescriptorProposal.builder()
                .proposalWeight(9999.0)
                .descriptor(
  ...
  private SourceRoot initSourceRoot(File nd4jApiRootDir) {
    ...
    SourceRoot sourceRoot =
        new SourceRoot(
            nd4jApiRootDir.toPath(), new ParserConfiguration().setSymbolResolver(symbolSolver));
    return sourceRoot;
  }
