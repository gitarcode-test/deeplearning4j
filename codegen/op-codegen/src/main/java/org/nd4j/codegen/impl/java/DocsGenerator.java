...
public class DocsGenerator {
    ...
    for (DocSection ds : docSections) {
      ...
      for (int i = 0; i < lines.length; i++) {
        if (!lines[i].endsWith("<br>")) {
          String filteredLine =
              new JavaDocToMDAdapter(lines[i])
                  .filter(JD_CODE, "`")
                  .filter(JD_CODE_END, "`")
                  .filter(JD_INPUT_TYPE, "INDArray")
                  .toString();
  ...
  public static void generateDocs(
      int namespaceNum, NamespaceOps namespace, String docsDirectory, String basePackage)
      throws IOException {
    File outputDirectory = new File(docsDirectory);
    StringBuilder sb = new StringBuilder();
    String headerName = namespace.getName();
    if (headerName.startsWith("SD")) headerName = headerName.substring(2);
    ...
    if (ops.size() > 0) sb.append("# Operation classes").append(System.lineSeparator());
    for (Op op : ops) {
      ...
      if (!doc.isEmpty()) {
        ...
        for (Signature s : op.getSignatures()) {
          if (first) {
            ...
            sb.append(MD_CODE)
                .append(lang.equals(Language.ANY) ? Language.JAVA : lang)
                    ...
                    .append(
                        arg.getDescription() == null
                            ...
                            : DocTokens.processDocText(
                                arg.getDescription(),
                                op,
                                DocTokens.GenerationType.ND4J)); // .append(System.lineSeparator());
              } else {
                sb.append("* **")
                    .append(arg.getName())
                    .append("** - ")
    ...
    for (Config config : namespace.getConfigs()) {
      ...
      for (Input i : config.getInputs()) {
        ...
        if (i.hasDefaultValue() && (i.defaultValue() != null))
          sb.append(" Default value:")
              .append(formatDefaultValue(i.defaultValue()))
              .append(System.lineSeparator());
      ...
      for (Arg arg : config.getArgs()) {
        ...
        if (arg.hasDefaultValue() && (arg.defaultValue() != null))
          sb.append(" - default = ")
              .append(formatDefaultValue(arg.defaultValue()))
              .append(System.lineSeparator());
        else sb.append(System.lineSeparator());
      }
          ...
          .forEach(
              op ->
                  sb.append("[")
                      ...
                      .append("]")
                      .append("(#")
                      .append(toAnchor(op.getOpName()))
                      .append(")")
                      .append(System.lineSeparator()));
    }
    File outFile =
        new File(
            outputDirectory + "/operation-namespaces",
            "/" + namespace.getName().toLowerCase() + ".md");
