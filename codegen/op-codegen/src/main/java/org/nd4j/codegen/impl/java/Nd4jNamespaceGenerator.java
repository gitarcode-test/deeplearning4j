...
public class Nd4jNamespaceGenerator {
  ...
  private static ConstraintCodeGenerator constraintCodeGenerator =
      new JavaConstraintCodeGenerator();

  private Nd4jNamespaceGenerator() {}

  public static void generate(
      NamespaceOps namespace,
  ...
  private static void generateOpFactory(
      NamespaceOps namespace,
      File outputDirectory,
      String className,
      String basePackage,
    ...
    } else addDefaultConstructor(builder);

    // Add ops
    namespace.getOps().stream()
    ...
    JavaFile jf =
        StringUtils.isEmpty(parentClass)
    ...
    File outFile =
        new File(outputDirectory, packageToDirectory(opsPackage) + "/" + className + ".java");
  ...
  private static void addDefaultConstructor(TypeSpec.Builder builder) {
    ...
    MethodSpec noArg = MethodSpec.constructorBuilder().addModifiers(Modifier.PUBLIC).build();
  ...
  private static void generateMethods(
      TypeSpec.Builder builder, Op op, boolean isSameDiff, boolean isLoss) {
    List<Signature> l = op.getSignatures();
    for (Signature s : l) {
      builder.addMethod(signatureCreatorMethod(op, s, isSameDiff, false, isLoss));
      if (isSameDiff) builder.addMethod(signatureCreatorMethod(op, s, true, true, isLoss));
    }
  }

  private static MethodSpec signatureCreatorMethod(
      Op op, Signature s, boolean isSameDiff, boolean withName, boolean isLoss) {
    MethodSpec.Builder c =
        MethodSpec.methodBuilder(GenUtil.ensureFirstIsNotCap(op.getOpName()))
            .addModifiers(Modifier.PUBLIC);
      ...
      for (Constraint constraint : constraints) {
        c.addJavadoc(
            constraint.getMessage()
                + ": "
                + constraintCodeGenerator.generateExpression(constraint.getCheck())
                + "<br>\n");
      }
      ...
      for (Parameter p : params) {
        if (p instanceof Input) {
          ...
          c.addJavadoc(
              "@param "
                  ...
                  + (i.getDescription() == null
                      ? ""
                      : DocTokens.processDocText(
                          i.getDescription(), op, DocTokens.GenerationType.ND4J))
                  + " ("
                  + i.getType()
                  + " type)\n");
            ...
            c.addJavadoc(
                "@param "
                    ...
                    + (arg.getDescription() == null
                        ? ""
                        : DocTokens.processDocText(
                            arg.getDescription(), op, DocTokens.GenerationType.ND4J))
                    + "\n");
          } else {
            c.addJavadoc(
                "@param "
          ...
          throw new RuntimeException(
              "Unknown parameter type: " + p + " - " + p.getClass() + " - op = " + op.getOpName());
        }
      }
    }
        ...
        c.addJavadoc(
            "@return "
                ...
                + (o.getDescription() == null
                    ? ""
                    : DocTokens.processDocText(
                        o.getDescription(), op, DocTokens.GenerationType.ND4J))
                + " ("
                + o.getType()
                + " type)\n");
      } else {
        // throw new UnsupportedOperationException("Javadoc for multi-output ops not yet
        // implemented");
  ...
  private static List<String> buildParameters(
      MethodSpec.Builder c, Op op, Signature s, boolean isSameDiff, boolean withName) {
    ...
    if (withName) {
      ...
      else c.addParameter(String[].class, "names");
    }
    if (!params.isEmpty()) {
      int pCount = 0;
      for (Parameter p : params) {
        ...
        if (p instanceof Input) {
          ...
          if (count == null || count.equals(exactlyOne)) {
            // Single input
            if (isSameDiff) c.addParameter(SDVariable.class, inputName);
            else c.addParameter(INDArray.class, inputName);
          } else {
            // Array input
            if (isSameDiff) c.addParameter(SDVariable[].class, inputName).varargs(isLast);
          ...
          if (validationName != null) {
            c.addStatement(
                CodeBlock.of(
                    "$T.$L($S, $S, $L)",
                    isSameDiff ? SDValidation.class : NDValidation.class,
                    validationName,
                    op.getOpName(),
                    inputName,
                    inputName));
          ...
          if (type == null) {
            throw new IllegalStateException(
                "No type mapping has been specified for type "
                    + arg.getType()
                    + " (op="
                    + op.getOpName()
                    + ", arg="
  ...
  public static TypeName getArgType(Arg arg) {
    ...
    if (argType == DataType.ENUM) {
      ...
      if (type == null) {
        throw new IllegalStateException(
            arg + " is using an unregistered ENUM. This is probably a bug.");
      }
    } else {
  ...
  private static void buildConstraints(MethodSpec.Builder c, List<Constraint> constraints) {
    ...
    for (Constraint constraint :
        constraints.stream()
            .filter(it -> !(it instanceof BackendConstraint))
  ...
  private static void buildExecution(
    ...
    if (singleOut) {
      if (isSameDiff) c.returns(SDVariable.class);
      else c.returns(INDArray.class);
    } else {
      if (isSameDiff) c.returns(SDVariable[].class);
      else c.returns(INDArray[].class);
    }
    ...
    // inNames tells us which parameters this signatures has. For all others we want to pass default
    // values
    List<String> parameters =
        op.allParameters().stream()
            .sorted(
                (p1, p2) -> {
                  ...
                  if (inNames.contains(it.name())) {
                    ...
                    if (!it.hasDefaultValue())
                      throw new IllegalStateException(
                          "The parameter "
                              + it.name()
                              + " has no default value, but is also not part of "
                              + inNames.toString());
    ...
    if (isSameDiff) {
      if (withName) {
        if (singleOut) sb.append("SDVariable out = ");
        else sb.append("SDVariable[] out = ");

        sb.append(" new ")
            .append(op.getJavaPackage())
            .append(".")
            .append(
                op.getJavaOpClass() == null
                    ? GenUtil.ensureFirstIsCap(op.getOpName())
                    : op.getJavaOpClass())
            .append("(sd,")
            .append(String.join(", ", parameters))
            .append(")");

        if (singleOut) sb.append(".outputVariable()");
        else sb.append(".outputVariables()");

        c.addStatement(sb.toString());
        if (isLoss) c.addStatement("out.markAsLoss()");

        if (singleOut) c.addStatement("return sd.updateVariableNameAndReference(out, name)");
        else c.addStatement("return sd.updateVariableNamesAndReferences(out, names)");
      } else {
        if (isLoss) {
      ...
      sb.append("return $T.exec(new ")
          ...
          .append("))");
      if (!op.getLegacy()
          && singleOut) // Note: legacy ops Nd4j.exec(Op) returns INDArray; Nd4j.exec(CustomOp)
                        // returns INDArray[]
      sb.append("[0]");
  ...
  private static String countToJava(Count count, String paramName) {
    ...
    if (count instanceof Exactly) {
      ...
      throw new IllegalArgumentException(
          "Can not deal with Count of type " + count.getClass().getName());
    }
  }
  ...
  private static void checkParameterCount(MethodSpec.Builder c, Count count, String paramName) {
    ...
    if (count != null && !count.equals(exactlyOne)) {
      final String errorMessage =
          paramName
              + " has incorrect size/length. Expected: "
              + countToJava(count, paramName)
              + ", got %s";
      if (count instanceof Exactly) {
        c.addStatement(
            CodeBlock.of(
                "$T.checkArgument($L.length == $L, $S, $L)",
                Preconditions.class,
                paramName,
                ((Exactly) count).getCount(),
                errorMessage,
                paramName + ".length"));
      } else if (count instanceof AtLeast) {
        c.addStatement(
        ...
        c.addStatement(
            CodeBlock.of(
                "$T.checkArgument($L.length >= $L && $L.length <= $L, $S, $L)",
                Preconditions.class,
                paramName,
                ((Range) count).getFrom(),
                paramName,
  ...
  private static String generateMethodText(
      Op op, Signature s, boolean isSameDiff, boolean isLoss, boolean withName) {
    StringBuilder sb = new StringBuilder();
    ...
    for (Parameter param : params) {
      if (param instanceof Arg) {
        Arg arg = (Arg) param;
        if (!first) sb.append(",");
        else if (withName) sb.append("String name,");
  ...
  private static void generateDocs(NamespaceOps namespace, File outputDirectory, String basePackage)
      throws IOException {
    StringBuilder sb = new StringBuilder();
    ...
    for (Config config : Registry.INSTANCE.configs()) {
      ...
      for (Arg arg : config.getArgs()) {
        ...
        sb.append(
            "* " + arg.getName() + " " + " (" + arg.getType() + " type)" + System.lineSeparator());
      }
      StringBuilder tsb = buildDocSectionText(config.getDoc());
      sb.append(tsb.toString());
      sb.append("````" + System.lineSeparator());
      ops.stream()
          .filter(op -> op.getConfigs().contains(config))
          .forEach(
              op ->
                  sb.append(
                      "["
  ...
  private static void generateEnum(File outputDirectory, String targetPackage, Arg arg)
      throws IOException {
    ...
    JavaFile jf = JavaFile.builder(targetPackage, ts).build();
    ...
    File outFile =
        new File(outputDirectory, packageToDirectory(targetPackage) + "/" + className + ".java");
    FileUtils.writeStringToFile(outFile, sb.toString(), StandardCharsets.UTF_8);
  }
  ...
  private static void generateConfig(File outputDirectory, String targetPackage, Config config)
      throws IOException {
    ...
    final TypeSpec.Builder sdb =
        TypeSpec.classBuilder("SdBuilder").addModifiers(Modifier.STATIC, Modifier.PUBLIC);
    final TypeSpec.Builder ndb =
        TypeSpec.classBuilder("NdBuilder").addModifiers(Modifier.STATIC, Modifier.PUBLIC);

    for (Input input : config.getInputs()) {
      addConfigBuilderParam(
          className,
          sdb,
          input.getName(),
          input.getType(),
          getType(TypeName.get(SDVariable.class), input.getCount()),
      ...
      addConfigBuilderParam(
          ...
          input.getName(),
          input.getType(),
          getType(TypeName.get(INDArray.class), input.getCount()),
          input.getDescription(),
          input.getCount());
    }
    ...
    for (Input input : config.getInputs()) {
      ...
      parameters.add(
          input.hasDefaultValue()
              ? input.name()
                  + " == null ? "
                  + ((Input) input.defaultValue()).getName()
                  + " : "
                  + input.name()
              : input.name());
    }
    for (Arg input : config.getArgs()) {
      ...
      parameters.add(
          input.hasDefaultValue()
              ? input.name()
                  + " == null ? "
                  + anyToCode(input, input.defaultValue())
                  + " : "
                  + input.name()
              : input.name());
    }
    ...
    final MethodSpec.Builder ndConstructorBuilder =
        MethodSpec.constructorBuilder().addModifiers(Modifier.PRIVATE);
    final MethodSpec.Builder sdConstructorBuilder =
        MethodSpec.constructorBuilder().addModifiers(Modifier.PRIVATE);
    ...
    for (Arg arg : config.getArgs()) {
      addConfigParam(
          holder, ndConstructorBuilder, arg.getName(), getArgType(arg), arg.getDescription(), true);
      addConfigParam(
          holder,
          sdConstructorBuilder,
          arg.getName(),
    ...
    holder.addMethod(
        MethodSpec.methodBuilder("sdBuilder")
    ...
    holder.addMethod(
        MethodSpec.methodBuilder("ndBuilder")
            .addModifiers(Modifier.STATIC, Modifier.PUBLIC)
  ...
  private static void addConfigParam(
      TypeSpec.Builder builder,
  ...
  private static void addConfigBuilderParam(
    ...
    if (inputType != null) {
      if (builderName.equals("SdBuilder")) {
        setter.addStatement(
            "$T.$L($S, $S, $L)",
            SDValidation.class,
            validationMapping.get(inputType),
  ...
  @NotNull
  private static MethodSpec generateGetter(
      TypeName typeVariable, String paramName, String paramDescription, boolean fluent) {
    final MethodSpec.Builder getter =
        MethodSpec.methodBuilder((fluent ? paramName : "get" + GenUtil.ensureFirstIsCap(paramName)))
            .addModifiers(Modifier.PUBLIC)
  ...
  private static String anyToCode(Parameter parameter, Object v) {
    if (v == null) {
      ...
      return "new int[]" + Arrays.toString((int[]) v).replace("[", "{").replace("]", "}");
    } else if (v instanceof long[]) {
      return "new long[]" + Arrays.toString((long[]) v).replace("[", "{").replace("]", "}");
    } else if (v instanceof float[]) {
      return "new float[]" + Arrays.toString((float[]) v).replace("[", "{").replace("]", "}");
    } else if (v instanceof double[]) {
      return "new double[]" + Arrays.toString((double[]) v).replace("[", "{").replace("]", "}");
    } else if (v instanceof boolean[]) {
      return "new boolean[]" + Arrays.toString((boolean[]) v).replace("[", "{").replace("]", "}");
    } else if (v instanceof Input) {
      return ((Input) v).getName();
    } else if (v instanceof org.nd4j.linalg.api.buffer.DataType) {
      return "DataType." + v;
    } else if (v instanceof LossReduce || v instanceof org.nd4j.autodiff.loss.LossReduce) {
