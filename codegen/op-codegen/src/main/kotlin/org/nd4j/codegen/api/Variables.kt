...
open class Constraint(var message: String? = null, var check: Expression)
...
data class BooleanExpression(val left: Reference, val right: Reference, val op: BooleanOperation) :
    Expression()
...
interface Parameter {
    ...
    /**
     * A default value only is applicable if it is a literal value, or the referenced value is
     * either directly a part of the signature, or there is a reference chain that ends in something
     * that is actually a part of the signature
     */
...
data class Arg(
        ...
        set(value) =
            if (isAssignableFrom(value)) {
                ...
                throw IllegalArgumentException(
                    "Illegal default value for $this. Got ${value.toDescriptiveString()} (${value?.javaClass?.name})"
                )
            }
        ...
        set(value) =
            if (type == DataType.ENUM)
                when {
                    ...
                    value.isEmpty() ->
                        throw IllegalArgumentException("$this: Can not set empty possibleValues.")
                    else -> field = value
                }
            else {
                throw IllegalArgumentException(
                    "$this: Can not set possibleValues on non ENUM typed Arg."
                )
            }

    var count: Count? = null
        set(value) =
            if (type == DataType.ENUM && value != Exactly(1)) {
                throw IllegalArgumentException("$this: ENUM typed Arg can not be array")
            } else {
                field = value
            }

    private fun matchesDataType(value: Any?) =
        when (type) {
    ...
    private fun isAssignableFrom(value: Any?) =
        when (value) {
            ...
            is IntArray ->
                isArray() &&
                    (type == DataType.INT || type == DataType.NUMERIC) &&
                    countMatches(value.size)
            is DoubleArray ->
                isArray() &&
                    (type == DataType.FLOATING_POINT || type == DataType.NUMERIC) &&
                    countMatches(value.size)
            ...
            is String ->
                type == DataType.STRING ||
                    type == DataType.ENUM &&
                        possibleValues != null &&
                        possibleValues?.contains(value) ?: false
            // is String -> type == DataType.ENUM && possibleValues != null &&
            // possibleValues?.contains(value) ?: false
...
data class Input(
    ...
    var defaultValue: Input? = null
        set(value) =
            if (matchesDataType(value)) {
                field = value
                defaultValueIsSet = true
            } else {
                throw IllegalArgumentException(
                    "Illegal default value for Input($name). Allowed values have to match data type $type, but got ${value.toDescriptiveString()} (${value?.javaClass?.name})"
                )
            }

    private fun matchesDataType(value: Input?) =
        when (value) {
            null -> true
...
data class Signature(val parameters: List<Parameter>, val description: String? = null) {
