...
class KotlinExamplePythonGenerator : Generator {
    ...
    override fun generateNamespaceNd4j(
    ...
    fun genDocString(op: Op): String {
        ...
        // python docstring / multiline string delimiter is the same as in kotlin, so use this
        // little workaround
        ...
        if (op.outputs.size != 1)
            throw UnsupportedOperationException(
                "Not yet implemented: Python docstring generation for multiple output ops"
            )
    ...
    @Throws(IOException::class)
    override fun generateNamespaceSameDiff(
        namespace: NamespaceOps?,
        config: GeneratorConfig?,
        directory: File?,
        className: String?
