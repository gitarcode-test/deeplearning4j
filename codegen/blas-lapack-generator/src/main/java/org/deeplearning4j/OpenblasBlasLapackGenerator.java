...
public class OpenblasBlasLapackGenerator {
        ...
        .forEach(
            method -> {
              ...
              Arrays.stream(method.getParameters())
                  .forEach(
                      param -> {
                        if (casting.containsKey(method.getName())
                            && param.getType().equals(Pointer.class)) {
                          System.out.println("In function casting for " + method.getName());
                          codeStatement.append(
                              "((" + casting.get(method.getName()) + ")" + param.getName() + ")");
                          codeStatement.append(",");
                        } else {
                        ...
                        builder.addParameter(
                            ParameterSpec.builder(param.getType(), param.getName()).build());
                      });
  ...
  public static void main(String... args) throws Exception {
    OpenblasBlasLapackGenerator openblasBlasLapackGenerator =
        new OpenblasBlasLapackGenerator(
            new File("nd4j/nd4j-backends/nd4j-backend-impls/nd4j-native/src/main/java"));
    openblasBlasLapackGenerator.parse();
    String generated =
        FileUtils.readFileToString(
            openblasBlasLapackGenerator.getTargetFile(), Charset.defaultCharset());
    ...
    generated =
        generated.replaceAll(
            ...
            "import org.bytedeco.openblas.global.openblas_nolapack");
    FileUtils.write(
        openblasBlasLapackGenerator.getTargetFile(), generated, Charset.defaultCharset());
  }
}
