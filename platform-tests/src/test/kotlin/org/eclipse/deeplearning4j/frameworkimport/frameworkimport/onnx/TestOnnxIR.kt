...
data class OnnxGraphInput(
    val graphDef: Onnx.GraphProto,
    val inputNames: List<String>,
    val outputNames: List<String>,
    val inputArrays: Map<String, INDArray>,
    val dynamicArrays: Map<String, INDArray>
)
...
class TestOnnxIR {
    ...
    fun testSequenceConstruct() {
        ...
        val importedGraph =
            importGraph.importGraph(
                ...
                null,
                null,
                convertToOnnxTensors(mutableMapOf("W" to w, "x" to inputTensor)),
                onnxOpRegistry,
                false
            )
    ...
    fun testSequenceErase() {
        ...
        val onnxGraphRunner =
            OnnxIRGraphRunner(
                onnxIRGraph,
                listOf("x", "W", "index", "indexTwo", "insert"),
                listOf("sequenceAt")
            )
        val importedGraph =
            importGraph.importGraph(
                ...
                null,
                null,
                convertToOnnxTensors(
                    mutableMapOf(
                        "W" to w,
                        "x" to inputTensor,
                        "index" to index,
                        "indexTwo" to indexTwo,
                        "insert" to insert
                    )
                ),
                onnxOpRegistry,
        ...
        val inputs =
            mapOf(
                "x" to arrayOf(inputTensor),
                "W" to arrayOf(w),
                "index" to arrayOf(index),
                "indexTwo" to arrayOf(indexTwo),
                "insert" to arrayOf(insert)
            )
        val inputs2 =
            mapOf(
                "x" to SDValue.create(inputTensor),
                "W" to SDValue.create(w),
                "index" to SDValue.create(index),
                "indexTwo" to SDValue.create(indexTwo),
    ...
    fun testSequenceLength() {
        ...
        val graphToRun = GraphProto {
            ...
            Output(
                createValueInfoFromTensor(
                    inputTensor.castTo(DataType.INT64),
                    "sequenceLength",
                    false
                )
            )
        ...
        val onnxIRGraph = OnnxIRGraph(graphToRun, onnxOpRegistry)
        val onnxGraphRunner =
            OnnxIRGraphRunner(onnxIRGraph, listOf("x", "W"), listOf("sequenceLength"))
        val importedGraph =
            importGraph.importGraph(
                onnxIRGraph,
                null,
    ...
    fun testSequenceAt() {
        ...
        val importedGraph =
            importGraph.importGraph(
                ...
                null,
                null,
                convertToOnnxTensors(mutableMapOf("W" to w, "x" to inputTensor, "index" to index)),
                onnxOpRegistry,
                false
            )
        println(importedGraph.summary())
        val inputs =
            mapOf("x" to arrayOf(inputTensor), "W" to arrayOf(w), "index" to arrayOf(index))
        val inputs2 =
            mapOf(
                "x" to SDValue.create(inputTensor),
                "W" to SDValue.create(w),
    ...
    fun testSequenceRemove() {
        ...
        val graphToRun = GraphProto {
            ...
            Output(
                createEmptySequence(convertToOnnxDataType(inputTensor.dataType()), "sequenceRemove")
            )
        }

        val onnxIRGraph = OnnxIRGraph(graphToRun, onnxOpRegistry)
        val onnxGraphRunner =
            OnnxIRGraphRunner(onnxIRGraph, listOf("x", "W"), listOf("sequenceRemove"))
        val importedGraph =
            importGraph.importGraph(
                onnxIRGraph,
                null,
    ...
    fun testLoop() {
        ...
        val onnxGraphRunner =
            OnnxIRGraphRunner(
                onnxIRGraph,
                listOf("trip_count", "cond", "seq_empty"),
                listOf("res_y", "res_scan")
            )
        val inputs =
            mapOf(
                ...
                "begin_axes" to axes,
                "end_axes" to axes,
                "iter_count" to Nd4j.create(Nd4j.createBuffer(longArrayOf(1)))
            )
        val sequenceInputValues =
            mapOf(
                "trip_count" to SDValue.create(tripCount),
                "cond" to SDValue.create(cond),
                "y" to SDValue.create(y),
                "begin_axes" to SDValue.create(axes),
                "end_axes" to SDValue.create(axes),
        ...
        val importGraph =
            ...
            >()
        val importedGraph =
            importGraph.importGraph(onnxIRGraph, null, null, inputsOnnx, onnxOpRegistry, false)
    ...
    fun testConvPaddingGroups() {
        ...
        val inputsOnnx =
            mutableMapOf(
                "x" to convertToOnnxTensor(inputTensor, "x"),
                "W" to convertToOnnxTensor(w, "W")
            )
        val importedGraph =
            importGraph.importGraph(onnxIRGraph, null, null, inputsOnnx, onnxOpRegistry, false)
        val assertion = onnxGraphRunner.run(inputs)
    ...
    fun testConvPadding() {
        ...
        val importedGraph =
            importGraph.importGraph(
                ...
                null,
                null,
                convertToOnnxTensors(mutableMapOf("x" to inputTensor, "W" to w)),
                onnxOpRegistry,
                false
            )
        val inputs = mapOf("x" to inputTensor, "W" to w)
    ...
    @Test
    @Disabled(
        "See: https://github.com/eclipse/deeplearning4j/issues/9525 we need to support asymmetrics padding"
    )
    fun testConvStridesAsymmetricPadding() {
        ...
        val onnxGraphRunner = OnnxIRGraphRunner(onnxIRGraph, listOf("x", "W"), listOf("y"))
        val importedGraph =
            importGraph.importGraph(onnxIRGraph, null, null, HashMap(), onnxOpRegistry, false)
        val inputs = mapOf("x" to inputTensor, "W" to w)
        val assertion = onnxGraphRunner.run(inputs)
        val result = importedGraph.output(inputs, "y")
        assertEquals(assertion, result)
    ...
    fun testOpExecutionHooks() {
        ...
        val importedGraph =
            importGraph.importGraph(
                ...
                null,
                null,
                convertToOnnxTensors(mutableMapOf("x" to inputTensor)),
                onnxOpRegistry,
                false
            )
    ...
    fun testExpand() {
        ...
        val inputs =
            mapOf(
                "data" to Nd4j.arange(1.0, ArrayUtil.prod(*shape).toDouble() + 1.0).reshape(*shape),
    ...
    fun testSlice() {
        ...
        /**
         * Note that this test case is manual due to subtle differences in how onnxruntime and
         * tensorflow appear to interpret their nearest neighbor results. In our test case here, we
         * are verifying against tensorflow-onnx as the implementation.
         */
        Nd4j.getExecutioner().enableDebugMode(true)
        Nd4j.getExecutioner().enableVerboseMode(true)
        ...
        val input =
            mapOf("x" to x, "starts" to starts, "ends" to ends, "axes" to axes, "steps" to steps)
    ...
    fun testClip() {
        ...
        val inputs =
            mutableMapOf(
                "input" to Nd4j.linspace(1, 4, 4).castTo(DataType.DOUBLE),
                "min" to Nd4j.scalar(1.0).castTo(DataType.DOUBLE),
                "max" to Nd4j.scalar(2.0).castTo(DataType.DOUBLE)
            )
        val output = listOf("output")
        val createdGraph =
            createSingleNodeGraph(inputs, "Clip", emptyMap(), output, inputs.keys.toList())
    ...
    fun testNonZero() {
        ...
        val createdGraph =
            createSingleNodeGraph(
                ...
                output,
                inputs.keys.toList(),
                templateTensor = Nd4j.ones(DataType.INT64)
            )
        val importGraph =
            ImportGraph<
                Onnx.GraphProto,
    ...
    fun testRoiAligned() {
        ...
        val rois =
            Nd4j.create(Nd4j.createBuffer(longArrayOf(0, 0, 9, 9, 0, 5, 4, 9, 5, 5, 9, 9)))
                .reshape(3, 4)
        ...
        val attributes =
            mapOf(
                "spatial_scale" to 1.0f,
                "output_height" to 5,
                "output_width" to 5,
                "sampling_ratio" to 2
            )
        val createdGraph =
            createSingleNodeGraph(inputs, "RoiAlign", attributes, outputs, inputs.keys.toList())
    ...
    fun testMaximum() {
        ...
        val output = listOf("output")
        val createdGraph =
            createSingleNodeGraph(inputs, "Max", emptyMap(), output, inputs.keys.toList())
        runAssertion(createdGraph, inputs, output)
    }
    ...
    fun testMinimum() {
        ...
        val output = listOf("output")
        val createdGraph =
            createSingleNodeGraph(inputs, "Min", emptyMap(), output, inputs.keys.toList())
        runAssertion(createdGraph, inputs, output)
    }
    ...
    fun testUnsqueeze() {
        ...
        // TODO: add coefficients for better eps comparison, see:
        // https://github.com/eclipse/deeplearning4j/issues/9467
    ...
    fun testCast() {
        ...
        val graph =
            createSingleNodeGraph(
                ...
                outputs,
                listOf("x"),
                Nd4j.ones(2).castTo(DataType.FLOAT)
            )
    ...
    fun testResize() {
        ...
        val input =
            mapOf("x" to inputData, "scales" to scales, "roi-empty" to Nd4j.zeros(1, 1, 1, 1))
    ...
    fun testOpExecution() {
        ...
        val singleInputBooleanOps = mapOf("not" to false)

        val singleOutputBooleanOps =
            mapOf(
                "isfinite" to 1.0f,
                "isinf" to 1.0f,
                "isnan" to 1.0f,
        ...
        /**
         * NOTE WHEN WRITING TESTS, IF YOU SEE AN ERROR like: java.lang.RuntimeException: Could not
         * find an implementation for the node output:Cos(7)
         *
        ...
        onnxOpRegistry
            ...
            .map { onnxOpRegistry.lookupOpMappingProcess(it) }
            .forEach { mappingProcess ->
                val nd4jOpDef = onnxOpRegistry.lookupNd4jOpDef(mappingProcess.opName())
                val onnxOpDef =
                    onnxOpRegistry.lookupInputFrameworkOpDef(mappingProcess.inputFrameworkOpName())
                if (scalarInputs.containsKey(nd4jOpDef.name)) {
                    print("Running op $nd4jOpDef.name")
                    val input =
                        Nd4j.scalar(scalarInputs[mappingProcess.opName()])
                    ...
                    val onnxIRGraph = OnnxIRGraph(graphToRun, onnxOpRegistry)
                    val onnxGraphRunner =
                        OnnxIRGraphRunner(onnxIRGraph, listOf("input"), listOf("output"))
                    val importedGraph =
                        importGraph.importGraph(
                            onnxIRGraph,
                            null,
                    ...
                    val result = importedGraph.output(inputs, "output")
                    assertEquals(
                        assertion["output"]!!.reshape(1, 1),
                        result["output"]!!.reshape(1, 1),
                        "Function ${nd4jOpDef.name} failed with input $input"
                    )
                    finishedOps.add(nd4jOpDef.name)
                } else if (scalarFloatOps.containsKey(nd4jOpDef.name)) {
                    print("Running op $nd4jOpDef.name")
                    val input =
                        Nd4j.scalar(scalarFloatOps[mappingProcess.opName()]).castTo(DataType.FLOAT)
                    ...
                    print("Running op $nd4jOpDef.name")
                    val input =
                        Nd4j.scalar(singleOutputBooleanOps[mappingProcess.opName()])
                            .castTo(DataType.FLOAT)
                    ...
                    val graphToRun = GraphProto {
                        ...
                        Output(
                            createValueInfoFromTensor(
                                Nd4j.create(booleanArrayOf(true)).reshape(),
                                "output"
                            )
                        )
                    }
                    ...
                    val x =
                        Nd4j.scalar(pairwise[mappingProcess.opName()]!![0]!!)
                            .castTo(org.nd4j.linalg.api.buffer.DataType.DOUBLE)
                    val y =
                        Nd4j.scalar(pairwise[mappingProcess.opName()]!![1]!!)
                            .castTo(org.nd4j.linalg.api.buffer.DataType.DOUBLE)
                    ...
                    val onnxIRGraph = OnnxIRGraph(graphToRun, onnxOpRegistry)
                    val onnxGraphRunner =
                        OnnxIRGraphRunner(onnxIRGraph, listOf("x", "y"), listOf("output"))
                    val importedGraph =
                        importGraph.importGraph(
                            onnxIRGraph,
                            null,
                            null,
                            hashMapOf(
                                "x" to convertToOnnxTensor(x, "x"),
                                "y" to convertToOnnxTensor(y, "y")
                            ),
                            onnxOpRegistry,
                            false
                        )
                    ...
                    val assertion = onnxGraphRunner.run(inputs)
                    assertEquals(
                        assertion["output"]!!.getDouble(0),
                        result["output"]!!.getDouble(0),
                        "Function ${nd4jOpDef.name} failed with input $x $y"
                    )
                    finishedOps.add(nd4jOpDef.name)
                } else if (pairWiseBooleanInputs.containsKey(nd4jOpDef.name)) {
                    print("Running op def $nd4jOpDef.name")
                    val x =
                        Nd4j.scalar(pairWiseBooleanInputs[mappingProcess.opName()]!![0]!!)
                            .castTo(org.nd4j.linalg.api.buffer.DataType.BOOL)
                    val y =
                        Nd4j.scalar(pairWiseBooleanInputs[mappingProcess.opName()]!![1]!!)
                            .castTo(org.nd4j.linalg.api.buffer.DataType.BOOL)
                    ...
                    print("Running op def $nd4jOpDef.name")
                    val x =
                        Nd4j.scalar(pairWiseBooleanOps[mappingProcess.opName()]!![0]!!)
                            .castTo(DataType.FLOAT)
                    val y =
                        Nd4j.scalar(pairWiseBooleanOps[mappingProcess.opName()]!![1]!!)
                            .castTo(DataType.FLOAT)
                    val output =
                        Nd4j.scalar(pairWiseBooleanOps[mappingProcess.opName()]!![1]!!)
                            .castTo(org.nd4j.linalg.api.buffer.DataType.BOOL)
                    ...
                    val x =
                        Nd4j.create(
                                booleanArrayOf(singleInputBooleanOps[mappingProcess.opName()]!!)
                            )
                            .castTo(org.nd4j.linalg.api.buffer.DataType.BOOL)
                    val output =
                        Nd4j.create(
                    ...
                    val onnxIRGraph = OnnxIRGraph(graphToRun, onnxOpRegistry)
                    val onnxGraphRunner =
                        OnnxIRGraphRunner(onnxIRGraph, listOf("x"), listOf("output"))
                    val importedGraph =
                        importGraph.importGraph(
                            onnxIRGraph,
                            null,
                    ...
                    // assertEquals("Function ${nd4jOpDef.name} failed with input
                    // $x",assertion["output"]!!.reshape(1,1),result["output"]!!.reshape(1,1))
                    ...
                    val output = x.mean(0).reshape(2).castTo(DataType.FLOAT)
                    ...
                    val importedGraph =
                        importGraph.importGraph(
                            ...
                            hashMapOf(
                                "x" to convertToOnnxTensor(x, "x"),
                                "axes" to convertToOnnxTensor(axes, "axes")
                            ),
                            onnxOpRegistry,
                            false
                        )
                    val result = importedGraph.output(inputs, "output")
                    val onnxGraphRunner =
                        OnnxIRGraphRunner(onnxIRGraph, listOf("x", "axes"), listOf("output"))
                    val assertion = onnxGraphRunner.run(inputs)
                    assertEquals(
                        assertion["output"]!!.reshape(1, 2),
                        result["output"]!!.reshape(1, 2),
                        "Function ${nd4jOpDef.name} failed with input $x"
                    )
                    finishedOps.add(nd4jOpDef.name)
                    ...
                    graphForOp.forEach { graph ->
                        ...
                        val onnxGraphRunner =
                            OnnxIRGraphRunner(onnxIRGraph, graph.inputNames, graph.outputNames)
                        val assertion = onnxGraphRunner.run(inputs)
                        ...
                        result.forEach { name, arr ->
                            if (arr.length().toInt() == 1) {
                                assertEquals(
                                    assertion[name]!!.getDouble(0),
                                    arr.getDouble(0),
                                    1e-3,
                                    "Function ${nd4jOpDef.name} failed with input ${graph.inputNames}"
                                )
                            } else {
                                assertEquals(
                                    assertion[name],
                    ...
                    print("Running op $nd4jOpDef.name")
                    val input =
                        singleInputIntOutput[mappingProcess.opName()]!!.castTo(
        ...
        println(
            "Finished ops totaling ${finishedOps.size} out of ${onnxOpRegistry.mappedNd4jOpNames().size}"
        )
    }
    ...
    fun graphForOp(opName: String): List<OnnxGraphInput> {
        when (opName) {
            "non_max_suppression_v3" -> {
                /**
                 * TODO: Add pre and post processing for each node. Our NMS requires 2d, but onnx is
                 *   3d. Attempt to see if generalized pre/post processing node additions as part of
                 *   a mapping process can work.
                 */
                ...
                val scoresVal =
                    Nd4j.create(listOf(0.9f, 0.75f, 0.6f, 0.95f).toFloatArray())
                        .reshape(1, 1, 4)
                        .castTo(DataType.FLOAT)
                ...
                val inputs =
                    mapOf(
                        "boxes" to boxesVal,
                        "scores" to scoresVal,
                        "max_output_boxes_per_class" to maxOutputSize,
                        "iou_threshold" to iouThreshold,
                        "score_threshold" to scoreThreshold
                    )
                val output = Nd4j.scalar(1).castTo(org.nd4j.linalg.api.buffer.DataType.INT64)
            ...
            "argmax" -> {
                ...
                val output = x.mean(0).reshape(2).castTo(org.nd4j.linalg.api.buffer.DataType.INT64)
                ...
                return listOf(
                    OnnxGraphInput(graphToRun, listOf("x"), listOf("output"), inputMap, inputMap)
                )
            }
            "top_k" -> {
