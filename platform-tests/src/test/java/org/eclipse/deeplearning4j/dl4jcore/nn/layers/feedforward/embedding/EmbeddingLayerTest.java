...
class EmbeddingLayerTest extends BaseDL4JTest {
    ...
    for (boolean hasBias : new boolean[] {true, false}) {
      MultiLayerConfiguration conf =
          new NeuralNetConfiguration.Builder()
              ...
              .layer(
                  1,
                  new OutputLayer.Builder().nIn(5).nOut(4).activation(Activation.SOFTMAX).build())
              .build();
      MultiLayerNetwork net = new MultiLayerNetwork(conf);
      net.init();
      Layer l0 = net.getLayer(0);
      assertEquals(
          org.deeplearning4j.nn.layers.feedforward.embedding.EmbeddingLayer.class, l0.getClass());
  ...
  void testEmbeddingSequenceLayerConfig() {
    ...
    for (boolean hasBias : new boolean[] {true, false}) {
      ...
      Layer l0 = net.getLayer(0);
      assertEquals(
          org.deeplearning4j.nn.layers.feedforward.embedding.EmbeddingSequenceLayer.class,
          l0.getClass());
      assertEquals(10, ((FeedForwardLayer) l0.conf().getLayer()).getNIn());
      assertEquals(5, ((FeedForwardLayer) l0.conf().getLayer()).getNOut());
      INDArray weights = l0.getParam(DefaultParamInitializer.WEIGHT_KEY);
        ...
        new NeuralNetConfiguration.Builder()
            ...
            .layer(
                ...
                new DenseLayer.Builder()
                    ...
                    .build())
            .layer(
                1, new OutputLayer.Builder().nIn(5).nOut(4).activation(Activation.SOFTMAX).build())
  ...
  void testEmbeddingForwardPass() {
    // With the same parameters, embedding layer should have same activations as the equivalent
  ...
  void testEmbeddingLayerWithMasking() {
    ...
    // Ensure that the parameter gradients for the inputs don't depend on the inputs when inputs are
    // masked
            ...
            new NeuralNetConfiguration.Builder()
                ...
                .layer(
                    ...
                    new EmbeddingLayer.Builder()
                        ...
                        .build())
                .layer(
                    1, new DenseLayer.Builder().activation(Activation.TANH).nIn(5).nOut(4).build())
          ...
          for (int nExamples : miniBatchSizes) {
            ...
            MultiLayerConfiguration conf =
                new NeuralNetConfiguration.Builder()
                    ...
                    .layer(
                        1,
                        new DenseLayer.Builder().activation(Activation.TANH).nIn(5).nOut(4).build())
                    .layer(2, new LSTM.Builder().activation(Activation.TANH).nIn(4).nOut(3).build())
                    .layer(
                        3,
                        new RnnOutputLayer.Builder()
                            ...
                            .build())
                    .setInputType(
                        InputType.recurrent(numInputClasses, timeSeriesLength, RNNFormat.NCW))
                    .build();
            MultiLayerNetwork net = new MultiLayerNetwork(conf);
            net.init();
            MultiLayerConfiguration conf2 =
                new NeuralNetConfiguration.Builder()
                    ...
                    .layer(
                        ...
                        new RnnOutputLayer.Builder()
                            ...
                            .nOut(4)
                            .build())
                    .setInputType(InputType.recurrent(numInputClasses, 1, RNNFormat.NCW))
                    .build();
            MultiLayerNetwork net2 = new MultiLayerNetwork(conf2);
            net2.init();
            net2.setParams(net.params().dup());
            INDArray inEmbedding =
                Nd4j.zeros(
                    inLabelDtype,
                    inputRank == 2
                        ? new long[] {nExamples, timeSeriesLength}
                        : new long[] {nExamples, 1, timeSeriesLength});
            INDArray inDense =
                Nd4j.zeros(inLabelDtype, nExamples, numInputClasses, timeSeriesLength);
            INDArray labels = Nd4j.zeros(inLabelDtype, nExamples, 4, timeSeriesLength);
            for (int i = 0; i < nExamples; i++) {
              for (int j = 0; j < timeSeriesLength; j++) {
                int inIdx = r.nextInt(numInputClasses);
                inEmbedding.putScalar(
                    inputRank == 2 ? new int[] {i, j} : new int[] {i, 0, j}, inIdx);
                inDense.putScalar(new int[] {i, inIdx, j}, 1.0);
                int outIdx = r.nextInt(4);
                labels.putScalar(new int[] {i, outIdx, j}, 1.0);
              }
  ...
  void testEmbeddingWeightInit() {
    ...
    // The embedding layer weight initialization should be independent of the vocabulary size (nIn
    // setting)
    for (WeightInit wi :
        new WeightInit[] {
          WeightInit.XAVIER, WeightInit.RELU, WeightInit.XAVIER_UNIFORM, WeightInit.LECUN_NORMAL
        }) {
