...
public class TestTransformOpValidation extends BaseOpValidation {
  ...
  public void testScalarOps(Nd4jBackend backend) {
    ...
    for (int i = 0; i < 11; i++) {
      for (char inOrder : new char[] {'c', 'f'}) {
        ...
        INDArray inArr =
            Nd4j.linspace(1, n, n, DataType.DOUBLE).reshape('c', d0, d1, d2).dup(inOrder);
  ...
  public void testCross(Nd4jBackend backend) {
    ...
    String err =
        OpValidation.validate(new TestCase(sd).expectedOutput("cross", expOut).gradientCheck(true));
    assertNull(err, err);
  }
  ...
  public void testSpaceToDepth(Nd4jBackend backend) {
    ...
    String err =
        OpValidation.validate(new TestCase(sd).expectedOutput("std", expOut).gradientCheck(true));
    assertNull(err);
  }
  ...
  public void testDepthToSpace(Nd4jBackend backend) {
    ...
    String err =
        OpValidation.validate(new TestCase(sd).expectedOutput("dts", expOut).gradientCheck(true));
    assertNull(err, err);
  }
  ...
  public void testBatchToSpace(Nd4jBackend backend) {
    ...
    DynamicCustomOp op =
        DynamicCustomOp.builder("batch_to_space")
    ...
    SDVariable t =
        sd.cnn().batchToSpace("bts", sdInput, new int[] {2, 2}, new int[] {0, 0}, new int[] {0, 0});
    SDVariable loss = sd.mean("loss", t);

    String err =
        OpValidation.validate(new TestCase(sd).expectedOutput("bts", expOut).gradientCheck(true));
    assertNull(err, err);
  }
  ...
  public void testSpaceToBatch(Nd4jBackend backend) {
    // OpValidationSuite.ignoreFailing();          //TODO:
    // https://github.com/eclipse/deeplearning4j/issues/6863
    ...
    DynamicCustomOp op =
        DynamicCustomOp.builder("space_to_batch")
            .addIntegerArguments(2)
    ...
    SDVariable t =
        sd.cnn().spaceToBatch("stb", sdInput, new int[] {2, 2}, new int[] {0, 0}, new int[] {0, 0});
    SDVariable loss = sd.mean("loss", t);

    String err =
        OpValidation.validate(new TestCase(sd).expectedOutput("stb", expOut).gradientCheck(true));
    assertNull(err, err);
  }
  ...
  public void testDynamicPartition(Nd4jBackend backend) {
    ...
    DynamicCustomOp dynamicPartition =
        DynamicCustomOp.builder("dynamic_partition")
            .addInputs(ia, partitions)
            .addIntegerArguments(numPartitions)
            .addOutputs(expOut1, expOut2)
            .build();
    Nd4j.getExecutioner().exec(dynamicPartition);

    SDVariable[] parts =
        sd.dynamicPartition(new String[] {"dp0", "dp1"}, in, sdPartitions, numPartitions);
  ...
  public void testDynamicStitch(Nd4jBackend backend) {
    ...
    DynamicCustomOp dynamicStitch =
        DynamicCustomOp.builder("dynamic_stitch")
    ...
    SDVariable t =
        sd.dynamicStitch("ds", new SDVariable[] {index1, index2}, new SDVariable[] {in1, in2});
  ...
  public void testDiag(Nd4jBackend backend) {
    ...
    DynamicCustomOp diag =
        DynamicCustomOp.builder("diag").addInputs(ia).addOutputs(expOut2).build();
    ...
    String err =
        OpValidation.validate(new TestCase(sd).expectedOutput("diag", expOut).gradientCheck(true));
    assertNull(err);
  }
  ...
  public void testDiagPart(Nd4jBackend backend) {
    ...
    String err =
        OpValidation.validate(new TestCase(sd).expectedOutput("dp", expOut).gradientCheck(true));
    assertNull(err);
  }
  ...
  public void testEye(Nd4jBackend backend) {
    ...
    for (int i = 0; i < 4; i++) {
      expOut[2]
          .get(NDArrayIndex.point(i), NDArrayIndex.all(), NDArrayIndex.all())
          .assign(expOut[1]);
    }
    ...
    for (int i = 0; i < 82; i++) {
      ...
      switch (i) {
        ...
        case 45:
          ...
          ia =
              Nd4j.linspace(1, minibatch * nOut, minibatch * nOut, DataType.DOUBLE)
                  .reshape('c', minibatch, nOut);
        ...
        case 49:
          ...
          ia.diviRowVector(ia.norm2(0))
              .muli(clip); // Norm2 is now 'clip' (i.e., exactly at threshold
          // System.out.println(ia.norm2(0));
          ia.muliColumnVector(
              Nd4j.linspace(0.9, 1.1, ia.size(0), DataType.DOUBLE).reshape(ia.size(0), 1));
          // System.out.println(ia.norm2(0));
        ...
        case 50:
          ...
          DynamicCustomOp reverse =
              DynamicCustomOp.builder("reverse")
        ...
        case 51:
          ...
          DynamicCustomOp cumsum =
              DynamicCustomOp.builder("cumsum")
        ...
        case 53:
          ...
          DynamicCustomOp op =
              DynamicCustomOp.builder("diag").addInputs(ia).addOutputs(expOut53).build();
          Nd4j.getExecutioner().exec(op);
        ...
        case 55:
          ...
          tc.expectedOutput(
              t.name(),
              Nd4j.getExecutioner()
                  .exec(new Erfc(ia, Nd4j.createUninitialized(ia.shape(), ia.ordering()))));
          break;
        case 56:
          t = sd.math().expm1(in);
        ...
        case 59:
          ...
          tc.expectedOutput(
              t.name(),
              Nd4j.getExecutioner().exec(new RSqrt(ia, Nd4j.create(ia.shape(), ia.ordering()))));
          break;
        case 60:
          t = sd.nn().relu6(in, 0);
          ia = Nd4j.rand(DataType.DOUBLE, minibatch, nOut);
        ...
        case 68:
          ...
          tc.expectedOutput(t.name(), Nd4j.create(ArrayUtil.toDouble(ia.shape())))
              .gradientCheck(false);
          break;
        case 69:
      ...
      if (stdevLoss) {
        loss =
            sd.standardDeviation(
                ...
                t,
                false,
                Integer.MAX_VALUE); // .standardDeviation("loss", t, true, Integer.MAX_VALUE);
      } else {
        loss = sd.mean("loss", t);
      }
    ...
    for (int i = 0; i < 23; i++) {
      ...
      switch (i) {
        ...
        case 10:
          ...
          Nd4j.getExecutioner()
              .exec(new GreaterThanOrEqual(new INDArray[] {ia, ib}, new INDArray[] {expOut10}));
        ...
        case 11:
          ...
          Nd4j.getExecutioner()
              .exec(new LessThanOrEqual(new INDArray[] {ia, ib}, new INDArray[] {expOut11}));
        ...
        case 12:
          ...
          opName = "or";
          tc.expectedOutput(
                  t.name(), Transforms.or(ia.castTo(DataType.BOOL), ib.castTo(DataType.BOOL)))
              .gradientCheck(false);
          break;
        case 13:
        ...
        case 16:
          ...
          opName = "and";
          tc.expectedOutput(
                  t.name(), Transforms.and(ia.castTo(DataType.BOOL), ib.castTo(DataType.BOOL)))
              .gradientCheck(false);
          break;
        case 17:
          ia = Nd4j.getExecutioner().exec(new BernoulliDistribution(ia, 0.5));
          ...
          opName = "xor";
          tc.expectedOutput(
                  t.name(), Transforms.xor(ia.castTo(DataType.BOOL), ib.castTo(DataType.BOOL)))
              .gradientCheck(false);
          break;
        case 18:
          continue; // assign op was removed.
        case 19:
          ...
          tc.expectedOutput(
              t.name(),
              Transforms.atan2(ib, ia)); // Note: y,x order for samediff; x,y order for transforms
          break;
        case 20:
        ...
        case 21:
          ...
          DynamicCustomOp squareDiff =
              DynamicCustomOp.builder("squaredsubtract")
                  .addInputs(ia, ib)
                  .addOutputs(expOut21)
                  .build();
  ...
  public void testIsX(Nd4jBackend backend) {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      switch (i) {
        case 0:
          inArr =
              Nd4j.create(new double[] {10, Double.POSITIVE_INFINITY, 0, Double.NEGATIVE_INFINITY});
        ...
        case 1:
          ...
          break;
        case 2:
          // TODO: IsMax supports both bool and float out:
  ...
  public void testReplaceWhereScalar(Nd4jBackend backend) {
    for (Condition c :
        new Condition[] {
          Conditions.lessThan(0.5), Conditions.greaterThan(0.5), Conditions.equals(0.5)
        }) {
  ...
  public void testAtan2BroadcastShape(Nd4jBackend backend) {
    ...
    assertArrayEquals(
        new long[] {3, 2, 4},
        outShapes.get(0).getShape(),
        Arrays.toString(outShapes.get(0).getShape()));
  }
  ...
  public void testBooleanAnd(Nd4jBackend backend) {
    ...
    DynamicCustomOp op =
        DynamicCustomOp.builder("boolean_and").addInputs(arr1, arr2).addOutputs(out).build();
    Nd4j.getExecutioner().exec(op);
  }
  ...
  public void testScatterOpsScalar(Nd4jBackend backend) {
    for (String s : new String[] {"add", "sub", "mul", "div"}) {
      ...
      DynamicCustomOp op =
          DynamicCustomOp.builder("scatter_" + s)
  ...
  public void testPad(Nd4jBackend backend) {
    ...
    String err2 =
        OpValidation.validate(new TestCase(sd).expected(padded, exp).gradientCheck(false));
    assertNull(err2);
  }
  ...
  public void testMirrorPad(Nd4jBackend backend) {
    ...
    DynamicCustomOp op =
        DynamicCustomOp.builder("mirror_pad")
            .addInputs(in, pad)
            .addOutputs(out)
    ...
    String err = OpValidation.validate(new OpTestCase(op).expectedOutput(0, exp));
    ...
    SDVariable padded =
        sd.nn()
            .pad(
                s,
                sd.constant(Nd4j.createFromArray(new int[][] {{1, 1}, {2, 2}})),
                PadMode.REFLECT,
                0.0);
    String err2 =
        OpValidation.validate(new TestCase(sd).expected(padded, exp).gradientCheck(false));
  ...
  public void testUnique(Nd4jBackend backend) {
    ...
    INDArray expUniqueIdxs =
        Nd4j.create(new double[] {0, 1, 0, 2, 0, 3, 4, 1, 4, 1}).castTo(DataType.LONG);
    ...
    DynamicCustomOp op =
        DynamicCustomOp.builder("unique")
            .addInputs(in)
    ...
    String err =
        OpValidation.validate(
            new OpTestCase(op).expectedOutput(0, expUnique).expectedOutput(1, expUniqueIdxs));
  ...
  public void testInTopK(Nd4jBackend backend) {
    for (int k = 4; k >= 1; k--) {
      ...
      DynamicCustomOp op =
          DynamicCustomOp.builder("in_top_k")
              .addInputs(in, idxs)
              .addOutputs(out)
  ...
  public void testBroadcastEmpty(Nd4jBackend backend) {
    ...
    // Check broadcast behaviour with empty arrays. The idea is to match TF import behaviour, for
    // import
    ...
    for (int i = 0; i < 3; i++) {
      for (boolean scalar : new boolean[] {true, false}) {
        ...
        for (String opName :
          ...
          //                    log.info("Starting op: {}, case {} - x.isScalar()={},
          // x.isEmpty()={}, y.isScalar()={}, y.isEmpty()={}", opName, i,
  ...
  public void testStandardize(Nd4jBackend backend) {
    ...
    String err =
        OpValidation.validate(new TestCase(sd).expectedOutput("out", expOut).gradientCheck(true));
    assertNull(err, err);
  }
    ...
    for (boolean transposeA : new boolean[] {false, true}) {
      for (boolean transposeB : new boolean[] {false, true}) {
        for (boolean transposeResult : new boolean[] {false, true}) {
          log.info(
              "Testing with transposeA={}; transposeB={}; transposeResult={};",
              transposeA,
              transposeB,
              transposeResult);
  ...
  public void testCRELU(Nd4jBackend backend) {
    ...
    String err =
        OpValidation.validate(
            new TestCase(sd).expectedOutput("crelu", expected).gradientCheck(true));
  ...
  public void testClipByAvgNorm(Nd4jBackend backend) {
    ...
    String err =
        OpValidation.validate(
            new TestCase(sd).expectedOutput("clipbyavgnorm", expected.eval()).gradientCheck(false));
    assertNull(err);
  }
  ...
  public void testImageResize(Nd4jBackend backend) {
    ...
    for (ImageResizeMethod method : ImageResizeMethod.values()) {
      if (method == ImageResizeMethod.ResizeLanczos5
          || method == ImageResizeMethod.ResizeArea
          || method == ImageResizeMethod.ResizeMitchellcubic) {
        continue;
      }
      ...
      Function<INDArray, String> checkFunction =
          in -> {
            boolean shapeOk = Arrays.equals(expectedShape, in.shape());
            if (shapeOk) return null;
            return "Failed: shape differs - expected "
      ...
      SDVariable out =
          new ImageResize(sd, inputImage, requestedSize, preserveAspectRatio, antialias, method)
              .outputVariable()
              .std(true);

      String err =
          OpValidation.validate(
              new TestCase(sd).gradientCheck(false).expected("image_resize", checkFunction));
  ...
  public void testMergeAddBp(Nd4jBackend backend) {
    ...
    SDVariable out =
        new MergeAddOp(sd, new SDVariable[] {inputX, inputY, inputZ}).outputVariable().std(true);
    out.markAsLoss();
    String err = OpValidation.validate(new TestCase(sd).gradientCheck(true));
    assertNull(err);
  }
  ...
  public void testUpsampling3dBp(Nd4jBackend backend) {
    ...
    for (boolean dataformat : new boolean[] {true, false}) {
      ...
      SDVariable input =
          dataformat
              ? sd.var(Nd4j.rand(DataType.DOUBLE, 2, 1, 5, 5, 5))
              : sd.var(Nd4j.rand(DataType.DOUBLE, 2, 5, 5, 5, 1));
      int scaleD = 2;
      int scaleH = 2;
      int scaleW = 2;
      SDVariable out =
          new Upsampling3d(sd, input, true, scaleD, scaleH, scaleW).outputVariable().std(true);
      out.markAsLoss();
      String err = OpValidation.validate(new TestCase(sd).gradientCheck(true));
      assertNull(err);
    }
