...
public class ListenerTest extends BaseNd4jTestWithBackends {
  ...
  public void testListenerCalls() {
    ...
    for (int i = 1; i <= 3; i++) {
      ...
      assertEquals(
          7 * i,
          tl.preOpExecutionCount); // mmul, add, softmax, loss grad, softmax backward, add backward,
                                   // mmul backward
      assertEquals(7 * i, tl.opExecutionCount);
      assertEquals(
          11 * i,
          tl.activationAvailableCount); // mmul, add, softmax, loss grad (weight, in, label),
                                        // softmax bp, add backward (z, b), mmul (in, w)
      assertEquals(0, tl.preUpdateCount);
    }

    // Check listener NOT called during gradient calculation - when listener is still set to
    // INFERENCE mode
    ...
    SingletonDataSetIterator dsi =
        new SingletonDataSetIterator(new DataSet(phMap.get("in"), phMap.get("label")));
    for (int i = 1; i <= 3; i++) {
      sd.fit(dsi, 1);
    ...
    TrainingConfig config =
        new TrainingConfig.Builder()
            ...
            .dataSetFeatureMapping(
                "input") // DataSet features array should be associated with variable "input"
            .dataSetLabelMapping("label")
  ...
  private static class TestListener implements Listener {
    ...
    public ListenerResponse epochEnd(
        SameDiff sd, At at, LossCurve lossCurve, long epochTimeMillis) {
      epochEndCount++;
    ...
    public void iterationDone(
        final SameDiff sd, final At at, final MultiDataSet dataSet, final Loss loss) {
      iterationDoneCount++;
    }
