...
public class NumpyFormatTests extends BaseNd4jTestWithBackends {
  ...
  public void testToNpyFormat(Nd4jBackend backend) throws Exception {
    ...
    for (File f : files) {
      ...
      assertEquals(
          expectedArr, resultArr, "Failed with file [" + f.getName() + "] on data type " + dt);
      cnt++;
    }
  ...
  public void testNumpyConversion() throws Exception {
    ...
    Pointer pointer1 =
        NativeOpsHolder.getInstance().getDeviceNativeOps().dataPointForNumpyStruct(pointer);
  ...
  @Test
  @Disabled(
      "Test is very large compared to most tests. It needs to be to test the limits of memcpy/heap"
          + " space.")
  ...
  public void testNumpyWrite() throws Exception {
    ...
    File tmpFile =
        new File(
            ...
            "nd4j-numpy-tmp-" + UUID.randomUUID().toString() + ".bin");
