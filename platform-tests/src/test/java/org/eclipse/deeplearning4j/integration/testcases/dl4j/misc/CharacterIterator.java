...
public class CharacterIterator implements DataSetIterator {
  ...
  public CharacterIterator(
    ...
    int maxSize =
        lines.size(); // add lines.size() to account for newline characters at end of each line
    ...
    //        System.out.println("Loaded and converted file: " + fileCharacters.length + " valid
    // characters of "
  ...
  public DataSet next(int num) {
    ...
    // Why 'f' order here? See https://deeplearning4j.konduit.ai/models/recurrent data section
    // "Alternative: Implementing a custom DataSetIterator"
    INDArray input =
        Nd4j.create(new int[] {currMinibatchSize, validCharacters.length, exampleLength}, 'f');
    INDArray labels =
        Nd4j.create(new int[] {currMinibatchSize, validCharacters.length, exampleLength}, 'f');
  ...
  private void initializeOffsets() {
    ...
    int nMinibatchesPerEpoch =
        (fileCharacters.length - 1) / exampleLength
            - 2; // -2: for end index, and for partial example
  ...
  public static CharacterIterator getShakespeareIterator(int miniBatchSize, int sequenceLength)
    ...
    String url =
        "https://raw.githubusercontent.com/KonduitAI/dl4j-test-resources/master/src/main/resources/word2vec/shakespeare.txt";
    ...
    char[] validCharacters =
        CharacterIterator
            .getMinimalCharacterSet(); // Which characters are allowed? Others will be removed
