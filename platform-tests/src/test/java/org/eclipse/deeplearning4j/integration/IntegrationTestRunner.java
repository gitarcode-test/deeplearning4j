...
public class IntegrationTestRunner {
  ...
  public static final String RANDOM_INIT_UNTRAINED_MODEL_FILENAME =
      "Model_RANDOM_INIT_UNTRAINED.zip";
  ...
  public static void setup() throws Exception {
    ...
    try {
      ...
      info =
          ClassPath.from(DifferentialFunctionClassHolder.class.getClassLoader())
  ...
  public static void runTest(TestCase tc, Path testDir) throws Exception {
    BaseDL4JTest
        .skipUnlessIntegrationTests(); // Tests will ONLY be run if integration test profile is
                                       // enabled.
    ...
    // tc.getTestName()).copyDirectory(testBaseDir);
    Resources.copyDirectory(
        (modelType == ModelType.SAMEDIFF
                ? "samediff-integration-tests/"
    ...
    if (tc.getTestType() == TestCase.TestType.RANDOM_INIT) {
      ...
      File savedModel =
          new File(testBaseDir, IntegrationTestRunner.RANDOM_INIT_UNTRAINED_MODEL_FILENAME);
      ...
      if (config instanceof MultiLayerConfiguration) {
        ...
        assertEquals(
            loaded.getLayerWiseConfigurations(),
            mln.getLayerWiseConfigurations(),
            "Configs not equal");
    ...
    if (tc.isTestPredictions()) {
      ...
      List<Pair<INDArray[], INDArray[]>> inputs =
          modelType != ModelType.SAMEDIFF ? tc.getPredictionsTestData() : null;
      List<Map<String, INDArray>> inputsSd =
          modelType == ModelType.SAMEDIFF ? tc.getPredictionsTestDataSameDiff() : null;
      Preconditions.checkState(
          modelType == ModelType.SAMEDIFF || inputs != null && inputs.size() > 0,
          "Input data is null or length 0 for test: %s",
          tc.getTestName());
      ...
      if (modelType == ModelType.MLN) {
        for (Pair<INDArray[], INDArray[]> p : inputs) {
          ...
          INDArray predictionExceedsRE =
              exceedsRelError(
                  outSaved, out, tc.getMaxRelativeErrorOutput(), tc.getMinAbsErrorOutput());
        ...
        for (Pair<INDArray[], INDArray[]> p : inputs) {
          ...
          for (int i = 0; i < outSaved.length; i++) {
            INDArray predictionExceedsRE =
                exceedsRelError(
                    outSaved[i], out[i], tc.getMaxRelativeErrorOutput(), tc.getMinAbsErrorOutput());
            int countExceeds = predictionExceedsRE.sumNumber().intValue();
            assertEquals(
                0, countExceeds, "Predictions do not match saved predictions - output " + i);
          }
        }
      } else {
          ...
          for (String s : outNames) {
            INDArray predictionExceedsRE =
                exceedsRelError(
                    outSaved.get(s),
                    out.get(s),
                    tc.getMaxRelativeErrorOutput(),
                    tc.getMinAbsErrorOutput());
            int countExceeds = predictionExceedsRE.sumNumber().intValue();
            assertEquals(
                0,
                countExceeds,
                "Predictions do not match saved predictions - output \"" + s + "\"");
          }
        }
      }
    ...
    if (tc.isTestGradients()) {
      ...
      if (modelType != ModelType.SAMEDIFF) {
        ...
        INDArray gradExceedsRE =
            exceedsRelError(
                gradientFlatSaved,
                gradientFlat,
                tc.getMaxRelativeErrorGradients(),
                tc.getMinAbsErrorGradients());
      ...
      for (File f : gradientDir.listFiles()) {
        ...
        INDArray gradExceedsRE =
            exceedsRelError(
                loaded, now, tc.getMaxRelativeErrorGradients(), tc.getMinAbsErrorGradients());
        int count = gradExceedsRE.sumNumber().intValue();
    ...
    if (tc.isTestUnsupervisedTraining()) {
      ...
      INDArray exceedsRelError =
          exceedsRelError(
              expParams,
              paramsPostTraining,
              tc.getMaxRelativeErrorPretrainParams(),
      ...
      // Set params to saved ones - to avoid accumulation of roundoff errors causing later
      // failures...
    ...
    if (tc.isTestTrainingCurves() || tc.isTestParamsPostTraining()) {
      ...
      CountingMultiDataSetIterator countingIter =
          new CountingMultiDataSetIterator(trainData, isTbptt, tbpttLength);
      ...
      Map<String, INDArray> frozenParamsBefore =
          modelType != ModelType.SAMEDIFF ? getFrozenLayerParamCopies(m) : getConstantCopies(sd);
      org.deeplearning4j.nn.api.Layer[] layers = null;
      History h = null;
      if (modelType == ModelType.MLN) {
        ...
        if (modelType != ModelType.SAMEDIFF) {
          ...
          INDArray z =
              exceedsRelError(
                  m.params(),
                  paramsExp,
                  tc.getMaxRelativeErrorParamsPostTraining(),
                  tc.getMinAbsErrorParamsPostTraining());
          ...
          for (SDVariable v : sd.variables()) {
            ...
            assertEquals(
                0,
                count,
                "Number of params exceeded max relative error for parameter: \"" + v.name() + "\"");
          }
        }
      }
      ...
      for (int i = 0; i < evals.length; i++) {
        ...
        if (evals[i].getClass() == Evaluation.class) {
          ...
          throw new RuntimeException(
              "Unknown/not implemented evaluation type: " + evals[i].getClass());
        }

        assertEquals(e, evals[i], "Evaluation not equal: " + evals[i].getClass());

        // Evaluation coverage information:
        evaluationClassesSeen.put(
            evals[i].getClass(), evaluationClassesSeen.getOrDefault(evals[i].getClass(), 0) + 1);
    ...
    if (tc.isTestOverfitting()) {
      ...
      for (int i = 0; i < n; i++) {
        INDArray out =
            modelType == ModelType.SAMEDIFF
                ? outSd.get(tc.getPredictionsNamesSameDiff().get(i))
                : output[i];
        INDArray label = toOverfit.getLabels(i);

        INDArray z =
            exceedsRelError(
                out, label, tc.getMaxRelativeErrorOverfit(), tc.getMinAbsErrorOverfit());
        int count = z.sumNumber().intValue();
        if (count > 0) {
  ...
  // Work out which layers, vertices etc we have seen - so we can (at the end of all tests) log our
  ...
  private static void collectCoverageInformation(Model m) {
    ...
    for (org.deeplearning4j.nn.api.Layer l : layers) {
      ...
      layerConfClassesSeen.put(
          lConf.getClass(), layerConfClassesSeen.getOrDefault(lConf.getClass(), 0) + 1);
    }
    ...
    if (isMLN) {
      ...
      for (org.deeplearning4j.nn.conf.graph.GraphVertex gv :
          cg.getConfiguration().getVertices().values()) {
    ...
    for (InputPreProcessor ipp : preProcessors) {
      preprocessorConfClassesSeen.put(
          ipp.getClass(), preprocessorConfClassesSeen.getOrDefault(ipp.getClass(), 0) + 1);
    }
    ...
    if (!isMLN) {
      for (org.deeplearning4j.nn.conf.graph.GraphVertex gv :
          cg.getConfiguration().getVertices().values()) {
        vertexConfClassesSeen.put(
            gv.getClass(), vertexConfClassesSeen.getOrDefault(gv.getClass(), 0) + 1);
      }
    }
  }
    ...
    for (Class<?> c : graphVertexClasses) {
      if (vertexConfClassesSeen.containsKey(c)) {
        log.info(
            ...
            preprocessorConfClassesSeen.get(c),
            c.getName());
      }
    }
  ...
  private static INDArray exceedsRelError(
      INDArray first, INDArray second, double maxRel, double minAbs) {
  ...
  private static INDArray relativeError(
      @NonNull INDArray a1, @NonNull INDArray a2, double minAbsError) {
    long numNaN1 =
        Nd4j.getExecutioner()
            .exec(new MatchCondition(a1, Conditions.isNan(), Integer.MAX_VALUE))
            .getInt(0);
    long numNaN2 =
        Nd4j.getExecutioner()
            .exec(new MatchCondition(a2, Conditions.isNan(), Integer.MAX_VALUE))
            .getInt(0);
  ...
  public static void testParallelInference(
      @NonNull ParallelInference inf, List<Pair<INDArray[], INDArray[]>> in, List<INDArray[]> exp)
      throws Exception {
  ...
  public static void logFailedParams(
      int maxNumToPrintOnFailure,
      String prefix,
      org.deeplearning4j.nn.api.Layer[] layers,
      INDArray exceedsRelError,
      INDArray exp,
    ...
    for (int i = 0; i < length; i++) {
      if (exceedsRelError.getDouble(i) > 0) {
        ...
        log.info(
            "{} {} ({}) failed: expected {} vs actual {} (RelativeError: {}, AbsError: {})",
            i,
            prefix,
            pName,
            dExp,
