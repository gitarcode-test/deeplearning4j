...
public class TFGraphTestAllHelper {
  ...
  public static TensorflowFrameworkImporter tensorflowFrameworkImporter =
      new TensorflowFrameworkImporter();
  public static final String PRINT_GRAPH_PROP = "org.nd4j.imports.tfgraphs.printgraphs";
  // stop on first failure
  private static boolean failFast =
      System.getProperty("org.nd4j.imports.tfgraphs.failfast", "false").equalsIgnoreCase("true");
  ...
  public static class DefaultGraphLoader implements BiFunction<File, String, ModelLoadResult> {
    ...
    public ModelLoadResult apply(File file, String name) {
      ...
      try {
        SameDiff result =
            tensorflowFrameworkImporter.runImport(
                file.getAbsolutePath(), dynamicVariables, suggestDynamicVariables, false);
  ...
  private static ExecutorConfiguration configuration =
  ...
  public static List<Object[]> fetchTestParams(
      String baseDir,
    ...
    if (endIndex < 0) endIndex = modelNames.length;
    ...
    for (int i = startIndex; i < endIndex; i++) {
      ...
      currentParams[0] =
          inputVars(modelNames[i], baseDir, localTestDir); // input variable map - could be null
  ...
  public static void checkOnlyOutput(
      Map<String, INDArray> inputs,
      Map<String, INDArray> predictions,
      String modelName,
      String baseDir,
      String modelFilename,
      ExecuteWith execType,
      BiFunction<File, String, ModelLoadResult> loader,
      Double maxRelErrorOverride,
      Double minAbsErrorOverride,
      boolean printArraysDebugging)
      throws IOException {
    Preconditions.checkArgument(
        (maxRelErrorOverride == null) == (minAbsErrorOverride == null),
        "Both maxRelErrorOverride and minAbsErrorOverride"
    ...
    for (String s : predictions.keySet()) {
      // we need to convert name from python name format with . on indices, to :. i.e.: output.1 ->
      // output:1
      if (s.matches(".*\\.\\d+")) {
    ...
    Pair<SameDiff, Map<String, INDArray>> p =
        getGraphAfterExec(
            baseDir,
            modelFilename,
            modelName,
            ...
            null,
            outputsToCheck,
            printArraysDebugging);
    if (p == null) {
      // for some reason fail fast doesn't happen when it should even before model loading this is a
      // way
    ...
    if (!execType.equals(ExecuteWith.JUST_PRINT)) {
      ...
      for (String outputNode : predictions.keySet()) {
        ...
        // we need to convert name from python name format with . on indices, to :. i.e.: output.1
        // -> output:1
        if (outputNode.contains(".")) nd4jNode = outputNode.replaceAll("\\.", ":");
        ...
        try {
          ...
          throw new NullPointerException(
              "Can't find SameDiff variable with name [" + nd4jNode + "]");
        }
        ...
        try {
          ...
          throw new NullPointerException(
              "Can't find predicted variable with name [" + outputNode + "]");
        }
        ...
        if (maxRelErrorOverride == null) {
          ...
          boolean eq =
              getEqualityFunction(modelName, outputNode, tfPred, nd4jPred).apply(tfPred, nd4jPred);

          if (!eq) {
            // Check for both NaN, both inf
            if (tfPred.dataType().isFPType()
                && tfPred.equalShapes(nd4jPred)
                && tfPred.isNaN().castTo(DataType.INT).sumNumber().intValue() == tfPred.length()
                && nd4jPred.isNaN().castTo(DataType.INT).sumNumber().intValue()
                    == nd4jPred.length()) {
              // All NaNs in both arrays
              eq = true;
            } else if (tfPred.dataType().isFPType()
                && tfPred.equalShapes(nd4jPred)
                && tfPred.isInfinite().castTo(DataType.INT).sumNumber().intValue()
                    == tfPred.length()
                && nd4jPred.isInfinite().castTo(DataType.INT).sumNumber().intValue()
                    == nd4jPred.length()) {
              // All infinite in both arrays. But need to check that it's all positive vs. negative
              // infinite in both cases...
              NdIndexIterator iter = new NdIndexIterator(tfPred.shape());
              eq = true;
              while (iter.hasNext()) {
                long[] next = iter.next();
                // Already know they are both infinite, only question is whether they are both
          ...
          if (!tfPred.equalShapes(nd4jPred)) {
            ...
            fail(
                "Output node \""
                    + outputNode
                    + "\" SameDiff output shape does not match TF output shape: SameDiff shape: "
                    + Arrays.toString(nd4jPred.shape())
                    + " vs. TF shape: "
                    + Arrays.toString(tfPred.shape()));
          }
          ...
          if (tfPred.dataType() != nd4jPred.dataType()) {
            ...
            fail(
                "Output node \""
                    + outputNode
                    + "\" SameDiff output datatype does not match TF output : SameDiff type: "
                    + nd4jPred.dataType()
                    + " vs. TF datatype: "
                    + tfPred.dataType());
          }
          ...
          INDArray absErrorMask =
              diff.gte(minAbsErrorOverride)
                  .castTo(
                      tfPred
                          .dataType()); // value 1 if x[i] > minAbsError; value 0 otherwise. Used to
                                        // get rid of 1e-30 vs. 1e-29 type failures
          INDArray sumAbs = Transforms.abs(tfPred, true).addi(Transforms.abs(nd4jPred, true));
          BooleanIndexing.replaceWhere(
              sumAbs,
              1.0,
              Conditions.equals(0.0)); // Can only get 0.0 if both are zeros - need to avoid 0/0=NaN
          ...
          */
          ...
          INDArray maxAbs =
              Transforms.max(
                  Transforms.abs(tfPred.castTo(DataType.DOUBLE), true),
                  Transforms.abs(nd4jPred.castTo(DataType.DOUBLE), true),
                  true);
          long countMaxAbsGTThreshold =
              maxAbs.gte(minAbsErrorOverride).castTo(DataType.INT).sumNumber().intValue();
          long countNotMasked =
              absErrorMask
                  .sumNumber()
                  .intValue(); // Values are 0 or 1... if all 0s -> nothing being tested
          if (countNotMasked == 0 && countMaxAbsGTThreshold == 0) {
            ...
            fail(
                "All values for node "
                    + outputNode
                    + " are masked out due to minAbsError="
                    + minAbsErrorOverride
          ...
          int countExceeds =
              Nd4j.getExecutioner()
                  .exec(new MatchCondition(relError, Conditions.greaterThan(maxRelErrorOverride)))
                  .getInt(0);
          ...
          assertEquals(
              ...
              outputNode
                  + ": "
                  + countExceeds
                  + " values exceed maxRelError="
                  + maxRelErrorOverride
                  + " with minAbsError="
                  + minAbsErrorOverride
                  + "; largest observed relError="
                  + maxRE);
        }
      }
    ...
    OpValidation.checkDeserializedEquality(
        graph, serialized, new TestCase(graph).testName(modelName).placeholderValues(inputs));
  ...
  public static void checkIntermediate(
    ...
    if (!execType.equals(ExecuteWith.JUST_PRINT)) {
      int count = 0;
      // Evaluate the nodes in their execution order - this is useful for debugging (as we want the
      // *first* failure
      ...
      for (String varName : varNames) {
        if (!inputs.containsKey(varName)) { // avoiding placeholders
          ...
          if (skipNode(modelName, varName)) {
            ...
            // assertArrayEquals("Shape not equal on node " + varName, tfValue.shape(),
            // graph.getVariable(varName).getShape());
            INDArray sdVal = sdPredictions.get(varName);
            if (maxRelErrorOverride != null) {
              INDArray diff = Transforms.abs(tfValue.sub(sdVal), false);
              INDArray absErrorMask =
                  diff.gte(
                      minAbsErrorOverride); // value 1 if x[i] > minAbsError; value 0 otherwise.
                                            // Used to get rid of 1e-30 vs. 1e-29 type failures
              INDArray sumAbs = Transforms.abs(tfValue, true).addi(Transforms.abs(sdVal, true));
              BooleanIndexing.replaceWhere(
                  sumAbs,
                  1.0,
                  Conditions.equals(
                      0.0)); // Can only get 0.0 if both are zeros - need to avoid 0/0=NaN
              INDArray relError = diff.divi(sumAbs);
              relError.muli(absErrorMask);
              ...
              int countExceeds =
                  Nd4j.getExecutioner()
                      .exec(
                          new MatchCondition(relError, Conditions.greaterThan(maxRelErrorOverride)))
                      .getInt(0);
   ...
   * @param originalResultOutputs the original expected outputs. THis is just in case we are missing
   *     something. This is common when some output nodes output more than 1 result but we are
   *     testing for it.
   * @return
   */
  public static Map<String, INDArray> runTfResults(
      GraphDef result,
      Map<String, INDArray> inputs,
      File modelPath,
      Set<String> originalResultOutputs) {
    List<String> inputNames = new ArrayList<>(inputs.keySet());

    List<String> outputNames =
        new ArrayList<>(
            result.getNodeList().stream()
  ...
  public static Pair<SameDiff, Map<String, INDArray>> getGraphAfterExec(
      String baseDir,
      String modelFilename,
      String modelName,
      Map<String, INDArray> inputs,
      ExecuteWith executeWith,
      BiFunction<File, String, ModelLoadResult> graphLoaderFunction,
    ...
    */
    ModelLoadResult result =
        graphLoaderFunction.apply(
            new ClassPathResource(baseDir + "/" + modelName + "/" + modelFilename).getFile(),
            modelName);
  ...
  private static String[] modelDirNames(
      String base_dir, ExecuteWith executeWith, String modelFileName) throws IOException {
    ResourcePatternResolver resolver =
        new PathMatchingResourcePatternResolver(new ClassPathResource(base_dir).getClassLoader());
    ...
    for (int i = 0; i < resources.length; i++) {
      ...
      exampleNames[i] =
          nestedName.replaceAll(Pattern.quote(base_dir), "").replaceAll("/" + modelFileName, "");
    }

    // only load models we need
    if (EXECUTE_ONLY_MODELS.isEmpty()) return exampleNames;
    else {
      return Arrays.stream(exampleNames)
          .filter(s -> EXECUTE_ONLY_MODELS.contains(s))
          .toArray(String[]::new);
    }
  }

  protected static Map<String, INDArray> inputVars(
      String modelName, String base_dir, File localTestDir) throws IOException {
    return readVars(modelName, base_dir, "**.placeholder", true, localTestDir);
  }

  protected static Map<String, INDArray> outputVars(
      String modelName, String base_dir, File localTestDir) throws IOException {
    return readVars(modelName, base_dir, "**.prediction", true, localTestDir);
  }

  protected static Map<String, INDArray> inbetweenVars(
      String modelName, String base_dir, File localTestDir) throws IOException {
   ...
   *
   * <p>How is a node that has a list of outputs like in the case of "node_multiple_out" work Below
   * is hardcoded for a single node
   */
  protected static INDArray intermediateVars(
      String modelName, String base_dir, String varName, File localTestDir) throws IOException {
    // convert varName to convention used in naming files
    // "/" replaced by "____"; followed by a digit indicating the output number followed by
    // prediction_inbw.(shape|csv)
    if (varName.contains(":")) {
    ...
    if (nodeSepOutput.isEmpty()) {
      // Edge case: intermediates were generated with help of import_graph_def method, which by
      // default adds "import/" to names
      // for some reason. https://www.tensorflow.org/api_docs/python/tf/graph_util/import_graph_def
      // So many of earlier intermediate nodes test data were generated with filenames like
      // "import___X..." instead of "X..."
    ...
    Set<String> removeList =
        confirmPatternMatch(
            nodeSepOutput.keySet(), importNameWorkaround ? "import/" + varName : varName);
    for (String toRemove : removeList) {
      nodeSepOutput.remove(toRemove);
    }
    if (importNameWorkaround) {
      return nodeSepOutput.get(
          "import/" + varName); // this *should* return a list of the indarrays for each node
    } else {
      return nodeSepOutput.get(
          varName); // this *should* return a list of the indarrays for each node
    }
  }
  ...
  public static Set<String> confirmPatternMatch(Set<String> setOfNames, String varName) {
    ...
    for (String name : setOfNames) {
      ...
      if (!NumberUtils.isNumber(splitByPeriod[splitByPeriod.length - 1])) {
        removeList.add(name);
      } else if (!String.join(".", Arrays.copyOfRange(splitByPeriod, 0, splitByPeriod.length - 1))
          .equals(varName)) {
        removeList.add(name);
      }
    }
  ...
  protected static Map<String, INDArray> readVars(
      String modelName, String base_dir, String pattern, boolean recursive, File localTestDir)
      throws IOException {
    ...
    if (recursive) {
      ...
      if (localPath != null
          && (!localPath.contains("src/main/resources")
              && !localPath.contains("src\\main\\resources"))) {
      ...
      File baseDir =
          localPath == null
              ? new File(localTestDir, "extracted/" + modelName)
              : new File(localPath, base_dir + "/" + modelName);
      ...
      if (!baseDir.exists() || arr == null || arr.length == 0) {
        ...
        if (localPath == null) {
          ...
          throw new IllegalStateException(
              "local directory declared but could not find files: " + baseDir.getAbsolutePath());
        }
      }
          ...
          for (File f : files) {
            if (f.isDirectory()) {
              ...
              if (filename.matches(nameRegex)) {
                ...
                resources.add(
                    new Pair<>(new FileSystemResource(f), new FileSystemResource(csvFile)));
      ...
      ResourcePatternResolver resolver =
          new PathMatchingResourcePatternResolver(new ClassPathResource(modelDir).getClassLoader());
      Resource[] r = resolver.getResources("classpath*:" + modelDir + "/" + pattern + ".shape");
      for (Resource res : r) {
        String fileName = res.getFilename();
        String varPath = modelDir + "/" + fileName;
        Resource r2 =
            new org.springframework.core.io.ClassPathResource(varPath.replace(".shape", ".csv"));
    ...
    for (int i = 0; i < resources.size(); i++) {
      ...
      if (filtered.size() == 0) {
        // Scalar
        String content =
            IOUtils.toString(resources.get(i).getSecond().getInputStream(), StandardCharsets.UTF_8);
        switch (type) {
          case DOUBLE:
          case FLOAT:
          case HALF:
        ...
        try {
          ...
          // note: we used to auto convert [0] to [] here. This affects results and has been
          // removed.
          ...
          switch (type) {
            ...
            default:
              throw new UnsupportedOperationException(
                  "Unknown / not implemented datatype: " + type);
          }
  ...
  private static long parseLong(String line) {
    ...
    if (line.matches("-?\\d+\\.0+")) {
      // Annoyingly, some integer data is stored with redundant/unnecessary zeros - like
      // "-7.0000000"
  ...
  private static boolean parseBoolean(String line) {
    line = line.trim();
    if (line.matches(
        "1(\\.0*)?")) { // Booleans are occasionally represented like 1.000000 or 0.000000
      return true;
  ...
  public static BiFunction<INDArray, INDArray, Boolean> getEqualityFunction(
      String modelName, String varName, INDArray tf, INDArray sd) {
    ...
    if (modelName.startsWith("random_gamma")) {
      return (t, s) -> {
        boolean areEqualShapes = t.equalShapes(s);
        boolean nonNegativeValues =
            (t.minNumber().doubleValue() > 0) && (t.minNumber().doubleValue() > 0);
        double meanS = s.meanNumber().doubleValue();
        double meanT = t.meanNumber().doubleValue();
        double stdS = s.stdNumber().doubleValue();
        double stdT = t.stdNumber().doubleValue();
        double eps = 1;
        return areEqualShapes
            && nonNegativeValues
            && (Math.abs(meanS - meanT) < eps)
            && (Math.abs(stdS - stdT) < eps);
      };
    }
    ...
    if (modelName.startsWith("alpha_dropout")
        || modelName.startsWith("layers_dropout")
        || modelName.startsWith("dropout"))
        ...
        for (int i = 0; i < tfNums.length; i++) {
          if (!equalsWithEps(tfNums[i], sdNums[i])) {
            ...
            if (seen1 != null && seen2 != null) {
              if (equalsWithEps(tfNums[i], seen1)
                  || equalsWithEps(sdNums[i], seen1)) // the dropout is in seen1
              seen2 = null;
              else if (equalsWithEps(tfNums[i], seen2)
                  || equalsWithEps(sdNums[i], seen2)) { // the dropout is in seen2
                seen1 = seen2;
                seen2 = null;
              } else // neither match
              return false;
