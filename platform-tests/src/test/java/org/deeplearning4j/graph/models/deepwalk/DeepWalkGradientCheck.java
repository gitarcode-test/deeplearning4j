...
public class DeepWalkGradientCheck extends BaseDL4JTest {
  ...
  public void checkGradients() throws IOException {
    ...
    Graph<String, String> graph =
        GraphLoader.loadUndirectedGraphEdgeListFile(
            cpr.getTempFileFromArchive().getAbsolutePath(), 7, ",");
      ...
      for (int j = 0; j < 7; j++) { // out
        ...
        for (int p = 0; p < pathInnerNodes.length; p++) {
          ...
          for (int v = 0; v < innerNodeVector.length(); v++) {
            ...
            innerNodeVector.putScalar(
                v, origParamValue); // reset param so it doesn't affect later calcs
            ...
            if (relError > MAX_REL_ERROR && absErr > MIN_ABS_ERROR) fail(msg);
            //                        else
  ...
  public void checkGradients2() throws IOException {
    ...
    Graph<String, String> graph =
        GraphLoader.loadUndirectedGraphEdgeListFile(
            cpr.getTempFileFromArchive().getAbsolutePath(), 13, ",");
      ...
      for (int j = 0; j < nVertices; j++) { // out
        ...
        for (int p = 0; p < pathInnerNodes.length; p++) {
          ...
          for (int v = 0; v < innerNodeVector.length(); v++) {
            ...
            if (relError > MAX_REL_ERROR && absErr > minAbsError) fail(msg);
            //                        else
            //                            System.out.println(msg);
          }
        }
