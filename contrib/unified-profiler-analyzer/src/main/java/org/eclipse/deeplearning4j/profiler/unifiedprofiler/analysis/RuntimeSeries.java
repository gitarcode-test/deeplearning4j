...
public class RuntimeSeries {
  private Series runtimeFreeMemory,
      runtimeMaxMemory,
  ...
  public synchronized void record(List<FieldVector> inputVectors) {
    ...
    for (int i = 0; i < inputVectors.get(0).getValueCount(); i++) {
      ...
      javacppAvailablePhysicalBytes
          .getData()
          .add(new XYChart.Data<>(javacppAvailablePhysicalBytesVal, eventTimeMs));
      javacppMaxBytes.getData().add(new XYChart.Data<>(javacppMaxBytesVal, eventTimeMs));
      javacppMaxPhysicalBytes
          .getData()
          .add(new XYChart.Data<>(javacppMaxPhysicalBytesVal, eventTimeMs));
  ...
  public synchronized void record(LogEvent logEvent) {
    runtimeFreeMemory
        ...
        .add(
            new XYChart.Data<>(
                logEvent.getRunTimeMemory().getRuntimeFreeMemory(), logEvent.getEventTimeMs()));
    runtimeMaxMemory
        .getData()
        .add(
            new XYChart.Data<>(
                logEvent.getRunTimeMemory().getRuntimeMaxMemory(), logEvent.getEventTimeMs()));
    javacppAvailablePhysicalBytes
        .getData()
        .add(
            new XYChart.Data<>(
    ...
    javacppMaxBytes
        ...
        .add(
            new XYChart.Data<>(
                logEvent.getRunTimeMemory().getJavacppMaxBytes(), logEvent.getEventTimeMs()));
    javacppMaxPhysicalBytes
        .getData()
        .add(
            new XYChart.Data<>(
    ...
    javacppTotalBytes
        ...
        .add(
            new XYChart.Data<>(
                logEvent.getRunTimeMemory().getJavacppTotalBytes(), logEvent.getEventTimeMs()));
    javacppPointerCount
        .getData()
        .add(
            new XYChart.Data<>(
                logEvent.getRunTimeMemory().getJavacppPointerCount(), logEvent.getEventTimeMs()));
  }
