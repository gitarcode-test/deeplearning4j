...
public class OpLogEventComparator {
  ...
  public static void main(String[] args) throws Exception {
    if (args.length != 3) {
      System.out.println(
          "Please provide two database file paths and an epsilon value as arguments.");
      return;
    }
    ...
    try {
      ...
      if (!differences.isEmpty()) {
        ...
        for (Map.Entry<String, List<OpDifference>> entry : differences.entrySet()) {
          ...
          for (OpDifference diff : entry.getValue()) {
            ...
            System.out.println(
                "  Op Name: "
                    + (diff.getOpLog1() != null
                        ? diff.getOpLog1().getOpName()
                        : diff.getOpLog2().getOpName()));
        ...
        System.out.println(
            "No differences found for the same inputs within the specified epsilon.");
      }
  ...
  public static Map<String, List<OpDifference>> findDifferences(
      String jdbcUrl1, String jdbcUrl2, double epsilon) throws SQLException {
    String query =
        "SELECT id, sourceCodeLine, opName, inputs, outputs, stackTrace FROM OpLogEvent ORDER BY"
            + " id";
    ...
    try (Connection conn1 =
            DriverManager.getConnection(
                jdbcUrl1, InterceptorEnvironment.USER, InterceptorEnvironment.PASSWORD);
        Connection conn2 =
            DriverManager.getConnection(
                jdbcUrl2, InterceptorEnvironment.USER, InterceptorEnvironment.PASSWORD);
  ...
  private static void processResultSet(ResultSet rs, Map<String, List<OpLogEvent>> events)
      throws SQLException {
    while (rs.next()) {
  ...
  private static Map<String, List<OpDifference>> compareOpLogArrays(
      Map<String, List<OpLogEvent>> events1,
      Map<String, List<OpLogEvent>> events2,
      double epsilon) {
    ...
    for (String line : events1.keySet()) {
      ...
      for (int i = 0; i < minSize; i++) {
        ...
        // Compare inputs
        OpDifference inputDifference =
            compareInputs(
                opLogEvent1.getInputs(),
                opLogEvent2.getInputs(),
                epsilon,
                opLogEvent1,
                opLogEvent2);
        if (isValidDifference(inputDifference)
            && isSignificantDifference(inputDifference, epsilon)) {
          lineDifferences.add(inputDifference);
          earliestDifference = updateEarliestDifference(earliestDifference, inputDifference);
          earliestSignificantDifference =
              updateEarliestDifference(earliestSignificantDifference, inputDifference);
        }

        // Compare outputs
        OpDifference outputDifference =
            compareOutputs(
                opLogEvent1.getOutputs(),
                opLogEvent2.getOutputs(),
                epsilon,
                opLogEvent1,
                opLogEvent2);
        if (isValidDifference(outputDifference)
            && isSignificantDifference(outputDifference, epsilon)) {
          lineDifferences.add(outputDifference);
          earliestDifference = updateEarliestDifference(earliestDifference, outputDifference);
          earliestSignificantDifference =
              updateEarliestDifference(earliestSignificantDifference, outputDifference);
        }
      }
    ...
    for (String line : events2.keySet()) {
      if (!events1.containsKey(line)) {
        ...
        OpDifference missingLineDifference =
            OpDifference.builder()
                .opLog1(null)
                ...
                .build();
        if (isValidDifference(missingLineDifference)
            && isSignificantDifference(missingLineDifference, epsilon)) {
    ...
    List<Map.Entry<String, OpDifference>> sortedEarliestDifferences =
        sortDifferences(earliestDifferences);
    ...
    List<Map.Entry<String, OpDifference>> sortedSignificantDifferences =
        sortDifferences(earliestSignificantDifferences);
  ...
  private static Map<String, List<OpLogEvent>> loadEvents(String jdbcUrl) throws SQLException {
    ...
    try (Connection conn =
            DriverManager.getConnection(
                jdbcUrl, InterceptorEnvironment.USER, InterceptorEnvironment.PASSWORD);
  ...
  private static class LineComparison {
    ...
    LineComparison(
        String line, List<OpLogEvent> events1, List<OpLogEvent> events2, String difference) {
      this.line = line;
  ...
  public static void compareLinesBySide(String jdbcUrl1, String jdbcUrl2, double epsilon)
      throws SQLException {
    ...
    for (String line : allLines) {
      ...
      Pair<String, Integer> difference =
          findSignificantDifference(lineEvents1, lineEvents2, epsilon);
      if (difference != null) {
    ...
    System.out.println(
        "Side-by-side comparison of lines with significant differences (sorted by earliest event"
            + " time):");
    System.out.println("----------------------------------------------------");
  ...
  private static Pair<String, Integer> findSignificantDifference(
      List<OpLogEvent> events1, List<OpLogEvent> events2, double epsilon) {
  ...
  private static String compareWithEpsilon(
      Map<Integer, String> map1, Map<Integer, String> map2, double epsilon) {
  ...
  private static String compareArraysWithEpsilon(JSONArray arr1, JSONArray arr2, double epsilon)
  ...
  private static Map<String, List<OpLogEvent>> loadAndNormalizeEvents(String jdbcUrl)
      throws SQLException {
  ...
  private static OpDifference compareInputs(
      Map<Integer, String> inputs1,
      Map<Integer, String> inputs2,
      double epsilon,
      OpLogEvent opLogEvent1,
      OpLogEvent opLogEvent2) {
  ...
  private static OpDifference compareOutputs(
      Map<Integer, String> outputs1,
      Map<Integer, String> outputs2,
      double epsilon,
      OpLogEvent opLogEvent1,
      OpLogEvent opLogEvent2) {
  ...
  private static JSONComparisonResult compareJSONArraysWithEpsilon(
      JSONArray arr1, JSONArray arr2, double epsilon) {
    if (arr1.length() != arr2.length()) {
    ...
    for (int i = 0; i < arr1.length(); i++) {
      if (arr1.get(i) instanceof JSONArray && arr2.get(i) instanceof JSONArray) {
        JSONComparisonResult result =
            compareJSONArraysWithEpsilon(arr1.getJSONArray(i), arr2.getJSONArray(i), epsilon);
        if (!result.isSame()) {
          return result;
        }
      } else {
  ...
  private static OpDifference updateEarliestDifference(
      OpDifference currentEarliest, OpDifference newDifference) {
  ...
  private static List<Map.Entry<String, OpDifference>> sortDifferences(
      Map<String, OpDifference> differences) {
    List<Map.Entry<String, OpDifference>> sortedDifferences =
        new ArrayList<>(differences.entrySet());
    sortedDifferences.sort(
        (e1, e2) -> {
  ...
  private static void printSortedDifferences(
      List<Map.Entry<String, OpDifference>> sortedDifferences) {
