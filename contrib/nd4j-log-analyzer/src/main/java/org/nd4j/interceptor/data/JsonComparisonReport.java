...
public class JsonComparisonReport {
  ...
  private static Map<String, OpDifference> compareDirectories(
      String directory1, String directory2, double epsilon) {
      ...
      for (File file1 : files1) {
        ...
        if (file2.exists()) {
          try {
            ...
            Map<String, OpDifference> opLogDifferences =
                compareOpLogArrays(
                    eventsGrouped.getOpLogEvents(), eventsGrouped2.getOpLogEvents(), epsilon);
  ...
  private static SourceCodeOpEvent convertJsonToSourceCodeOpEvent(JSONObject jsonObject) {
    ...
    return SourceCodeOpEvent.builder().opLogEvents(opLogEvents).build();
  }
  ...
  private static Map<String, OpDifference> compareOpLogArrays(
      Map<String, List<OpLogEvent>> jsonArray1,
      Map<String, List<OpLogEvent>> jsonArray2,
      double epsilon) {
    Map<String, OpDifference> differences = new HashMap<>();
    for (String key : jsonArray1.keySet()) {
      ...
      if (opLogEvents1 == null || opLogEvents2 == null) continue;
        ...
        for (int i = 0; i < minEventSize; i++) {
          ...
          for (int j = 0; j < inputs.size(); j++) {
            ...
            JSONComparisonResult result =
                compareJSONArraysWithEpsilon(jsonArray, jsonArray3, epsilon);
            if (!result.isSame()) {
          ...
          for (int j = 0; j < outputs.size(); j++) {
            ...
            if (cast instanceof Number) {
              cast = new double[] {((Number) cast).doubleValue()};
            } else if (cast instanceof String) {
              // if string matches a single double between []

              if (cast.toString().matches("-*\\d+\\.\\d+")) {
                cast = new JSONArray(new double[] {Double.parseDouble((String) cast)});
            ...
            if (cast2 instanceof Number) {
              cast2 = new double[] {((Number) cast2).doubleValue()};
            } else if (cast2 instanceof String) {
              // if string matches a single double between []

              if (cast2.toString().matches("-*\\d+\\.\\d+")) {
                cast2 = new JSONArray(new double[] {Double.parseDouble((String) cast2)});
  ...
  private static void generateReport(Map<String, OpDifference> differences, double epsilon) {
    ...
    Map<String, OpDifference> filteredDifferences =
        filterDifferencesByEpsilon(differences, epsilon);

    try {
      InterceptorEnvironment.mapper.writeValue(new File(reportFile), filteredDifferences);
      InterceptorEnvironment.mapper.writeValue(
          new File(earliestDifferenceFile), OpDifference.earliestDifference(filteredDifferences));
  ...
  private static Map<String, OpDifference> filterDifferencesByEpsilon(
      Map<String, OpDifference> differences, double epsilon) {
    Map<String, OpDifference> filteredDifferences = new HashMap<>();

    for (Map.Entry<String, OpDifference> difference : differences.entrySet()) {
      if (isDifferentWithEpsilon(
          difference.getValue().getOpLog1(), difference.getValue().getOpLog2(), epsilon)) {
  ...
  private static JSONComparisonResult compareJSONArraysWithEpsilon(
      JSONArray jsonArray1, JSONArray jsonArray2, double epsilon) {
    ...
    for (int i = 0; i < jsonArray1.length(); i++) {
      ...
      if (value1 instanceof JSONArray) {
        JSONComparisonResult result =
            compareJSONArraysWithEpsilon((JSONArray) value1, (JSONArray) value2, epsilon);
        if (!result.isSame()) {
          return result;
        }
  ...
  private static JSONComparisonResult compareJSONArraysWithEpsilon(
      JSONObject jsonArray1, JSONObject jsonArray2, double epsilon) {
    if (jsonArray1.length() != jsonArray2.length()) {
      return JSONComparisonResult.noDifference();
    }
