...
public class FilterInvalidValues implements Filter {
  ...
  public FilterInvalidValues(String... columnsToFilterIfInvalid) {
    if (columnsToFilterIfInvalid == null || columnsToFilterIfInvalid.length == 0)
      throw new IllegalArgumentException(
          "Cannot filter 0/null columns: columns to filter on must be specified");
  ...
  private boolean filterColumn(List<?> row, int i) {
    ...
    if (row.get(i) instanceof Float) {
      if (!meta.isValid(new FloatWritable((Float) row.get(i)))) return true;
    } else if (row.get(i) instanceof Double) {
      if (!meta.isValid(new DoubleWritable((Double) row.get(i)))) return true;
    } else if (row.get(i) instanceof String) {
      if (!meta.isValid(new Text(((String) row.get(i)).toString()))) return true;
    } else if (row.get(i) instanceof Integer) {
      if (!meta.isValid(new IntWritable((Integer) row.get(i)))) return true;

    } else if (row.get(i) instanceof Long) {
      if (!meta.isValid(new LongWritable((Long) row.get(i)))) return true;
    } else if (row.get(i) instanceof Boolean) {
      if (!meta.isValid(new BooleanWritable((Boolean) row.get(i)))) return true;
    }
    return false;
  }
