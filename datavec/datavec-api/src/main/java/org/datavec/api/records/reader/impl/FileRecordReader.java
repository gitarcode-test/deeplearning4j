...
public class FileRecordReader extends BaseRecordReader {
  ...
  protected String charset =
      StandardCharsets.UTF_8.name(); // Using String as StandardCharsets.UTF_8 is not serializable
  ...
  public int getLabel(URI uri) {
    ...
    int lastIdx =
        Math.max(
            s.lastIndexOf('/'), s.lastIndexOf('\\')); // Note: if neither are found, -1 is fine here
  ...
  public Record nextRecord() {
    ...
    return new org.datavec.api.records.impl.Record(
        ret, new RecordMetaDataURI(next, FileRecordReader.class));
  }
