...
public class SVMLightRecordWriter extends FileRecordWriter {
  ...
  protected boolean zeroBasedIndexing =
      false; // whether to use zero-based indexing, false is safest
  ...
  public void setConf(Configuration conf) {
    ...
    featureLastColumn =
        conf.getInt(FEATURE_LAST_COLUMN, labelFirstColumn > 0 ? labelFirstColumn - 1 : -1);
  ...
  public PartitionMetaData write(List<Writable> record) throws IOException {
    if (!record.isEmpty()) {
      List<Writable> recordList =
          record instanceof List ? (List<Writable>) record : new ArrayList<>(record);
       ...
       */
      if (hasLabel) {
        if (labelLastColumn < 0) labelLastColumn = record.size() - 1;
        if (labelFirstColumn < 0) {
          if (featureLastColumn > 0) labelFirstColumn = featureLastColumn + 1;
          else labelFirstColumn = record.size() - 1;
        }
      }
      ...
      if (featureLastColumn < 0) {
        if (labelFirstColumn > 0) featureLastColumn = labelFirstColumn - 1;
        else featureLastColumn = recordList.size() - 1;
      }
              ...
              if (multilabel) {
                if (val == 1.0) {
                  ...
                  throw new NumberFormatException(
                      "Expect value -1, 0, or 1 for multilabel targets (found " + val + ")");
