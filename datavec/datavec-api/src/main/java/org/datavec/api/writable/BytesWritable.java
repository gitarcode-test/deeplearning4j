...
public class BytesWritable extends ArrayWritable {
  ...
  /**
   * Convert the underlying contents of this {@link Writable} to an nd4j {@link DataBuffer}. Note
   * that this is a *copy* of the underlying buffer. Also note that {@link
   * java.nio.ByteBuffer#allocateDirect(int)} is used for allocation. This should be considered an
   * expensive operation.
   *
   * <p>This buffer should be cached when used. Once used, this can be used in standard Nd4j
   * operations.
   *
   * <p>Beyond that, the reason we have to use allocateDirect is due to nd4j data buffers being
   * stored off heap (whether on cpu or gpu)
   *
