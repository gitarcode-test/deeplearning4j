...
public class TransformProcessSequenceRecordReader implements SequenceRecordReader {
  ...
  /**
   * Load a sequence record from the given DataInputStream Unlike {@link #next()} the internal state
   * of the RecordReader is not modified Implementations of this method should not close the
   * DataInputStream
   *
   * @param uri
  ...
  public List<List<Writable>> sequenceRecord(URI uri, DataInputStream dataInputStream)
      throws IOException {
    return transformProcess.executeSequence(
        sequenceRecordReader.sequenceRecord(uri, dataInputStream));
  }

  /**
   * Similar to {@link #sequenceRecord()}, but returns a {@link Record} object, that may include
   ...
   * Note: that for data that isn't splittable (i.e., text data that needs to be scanned/split), it
   * is more efficient to load multiple records at once using {@link
   * #loadSequenceFromMetaData(List)}
   *
   ...
   * Note: that for data that isn't splittable (i.e., text data that needs to be scanned/split), it
   * is more efficient to load multiple records at once using {@link #loadFromMetaData(List)}
   *
  ...
  public List<Record> loadFromMetaData(List<RecordMetaData> recordMetaDatas) throws IOException {
    ...
    for (Record record : records) record.setRecord(transformProcess.execute(record.getRecord()));
    return records;
  }
