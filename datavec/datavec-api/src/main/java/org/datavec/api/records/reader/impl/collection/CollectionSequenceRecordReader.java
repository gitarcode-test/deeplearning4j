...
public class CollectionSequenceRecordReader extends BaseRecordReader
  ...
  /**
   * @param records Collection of sequences. For example, List<List<List<Writable>>> where the inner
   *     two lists are a sequence, and the outer list/collection is a list of sequences
   */
  public CollectionSequenceRecordReader(
      Collection<? extends Collection<? extends Collection<Writable>>> records) {
  ...
  public List<SequenceRecord> loadSequenceFromMetaData(List<RecordMetaData> recordMetaDatas)
    ...
    while (iter.hasNext()) {
      ...
      SequenceRecord r =
          new org.datavec.api.records.impl.SequenceRecord(
              record, new RecordMetaDataIndex(i - 1, null, CollectionSequenceRecordReader.class));
      out.add(r);
    }
    return out;
  }
