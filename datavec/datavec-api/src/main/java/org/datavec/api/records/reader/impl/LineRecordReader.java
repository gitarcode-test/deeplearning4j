...
public class LineRecordReader extends BaseRecordReader {
  ...
  public Record nextRecord() {
    ...
    RecordMetaData meta =
        new RecordMetaDataLine(
            this.lineIndex - 1,
            uri,
            LineRecordReader.class); // -1 as line number has been incremented already...
  ...
  public List<Record> loadFromMetaData(List<RecordMetaData> recordMetaDatas) throws IOException {
    ...
    // Sort by URI first (if possible - don't always have URIs though, for String split etc), then
    // sort by line number:
    Collections.sort(
        list,
        new Comparator<Triple<Integer, RecordMetaDataLine, List<Writable>>>() {
          @Override
          public int compare(
              Triple<Integer, RecordMetaDataLine, List<Writable>> o1,
    ...
    if (uris.size() > 0 && sortedURIs != null) {
      ...
      Iterator<Triple<Integer, RecordMetaDataLine, List<Writable>>> metaIter =
          list.iterator(); // Currently sorted by URI, then line number

      URI currentURI = sortedURIs.get(0);
      Iterator<String> currentUriIter =
          IOUtils.lineIterator(streamCreatorFn.apply(currentURI), charset);
