...
public class Schema implements Serializable {
   ...
   * @return true if the schema has the same types at every index as this one,false otherwise
   */
  public boolean sameTypes(Schema schema) {
    if (schema.numColumns() != numColumns()) return false;
    for (int i = 0; i < schema.numColumns(); i++) {
      if (getType(i) != schema.getType(i)) return false;
    }
  ...
  /**
   * Compute the difference in {@link ColumnMetaData} between this schema and the passed in schema.
   * This is useful during the {@link org.datavec.api.transform.TransformProcess} to identify what a
   * process will do to a given {@link Schema}.
   *
  ...
  public List<ColumnMetaData> differences(Schema schema) {
    ...
    for (int i = 0; i < schema.numColumns(); i++) {
      if (!columnMetaData.contains(schema.getMetaData(i))) ret.add(schema.getMetaData(i));
    }
   ...
   *
   * @param columnMetaData the new metadata to create the schema from
  ...
  /**
   * A copy of the list of {@link ColumnType} for this schema
   *
   * @return the list of column types in order based on column index for this schema
   */
  ...
  /**
   * Returns a copy of the underlying schema {@link ColumnMetaData}
   *
   ...
   * @return the index of the given column name for the schema
   */
  ...
  public String toString() {
    ...
    // Header:
    sb.append("Schema():\n");
    sb.append(String.format("%-6s", "idx"))
        .append(String.format("%-" + (maxNameLength + 8) + "s", "name"))
  ...
  public static class Builder {
    ...
    /**
     * Add a Float column with no restrictions on the allowable values, except for no NaN/infinite
     * values allowed
     *
    ...
    public Builder addColumnFloat(
      ...
      return addColumn(
          new FloatMetaData(name, minAllowedValue, maxAllowedValue, allowNaN, allowInfinite));
    }

    /**
     * Add multiple Float columns with no restrictions on the allowable values of the columns (other
     * than no NaN/Infinite)
     *
     * @param columnNames Names of the columns to add
     */
    public Builder addColumnsFloat(String... columnNames) {
      for (String s : columnNames) addColumnFloat(s);
      return this;
    }

    /**
     * A convenience method for adding multiple Float columns. For example, to add columns
     * "myFloatCol_0", "myFloatCol_1", "myFloatCol_2", use {@code
    ...
    /**
     * A convenience method for adding multiple Float columns, with additional restrictions that
     * apply to all columns For example, to add columns "myFloatCol_0", "myFloatCol_1",
     ...
     * @param allowInfinite If false: don't allow infinite values. If true: allow
     */
    public Builder addColumnsFloat(
        String pattern,
        int minIdxInclusive,
        int maxIdxInclusive,
        Float minAllowedValue,
      ...
      for (int i = minIdxInclusive; i <= maxIdxInclusive; i++) {
        addColumnFloat(
            String.format(pattern, i), minAllowedValue, maxAllowedValue, allowNaN, allowInfinite);
      }
      return this;
    }

    /**
     * Add a Double column with no restrictions on the allowable values, except for no NaN/infinite
     * values allowed
     *
     * @param name Name of the column
     */
     ...
     * @param allowInfinite If false: don't allow infinite values. If true: allow
     */
    public Builder addColumnDouble(
        String name,
        ...
        boolean allowInfinite) {
      return addColumn(
          new DoubleMetaData(name, minAllowedValue, maxAllowedValue, allowNaN, allowInfinite));
    }

    /**
     * Add multiple Double columns with no restrictions on the allowable values of the columns
     * (other than no NaN/Infinite)
     *
     * @param columnNames Names of the columns to add
     */
    ...
    /**
     * A convenience method for adding multiple Double columns. For example, to add columns
     * "myDoubleCol_0", "myDoubleCol_1", "myDoubleCol_2", use {@code
    ...
    /**
     * A convenience method for adding multiple Double columns, with additional restrictions that
     * apply to all columns For example, to add columns "myDoubleCol_0", "myDoubleCol_1",
     * "myDoubleCol_2", use {@code addColumnsDouble("myDoubleCol_%d",0,2,null,null,false,false)}
     *
     * @param pattern Pattern to use (via String.format). "%d" is replaced with column numbers
     * @param minIdxInclusive Minimum column index to use (inclusive)
     * @param maxIdxInclusive Maximum column index to use (inclusive)
     ...
     * @param allowInfinite If false: don't allow infinite values. If true: allow
     */
    public Builder addColumnsDouble(
        String pattern,
        int minIdxInclusive,
        int maxIdxInclusive,
        Double minAllowedValue,
    ...
    /**
     * A convenience method for adding multiple Integer columns. For example, to add columns
     * "myIntegerCol_0", "myIntegerCol_1", "myIntegerCol_2", use {@code
     ...
     * @param maxAllowedValue Maximum allowed value (inclusive). If null: no restriction
     */
    public Builder addColumnsInteger(
        String pattern,
        int minIdxInclusive,
        int maxIdxInclusive,
    ...
    /**
     * A convenience method for adding multiple Long columns. For example, to add columns
     * "myLongCol_0", "myLongCol_1", "myLongCol_2", use {@code addColumnsLong("myLongCol_%d",0,2)}
     *
     * @param pattern Pattern to use (via String.format). "%d" is replaced with column numbers
     * @param minIdxInclusive Minimum column index to use (inclusive)
     * @param maxIdxInclusive Maximum column index to use (inclusive)
     ...
     *
     * @param name Name of the column
     * @param regex Regex that the String must match in order to be considered valid. If null: no
     *     regex restriction
     ...
     */
    public Builder addColumnString(
        String name, String regex, Integer minAllowableLength, Integer maxAllowableLength) {
      return addColumn(new StringMetaData(name, regex, minAllowableLength, maxAllowableLength));
    }

    /**
     * A convenience method for adding multiple numbered String columns. For example, to add columns
     * "myStringCol_0", "myStringCol_1", "myStringCol_2", use {@code
     ...
     * @param regex Regex that the String must match in order to be considered valid. If null: no
     *     regex restriction
     * @param minAllowedLength Minimum allowed length of strings (inclusive). If null: no
     *     restriction
     * @param maxAllowedLength Maximum allowed length of strings (inclusive). If null: no
     *     restriction
     */
    public Builder addColumnsString(
        String pattern,
        int minIdxInclusive,
        int maxIdxInclusive,
        String regex,
    ...
    /**
     * Add a Time column with no restrictions on the min/max allowable times <b>NOTE</b>: Time
     * columns are represented by LONG (epoch millisecond) values. For time values in human-readable
     * formats, use String columns + StringToTimeTransform
     *
    ...
    /**
     * Add a Time column with the specified restrictions <b>NOTE</b>: Time columns are represented
     * by LONG (epoch millisecond) values. For time values in human-readable formats, use String
     * columns + StringToTimeTransform
     *
     * @param columnName Name of the column
     * @param timeZone Time zone of the time column
     ...
     */
    public Builder addColumnTime(
        String columnName, DateTimeZone timeZone, Long minValidValue, Long maxValidValue) {
     ...
     *
     * @param columnName Name of the column
     * @param shape shape of the NDArray column. Use -1 in entries to specify as "variable length"
     *     in that dimension
     */
    ...
    for (int i = 0; i < record.size(); i++) {
      if (record.get(i) instanceof DoubleWritable) builder.addColumnDouble(String.valueOf(i));
        ...
        throw new IllegalStateException(
            "Illegal writable for infering schema of type "
                + record.get(i).getClass().toString()
                + " with record "
                + record);
    }
