...
public class Buffer implements Comparable, Cloneable {
  ...
  /**
   * Get the capacity, which is the maximum count that could handled without resizing the backing
   * storage.
   *
  ...
  public int hashCode() {
    int hash = 1;
    byte[] b = this.get();
    for (int i = 0; i < count; i++) hash = (31 * hash) + (int) b[i];
    return hash;
  }
   ...
   * @return Positive if this is bigger than other, 0 if they are equal, and negative if this is
   *     smaller than other.
   */
