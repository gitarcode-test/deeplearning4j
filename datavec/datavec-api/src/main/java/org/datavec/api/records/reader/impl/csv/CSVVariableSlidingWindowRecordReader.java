...
public class CSVVariableSlidingWindowRecordReader extends CSVRecordReader
  ...
  /**
   * @param maxLinesPerSequence Number of lines in each sequence, use default delemiter(,) between
  ...
  @Deprecated
  public CSVVariableSlidingWindowRecordReader(
      int maxLinesPerSequence, int stride, String delimiter) {
    this(maxLinesPerSequence, 0, stride, CSVRecordReader.DEFAULT_DELIMITER);
  }

  /**
   * @param maxLinesPerSequence Number of lines in each sequences
   * @param skipNumLines Number of lines to skip at the start of the file (only skipped once, not
   *     per sequence)
   * @param stride Number of lines between records (increment window > 1 line)
  ...
  @Deprecated
  public CSVVariableSlidingWindowRecordReader(
      int maxLinesPerSequence, int skipNumLines, int stride, String delimiter) {
    super(skipNumLines, delimiter.charAt(0));
    if (stride < 1) throw new IllegalArgumentException("Stride must be greater than 1");
   ...
   */
  public CSVVariableSlidingWindowRecordReader(
      int maxLinesPerSequence, int skipNumLines, int stride, char delimiter) {
  ...
  public List<List<Writable>> sequenceRecord() {
    ...
    for (int i = 0; i < stride; i++) {
      if (super.hasNext()) queue.addFirst(super.next());
      else exhausted = true;

      if (exhausted && queue.size() < 1) throw new NoSuchElementException("No next element");

      if (queue.size() > maxLinesPerSequence || exhausted) queue.pollLast();
    }

    List<List<Writable>> sequence = new ArrayList<>();
    sequence.addAll(queue);

    if (exhausted && queue.size() == 1) queue.pollLast();
  ...
  public SequenceRecord nextSequence() {
    ...
    RecordMetaData meta =
        new RecordMetaDataLineInterval(
            lineBefore, lineAfter - 1, uri, CSVVariableSlidingWindowRecordReader.class);
