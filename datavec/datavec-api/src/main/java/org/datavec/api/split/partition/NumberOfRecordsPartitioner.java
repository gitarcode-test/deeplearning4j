...
public class NumberOfRecordsPartitioner implements Partitioner {
  ...
  public OutputStream openNewStream() {
    ...
    // only append when directory, also ensure we can bootstrap and we can write to the current
    // location
    if (currLocation >= locations.length - 1 && locations.length >= 1 && needsNewPartition()
        || inputSplit.needsBootstrapForWrite()
        || locations.length < 1
        || currLocation >= locations.length
        || !inputSplit.canWriteToLocation(locations[currLocation]) && needsNewPartition()) {
