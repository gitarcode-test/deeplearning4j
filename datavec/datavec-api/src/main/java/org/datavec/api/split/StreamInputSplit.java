...
public class StreamInputSplit implements InputSplit {
  ...
  public StreamInputSplit(
      @NonNull List<URI> uris, @NonNull Function<URI, InputStream> streamCreatorFn) {
   ...
   * @param uris The list of URIs to load
   * @param streamCreatorFn The function to be used to create InputStream objects for a given URI
   * @param rng Random number generator instance. If non-null: streams will be iterated over in a
   *     random order. If null: no randomization (iteration order is according to the URIs list)
   */
  public StreamInputSplit(
      @NonNull List<URI> uris, @NonNull Function<URI, InputStream> streamCreatorFn, Random rng) {
    this.uris = uris;
