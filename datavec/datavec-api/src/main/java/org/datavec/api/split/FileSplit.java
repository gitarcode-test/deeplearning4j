...
public class FileSplit extends BaseInputSplit {
  ...
  protected FileSplit(
      File rootDir, String[] allowFormat, boolean recursive, Random random, boolean runMain) {
  ...
  protected void initialize() {
    ...
    if (rootDir == null) throw new IllegalArgumentException("File path must not be null");
      ...
      // Lists one file
      String toString =
          URIUtil.fileToURI(rootDir)
              .toString(); // URI.getPath(), getRawPath() etc don't have file:/ prefix necessary for
                           // conversion back to URI
      ...
      return addNewLocation(
          new File(
                  ...
                  UUID.randomUUID().toString()
                      + "."
                      + FilenameUtils.getExtension(rootDir.getAbsolutePath()))
              .toURI()
              .toString());
    }
  }
