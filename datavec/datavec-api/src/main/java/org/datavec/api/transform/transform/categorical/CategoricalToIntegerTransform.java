    ...
    if (!(meta instanceof CategoricalMetaData))
      throw new IllegalStateException(
          "Cannot convert column \""
              + columnName
              + "\" from categorical to one-hot: column is not categorical (is: "
              + meta.getColumnType()
              + ")");
  ...
  public List<Writable> map(List<Writable> writables) {
    ...
    for (Writable w : writables) {
      if (i++ == idx) {
        ...
        if (classIdx == null) {
          throw new IllegalStateException(
              "Cannot convert categorical value to integer value: input value (\""
                  + str
                  + "\") is not in the list of known categories (state names/categories: "
                  + stateNames
                  + ")");
        }
