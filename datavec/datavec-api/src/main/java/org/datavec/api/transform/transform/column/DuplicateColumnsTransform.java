...
public class DuplicateColumnsTransform implements Transform, ColumnOp {
   ...
   */
  public DuplicateColumnsTransform(
      @JsonProperty("columnsToDuplicate") List<String> columnsToDuplicate,
    ...
    for (String s : columnsToDuplicate) {
      ...
      if (idx == -1)
        throw new IllegalStateException(
            "Invalid state: column to duplicate \""
                + s
                + "\" does not appear "
                + "in input schema");
  ...
  public String toString() {
    return "DuplicateColumnsTransform(toDuplicate="
        + columnsToDuplicate
        + ",newNames="
        + newColumnNames
        + ")";
