...
public class NDArrayWritable extends ArrayWritable implements WritableComparable {
  ...
  public void readFields(DataInput in) throws IOException {
    ...
    if (header != NDARRAY_SER_VERSION_HEADER && header != NDARRAY_SER_VERSION_HEADER_NULL) {
      throw new IllegalStateException(
          "Unexpected NDArrayWritable version header - stream corrupt?");
    }
  ...
  public int hashCode() {
    ...
    // Hashcode needs to be invariant to array order - otherwise, equal arrays can have different
    // hash codes
  ...
  public int compareTo(@NonNull Object o) {
    ...
    // Conventions used here for ordering NDArrays: x.compareTo(y): -ve if x < y, 0 if x == y, +ve
    // if x > y
    // Null first
