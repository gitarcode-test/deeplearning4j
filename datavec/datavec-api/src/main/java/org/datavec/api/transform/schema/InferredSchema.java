...
public class InferredSchema {
  ...
  public InferredSchema(
      String pathToCsv, DataType defaultType, char delimiter, char quote, char escape) {
    this.pathToCsv = pathToCsv;
    this.defaultType = defaultType;
  ...
  public Schema build() throws IOException {
    ...
    if (headers.size() != samples.size())
      throw new IllegalStateException(
          "CSV headers length does not match number of sample columns. Please check that your CSV"
              + " is valid, or check the delimiter used to parse the CSV.");
  ...
  private static Schema.Builder addOn(
      Schema.Builder builder, String columnName, DataType columnType) {
    switch (columnType) {
      case DOUBLE:
