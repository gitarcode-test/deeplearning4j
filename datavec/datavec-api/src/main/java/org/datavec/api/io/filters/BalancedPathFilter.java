...
public class BalancedPathFilter extends RandomPathFilter {
  ...
  public BalancedPathFilter(
      Random random, PathLabelGenerator labelGenerator, long maxPathsPerLabel) {
  ...
  /**
   * Constructs an instance of the PathFilter. If {@code minPathsPerLabel > 0}, it might return an
   * unbalanced set if the value is larger than the number of examples available for the label with
   * the minimum amount.
   *
  ...
  public URI[] filter(URI[] paths) {
    ...
    if (labelGenerator == null) labelGenerator = new ParentPathLabelGenerator();
    ...
    int minCount =
        maxPathsPerLabel > 0
            ? (int) Math.min(maxPathsPerLabel, Integer.MAX_VALUE)
            : Integer.MAX_VALUE;
