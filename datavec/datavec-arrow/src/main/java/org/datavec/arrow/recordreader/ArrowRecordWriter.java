...
public class ArrowRecordWriter implements RecordWriter {
  ...
  @Override
  public void initialize(Configuration configuration, InputSplit split, Partitioner partitioner)
      throws Exception {
  ...
  public PartitionMetaData writeBatch(List<List<Writable>> batch) throws IOException {
    ...
    if (batch instanceof ArrowWritableRecordBatch) {
      ...
      ArrowConverter.writeRecordBatchTo(
          arrowWritableRecordBatch, schema, partitioner.currentOutputStream());
    } else {
