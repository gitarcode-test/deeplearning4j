...
public abstract class BaseImageRecordReader extends BaseRecordReader {
  ...
  @Getter @Setter protected boolean logLabelCountOnInit = true;
  @Getter @Setter protected boolean nchw_channels_first = true;
  ...
  public BaseImageRecordReader(
      long height, long width, long channels, PathLabelGenerator labelGenerator) {
  ...
  protected BaseImageRecordReader(
      long height,
      long width,
      long channels,
      PathLabelGenerator labelGenerator,
  ...
  protected boolean containsFormat(String format) {
    for (String format2 : imageLoader.getAllowedFormats())
      if (format.endsWith("." + format2)) return true;
    return false;
  }
  ...
  public void initialize(InputSplit split) throws IOException {
    ...
    if (locations != null && locations.length >= 1) {
      if (appendLabel && labelGenerator != null && labelGenerator.inferLabelClasses()) {
        ...
        if (logLabelCountOnInit) {
          log.info(
              "ImageRecordReader: {} label classes inferred using label generator {}",
              labelsSet.size(),
              labelGenerator.getClass().getSimpleName());
        }
      }
      iter =
          new FileFromPathIterator(
              inputSplit
                  .locationsPathIterator()); // This handles randomization internally if necessary
    } else throw new IllegalArgumentException("No path locations found in the split.");
    ...
    // To ensure consistent order for label assignment (irrespective of file iteration order), we
    // want to sort the list of labels
    ...
    if (inputSplit instanceof InputStreamInputSplit) {
      ...
      try {
        NDArrayWritable ndArrayWritable =
            new NDArrayWritable(imageLoader.asMatrix(inputStreamInputSplit.getIs()));
  ...
  public List<List<Writable>> next(int num) {
    ...
    while (cnt < num && iter.hasNext()) {
      ...
      if (appendLabel || writeLabel) {
        ...
        if (labelMultiGenerator != null) {
          if (multiGenLabels == null) multiGenLabels = new ArrayList<>();

          multiGenLabels.add(labelMultiGenerator.getLabels(currentFile.getPath()));
        } else {
          if (labelGenerator.inferLabelClasses()) {
            if (currLabels == null) currLabels = new ArrayList<>();
            currLabels.add(labels.indexOf(getLabel(currentFile.getPath())));
          } else {
            if (currLabelsWritable == null) currLabelsWritable = new ArrayList<>();
    ...
    for (int i = 0; i < cnt; i++) {
      try {
        ((NativeImageLoader) imageLoader)
            .asMatrixView(currBatch.get(i), features.tensorAlongDimension(i, 1, 2, 3));
      } catch (Exception e) {
  ...
  public List<Writable> record(URI uri, DataInputStream dataInputStream) throws IOException {
    ...
    if (!nchw_channels_first) array = array.permute(0, 2, 3, 1);
  ...
  public Record nextRecord() {
    ...
    return new org.datavec.api.records.impl.Record(
        list, new RecordMetaDataURI(uri, BaseImageRecordReader.class));
  }
