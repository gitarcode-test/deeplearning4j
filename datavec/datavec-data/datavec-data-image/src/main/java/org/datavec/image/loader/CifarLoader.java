...
public class CifarLoader extends NativeImageLoader implements Serializable {
  ...
  private static final int numToConvertDS =
      10000; // Each file is 10000 images, limiting for file preprocess load
  ...
  public CifarLoader(
      int height, int width, int channels, boolean train, boolean useSpecialPreProcessCifar) {
  ...
  public CifarLoader(
    ...
    testFilesSerialized =
        FilenameUtils.concat(this.fullDir.toString(), "cifar_test_serialized.ser");
    ...
    try {
      ...
      trainInputStream =
          new SequenceInputStream(
              new FileInputStream(trainIter.next()), new FileInputStream(trainIter.next()));
      while (trainIter.hasNext()) {
        File nextFile = trainIter.next();
        if (!TESTFILENAME.equals(nextFile.getName()))
          trainInputStream =
              new SequenceInputStream(trainInputStream, new FileInputStream(nextFile));
      }
  ...
  /**
   * Preprocess and store cifar based on successful Torch approach by Sergey Zagoruyko Reference: <a
   * href="https://github.com/szagoruyko/cifar.torch">https://github.com/szagoruyko/cifar.torch</a>
   */
  public Mat convertCifar(Mat orgImage) {
    numExamples++;
    Mat resImage = new Mat();
    OpenCVFrameConverter.ToMat converter = new OpenCVFrameConverter.ToMat();
    //        ImageTransform yuvTransform = new ColorConversionTransform(new Random(seed),
    // COLOR_BGR2Luv);
    //        ImageTransform histEqualization = new EqualizeHistTransform(new Random(seed),
    // COLOR_BGR2Luv);
    ...
    if (converter != null) {
      ...
      // TODO determine if need to normalize y before transform - opencv docs rec but currently
      // doing after
      writable =
          yuvTransform.transform(
              writable); // Converts to chrome color to help emphasize image objects
      writable =
          histEqualization.transform(
              writable); // Normalizes values to further clarify object of interest
  ...
  /**
   * Normalize and store cifar based on successful Torch approach by Sergey Zagoruyko Reference: <a
   * href="https://github.com/szagoruyko/cifar.torch">https://github.com/szagoruyko/cifar.torch</a>
   */
  ...
  public Pair<INDArray, Mat> convertMat(byte[] byteFeature) {
    INDArray label =
        FeatureUtil.toOutcomeVector(
            byteFeature[0], NUM_LABELS); // first value in the 3073 byte array
    ...
    for (DataSet data : result) {
      try {
        ...
        throw new IllegalStateException(
            "The number of channels must be 3 to special preProcess Cifar with.");
      }
    }
  ...
  public double varManual(INDArray x, double mean) {
    ...
    double accum =
        Nd4j.getExecutioner().execAndReturn(new Sum(squared)).getFinalResult().doubleValue();
  ...
  public DataSet next(int batchSize, int exampleNum) {
    ...
    if (cifarProcessedFilesExists() && useSpecialPreProcessCifar) {
      ...
      // TODO loading full train dataset when using cuda causes memory error - find way to load into
      // list off gpu
