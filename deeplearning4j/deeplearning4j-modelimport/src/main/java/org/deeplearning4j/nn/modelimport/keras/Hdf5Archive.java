...
public class Hdf5Archive implements Closeable {
  ...
  /**
   * HDF5 library is not thread safe - possible to crash if multiple reads etc are performed
   * concurrently in multiple threads. This object is used for locking read etc activity using
   * synchronized blocks
   */
   ...
   */
  public INDArray readDataSet(String datasetName, String... groups)
      throws UnsupportedKerasConfigurationException {
    synchronized (Hdf5Archive.LOCK_OBJECT) {
      if (groups.length == 0) return readDataSet(this.file, datasetName);
  ...
  public List<String> getDataSets(String... groups) {
    synchronized (Hdf5Archive.LOCK_OBJECT) {
      if (groups.length == 0) return getObjects(this.file, H5O_TYPE_DATASET);
      Group[] groupArray = openGroups(groups);
  ...
  public List<String> getGroups(String... groups) {
    synchronized (Hdf5Archive.LOCK_OBJECT) {
      if (groups.length == 0) return getObjects(this.file, H5O_TYPE_GROUP);
      Group[] groupArray = openGroups(groups);
    ...
    synchronized (Hdf5Archive.LOCK_OBJECT) {
      ...
      switch (nbDims) {
        ...
        case 3:
          ...
          for (int i1 = 0; i1 < dims[0]; i1++)
            for (int i2 = 0; i2 < dims[1]; i2++)
              for (int i3 = 0; i3 < dims[2]; i3++) data.putScalar(i1, i2, i3, dataBuffer[j++]);
          break;
        case 2: /* Dense and Recurrent weights */
          ...
          j = 0;
          for (int i1 = 0; i1 < dims[0]; i1++)
            for (int i2 = 0; i2 < dims[1]; i2++) data.putScalar(i1, i2, dataBuffer[j++]);
          break;
        case 1: /* Bias */
        ...
        default:
          throw new UnsupportedKerasConfigurationException(
              "Cannot import weights with rank " + nbDims);
      }
      space.deallocate();
      dataset.deallocate();
      return data;
    ...
    synchronized (Hdf5Archive.LOCK_OBJECT) {
      ...
      while (true) {
        ...
        try {
          ...
          // OK - we don't know how long the buffer needs to be, so we'll try again with larger
          // buffer
        }

        if (currBufferLength == MAX_BUFFER_SIZE_BYTES) {
          throw new UnsupportedKerasConfigurationException(
              "Could not read abnormally long HDF5 attribute: size exceeds "
                  + currBufferLength
                  + " bytes");
        } else {
