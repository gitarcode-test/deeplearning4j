...
public class KerasLayer {
  ...
  @Getter protected String className; // Keras layer class name
  @Getter protected String layerName; // Keras layer name
  @Getter protected int[] inputShape; // Keras layer input shape
  @Getter protected DimOrder dimOrder; // Keras layer backend dimension order
  @Getter protected List<String> inboundLayerNames; // List of inbound layers
  @Getter protected List<String> outboundLayerNames; // List of outbound layers
  ...
  @Getter protected KerasLayerConfiguration conf;
  @Getter protected Map<String, Object> originalLayerConfig;
  ...
  /**
   * Constructor. "enforceTrainingConfig" parameter controls whether layer is built for training.
   * This controls behavior of certain exceptions. In training mode, passing an unsupported
   * regularizer will generate an error. In non-training mode, it generates only a warning.
   *
   * @param layerConfig dictionary containing Keras layer configuration
   * @param enforceTrainingConfig whether layer should be built for training (controls certain
   *     exceptions)
   */
  protected KerasLayer(Map<String, Object> layerConfig, boolean enforceTrainingConfig)
    ...
    this.weightL1Regularization =
        KerasRegularizerUtils.getWeightRegularizerFromConfig(
            layerConfig,
            conf,
            conf.getLAYER_FIELD_W_REGULARIZER(),
            conf.getREGULARIZATION_TYPE_L1());
    this.weightL2Regularization =
        KerasRegularizerUtils.getWeightRegularizerFromConfig(
            layerConfig,
            conf,
            conf.getLAYER_FIELD_W_REGULARIZER(),
            conf.getREGULARIZATION_TYPE_L2());
   ...
   */
  public static void registerLambdaLayer(
      String lambdaLayerName, SameDiffLambdaLayer sameDiffLambdaLayer) {
   ...
   */
  public static void registerCustomLayer(
      String layerName, Class<? extends KerasLayer> configClass) {
  ...
  public int[] getInputShape() {
    if (this.inputShape == null) return null;
  ...
  public List<String> getInboundLayerNames() {
    if (this.inboundLayerNames == null) this.inboundLayerNames = new ArrayList<>();
  ...
  public boolean usesRegularization() {
    return (this.weightL1Regularization > 0.0
        || this.weightL2Regularization > 0.0
        || this.dropout < 1.0);
  }
   ...
   */
  public void copyWeightsToLayer(org.deeplearning4j.nn.api.Layer layer)
    ...
    if (this.getNumParams() > 0) {
      ...
      String msg =
          "Error when attempting to copy weights from Keras layer "
              + kerasLayerName
              + " to DL4J layer "
              + dl4jLayerName;
        ...
        try {
          ...
          if (!weights.containsKey(paramName)) {
            throw new IllegalArgumentException(
                "No weights found for parameter " + paramName + " in layer " + kerasLayerName);
          }
          ...
          throw new InvalidKerasConfigurationException(
              e.getMessage()
                  + "\nTried to set weights for layer with name "
                  + this.getLayerName()
                  + ", of "
                  + layer.conf().getLayer().getClass()
                  + ".\n"
                  + "Failed to set weights for parameter "
                  + paramName
                  + "\n"
                  + "Expected shape for this parameter: "
                  + layer.getParam(paramName).shapeInfoToString()
                  + ", \ngot: "
                  + this.weights.get(paramName).shapeInfoToString());
        }
      }
    }
  }
  ...
  /**
   * Some DL4J layers need explicit specification of number of inputs, which Keras does infer. This
   * method searches through previous layers until a FeedForwardLayer is found. These layers have
   * nOut values that subsequently correspond to the nIn value of this layer.
   *
   * @param previousLayers
   * @return
   * @throws UnsupportedKerasConfigurationException
   */
  protected long getNInFromConfig(Map<String, ? extends KerasLayer> previousLayers)
      throws UnsupportedKerasConfigurationException {
    ...
    throw new UnsupportedKerasConfigurationException(
        "Could not determine number of input channels for" + "depthwise convolution.");
  }
  ...
  public InputPreProcessor getInputPreprocessor(InputType... inputType)
    ...
    if (this.layer != null) {
      ...
      } else preprocessor = this.layer.getPreProcessorForInputType(inputType[0]);
    }
    return preprocessor;
  }
  ...
  /**
   * Indicates whether this layer a valid inbound layer. Currently, only (known) DL4J Layers and
   * inputs are valid inbound layers. "Preprocessor" layers (reshaping, merging, etc.) are replaced
   * by their own inbound layers.
   *
