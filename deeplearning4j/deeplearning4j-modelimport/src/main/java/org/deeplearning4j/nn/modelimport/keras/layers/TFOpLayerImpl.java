...
public class TFOpLayerImpl extends AbstractLayer<TFOpLayer> {
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    throw new RuntimeException(
        "Backprop through TFOpLayerImpl is not supported yet."
  ...
  /**
   * Converts a Map representation of Nodedef to a singleton TF Graph and instantiates a
   * GraphRunner.
   */
    ...
    try {
      ...
      for (int i = 0; i < nodeDef.getInputCount(); i++) {
        ...
        if (constants.containsKey(String.valueOf(i))) {
          constArrays.put(
              nodeDef.getInput(i), Nd4j.create((List<Number>) constants.get(String.valueOf(i))));
        } else {
      ...
      ServiceLoader<TFGraphRunnerService> sl =
          DL4JClassLoading.loadService(TFGraphRunnerService.class);
      Iterator<TFGraphRunnerService> iter = sl.iterator();
      if (!iter.hasNext()) {
        throw new RuntimeException(
            "The model contains a Tensorflow Op, which requires the nd4j-tensorflow dependency to"
                + " execute.");
      }

      this.graphRunnerService =
          iter.next().init(allInputNames, outputNames, graphBytes, constArrays, inputDataTypes);
    } catch (Exception e) {
