...
public class KerasBatchNormalization extends KerasLayer {
   ...
   */
  public KerasBatchNormalization(Integer kerasVersion)
   ...
   * @throws UnsupportedKerasConfigurationException Unsupported Keras config
   */
  public KerasBatchNormalization(
    ...
    if (inboundNodes != null && !inboundNodes.isEmpty()) {
      ...
      DimOrder dimOrderFromConfig =
          KerasLayerUtils.getDimOrderFromConfig(
              kerasLayer.getOriginalLayerConfig(), kerasLayer.getConf());
    ...
    if (batchNormMode != 0)
      throw new UnsupportedKerasConfigurationException(
          "Unsupported batch normalization mode "
              + batchNormMode
    ...
    if (!(batchNormAxis == 3 || batchNormAxis == -1))
      OneTimeLogger.warn(
          log,
          "Warning: batch normalization axis "
              + batchNormAxis
    ...
    LayerConstraint betaConstraint =
        KerasConstraintUtils.getConstraintsFromConfig(
            layerConfig,
            conf.getLAYER_FIELD_BATCHNORMALIZATION_BETA_CONSTRAINT(),
            conf,
            kerasMajorVersion);
    LayerConstraint gammaConstraint =
        KerasConstraintUtils.getConstraintsFromConfig(
            layerConfig,
    ...
    if (betaConstraint != null) builder.constrainBeta(betaConstraint);
    if (gammaConstraint != null) builder.constrainGamma(gammaConstraint);
  ...
  public void setWeights(Map<String, INDArray> weights) throws InvalidKerasConfigurationException {
    ...
    if (center) {
      if (weights.containsKey(PARAM_NAME_BETA))
        ...
        throw new InvalidKerasConfigurationException(
            "Parameter " + PARAM_NAME_BETA + " does not exist in weights");
    } else {
    ...
    if (scale) {
      ...
      INDArray dummyGamma =
          weights.containsKey(PARAM_NAME_GAMMA)
    ...
    if (weights.containsKey(conf.getLAYER_FIELD_BATCHNORMALIZATION_MOVING_MEAN()))
      this.weights.put(
          ...
          weights.get(conf.getLAYER_FIELD_BATCHNORMALIZATION_MOVING_MEAN()));
    else
      throw new InvalidKerasConfigurationException(
          "Parameter "
              + conf.getLAYER_FIELD_BATCHNORMALIZATION_MOVING_MEAN()
    ...
    if (weights.containsKey(conf.getLAYER_FIELD_BATCHNORMALIZATION_MOVING_VARIANCE()))
      this.weights.put(
          ...
          weights.get(conf.getLAYER_FIELD_BATCHNORMALIZATION_MOVING_VARIANCE()));
    else
      throw new InvalidKerasConfigurationException(
          "Parameter "
              + conf.getLAYER_FIELD_BATCHNORMALIZATION_MOVING_VARIANCE()
              + " does not exist in weights");
    if (weights.size() > 4) {
      ...
      log.warn(
          "Attempting to set weights for unknown parameters: "
   ...
   * @throws InvalidKerasConfigurationException Invalid Keras config
   */
  private double getEpsFromConfig(Map<String, Object> layerConfig)
   ...
   * @throws InvalidKerasConfigurationException Invalid Keras config
   */
  private double getMomentumFromConfig(Map<String, Object> layerConfig)
      throws InvalidKerasConfigurationException {
    Map<String, Object> innerConfig =
        KerasLayerUtils.getInnerLayerConfigFromConfig(layerConfig, conf);
   ...
   * @throws InvalidKerasConfigurationException Invalid Keras config
   */
  private void getGammaRegularizerFromConfig(
  ...
  private int getBatchNormMode(Map<String, Object> layerConfig, boolean enforceTrainingConfig)
    ...
    if (this.kerasMajorVersion == 1) batchNormMode = (int) innerConfig.get(LAYER_FIELD_MODE);
    switch (batchNormMode) {
      case LAYER_BATCHNORM_MODE_1:
        throw new UnsupportedKerasConfigurationException(
            "Keras BatchNormalization mode "
                + LAYER_BATCHNORM_MODE_1
                + " (sample-wise) not supported");
