  ...
  private void receiveData(RoutingContext rc) {
    if (!enabled.get()) {
      rc.response()
          ...
          .end(
              "UI server remote listening is currently disabled. Use"
    ...
    if (statsStorage == null) {
      rc.response()
          ...
          .end(
              "UI Server remote listener: no StatsStorage instance is set/available to store"
                  + " results");
      return;
    }
    ...
    if (type == null || dataClass == null || data == null) {
      log.warn(
          "Received incorrectly formatted data from remote listener (has type = "
              + (type != null)
              + ", has data class = "
              + (dataClass != null)
              + ", has data = "
  ...
  private StorageMetaData getMetaData(String dataClass, String content) {
    ...
    try {
      ...
      if (StorageMetaData.class.isAssignableFrom(clazz)) {
        meta = clazz.asSubclass(StorageMetaData.class).getDeclaredConstructor().newInstance();
      } else {
        log.warn(
            "Skipping invalid remote data: class {} in not an instance of {}",
            dataClass,
  ...
  private Persistable getPersistable(String dataClass, String content) {
    ...
    try {
      ...
      if (Persistable.class.isAssignableFrom(clazz)) {
        persistable = clazz.asSubclass(Persistable.class).getDeclaredConstructor().newInstance();
      } else {
        log.warn(
            "Skipping invalid remote data: class {} in not an instance of {}",
            dataClass,
