...
public class VertxUIServer extends AbstractVerticle implements UIServer {
  ...
  @Getter private static VertxUIServer instance;

  @Getter private static AtomicBoolean multiSession = new AtomicBoolean(false);
  @Getter @Setter private static Function<String, StatsStorage> statsStorageProvider;

  private static Integer instancePort;
  @Getter private static Thread shutdownHook;

  /**
   * Get (and, initialize if necessary) the UI server. This synchronous function will wait until the
   * server started.
   *
   * @param port TCP socket port for {@link HttpServer} to listen
   * @param multiSession in multi-session mode, multiple training sessions can be visualized in
   *     separate browser tabs. <br>
   *     URL path will include session ID as a parameter, i.e.: /train becomes /train/:sessionId
   * @param statsStorageProvider function that returns a StatsStorage containing the given session
   *     ID. <br>
   *     Use this to auto-attach StatsStorage if an unknown session ID is passed as URL path
   *     parameter in multi-session mode, or leave it {@code null}.
   * @return UI instance for this JVM
   * @throws DL4JException if UI server failed to start; if the instance has already started in a
   *     different mode (multi/single-session); if interrupted while waiting for completion
   */
  public static VertxUIServer getInstance(
      Integer port, boolean multiSession, Function<String, StatsStorage> statsStorageProvider)
      throws DL4JException {
    return getInstance(port, multiSession, statsStorageProvider, null);
  }

  /**
   * Get (and, initialize if necessary) the UI server. This function will wait until the server
   * started (synchronous way), or pass the given callback to handle success or failure
   * (asynchronous way).
   *
   * @param port TCP socket port for {@link HttpServer} to listen
   * @param multiSession in multi-session mode, multiple training sessions can be visualized in
   ...
   *     Use this to auto-attach StatsStorage if an unknown session ID is passed as URL path
   *     parameter in multi-session mode, or leave it {@code null}.
   * @param startCallback asynchronous deployment handler callback that will be notify of success or
   *     failure. If {@code null} given, then this method will wait until deployment is complete. If
   *     the deployment is successful the result will contain a String representing the unique
  ...
  public static VertxUIServer getInstance(
    ...
    if (instance == null || instance.isStopped()) {
      ...
      if (startCallback != null) {
        // Launch UI server verticle and pass asynchronous callback that will be notified of
        // completion
      ...
      if (multiSession && !instance.isMultiSession()) {
        throw new DL4JException(
            "Cannot return multi-session instance."
                + " UIServer has already started in single-session mode at "
                + instance.getAddress()
                + " You may stop the UI server instance, and start a new one.");
      } else if (!multiSession && instance.isMultiSession()) {
        throw new DL4JException(
            "Cannot return single-session instance."
                + " UIServer has already started in multi-session mode at "
                + instance.getAddress()
                + " You may stop the UI server instance, and start a new one.");
      }
    }
   ...
   *
   * @throws DL4JException if UI server failed to start; if interrupted while waiting for completion
   */
  ...
  /**
   * Deploy (start) {@link VertxUIServer}, and pass callback to handle successful or failed
   * completion of deployment.
   *
   * @param startCallback promise that will handle success or failure of deployment. If the
   *     deployment is successful the result will contain a String representing the unique
   *     deployment ID of the deployment.
   */
            ...
            () -> {
              if (VertxUIServer.instance != null && !VertxUIServer.instance.isStopped()) {
                ...
                try {
                  ...
                  log.error(
                      "Interrupted stopping of Deeplearning4j UI server in shutdown hook.", e);
                }
              }
            });
  ...
  /**
   * Loader that attaches {@code StatsStorage} provided by {@code #statsStorageProvider} for the
   * given session ID
   */
  @Getter private Function<String, Boolean> statsStorageLoader;
  ...
  /**
   * Auto-attach StatsStorage if an unknown session ID is passed as URL path parameter in
   * multi-session mode
   *
   * @param statsStorageProvider function that returns a StatsStorage containing the given session
   *     ID
   */
  public void autoAttachStatsStorageBySessionId(
      Function<String, StatsStorage> statsStorageProvider) {
    if (statsStorageProvider != null) {
      this.statsStorageLoader =
          (sessionId) -> {
            log.info(
                "Loading StatsStorage via StatsStorageProvider for session ID ("
                    + sessionId
                    + ").");
            ...
            if (statsStorage != null) {
              ...
              log.info(
                  "Failed to load StatsStorage via StatsStorageProvider for session ID. "
                      + "Session ID ("
                      + sessionId
                      + ") does not exist in StatsStorage.");
              return false;
            } else {
              log.info(
                  "Failed to load StatsStorage via StatsStorageProvider for session ID ("
  ...
  public void start(Promise<Void> startCallback) throws Exception {
    ...
    File uploadDir =
        new File(System.getProperty("java.io.tmpdir"), "DL4JUI_" + System.currentTimeMillis());
    uploadDir.mkdirs();
    Router r = Router.router(vertx);
    r.route()
        .handler(
            BodyHandler
                .create() // NOTE: Setting this is required to receive request body content at all
        ...
        switch (route.getHttpMethod()) {
          case GET:
            r.get(route.getRoute())
                .handler(
                    rc ->
                        route.getConsumer().accept(extractArgsFromRoute(route.getRoute(), rc), rc));
            break;
          case PUT:
            r.put(route.getRoute())
                .handler(
                    rc ->
                        route.getConsumer().accept(extractArgsFromRoute(route.getRoute(), rc), rc));
            break;
          case POST:
            r.post(route.getRoute())
                .handler(
                    rc ->
                        route.getConsumer().accept(extractArgsFromRoute(route.getRoute(), rc), rc));
            break;
          default:
            throw new IllegalStateException(
                "Unknown or not supported HTTP method: " + route.getHttpMethod());
        }
      }
    ...
    if (port < 0 || port > 0xFFFF) {
      throw new IllegalStateException(
          "Valid port range is 0 <= port <= 65535. The given port was " + port);
    }
                  ...
                  if (result.succeeded()) {
                    ...
                    startCallback.fail(
                        new RuntimeException(
                            "Deeplearning4j UI server failed to listen on port "
                                + server.actualPort(),
                            result.cause()));
                  }
                });
  ...
  public void stopAsync(Promise<Void> stopCallback) {
    /**
     * Stop Vertx instance and release any resources held by it. Pass promise to {@link
     * #stop(Promise)}.
     */
  ...
  public void enableRemoteListener() {
    if (remoteReceiverModule == null) remoteReceiverModule = new RemoteReceiverModule();
  ...
  private static class CLIParams {
    @Parameter(
        names = {"-r", "--enableRemote"},
        description = "Whether to enable remote or not",
        arity = 1)
    private boolean cliEnableRemote;

    @Parameter(
        names = {"-p", "--uiPort"},
        description = "Custom HTTP port for UI",
        arity = 1)
    private int cliPort = DEFAULT_UI_PORT;

    @Parameter(
        names = {"-f", "--customStatsFile"},
        description = "Path to create custom stats file (remote only)",
        arity = 1)
    private String cliCustomStatsFile;

    @Parameter(
        names = {"-m", "--multiSession"},
        description = "Whether to enable multiple separate browser sessions or not",
        arity = 1)
