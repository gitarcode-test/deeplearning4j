...
public class MapDBStatsStorage extends BaseCollectionStatsStorage {
  ...
  private MapDBStatsStorage(Builder builder) {
    ...
    if (f == null) {
      ...
      db =
          DBMaker.fileDB(f)
              .closeOnJvmShutdown()
              .transactionEnable() // Default to Write Ahead Log - lower performance, but has crash
                                   // protection
              .make();
    }
  ...
  @Override
  protected Map<Long, Persistable> getUpdateMap(
      String sessionID, String typeID, String workerID, boolean createIfRequired) {
  ...
  public void putStaticInfo(Persistable staticInfo) {
    ...
    SessionTypeWorkerId id =
        new SessionTypeWorkerId(
            staticInfo.getSessionID(), staticInfo.getTypeID(), staticInfo.getWorkerID());
    ...
    if (!listeners.isEmpty())
      sse =
          new StatsStorageEvent(
              ...
              staticInfo.getSessionID(),
              staticInfo.getTypeID(),
              staticInfo.getWorkerID(),
              staticInfo.getTimeStamp());
  ...
  public void putStorageMetaData(StorageMetaData storageMetaData) {
    ...
    SessionTypeId id =
        new SessionTypeId(storageMetaData.getSessionID(), storageMetaData.getTypeID());
    ...
    if (!listeners.isEmpty())
      sse =
          new StatsStorageEvent(
              ...
              storageMetaData.getSessionID(),
              storageMetaData.getTypeID(),
              storageMetaData.getWorkerID(),
  ...
  private static class SessionTypeWorkerIdSerializer implements Serializer<SessionTypeWorkerId> {
    @Override
    public void serialize(@NonNull DataOutput2 out, @NonNull SessionTypeWorkerId value)
        throws IOException {
    ...
    @Override
    public SessionTypeWorkerId deserialize(@NonNull DataInput2 in, int available)
        throws IOException {
      try {
  ...
  private static class SessionTypeIdSerializer implements Serializer<SessionTypeId> {
    @Override
    public void serialize(@NonNull DataOutput2 out, @NonNull SessionTypeId value)
        throws IOException {
      ObjectOutputStream out2 = new ObjectOutputStream(out);
      out2.writeObject(value);
      out2.flush();
