...
public class SbeStatsInitializationReport implements StatsInitializationReport, AgronaPersistable {
  ...
  @Override
  public void reportSoftwareInfo(
      String arch,
      String osName,
      String jvmName,
      String jvmVersion,
  ...
  public int encodingLengthBytes() {
    ...
    // (c) Group 1: Hardware devices (GPUs) max memory: 4 bytes header + nEntries * 8 (int64) +
    // nEntries * variable length Strings (header + content)  = 4 + 8*n + content
    // (d) Group 2: Software device info: 4 bytes header + 2x variable length Strings for each
    // (d) Group 3: Parameter names: 4 bytes header + nEntries * variable length strings (header +
    // content) = 4 + content
    // (e) Variable length fields: 15 String length fields. Size: 4 bytes header, plus content. 60
    // bytes header
    ...
    int bufferSize =
        8
            ...
            + 4
            + 4
            + 60; // header + fixed values + group headers + variable length headers

    // For variable length field lengths: easist way is simply to convert to UTF-8
    // Of course, it is possible to calculate it first - but we might as well convert (1 pass),
    // rather than count then convert (2 passes)
    ...
    if (hasHardwareInfo) {
      ...
      bufferSize +=
          hwNumDevices
              * 8; // fixed content in group: int64 -> 8 bytes. Encode an entry, even if
                   // hwDeviceTotalMemory is null
      bufferSize +=
          hwNumDevices * 4; // uint32: 4 bytes per entry for var length header...; as above
    ...
    if (hasModelInfo) {
      ...
      bufferSize +=
          (bModelParamNames == null
              ? 0
              : bModelParamNames.length * 4); // uint32: 4 bytes per entry for var length header...
    }
  ...
  public void encode(MutableDirectBuffer buffer) {
    ...
    for (int i = 0; i < nHWDeviceStats; i++) {
      long maxMem =
          hwDeviceTotalMemory == null || hwDeviceTotalMemory.length <= i
              ? 0
              : hwDeviceTotalMemory[i];
      byte[] descr =
          bhwDeviceDescription == null || bhwDeviceDescription.length <= i
              ? SbeUtil.EMPTY_BYTES
    ...
    int numEnvValues =
        (hasSoftwareInfo && swEnvironmentInfo != null ? swEnvironmentInfo.size() : 0);
    ...
    if (numEnvValues > 0) {
      ...
      for (byte[][] entryBytes : mapAsBytes) {
        swEnv
            .next()
            .putEnvKey(entryBytes[0], 0, entryBytes[0].length)
    ...
    // In the case of !hasSoftwareInfo: these will all be empty byte arrays... still need to encode
    // them (for 0 length) however
  ...
  public void decode(DirectBuffer buffer) {
    ...
    // TODO: in general, we should check the header, version, schema etc. But we don't have any
    // other versions yet.
