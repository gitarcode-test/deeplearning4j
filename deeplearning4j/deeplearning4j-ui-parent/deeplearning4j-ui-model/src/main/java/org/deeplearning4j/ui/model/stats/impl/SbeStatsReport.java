...
public class SbeStatsReport implements StatsReport, AgronaPersistable {
  ...
  // Store in serialized form; deserialize iff required. Might save us some class not found (or,
  // version) errors, if
  ...
  public void reportDataSetMetaData(List<Serializable> dataSetMetaData, Class<?> metaDataClass) {
    reportDataSetMetaData(
        dataSetMetaData, (metaDataClass == null ? null : metaDataClass.getName()));
  }
  ...
  public List<Serializable> getDataSetMetaData() {
    if (dataSetMetaData == null || dataSetMetaData.isEmpty()) return null;
  ...
  private static void appendOrDefault(
      UpdateEncoder.PerParameterStatsEncoder.SummaryStatEncoder sse,
      String param,
      StatsType statsType,
  ...
  private static StatsType translate(org.deeplearning4j.ui.model.stats.sbe.StatsType statsType) {
    switch (statsType) {
      case Parameters:
  ...
  private static org.deeplearning4j.ui.model.stats.sbe.StatsType translate(StatsType statsType) {
    switch (statsType) {
      case Parameters:
  ...
  private static SummaryType translate(
      org.deeplearning4j.ui.model.stats.sbe.SummaryType summaryType) {
    switch (summaryType) {
      case Mean:
  ...
  public int encodingLengthBytes() {
    ...
    // Variable length String fields: 4 - session/type/worker IDs and metadata -> 4*4=16 bytes
    // header, plus content
    ...
    if (!memoryUsePresent) {
      ...
      memoryUseCount =
          4
              + (deviceCurrentBytes == null ? 0 : deviceCurrentBytes.length)
              + (deviceMaxBytes == null ? 0 : deviceMaxBytes.length);
    }
    bufferSize +=
        4
            + 9
                * memoryUseCount; // Group header: 4 bytes (always present); Each entry in group -
                                  // 1x MemoryType (uint8) + 1x int64 -> 1+8 = 9 bytes
    ...
    bufferSize +=
        4
            + (performanceStatsPresent
                ? 32
                : 0); // Group header: 4 bytes (always present); Only 1 group: 3xint64 + 2xfloat =
                      // 32 bytes
    ...
    if (gcStats != null && !gcStats.isEmpty()) {
      ...
      for (int i = 0; i < gcStats.size(); i++) {
        ...
        bufferSize +=
            12; // Fixed per group entry: 2x int32 -> 8 bytes PLUS the header for the variable
                // length GC name: another 4 bytes
    ...
    bufferSize +=
        nEntries
            * 12; // Each parameter/layer entry: has learning rate -> float -> 4 bytes PLUS headers
                  // for 2 nested groups: 2*4 = 8 each -> 12 bytes total
    bufferSize +=
        entrySize(paramNames, StatsType.Parameters, StatsType.Gradients, StatsType.Updates);
  ...
  private int entrySize(List<String> entryNames, StatsType... statsTypes) {
    ...
    for (String s : entryNames) {
      // For each parameter: MAY also have a number of summary stats (mean, stdev etc), and
      // histograms (both as nested groups)
      ...
      // Each summary stat value: StatsType (uint8), SummaryType (uint8), value (double) -> 1+1+8 =
      // 10 bytes
      ...
      // For each histogram: StatsType (uint8) + 2x double + int32 -> 1 + 2*8 + 4 = 21 bytes PLUS
      // counts group header (4 bytes) -> 25 bytes fixed per histogram
  ...
  public void encode(MutableDirectBuffer buffer) {
    ...
    ue.time(timeStamp)
        ...
        .meanMagnitudeParameters(
            meanMagnitudeValues != null && meanMagnitudeValues.containsKey(StatsType.Parameters))
        .meanMagnitudeGradients(
            meanMagnitudeValues != null && meanMagnitudeValues.containsKey(StatsType.Gradients))
        .meanMagnitudeUpdates(
            meanMagnitudeValues != null && meanMagnitudeValues.containsKey(StatsType.Updates))
        .meanMagnitudeActivations(
            meanMagnitudeValues != null && meanMagnitudeValues.containsKey(StatsType.Activations))
    ...
    if (memoryUsePresent) {
      mue.next()
          .memoryType(MemoryType.JvmCurrent)
          .memoryBytes(jvmCurrentBytes)
          .next()
          .memoryType(MemoryType.JvmMax)
          .memoryBytes(jvmMaxBytes)
          .next()
          .memoryType(MemoryType.OffHeapCurrent)
          .memoryBytes(offHeapCurrentBytes)
          .next()
          .memoryType(MemoryType.OffHeapMax)
          .memoryBytes(offHeapMaxBytes);
    ...
    UpdateEncoder.GcStatsEncoder gce =
        ue.gcStatsCount(gcStats == null || gcStats.isEmpty() ? 0 : gcStats.size());
    List<byte[]> gcStatsLabelBytes = null;
    if (gcStats != null && !gcStats.isEmpty()) {
      gcStatsLabelBytes = new ArrayList<>();
    ...
    // +++++ Per Parameter Stats +++++
    UpdateEncoder.PerParameterStatsEncoder ppe =
        ue.perParameterStatsCount(paramNames.size() + layerNames.size());
    ...
    for (String s : paramNames) {
      ...
      UpdateEncoder.PerParameterStatsEncoder.SummaryStatEncoder sse =
          ppe.summaryStatCount(summaryStatsCount);
      ...
      // Histograms
      UpdateEncoder.PerParameterStatsEncoder.HistogramsEncoder sshe =
          ppe.histogramsCount(nHistogramsThisParam);
      if (nHistogramsThisParam > 0) {
        for (StatsType statsType : st) {
          ...
          sshe =
              sshe.next().statType(translate(statsType)).minValue(min).maxValue(max).nBins(nBins);
          UpdateEncoder.PerParameterStatsEncoder.HistogramsEncoder.HistogramCountsEncoder
              histCountsEncoder = sshe.histogramCountsCount(nBins);
    ...
    for (String s : layerNames) {
      ...
      // Histograms
      UpdateEncoder.PerParameterStatsEncoder.HistogramsEncoder sshe =
          ppe.histogramsCount(nHistogramsThisLayer);
      if (nHistogramsThisLayer > 0) {
        ...
        sshe =
            sshe.next()
                .statType(translate(StatsType.Activations))
                .minValue(min)
                .maxValue(max)
                .nBins(nBins);
        UpdateEncoder.PerParameterStatsEncoder.HistogramsEncoder.HistogramCountsEncoder
  ...
  public void decode(DirectBuffer buffer) {
    ...
    for (UpdateDecoder.PerParameterStatsDecoder ppsd : ud.perParameterStats()) {
      ...
      for (UpdateDecoder.PerParameterStatsDecoder.HistogramsDecoder hd : ppsd.histograms()) {
        ...
        int i = 0;
        for (UpdateDecoder.PerParameterStatsDecoder.HistogramsDecoder.HistogramCountsDecoder hcd :
            hd.histogramCounts()) {
