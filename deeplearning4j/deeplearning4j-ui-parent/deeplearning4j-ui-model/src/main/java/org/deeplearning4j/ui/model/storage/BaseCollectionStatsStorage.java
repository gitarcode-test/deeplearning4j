...
public abstract class BaseCollectionStatsStorage implements StatsStorage {
  ...
  protected abstract Map<Long, Persistable> getUpdateMap(
      String sessionID, String typeID, String workerID, boolean createIfRequired);

  // Return any relevant storage events
  // We want to return these so they can be logged later. Can't be logged immediately, as this may
  // case a race
  // condition with whatever is receiving the events: i.e., might get the event before the contents
  // are actually
  // available in the DB
  ...
  public long[] getAllUpdateTimes(String sessionID, String typeID, String workerID) {
    ...
    for (Long l : m.keySet()) {
      ret[i++] = l;
      if (i >= ret.length)
        break; // Map "m" can in principle be modified concurrently while iterating here - resulting
               // in an array index exception
    }
    Arrays.sort(ret);
    return ret;
  }
