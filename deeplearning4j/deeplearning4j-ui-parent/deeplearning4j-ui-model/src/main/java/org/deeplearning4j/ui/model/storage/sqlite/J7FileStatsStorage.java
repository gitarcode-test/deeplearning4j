...
public class J7FileStatsStorage implements StatsStorage {
  ...
  private List<String> selectDistinct(
      String columnName,
      boolean queryMeta,
      boolean queryStatic,
      boolean queryUpdates,
    ...
    try (Statement statement = connection.createStatement()) {
      if (queryMeta) {
        queryHelper(
            statement,
            querySqlHelper(columnName, TABLE_NAME_METADATA, conditionColumn, conditionValue),
            unique);
      }
      ...
      if (queryUpdates) {
        queryHelper(
            statement,
            querySqlHelper(columnName, TABLE_NAME_UPDATES, conditionColumn, conditionValue),
            unique);
      }
    } catch (SQLException e) {
      throw new RuntimeException(e);
  ...
  private String querySqlHelper(
      String columnName, String table, String conditionColumn, String conditionValue) {
  ...
  public void putStorageMetaData(Collection<? extends StorageMetaData> collection) {
    ...
    try {
      ...
      for (StorageMetaData storageMetaData : collection) {
        ...
        // Normally we'd batch these... sqlite has an autocommit feature that messes up batching
        // with .addBatch() and .executeUpdate()
  ...
  public Persistable getStaticInfo(String sessionID, String typeID, String workerID) {
    String selectStaticSQL =
        "SELECT ObjectBytes FROM "
            + TABLE_NAME_STATIC_INFO
            + " WHERE SessionID = '"
            + sessionID
            + "' AND TypeID = '"
  ...
  public List<Persistable> getLatestUpdateAllWorkers(String sessionID, String typeID) {
    String sql =
        "SELECT workerId, MAX(Timestamp) FROM "
            + TABLE_NAME_UPDATES
            + " WHERE SessionID ='"
            + sessionID
            + "' AND "
  ...
  @Override
  public List<Persistable> getAllUpdatesAfter(
  ...
  public List<Persistable> getUpdates(
      String sessionID, String typeID, String workerID, long[] timestamps) {
    if (timestamps == null || timestamps.length == 0) {
      return Collections.emptyList();
    }

    StringBuilder sb = new StringBuilder();
    sb.append("SELECT ObjectBytes FROM ")
        .append(TABLE_NAME_UPDATES)
  ...
  protected void notifyListeners(List<StatsStorageEvent> sses) {
    if (sses == null || sses.isEmpty() || listeners.isEmpty()) return;
