...
public abstract class BaseStatsListener implements RoutingIterationListener {
  ...
  // NOTE: may have multiple models, due to multiple pretrain layers all using the same
  // StatsListener
  private List<ModelInfo> modelInfos = new ArrayList<>();

  private Map<String, Histogram> activationHistograms;
  private Map<String, Double>
      meanActivations; // TODO replace with Eclipse collections primitive maps...
  ...
  private Map<String, Double>
      meanGradients; // TODO replace with Eclipse collections primitive maps...
   ...
   *
   * @param router Where/how to store the calculated stats. For example, {@link
   *     InMemoryStatsStorage} or {@link FileStatsStorage}
   */
  ...
  public BaseStatsListener(StatsStorageRouter router, int listenerFrequency) {
    this(
        router,
        null,
        new DefaultStatsUpdateConfiguration.Builder().reportingFrequency(listenerFrequency).build(),
        null,
        null);
  }
  ...
  public abstract StorageMetaData getNewStorageMetaData(
      long initTime, String sessionID, String workerID);

  //                                                          Class<? extends
  // StatsInitializationReport> initializationReportClass,
  //                                                          Class<? extends StatsReport>
  // statsReportClass);
  // new SbeStorageMetaData(initTime, getSessionID(model), TYPE_ID, workerID,
  ...
  public void onForwardPass(Model model, Map<String, INDArray> activations) {
    int iterCount = getModelInfo(model).iterCount;
    if (calcFromActivations()
        && updateConfig.reportingFrequency() > 0
        && (iterCount == 0 || iterCount % updateConfig.reportingFrequency() == 0)) {
      if (updateConfig.collectHistograms(StatsType.Activations)) {
        activationHistograms =
            getHistograms(activations, updateConfig.numHistogramBins(StatsType.Activations));
      }
  ...
  public void onGradientCalculation(Model model) {
    ...
    if (calcFromGradients()
      ...
      if (updateConfig.collectHistograms(StatsType.Gradients)) {
        gradientHistograms =
            getHistograms(
                g.gradientForVariable(), updateConfig.numHistogramBins(StatsType.Gradients));
      }
  ...
  private boolean calcFromActivations() {
    return updateConfig.collectMean(StatsType.Activations)
        || updateConfig.collectStdev(StatsType.Activations)
  ...
  public void iterationDone(Model model, int iteration, int epoch) {
    ...
    if (updateConfig.reportingFrequency() > 1
        && (iteration == 0 || iteration % updateConfig.reportingFrequency() != 0)) {
    ...
    report.reportIDs(
        ...
        TYPE_ID,
        workerID,
        System.currentTimeMillis()); // TODO support NTP time

    // --- Performance and System Stats ---
    if (updateConfig.collectPerformanceStats()) {
      // Stats to collect: total runtime, total examples, total minibatches, iterations/second,
      // examples/second
    ...
    if (updateConfig.collectMemoryStats()) {
      ...
      report.reportMemoryUse(
          jvmTotal, jvmMax, offheapTotal, offheapMax, gpuCurrentBytes, gpuMaxBytes);
    }
        ...
        for (Layer l : ((MultiLayerNetwork) model).getLayers()) {
          ...
          for (String s : paramkeys) {
            double lr =
                conf.getLayer()
                    .getUpdaterByParam(s)
                    .getLearningRate(l.getIterationCount(), l.getEpochCount());
            if (Double.isNaN(lr)) {
              // Edge case: No-Op updater, AdaDelta etc - don't have a LR hence return NaN for
              // IUpdater.getLearningRate
              lr = 0.0;
            }
    ...
    if (updateConfig.collectHistograms(StatsType.Parameters)) {
      Map<String, Histogram> paramHistograms =
          getHistograms(
    ...
    if (updateConfig.collectHistograms(StatsType.Updates)) {
      Map<String, Histogram> updateHistograms =
          getHistograms(
              model.gradient().gradientForVariable(),
    ...
    if (updateConfig.collectMean(StatsType.Parameters)) {
      Map<String, Double> meanParams =
          calculateSummaryStats(model.paramTable(backpropParamsOnly), StatType.Mean);
    ...
    report.reportStatsCollectionDurationMS(
        (int)
            (endTime
                - currentTime)); // Amount of time required to alculate all histograms, means etc.
    ...
    if (initConfig.collectModelInfo()) {
      ...
      if (model instanceof MultiLayerNetwork) {
        ...
        throw new RuntimeException(
            "Invalid model: Expected MultiLayerNetwork or ComputationGraph. Got: "
      ...
      for (String s :
          paramMap
              .keySet()) { // Assuming sensible iteration order - LinkedHashMaps are used in MLN/CG
                           // for example
        paramNames[i++] = s;
      }

      initReport.reportModelInfo(
          model.getClass().getName(), jsonConf, paramNames, numLayers, numParams);
    }
  ...
  private boolean backpropParamsOnly(Model model) {
    // For pretrain layers (VAE, AE) we *do* want pretrain params also; for MLN and CG we only want
    // backprop params
    // as we only have backprop gradients
    return model instanceof MultiLayerNetwork || model instanceof ComputationGraph;
  }

  private static Map<String, Double> calculateSummaryStats(
      Map<String, INDArray> source, StatType statType) {
