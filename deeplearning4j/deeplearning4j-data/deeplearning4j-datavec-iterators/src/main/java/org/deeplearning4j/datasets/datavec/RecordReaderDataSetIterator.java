...
public class RecordReaderDataSetIterator implements DataSetIterator {
  ...
  @Getter protected RecordReader recordReader;
  ...
  @Getter private boolean collectMetaData = false;
  ...
  /**
   * Main constructor for classification. This will convert the input class index (at position
   * labelIndex, with integer values 0 to numPossibleLabels-1 inclusive) to the appropriate one-hot
   * output/labels representation.
   *
   ...
   * @param labelIndex Index of the label Writable (usually an IntWritable), as obtained by
   *     recordReader.next()
   * @param numPossibleLabels Number of classes (possible labels) for classification
   */
  public RecordReaderDataSetIterator(
  ...
  /**
   * Constructor for classification, where the maximum number of returned batches is limited to the
   * specified value
   *
   * @param recordReader the recordreader to use
   * @param labelIndex the index/column of the label (for classification)
   * @param numPossibleLabels the number of possible labels for classification. Not used if
   *     regression == true
   * @param maxNumBatches The maximum number of batches to return between resets. Set to -1 to
  ...
  /**
   * Main constructor for multi-label regression (i.e., regression with multiple outputs). Can also
   * be used for single output regression with labelIndexFrom == labelIndexTo
   *
   ...
   * @param regression Require regression = true. Mainly included to avoid clashing with other
   ...
   * @param labelIndexFrom the index of the label (for classification), or the first index of the
   *     labels for multi-output regression
   * @param labelIndexTo only used if regression == true. The last index <i>inclusive</i> of the
   *     multi-output regression
   * @param numPossibleLabels the number of possible labels for classification. Not used if
   *     regression == true
   * @param maxNumBatches Maximum number of batches to return
   * @param regression if true: regression. If false: classification (assume labelIndexFrom is the
   *     class it belongs to)
   */
  public RecordReaderDataSetIterator(
      RecordReader recordReader,
  ...
  private void initializeUnderlying(Record next) {
    ...
    if (recordReader.resetSupported()) {
      ...
      // Hack around the fact that we need the first record to initialize the underlying RRMDSI, but
      // can't reset
      // the original reader
      recordReader =
          new ConcatenatingRecordReader(
              new CollectionRecordReader(Collections.singletonList(next.getRecord())),
              recordReader);
    }

    RecordReaderMultiDataSetIterator.Builder builder =
        new RecordReaderMultiDataSetIterator.Builder(batchSize);
    ...
    // In general: can't assume label indices are all at the start or end (event though 99% of the
    // time they are)
    ...
    if (labelIndex >= 0 && (labelIndex == 0 || labelIndexTo == totalSize - 1)) {
      ...
      Preconditions.checkState(
          labelIndex < next.getRecord().size(),
          "Invalid label (from) index: index must be in range 0 to first record size of (0 to %s"
              + " inclusive), got %s",
          next.getRecord().size() - 1,
          labelIndex);
      Preconditions.checkState(
          labelIndexTo < next.getRecord().size(),
          "Invalid label (to) index: index must be in range 0 to first record size of (0 to %s"
              + " inclusive), got %s",
          next.getRecord().size() - 1,
          labelIndexTo);
  ...
  private DataSet mdsToDataSet(MultiDataSet mds) {
    ...
    // If labelIdx == -1 && numPossibleLabels == -1 -> no labels -> set labels array to features
    // array
   ...
   *     record reader provided to the RecordReaderDataSetIterator constructor
  ...
  public static class Builder {
    ...
    /**
     * Optional argument, usually not used. If set, can be used to limit the maximum number of
     * minibatches that will be returned (between resets). If not set, will always return as many
     * minibatches as there is data available.
     *
    ...
    /**
     * Use this for multiple output regression (1 or more output/regression targets). Note that all
     * regression targets must be contiguous (i.e., positions x to y, without gaps)
     *
     ...
     *
     * @param labelIndex Index that contains the label index. Column (indexes start from 0) be an
     *     integer value, and contain values 0 to numClasses-1
