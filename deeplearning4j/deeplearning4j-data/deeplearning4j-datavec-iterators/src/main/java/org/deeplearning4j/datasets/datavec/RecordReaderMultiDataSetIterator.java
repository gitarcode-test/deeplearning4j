...
public class RecordReaderMultiDataSetIterator implements MultiDataSetIterator, Serializable {
  ...
  /**
   * When dealing with time series data of different lengths, how should we align the input/labels
   * time series? For equal length: use EQUAL_LENGTH For sequence classification: use ALIGN_END
   */
    ...
    for (Map.Entry<String, RecordReader> entry : recordReaders.entrySet()) {
      ...
      if (!collectMetaData && rr.batchesSupported()) {
        ...
        List<List<Writable>> writables =
            new ArrayList<>(
                Math.min(num, 100000)); // Min op: in case user puts batch size >> amount of data
  ...
  // Filter out the required columns before conversion. This is to avoid trying to convert String
  // etc columns
  ...
  public MultiDataSet nextMultiDataSet(
    ...
    if (minExamples == Integer.MAX_VALUE)
      throw new RuntimeException(
          "Error occurred during data set generation: no readers?"); // Should never happen

    // In order to align data at the end (for each example individually), we need to know the length
    // of the
    ...
    Pair<INDArray[], INDArray[]> features =
        convertFeaturesOrLabels(
    ...
    MultiDataSet mds =
        new org.nd4j.linalg.dataset.MultiDataSet(
            features.getFirst(), labels.getFirst(), features.getSecond(), labels.getSecond());
    if (collectMetaData) {
  ...
  private Pair<INDArray[], INDArray[]> convertFeaturesOrLabels(
      INDArray[] featuresOrLabels,
      INDArray[] masks,
  ...
  private INDArray convertWritablesBatched(List<INDArray> list, SubsetDetails details) {
    ...
    if (arr.size(1) != 1) {
      throw new UnsupportedOperationException(
          "Cannot do conversion to one hot using batched reader: "
              + details.oneHotNumClasses
              + " output classes, but array.size(1) is "
              + arr.size(1)
              + " (must be equal to 1 or numClasses = "
              + details.oneHotNumClasses
              + ")");
    }
    ...
    for (int i = from; i <= to; i++) {
      ...
      if (w instanceof NDArrayWritable) {
        ...
        if (!a.isRowVectorOrScalar()) {
          throw new UnsupportedOperationException(
              "Multiple writables present but NDArrayWritable is "
  ...
  private INDArray convertWritables(
      List<List<Writable>> list, int minValues, SubsetDetails details) {
    try {
      return convertWritablesHelper(list, minValues, details);
    } catch (NumberFormatException e) {
      throw new RuntimeException(
          "Error parsing data (writables) from record readers - value is non-numeric", e);
  ...
  private INDArray convertWritablesHelper(
      List<List<Writable>> list, int minValues, SubsetDetails details) {
    INDArray arr;
    if (details.entireReader) {
      ...
      if (details.subsetStart == details.subsetEndInclusive
        ...
        // Need to check for multiple NDArrayWritables, or mixed NDArrayWritable + DoubleWritable
        // etc
      ...
      if (details.entireReader) {
        ...
        if (classIdx >= details.oneHotNumClasses) {
          throw new IllegalStateException(
              "Cannot convert sequence writables to one-hot: class index "
                  + classIdx
                  + " >= numClass ("
                  + details.oneHotNumClasses
        ...
        // Special case: subsetStart == subsetEndInclusive && NDArrayWritable. Example:
        // ImageRecordReader
        if (details.subsetStart == details.subsetEndInclusive
          ...
          for (int j = details.subsetStart; j <= details.subsetEndInclusive; j++) {
            ...
            if (w instanceof NDArrayWritable) {
              ...
              arr.put(
                  new INDArrayIndex[] {
                    NDArrayIndex.point(i), NDArrayIndex.interval(k, k + toPut.length())
                  },
                  toPut);
              k += toPut.length();
            } else {
  ...
  private void putExample(INDArray arr, INDArray singleExample, int exampleIdx) {
    Preconditions.checkState(
        singleExample.size(0) == 1 && singleExample.rank() == arr.rank(),
        "Cannot put array: array should have leading dimension of 1 and equal rank to output array."
            + " Attempting to put array of shape %s into output array of shape %s",
        singleExample.shape(),
        arr.shape());
    ...
    for (int i = 1; i < arr.rank(); i++) {
      Preconditions.checkState(
          arrShape[i] == singleShape[i],
          "Single example array and output arrays differ at position %s:"
              + "single example shape %s, output array shape %s",
          i,
          singleShape,
          arrShape);
    }
    switch (arr.rank()) {
      case 2:
        arr.put(
            new INDArrayIndex[] {NDArrayIndex.point(exampleIdx), NDArrayIndex.all()},
            singleExample);
        break;
      case 3:
        arr.put(
            new INDArrayIndex[] {
              NDArrayIndex.point(exampleIdx), NDArrayIndex.all(), NDArrayIndex.all()
            },
            singleExample);
        break;
      case 4:
      ...
      default:
        throw new RuntimeException(
            "Unexpected array rank: "
                + arr.rank()
                + " with shape "
                + Arrays.toString(arr.shape())
                + " input arrays should be rank 2 to 5 inclusive");
    }
  }

  /**
   * Convert the writables to a sequence (3d) data set, and also return the mask array (if
   * necessary)
   */
  private Pair<INDArray, INDArray> convertWritablesSequence(
      List<List<List<Writable>>> list,
      int minValues,
      int maxTSLength,
      SubsetDetails details,
      int[] longestSequence,
      long rngSeed) {
    ...
    if (needMaskArray && alignmentMode == AlignmentMode.EQUAL_LENGTH) {
      throw new UnsupportedOperationException(
          "Alignment mode is set to EQUAL_LENGTH but variable length data was encountered. Use"
              + " AlignmentMode.ALIGN_START or AlignmentMode.ALIGN_END with variable length data");
    }
    ...
    // Don't use the global RNG as we need repeatability for each subset (i.e., features and labels
    // must be aligned)
    Random rng = null;
    ...
    for (int i = 0; i < minValues; i++) {
      ...
      if (alignmentMode == AlignmentMode.ALIGN_START
          || alignmentMode == AlignmentMode.EQUAL_LENGTH) {
        startOffset = 0;
      } else {
        // Align end
      ...
      for (List<Writable> timeStep : sequence) {
        ...
        if (details.entireReader) {
            ...
            if (w instanceof NDArrayWritable) {
              ...
              arr.put(
                  new INDArrayIndex[] {
                    ...
                    NDArrayIndex.interval(j, j + row.length()),
          ...
          // Convert a single column to a one-hot representation
          Writable w = null;
          if (timeStep instanceof List) w = timeStep.get(details.subsetStart);
          else {
            Iterator<Writable> iter = timeStep.iterator();
          ...
          for (int j = details.subsetStart; j <= details.subsetEndInclusive; j++) {
            ...
            if (w instanceof NDArrayWritable) {
              ...
              arr.put(
                  new INDArrayIndex[] {
                    NDArrayIndex.point(i),
                    NDArrayIndex.interval(l, l + row.length()),
                    NDArrayIndex.point(k)
                  },
                  row);
      ...
      if (needMaskArray) {
        ...
        if (timeSeriesRandomOffset
            || alignmentMode == AlignmentMode.ALIGN_START
            || lastStep < maxTSLength) {
  ...
  public void reset() {
    if (!resetSupported) {
      throw new IllegalStateException(
          "Cannot reset iterator - reset not supported (resetSupported() == false):"
  ...
  public static class Builder {
    ...
    /**
     * Set as an input, the entire contents (all columns) of the RecordReader or
    ...
    /**
     * Add as an input a single column from the specified RecordReader / SequenceRecordReader The
     * assumption is that the specified column contains integer values in range 0..numClasses-1;
    ...
    /**
     * Set as an output, the entire contents (all columns) of the RecordReader or
     * SequenceRecordReader
     */
    ...
    /**
     * Add an output, with a subset of the columns from the named RecordReader or
     * SequenceRecordReader
     *
     * @param readerName Name of the reader
    ...
    /**
     * An an output, where the output is taken from a single column from the specified RecordReader
     * / SequenceRecordReader The assumption is that the specified column contains integer values in
     * range 0..numClasses-1; this integer will be converted to a one-hot representation (usually
     * for classification)
     *
    ...
    /**
     * For use with timeseries trained with tbptt In a given minbatch, shorter time series are
     * padded and appropriately masked to be the same length as the longest time series. Cases with
     * a skewed distrbution of lengths can result in the last few updates from the time series
     * coming from mostly masked time steps. timeSeriesRandomOffset randomly offsettsthe time series
     * + masking appropriately to address this
     *
    ...
    public RecordReaderMultiDataSetIterator build() {
      ...
      if (recordReaders.isEmpty() && sequenceRecordReaders.isEmpty()) {
        throw new IllegalStateException(
            "Cannot construct RecordReaderMultiDataSetIterator with no readers");
      }
      ...
      for (SubsetDetails ssd : inputs) {
        if (!recordReaders.containsKey(ssd.readerName)
            && !sequenceRecordReaders.containsKey(ssd.readerName)) {
          throw new IllegalStateException(
  ...
  /**
   * Load a single example to a DataSet, using the provided RecordMetaData. Note that it is more
   * efficient to load multiple instances at once, using {@link #loadFromMetaData(List)}
   *
