...
public class SequenceRecordReaderDataSetIterator implements DataSetIterator {
  /**
   * Alignment mode for dealing with input/labels of differing lengths (for example, one-to-many and
   * many-to-one type situations). For example, might have 10 time steps total but only one label at
   * end for sequence classification.<br>
   * Currently supported modes:<br>
   * <b>EQUAL_LENGTH</b>: Default. Assume that label and input time series are of equal length, and
   * all examples are of the same length<br>
   * <b>ALIGN_START</b>: Align the label/input time series at the first time step, and zero pad
   * either the labels or the input at the end<br>
   * <b>ALIGN_END</b>: Align the label/input at the last time step, zero padding either the input or
   * the labels as required<br>
   * Note 1: When the time series for each example are of different lengths, the shorter time series
   * will be padded to the length of the longest time series.<br>
   * Note 2: When ALIGN_START or ALIGN_END are used, the DataSet masking functionality is used.
   * Thus, the returned DataSets will have the input and mask arrays set. These mask arrays identify
   * whether an input/label is actually present, or whether the value is merely masked.<br>
   */
  ...
  @Getter @Setter private boolean collectMetaData = false;

  private RecordReaderMultiDataSetIterator underlying;
  private boolean underlyingIsDisjoint;

  /**
   * Constructor where features and labels come from different RecordReaders (for example, different
   * files), and labels are for classification.
   *
   * @param featuresReader SequenceRecordReader for the features
   * @param labels Labels: assume single value per time step, where values are integers in the range
   *     0 to numPossibleLables-1
   * @param miniBatchSize Minibatch size for each call of next()
   * @param numPossibleLabels Number of classes for the labels
   */
  public SequenceRecordReaderDataSetIterator(
      SequenceRecordReader featuresReader,
  ...
  /**
   * Constructor where features and labels come from the SAME RecordReader (i.e., target/label is a
   * column in the same data as the features). Defaults to regression = false - i.e., for
   * classification
   *
   ...
   * @param labelIndex index in input of the label index. If in regression mode and
   *     numPossibleLabels > 1, labelIndex denotes the first index for labels. Everything before
   *     that index will be treated as input(s) and everything from that index (inclusive) to the
   *     end will be treated as output(s)
   */
  public SequenceRecordReaderDataSetIterator(
      SequenceRecordReader reader, int miniBatchSize, int numPossibleLabels, int labelIndex) {
    this(reader, miniBatchSize, numPossibleLabels, labelIndex, false);
  }

  /**
   * Constructor where features and labels come from the SAME RecordReader (i.e., target/label is a
   * column in the same data as the features)
   *
   * @param reader SequenceRecordReader with data
   * @param miniBatchSize size of each minibatch
   * @param numPossibleLabels number of labels/classes for classification
  ...
  private void initializeUnderlying(SequenceRecord nextF) {
    ...
    // Add readers
    RecordReaderMultiDataSetIterator.Builder builder =
        new RecordReaderMultiDataSetIterator.Builder(miniBatchSize);
  ...
  private DataSet mdsToDataSet(MultiDataSet mds) {
    ...
    if (underlyingIsDisjoint) {
      ...
      fm =
          RecordReaderDataSetIterator.getOrNull(
              mds.getFeaturesMaskArrays(),
              0); // Per-example masking only on the input -> same for both

      // Can assume 3d features here
      f = Nd4j.createUninitialized(new long[] {f1.size(0), f1.size(1) + f2.size(1), f1.size(2)});
      f.put(
          new INDArrayIndex[] {
            NDArrayIndex.all(), NDArrayIndex.interval(0, f1.size(1)), NDArrayIndex.all()
          },
          f1);
      f.put(
          new INDArrayIndex[] {
            NDArrayIndex.all(),
            NDArrayIndex.interval(f1.size(1), f1.size(1) + f2.size(1)),
            NDArrayIndex.all()
          },
          f2);
    } else {
    ...
    if (collectMetaData) {
      ...
      for (Serializable s : temp) {
        ...
        if (singleSequenceReaderMode) {
          ...
          RecordMetaDataComposable c =
              new RecordMetaDataComposable(
                  m.getMeta().get(READER_KEY), m.getMeta().get(READER_KEY_LABEL));
          temp2.add(c);
        }
      }
  ...
  @Override
  public void reset() {
    if (underlying != null) underlying.reset();
  ...
  /**
   * Load a single sequence example to a DataSet, using the provided RecordMetaData. Note that it is
   * more efficient to load multiple instances at once, using {@link #loadFromMetaData(List)}
   *
   * @param recordMetaData RecordMetaData to load from. Should have been produced by the given
   *     record reader
   ...
   *
   * @param list List of RecordMetaData instances to load from. Should have been produced by the
   *     record reader provided to the SequenceRecordReaderDataSetIterator constructor
