...
public class RandomMultiDataSetIterator implements MultiDataSetIterator {
  ...
  @Getter @Setter private MultiDataSetPreProcessor preProcessor;

  private int position;

  /**
   * @param numMiniBatches Number of minibatches per epoch
   * @param features Each triple in the list specifies the shape, array order and type of values for
   *     the features arrays
   * @param labels Each triple in the list specifies the shape, array order and type of values for
   *     the labels arrays
   */
  public RandomMultiDataSetIterator(
      int numMiniBatches,
      @NonNull List<Triple<long[], Character, Values>> features,
      @NonNull List<Triple<long[], Character, Values>> labels) {
    Preconditions.checkArgument(
        numMiniBatches > 0, "Number of minibatches must be positive: got %s", numMiniBatches);
    ...
    switch (values) {
      ...
      case ONE_HOT:
        ...
        if (shape.length == 1) {
          ...
          throw new RuntimeException(
              "Not supported: rank 6+ arrays. Shape: " + Arrays.toString(shape));
        }
        return out;
      case ZEROS:
      ...
      case BINARY:
        return Nd4j.getExecutioner()
            .exec(new BernoulliDistribution(Nd4j.createUninitialized(shape, order), 0.5));
      case INTEGER_0_10:
