...
public class JointMultiDataSetIterator implements MultiDataSetIterator {
  ...
  /**
   * @param outcome Index to get the label from. If < 0, labels from all iterators will be used to
   *     create the final MultiDataSet
   * @param iterators Underlying iterators to wrap
   */
  ...
  /**
   * Fetch the next 'num' examples. Similar to the next method, but returns a specified number of
   * examples
   *
  ...
  /**
   * Get the {@link MultiDataSetPreProcessor}, if one has previously been set. Returns null if no
   * preprocessor has been set
   *
   * @return Preprocessor
   */
  @Override
  ...
  /**
   * Does this MultiDataSetIterator support asynchronous prefetching of multiple MultiDataSet
   * objects? Most MultiDataSetIterators do, but in some cases it may not make sense to wrap this
   * iterator in an iterator that does asynchronous prefetching. For example, it would not make
   * sense to use asynchronous prefetching for the following types of iterators: (a) Iterators that
   * store their full contents in memory already (b) Iterators that re-use features/labels arrays
   * (as future next() calls will overwrite past contents) (c) Iterators that already implement some
   * level of asynchronous prefetching (d) Iterators that may return different data depending on
  ...
  public MultiDataSet next() {
    ...
    for (val i : iterators) {
      ...
      if (ds.getFeaturesMaskArray() != null) hasFM = true;

      if (ds.getLabelsMaskArray() != null) hasLM = true;
    ...
    val mds =
        new org.nd4j.linalg.dataset.MultiDataSet(
            features.toArray(new INDArray[0]), labels.toArray(new INDArray[0]), fm, lm);
