...
public abstract class BaseParallelDataSetIterator implements ParallelDataSetIterator {
  ...
  public boolean hasNext() {
    ...
    if (states.allFalse() || allDepleted.get()) return false;
    ...
    else states.set(hasNext, curIdx);

    if (states.allFalse()) return false;
    ...
    switch (inequalityHandling) {
      ...
      case RESET:
        {
          ...
          // we don't want to have endless loop here, so we only do reset until all producers
          // depleted at least once
      ...
      case STOP_EVERYONE:
        {
          if (!states.allTrue()) return false;
