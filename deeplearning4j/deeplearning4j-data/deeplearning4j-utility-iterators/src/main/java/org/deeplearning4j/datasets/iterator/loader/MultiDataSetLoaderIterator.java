...
public class MultiDataSetLoaderIterator implements MultiDataSetIterator {
  ...
  /**
   * NOTE: When using this constructor (with {@code Iterator<String>}) the MultiDataSetIterator
   * cannot be reset. Use the other construtor that takes {@code Collection<String>}
   *
   ...
   */
  public MultiDataSetLoaderIterator(
      Iterator<String> paths, Loader<MultiDataSet> loader, SourceFactory sourceFactory) {
    this.paths = null;
    this.iter = paths;
    this.loader = loader;
   ...
   */
  public MultiDataSetLoaderIterator(
      Collection<String> paths, Loader<MultiDataSet> loader, SourceFactory sourceFactory) {
   ...
   *
   * @param paths Paths to iterate over
   * @param rng Optional random instance to use for shuffling of order. If null, no shuffling will
   *     be used.
   * @param loader Loader to use when loading DataSets
   * @param sourceFactory The factory to use to convert the paths into streams via {@link Source}
   */
  ...
  public void reset() {
    if (!resetSupported())
      throw new UnsupportedOperationException(
          "Reset not supported when using Iterator<String> instead of Iterable<String>");
    position = 0;
    if (rng != null) {
      MathUtils.shuffleArray(order, rng);
    }
