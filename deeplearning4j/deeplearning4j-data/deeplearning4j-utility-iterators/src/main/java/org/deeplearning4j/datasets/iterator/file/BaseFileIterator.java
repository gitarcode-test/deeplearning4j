...
public abstract class BaseFileIterator<T, P> implements Iterator<T> {
  ...
  @Getter @Setter protected P preProcessor;
  ...
  protected BaseFileIterator(
      @NonNull File[] rootDirs,
      boolean recursive,
      Random rng,
      int batchSize,
    ...
    for (File rootDir : rootDirs) {
      ...
      if (c.isEmpty()) {
        throw new IllegalStateException(
            "Root directory is empty (no files found) "
                + (validExtensions != null ? " (or all files rejected by extension filter)" : ""));
      }
      for (File f : c) {
        list.add(f.getPath());
      }
