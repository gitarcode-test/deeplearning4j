...
public class BenchmarkDataSetIterator implements DataSetIterator {
  ...
  /**
   * Creates 2d (shape [minibatch, numLabels]) or 4d labels ([minibatch, numLabels, gridWidth,
   * gridHeight]), depending on value of gridWidth and gridHeight.
   *
   * @param featuresShape Shape of the features data to randomly generate
   * @param numLabels Number of label classes (for classification)
   ...
   */
  public BenchmarkDataSetIterator(
      int[] featuresShape, int numLabels, int totalIterations, int gridWidth, int gridHeight) {
    ...
    if (this.baseLabels.rank() == 2) {
      ...
      this.baseLabels.get(
          NDArrayIndex.all(), NDArrayIndex.point(1), NDArrayIndex.all(), NDArrayIndex.all());
    }
