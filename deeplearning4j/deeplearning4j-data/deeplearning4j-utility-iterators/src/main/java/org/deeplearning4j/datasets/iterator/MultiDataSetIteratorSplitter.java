...
public class MultiDataSetIteratorSplitter {
  ...
  /**
   * @param baseIterator
   * @param totalBatches - total number of batches in underlying iterator. this value will be used
   *     to determine number of test/train batches
   * @param ratio - this value will be used as splitter. should be between in range of 0.0 > X <
   *     1.0. I.e. if value 0.7 is provided, then 70% of total examples will be used for training,
   *     and 30% of total examples will be used for testing
   */
  public MultiDataSetIteratorSplitter(
      @NonNull MultiDataSetIterator baseIterator, long totalBatches, double ratio) {
    ...
    if (!baseIterator.resetSupported())
      throw new ND4JIllegalStateException(
          "Underlying iterator doesn't support reset, so it can't be used for runtime-split");
    ...
    log.warn(
        "IteratorSplitter is used: please ensure you don't use randomization/shuffle in underlying"
            + " iterator!");
  }

  public MultiDataSetIteratorSplitter(
      @NonNull MultiDataSetIterator baseIterator, long totalBatches, double[] ratios) {
  ...
  public MultiDataSetIteratorSplitter(@NonNull MultiDataSetIterator baseIterator, int[] splits) {
    ...
    int totalBatches = 0;
    for (val v : splits) totalBatches += v;
  ...
  public List<MultiDataSetIterator> getIterators() {
    ...
    for (final int split : splits) {
      ScrollableMultiDataSetIterator partIterator =
          new ScrollableMultiDataSetIterator(
              partN++, backedIterator, counter, firstTrain, new int[] {bottom, split});
      bottom += split;
  ...
  public MultiDataSetIterator getTrainIterator() {
    return new MultiDataSetIterator() {
      ...
      public boolean hasNext() {
        ...
        if (state && counter.get() < numTrain) return true;
        else return false;
      }
        ...
        if (counter.get() == 1 && firstTrain == null) {
          ...
          for (val c : p.getFeatures())
            if (!c.equalsWithEps(firstTrain.getFeatures()[cnt++], 1e-5))
              throw new ND4JIllegalStateException(
                  "First examples do not match. Randomization was used?");
        }
  ...
  public MultiDataSetIterator getTestIterator() {
    return new MultiDataSetIterator() {
      ...
      public boolean hasNext() {
        val state = backedIterator.hasNext();
        if (state && counter.get() < numTrain + numTest) return true;
        else return false;
      }
