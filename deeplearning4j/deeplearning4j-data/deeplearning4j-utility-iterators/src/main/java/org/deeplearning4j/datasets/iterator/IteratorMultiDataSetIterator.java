...
public class IteratorMultiDataSetIterator implements MultiDataSetIterator {
  ...
  private final LinkedList<MultiDataSet>
      queued; // Used when splitting larger examples than we want to return in a batch
  ...
  public MultiDataSet next(int num) {
    ...
    while ((!queued.isEmpty() || iterator.hasNext()) && countSoFar < batchSize) {
      ...
      if (countSoFar + nExamples <= batchSize) {
        ...
        INDArray[] fMaskToKeep =
            (next.getFeaturesMaskArrays() != null ? new INDArray[nFeatures] : null);
        INDArray[] lMaskToKeep =
            (next.getLabelsMaskArrays() != null ? new INDArray[nLabels] : null);
        INDArray[] fMaskToCache =
            (next.getFeaturesMaskArrays() != null ? new INDArray[nFeatures] : null);
        INDArray[] lMaskToCache =
            (next.getLabelsMaskArrays() != null ? new INDArray[nLabels] : null);
        ...
        MultiDataSet toCache =
            new org.nd4j.linalg.dataset.MultiDataSet(
                fToCache, lToCache, fMaskToCache, lMaskToCache);
        list.add(toKeep);
        queued.add(toCache);
      }
  ...
  private static INDArray getRange(INDArray arr, long exampleFrom, long exampleToExclusive) {
    ...
    switch (rank) {
      ...
      case 3:
        return arr.get(
            NDArrayIndex.interval(exampleFrom, exampleToExclusive),
            NDArrayIndex.all(),
            NDArrayIndex.all());
      case 4:
        return arr.get(
