...
public class AutoencoderScoreCalculator extends BaseScoreCalculator<Model> {
  ...
  protected INDArray output(Model net, INDArray input, INDArray fMask, INDArray lMask) {
    ...
    if (!(l instanceof AutoEncoder)) {
      throw new UnsupportedOperationException(
          "Can only score networks with autoencoder layers as first layer -"
              + " got "
  ...
  protected double scoreMinibatch(
      ...
      INDArray[] lMask,
      INDArray[] output) {
    return scoreMinibatch(
        network, get0(features), get0(labels), get0(fMask), get0(lMask), get0(output));
  }
