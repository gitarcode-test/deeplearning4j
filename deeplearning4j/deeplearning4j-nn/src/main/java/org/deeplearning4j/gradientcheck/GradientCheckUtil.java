...
public class GradientCheckUtil {
  ...
  public static boolean checkGradients(MLNConfig c) {
    ...
    if (dataType != DataType.DOUBLE) {
      throw new IllegalStateException(
          "Cannot perform gradient check: Datatype is not set to double precision ("
              + "is: "
              + dataType
              + "). Double precision must be used for gradient checks. Set "
    ...
    if (netDataType != DataType.DOUBLE) {
      throw new IllegalStateException(
          "Cannot perform gradient check: Network datatype is not set to double precision ("
              + "is: "
              + netDataType
              + "). Double precision must be used for gradient checks. Create network with"
              + " .dataType(DataType.DOUBLE) before using GradientCheckUtil");
    }

    if (netDataType != c.net.params().dataType()) {
      throw new IllegalStateException(
          "Parameters datatype does not match network configuration datatype ("
              + "is: "
              + c.net.params().dataType()
              + "). If network datatype is set to DOUBLE, parameters must also be DOUBLE.");
    }
    ...
    for (NeuralNetConfiguration n : c.net.getLayerWiseConfigurations().getConfs()) {
      ...
      if (n.getLayer().getIDropout() != null && c.callEachIter == null) {
        throw new IllegalStateException(
            "When gradient checking dropout, need to reset RNG seed each iter, or no"
                + " dropout should be present during gradient checks - got dropout = "
                + n.getLayer().getIDropout()
                + " for layer "
                + layerCount);
      }
    }
    ...
    updater.update(
        c.net, gradAndScore.getFirst(), 0, 0, c.net.batchSize(), LayerWorkspaceMgr.noWorkspaces());
    ...
    INDArray originalParams =
        c.net.params().dup(); // need dup: params are a *view* of full parameters
    ...
    if (c.subset) {
      ...
      stepSizeForParam.put(
          paramNames.get(0),
          (int) Math.max(1, paramTable.get(paramNames.get(0)).length() / c.maxPerParam));
    } else {
    ...
    for (int i = 1; i < paramEnds.length; i++) {
      ...
      if (c.subset) {
        ...
        if (ss > Integer.MAX_VALUE) throw new ND4JArraySizeException();
    ...
    INDArray params =
        c.net.params(); // Assumption here: params is a view that we can modify in-place
        ...
        if (absError < c.minAbsoluteError) {
          ...
          log.info(
              "MLN Param "
                  ...
                  + ", paramValue = "
                  + origValue);
          if (c.exitOnFirstError) return false;
          totalNFailures++;
        }
  ...
  public static boolean checkGradients(GraphConfig c) {
    ...
    if (c.net.getNumInputArrays() != c.inputs.length)
      throw new IllegalArgumentException(
          "Invalid input arrays: expect " + c.net.getNumInputArrays() + " inputs");
    ...
    for (String vertexName : c.net.getConfiguration().getVertices().keySet()) {
      ...
      if (lv.getLayerConf().getLayer().getIDropout() != null && c.callEachIter == null) {
        throw new IllegalStateException(
            "When gradient checking dropout, rng seed must be reset each iteration, or no"
                + " dropout should be present during gradient checks - got dropout = "
                + lv.getLayerConf().getLayer().getIDropout()
                + " for layer "
                + layerCount);
      }
    }
    ...
    for (int i = 0; i < c.inputs.length; i++) c.net.setInput(i, c.inputs[i]);
    for (int i = 0; i < c.labels.length; i++) c.net.setLabel(i, c.labels[i]);
    ...
    updater.update(
        gradAndScore.getFirst(), 0, 0, c.net.batchSize(), LayerWorkspaceMgr.noWorkspaces());
    ...
    for (long i = 0; i < nParams; i++) {
      ...
      double scorePlus =
          c.net.score(
              mds,
              true); // training == true for batch norm, etc (scores and gradients need to be
                     // calculated on same thing)
  ...
  public static boolean checkGradientsPretrainLayer(
    ...
    updater.update(
        layer, gradAndScore.getFirst(), 0, 0, layer.batchSize(), LayerWorkspaceMgr.noWorkspaces());
    ...
    INDArray gradientToCheck =
        gradAndScore
            ...
            .dup(); // need dup: gradients are a *view* of the full gradient array (which will
                    // change every time backprop is done)
    INDArray originalParams =
        layer.params().dup(); // need dup: params are a *view* of full parameters
    ...
    INDArray params =
        layer.params(); // Assumption here: params is a view that we can modify in-place
