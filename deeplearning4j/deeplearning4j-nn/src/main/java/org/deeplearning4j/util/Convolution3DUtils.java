...
public class Convolution3DUtils {
   ...
   */
  public static long[] get3DOutputSizeLong(
      INDArray inputData,
      long[] kernel,
      long[] strides,
      long[] padding,
    ...
    validateShapesLong(
        inputData.shape(), eKernel, strides, padding, convolutionMode, dilation, inShape, atrous);
   ...
   * @return Output size: int[3] with output depth/height/width
   */
  public static int[] get3DOutputSize(
      INDArray inputData,
      int[] kernel,
      int[] strides,
      int[] padding,
    ...
    val inShape = new long[] {inD, inH, inW};
    validateShapes(
        ArrayUtil.toInts(inputData.shape()),
        eKernel,
        strides,
        padding,
        convolutionMode,
  ...
  private static void validateShapesLong(
      long[] inputDataShape,
      long[] eKernel,
      long[] strides,
      long[] padding,
  ...
  private static String getCommonErrorMsgLong(
      long[] inputDatashape, long[] kernel, long[] strides, long[] padding, long[] dilation) {
    String s =
        "\nInput size: [numExamples, inputDepth, inputHeight, inputWidth]="
            + Arrays.toString(inputDatashape)
            + ", inputKernel="
  ...
  private static String getCommonErrorMsg(
      int[] inputDatashape, int[] kernel, int[] strides, int[] padding, int[] dilation) {
    String s =
        "\nInput size: [numExamples, inputDepth, inputHeight, inputWidth]="
            + Arrays.toString(inputDatashape)
            + ", inputKernel="
   ...
   * @return
   */
  public static long[] get3DSameModeTopLeftPaddingLong(
   ...
   * @return
   */
  public static int[] get3DSameModeTopLeftPadding(
  ...
  /**
   * Perform validation on the CNN3D layer kernel/stride/padding. Expect 3d int[], with values > 0
   * for kernel size and stride, and values >= 0 for padding.
   *
   * @param kernelSize Kernel size array to check
   * @param stride Stride array to check
   * @param padding Padding array to check
   */
  public static void validateCnn3DKernelStridePadding(
      int[] kernelSize, int[] stride, int[] padding) {
    validateCnn3DKernelStridePaddingLong(
        toLongArray(kernelSize), toLongArray(stride), toLongArray(padding));
  }

  /**
   * Perform validation on the CNN3D layer kernel/stride/padding. Expect 3d long[], with values > 0
   * for kernel size and stride, and values >= 0 for padding.
   *
   * @param kernelSize Kernel size array to check
   * @param stride Stride array to check
  ...
  /**
   * Returns the {@link org.deeplearning4j.nn.conf.layers.Convolution3D.DataFormat} for the
   * associated layer. Note that {@link #layerHasConvolution3DLayout(Layer)} should return true on
   * the given layer input or an {@link IllegalArgumentException} will be thrown
   *
  ...
  public static Convolution3D.DataFormat getFormatForLayer(Layer inputLayer) {
    if (inputLayer instanceof Convolution3D) {
      ...
      throw new IllegalArgumentException(
          "Illegal input format type "
              + inputLayer.getClass().getName()
              + " with layer name of "
              + inputLayer.getLayerName());
    }
  }

  /**
   * Returns true if any of the layers are 3d convolution, pooling, or upsampling layers including:
   * {@link Convolution3D}, {@link Deconvolution3D}, {@link Subsampling3DLayer}, {@link
   * Upsampling3D}
   *
