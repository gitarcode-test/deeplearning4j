...
public class ConvolutionUtils {
  ...
  public static final String NCHW_NHWC_ERROR_MSG =
      "Note: Convolution layers can be configured for either NCHW (channels first) or NHWC"
          + " (channels last) format for input images and activations.\n"
          + "Layers can be configured using .dataFormat(CNN2DFormat.NCHW/NHWC) when constructing"
          + " the layer, or for the entire net using .setInputType(InputType.convolutional(height,"
          + " width, depth, CNN2DForman.NCHW/NHWC)).\n"
          + "ImageRecordReader and NativeImageLoader can also be configured to load image data in"
          + " either NCHW or NHWC format which must match the network";
    ...
    switch (paddingMode) {
      ...
      default:
        throw new UnsupportedOperationException(
            "Unknown/not supported padding mode: " + paddingMode);
    }
  }
  ...
  /**
   * Return the configuration for a given value for values like stride, dilation, kernel size that
   * require 2 values If the input is already length 2, return that if the length is only 1, return
   * the value specified twice otherwise return the default value duplicated twice
   *
  ...
  public static long[] getLongConfig(long[] inputValue, long defaultValue) {
    ...
    else if (inputValue.length == 2) return inputValue;
  ...
  /**
   * For NCHW we expect: 4D input with shape [minibatch, inputChannels, inputHeight, inputWidth] for
   * NHWC: 4D input with shape [minibatch, inputHeight, inputWidth, inputChannels] Note this is also
   * tied to convolutions.h weightShape
   *
   * @param format
   * @return
   */
  ...
  public static long[] getWeightShape1d(
      WeightsFormat weightsFormat, long kernelSize, long inputDepth, long outputDepth) {
    // [kW, iC, oC]
  ...
  public static long[] getWeightShape(
      WeightsFormat weightsFormat, long[] kernelSize, long inputDepth, long outputDepth) {
    switch (weightsFormat) {
      case OIYX:
  ...
  /**
   * Get the output size of a deconvolution operation for given input data. In deconvolution, we
   * compute the inverse of the shape computation of a convolution.
   *
   ...
   */
  public static long[] getDeconvolutionOutputSizeLong(
      INDArray inputData,
      long[] kernel,
      long[] strides,
      long[] padding,
  ...
  public static int[] getDeconvolutionOutputSize(
    ...
    return Arrays.stream(
            getDeconvolutionOutputSizeLong(
                inputData,
                toLongArray(kernel),
                toLongArray(strides),
                toLongArray(padding),
                convolutionMode,
                toLongArray(dilation),
                format))
   ...
   * @return Output size: int[2] with output height/width
   */
  public static long[] getDeconvolution3DOutputSizeLong(
      INDArray inputData,
      long[] kernel,
      long[] strides,
      long[] padding,
   ...
   * @return Output size: int[2] with output height/width
   */
  public static int[] getDeconvolution3DOutputSize(
      INDArray inputData,
      int[] kernel,
      int[] strides,
      int[] padding,
    ...
    return Arrays.stream(
            getDeconvolution3DOutputSizeLong(
                inputData,
                toLongArray(kernel),
                toLongArray(strides),
                toLongArray(padding),
  ...
  /**
   * @deprecated Use {@link #getOutputSize(INDArray, long[], long[], long[], ConvolutionMode,
   *     long[], CNN2DFormat)}
   */
  @Deprecated
  public static int[] getOutputSize(
  ...
  /**
   * Get the output size for a 2D convolution operation based on the input data, kernel, strides,
   * padding, convolution mode, dilation, and CNN2DFormat.
   *
  ...
  public static long[] getOutputSize(
    ...
    if (inputData.rank() != 4) {
      throw new IllegalArgumentException(
          "Input data must have rank 4 (received input with rank " + inputData.rank() + ")");
    }
    if (kernel.length != 2) {
      throw new IllegalArgumentException(
          "Kernel size must be an array of length 2 (received array of length "
              + kernel.length
              + ")");
    }
    if (strides.length != 2) {
      throw new IllegalArgumentException(
          "Strides must be an array of length 2 (received array of length " + strides.length + ")");
    }
    if (padding.length != 2) {
      throw new IllegalArgumentException(
          "Padding must be an array of length 2 (received array of length " + padding.length + ")");
    }
    if (dilation.length != 2) {
      throw new IllegalArgumentException(
          "Dilation must be an array of length 2 (received array of length "
              + dilation.length
              + ")");
    }
  ...
  /**
   * Returns true if a layer has a {@link CNN2DFormat} property. This is currently in use for:
   * {@link ConvolutionLayer}, {@link SubsamplingLayer}, {@link Upsampling2D}, {@link
   * SpaceToBatchLayer}, {@link SpaceToDepthLayer}, {@link ZeroPaddingLayer}, {@link
   * SeparableConvolution2D}, {@link Cropping2D}, {@link DepthwiseConvolution2D}
   *
  ...
  /**
   * Get the format for a given layer. {@link #layerHasConvolutionLayout(Layer)} should return true
   * on the given {@link Layer} type or an {@link IllegalArgumentException} will be thrown
   *
   * @param layer the input layer
   * @return the {@link CNN2DFormat} for the given layer
   */
  ...
  /**
   * Convert {@link ConvolutionMode} to {@link PaddingMode} {@link ConvolutionMode#Same} : {@link
   * PaddingMode#SAME} {@link ConvolutionMode#Strict}, {@link ConvolutionMode#Truncate} : {@link
   * PaddingMode#VALID} {@link ConvolutionMode#Causal} : {@link PaddingMode#VALID}
   *
   ...
   */
  public static long[] getOutputSizeLong(
      long[] inputShape,
      long[] kernel,
      long[] strides,
      long[] padding,
  ...
  public static void validateShapes(
    ...
    if (t && (eKernel[1] <= 0 || eKernel[1] > inW + 2 * padding[1])) {
      ...
      if (atrous) sb.append("effective ");
      sb.append(
          "kernel width and input width must satisfy  0 < kernel width <= input width + 2 * padding"
              + " width. ");
      sb.append("\nGot ");
      if (atrous) sb.append("effective ");
      sb.append("kernel width = ")
          .append(eKernel[1])
          .append(", input width = ")
          .append(inW)
          .append(" and padding width = ")
          .append(padding[1])
          .append(" which do not satisfy 0 < ")
          .append(eKernel[1])
          .append(" <= ")
          .append(inW + 2 * padding[1])
    ...
    if (eKernel.length == 3 && t && (eKernel[2] <= 0 || eKernel[2] > inShape[2] + 2 * padding[2])) {
      ...
      if (atrous) sb.append("effective ");
      sb.append("kernel channels = ")
          .append(eKernel[2])
          .append(", input channels = ")
          .append(inD)
          .append(" and padding height = ")
          .append(padding[2])
          .append(" which do not satisfy 0 < ")
    ...
    if (convolutionMode == ConvolutionMode.Strict) {
      if ((inH - eKernel[0] + 2 * padding[0]) % strides[0] != 0) {
        ...
        StringBuilder sb = new StringBuilder();
        sb.append(
                "Invalid input data or configuration: Combination of kernel size, stride and"
                    + " padding are not valid for given input height, using"
            ...
            .append(
                "See \"Constraints on strides\" at http://cs231n.github.io/convolutional-networks/"
                    + " and ConvolutionType enumeration Javadoc.\n")
            .append("To truncate/crop the input, such that output height = floor(")
            .append(str)
            .append(") = ")
            .append(truncated)
            .append(", use ConvolutionType.Truncate.\n")
            .append(
                "Alternatively use ConvolutionType.Same, which will use padding to give an output"
                    + " height of ceil(")
            .append(inH)
            .append("/")
            .append(strides[0])
      ...
      if ((inW - eKernel[1] + 2 * padding[1]) % strides[1] != 0) {
        ...
        sb.append(
                "Invalid input data or configuration: Combination of kernel size, stride and"
                    + " padding are not valid for given input width, using"
                    + " ConvolutionMode.Strict\n")
            .append(
                "ConvolutionMode.Strict requires: output width = (input - kernelSize +"
                    + " 2*padding)/stride + 1 to be an integer. Got: (")
            .append(inW)
            .append(" - ")
            .append(eKernel[1])
            ...
            .append(
                "See \"Constraints on strides\" at http://cs231n.github.io/convolutional-networks/"
                    + " and ConvolutionType enumeration Javadoc.\n")
            .append("To truncate/crop the input, such that output width = floor(")
            .append(str)
            .append(") = ")
            .append(truncated)
            .append(", use ConvolutionType.Truncate.\n")
      ...
      if (eKernel.length == 3 && (inShape[2] - eKernel[2] + 2 * padding[2]) % strides[2] != 0) {
        ...
        sb.append(
                "Invalid input data or configuration: Combination of kernel size, stride and"
                    ...
                    + " ConvolutionMode.Strict\n")
            .append(
                "ConvolutionMode.Strict requires: output channels = (input - kernelSize +"
                    + " 2*padding)/stride + 1 to be an integer. Got: (")
            .append(inD)
            .append(" - ")
            .append(eKernel[2])
  ...
  public static long[] effectiveKernelSize(long[] kernel, long[] dilation) {
    ...
    if (kernel.length == 2) {
      ...
      throw new IllegalArgumentException(
          "Kernel size has to be either two or three, got: " + kernel.length);
    }
  }
   ...
   */
  public static long[] getSameModeTopLeftPadding(
      long[] outSize, long[] inSize, long[] kernel, long[] strides, long[] dilation) {
    long[] eKernel = effectiveKernelSize(kernel, dilation);
    ...
    for (int i = 0; i < kernel.length; i++) {
      outPad[i] =
          ((outSize[i] - 1) * strides[i] + eKernel[i] - inSize[i])
              / 2; // Note that padBottom is 1 bigger than this if bracketed term is not divisible
                   // by 2
      allGt0 &= outPad[i] >= 0;
    }
    ...
    Preconditions.checkState(
        allGt0,
        "Invalid padding values calculated: %s - layer configuration is invalid? Input size %s,"
            + " output size %s, kernel %s, strides %s, dilation %s",
        outPad,
        inSize,
        outSize,
        kernel,
   ...
   */
  public static long[] getSameModeBottomRightPadding(
      long[] outSize, long[] inSize, long[] kernel, long[] strides, long[] dilation) {
    long[] eKernel = effectiveKernelSize(kernel, dilation);
    long[] outPad = new long[2];
    outPad[0] =
        ((outSize[0] - 1) * strides[0] + eKernel[0] - inSize[0] + 1)
            / 2; // Note that padTop is 1 smaller than this if bracketed term is not divisible by 2
    outPad[1] = ((outSize[1] - 1) * strides[1] + eKernel[1] - inSize[1] + 1) / 2; // As above
    Preconditions.checkState(
        outPad[0] >= 0 && outPad[1] >= 0,
        "Invalid padding values calculated: %s - layer configuration is invalid? Input size %s,"
            + " output size %s, kernel %s, strides %s, dilation %s",
        outPad,
        inSize,
  ...
  public static long[] getHeightAndWidth(NeuralNetConfiguration conf) {
    return getHeightAndWidth(((ConvolutionLayer) conf.getLayer()).getKernelSize());
  }

  /**
   * @param conf the configuration to get the number of kernels from
  ...
  public static int[] getHeightAndWidth(int[] shape) {
    return Arrays.stream(getHeightAndWidth(toLongArray(shape)))
        .mapToInt(Math::toIntExact)
        .toArray();
  }
   ...
   *
   * @param shape the shape to get the number of feature maps for
   * @return the number of feature maps for a particular shape
   */
  ...
  /**
   * Perform validation on the CNN layer kernel/stride/padding. Expect 2d int[], with values > 0 for
   ...
   */
  public static void validateCnnKernelStridePadding(
      long[] kernelSize, long[] stride, long[] padding) {
    if (kernelSize == null || kernelSize.length != 2) {
      throw new IllegalStateException(
          "Invalid kernel size: expected int[] of length 2, got "
              + (kernelSize == null ? null : Arrays.toString(kernelSize)));
    }

    if (stride == null || stride.length != 2) {
      throw new IllegalStateException(
          "Invalid stride configuration: expected int[] of length 2, got "
              + (stride == null ? null : Arrays.toString(stride)));
    }

    if (padding == null || padding.length != 2) {
      throw new IllegalStateException(
          "Invalid padding configuration: expected int[] of length 2, got "
  ...
  public static INDArray reshape4dTo2d(
      INDArray in, LayerWorkspaceMgr workspaceMgr, ArrayType type) {
    return reshape4dTo2d(in, CNN2DFormat.NCHW, workspaceMgr, type);
  }

  public static INDArray reshape4dTo2d(
      INDArray in, CNN2DFormat format, LayerWorkspaceMgr workspaceMgr, ArrayType type) {
    if (in.rank() != 4)
      throw new IllegalArgumentException(
          "Invalid input: expect NDArray with rank 4, got rank "
              + in.rank()
              + " with shape "
    ...
    if (format == CNN2DFormat.NCHW) {
      ...
      return workspaceMgr.leverageTo(
          type, out.reshape('c', shape[0] * shape[2] * shape[3], shape[1]));
    } else {
      // Reshape: from [n,h,w,c] to [n*h*w,c]
      if (in.ordering() != 'c' || !Shape.strideDescendingCAscendingF(in))
        in = workspaceMgr.dup(type, in, 'c');
      return workspaceMgr.leverageTo(
          type, in.reshape('c', shape[0] * shape[1] * shape[2], shape[3]));
    }
  }

  public static INDArray reshape5dTo2d(
      @NonNull Convolution3D.DataFormat format,
      INDArray in,
      LayerWorkspaceMgr workspaceMgr,
      ArrayType type) {
    Preconditions.checkState(
        in.rank() == 5,
        "Invalid input: expect NDArray with rank 5, got rank %ndRank with shape %ndShape",
        in,
        in);
  ...
  public static INDArray reshapeCnn3dMask(
      @NonNull Convolution3D.DataFormat format,
      INDArray mask,
      INDArray label,
      LayerWorkspaceMgr workspaceMgr,
    ...
    Preconditions.checkState(
        mask.rank() == 5,
        "Expected rank 5 mask for Cnn3DLossLayer in a shape broadcastable to labels shape:"
            + " got mask shape %ndShape with label shape %ndShape",
        mask,
        label);

    if (mask.equalShapes(label)
        || (format == Convolution3D.DataFormat.NDHWC
            && mask.size(0) == label.size(0)
            && mask.size(1) == label.size(1)
            && mask.size(2) == label.size(2)
            && mask.size(3) == label.size(3))
        || (format == Convolution3D.DataFormat.NDHWC
            && mask.size(0) == label.size(0)
            && mask.size(2) == label.size(2)
            && mask.size(3) == label.size(3)
            && mask.size(4) == label.size(4))) {
  ...
  public static INDArray reshape2dTo4d(
      INDArray in2d,
      long[] toShape,
      CNN2DFormat format,
      LayerWorkspaceMgr workspaceMgr,
    ...
    if (toShape.length != 4)
      throw new IllegalArgumentException(
          "Invalid input: expect toShape with 4 elements: got " + Arrays.toString(toShape));
  ...
  public static INDArray reshape2dTo5d(
  ...
  /**
   * @deprecated Use {@link #reshapeMaskIfRequired(INDArray, INDArray, CNN2DFormat,
   *     LayerWorkspaceMgr, ArrayType)}
   */
  @Deprecated
  public static INDArray reshapeMaskIfRequired(
      INDArray mask, INDArray output, LayerWorkspaceMgr workspaceMgr, ArrayType type) {
  ...
  public static INDArray adapt2dMask(
      INDArray mask,
      INDArray output,
    ...
    if (format == CNN2DFormat.NCHW) {
      ...
      // So: We'll broadcast to [n,1,h,w] then reshape to [n*h*w,1] required for the current DL4J
      // loss functions...
      ...
      val s = output.shape();
      INDArray bMask =
          workspaceMgr.create(type, mask.dataType(), new long[] {s[0], 1, s[2], s[3]}, 'c');
      Nd4j.getExecutioner().exec(new BroadcastCopyOp(bMask, mask, bMask, 0, 1));

      INDArray bMaskPermute =
          bMask.permute(0, 2, 3, 1).dup('c'); // Not sure if dup is strictly necessary...
      ...
      // So: We'll broadcast to [n,h,w,1] then reshape to [n*h*w,1] required for the current DL4J
      // loss functions...
      val s = output.shape();
      INDArray bMask =
          workspaceMgr.create(type, mask.dataType(), new long[] {s[0], s[2], s[3], 1}, 'c');
  ...
  public static INDArray reshape3dMask(
      INDArray mask, LayerWorkspaceMgr workspaceMgr, ArrayType type) {
  ...
  public static INDArray reshape4dMask(
      INDArray mask, LayerWorkspaceMgr workspaceMgr, ArrayType arrayType) {
  ...
  public static int[] getHWDFromInputType(InputType inputType) {
    ...
    if (inputType instanceof InputType.InputTypeConvolutional) {
      ...
      if (conv.getHeight() > Integer.MAX_VALUE
          || conv.getWidth() > Integer.MAX_VALUE
  ...
  /**
   * Given a mask array for a 1D CNN layer of shape [minibatch, sequenceLength], reduce the mask
   * according to the 1D CNN layer configuration. Unlike RNN layers, 1D CNN layers may down-sample
   * the data; consequently, we need to down-sample the mask array in the same way, to maintain the
   * correspondence between the masks and the output activations
   *
   * @param in Input size
   ...
   * @return Reduced mask
   */
  public static INDArray cnn1dMaskReductionLong(
      INDArray in, long kernel, long stride, long padding, long dilation, ConvolutionMode cm) {
    Preconditions.checkState(
        in.rank() == 2, "Rank must be 2 for cnn1d mask array - shape ", in.shape());
    ...
    if (cm == ConvolutionMode.Same || cm == ConvolutionMode.Causal) {
      outSize =
          ConvolutionUtils.getOutputSize(
              reshaped4d, k, s, null, cm, d, CNN2DFormat.NCHW); // Also performs validation
    } else {
      pad = new long[] {padding, 0};
      outSize =
          ConvolutionUtils.getOutputSize(
              reshaped4d, k, s, pad, cm, d, CNN2DFormat.NCHW); // Also performs validation
    }
    long outH = outSize[0];
   ...
   * @return Reduced mask
   */
  public static INDArray cnn1dMaskReduction(
      INDArray in, int kernel, int stride, int padding, int dilation, ConvolutionMode cm) {
    return cnn1dMaskReductionLong(in, kernel, stride, padding, dilation, cm);
  }

  /**
   * Reduce a 2d CNN layer mask array (of 0s and 1s) according to the layer configuration. Note that
   * when a CNN layer changes the shape of the activations (for example, stride > 1) the
   * corresponding mask array needs to change shape also (as there is a correspondence between the
   * two). This method performs the forward pass for the mask.
   *
   ...
   */
  public static INDArray cnn2dMaskReduction(
      INDArray inMask,
      int[] kernel,
      int[] stride,
      int[] padding,
  ...
  public static INDArray cnn2dMaskReduction(
    ...
    if (inMask.rank() != 4) {
      throw new IllegalStateException(
          "Expected rank 4 mask array for 2D CNN layers. Mask arrays for 2D CNN layers must have"
              + " shape [batchSize,channels,X,Y] where X = (1 or activationsHeight) and Y = (1 or"
              + " activationsWidth): Got rank "
              + inMask.rank()
              + " array with shape "
              + Arrays.toString(inMask.shape()));
    }
