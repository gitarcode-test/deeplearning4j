...
public class OutputLayerUtil {
  ...
  private static final String COMMON_MSG =
      "\n"
          + "This configuration validation check can be disabled for MultiLayerConfiguration and"
          + " ComputationGraphConfiguration using validateOutputLayerConfig(false), however this is"
          + " not recommended.";

  /**
   * Validate the output layer (or loss layer) configuration, to detect invalid consfiugrations. A
   * DL4JInvalidConfigException will be thrown for invalid configurations (like softmax + nOut=1).
   * <br>
  ...
  public static void validateOutputLayer(String layerName, Layer layer) {
    ...
    OutputLayerUtil.validateOutputLayerConfiguration(
        layerName, nOut, isLossLayer, activation, loss);
  }
   ...
   * <br>
   *
   * <p>If the specified layer is not an output layer, this is a no-op
   *
   * @param layerName Name of the layer
   ...
   */
  public static void validateOutputLayerConfiguration(
      String layerName,
      long nOut,
      boolean isLossLayer,
      IActivation activation,
    ...
    if (!isLossLayer
        && nOut == 1
        && activation instanceof ActivationSoftmax) { // May not have valid nOut for LossLayer
      throw new DL4JInvalidConfigException(
          "Invalid output layer configuration for layer \""
              + layerName
              + "\": Softmax + nOut=1 networks are not supported. Softmax cannot be used with"
              + " nOut=1 as the output will always be exactly 1.0 regardless of the input. "
              + COMMON_MSG);
    }

    // loss function required probability, but activation is outside 0-1 range
    if (lossFunctionExpectsProbability(lossFunction)
        && activationExceedsZeroOneRange(activation, isLossLayer)) {
      throw new DL4JInvalidConfigException(
          "Invalid output layer configuration for layer \""
              + layerName
              + "\": loss function "
              + lossFunction
              + " expects activations to be in the range 0 to 1 (probabilities) but activation"
              + " function "
              + activation
              + " does not bound values to this 0 to 1 range. This indicates a likely invalid"
    ...
    if (activation instanceof ActivationSoftmax && lossFunction instanceof LossBinaryXENT) {
      throw new DL4JInvalidConfigException(
          "Invalid output layer configuration for layer \""
              + layerName
              + "\": softmax activation function in combination with LossBinaryXENT (binary cross"
              + " entropy loss function). For multi-class classification, use softmax + MCXENT"
              + " (multi-class cross entropy); for binary multi-label classification, use sigmoid +"
              + " XENT. "
              + COMMON_MSG);
    }
    ...
    if (activation instanceof ActivationSigmoid && lossFunction instanceof LossMCXENT) {
      throw new DL4JInvalidConfigException(
          "Invalid output layer configuration for layer \""
              + layerName
              + "\": sigmoid activation function in combination with LossMCXENT (multi-class cross"
              + " entropy loss function). For multi-class classification, use softmax + MCXENT"
              + " (multi-class cross entropy); for binary multi-label classification, use sigmoid +"
              + " XENT. "
              + COMMON_MSG);
  ...
  /**
   * Validates if the output layer configuration is valid for classifier evaluation. This is used to
   * try and catch invalid evaluation - i.e., trying to use classifier evaluation on a regression
   * model. This method won't catch all possible invalid cases, but should catch some common
   * problems.
   *
   * @param outputLayer Output layer
   * @param classifierEval Class for the classifier evaluation
   */
  public static void validateOutputLayerForClassifierEvaluation(
      Layer outputLayer, Class<? extends IEvaluation> classifierEval) {
    if (outputLayer instanceof Yolo2OutputLayer) {
      throw new IllegalStateException(
          "Classifier evaluation using "
              + classifierEval.getSimpleName()
              + " class cannot be applied for object"
              + " detection evaluation using Yolo2OutputLayer: "
              + classifierEval.getSimpleName()
              + "  class is for classifier evaluation only.");
    }

    // Check that the activation function provides probabilities. This can't catch everything, but
    // should catch a few
    ...
    if (outputLayer instanceof BaseLayer) {
      ...
      boolean isOutputLayer =
          outputLayer instanceof OutputLayer
              || outputLayer instanceof RnnOutputLayer
              || outputLayer instanceof CenterLossOutputLayer;

      if (activationExceedsZeroOneRange(bl.getActivationFn(), !isOutputLayer)) {
        throw new IllegalStateException(
            "Classifier evaluation using "
                + classifierEval.getSimpleName()
                + " class cannot be applied to output layers with activation functions that are not"
                + " probabilities (in range 0 to 1). Output layer type: "
                + outputLayer.getClass().getSimpleName()
                + " has activation function "
                + bl.getActivationFn().getClass().getSimpleName()
