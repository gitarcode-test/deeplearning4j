...
public class LocalResponseNormalization
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    for (int i = 1; i < halfN + 1; i++) {
      if (nchw) {
        tmp =
            sumPart.get(
                NDArrayIndex.all(), interval(i, channel), NDArrayIndex.all(), NDArrayIndex.all());
        addVal =
            reverse.get(
                NDArrayIndex.all(),
        ...
        sumPart.put(
            new INDArrayIndex[] {
              NDArrayIndex.all(), interval(0, channel - i), NDArrayIndex.all(), NDArrayIndex.all()
            },
            tmp.addi(addVal));
      } else {
        tmp =
            sumPart.get(
                NDArrayIndex.all(), NDArrayIndex.all(), NDArrayIndex.all(), interval(i, channel));
        addVal =
            reverse.get(
                NDArrayIndex.all(),
                NDArrayIndex.all(),
        ...
        sumPart.put(
            new INDArrayIndex[] {
              NDArrayIndex.all(), NDArrayIndex.all(), NDArrayIndex.all(), interval(0, channel - i)
            },
            tmp.addi(addVal));
      }
    }

    // gx = gy * unitScale**-beta - 2 * alpha * beta * sumPart/unitScale * a^i_{x,y}    - rearranged
    // for more in-place ops
    INDArray nextEpsilon =
        workspaceMgr.createUninitialized(
            ArrayType.ACTIVATION_GRAD, epsilon.dataType(), epsilon.shape(), epsilon.ordering());
  ...
  private Triple<INDArray, INDArray, INDArray> activateHelper(
      boolean training, LayerWorkspaceMgr workspaceMgr, boolean forBackprop) {
    ...
    INDArray activations =
        workspaceMgr.createUninitialized(
            ArrayType.ACTIVATIONS, input.dataType(), input.shape(), input.ordering());
    if (forBackprop) {
