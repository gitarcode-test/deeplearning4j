...
public class ElementWiseVertex extends BaseGraphVertex {
  ...
  public ElementWiseVertex(
      ComputationGraph graph, String name, int vertexIndex, Op op, DataType dataType) {
  ...
  public INDArray doForward(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    if (inputs.length == 1) return workspaceMgr.dup(ArrayType.ACTIVATIONS, inputs[0]);
    ...
    switch (op) {
      ...
      case Average:
        INDArray average =
            workspaceMgr.createUninitialized(ArrayType.ACTIVATIONS, dataType, outShape);
        if (isBc && !Arrays.equals(outShape, inputs[0].shape())) {
            ...
            new SubOp(
                ...
                new INDArray[] {
                  workspaceMgr.createUninitialized(
                      ArrayType.ACTIVATIONS, inputs[0].dataType(), outShape)
                }))[0];
      case Product:
        INDArray product =
            workspaceMgr.createUninitialized(ArrayType.ACTIVATIONS, dataType, outShape);
        ...
        if (!isBroadcast) {
          INDArray max =
              workspaceMgr.createUninitialized(
                  ...
                  inputs[0].ordering());
          CustomOp op =
              DynamicCustomOp.builder("mergemax")
                  .addInputs(inputs)
                  .addOutputs(max)
                  .callInplace(false)
                  .build();
  ...
  public Pair<Gradient, INDArray[]> doBackward(boolean tbptt, LayerWorkspaceMgr workspaceMgr) {
    ...
    if (nInForwardPass == 1)
      return new Pair<>(
          null, new INDArray[] {workspaceMgr.dup(ArrayType.ACTIVATION_GRAD, epsilon)});
    ...
    switch (op) {
      case Add:
        ...
        for (int i = 0; i < nInForwardPass; i++) {
          if (!broadcastCase) {
            ...
            // So if [mb,3]+[mb,1] -> input 0 backprops epsilon, input 1 backprops
      ...
      case Product:
        ...
        for (int i = 0; i < nInForwardPass; i++) {
          ...
          for (int j = 0; j < nInForwardPass; ++j) {
            if (i != j) out_product[i].muli(inBc[j]);
          }
      ...
      case Max:
        ...
        INDArray maxIndices =
            workspaceMgr.createUninitialized(
                ArrayType.BP_WORKING_MEM, DataType.INT, epsilon.shape(), epsilon.ordering());
        ...
        CustomOp op =
            DynamicCustomOp.builder("mergemaxindex")
                .addInputs(bcIn)
        ...
        for (int i = 0; i < nInForwardPass; i++) {
          ...
          outMax[i] =
              workspaceMgr.create(ArrayType.BP_WORKING_MEM, DataType.BOOL, maxIndices.shape());
          // generate a mask with 1s and 0s in the right places and muli with epsilon
          MatchConditionTransform nd4jop =
              new MatchConditionTransform(maxIndices, outMax[i], Conditions.equals(i));
          ...
          if (broadcastCase && !epsilon.equalShapes(inputs[i])) {
            ...
            outMax[i] =
                workspaceMgr.leverageTo(
                    ArrayType.ACTIVATION_GRAD, outMax[i].castTo(epsilon.dataType()).muli(epsilon));
          }
        }
    ...
    if (maskArrays.length == 1) {
      ...
      INDArray ret =
          Nd4j.createUninitialized(
              DataType.BOOL, maskArrays[0].shape()); // maskArrays[0].dup(maskArrays[0].ordering());
      Nd4j.getExecutioner()
          .exec(
              new Or(
                  maskArrays[0].castTo(DataType.BOOL), maskArrays[1].castTo(DataType.BOOL), ret));
