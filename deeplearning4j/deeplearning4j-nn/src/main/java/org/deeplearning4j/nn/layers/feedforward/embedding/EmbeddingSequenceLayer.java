...
public class EmbeddingSequenceLayer
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    INDArray delta =
        layerConf()
            .getActivationFn()
            .backprop(z, epsilon)
            .getFirst(); // Shape: [mb, vector, seqLength]
    ...
    Nd4j.scatterUpdate(
        org.nd4j.linalg.api.ops.impl.scatter.ScatterUpdate.UpdateOp.ADD,
        weightGradients,
        indices,
        delta,
        WEIGHT_DIM);
  ...
  protected INDArray preOutput(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    if ((input.rank() == 3 && input.size(1) != 1) || (input.rank() != 2 && input.rank() != 3)) {
      throw new IllegalStateException(
          "Invalid input: EmbeddingSequenceLayer expects either rank 2 input of shape "
              + "[minibatch,seqLength] or rank 3 input of shape [minibatch,1,seqLength]. Got rank "
              + input.rank()
              + " input of shape "
    ...
    if (in.columns() != layerConf().getInputLength()) {
      ...
      throw new DL4JInvalidInputException(
          "Sequence length of embedding input has to be equal to the specified "
              + "input length: "
              + layerConf().getInputLength()
              + " i.e. we expect input shape [numExamples, inputLength] (or [numExamples, 1,"
              + " inputLength] with each entry being an integer index,  got "
              + Arrays.toString(input.shape())
              + " instead, for layer with id: "
              + layerId());
    }
    ...
    indexes =
        in.data()
            .asInt(); // C order: minibatch dimension changes least rapidly when iterating over
                      // buffer

    for (int i = 0; i < indexes.length; i++) {
      if (indexes[i] < 0 || indexes[i] >= nIn) {
        throw new DL4JInvalidInputException(
            "Invalid index for embedding layer: got index "
                + indexes[i]
                + " for entry "
                + i
                + " in minibatch; indexes must be between 0 and nIn-1 inclusive (0 to "
                + (nIn - 1)
                + ")");
      }
    }
    ...
    INDArray destination =
        workspaceMgr.createUninitialized(
            ...
            weights.dataType(),
            new long[] {minibatch * inputLength, nOut},
            'c');
    ...
    if (layerConf().getOutputFormat() == RNNFormat.NCW) {
      ret =
          ret.permute(
              0, 2, 1); // [minibatch, seqLen, nOut] -> [minibatch, nOut, seqLen] i.e., NWC -> NCW
    }
  ...
  public INDArray activate(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    if (maskArray != null) {
      if (maskArray.rank() != 2
          ...
          || (input.rank() == 3
              && (input.size(0) != maskArray.size(0) || input.size(2) != maskArray.size(1)))) {
        throw new IllegalStateException(
            "Mask array for EmbeddingSequenceLayer (when defined) must be rank 2 andhave shape"
                + " equal to input shape (when input is rank 2, shape [mb,tsLength]) or equal to"
                + " input dimensions 0 and 2 (when input is rank 3, shape [mb,1,tsLength]). Input"
                + " shape: "
                + Arrays.toString(input.shape())
                + ", mask shape: "
  ...
  protected void applyDropOutIfNecessary(boolean training, LayerWorkspaceMgr workspaceMgr) {
    throw new UnsupportedOperationException(
        "Dropout not supported with EmbeddingLayer " + layerId());
  }
