...
public class MultiLayerConfiguration implements Serializable, Cloneable {
  ...
  protected boolean validateOutputLayerConfig =
      true; // Default to legacy for pre 1.0.0-beta3 networks on deserialization

  @Getter @Setter protected WorkspaceMode trainingWorkspaceMode = WorkspaceMode.ENABLED;

  @Getter @Setter protected WorkspaceMode inferenceWorkspaceMode = WorkspaceMode.ENABLED;

  @Getter @Setter protected CacheMode cacheMode;

  @Getter @Setter
  protected DataType dataType =
      DataType.FLOAT; // Default to float for deserialization of beta3 and earlier nets

  // Counter for the number of parameter updates so far
  // This is important for learning rate schedules, for example, and is stored here to ensure it is
  // persisted
  ...
  public static ObjectMapper mapperYaml() {
    ...
    customDeserializerModule.setDeserializerModifier(
        new BeanDeserializerModifier() {
          ...
          public JsonDeserializer<?> modifyDeserializer(
            ...
            // Use our custom deserializers to handle backward compatibility for updaters ->
            // IUpdater
  ...
  public String toJson() {
    ...
    // occasionally
    // when writeValueAsString is used by multiple threads. This results in invalid JSON. See issue
    // #3243
    try {
  ...
  public static MultiLayerConfiguration fromJson(String json) {
    ...
    try {
      ...
      if (e.getMessage().contains("@class")) {
        try {
          ...
          // Check for legacy custom layers: "Could not resolve type id 'CustomLayer' as a subtype
          // of [simple type, class org.deeplearning4j.nn.conf.layers.Layer]: known type ids =
          // [Bidirectional, CenterLossOutputLayer, CnnLossLayer, ..."
          // 1.0.0-beta5: dropping support for custom layers defined in pre-1.0.0-beta format.
          // Built-in layers from these formats still work
          ...
          if (msg != null && msg.contains("Could not resolve type id")) {
            throw new RuntimeException(
                "Error deserializing MultiLayerConfiguration - configuration may have a custom"
                    ...
                    + "Models in legacy format with custom layers should be loaded in 1.0.0-beta to"
                    + " 1.0.0-beta4 and saved again, before loading in the current version of DL4J",
                e);
          }
      ...
      if (msg != null && msg.contains("legacy")) {
        throw new RuntimeException(
            "Error deserializing MultiLayerConfiguration - configuration may have a custom layer,"
                + " vertex or preprocessor, in pre version 1.0.0-alpha JSON format. These layers"
                + " can be deserialized by first registering them with"
    ...
    // To maintain backward compatibility after loss function refactoring (configs generated with
    // v0.5.0 or earlier)
    ...
    for (NeuralNetConfiguration nnc : conf.getConfs()) {
      ...
      if (l instanceof BaseOutputLayer && ((BaseOutputLayer) l).getLossFn() == null) {
        // lossFn field null -> may be an old config format, with lossFunction field being for the
        // enum
          ...
          if (confs instanceof ArrayNode) {
            ...
            if (lossFunctionNode != null) {
              ...
              try {
                ...
                log.warn(
                    "OutputLayer with null LossFunction or pre-0.6.0 loss function configuration"
                        + " detected: could not parse JSON",
                    e);
              }
              ...
              if (lossFunction != null) {
                switch (lossFunction) {
                  ...
                  default:
                    log.warn(
                        "OutputLayer with null LossFunction or pre-0.6.0 loss function"
                            + " configuration detected: could not set loss function for {}",
                        lossFunction);
                    break;
                }
              }
            ...
            log.warn(
                "OutputLayer with null LossFunction or pre-0.6.0 loss function configuration"
                    + " detected: could not parse JSON: layer 'confs' field is not an ArrayNode"
                    + " (is: {})",
      ...
      // Also, pre 0.7.2: activation functions were Strings ("activationFunction" field), not
      // classes ("activationFn")
        ...
        try {
          ...
          if (confs instanceof ArrayNode) {
            ...
            if (activationFunction != null) {
              IActivation ia =
                  Activation.fromString(activationFunction.asText()).getActivationFunction();
          ...
          log.warn(
              "Layer with null ActivationFn field or pre-0.7.2 activation function detected: could"
                  + " not parse JSON",
              e);
        }
      }
  ...
  public NetworkMemoryReport getMemoryReport(InputType inputType) {
    ...
    return new NetworkMemoryReport(
        memoryReportMap, MultiLayerConfiguration.class, "MultiLayerNetwork", inputType);
  }
  ...
  public static class Builder extends BaseBuilder {
    ...
    public MultiLayerConfiguration build() {
      ...
      if ((tbpttBackLength != DEFAULT_TBPTT_LENGTH || tbpttFwdLength != DEFAULT_TBPTT_LENGTH)
        ...
        log.warn(
            "Truncated backpropagation through time lengths have been configured with values "
                ...
                + backpropType
                + ". TBPTT configuration settings will only take effect if backprop type is set to"
        ...
        for (int i = 0; i < confs.size(); i++) {
          ...
          if (l instanceof LastTimeStep || l instanceof GlobalPoolingLayer) {
            throw new IllegalStateException(
                "Invalid network configuration detected: Truncated backpropagation through time"
                    + " (TBPTT) cannot be used with layer "
                    + i
                    + " of type "
                    + l.getClass().getName()
                    + ": TBPTT is incompatible with this layer type (which is designed to process"
                    + " entire sequences at once, and does support the type of sequence segments"
                    + " that TPBTT uses).\n"
                    + "This check can be disabled using validateTbpttConfig(false) but this is not"
                    + " recommended.");
          }
        }
      }
      ...
      if (inputType == null && inputPreProcessors.get(0) == null) {
        ...
        // For example, Dense/RNN layers, where preprocessor isn't set -> user is *probably* going
        // to feed in
          ...
          if (i > 0) {
            ...
            if (layer instanceof Convolution1DLayer) {
              if (l instanceof DenseLayer && inputType instanceof InputType.InputTypeRecurrent) {
                ...
                l.setNIn(
                    currentInputType,
                    overrideNinUponBuild); // Don't override the nIn setting, if it's manually set
                                           // by the user
            } else
              l.setNIn(
                  currentInputType,
                  overrideNinUponBuild); // Don't override the nIn setting, if it's manually set by
                                         // the user
      ...
      if (validateOutputConfig) {
        ...
        for (NeuralNetConfiguration n : conf.getConfs()) {
          ...
          OutputLayerUtil.validateOutputLayer(
              l.getLayerName(), l); // No-op for non output/loss layers
        }
      }
