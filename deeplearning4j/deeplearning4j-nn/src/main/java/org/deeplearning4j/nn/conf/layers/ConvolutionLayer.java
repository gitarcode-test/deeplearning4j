...
public class ConvolutionLayer extends FeedForwardLayer {
  ...
  protected ConvolutionMode convolutionMode =
      ConvolutionMode
          .Truncate; // Default to truncate here - default for 0.6.0 and earlier networks on JSON
                     // deserialization
  ...
  @JsonIgnore @EqualsAndHashCode.Exclude private boolean defaultValueOverriden = false;
   ...
   * memory intensive, so if weird errors occur when using cuDNN, please try the "NO_WORKSPACE"
   * mode. Alternatively, it is possible to specify the algorithm manually by setting the
   * "USER_SPECIFIED" mode, but this is not recommended.
   *
   * <p>Note: Currently only supported with cuDNN.
   */
    ...
    if (builder.kernelSize.length != dim) {
      throw new IllegalArgumentException(
          "Kernel argument should be a "
              + dim
              + "d array, got "
              + Arrays.toString(builder.kernelSize));
    }
    ...
    if (builder.stride.length != dim) {
      throw new IllegalArgumentException(
          "Strides argument should be a "
              + dim
              + "d array, got "
              + Arrays.toString(builder.stride));
    }
    ...
    if (builder.padding.length != dim) {
      throw new IllegalArgumentException(
          "Padding argument should be a "
              + dim
              + "d array, got "
              + Arrays.toString(builder.padding));
    }
    ...
    if (builder.dilation.length != dim) {
      throw new IllegalArgumentException(
          "Dilation argument should be a "
              + dim
              + "d array, got "
              + Arrays.toString(builder.dilation));
    }
    this.dilation = builder.dilation;
  ...
  public Layer instantiate(
    ...
    LayerValidation.assertNInNOutSet(
        "ConvolutionLayer", getLayerName(), layerIndex, getNIn(), getNOut());
  ...
  public LayerMemoryReport getMemoryReport(InputType inputType) {
    ...
    // During backprop: im2col array for forward pass (possibly cached) + the epsilon6d array
    ...
    return new LayerMemoryReport.Builder(layerName, ConvolutionLayer.class, inputType, outputType)
        ...
        .workingMemory(
            0, im2colSizePerEx, MemoryReport.CACHE_MODE_ALL_ZEROS, trainWorkingMemoryPerEx)
  ...
  @Getter
  @Setter
  public abstract static class BaseConvBuilder<T extends BaseConvBuilder<T>>
    ...
    /**
     * Kernel dilation. Default: {1, 1}, which is standard convolutions. Used for implementing
     * dilated convolutions, which are also known as atrous convolutions.
     *
     * <p>For more details, see: <a href="https://arxiv.org/abs/1511.07122">Yu and Koltun (2014)</a>
     * and <a href="https://arxiv.org/abs/1412.7062">Chen et al. (2014)</a>, as well as <a
     * href="http://deeplearning.net/software/theano/tutorial/conv_arithmetic.html#dilated-convolutions">
    ...
    protected BaseConvBuilder(
        int[] kernelSize, int[] stride, int[] padding, int[] dilation, int dim) {
      this(
          toLongArray(kernelSize),
          toLongArray(stride),
          toLongArray(padding),
    ...
    protected BaseConvBuilder(
        long[] kernelSize, long[] stride, long[] padding, long[] dilation, long dim) {
      this.setKernelSize(kernelSize);
      this.setStride(stride);
      this.setPadding(padding);
    ...
    protected void setConvolutionMode(ConvolutionMode convolutionMode) {
      Preconditions.checkState(
          allowCausal() || convolutionMode != ConvolutionMode.Causal,
     ...
     * back to the user. If true, the built-in (non-MKL/CuDNN) implementation for ConvolutionLayer
     * will be used
     *
     * @param allowFallback Whether fallback to non-CuDNN implementation should be used
     */
