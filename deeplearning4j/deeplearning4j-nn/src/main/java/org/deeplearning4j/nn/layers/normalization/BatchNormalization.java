...
public class BatchNormalization
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    INDArray globalVar =
        params.get(
            BatchNormalizationParamInitializer
                .GLOBAL_VAR); // One of log10std will be null depending on config
    ...
    INDArray dGlobalLog10StdView =
        gradientViews.get(BatchNormalizationParamInitializer.GLOBAL_LOG_STD);
    ...
    if (epsilon.rank() == 2) {
      ...
      if (layerConf.isLockGammaBeta()) {
        ...
        // Standard case
        dxhat =
            epsilon.mulRowVector(
                gamma); // dL/dxHat = dL/dOut . gamma        Shape: [minibatchSize, nOut]
      }
      ...
      INDArray dLdVar =
          dxhat
              ...
              .sum(true, 0)
              .muli(-0.5)
              .muli(Transforms.pow(std, -3.0, true)); // Shape: [1, miniBatch]
      ...
      // Note the array reuse here: dxhat, xMu, dLdVar, dLdmu - all are invalid after this line (but
      // aren't used later anyway)
      INDArray dLdx =
          dxhat
              .diviRowVector(std)
                ...
                .exec(
                    new BroadcastMulOp(
                        ...
                        Nd4j.createUninitialized(
                            epsilon.dataType(), epsilon.shape(), epsilon.ordering()),
                        chIdx));
      }

      // dL/dVariance
      INDArray dLdVar =
          dxhat.mul(xMu).sum(nonChDims).muli(-0.5).muli(Transforms.pow(std, -3.0, true));
      ...
      INDArray dLdx =
          Nd4j.getExecutioner()
              .exec(new BroadcastDivOp(dxhat, std, dxhat, chIdx))
              .addi(
                  Nd4j.getExecutioner()
                      .exec(
                          new BroadcastMulOp(
                              xMu, dLdVar.muli(2.0 / effectiveBatchSize), xMu, chIdx)));
      Nd4j.getExecutioner()
          .execAndReturn(
              new BroadcastAddOp(dLdx, dLdmu.muli(1.0 / effectiveBatchSize), dLdx, chIdx));
    ...
    Nd4j.getExecutioner()
        .exec(
            new SubOp(
                globalMean,
                batchMean,
                dGlobalMeanView)); // deltaGlobalMean = globalMean[t] - batchMean
    dGlobalMeanView.muli(1 - layerConf().getDecay());

    if (layerConf().isUseLogStd()) {
      // Use log10(std) parameterization. This is more numerically stable for FP16 and better for
      ...
      // Use variance estimate parameterization. This was only option up to and including
      // 1.0.0-beta3
      Nd4j.getExecutioner()
          .exec(
              new SubOp(
                  globalVar, batchVar, dGlobalVarView)); // deltaGlobalVar = globalVar[t] - batchVar
    ...
    if (layerConf().isUseLogStd()) {
      retGradient.setGradientFor(
          BatchNormalizationParamInitializer.GLOBAL_LOG_STD, dGlobalLog10StdView);
    } else {
  ...
  public INDArray preOutput(INDArray x, TrainingMode training, LayerWorkspaceMgr workspaceMgr) {
    ...
    if (x.rank() == 4 && layerConf().getCnn2DFormat() == CNN2DFormat.NHWC) dim = 3;
    if (x.size(dim) != layerConf().getNOut()) {
      throw new IllegalArgumentException(
          "input.size("
              + dim
              + ") does not match expected input size of "
              + layerConf().getNIn()
              + " - got input array with shape "
    ...
    // Note that for CNNs, mean and variance are calculated per feature map (i.e., per activation)
    // rather than per activation
    // Pg5 of https://arxiv.org/pdf/1502.03167v3.pdf
    // "For convolutional layers, we additionally want the normalization to obey the convolutional
    // property – so that
    //  different elements of the same feature map, at different locations, are normalized in the
    // same way. To achieve
    ...
    if (training == TrainingMode.TRAIN) {
      switch (x.rank()) {
        ...
        default:
          throw new IllegalStateException(
              "Batch normalization on activations of rank "
                  + x.rank()
                  + " not supported "
                  + layerId());
      }
      ...
      std =
          Transforms.sqrt(workspaceMgr.dup(ArrayType.INPUT, var).addi(layerConf().getEps()), false);
    } else {
    ...
    if (x.rank() == 2) {
      ...
      if (layerConf.isLockGammaBeta()) {
        ...
        if (g != 1.0 && b != 0.0) {
          // Default and most common case: 1.0 and 0.0 for these parameters. No point executing 1 *
          // x + 0 op
      ...
      if (layerConf.isLockGammaBeta()) {
        ...
        activations =
            workspaceMgr.createUninitialized(
                ArrayType.ACTIVATIONS, x.dataType(), x.shape(), x.ordering());
        activations =
            Nd4j.getExecutioner().exec(new BroadcastMulOp(xHat, gamma, activations, chIdx));
        activations =
            Nd4j.getExecutioner().exec(new BroadcastAddOp(activations, beta, activations, chIdx));
      }
    } else {
      // TODO setup BatchNorm for RNN https://arxiv.org/pdf/1510.01378v1.pdf
      throw new IllegalStateException(
    ...
    activations =
        workspaceMgr.leverageTo(
            ArrayType.ACTIVATIONS, activations); // Most of the time this should be a no-op
    ...
    if (rnnInput) {
      ...
      activations =
          activations.reshape(activations.size(1), activations.size(2), activations.size(3));
    }
    return activations;
  }
  ...
  public long[] getShape(INDArray x) {
    if (x.rank() == 2) return new long[] {1, x.size(1)};
    if (x.rank() == 4) {
    ...
    if (x.rank() == 3) {
      ...
      throw new IllegalStateException(
          "Unable to process input of rank " + x.rank() + " " + layerId());
  }

  @Override
  public boolean updaterDivideByMinibatch(String paramName) {
    // Majority of params's gradients should be... Exception: batch norm mean/variance estimate
    if (BatchNormalizationParamInitializer.GLOBAL_MEAN.equals(paramName)
        || BatchNormalizationParamInitializer.GLOBAL_VAR.equals(paramName)
