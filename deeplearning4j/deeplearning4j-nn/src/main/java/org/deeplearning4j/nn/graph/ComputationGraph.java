...
public class ComputationGraph implements Serializable, Model, NeuralNetwork {
  ...
  @Getter
  protected transient INDArray
      flattenedGradients; // Gradients for all layers are a view/subset of this array

  @Getter @Setter protected Gradient gradient;
  ...
  @Getter protected transient Map<String, Pointer> helperWorkspaces = new HashMap<>();
  ...
  /**
   * Indexes of graph vertices, in topological order. The topological order defines the order in
   * which forward pass (and hence also backward pass, which is the opposite to this) is conducted
   * in the network.
   */
  ...
  /**
   * A list of layers. Each of these layers is present in a GraphVertex, but are here for easy
   * reference. This array also defines the order in which the getLayer(int) method returns layers.
   */
  protected Layer[] layers;

  /**
   * The number of input arrays to the network. Many networks only have 1 input; however, a
   * ComputationGraph may have an arbitrary number (>=1) separate input arrays
   */
  private int numInputArrays;

  /**
   * The number of output arrays to the network. Many networks only have 1 output; however, a
   * ComputationGraph may have an arbitrary number (>=1) separate output arrays
   */
  ...
  public ComputationGraph(ComputationGraphConfiguration configuration) {
    ...
    // Working memory: should learn over course of: (a) full forward pass, and (b) full backward
    // pass
    ...
    // Activations memory: opened once per layer - for every second layer (preprocessors are within
    // the loop).
    // Technically we could set learning to numLayers / 2, but will set to numLayers for simplicity,
    // and also to
  ...
  /**
   * Get the layer by the number of that layer, in range 0 to getNumLayers()-1 NOTE: This is
   * different from the internal GraphVertex index for the layer
   */
  ...
  public Layer getLayer(String name) {
    Preconditions.checkState(
        verticesMap.containsKey(name), "Layer with name %s does not exist in the network", name);
  ...
  public void setInputs(INDArray... inputs) {
    if (inputs != null && inputs.length != this.numInputArrays) {
      throw new IllegalArgumentException(
          "Invalid input array: network has "
              + numInputArrays
              + " inputs, but array is of length "
              + inputs.length);
    }
  ...
  public void setLabels(INDArray... labels) {
    if (labels != null && labels.length != this.numOutputArrays) {
      throw new IllegalArgumentException(
          "Invalid output array: network has "
              + numOutputArrays
              + " outputs, but array is of length "
              + labels.length);
    }
    this.labels = labels;
  }
  ...
  /**
   * Initialize the ComputationGraph, optionally with an existing parameters array. If an existing
  ...
  public void init(INDArray parameters, boolean cloneParametersArray) {
    ...
    if (parameters != null && parameters.dataType() != netDtype) {
      Preconditions.checkState(
          parameters.rank() == 2 && parameters.size(0) == 1,
      ...
      if (cloneParametersArray) {
        ...
        throw new IllegalStateException(
            "Error initializing network: Network datatype is set to "
                + netDtype
                + " but provided array has datatype "
                + parameters.dataType()
                + " with cloneParametersArray argument set to false. Cannot initialize net with"
                + " specified datatype array if that array does not match network datatype");
      }
    }
    ...
    if (configuration.getCacheMode() == null) configuration.setCacheMode(CacheMode.NONE);

    OneTimeLogger.info(
        log,
        "Starting ComputationGraph with WorkspaceModes set to [training: {}; inference: {}],"
            + " cacheMode set to [{}]",
        configuration.getTrainingWorkspaceMode(),
        configuration.getInferenceWorkspaceMode(),
        configuration.getCacheMode());

    // First: build topological ordering, based on configuration. Used for forward pass, backprop
    // and order of parameters/gradients
    ...
    Map<String, org.deeplearning4j.nn.conf.graph.GraphVertex> configVertexMap =
    ...
    for (String name : networkInputNames) {
      GraphVertex gv =
          new InputVertex(this, name, vertexNumber, null, netDtype); // Output vertices: set later
      allNamesReverse.put(name, vertexNumber);
      vertices[vertexNumber++] = gv;
    }

    // Go through layers, and work out total number of parameters. Then allocate full parameters
    // array
    long numParams = 0;
    ...
    for (; i < topologicalOrder.length; i++) {
      ...
      if (numParamsForVertex[i] < 0)
        throw new DL4JInvalidConfigException(
            "Layer " + name + " had parameters < 0 " + numParamsForVertex[i]);
    ...
    if (parameters != null) {
      ...
      if (parameters.length() != numParams)
        throw new IllegalArgumentException(
            "Invalid parameters: expected length "
                + numParams
                + ", got length "
                + parameters.length());
    ...
    if (flattenedParams == null) flattenedParams = Nd4j.zeros(DataType.FLOAT, 0);

    INDArray flattenedParamsReshape = flattenedParams.reshape(flattenedParams.length());
    // Given the topological ordering: work out the subset of the parameters array used for each
    // layer
    ...
    for (int vertexIdx : topologicalOrder) {
      ...
      if (nParamsThisVertex != 0) {
        paramsViewForVertex[vertexIdx] =
            flattenedParamsReshape.get(
    ...
    for (; i < topologicalOrder.length; i++) {
      ...
      if (gv == null) {
        throw new IllegalStateException(
            "Encountered null layer/vertex during initialization for layer \""
                + name
                + "\": "
                + n.getClass().getSimpleName()
                + " initialization returned null layer/vertex?");
      }
    ...
    Map<String, List<String>> verticesOutputTo =
        new HashMap<>(); // Key: vertex. Values: vertices that this node is an input for
    for (GraphVertex gv : vertices) {
    ...
    if (solver == null) {
      try (MemoryWorkspace wsO = Nd4j.getMemoryManager().scopeOutOfWorkspaces()) {
        solver =
            new Solver.Builder().configure(conf()).listeners(getListeners()).model(this).build();
        solver.initOptimizer();
      }
    }

    // Mark which layers can safely modify their input in-place. This is a performance optimization
    // for
    // dropout and similar operations.
    // Safe when the input is: (a) it's not a graph input, and (b) isn't shared by any other
    // layers/vertices
    ...
    for (Layer l : layers) {
      ...
      if (configuration.getNetworkInputs().contains(in)) {
        ...
        // For example dropout + iterating over List<MultiDataSet> that is used for multiple
        // epochs...
        continue;
      }
      ...
      if (seen.size() == 1) {
        ...
        // For the count > 1 case, we can work out if it's the last one in the topological order...
        // at which point,
        ...
        if (thisTopoPos == maxTopoPosition) {
          // Last one in the topo sort... all other layers have already consumed this input by the
          // time this layer's
   ...
   * manually when doing training via fit(DataSet), fit(DataSetIterator) or fit(MultiDataSet)
   * methods
   */
  public void initGradientsView() {
    try (MemoryWorkspace ws = Nd4j.getMemoryManager().scopeOutOfWorkspaces()) {
      ...
      if (flattenedGradients == null) flattenedGradients = Nd4j.zeros(DataType.FLOAT, 0);
  ...
  /**
   * Pretrain network with a single input and single output. DataSetIterators can only be used if
   * the number of input arrays for the ComputationGraph is 1.<br>
   * This method performs layerwise pretraining on all pre-trainable layers in the network (VAEs,
   * Autoencoders, etc), for the specified number of epochs each. For example, if numEpochs=3, then
   * layer 0 will be fit for 3 epochs, followed by layer 1 for 3 epochs, and so on.<br>
  ...
  private void pretrainLayerHelper(String layerName, MultiDataSetIterator iter, int numEpochs) {
    ...
    if (!verticesMap.containsKey(layerName)) {
      throw new IllegalStateException(
          "Invalid vertex name: " + layerName + " - all vertex names: " + verticesMap.keySet());
    }
    ...
    if (configuration.getTrainingWorkspaceMode() == WorkspaceMode.NONE) {
      ...
      workspaceMgr =
          LayerWorkspaceMgr.builder()
              ...
              .with(
                  ArrayType.UPDATER_WORKING_MEM, WS_LAYER_WORKING_MEM, WS_LAYER_WORKING_MEM_CONFIG)
              .build();
    }
    ...
    MultiDataSetIterator withAsync =
        iter.asyncSupported() ? new AsyncMultiDataSetIterator(iter) : iter;
    ...
    while (withAsync.hasNext()) {
      ...
      try (MemoryWorkspace ws = workspaceMgr.notifyScopeEntered(ArrayType.ACTIVATIONS)) {
        // FF - note should be using TEST mode here for the layers that feed into the specified
        // layer
  ...
  /**
   * Fit the ComputationGraph using a DataSet. Note that this method can only be used with
   * ComputationGraphs with 1 input and 1 output. For networks with more than one input or output,
   * use {@link #fit(MultiDataSetIterator)}
   */
  public void fit(DataSet dataSet) {
    if (numInputArrays != 1 || numOutputArrays != 1)
      throw new UnsupportedOperationException(
          "Cannot train ComputationGraph network with "
              + " multiple inputs or outputs using a DataSet");

    boolean hasMaskArrays = dataSet.hasMaskArrays();
    if (hasMaskArrays) {
      INDArray[] fMask =
          (dataSet.getFeaturesMaskArray() != null
              ? new INDArray[] {dataSet.getFeaturesMaskArray()}
              : null);
      INDArray[] lMask =
          (dataSet.getLabelsMaskArray() != null
              ? new INDArray[] {dataSet.getLabelsMaskArray()}
              : null);
      fit(
          new INDArray[] {dataSet.getFeatures()},
          new INDArray[] {dataSet.getLabels()},
          fMask,
          lMask);
    } else {
  ...
  public void fit(@NonNull DataSetIterator iterator, int numEpochs) {
    Preconditions.checkArgument(
        numEpochs > 0, "Number of epochs much be > 0. Got numEpochs = %s", numEpochs);
    Preconditions.checkArgument(
        numEpochs == 1 || iterator.resetSupported(),
        "Cannot perform multiple epochs training usingiterator thas does not support resetting"
            + " (iterator.resetSupported() returned false)");
  ...
  public void fit(MultiDataSetIterator multi) {
    ...
    if (multi.asyncSupported()) {
      multiDataSetIterator =
          new AsyncMultiDataSetIterator(
              multi, Math.max(Nd4j.getAffinityManager().getNumberOfDevices() * 2, 2), true);
      destructable = true;
    ...
    if (destructable) ((AsyncMultiDataSetIterator) multiDataSetIterator).shutdown();
   ...
   * @param featureMaskArrays Mask arrays for inputs/features. Typically used for RNN training. May
   *     be null.
   * @param labelMaskArrays Mas arrays for the labels/outputs. Typically used for RNN training. May
   *     be null.
   */
  public void fit(
      INDArray[] inputs,
      ...
      workspaceMgr =
          LayerWorkspaceMgr.builder()
              ...
              .with(
                  ...
                  WS_RNN_LOOP_WORKING_MEM,
                  WS_RNN_LOOP_WORKING_MEM_CONFIG)
              // Note for updater working memory, we have the option to re-use WS_ALL_LAYERS_ACT or
              // FF/BP_WORKING_MEM
  ...
  /**
   * Calculate a topological sort order for the vertices in the graph. Note that this is used for
   * (a) working out what order to do forward pass, (b) what order to do backprop (i.e., reverse of
   * this) (c) order to flatten parameters (and gradients)
   *
   * <p>Specifically, gradients/params/forward pass are executed on vertex[topologicalSortOrder[i]],
   * for i=0..nVertices-1
   */
  ...
  public GraphIndices calculateIndices() {
    if (graphIndices != null) return graphIndices;

    // Get cached topological sort order from config, if present
    if (configuration.getTopologicalOrder() != null
        && configuration.getTopologicalOrderStr() != null) {
      ...
      graphIndices =
          GraphIndices.builder().topologicalSortOrder(t).nameToIdx(m1).idxToName(m2).build();
      return graphIndices;
    }
    ...
    // First: represent the graph more usefully as a Map<Integer,Set<Integer>>, where map represents
    // edges i -> j
    ...
    for (Map.Entry<String, org.deeplearning4j.nn.conf.graph.GraphVertex> entry :
        nodeMap.entrySet()) {
    ...
    Map<Integer, Set<Integer>> inputEdges =
        new HashMap<>(); // key: vertex. Values: vertices that the key vertex receives input from
    Map<Integer, Set<Integer>> outputEdges =
        new HashMap<>(); // key: vertex. Values: vertices that the key vertex outputs to
    ...
    for (Map.Entry<Integer, Set<Integer>> entry : inputEdges.entrySet()) {
      ...
      if (!set.isEmpty())
        throw new IllegalStateException(
            "Invalid configuration: cycle detected in graph. Cannot calculate topological ordering"
                + " with graph cycle (cycle includes vertex \""
                + vertexNamesMap.get(entry.getKey())
                + "\")");
    }

    // Store: the topological sort order in the configuraation... this is to ensure that when the
    // config is
  ...
  public void computeGradientAndScore() {
    ...
    FwdPassType fwdType =
        (tbptt ? FwdPassType.RNN_ACTIVATE_WITH_STORED_STATE : FwdPassType.STANDARD);
    synchronizeIterEpochCounts();

    // Calculate activations (which are stored in each layer, and used in backprop)
    try (MemoryWorkspace wsAllActivations =
        workspaceMgr.notifyScopeEntered(ArrayType.ACTIVATIONS)) {
      ...
      for (String s : configuration.getNetworkOutputs()) {
        ...
        if (gv instanceof LayerVertex) {
          // At this point: the input to the output layer might not be set on the layer itself -
          // just the vertex
  ...
  /**
   * Conduct forward pass using a single input array. Note that this method can only be used with
   * ComputationGraphs with a single input array.
   *
   * @param input The input array
   * @param layerTillIndex the layer to feed forward to
   * @param train If true: do forward pass at training time
   * @return A map of activations for each layer (not each GraphVertex). Keys = layer name, values =
   *     layer activations
   */
  ...
  /**
   * Conduct forward pass using an array of inputs. This overload allows the forward pass to be
   * conducted, optionally (not) clearing the layer input arrays.<br>
   * Note: when using clearInputs=false, there can be some performance and memory overhead: this is
   * because the arrays are defined outside of workspaces (which are enabled by default) -
   * otherwise, old/invalidated arrays could still be accessed after calling this method.
   * Consequently: Don't use clearInputs=false unless you have a use case that requires them to
   * remain after feed-forward has been completed
   *
   ...
   * @param clearInputs If true (default for other methods): clear the inputs of all layers after
   *     doing forward pass. False don't clear layer inputs.
   * @return A map of activations for each layer (not each GraphVertex). Keys = layer name, values =
   *     layer activations
   */
  public Map<String, INDArray> feedForward(
      INDArray[] input, int layerTillIndex, boolean train, boolean clearInputs) {
    setInputs(input);
    try {
   ...
   * Conduct forward pass using an array of inputs. This overload allows the forward pass to be
   * conducted, optionally (not) clearing the layer input arrays.<br>
   * Note: this method should NOT be used with clearInputs = true, unless you know what you are
   * doing. Specifically: when using clearInputs=false, in combination with workspaces, the layer
   * input fields may leak outside of the workspaces in which they were defined - potentially
   * causing a crash. See <a
   * href="https://deeplearning4j.konduit.ai/config/config-memory/config-workspaces">
   * https://deeplearning4j.konduit.ai/config/config-memory/config-workspaces</a> for more details
   *
   * @param input An array of ComputationGraph inputs
   * @param train If true: do forward pass at training time; false: do forward pass at test time
   * @param clearInputs If true (default for other methods): clear the inputs of all layers after
   ...
   * @param excludeOutputLayers Should we exclude the output layers during forward pass? (usually:
   *     false)
   ...
   */
  public Map<String, INDArray> feedForward(
      boolean train, boolean excludeOutputLayers, boolean includeNonLayerVertexActivations) {
    int[] exclude = null;
    ...
    if (includeNonLayerVertexActivations) {
      ...
      // Include only layers - in previous versions, we've always included inputs too for this
      // method...
  ...
  /**
   * Return an array of network outputs (predictions) at test time, given the specified network
   * inputs Network outputs are for output layers only.
   *
  ...
  /**
   * A convenience method that returns a single INDArray, instead of an INDArray[]. Useful for
   * ComputationGraphs that have only a single output. Otherwise identical to {@link
   * #output(INDArray...)}
   *
   * @param input Inputs to the network
  ...
  /**
   * Return an array of network outputs (predictions), given the specified network inputs Network
   * outputs are for output layers only.
   *
   * @param train If true: do forward pass at training time; false: do forward pass at test time
   * @param input Inputs to the network
   * @return Output activations (order: same as defined in network configuration)
  ...
  /**
   * Return an array of network outputs (predictions), given the specified network inputs Network
   * outputs are for output layers only.<br>
   * If no memory workspace is provided, the output will be detached (not in any workspace).<br>
   * If a memory workspace is provided, the output activation array (i.e., the INDArray returned by
   * this method) will be placed in the specified workspace. This workspace must be opened by the
   * user before calling this method - and the user is responsible for (a) closing this workspace,
   * and (b) ensuring the output array is not used out of scope (i.e., not used after closing the
  ...
  public INDArray[] output(
      boolean train, @NonNull INDArray[] input, INDArray[] inputMasks, INDArray[] labelMasks) {
   ...
   *
   * <p>PLEASE NOTE: This method uses dedicated Workspace for output generation to avoid redundant
   * allocations
   *
  ...
  public <T> T output(
    ...
    try (val ws =
      ...
      else return outputAdapter.apply(output(false, inputs, inputMasks, labelMasks, ws));
    }
  }
  ...
  /**
   * Identical to {@link #outputSingle(boolean, boolean, INDArray...)} but has the option of not
   * clearing the input arrays (useful when later backpropagating external errors). Most users
   * should use {@link #outputSingle(boolean, INDArray...)} in preference to this method.
   */
  public INDArray outputSingle(boolean train, boolean clearInputs, INDArray... input) {
    if (numOutputArrays != 1) {
      throw new IllegalStateException(
          "Cannot use outputSingle with ComputationGraph that does not have exactly 1 output."
              + " nOutputs: "
              + numOutputArrays);
    }
   ...
   * Note: most users should use {@link #output(boolean, INDArray...)} or similar methods, unless
   * they are doing non-standard operations (like providing the input arrays externally)
   *
   * @param train If true: output during training. False: output during testing. Affects some things
   *     such as dropout
   * @param clearInputs If true: clear the input arrays for all layers. False: leave the input
   *     arrays as-is - which can be useful for "external errors" (no output layer) backprop use
   *     cases
  ...
  public INDArray[] output(boolean train, boolean clearInputs, INDArray... input) {
    boolean detachedInputs =
        !clearInputs; // If !clearInputs, then inputs should be detached (otherwise: will be out of
                      // scope)
    try {
   ...
   * single array per network output
   *
  ...
  public INDArray[] output(MultiDataSetIterator iterator) {
    ...
    while (iterator.hasNext()) {
      ...
      INDArray[] out =
          output(
              false, next.getFeatures(), next.getFeaturesMaskArrays(), next.getLabelsMaskArrays());
      outputs.add(out);
    }
  ...
  /**
   * Generate the output for all examples/batches in the input iterator, and concatenate them into a
   * single array. Can only be used with ComputationGraphs with 1 output
   *
   * @param iterator Data to pass through the network
   * @return output for all examples in the iterator
   */
  public INDArray outputSingle(DataSetIterator iterator) {
    Preconditions.checkArgument(
        numOutputArrays == 1,
        "Cannot use this method with nets that have more"
            + " than 1 output array. This network has %s outputs",
        numOutputArrays);
   ...
   */
  public INDArray[] output(
      List<String> layers, boolean train, INDArray[] features, INDArray[] featureMasks) {
    Preconditions.checkState(
        layers != null && layers.size() > 0, "Layers must not be null: got later names %s", layers);
    int[] layerNums = new int[layers.size()];
    for (int i = 0; i < layers.size(); i++) {
      String n = layers.get(i);
      Preconditions.checkState(
          verticesMap.containsKey(n), "Layer with name %s not found in network", n);
  ...
  protected void validateArrayWorkspaces(
    ...
    try {
      ...
      throw new IllegalStateException(
          op
              + ": array ("
              + arrayType
              + ") workspace validation failed (vertex "
              + vertexName
              + " - class: "
              + clazz
              + ") - array is defined in incorrect workspace",
          e);
    }
  }
   ...
   * Note that no workspace should be active externally when calling this method (an exception will
   * be thrown if a workspace is open externally)
   *
   * @param train Training mode (true) or test/inference mode (false)
   * @param fwdPassType Type of forward pass to perform (STANDARD or RNN_ACTIVATE_WITH_STORED_STATE
   *     only)
   ...
   * @param excludeIdxs Layers (vertices) to exclude from forward pass. These layers will be
   *     skipped, and hence are usually output layers or at the end of the network. May be null.
   ...
   */
  protected Map<String, INDArray> ffToLayerActivationsDetached(
      boolean train,
    ...
    if (layerIndex < 0 || layerIndex >= topologicalOrder.length) {
      throw new IllegalArgumentException(
          "Invalid layer index - index must be >= 0 and < "
    ...
    WorkspaceUtils.assertNoWorkspacesOpen(
        "Expected no workspace active before call to ffToLayerActivationsDetached", true);
    ...
    for (int i = 0; i <= layerIndex; i++) {
      ...
      if (traceLog) {
        log.trace(
            "About forward pass: {} (\"{}\") - {}", i, vName, current.getClass().getSimpleName());
      }
      ...
      if (current.isInputVertex()) {
          ...
          if (current.hasLayer()) {
            ...
            if (l instanceof RecurrentLayer) {
              out = ((RecurrentLayer) l).rnnTimeStep(reshapeTimeStepInput(input), workspaceMgr);
                ...
                && ((org.deeplearning4j.nn.layers.wrapper.BaseWrapperLayer) l).getUnderlying()
                    instanceof RecurrentLayer) {
              RecurrentLayer rl =
                  ((RecurrentLayer)
                      ((org.deeplearning4j.nn.layers.wrapper.BaseWrapperLayer) l).getUnderlying());
              ...
              out =
                  ((RecurrentLayer) l)
                      .rnnActivateUsingStoredState(
                          current.getInputs()[0], train, storeLastForTBPTT, workspaceMgr);
            } else if (l instanceof org.deeplearning4j.nn.layers.wrapper.BaseWrapperLayer
                && ((org.deeplearning4j.nn.layers.wrapper.BaseWrapperLayer) l).getUnderlying()
                    instanceof RecurrentLayer) {
              RecurrentLayer rl =
                  (RecurrentLayer)
                      ((org.deeplearning4j.nn.layers.wrapper.BaseWrapperLayer) l).getUnderlying();
              out =
                  rl.rnnActivateUsingStoredState(
                      current.getInputs()[0], train, storeLastForTBPTT, workspaceMgr);
            } else if (l instanceof MultiLayerNetwork) {
              List<INDArray> temp =
                  ((MultiLayerNetwork) l)
        ...
        validateArrayWorkspaces(
            workspaceMgr, out, ArrayType.ACTIVATIONS, vName, false, "Feed forward (inference)");
      }

      activations.put(current.getVertexName(), out.detach());

      if (inputsTo
          != null) { // May be null for output vertices (which don't feed into any other vertices)
        for (VertexIndices v : inputsTo) {
          // Note that we don't have to do anything special here: the activations are always
          // detached in
          // this method
  ...
  /**
   * Feed-forward through the network - if workspaces are used, all returned activations will be
   * present in workspace WS_ALL_LAYERS_ACT.<br>
   * Note: if using workspaces for training, requires that WS_ALL_LAYERS_ACT is open externally. If
   * using NO workspaces, requires that no external workspace is open
   *
   * @param train Training mode (true) or test/inference mode (false)
   ...
   * @return Map of activations (including the input), in workspace WS_ALL_LAYERS_ACT if workspaces
   *     are used (detached otherwise)
   */
  protected Map<String, INDArray> ffToLayerActivationsInWS(
      boolean train,
      int layerIndex,
      int[] excludeIdxs,
    ...
    if (layerIndex != -1 && (layerIndex < 0 || layerIndex >= topologicalOrder.length)) {
      throw new IllegalArgumentException(
          "Invalid input index - index must be >= 0 and < "
              + topologicalOrder.length
              + ", got index "
              + layerIndex);
    }
    ...
    if (wsm == WorkspaceMode.NONE) {
      // Verify that no workspace is open externally
      WorkspaceUtils.assertNoWorkspacesOpen(
          "Expected no workspace active in ffToLayerActivationsDetached", true);

      workspaceMgr = LayerWorkspaceMgr.noWorkspaces();
    } else {
      WorkspaceUtils.assertOpenAndActive(
          WS_ALL_LAYERS_ACT,
    ...
    for (int i = 0; i <= stopIndex; i++) {
      ...
      try (MemoryWorkspace wsFFWorking =
          workspaceMgr.notifyScopeEntered(ArrayType.FF_WORKING_MEM)) {
  ...
  /**
   * Provide the output of the specified layers, detached from any workspace. This is most commonly
   * used at inference/test time, and is more memory efficient than {@link
   * #ffToLayerActivationsDetached(boolean, FwdPassType, boolean, int, int[], INDArray[],
   * INDArray[], INDArray[], boolean)} and {@link #ffToLayerActivationsInWS(boolean, int, int[],
   * FwdPassType, boolean, INDArray[], INDArray[], INDArray[], boolean)}.<br>
   * This method clears all layer inputs.
   *
   * <p>NOTE: in general, no workspaces should be activated externally for this method! This method
   ...
   * @param detachedInputs If true: the layer input fields will be detached. Usually used for
   *     external errors cases
   ...
   */
  protected INDArray[] outputOfLayersDetached(
      boolean train,
      @NonNull FwdPassType fwdPassType,
    ...
    if (features.length != numInputArrays) {
      throw new IllegalArgumentException(
          "Invalid number of input arrays: network has "
              + numInputArrays
              + " inputs, got "
              + features.length
              + " input arrays");
    ...
    if (outputWorkspace == null || outputWorkspace instanceof DummyWorkspace) {
      ...
      Preconditions.checkState(
          outputWorkspace.isScopeActive(),
          "Workspace \""
              + outputWorkspace.getId()
              + "\" was provided for the network/layer outputs. When provided, this workspace must"
              + " be opened before calling the output method; furthermore, closing the workspace is"
    ...
    // Then: for each vertex, determine the forward pass step that each vertex's output has been
    // fully consumed on
    // In other words, if vertex X -> Y and X -> Z, and topological sort order is X,a,Y,b,Z,
    // Then we know X's output activations have been fully consumed by step index 4 in the
    // topological sort
    // thus vertexOutputsFullyConsumedByStep[X.index] = IndexOf(topologicalSort, Z.index)

    // Position in array: index of vertex. Value at position: the step (in topological order) that
    // the activations
    // have been consumed by
    // Put another way: this is the step that it's safe to deallocate the layer's activations by
    // closing the
    ...
    for (GraphVertex gv : vertices) {
      ...
      if (outputsTo != null) {
        ...
        for (VertexIndices vi : outputsTo) {
          ...
          if (posInTopoSort == -1) {
            throw new IllegalStateException(
                "Did not find vertex " + vi.getVertexIndex() + " in topological sort array");
          }
          ...
          if (freeWorkspaceManagers.size() > 0) {
            ...
            workspaceMgr =
                LayerWorkspaceMgr.builder()
                    ...
                    .with(
                        ArrayType.FF_WORKING_MEM, WS_LAYER_WORKING_MEM, WS_LAYER_WORKING_MEM_CONFIG)
                    .with(
                        ArrayType.RNN_FF_LOOP_WORKING_MEM,
                        WS_RNN_LOOP_WORKING_MEM,
                        WS_RNN_LOOP_WORKING_MEM_CONFIG)
            ...
            if (detachedInputs) {
              // Sometimes (like: external errors use cases) we don't want the activations/inputs to
              // be
              // in a workspace
        ...
        if (current.isInputVertex()) {
          ...
          if (fwdPassType == FwdPassType.STANDARD) {
            ...
            if (i > 0
                ...
                && ConvolutionUtils.layerHasConvolutionLayout(
                    current.getLayer().conf().getLayer())) {

              /**
               * Not QUITE the proper fix, but getting close. Able to detect this happens mid graph
               * and do something about it. Need to play with output sizes a bit to make sure we put
               * the right parameters in there to get correct behavior.
               */
              CNN2DFormat preLayerFormat =
                  ConvolutionUtils.getFormatForLayer(prev.getLayer().conf().getLayer());
              CNN2DFormat currLayerFormat =
                  ConvolutionUtils.getFormatForLayer(current.getLayer().conf().getLayer());
              if (preLayerFormat != currLayerFormat) {
                int inputIdx = -1;
                for (int inputVertex = 0;
                    inputVertex < current.getInputVertices().length;
                    inputVertex++) {
                  if (current.getInputVertices()[inputVertex].getVertexIndex()
                      == prev.getVertexIndex()) {
                ...
                if (preLayerFormat == CNN2DFormat.NCHW) {
                  current.setInput(
                      inputIdx, current.getInputs()[inputIdx].permute(0, 3, 1, 2), workspaceMgr);
                }
                // NCHW case
                else if (preLayerFormat == CNN2DFormat.NHWC) {
                  current.setInput(
                      inputIdx, current.getInputs()[inputIdx].permute(0, 2, 3, 1), workspaceMgr);
                ...
                out = current.doForward(train, workspaceMgr);
              } else out = current.doForward(train, workspaceMgr);
            } else if (i > 0
                && current.hasLayer()
                && prev.hasLayer()
              ...
              RNNFormat preLayerFormat =
                  Convolution1DUtils.getRnnFormatFromLayer(prev.getLayer().conf().getLayer());
              RNNFormat currLayerFormat =
                  Convolution1DUtils.getRnnFormatFromLayer(current.getLayer().conf().getLayer());
              int inputIdx = -1;
              for (int inputVertex = 0;
                  inputVertex < current.getInputVertices().length;
                  inputVertex++) {
              ...
              if (preLayerFormat != currLayerFormat) {
                current.setInput(
                    inputIdx, current.getInputs()[inputIdx].permute(0, 2, 1), workspaceMgr);
              }
              out = current.doForward(train, workspaceMgr);
            ...
            if (current.hasLayer()) {
              ...
              if (l instanceof RecurrentLayer) {
                ...
                RecurrentLayer rl =
                    ((RecurrentLayer)
                        ((org.deeplearning4j.nn.layers.wrapper.BaseWrapperLayer) l)
                            .getUnderlying());
                out = rl.rnnTimeStep(reshapeTimeStepInput(input), workspaceMgr);
              } else if (l instanceof MultiLayerNetwork) {
                out = ((MultiLayerNetwork) l).rnnTimeStep(reshapeTimeStepInput(input));
  ...
  /**
   * Calculate the gradient of the network with respect to some external errors. Note that this is
   * typically used for things like reinforcement learning, not typical networks that include an
   * OutputLayer or RnnOutputLayer
   *
   * @param epsilons Epsilons (errors) at the output. Same order with which the output layers are
   *     defined in configuration setOutputs(String...)
  ...
  public Gradient backpropGradient(INDArray... epsilons) {
    ...
    try {
      calcBackpropGradients(
          true, configuration.getBackpropType() == BackpropType.TruncatedBPTT, epsilons);
      return gradient;
    } catch (OutOfMemoryError e) {
      CrashReportingUtil.writeMemoryCrashDump(this, e);
      throw e;
   ...
   *
   * @param truncatedBPTT false: normal backprop. true: calculate gradients using truncated BPTT for
   *     RNN layers
   * @param externalEpsilons null usually (for typical supervised learning). If not null (and length
   *     > 0) then assume that the user has provided some errors externally, as they would do for
   *     example in reinforcement learning situations.
   */
  protected void calcBackpropGradients(
      boolean clearLayers, boolean truncatedBPTT, INDArray... externalEpsilons) {
    if (flattenedGradients == null) {
      initGradientsView();
    }
    ...
    */
    ...
    if (externalEpsilons != null && externalEpsilons.length > 0) {
      ...
      for (String s : outputLayers) {
        ...
        if (gv instanceof LayerVertex && ((LayerVertex) gv).getLayer() instanceof IOutputLayer) {
          throw new IllegalStateException(
              "Cannot perform backprop with external errors in conjunction with an output layer:"
                  + " output layers cannot use external errors for backprop. Layer name: "
                  + s);
        }
      }
    }
    ...
    // Put another way: this is the step that it's safe to deallocate the layer's activation
    // gradients by closing the
    // corresponding workspace
    ...
    List<MemoryWorkspace>[] closeAtEndIteraton =
        (List<MemoryWorkspace>[]) new List[topologicalOrder.length];

    // Do backprop, in reverse topological order
    LinkedList<Triple<String, INDArray, Character>> gradients = new LinkedList<>();
    boolean[] setVertexEpsilon =
        new boolean
            [topologicalOrder
                .length]; // If true: already set epsilon for this vertex; later epsilons should be
                          // *added* to the existing one, not set
    MemoryWorkspace initialWorkspace = Nd4j.getMemoryManager().getCurrentWorkspace();
    ...
    try {
      for (int i = topologicalOrder.length - 1; i >= 0; i--) {
        ...
        if (current.hasLayer() && current.getLayer() instanceof FrozenLayer
            || current instanceof FrozenVertex) {
          hitFrozen = true;
        }

        // First: determine what workspace manager we should use for the activation gradients from
        // this vertex
        LayerWorkspaceMgr workspaceMgr;
        if (noWS) {
          workspaceMgr = allNone;
            ...
            workspaceMgr =
                LayerWorkspaceMgr.builder()
                    ...
                    .with(
                        ...
                        WS_LAYER_WORKING_MEM_CONFIG) // For forward pass in the context of BP
                    .with(
                        ArrayType.FF_WORKING_MEM, WS_LAYER_WORKING_MEM, WS_LAYER_WORKING_MEM_CONFIG)
                    .with(
                        ArrayType.BP_WORKING_MEM, WS_LAYER_WORKING_MEM, WS_LAYER_WORKING_MEM_CONFIG)
                    .with(
                        ArrayType.RNN_FF_LOOP_WORKING_MEM,
                        WS_RNN_LOOP_WORKING_MEM,
                        WS_RNN_LOOP_WORKING_MEM_CONFIG)
        ...
        if (current.isOutputVertex()) {
          ...
          // (b) it's a normal layer, but it has been marked as an output layer for use in external
          // errors - for reinforcement learning, for example
          ...
          if (currentLayer instanceof IOutputLayer) {
            ...
            if ((externalEpsilons == null || externalEpsilons.length == 0)
              ...
              throw new DL4JException(
                  "Layer \""
                      + current.getVertexName()
                      + "\" of type "
                      + current.getLayer().getClass().getSimpleName()
                      + " is set as network output (but isn't an IOutputLayer). Only IOutputLayer"
                      + " layers can be fit via backprop with a labels array. ");
            }
        ...
        // Note that this will be closed only once the current vertex's activations have been
        // consumed
        MemoryWorkspace wsActivationGrads =
            workspaceMgr.notifyScopeEntered(ArrayType.ACTIVATION_GRAD);
        openActivationsWorkspaces.put(wsActivationGrads, workspaceMgr);

        // Note that because we're opening activation gradient workspaces not in any defined order
        // (i.e., workspace
        // use isn't simply nested), we'll manually override the previous workspace setting.
        // Otherwise, when we
        ...
        INDArray[] epsilons;
        try (MemoryWorkspace wsWorkingMem =
            workspaceMgr.notifyScopeEntered(ArrayType.BP_WORKING_MEM)) {
        ...
        if (pair.getFirst() != null) {
          ...
          for (Triple<String, INDArray, Character> triple : tempList) gradients.addFirst(triple);
        }
  ...
  public ComputationGraph clone() {
    ...
    if (solver != null) {
      // If  solver is null: updater hasn't been initialized -> getUpdater call will force
  ...
  public void setListeners(TrainingListener... listeners) {
    ...
    // Check: user might have done setListeners(null) thinking this would clear the current
    // listeners.
  ...
  public void addListeners(TrainingListener... listeners) {
    if (this.trainingListeners == null) {
      ...
      List<TrainingListener> newListeners =
          new ArrayList<>(this.trainingListeners); // To avoid immutable list issues
   ...
   *
   * @param initializeIfAbsent If true: create the updater if one is absent. False: return null if
   *     absent.
   * @return Updater
   */
  ...
  /**
   * Get the specified output layer, by index. The index of the output layer may be 0 to {@link
   * #getNumOutputArrays()}-1
   */
  public Layer getOutputLayer(int outputLayerIdx) {
    if (outputLayerIdx >= numOutputArrays)
      throw new IllegalArgumentException(
          "Invalid index: cannot get output layer "
              + outputLayerIdx
  ...
  /**
   * Sets the input and labels and returns a score for the prediction with respect to the true
   * labels<br>
   * This is equivalent to {@link #score(DataSet, boolean)} with training==true.<br>
   * <b>NOTE:</b> this version of the score function can only be used with ComputationGraph networks
   * that have a single input and a single output.
   *
   ...
   * labels<br>
   * <b>NOTE:</b> this version of the score function can only be used with ComputationGraph networks
   * that have a single input and a single output. Use {@link #score(MultiDataSet, boolean)} for
  ...
  private double scoreHelper(MultiDataSet dataSet, boolean training) {
    ...
    Map<String, INDArray> stringINDArrayMap =
        ffToLayerActivationsDetached(
            training,
            FwdPassType.STANDARD,
            false,
   ...
   * for example for autoencoder architectures and the like.<br>
   * Each row of the output (assuming addRegularizationTerms == true) is equivalent to calling
   * score(DataSet) with a single example.
   *
  ...
  /**
   * Calculate the score for each example in a DataSet individually. Unlike {@link
   * #score(MultiDataSet)} and {@link #score(MultiDataSet, boolean)} this method does not
   * average/sum over examples. This method allows for examples to be scored individually (at test
   * time only), which may be useful for example for autoencoder architectures and the like.<br>
   * Each row of the output (assuming addRegularizationTerms == true) is equivalent to calling
   * score(MultiDataSet) with a single example.
   *
   * @param dataSet The data to score
   * @param addRegularizationTerms If true: add l1/l2 regularization terms (if any) to the score. If
   *     false: don't add regularization terms
  ...
  private INDArray scoreExamplesHelper(MultiDataSet dataSet, boolean addRegularizationTerms) {
    ...
    for (String s : configuration.getNetworkOutputs()) {
      ...
      if (out == null) out = scoreCurrLayer.detach();
    ...
    if (dataSet.hasMaskArrays()) clearLayerMaskArrays();
    clearLayersStates();
    return out;
  }
  ...
  public INDArray params() {
    if (flattenedParams == null) return Nd4j.zeros(DataType.FLOAT, 0);
  ...
  public void setBackpropGradientsViewArray(INDArray gradient) {
    ...
    for (int i = 0; i < topologicalOrder.length; i++) {
      ...
      long range = layer.numParams();
      if (range <= 0) continue; // Some layers: no parameters (subsampling etc)
      layer.setBackpropGradientsViewArray(
          gradientReshape.get(NDArrayIndex.interval(paramsSoFar, paramsSoFar + range)));
      paramsSoFar += range;
    }
  }
  ...
  public void fit(INDArray data, LayerWorkspaceMgr workspaceMgr) {
    throw new UnsupportedOperationException(
        "Cannot pretrain ComputationGraph with single INDArray");
  }
  ...
  public INDArray getParam(String paramName) {
    ...
    if (idx == -1)
      throw new IllegalStateException(
          "Invalid param key: not have layer separator: \"" + paramName + "\"");
  ...
  public void setParamTable(@NonNull Map<String, INDArray> paramTable) {
    ...
    Preconditions.checkArgument(
        paramTable.keySet().equals(m.keySet()),
        "Cannot set param table: parameter set keys are not equal");
    ...
    for (String s : current.keySet()) {
      ...
      Preconditions.checkState(
          Arrays.equals(shapeCurrent, shapeNew),
          "Cannot set parameters: shape array for parameter \"%s\" does not match existing shape:"
              + " parameter shape = %s, new param shape = %s",
          s,
          shapeCurrent,
          arrNew);
    }
  ...
  /**
   * If this ComputationGraph contains one or more RNN layers: conduct forward pass (prediction) but
   * using previous stored state for any RNN layers. The activations for the final step are also
   * stored in the RNN layers for use next time rnnTimeStep() is called.<br>
   * This method can be used to generate output one or more steps at a time instead of always having
   * to do forward pass from t=0. Example uses are for streaming data, and for generating samples
   * from network output one step at a time (where samples are then fed back into the network as
   * input)<br>
   * If no previous state is present in RNN layers (i.e., initially or after calling
   * rnnClearPreviousState()), the default initialization (usually 0) is used.<br>
   * Supports mini-batch (i.e., multiple predictions/forward pass in parallel) as well as for single
   * examples.<br>
   *
   * @param inputs Input to network. May be for one or multiple time steps. For single time step:
   *     input has shape [miniBatchSize,inputSize] or [miniBatchSize,inputSize,1]. miniBatchSize=1
   *     for single example.<br>
   *     For multiple time steps: [miniBatchSize,inputSize,inputTimeSeriesLength]
   * @return Output activations. If output is RNN layer (such as RnnOutputLayer): if all inputs have
   *     shape [miniBatchSize,inputSize] i.e., is 2d, then outputs have shape
   *     [miniBatchSize,outputSize] (i.e., also 2d) instead of [miniBatchSize,outputSize,1].<br>
   *     Otherwise output is 3d [miniBatchSize,outputSize,inputTimeSeriesLength] when using
   *     RnnOutputLayer (or unmodified otherwise).
   */
  ...
  /**
   * Get a map of states for ALL RNN layers, as used in {@link #rnnTimeStep(INDArray...)}. Layers
   * that are not RNN layers will not have an entry in the returned map
   *
   ...
   *
   * @param previousStates The previous time step states for all layers (key: layer name. Value:
   *     layer states)
  ...
  protected void doTruncatedBPTT(
      INDArray[] inputs,
      INDArray[] labels,
    ...
    // Approach used here to implement truncated BPTT: if input is 3d, split it. Otherwise: input is
    // unmodified
    ...
    for (int i = 0; i < nSubsets; i++) {
      ...
      if (startTimeIdx > Integer.MAX_VALUE) throw new ND4JArraySizeException();
      List<INDArray[]> list =
          getSubsetsForTbptt(
              (int) startTimeIdx, endTimeIdx, inputs, labels, featureMasks, labelMasks);
  ...
  private List<INDArray[]> getSubsetsForTbptt(
      int startTimeIdx,
      long endTimeIdx,
      INDArray[] inputs,
      INDArray[] labels,
    ...
    if (featureMasks != null) {
      for (int j = 0; j < featureMasks.length; j++) {
        ...
        newFeatureMasks[j] =
            featureMasks[j].get(
                NDArrayIndex.all(), NDArrayIndex.interval(startTimeIdx, endTimeIdx));
      }
    }
    if (labelMasks != null) {
      for (int j = 0; j < labelMasks.length; j++) {
        ...
        newLabelMasks[j] =
            labelMasks[j].get(NDArrayIndex.all(), NDArrayIndex.interval(startTimeIdx, endTimeIdx));
      }
    }
   ...
   */
  public Map<String, INDArray> rnnActivateUsingStoredState(
      INDArray[] inputs, boolean training, boolean storeLastForTBPTT) {
    return ffToLayerActivationsDetached(
        training,
   ...
   * supporting time series of varying lengths within the same minibatch.<br>
   * For example, with RNN data sets with input of shape [miniBatchSize,nIn,timeSeriesLength] and
   * outputs of shape [miniBatchSize,nOut,timeSeriesLength], the features and mask arrays will have
   * shape [miniBatchSize,timeSeriesLength] and contain values 0 or 1 at each element (to specify
   * whether a given input/example is present - or merely padding - at a given time step).<br>
   * <b>NOTE</b>: This method is not usually used directly. Instead, the various feedForward and fit
   * methods handle setting of masking internally.
   *
  ...
  public void setLayerMaskArrays(INDArray[] featureMaskArrays, INDArray[] labelMaskArrays) {
    ...
    if (featureMaskArrays != null) {
      ...
      // Here: need to do forward pass through the network according to the topological ordering of
      // the network
      ...
      for (int i = 0; i < topologicalOrder.length; i++) {
        ...
        if (current.isInputVertex()) {
          ...
          if (minibatchSize > Integer.MAX_VALUE) throw new ND4JArraySizeException();
  ...
  /**
   * Evaluate the network on the provided data set (single output ComputationGraphs only). Used for
   ...
   *     DataSetIterator
   */
  public <T extends Evaluation> T evaluate(
      DataSetIterator iterator, List<String> labelsList, int topN) {
   ...
   */
  public <T extends Evaluation> T evaluate(
      MultiDataSetIterator iterator, List<String> labelsList, int topN) {
    Layer outputLayer = getOutputLayer(0);
    if (getConfiguration().isValidateOutputLayerConfig()) {
      OutputLayerUtil.validateOutputLayerForClassifierEvaluation(
          outputLayer.conf().getLayer(), Evaluation.class);
   ...
   * @return Regression evaluation
   */
  public <T extends RegressionEvaluation> T evaluateRegression(
      DataSetIterator iterator, List<String> columnNames) {
    return (T)
        doEvaluation(iterator, new org.deeplearning4j.eval.RegressionEvaluation(columnNames))[0];
  }
   ...
   */
  public <T extends RegressionEvaluation> T evaluateRegression(
      MultiDataSetIterator iterator, List<String> columnNames) {
    return (T)
        doEvaluation(iterator, new org.deeplearning4j.eval.RegressionEvaluation(columnNames))[0];
  }
   ...
   */
  public <T extends ROCMultiClass> T evaluateROCMultiClass(
      DataSetIterator iterator, int rocThresholdSteps) {
    Layer outputLayer = getOutputLayer(0);
    if (getConfiguration().isValidateOutputLayerConfig()) {
      OutputLayerUtil.validateOutputLayerForClassifierEvaluation(
    ...
    return (T)
        doEvaluation(iterator, new org.deeplearning4j.eval.ROCMultiClass(rocThresholdSteps))[0];
  }
   ...
   */
  public <T extends ROCMultiClass> T evaluateROCMultiClass(
      MultiDataSetIterator iterator, int rocThresholdSteps) {
    Layer outputLayer = getOutputLayer(0);
    if (getConfiguration().isValidateOutputLayerConfig()) {
      OutputLayerUtil.validateOutputLayerForClassifierEvaluation(
          outputLayer.conf().getLayer(), ROCMultiClass.class);
  ...
  /**
   * Perform evaluation on the given data (DataSetIterator) with the given {@link IEvaluation}
   * instance
   *
  ...
  /**
   * Perform evaluation on the given data (MultiDataSetIterator) with the given {@link IEvaluation}
   * instance
   *
   * @param iterator Test data to evaluate on
   ...
   * @param evaluations Evaluation instances. Key: the network output number (0 to numOutputs-1).
   *     Value: the IEvaluation instances to perform evaluation with, for that output only. Note
   *     that not every output needs to have an IEvaluation[] defined.
   * @return The same evaluation map, after performing evaluation
   */
  public <T extends IEvaluation> Map<Integer, T[]> evaluate(
      DataSetIterator iterator, Map<Integer, T[]> evaluations) {
   ...
   */
  public <T extends IEvaluation> Map<Integer, T[]> evaluate(
      MultiDataSetIterator iterator, Map<Integer, T[]> evaluations) {
    try {
      return doEvaluationHelper(iterator, evaluations);
    } catch (OutOfMemoryError e) {
      CrashReportingUtil.writeMemoryCrashDump(this, e);
  ...
  @SafeVarargs
  private final <T extends IEvaluation> T[] doEvaluationHelper(
      MultiDataSetIterator iterator, T... evaluations) {
  ...
  private <T extends IEvaluation> Map<Integer, T[]> doEvaluationHelper(
      MultiDataSetIterator iterator, Map<Integer, T[]> evaluations) {
    ...
    WorkspaceUtils.assertNoWorkspacesOpen(
        "Expected no external workspaces open at start of evaluation (doEvaluationHelper)");

    if (iterator.resetSupported() && !iterator.hasNext()) iterator.reset();
    ...
    while (iter.hasNext()) {
      ...
      if (!useRnnSegments) {
        ...
        try (MemoryWorkspace ws = outputWs.notifyScopeEntered()) {
          ...
          for (Integer i : evaluations.keySet()) {
            Preconditions.checkState(
                i >= 0 && i < labels.length,
                "Invalid output index: evaluation/output indices must be between 0"
                    + " and numOutputs-1 (%s), got index %s",
                numOutputArrays,
                (int) i);
            ...
            Preconditions.checkState(
                i >= 0 && i < getNumOutputArrays(),
                "Invalid output index: indices for outputs "
                    + "must be between 0 and %s inclusive - found index %s",
                numOutputArrays,
                (int) i);
        ...
        if (tsLength < 0) {
          throw new IllegalStateException(
              "Invalid configuration: detected TBPTT backprop type without"
        ...
        for (int i = 0; i < nSubsets; i++) {
          ...
          List<INDArray[]> subset =
              getSubsetsForTbptt(
                  startTimeIdx,
                  endTimeIdx,
                  next.getFeatures(),
                  next.getLabels(),
                  next.getFeaturesMaskArrays(),
      ...
      // Clear inputs, masks etc. Important to avoid leaking invalidated/out of scope arrays between
      // iterations
      clearLayersStates();
    }

    if (iterator.asyncSupported()) ((AsyncMultiDataSetIterator) iter).shutdown();
  ...
  /**
   * String detailing the architecture of the computation graph. Vertices are printed in a
   * topological sort order. Columns are Vertex Names with layer/vertex type, nIn, nOut, Total
   * number of parameters and the Shapes of the parameters And the inputs to the vertex Will also
   * give information about frozen layers/vertices, if any.
   *
  ...
  /**
   * String detailing the architecture of the computation graph. Will also display activation size
   * when given an input type. Vertices are printed in a topological sort order. Columns are Vertex
   * Names with layer/vertex type, nIn, nOut, Total number of parameters and the Shapes of the
   * parameters And the inputs to the vertex Will also give information about frozen
   * layers/vertices, if any.
   *
   * @return Summary as a string
   * @see #memoryInfo(int, InputType...)
   */
      ...
      if (currentVertex.isInputVertex()) {
          ...
          vertexOutputs.put(
              ...
              inputTypes[
                  configuration
                      .getNetworkInputs()
                      .indexOf(
                          currentVertexName)]); // for input vertices the outputs are just the input
                                                // types (only layer vertices have preprocessing?)
      } else {
        ...
        if (currentVertex.hasLayer()) {
            ...
            if (currentLayer instanceof BidirectionalLayer) { // Bidirectional layer is not an FFL
              ...
              try {
                ...
                out = String.valueOf(((FeedForwardLayer) currentLayer.conf().getLayer()).getNOut());
                  ...
                  Exception
                      e) { // Some layers, like PReLU, are just BaseLayers (but have parameters)
              }
            }
          ...
          if (currentLayer instanceof FrozenLayer) {
            ...
            classNameArr =
                ((FrozenLayer) currentLayer).getInsideLayer().getClass().getName().split("\\.");
        ...
        if (inputTypes != null) {
          InputType currentVertexOutputType =
              configuration
                  .getVertices()
                  .get(currentVertexName)
                  .getOutputType(
                      currLayerIdx, inputTypeList.toArray(new InputType[inputTypeList.size()]));
  ...
  /**
   * Generate information regarding memory use for the network, for the given input types and
   ...
   *
   * <p>Note also that this is the same information that is generated during an OOM crash when
   * training or performing inference.
   *
  ...
  /**
   * Increment the epoch count (in the underlying {@link ComputationGraphConfiguration} by 1). Note
   * that this is done <i>automatically</i> when using iterator-based fitting methods, such as
   * {@link #fit(DataSetIterator)} or {@link #fit(MultiDataSet)}. However, when using non-iterator
   * fit methods (DataSet, MultiDataSet, INDArrays etc), the network has no way to know when one
   * epoch ends and another starts. In such situations, this method can be used to increment the
   * epoch counter.<br>
   * Note that the epoch counter is used for situations such as some learning rate schedules, and
   * the like.
   *
   * <p>The current epoch count can be obtained using {@code
   * ComputationGraph.getConfiguration().getEpochCount()}
   */
  ...
  /**
   * Returns the number of epochs that the ComputationGraph has done. Note that the epoch count is
   * incremented only when {@link #fit(DataSetIterator)}, {@link #fit(MultiDataSetIterator)}, {@link
   * #fit(DataSetIterator, int)} or {@link #fit(MultiDataSetIterator, int)} are used. The epoch
   * count can also be manually incremented using {@link #incrementEpochCount()}
   *
  ...
  /**
   * Save the ComputationGraph to a file. Restore using {@link #load(File, boolean)}. Note that this
   * saves the updater (i.e., the state array for momentum/Adam/rmsprop etc), which is desirable if
   * further training will be undertaken.
   *
  ...
  /**
   * Restore a ComputationGraph to a file, saved using {@link #save(File)} or {@link
   * ModelSerializer}
   *
   ...
   *     momentum/Adam/rmsprop etc) - use <i>false</i> if no further training is required, or
   *     <i>true</i> if further training will be undertaken
   * @see ModelSerializer ModelSerializer for more details (and saving/loading via streams)
   */
  ...
  public ComputationGraph convertDataType(@NonNull DataType dataType) {
    Preconditions.checkState(
        dataType.isFPType(),
        "Invalid DataType: %s. Can only convert network to a floating point type",
        dataType);
   ...
   * rate schedule should be used in preference to calling this method at every iteration.
   *
   ...
   * Note that the iteration/epoch counts will <i>not</i> be reset. Use {@link
   * ComputationGraphConfiguration#setIterationCount(int)} and {@link
   * ComputationGraphConfiguration#setEpochCount(int)} if this is required
   *
   ...
   * <br>
   * <b>Note</b>: <i>This method not free from a performance point of view</i>: a proper learning
   * rate schedule should be used in preference to calling this method at every iteration. Note also
   * that {@link #setLearningRate(double)} should also be used in preference, when all layers need
   ...
   * <br>
   * Note also that the iteration/epoch counts will <i>not</i> be reset. Use {@link
   * ComputationGraphConfiguration#setIterationCount(int)} and {@link
   * ComputationGraphConfiguration#setEpochCount(int)} if this is required
   *
  ...
  /**
   * Return the layer size (number of units) for the specified layer. Note that the meaning of the
   * "layer size" can depend on the type of layer. For example:<br>
  ...
  public long layerSize(int layer) {
    if (layer < 0 || layer > layers.length) {
      throw new IllegalArgumentException(
          "Invalid layer index: "
              + layer
              + ". Layer index must be between 0 and "
              + (layers.length - 1)
              + " inclusive");
    }
  ...
  public void close() {
    ...
    if (flattenedParams.closeable()) flattenedParams.close();

    if (flattenedGradients != null && flattenedGradients.closeable()) flattenedGradients.close();

    Updater u = getUpdater(false);
    if (u != null && u.getStateViewArray() != null) {
      INDArray state = u.getStateViewArray();
      if (state.closeable()) state.close();
    }
  }
}
