...
public class ConvolutionLayer
  ...
  @Getter @Setter protected ConvolutionMode convolutionMode;
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    INDArray weightGradView =
        gradientViews
            .get(ConvolutionParamInitializer.WEIGHT_KEY)
            .reshape(weights.shape()); // 4d, c order. Shape: [outDepth,inDepth,kH,kW]

    INDArray delta;
    IActivation afn = layerConf().getActivationFn();

    delta =
        afn.backprop(workspaceMgr.dup(ArrayType.BP_WORKING_MEM, lastZ), epsilon)
    ...
    INDArray im2col2d =
        this.im2col2d; // Re-use im2col2d array from forward pass if available; recalculate if not

    OpContext ctx = Nd4j.getExecutioner().buildContext();
    ctx.addIntermediateResult(im2col2d);

    INDArray epsOut =
        workspaceMgr.create(ArrayType.ACTIVATION_GRAD, epsilon.dataType(), input.shape());
    CNN2DFormat format = ConvolutionUtils.getFormatForLayer(layerConf());

    Conv2DDerivative conv2DDerivative =
        Conv2DDerivative.derivativeBuilder()
            .config(
                Conv2DConfig.builder()
                    ...
                    .paddingMode(
                        ConvolutionUtils.paddingModeForConvolutionMode(
  ...
  /**
   * preOutput4d: Used so that ConvolutionLayer subclasses (such as Convolution1DLayer) can maintain
  ...
  protected Pair<INDArray, INDArray> preOutput(
    ...
    Conv2DConfig config =
        Conv2DConfig.builder()
            ...
            .weightsFormat(ConvolutionUtils.getWeightFormat(format))
            .paddingMode(
                ConvolutionUtils.paddingModeForConvolutionMode(layerConf().getConvolutionMode()))
    ...
    try (MemoryWorkspace ws1 = Nd4j.getMemoryManager().scopeOutOfWorkspaces()) {
      /**
       * TODO: dup seems to change underlyuing buffer here. We need to preserve everything about the
       * buffer.
       */
      this.lastZ = z.dup();
