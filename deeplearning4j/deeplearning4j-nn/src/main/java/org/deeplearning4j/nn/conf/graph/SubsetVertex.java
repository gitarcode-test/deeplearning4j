...
public class SubsetVertex extends GraphVertex {
  ...
  public org.deeplearning4j.nn.graph.vertex.GraphVertex instantiate(
    ...
    return new org.deeplearning4j.nn.graph.vertex.impl.SubsetVertex(
        graph, name, idx, from, to, networkDatatype);
  }
  ...
  public InputType getOutputType(int layerIndex, InputType... vertexInputs)
    ...
    switch (vertexInputs[0].getType()) {
      ...
      case CNN:
        ...
        if (to >= depth) {
          throw new InvalidInputTypeException(
              "Invalid range: Cannot select channels subset ["
                  + from
                  + ","
                  + to
                  + "] inclusive from CNN activations with "
