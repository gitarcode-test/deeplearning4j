...
public class SimpleRnnParamInitializer implements ParamInitializer {
  ...
  private static final List<String> WEIGHT_KEYS =
      Collections.unmodifiableList(Arrays.asList(WEIGHT_KEY, RECURRENT_WEIGHT_KEY));
  ...
  public List<String> paramKeys(Layer layer) {
    ...
    if (simpleRnn.isUseBias()) keys.addAll(biasKeys(layer));
    return keys;
  }
  ...
  public Map<String, INDArray> init(
    ...
    if (initializeParams) {
      ...
      INDArray w =
          c.getWeightInitFn().init(nIn, nOut, new long[] {nIn, nOut}, 'f', m.get(WEIGHT_KEY));
      m.put(WEIGHT_KEY, w);
      ...
      INDArray rw =
          rwInit.init(nOut, nOut, new long[] {nOut, nOut}, 'f', m.get(RECURRENT_WEIGHT_KEY));
      m.put(RECURRENT_WEIGHT_KEY, rw);
      if (c.isUseBias()) m.get(BIAS_KEY).assign(c.getBiasInit());
    ...
    if (c.isUseBias()) conf.addVariable(BIAS_KEY);
    if (hasLayerNorm(c)) {
  ...
  private static Map<String, INDArray> getSubsets(
      INDArray in, long nIn, long nOut, boolean reshape, boolean hasLayerNorm, boolean useBias) {
    long pos = nIn * nOut;
