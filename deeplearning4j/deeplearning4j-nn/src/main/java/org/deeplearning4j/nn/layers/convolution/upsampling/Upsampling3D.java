  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    boolean ncdhw =
        layerConf().getDataFormat()
            == org.deeplearning4j.nn.conf.layers.Convolution3D.DataFormat.NCDHW;
    // Assumes NCDHW order
    ...
    CustomOp op =
        DynamicCustomOp.builder("upsampling3d_bp")
  ...
  protected INDArray preOutput(
    ...
    if (input.rank() != 5) {
      throw new DL4JInvalidInputException(
          "Got rank "
              + input.rank()
              + " array as input to Upsampling3DLayer with shape "
    ...
    CustomOp upsampling =
        DynamicCustomOp.builder("upsampling3d")
            .addIntegerArguments(intArgs)
            .addInputs(input)
            .addOutputs(output)
            .callInplace(false)
