...
public class SameDiffLayer extends AbstractLayer<AbstractSameDiffLayer> {
  ...
  public INDArray activate(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    org.deeplearning4j.nn.conf.layers.samediff.SameDiffLayer bl =
        (org.deeplearning4j.nn.conf.layers.samediff.SameDiffLayer) layerConf();
    ...
    Preconditions.checkState(
        actScopedOut || wsNameOutput != null,
        "Activations must have a workspace or must be scoped out");
    SessionMemMgr mmgr =
        new DL4JSameDiffMemoryMgr(wsNameWorking, wsNameOutput, confWorking, confOutput);
    ...
    if (!actScopedOut
        && result.data().getParentWorkspace() != null
        && !result.data().getParentWorkspace().getId().equals(wsNameOutput)) {
      result = workspaceMgr.dup(ArrayType.ACTIVATIONS, result);
    } else if (actScopedOut && result.isAttached()) {
      result = result.detach();
    }

    // Clear placeholders and op inputs to ensure no out-of-scope arrays are still referenced
    // anywhere
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    if (!sessionMap.containsKey(Thread.currentThread().getId())) {
      sessionMap.put(
          ...
          SameDiff.getInferenceFactory().create(sameDiff.getFunction("grad")));
    }
    ...
    Preconditions.checkState(
        actGradScopedOut || wsNameActGrad != null,
        "Activation gradients must have a workspace or be scoped out");
    SessionMemMgr mmgr =
        new DL4JSameDiffMemoryMgr(wsNameWorking, wsNameActGrad, confWorking, confOutput);
    ...
    Pair<Gradient, INDArray> ret =
        new Pair<>(g, workspaceMgr.dup(ArrayType.ACTIVATION_GRAD, dLdIn)); // TODO OPTIMIZE THIS
    return ret;
  }
  ...
  public void setParam(String key, INDArray val) {
    if (!paramTable.containsKey(key)) {
      throw new IllegalArgumentException(
          "Cannot set parameter, invalid/unknown parameter key: " + key);
    }
    ...
    if (!Arrays.equals(current.shape(), val.shape())) {
      throw new IllegalArgumentException(
          "Cannot set parameter \""
              + key
              + "\", invalid shape: parameter array has shape "
              + Arrays.toString(current.shape())
              + ", trying to set parameter of shape "
              + Arrays.toString(val.shape()));
    }
  }

  @Override
  public void setParams(INDArray params) {
    if (this.params == null && params == null) return;
    if (this.params == null)
      throw new IllegalStateException(
          "Cannot set parameters of length " + params.length() + " to a layer with no parameters");
    if (params == null) throw new IllegalStateException("Cannot set null parameters");

    Preconditions.checkState(
        this.params.length() == params.length(),
        "Cannot assign parameter vector of length %s to a layer with %s parameters",
        params.length(),
        this.params.length());
  ...
  protected void doInit() {
    ...
    // Use SingleThreadArrayHolder so we can use views (also don't nede multithreading here, DL4J is
    // not thread safe)
