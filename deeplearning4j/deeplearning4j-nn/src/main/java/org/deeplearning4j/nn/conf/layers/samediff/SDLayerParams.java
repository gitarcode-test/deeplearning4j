...
public class SDLayerParams implements Serializable {
  ...
  public SDLayerParams(
      @JsonProperty("weightParams") Map<String, long[]> weightParams,
  ...
  /**
   * Add a weight parameter to the layer, with the specified shape. For example, a standard fully
   * connected layer could have weight parameters with shape [numInputs, layerSize]
   *
  ...
  public void addWeightParam(@NonNull String paramKey, @NonNull long... paramShape) {
    Preconditions.checkArgument(
        paramShape.length > 0,
        "Provided weight parameter shape is"
  ...
  /**
   * Add a bias parameter to the layer, with the specified shape. For example, a standard fully
   * connected layer could have bias parameters with shape [1, layerSize]
   *
  ...
  /**
   * @return Get a list of parameter names / keys (previously added via {@link
   *     #addWeightParam(String, long...)} and {@link #addBiasParam(String, long...)}
   */
  @JsonIgnore
