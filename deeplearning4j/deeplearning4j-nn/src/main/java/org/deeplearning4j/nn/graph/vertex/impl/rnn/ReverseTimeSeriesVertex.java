...
public class ReverseTimeSeriesVertex extends BaseGraphVertex {
  ...
  public ReverseTimeSeriesVertex(
      ComputationGraph graph, String name, int vertexIndex, String inputName, DataType dataType) {
  ...
  public INDArray doForward(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    return workspaceMgr.leverageTo(
        ArrayType.ACTIVATIONS, revertTimeSeries(input, mask, workspaceMgr, ArrayType.INPUT));
  }
  ...
  /**
   * Reverts the element order of a tensor along the 3rd axis (time series axis). A masking tensor
   * is used to restrict the revert to meaningful elements and keep the padding in place.
   *
   * <p>This method is self-inverse in the following sense: {@code revertTensor( revertTensor
   * (input, mask), mask )} equals {@code input}
   *
   * @param input The input tensor
   * @param mask The masking tensor (1 for meaningful entries, 0 for padding)
   * @return The reverted mask.
   */
  private static INDArray revertTimeSeries(
      INDArray input, INDArray mask, LayerWorkspaceMgr workspaceMgr, ArrayType type) {
    ...
    for (int s = 0; s < n; s++) {
      ...
      while (t1 < m && t2 >= 0) {
        ...
        INDArray vec = input.get(NDArrayIndex.point(s), NDArrayIndex.all(), NDArrayIndex.point(t1));

        // Put the feature vector to the given destination in the output
        out.put(
            new INDArrayIndex[] {NDArrayIndex.point(s), NDArrayIndex.all(), NDArrayIndex.point(t2)},
            vec);
  ...
  public Pair<INDArray, MaskState> feedForwardMaskArrays(
    ...
    if (maskArrays.length > 1) {
      throw new IllegalArgumentException(
          "This vertex can only handle one input and hence only one mask");
    }
