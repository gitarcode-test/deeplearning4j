...
public class CompositeReconstructionDistribution implements ReconstructionDistribution {
  ...
  public CompositeReconstructionDistribution(
      @JsonProperty("distributionSizes") int[] distributionSizes,
      @JsonProperty("reconstructionDistributions")
  ...
  public INDArray computeLossFunctionScoreArray(INDArray data, INDArray reconstruction) {
    if (!hasLossFunction()) {
      throw new IllegalStateException(
          "Cannot compute score array unless hasLossFunction() == true");
    }
    ...
    for (int i = 0; i < distributionSizes.length; i++) {
      ...
      INDArray dataSubset =
          data.get(
              NDArrayIndex.all(), NDArrayIndex.interval(inputSoFar, inputSoFar + thisInputSize));
      INDArray reconstructionSubset =
          reconstruction.get(
              NDArrayIndex.all(), NDArrayIndex.interval(paramsSoFar, paramsSoFar + thisParamsSize));

      if (i == 0) {
        reconstructionScores =
            getScoreArray(reconstructionDistributions[i], dataSubset, reconstructionSubset);
      } else {
        reconstructionScores.addi(
            getScoreArray(reconstructionDistributions[i], dataSubset, reconstructionSubset));
      }
  ...
  private INDArray getScoreArray(
    ...
    if (reconstructionDistribution instanceof LossFunctionWrapper) {
      ...
      return lossFunction.computeScoreArray(
          dataSubset, reconstructionSubset, new ActivationIdentity(), null);
      ...
      throw new UnsupportedOperationException(
          "Cannot calculate composite reconstruction distribution");
    }
  }
  ...
  public int distributionInputSize(int dataSize) {
    if (dataSize != totalSize) {
      throw new IllegalStateException(
          "Invalid input size: Got input size "
              + dataSize
              + " for data, but expected input"
              + " size for all distributions is "
              + totalSize
              + ". Distribution sizes: "
              + Arrays.toString(distributionSizes));
    }
  ...
  public double negLogProbability(INDArray x, INDArray preOutDistributionParams, boolean average) {
    ...
    for (int i = 0; i < distributionSizes.length; i++) {
      ...
      INDArray paramsSubset =
          preOutDistributionParams.get(
              NDArrayIndex.all(), NDArrayIndex.interval(paramsSoFar, paramsSoFar + thisParamsSize));

      logProbSum +=
          reconstructionDistributions[i].negLogProbability(inputSubset, paramsSubset, average);
  ...
  private INDArray randomSample(INDArray preOutDistributionParams, boolean isMean) {
    ...
    INDArray out =
        Nd4j.createUninitialized(
            preOutDistributionParams.dataType(),
            new long[] {preOutDistributionParams.size(0), totalSize});
    for (int i = 0; i < distributionSizes.length; i++) {
      ...
      out.put(
          new INDArrayIndex[] {
            NDArrayIndex.all(), NDArrayIndex.interval(inputSoFar, inputSoFar + thisDataSize)
          },
          thisRandomSample);
  ...
  public static class Builder {
    ...
    /**
     * Add another distribution to the composite distribution. This will add the distribution for
     * the next 'distributionSize' values, after any previously added. For example, calling
     * addDistribution(10, X) once will result in values 0 to 9 (inclusive) being modelled by the
     * specified distribution X. Calling addDistribution(10, Y) after that will result in values 10
     * to 19 (inclusive) being modelled by distribution Y.
     *
