...
public class SameDiffGraphVertex extends BaseGraphVertex {
  ...
  public SameDiffGraphVertex(
    ...
    paramTable =
        SameDiffParamInitializer.getInstance()
            .subsetAndReshape(vp.getParameterKeys(), vp.getParamShapes(), paramsView, null, config);
    if (initParams) {
  ...
  public Pair<Gradient, INDArray[]> doBackward(boolean tbptt, LayerWorkspaceMgr workspaceMgr) {
    ...
    List<String> required =
        new ArrayList<>(
            config
                .getVertexParams()
                .getInputs()); // Ensure that the input placeholder gradients are calculated
    ...
    for (int j = 0; j < inputs.size(); j++) {
      ...
      if (dLdIns[j] == null && fnName.equals(gradName)) {
        ...
        // So, this getArr() can be trying to get placeholder from SameDiff instance, when it's
        // available here
        dLdIns[j] = epsilon;
      }
      ...
      // TODO there may be a cleaner way to do this...
      if (!actGradScopedOut
          && !dLdIns[j].data().getParentWorkspace().getId().equals(wsNameActGrad)) {
  ...
  protected void doInit() {
    try (MemoryWorkspace ws = Nd4j.getWorkspaceManager().scopeOutOfWorkspaces()) {
      ...
      // Use SingleThreadArrayHolder so we can use views (also don't need multithreading here, DL4J
      // is not thread safe)
  ...
  static INDArray createMask(DataType dataType, long[] shape) {
    switch (shape.length) {
      ...
      default:
        Preconditions.throwEx(
            "Can not create all-ones-mask for given input shape %s.", Arrays.toString(shape));
        return null;
    }
  }
}
