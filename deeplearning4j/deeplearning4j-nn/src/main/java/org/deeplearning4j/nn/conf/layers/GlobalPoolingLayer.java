...
public class GlobalPoolingLayer extends NoParamLayer {
  ...
  public InputType getOutputType(int layerIndex, InputType inputType) {
    ...
    switch (inputType.getType()) {
      ...
      case CNNFlat:
        InputType.InputTypeConvolutionalFlat convFlat =
            (InputType.InputTypeConvolutionalFlat) inputType;
        if (collapseDimensions) {
  ...
  public InputPreProcessor getPreProcessorForInputType(InputType inputType) {
    switch (inputType.getType()) {
      ...
      case FF:
        ...
        if (feedForward.getTimeDistributedFormat() != null
            && feedForward.getTimeDistributedFormat() instanceof RNNFormat) {
          RNNFormat rnnFormat = (RNNFormat) feedForward.getTimeDistributedFormat();
          return new FeedForwardToRnnPreProcessor(rnnFormat);
        } else if (feedForward.getTimeDistributedFormat() != null
            && feedForward.getTimeDistributedFormat() instanceof CNN2DFormat) {
      ...
      case CNN3D:
        ...
        return null;
      case CNNFlat:
        InputType.InputTypeConvolutionalFlat cFlat =
            (InputType.InputTypeConvolutionalFlat) inputType;
        return new FeedForwardToCnnPreProcessor(
            cFlat.getHeight(), cFlat.getWidth(), cFlat.getDepth());
    }
  ...
  public LayerMemoryReport getMemoryReport(InputType inputType) {
    ...
    return new LayerMemoryReport.Builder(layerName, GlobalPoolingLayer.class, inputType, outputType)
        ...
        // Train + Inference: no additional working memory (except pnorm) - the reduction is the
        // output activations
  ...
  public static class Builder extends Layer.Builder<Builder> {
    ...
    /**
     * Pooling dimensions. Note: most of the time, this doesn't need to be set, and the defaults can
     * be used. Default for RNN data: pooling dimension 2 (time). Default for CNN data: pooling
     * dimensions 2,3 (height and width) Default for CNN3D data: pooling dimensions 2,3,4 (depth,
     * height and width)
     */
    private long[] poolingDimensions;
    ...
    /**
     * Whether to collapse dimensions when pooling or not. Usually you *do* want to do this.
     * Default: true. If true:<br>
     * - 3d (time series) input with shape [miniBatchSize, vectorSize, timeSeriesLength] -> 2d
     * output [miniBatchSize, vectorSize]<br>
     * - 4d (CNN) input with shape [miniBatchSize, channels, height, width] -> 2d output
     * [miniBatchSize, channels]<br>
     * - 5d (CNN3D) input with shape [miniBatchSize, channels, depth, height, width] -> 2d output
     * [miniBatchSize, channels]<br>
     * If false:<br>
     * - 3d (time series) input with shape [miniBatchSize, vectorSize, timeSeriesLength] -> 3d
     * output [miniBatchSize, vectorSize, 1]<br>
     * - 4d (CNN) input with shape [miniBatchSize, channels, height, width] -> 2d output
     * [miniBatchSize, channels, 1, 1]<br>
     * - 5d (CNN3D) input with shape [miniBatchSize, channels, depth, height, width] -> 2d output
     * [miniBatchSize, channels, 1, 1, 1]<br>
     */
    private boolean collapseDimensions = true;
    ...
    public Builder pnorm(int pnorm) {
      if (pnorm <= 0) {
        throw new IllegalArgumentException(
            "Invalid input: p-norm value must be greater than 0. Got: " + pnorm);
      }
      this.setPnorm(pnorm);
      return this;
    }
