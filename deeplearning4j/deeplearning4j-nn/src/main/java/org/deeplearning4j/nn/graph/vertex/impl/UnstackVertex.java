...
public class UnstackVertex extends BaseGraphVertex {
  ...
  public INDArray doForward(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    INDArray ret;
    switch (inputs[0]
        .rank()) { // TODO remove the dups here if/when possible (gradient checks must pass)
      case 2:
      ...
      case 3:
        ret =
            inputs[0].get(
                NDArrayIndex.interval(start, end), NDArrayIndex.all(), NDArrayIndex.all());
        break;
      case 4:
        ret =
            inputs[0].get(
  ...
  public Pair<Gradient, INDArray[]> doBackward(boolean tbptt, LayerWorkspaceMgr workspaceMgr) {
    ...
    INDArray out =
        workspaceMgr.create(ArrayType.ACTIVATION_GRAD, inputs[0].dataType(), forwardShape);
    long start = from * step;
    long end = (from + 1) * step;
    ...
    switch (forwardShape.length) {
      case 2:
        out.put(
            new INDArrayIndex[] {NDArrayIndex.interval(start, end), NDArrayIndex.all()}, epsilon);
        break;
      case 3:
        out.put(
            new INDArrayIndex[] {
