...
public class SeparableConvolution2D extends ConvolutionLayer {
  ...
  /**
   * SeparableConvolution2D layer nIn in the input layer is the number of channels nOut is the
   * number of filters to be used in the net or in other words the channels The builder specifies
   * the filter/kernel size, the stride and padding The pooling layer takes the kernel size
   */
  ...
  public Layer instantiate(
    ...
    LayerValidation.assertNInNOutSet(
        "SeparableConvolution2D", getLayerName(), layerIndex, getNIn(), getNOut());

    SeparableConvolution2DLayer ret = new SeparableConvolution2DLayer(conf, networkDataType);
  ...
  public InputType getOutputType(int layerIndex, InputType inputType) {
    ...
    return InputTypeUtil.getOutputTypeCnnLayersLong(
        inputType,
        kernelSize,
        stride,
        padding,
  ...
  public static class Builder extends BaseConvBuilder<Builder> {
     ...
     * have been updated.
     *
     * @param constraints Constraints to apply to the point-wise convolution parameters of this
     *     layer
     */
