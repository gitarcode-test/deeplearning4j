...
public class SubsamplingLayer extends NoParamLayer {
  ...
  @JsonIgnore @EqualsAndHashCode.Exclude private boolean defaultValueOverridden = false;
  ...
  public org.deeplearning4j.nn.api.Layer instantiate(
    ...
    org.deeplearning4j.nn.layers.convolution.subsampling.SubsamplingLayer ret =
        new org.deeplearning4j.nn.layers.convolution.subsampling.SubsamplingLayer(
  ...
  public InputType getOutputType(int layerIndex, InputType inputType) {
    ...
    return InputTypeUtil.getOutputTypeCnnLayersLong(
        ...
        dilation,
        convolutionMode,
        ((InputType.InputTypeConvolutional) inputType).getChannels(),
        layerIndex,
        getLayerName(),
        cnn2dDataFormat,
  ...
  public static class Builder extends BaseSubsamplingBuilder<Builder> {
     ...
     * dilated convolutions, which are also known as atrous convolutions.<br>
     * NOTE: Kernel dilation is less common in practice for subsampling layers, compared to
  ...
  protected abstract static class BaseSubsamplingBuilder<T extends BaseSubsamplingBuilder<T>>
     ...
     * used
     *
     * <p>Whether fallback to non-CuDNN implementation should be used
     */
    protected boolean cudnnAllowFallback = true;
    ...
    protected BaseSubsamplingBuilder(
        PoolingType poolingType, long[] kernelSize, long[] stride, long[] padding) {
      this.setPoolingType(poolingType.toPoolingType());
      this.setKernelSize(kernelSize);
      this.setStride(stride);
      this.setPadding(padding);
    ...
    protected BaseSubsamplingBuilder(
        org.deeplearning4j.nn.conf.layers.PoolingType poolingType, long[] kernelSize) {
      this.setPoolingType(poolingType);
      this.setKernelSize(kernelSize);
    }
     ...
     * When using CuDNN or MKLDNN and an error is encountered, should fallback to the non-helper
     * implementation be allowed? If set to false, an exception in the helper will be propagated
     * back to the user. If true, the built-in (non-MKL/CuDNN) implementation for SubsamplingLayer
     * will be used
     *
     * @param allowFallback Whether fallback to non-CuDNN implementation should be used
     */
     ...
     * Users should not usually set this - instead, leave it as the default (false). It is included
     * mainly for backward compatibility of older models<br>
     ...
     * <br>
     *
     * @param avgPoolIncludePadInDivisor Whether the divisor should include or exclude padding for
     *     average pooling
     */
