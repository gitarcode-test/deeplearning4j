...
public abstract class BaseLayer<LayerConfT extends org.deeplearning4j.nn.conf.layers.BaseLayer>
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    INDArray z =
        zAndPreNorm
            .getFirst(); // Note: using preOutput(INDArray) can't be used as this does a
    ...
    Nd4j.gemm(
        input.castTo(weightGrad.dataType()),
        delta,
        weightGrad,
        true,
        false,
  ...
  protected Pair<INDArray, INDArray> preOutputWithPreNorm(
    ...
    INDArray ret =
        workspaceMgr.create(
            ArrayType.ACTIVATIONS, W.dataType(), new long[] {input.size(0), W.size(1)}, 'f');
    input.mmuli(W, ret);
    ...
    return new Pair<>(
        ...
        workspaceMgr.leverageTo(ArrayType.ACTIVATIONS, preNorm));
  }
  ...
  public long numParams() {
    int ret = 0;
    for (INDArray val : params.values()) ret += val.length();
    return ret;
  }
