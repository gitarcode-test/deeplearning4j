...
public abstract class BaseMultiLayerUpdater<T extends Model> implements Updater {
   ...
   * @param updaterState The updater state to use. Note: This array is used *directly* and isn't
   *     copied/cloned
   */
  public BaseMultiLayerUpdater(T network, INDArray updaterState) {
    ...
    // While the updater configuration is the same: combine into one op, rather than doing a lot of
    // smaller
    // (yet identical) ops.
    ...
    for (int i = 0; i < layers.length; i++) {
      ...
      if (layerParamTable != null) {
        List<String> variables =
            new ArrayList<>(
                layerParamTable
                    .keySet()); // Is from a set, but iteration order should be fixed per layer as
                                // it's a from a LinkedHashSet
        for (int j = 0; j < variables.size(); j++) {
          ...
          Preconditions.checkNotNull(
              u,
              "Updater for parameter %s, layer \"%s\" was null",
              var,
              layers[i].getConfig().getLayerName());
          ...
          if (paramSizeThisVariable > 0) {
            paramsViewSubset =
                paramsViewReshape.get(
                    NDArrayIndex.interval(
                        paramsViewSoFar, paramsViewSoFar + paramSizeThisVariable));
            gradientViewSubset =
                gradientViewReshape.get(
                    NDArrayIndex.interval(
                        paramsViewSoFar, paramsViewSoFar + paramSizeThisVariable));
          }
          ...
          if (currentBlock == null
              || !UpdaterUtils.updaterConfigurationsEquals(
                  lastLayer, lastVariable, layers[i], var)) {

            if (paramsViewSoFar + paramSizeThisVariable > Integer.MAX_VALUE
                || paramsViewSoFar + paramSizeThisVariable > Integer.MAX_VALUE)
              throw new ND4JArraySizeException();
            // Create a new block
            List<UpdaterBlock.ParamState> list = new ArrayList<>();
            list.add(
                new UpdaterBlock.ParamState(
                    layers[i],
                    var,
                    paramsViewSoFar,
                    (int) (paramsViewSoFar + paramSizeThisVariable),
                    paramsViewSubset,
                    gradientViewSubset));
            currentBlock =
                new UpdaterBlock(
            ...
            if (newOffset > Integer.MAX_VALUE) throw new ND4JArraySizeException();
    ...
    if (updaterState != null) {
      ...
      updaterStateViewArray =
          Nd4j.createUninitialized(
              network.params().dataType(), new long[] {updaterStateSize}, Nd4j.order());
    ...
    for (int i = 0; i < updaterBlocks.size(); i++) {
      ...
      if (viewStateSize > 0) {
        INDArray updaterViewSubset =
            updaterStateViewArray.get(
      ...
      if (gradSize > 0) {
        INDArray gradientViewSubset =
            gradientView
                .reshape(gradientView.length())
                .get(NDArrayIndex.interval(paramsViewSoFar, paramsViewSoFar + gradSize));
  ...
  /**
   * @return Array of layers, in the correct order (i.e., same order as the
   *     parameter/gradient/updater flattening order - input to output for MultiLayerNetwork, or
   *     topological order for ComputationGraph)
   */
  ...
  /**
   * @return True if the configuration for the model is set to minibatch (divide by minibatch size),
   *     false otherwise
   */
  protected abstract boolean isMiniBatch();

  /**
   * Set the view array. Note that this does an assign operation - the provided array is not stored
   * internally.
   *
    ...
    if (this.updaterStateViewArray == null) {
      if (viewArray == null)
        ...
        throw new IllegalStateException(
            "Attempting to set updater state view array with null value");
      }
    }
    if (this.updaterStateViewArray.length() != viewArray.length())
      throw new IllegalStateException(
          "Invalid input: view arrays differ in length. "
              + "Expected length "
              + this.updaterStateViewArray.length()
              + ", got length "
              + viewArray.length());
  ...
  /**
   * A synchronized version of {@link #getStateViewArray()} that duplicates the view array
   * internally. This should be used in preference to {@link #getStateViewArray()} when the updater
   * state is accessed in one thread while another thread is using the updater for training.
   *
  ...
  /**
   * Update the gradient for the model. This operates in 3 steps: 1. Pre-apply: gradient clipping,
   * etc on a per-layer basis 2. Execute the updater (Adam, Nesterov momentum, etc) - in blocks of
   * layers at a time 3. Divide by minibatch size
   *
   ...
   */
  public void update(
      Gradient gradient, int iteration, int epoch, int batchSize, LayerWorkspaceMgr workspaceMgr) {

    // First: check if gradient is standard or external...
    // In a MultiLayerNetwork, the INDArray returned by .gradient() is always the standard full view
    // array
  ...
  protected void divideByMinibatch(boolean isExternal, Gradient gradient, int batchSize) {
    // Challenge here: most gradients are actual gradients, and should be divided by the minibatch
    // to get the average
    // However, some 'gradients' are actually updates - an example being BatchNorm mean/variance
    // estimates... these
  ...
  protected List<INDArray> getMinibatchDivisionSubsets(INDArray from) {
    ...
    if (currentEnd > currentStart && currentStart < from.length()) {
      ...
      INDArray subset =
          from.reshape(from.length()).get(NDArrayIndex.interval(currentStart, currentEnd));
      out.add(subset);
    }
    return out;
  }
    ...
    switch (normalization) {
      case RenormalizeL2PerLayer:
        if (layerGradientView != null) {
          ...
          if (l2 == 0.0) l2 = 1e-5; // Avoid 0/0 -> NaN
      ...
      case ClipElementWiseAbsoluteValue:
        if (layerGradientView != null) {
          CustomOp op =
              DynamicCustomOp.builder("clipbyvalue")
