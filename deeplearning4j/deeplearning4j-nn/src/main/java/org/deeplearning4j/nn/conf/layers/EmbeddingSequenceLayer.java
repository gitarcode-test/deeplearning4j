...
public class EmbeddingSequenceLayer extends FeedForwardLayer {
  ...
  public Layer instantiate(
    ...
    org.deeplearning4j.nn.layers.feedforward.embedding.EmbeddingSequenceLayer ret =
        new org.deeplearning4j.nn.layers.feedforward.embedding.EmbeddingSequenceLayer(
  ...
  public InputType getOutputType(int layerIndex, InputType inputType) {
    if (inputType == null
      ...
      throw new IllegalStateException(
          "Invalid input for Embedding layer (layer index = "
              + layerIndex
              + ", layer name = \""
              + getLayerName()
              + "\"): expect FF/RNN input type. Got: "
              + inputType);
    }
  ...
  public LayerMemoryReport getMemoryReport(InputType inputType) {
    ...
    return new LayerMemoryReport.Builder(
            layerName, EmbeddingSequenceLayer.class, inputType, outputType)
  ...
  public static class Builder extends FeedForwardLayer.Builder<Builder> {
    ...
    public Builder() {
      ...
      // For example, if user sets ReLU as global default, they very likely don't intend to use it
      // for Embedding layer also
