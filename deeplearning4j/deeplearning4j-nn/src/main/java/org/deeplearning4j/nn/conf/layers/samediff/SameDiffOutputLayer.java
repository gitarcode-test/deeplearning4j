...
public abstract class SameDiffOutputLayer extends AbstractSameDiffLayer {
   ...
   * @return The final layer variable corresponding to the score/loss during forward pass. This must
   *     be a single scalar value.
   */
  public abstract SDVariable defineLayer(
      SameDiff sameDiff,
  ...
  /**
   * Output layers should terminate in a single scalar value (i.e., a score) - however, sometimes
   * the output activations (such as softmax probabilities) need to be returned. When this is the
   * case, we need to know the name of the SDVariable that corresponds to these.<br>
   ...
   * can be calculated without labels (for example, in some output layers used for unsupervised
   * learning) this can be set to false.
   *
