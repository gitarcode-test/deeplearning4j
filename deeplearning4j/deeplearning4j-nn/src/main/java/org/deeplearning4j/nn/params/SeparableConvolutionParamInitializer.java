...
public class SeparableConvolutionParamInitializer implements ParamInitializer {
  ...
  private static final SeparableConvolutionParamInitializer INSTANCE =
  ...
  /**
   * For the point-wise convolution part we have (nIn * depthMultiplier) many input maps and nOut
   * output maps. Kernel size is (1, 1) for this operation.
   *
   * @param layerConf layer configuration of the separable conv2d layer
  ...
  public Map<String, INDArray> init(
    ...
    INDArray pointWiseWeightView =
        paramsViewReshape.get(NDArrayIndex.interval(biasParams + depthWiseParams, numParams(conf)));

    params.put(
        DEPTH_WISE_WEIGHT_KEY,
        createDepthWiseWeightMatrix(conf, depthWiseWeightView, initializeParams));
    conf.addVariable(DEPTH_WISE_WEIGHT_KEY);
    params.put(
        POINT_WISE_WEIGHT_KEY,
        createPointWiseWeightMatrix(conf, pointWiseWeightView, initializeParams));
  ...
  public Map<String, INDArray> getGradientsFromFlattened(
    ...
    INDArray pointWiseWeightGradientView =
        gradientViewReshape
            .get(NDArrayIndex.interval(biasParams + depthWiseParams, numParams(conf)))
  ...
  protected INDArray createPointWiseWeightMatrix(
      NeuralNetConfiguration conf, INDArray weightView, boolean initializeParams) {
    /*
    ...
    */
    ...
    if (initializeParams) {
      ...
      return WeightInitUtil.reshapeWeights(
          new long[] {layerConf.getNOut(), depthMultiplier * layerConf.getNIn(), 1, 1},
          weightView,
          'c');
    }
  }
