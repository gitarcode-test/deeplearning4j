...
public class SpaceToDepth
    extends AbstractLayer<org.deeplearning4j.nn.conf.layers.SpaceToDepthLayer> {
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    INDArray outEpsilon =
        workspaceMgr.create(ArrayType.ACTIVATION_GRAD, input.dataType(), epsShape, 'c');
    ...
    if (!Shape.hasDefaultStridesForShape(epsilon)) epsilon = epsilon.dup('c');

    CustomOp op =
        DynamicCustomOp.builder("depth_to_space")
            .addInputs(epsilon)
  ...
  protected INDArray preOutput(
    ...
    if (input.rank() != 4) {
      throw new DL4JInvalidInputException(
          "Got rank "
              + input.rank()
              + " array as input to space to channels with shape "
              + Arrays.toString(input.shape())
              + ". Expected rank 4 array with shape "
              + layerConf().getDataFormat().dimensionNames()
              + ". "
              + layerId());
    }
    ...
    long[] outShape =
        nchw
            ? new long[] {miniBatch, outDepth, outH, outW}
            : new long[] {miniBatch, outH, outW, outDepth};
    INDArray out = workspaceMgr.create(ArrayType.ACTIVATIONS, input.dataType(), outShape, 'c');

    // Workaround for issue: https://github.com/eclipse/deeplearning4j/issues/8859
    INDArray input = this.input;
    if (!Shape.hasDefaultStridesForShape(input)) input = input.dup('c');

    CustomOp op =
        DynamicCustomOp.builder("space_to_depth")
            .addInputs(input)
            .addIntegerArguments(blockSize, nchw ? 0 : 1) // nchw = 0, nhwc = 1
            .addOutputs(out)
            .build();
