...
public class DuplicateToTimeSeriesVertex extends BaseGraphVertex {
    ...
    if (inputVertexIndex == -1)
      throw new IllegalArgumentException(
          "Invalid input name: \""
              ...
              + "\" not found in list "
              + "of network inputs ("
              + graph.getConfiguration().getNetworkInputs()
              + ")");
  }
  ...
  public INDArray doForward(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    for (int i = 0; i < tsLength; i++) {
      out.put(
          new INDArrayIndex[] {NDArrayIndex.all(), NDArrayIndex.all(), NDArrayIndex.point(i)},
          inputs[0]);
    }
  ...
  public Pair<Gradient, INDArray[]> doBackward(boolean tbptt, LayerWorkspaceMgr workspaceMgr) {
    ...
    INDArray ret =
        epsilon.sum(
            workspaceMgr.create(
                ArrayType.ACTIVATION_GRAD, epsilon.dataType(), epsilon.size(0), epsilon.size(1)),
            2);
