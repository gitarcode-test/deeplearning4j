...
public class LSTMParamInitializer implements ParamInitializer {
  ...
  private static final List<String> LAYER_PARAM_KEYS =
      Collections.unmodifiableList(Arrays.asList(INPUT_WEIGHT_KEY, RECURRENT_WEIGHT_KEY, BIAS_KEY));
  private static final List<String> WEIGHT_KEYS =
      Collections.unmodifiableList(Arrays.asList(INPUT_WEIGHT_KEY, RECURRENT_WEIGHT_KEY));
  private static final List<String> BIAS_KEYS =
      Collections.unmodifiableList(Collections.singletonList(BIAS_KEY));
  ...
  public Map<String, INDArray> init(
    ...
    Map<String, INDArray> params =
        Collections.synchronizedMap(new LinkedHashMap<String, INDArray>());
    ...
    INDArray recurrentWeightView =
        paramsViewReshape.get(NDArrayIndex.interval(nParamsIn, nParamsIn + nParamsRecurrent));
    INDArray biasView =
        paramsViewReshape.get(
            NDArrayIndex.interval(
                nParamsIn + nParamsRecurrent, nParamsIn + nParamsRecurrent + nBias));
    ...
    if (initializeParams) {
      ...
      params.put(
          ...
          layerConf
              ...
              .init(
                  ...
                  fanOut,
                  inputWShape,
                  IWeightInit.DEFAULT_WEIGHT_INIT_ORDER,
                  inputWeightView));
      INDArray init =
          rwInit.init(
              fanIn,
      ...
      biasView.put(
          new INDArrayIndex[] {NDArrayIndex.interval(nL, 2 * nL)},
          Nd4j.valueArrayOf(
              new long[] {nL},
              forgetGateInit)); // Order: input, forget, output, input modulation, i.e., IFOG}
      ...
      params.put(
          INPUT_WEIGHT_KEY,
          WeightInitUtil.reshapeWeights(new long[] {nLast, 4 * nL}, inputWeightView));
      params.put(
  ...
  public Map<String, INDArray> getGradientsFromFlattened(
    ...
    INDArray inputWeightGradView =
        gradientViewReshape.get(NDArrayIndex.interval(0, nParamsIn)).reshape('f', nLast, 4 * nL);
    INDArray recurrentWeightGradView =
        gradientViewReshape
    ...
    INDArray biasGradView =
        gradientViewReshape.get(
            NDArrayIndex.interval(
                nParamsIn + nParamsRecurrent,
                nParamsIn + nParamsRecurrent + nBias)); // already a row vector
