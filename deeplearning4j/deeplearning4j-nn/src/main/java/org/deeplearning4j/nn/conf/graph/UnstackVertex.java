...
public class UnstackVertex extends GraphVertex {
   ...
   * @param stackSize The total number of stacked inputs. An interval is automatically calculated
   *     according to the size of the first dimension.
   */
  ...
  public org.deeplearning4j.nn.graph.vertex.GraphVertex instantiate(
    ...
    return new org.deeplearning4j.nn.graph.vertex.impl.UnstackVertex(
        graph, name, idx, null, null, from, stackSize, networkDatatype);
  }
  ...
  public InputType getOutputType(int layerIndex, InputType... vertexInputs)
    ...
    if (first.getType() == InputType.Type.CNNFlat) {
      ...
      for (int i = 1; i < vertexInputs.length; i++) {
        ...
        if (fw != ow || fh != oh) {
          throw new InvalidInputTypeException(
              "Invalid input: UnstackVertex cannot merge CNN activations of different"
                  + " width/heights:first [channels,width,height] = ["
                  + fd
                  + ","
                  + fw
                  + ","
