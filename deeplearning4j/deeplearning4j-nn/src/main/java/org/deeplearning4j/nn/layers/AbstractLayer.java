...
@Data
@NoArgsConstructor
public abstract class AbstractLayer<LayerConfT extends org.deeplearning4j.nn.conf.layers.Layer>
    implements Layer {
  ...
  public AbstractLayer(NeuralNetConfiguration conf, DataType dataType) {
    this.conf = conf;
    if (conf != null) cacheMode = conf.getCacheMode();
  ...
  public Pair<INDArray, MaskState> feedForwardMaskArray(
    ...
    // Most layers: CNN, dense, activation, etc - set mask array, mask state and then leave the mask
    // unmodified
      ...
      if (backprop) {
        throw new IllegalStateException(
            "Cannot perform backprop in layer "
                + getClass().getSimpleName()
                + ": layer input field is not set. In order to set this input, ensure we call"
                + " feedForward(...) or activate(...) first. Ensure clearInput (the second boolean"
                + " argument) is false. like feedForward(input,false)");
      } else {
        throw new IllegalStateException(
