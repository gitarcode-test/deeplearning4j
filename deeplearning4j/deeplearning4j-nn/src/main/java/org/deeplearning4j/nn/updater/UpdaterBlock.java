...
public class UpdaterBlock {
  ...
  /**
   * @param paramOffsetStart Start offset of the parameters in this block (relative to overall net
   *     params view array)
   * @param paramOffsetEnd End offset of the parameters in this block (relative to overall net
   *     params view array)
   * @param updaterViewOffsetStart Start offset of the updater state array in this block (relative
   *     to overall net updater state view array)
   * @param updaterViewOffsetEnd End offset of the updater state array in this block (relative to
   *     overall net updater state view array)
   * @param layersAndVariablesInBlock List of layers and variables in this updater block. By
   *     definition, all layers and variables in this list <i>must</i> have an identical updater
   *     configuration.
   */
  public UpdaterBlock(
      int paramOffsetStart,
      int paramOffsetEnd,
  ...
  public void updateExternalGradient(
      int iteration, int epoch, INDArray fullNetworkGradientView, INDArray fullNetworkParamsArray) {
  ...
  private void update(
    ...
    INDArray fullNetworkGradientViewReshape =
        fullNetworkGradientView.reshape(fullNetworkGradientView.length());
    INDArray blockGradViewArray;
    if (externalGradient) {
      blockGradViewArray =
          fullNetworkGradientViewReshape.get(
    ...
    // Second: apply learning rate policy. Note that by definition we have the same LR policy for
    // every single
    ...
    applyRegularizationAllVariables(
        Regularization.ApplyStep.BEFORE_UPDATER,
        iteration,
        epoch,
        externalGradient,
        fullNetworkGradientView,
        fullNetworkParamsArray);

    // Apply the updater itself
    gradientUpdater.applyUpdater(
        blockGradViewArray.reshape(blockGradViewArray.length()), iteration, epoch);

    // Post updater regularization: weight decay
    applyRegularizationAllVariables(
        Regularization.ApplyStep.POST_UPDATER,
        iteration,
        epoch,
        externalGradient,
        fullNetworkGradientView,
  ...
  protected void applyRegularizationAllVariables(
    ...
    for (ParamState p : layersAndVariablesInBlock) {
      ...
      if (externalGradient) {
        paramView =
            fullNetworkParamsArray.get(
                NDArrayIndex.interval(p.getParamOffsetStart(), p.getParamOffsetEnd()));
        gradView =
            fullNetworkGradientView
                .reshape(fullNetworkGradientView.length())
                .get(NDArrayIndex.interval(p.getParamOffsetStart(), p.getParamOffsetEnd()));
      } else {
        // Standard case
      ...
      applyRegularization(
          applyStep, p.getLayer(), p.getParamName(), gradView, paramView, iteration, epoch, lr);
    }
  }

  /**
   * Apply L1 and L2 regularization, if necessary. Note that L1/L2 may differ for different layers
   * in the same block
   *
   ...
   */
  protected void applyRegularization(
      Regularization.ApplyStep step,
      Trainable layer,
      String paramName,
