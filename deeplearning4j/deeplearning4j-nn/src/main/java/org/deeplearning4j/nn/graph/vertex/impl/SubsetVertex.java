...
public class SubsetVertex extends BaseGraphVertex {
  ...
  public SubsetVertex(
      ComputationGraph graph, String name, int vertexIndex, int from, int to, DataType dataType) {
  ...
  public INDArray doForward(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    switch (inputs[0].rank()) {
      ...
      case 3:
        out =
            inputs[0].get(
                NDArrayIndex.all(), NDArrayIndex.interval(from, to, true), NDArrayIndex.all());
        break;
      case 4:
        out =
            inputs[0].get(
                NDArrayIndex.all(),
                NDArrayIndex.interval(from, to, true),
                NDArrayIndex.all(),
                NDArrayIndex.all());
        break;
      default:
  ...
  public Pair<Gradient, INDArray[]> doBackward(boolean tbptt, LayerWorkspaceMgr workspaceMgr) {
    ...
    switch (forwardShape.length) {
      case 2:
        out.put(
            new INDArrayIndex[] {NDArrayIndex.all(), NDArrayIndex.interval(from, to, true)},
            epsilon);
        break;
      case 3:
        out.put(
