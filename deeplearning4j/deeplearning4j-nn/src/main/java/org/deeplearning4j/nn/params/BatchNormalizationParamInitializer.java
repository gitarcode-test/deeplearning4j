...
public class BatchNormalizationParamInitializer implements ParamInitializer {
  ...
  private static final BatchNormalizationParamInitializer INSTANCE =
  ...
  public long numParams(Layer l) {
    ...
    // latter 2 are treated as parameters, which greatly simplifies spark training and model
    // serialization

    if (layer.isLockGammaBeta()) {
      // Special case: gamma and beta are fixed values for all outputs -> no parameters for gamma
      // and  beta in this case
      return 2 * layer.getNOut();
    } else {
      // Standard case: gamma and beta are learned per output; additional 2*nOut for global
  ...
  public Map<String, INDArray> init(
      NeuralNetConfiguration conf, INDArray paramView, boolean initializeParams) {
    ...
    INDArray globalVarView =
        paramViewReshape.get(NDArrayIndex.interval(meanOffset + nOut, meanOffset + 2 * nOut));

    if (initializeParams) {
      globalMeanView.assign(0);
      if (layer.isUseLogStd()) {
        // Global log stdev: assign 0.0 as initial value (s=sqrt(v), and log10(s) = log10(sqrt(v))
        // -> log10(1) = 0
  ...
  public Map<String, INDArray> getGradientsFromFlattened(
    ...
    out.put(
        GLOBAL_MEAN, gradientViewReshape.get(NDArrayIndex.interval(meanOffset, meanOffset + nOut)));
    if (layer.isUseLogStd()) {
      out.put(
          GLOBAL_LOG_STD,
          gradientViewReshape.get(NDArrayIndex.interval(meanOffset + nOut, meanOffset + 2 * nOut)));
    } else {
      out.put(
          GLOBAL_VAR,
          gradientViewReshape.get(NDArrayIndex.interval(meanOffset + nOut, meanOffset + 2 * nOut)));
  ...
  private INDArray createBeta(
      NeuralNetConfiguration conf, INDArray betaView, boolean initializeParams) {
    BatchNormalization layer = (BatchNormalization) conf.getLayer();
    if (initializeParams) betaView.assign(layer.getBeta());
    return betaView;
  }

  private INDArray createGamma(
      NeuralNetConfiguration conf, INDArray gammaView, boolean initializeParams) {
    BatchNormalization layer = (BatchNormalization) conf.getLayer();
