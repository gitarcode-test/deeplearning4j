...
public class DepthwiseConvolutionParamInitializer implements ParamInitializer {
  ...
  private static final DepthwiseConvolutionParamInitializer INSTANCE =
      new DepthwiseConvolutionParamInitializer();
  ...
  /**
   * For each input feature we separately compute depthMultiplier many output maps for the given
   * kernel size
   *
  ...
  public Map<String, INDArray> init(
    ...
    INDArray depthWiseWeightView =
        paramsViewReshape.get(NDArrayIndex.interval(biasParams, biasParams + depthWiseParams));

    params.put(
        WEIGHT_KEY, createDepthWiseWeightMatrix(conf, depthWiseWeightView, initializeParams));
  ...
  public Map<String, INDArray> getGradientsFromFlattened(
    ...
    INDArray depthWiseWeightGradientView =
        gradientViewReshape
            .get(NDArrayIndex.interval(biasParams, biasParams + depthWiseParams))
  ...
  protected INDArray createDepthWiseWeightMatrix(
