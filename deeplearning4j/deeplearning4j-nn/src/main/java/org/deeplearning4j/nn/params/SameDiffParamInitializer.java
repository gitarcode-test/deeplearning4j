...
public class SameDiffParamInitializer implements ParamInitializer {
  ...
  public Map<String, INDArray> init(
    ...
    Map<String, INDArray> out =
        subsetAndReshape(
            sd.getLayerParams().getParameterKeys(),
            sd.getLayerParams().getParamShapes(),
            paramsView,
            sd);
    if (initializeParams) {
  ...
  private Map<String, INDArray> subsetAndReshape(
      List<String> params,
    ...
    for (String s : params) {
      ...
      if (length <= 0) {
        throw new IllegalStateException(
            "Invalid array state for parameter \""
                + s
                + "\" in layer "
                + layerName
                + " of type "
                ...
                + ": parameter length ("
                + length
                + ") must be > 0 - parameter array shape: "
                + Arrays.toString(sh));
      }
