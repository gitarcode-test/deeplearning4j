...
public class Deconvolution3DLayer extends BaseLayer<Deconvolution3D> {
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    if (input.rank() != 5) {
      throw new DL4JInvalidInputException(
          "Got rank "
              + input.rank()
              + " array as input to Deconvolution3DLayer with shape "
              + Arrays.toString(input.shape())
              + ". Expected rank 5 array with shape [minibatchSize, channels, inputHeight,"
              + " inputWidth, inputDepth] or [minibatchSize, inputHeight, inputWidth, inputDepth,"
              + " channels]. "
              + layerId());
    }

    INDArray weights =
        getParamWithNoise(DeconvolutionParamInitializer.WEIGHT_KEY, true, workspaceMgr);
    ...
    INDArray outEps =
        workspaceMgr.create(ArrayType.ACTIVATION_GRAD, weights.dataType(), input.shape(), 'c');
    ...
    CustomOp op =
        DynamicCustomOp.builder("deconv3d_bp")
            .addInputs(opInputs)
  ...
  protected INDArray preOutput(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    INDArray bias =
        getParamWithNoise(DeconvolutionParamInitializer.BIAS_KEY, training, workspaceMgr);
    INDArray weights =
        getParamWithNoise(DeconvolutionParamInitializer.WEIGHT_KEY, training, workspaceMgr);
    ...
    if (input.size(chDim) != layerConf().getNIn()) {
      ...
      throw new DL4JInvalidInputException(
          "Cannot do forward pass in Deconvolution3D layer (layer name = "
              + layerName
              + ", layer index = "
              + index
              ...
              + ", "
              + (ncdhw
                  ? "[minibatch,channels,height,width,depth]="
                  : "[minibatch,height,width,depth,channels]=")
              + Arrays.toString(input.shape())
              + "; expected"
              + " input channels = "
              + layerConf().getNIn()
    ...
    long[] outSize;
    long[] inSize =
        df == Convolution3D.DataFormat.NCDHW
            ? new long[] {(int) input.size(2), (int) input.size(3), (int) input.size(4)}
            : new long[] {(int) input.size(1), (int) input.size(2), (int) input.size(3)};
    if (cm == ConvolutionMode.Same) {
      outSize =
          ConvolutionUtils.getDeconvolution3DOutputSizeLong(
              input,
              kernel,
              strides,
              null,
              dilation,
              cm,
              layerConf().getDataFormat()); // Also performs validation
    ...
    long[] outShape =
        df == Convolution3D.DataFormat.NCDHW
            ? new long[] {miniBatch, layerConf().getNOut(), outH, outW, outD}
            : new long[] {miniBatch, outH, outW, outD, layerConf().getNOut()};
    ...
    CustomOp op =
        DynamicCustomOp.builder("deconv3d")
            .addInputs(opInputs)
            .addIntegerArguments(args)
            .addOutputs(output)
            .callInplace(false)
