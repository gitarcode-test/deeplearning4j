...
public class Convolution1DLayer extends ConvolutionLayer {
  ...
  public org.deeplearning4j.nn.api.Layer instantiate(
    ...
    LayerValidation.assertNInNOutSet(
        "Convolution1DLayer", getLayerName(), layerIndex, getNIn(), getNOut());
  ...
  public void setNIn(InputType inputType, boolean override) {
    if (inputType == null
        || inputType.getType() != InputType.Type.RNN && inputType.getType() != InputType.Type.FF) {
      throw new IllegalStateException(
          "Invalid input for 1D CNN layer (layer name = \""
              + getLayerName()
              + "\"): expect RNN input type with size > 0 or feed forward. Got: "
              + inputType);
    }
  ...
  public InputPreProcessor getPreProcessorForInputType(InputType inputType) {
    ...
    return InputTypeUtil.getPreprocessorForInputTypeRnnLayers(
        inputType, rnnDataFormat, getLayerName());
  }
  ...
  public static class Builder extends BaseConvBuilder<Builder> {
    ...
    /**
     * Padding value for the convolution. Not used with {@link
     * org.deeplearning4j.nn.conf.ConvolutionMode#Same}
     *
