...
public class SpaceToBatch
    extends AbstractLayer<org.deeplearning4j.nn.conf.layers.SpaceToBatchLayer> {
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    INDArray input =
        this.input.castTo(
            dataType); // Cast to network dtype if required (no-op if already correct type)
    ...
    CustomOp op =
        DynamicCustomOp.builder("batch_to_space_nd")
  ...
  protected INDArray preOutput(
    ...
    if (input.rank() != 4) {
      throw new DL4JInvalidInputException(
          "Got rank "
              + input.rank()
              + " array as input to space to batch with shape "
    ...
    long[] outShape =
        nchw
            ? new long[] {outMiniBatch, depth, outH, outW}
            : new long[] {outMiniBatch, outH, outW, depth};
    ...
    CustomOp op =
        DynamicCustomOp.builder("space_to_batch_nd")
