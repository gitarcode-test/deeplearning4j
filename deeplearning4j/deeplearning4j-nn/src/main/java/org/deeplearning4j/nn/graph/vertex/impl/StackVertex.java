...
public class StackVertex extends BaseGraphVertex {
  ...
  public INDArray doForward(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    if (inShape.length == 3) {
      ...
      for (int i = 0; i < inputs.length; i++) {
        out.put(
            new INDArrayIndex[] {
              NDArrayIndex.interval(i * numExamples, (i + 1) * numExamples),
              NDArrayIndex.all(),
              NDArrayIndex.interval(0, inputs[i].size(2))
            },
            inputs[i]);
      ...
      switch (epsilon.rank()) {
        ...
        case 3:
          if (lastInputShapes != null) {
            ...
            out[i] =
                epsilon.get(
                    NDArrayIndex.interval(i * step, (i + 1) * step),
                    NDArrayIndex.all(),
  ...
  public Pair<INDArray, MaskState> feedForwardMaskArrays(
    ...
    // stacking along dimension 0
    // Given masks are all either 1d (column vector) or 2d (examples, timeSeriesLength) we can just
    // vStack the masks
    ...
    if (allSameLength) {
      ...
      for (int i = 0; i < maskArrays.length; i++) {
        outMask.put(
            new INDArrayIndex[] {
              NDArrayIndex.interval(i * numExamples, (i + 1) * numExamples),
              NDArrayIndex.interval(0, maskArrays[i].size(1))
            },
            maskArrays[i]);
      }
