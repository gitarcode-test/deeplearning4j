...
public class BidirectionalLayer implements RecurrentLayer {
  ...
  @Getter private Layer fwd;
  @Getter private Layer bwd;
  ...
  public Map<String, INDArray> rnnGetPreviousState() {
    throw new UnsupportedOperationException(
        "Not supported: cannot RnnTimeStep bidirectional layers therefore "
  ...
  public INDArray rnnActivateUsingStoredState(
    ...
    throw new UnsupportedOperationException(
        "Not supported: cannot use this method (or truncated BPTT) with bidirectional layers");
  }
  ...
  public double calcRegularizationScore(boolean backpropParamsOnly) {
    return fwd.calcRegularizationScore(backpropParamsOnly)
        + bwd.calcRegularizationScore(backpropParamsOnly);
  }
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    // workspaceMgr.keepOpen(ArrayType.INPUT, ArrayType.ACTIVATION_GRAD,
    // ArrayType.BP_WORKING_MEM,ArrayType.ACTIVATIONS);
    ...
    eBwd =
        TimeSeriesUtils.reverseTimeSeries(
            eBwd, workspaceMgr, ArrayType.BP_WORKING_MEM, getRNNDataFormat());
    ...
    for (Map.Entry<String, INDArray> e : g1.getFirst().gradientForVariable().entrySet()) {
      g.gradientForVariable()
          .put(BidirectionalParamInitializer.FORWARD_PREFIX + e.getKey(), e.getValue());
    }
    for (Map.Entry<String, INDArray> e : g2.getFirst().gradientForVariable().entrySet()) {
      g.gradientForVariable()
          .put(BidirectionalParamInitializer.BACKWARD_PREFIX + e.getKey(), e.getValue());
    }

    INDArray g2Right = g2.getRight();
    INDArray g2Reversed =
        TimeSeriesUtils.reverseTimeSeries(g2Right, workspaceMgr, ArrayType.BP_WORKING_MEM);
  ...
  public INDArray activate(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    out2 =
        out2.rank() == 2
            ? out2
            : TimeSeriesUtils.reverseTimeSeries(
                out2, workspaceMgr, ArrayType.FF_WORKING_MEM, getRNNDataFormat());
  ...
  public void setMaskArray(INDArray maskArray) {
    ...
    bwd.setMaskArray(
        TimeSeriesUtils.reverseTimeSeriesMask(
  ...
  public Pair<INDArray, MaskState> feedForwardMaskArray(
    ...
    Pair<INDArray, MaskState> ret =
        fwd.feedForwardMaskArray(maskArray, currentMaskState, minibatchSize);
    bwd.feedForwardMaskArray(
        TimeSeriesUtils.reverseTimeSeriesMask(
            maskArray, LayerWorkspaceMgr.noWorkspaces(), ArrayType.INPUT), // TODO
        currentMaskState,
        minibatchSize);
