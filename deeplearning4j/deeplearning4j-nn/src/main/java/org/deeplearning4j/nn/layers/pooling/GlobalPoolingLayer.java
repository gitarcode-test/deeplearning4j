...
public class GlobalPoolingLayer
  ...
  public INDArray activate(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    if (input.rank() == 3) {
      ...
      throw new UnsupportedOperationException(
          "Received rank "
              ...
              + "). Only rank 3 (time series), rank 4 (images"
    ...
    if (maskArray == null) {
      ...
      if (input.rank() == 3) {
        ...
        reduced2d =
            MaskedReductionUtil.maskedPoolingTimeSeries(
                poolingType, input, maskArray, pNorm, dataType);
      } else if (input.rank() == 4) {
        ...
        if (maskArray.rank() != 4) {
          throw new UnsupportedOperationException(
              "Only 4d mask arrays are currently supported for masked global reductions "
                  ...
                  + ") "
                  + " - when used in conjunction with input data of shape [batch,channels,h,w]="
                  + Arrays.toString(input.shape())
                  + " 4d masks should have shape [batchSize,1,h,1] or [batchSize,1,w,1] or"
                  + " [batchSize,1,h,w]"
                  + layerId());
        }

        reduced2d =
            MaskedReductionUtil.maskedPoolingConvolution(
                poolingType, input, maskArray, pNorm, dataType);
      } else {
        throw new UnsupportedOperationException(
            "Invalid input: is rank " + input.rank() + " " + layerId());
      }
    }
    ...
    if (layerConf().isCollapseDimensions()) {
      ...
      if (input.rank() == 3) {
        return workspaceMgr.leverageTo(
            ...
            reduced2d.reshape(reduced2d.ordering(), inputShape[0], inputShape[1], 1));
      } else if (input.rank() == 4) {
        return workspaceMgr.leverageTo(
            ArrayType.ACTIVATIONS,
            reduced2d.reshape(reduced2d.ordering(), inputShape[0], inputShape[1], 1, 1));
      } else {
        return workspaceMgr.leverageTo(
            ArrayType.ACTIVATIONS,
            reduced2d.reshape(reduced2d.ordering(), inputShape[0], inputShape[1], 1, 1, 1));
      }
    }
  }
    ...
    switch (poolingType) {
      ...
      default:
        throw new RuntimeException(
            "Unknown or not supported pooling type: " + poolingType + " " + layerId());
    }
  }
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    if (!layerConf().isCollapseDimensions() && epsilon.rank() != 2) {
      ...
      // Don't collapse dims case: error should be [minibatch, vectorSize, 1] or [minibatch,
      // channels, 1, 1]
    ...
    if (maskArray == null) {
      ...
      if (input.rank() == 3) {
        epsilonNd =
            MaskedReductionUtil.maskedPoolingEpsilonTimeSeries(
                poolingType, input, maskArray, epsilon, pNorm);
      } else if (input.rank() == 4) {
        epsilonNd =
            MaskedReductionUtil.maskedPoolingEpsilonCnn(
                poolingType, input, maskArray, epsilon, pNorm, dataType);
      } else {
  ...
  private INDArray epsilonHelperFullArray(INDArray inputArray, INDArray epsilon, long[] poolDim) {
    ...
    switch (poolingType) {
      ...
      case PNORM:
        ...
        denom.rdivi(epsilon);
        Nd4j.getExecutioner()
            .execAndReturn(new BroadcastMulOp(numerator, denom, numerator, broadcastDims));
  ...
  public Pair<INDArray, MaskState> feedForwardMaskArray(
    ...
    // Global pooling layer: no masking is possible after this point... i.e., masks have been taken
    // into account
