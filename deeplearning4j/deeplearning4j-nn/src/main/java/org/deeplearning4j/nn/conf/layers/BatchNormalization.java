...
public class BatchNormalization extends FeedForwardLayer {
    ...
    if (nIn <= 0 || override) {
      switch (inputType.getType()) {
        ...
        case CNNFlat:
          ...
          break;
        case RNN:
          InputType.InputTypeRecurrent inputTypeRecurrent =
  ...
  public static class Builder extends FeedForwardLayer.Builder<Builder> {
    ...
    /**
     * At test time: we can use a global estimate of the mean and variance, calculated using a
     * moving average of the batch means/variances. This moving average is implemented as:<br>
    ...
    /**
     * If doing minibatch training or not. Default: true. Under most circumstances, this should be
     * set to true. If doing full batch training (i.e., all examples in a single DataSet object -
     * very small data sets) then this should be set to false. Affects how global mean/variance
     * estimates are calculated.
     */
    protected boolean isMinibatch = true; // TODO auto set this if layer conf is batch

    /**
     * If set to true: lock the gamma and beta parameters to the values for each activation,
     * specified by {@link #gamma(double)} and {@link #beta(double)}. Default: false -> learn gamma
     * and beta parameter values during network training.
     */
    protected boolean lockGammaBeta = false;

    /**
     * Used only when 'true' is passed to {@link #lockGammaBeta(boolean)}. Value is not used
     * otherwise.<br>
     * Default: 1.0
     */
    protected double gamma = 1.0;
     ...
     * supported. useLogStd(false): equivalent to 1.0.0-beta3 and earlier. The variance "parameter"
     * is stored directly as variable<br>
     ...
     * storing the variance directly can cause numerical issues. For example, a standard deviation
     * of 1e-3 (something that could be encountered in practice) gives a variance of 1e-6, which can
     * be problematic for 16-bit floating point
     */
     ...
     * Default: 0.0
     *
     * @param beta Beta parameter for all activations, used only with locked gamma/beta
     *     configuration mode
     */
     ...
     * BatchNormalizationLayer will be used
     *
     * @param allowFallback Whether fallback to non-CuDNN implementation should be used
     */
