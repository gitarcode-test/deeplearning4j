...
public class VariationalAutoencoder extends BasePretrainNetwork {
  ...
  public Layer instantiate(
    ...
    LayerValidation.assertNInNOutSet(
        "VariationalAutoencoder", getLayerName(), layerIndex, getNIn(), getNOut());
  ...
  public LayerMemoryReport getMemoryReport(InputType inputType) {
    ...
    return new LayerMemoryReport.Builder(
            layerName, VariationalAutoencoder.class, inputType, outputType)
  ...
  public static class Builder extends BasePretrainNetwork.Builder<Builder> {
    ...
    /**
     * Size of the encoder layers, in units. Each encoder layer is functionally equivalent to a
     * {@link org.deeplearning4j.nn.conf.layers.DenseLayer}. Typically the number and size of the
     * decoder layers (set via {@link #decoderLayerSizes(int...)} is similar to the encoder layers.
     */
    private int[] encoderLayerSizes = new int[] {100};

    /**
     * Size of the decoder layers, in units. Each decoder layer is functionally equivalent to a
     * {@link org.deeplearning4j.nn.conf.layers.DenseLayer}. Typically the number and size of the
     * decoder layers is similar to the encoder layers (set via {@link #encoderLayerSizes(int...)}.
     */
     ...
     * - {@link GaussianReconstructionDistribution} + {identity or tanh} for real-valued (Gaussian)
     * data<br>
     ...
     * Care should be taken with this, as some activation functions (relu, etc) are not suitable due
     * to being bounded in range [0,infinity).
     */
     ...
     * Default value: 1.
     *
     * <p>This is parameter L from Kingma and Welling: "In our experiments we found that the number
     * of samples L per datapoint can be set to 1 as long as the minibatch size M was large enough,
     * e.g. M = 100."
     */
     ...
     * ReconstructionDistribution. Note that this is NOT following the standard VAE design (as per
     * Kingma & Welling), which assumes a probabilistic output - i.e., some p(x|z). It is however a
     * valid network configuration, allowing for optimization of more traditional objectives such as
     * mean squared error.<br>
     * Note: clearly, setting the loss function here will override any previously set recontruction
     * distribution
     *
     * @param outputActivationFn Activation function for the output/reconstruction
     * @param lossFunction Loss function to use
     */
    public Builder lossFunction(
        IActivation outputActivationFn, LossFunctions.LossFunction lossFunction) {
     ...
     */
    public Builder lossFunction(
        Activation outputActivationFn, LossFunctions.LossFunction lossFunction) {
      return lossFunction(
          outputActivationFn.getActivationFunction(), lossFunction.getILossFunction());
    }
    ...
    /**
     * Set the size of the VAE state Z. This is the output size during standard forward pass, and
     * the size of the distribution P(Z|data) during pretraining.
     *
