...
public class Subsampling3DLayer extends NoParamLayer {
  ...
  protected Convolution3D.DataFormat dataFormat =
      Convolution3D.DataFormat.NCDHW; // Default for 1.0.0-beta3 and earlier (before config added)
  ...
  public org.deeplearning4j.nn.api.Layer instantiate(
    ...
    org.deeplearning4j.nn.layers.convolution.subsampling.Subsampling3DLayer ret =
        new org.deeplearning4j.nn.layers.convolution.subsampling.Subsampling3DLayer(
  ...
  public InputType getOutputType(int layerIndex, InputType inputType) {
    if (inputType == null || inputType.getType() != InputType.Type.CNN3D) {
      throw new IllegalStateException(
          "Invalid input for Subsampling 3D layer (layer name=\""
              + getLayerName()
    ...
    if (inChannels > Integer.MAX_VALUE) throw new ND4JArraySizeException();
    return InputTypeUtil.getOutputTypeCnn3DLayers(
        inputType,
        dataFormat,
        kernelSize,
        stride,
  ...
  public LayerMemoryReport getMemoryReport(InputType inputType) {
    ...
    // During forward pass: im2col array + reduce. Reduce is counted as activations, so only im2col
    // is working mem
    val im2colSizePerEx =
        c.getChannels()
            * outputType.getHeight()
            * outputType.getWidth()
            * outputType.getDepth()
            * kernelSize[0]
            * kernelSize[1];

    // Current implementation does NOT cache im2col etc... which means: it's recalculated on each
    // backward pass
  ...
  @Setter
  @NoArgsConstructor
  protected abstract static class BaseSubsamplingBuilder<T extends BaseSubsamplingBuilder<T>>
    ...
    /**
     * When using CuDNN and an error is encountered, should fallback to the non-CuDNN
     * implementatation be allowed? If set to false, an exception in CuDNN will be propagated back
     * to the user. If false, the built-in (non-CuDNN) implementation for ConvolutionLayer will be
     * used
     */
    protected boolean cudnnAllowFallback = true;
    ...
    public void setDilation(int... dilation) {
      Preconditions.checkArgument(
          dilation.length == 1 || dilation.length == 3,
          "Must have 1 or 3 dilation values - got %s",
          dilation);
    ...
    protected BaseSubsamplingBuilder(
        PoolingType poolingType, int[] kernelSize, int[] stride, int[] padding) {
      this.setPoolingType(poolingType.toPoolingType());
      this.setKernelSize(kernelSize);
      this.setStride(stride);
      this.setPadding(padding);
    ...
    protected BaseSubsamplingBuilder(
        org.deeplearning4j.nn.conf.layers.PoolingType poolingType, int[] kernelSize) {
    ...
    protected void setConvolutionMode(ConvolutionMode convolutionMode) {
      Preconditions.checkState(
          ...
          "Causal convolution mode can only be used with 1D"
     ...
     * When using CuDNN and an error is encountered, should fallback to the non-CuDNN
     * implementatation be allowed? If set to false, an exception in CuDNN will be propagated back
     * to the user. If true, the built-in (non-CuDNN) implementation for ConvolutionLayer will be
     * used
     *
     ...
     * implementation be allowed? If set to false, an exception in the helper will be propagated
     * back to the user. If true, the built-in (non-MKL/CuDNN) implementation for Subsampling3DLayer
     * will be used
     *
     * @param allowFallback Whether fallback to non-CuDNN implementation should be used
     */
