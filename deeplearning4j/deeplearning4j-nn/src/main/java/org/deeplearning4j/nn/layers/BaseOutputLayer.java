...
public abstract class BaseOutputLayer<
        LayerConfT extends org.deeplearning4j.nn.conf.layers.BaseOutputLayer>
  ...
  public double computeScore(
    ...
    double score =
        lossFunction.computeScore(
            labels2d, preOut, layerConf().getActivationFn(), maskArray, false);
  ...
  private Pair<Gradient, INDArray> getGradientsAndDelta(
    ...
    Nd4j.gemm(
        input.castTo(weightGradView.dataType()),
        delta,
        weightGradView,
        true,
        false,
        1.0,
        0.0); // Equivalent to:  weightGradView.assign(input.transpose().mmul(delta));
  ...
  protected void applyMask(INDArray to) {
    ...
    if (maskArray.isColumnVectorOrScalar()) {
      ...
      throw new IllegalStateException(
          "Invalid mask array: per-example masking should be a column vector, per output masking"
              + " arrays should be the same shape as the output/labels arrays. Mask shape: "
              + Arrays.toString(maskArray.shape())
              + ", output shape: "
              + Arrays.toString(to.shape())
              + layerId());
    }
  }
