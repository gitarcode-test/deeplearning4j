...
public class Subsampling3DLayer
  ...
  public Subsampling3DLayer(NeuralNetConfiguration conf, DataType dataType) {
    ...
    this.convolutionMode =
        ((org.deeplearning4j.nn.conf.layers.Subsampling3DLayer) conf.getLayer())
            .getConvolutionMode();
  }
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    if (convolutionMode == ConvolutionMode.Same) {
      outSize =
          Convolution3DUtils.get3DOutputSize(
    ...
    INDArray outEpsilon =
        workspaceMgr.createUninitialized(
            ...
            epsilon.dataType(),
            isNCDHW
                ? new long[] {miniBatch, inChannels, inD, inH, inW}
                : new long[] {miniBatch, inD, inH, inW, inChannels},
            'c');
    ...
    String opName =
        layerConf().getPoolingType() == PoolingType.MAX ? "maxpool3dnew_bp" : "avgpool3dnew_bp";

    CustomOp op =
        DynamicCustomOp.builder(opName)
  ...
  public INDArray activate(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    if (input.rank() != 5) {
      if (isNCDHW) {
        throw new DL4JInvalidInputException(
            "Got rank "
                + input.rank()
                + " array as input to Subsampling3DLayer with shape "
        ...
        throw new DL4JInvalidInputException(
            "Got rank "
                ...
                + " array as input to Subsampling3DLayer with shape "
                + Arrays.toString(input.shape())
                + ". Expected rank 5 array with shape [minibatchSize, inputDepth, inputHeight,"
                + " inputWidth, channels] when dataFormat=NDHWC. "
                + layerId());
      }
    }
    ...
    if (convolutionMode == ConvolutionMode.Same) {
      ...
      pad =
          Convolution3DUtils.get3DSameModeTopLeftPadding(
              outSize, inShape, kernel, strides, dilation);
    } else {
      pad = layerConf().getPadding();
      outSize =
          Convolution3DUtils.get3DOutputSize(
    ...
    String opName =
        layerConf().getPoolingType() == PoolingType.MAX ? "maxpool3dnew" : "avgpool3dnew";
    ...
    INDArray output =
        workspaceMgr.createUninitialized(
            ...
            input.dataType(),
            isNCDHW
                ? new long[] {miniBatch, inChannels, outD, outH, outW}
                : new long[] {miniBatch, outD, outH, outW, inChannels},
            'c');
    ...
    int[] intArgs =
        new int[] {
          ...
          dilation[2],
          convolutionMode == ConvolutionMode.Same ? 1 : 0,
          0, // Extra param - 0 = exclude padding for average divisor (only applicable for average
             // pooling)
          isNCDHW ? 0 : 1
        };
