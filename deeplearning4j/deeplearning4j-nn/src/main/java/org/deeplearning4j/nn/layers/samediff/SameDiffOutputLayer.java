...
public class SameDiffOutputLayer
  ...
  @Getter @Setter protected INDArray labels;
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    assertInputSet(true);
    Preconditions.checkState(
        !layerConf().labelsRequired() || labels != null,
        "Cannot execute backprop: Labels are not set. If labels are not required for this SameDiff"
            + " output layer, override SameDiffOutputLayer.labelsRequired() to return false"
            + " instead");
    ...
    try (MemoryWorkspace ws = Nd4j.getWorkspaceManager().scopeOutOfWorkspaces()) {
      ...
      if (sameDiff.getFunction("grad") == null) sameDiff.createGradFunction(INPUT_KEY);
    }
  ...
  protected void doInit() {
    try (MemoryWorkspace ws = Nd4j.getWorkspaceManager().scopeOutOfWorkspaces()) {
      ...
      // Use SingleThreadArrayHolder so we can use views (also don't nede multithreading here, DL4J
      // is not thread safe)
