...
public abstract class AbstractSameDiffLayer extends Layer {
  ...
  protected AbstractSameDiffLayer(Builder builder) {
    ...
    // Check that this class has a no-arg constructor for JSON: better to detect this now provide
    // useful information
    ...
    try {
      ...
      log.warn(
          "***SameDiff layer {} does not have a zero argument (no-arg) constructor.***\n"
              + "A no-arg constructor is required for JSON deserialization, which is used for both"
              + " model saving and distributed (Spark) training.\n"
              + "A no-arg constructor (private, protected or public) as well as setters (or simply"
              + " a Lombok @Data annotation) should be added to avoid JSON errors later.",
          getClass().getName());
  ...
  public INDArray onesMaskForInput(INDArray input) {
    if (input.rank() == 2) {
      ...
      throw new IllegalStateException(
          "When using masking with rank 1 or 6+ inputs, the onesMaskForInput method must be"
              + " implemented, in order to determine the correct mask shape for this layer");
    }
  }
  ...
  public abstract static class Builder<T extends Builder<T>> extends Layer.Builder<T> {
    ...
    /**
     * Gradient updater configuration, for the biases only. If not set, biases will use the updater
     * as set by {@link #updater(IUpdater)}
     */
    ...
    /**
     * Set the regularization for the parameters (excluding biases) - for example {@link
     * WeightDecay}<br>
     *
