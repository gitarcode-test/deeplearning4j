...
public class OCNNOutputLayer
  ...
  @Setter @Getter private IActivation activation = new ActivationReLU();
  ...
  public OCNNOutputLayer(NeuralNetConfiguration conf, DataType dataType) {
    ...
    org.deeplearning4j.nn.conf.ocnn.OCNNOutputLayer ocnnOutputLayer =
        (org.deeplearning4j.nn.conf.ocnn.OCNNOutputLayer) conf.getLayer();
   ...
   * @param training whether score should be calculated at train or test time (this affects things
   *     like application of dropout, etc)
  ...
  public double computeScore(
    ...
    double score =
        lossFunction.computeScore(
            ...
            maskArray,
            false);
    if (conf().isMiniBatch()) score /= getInputMiniBatchSize();
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    Pair<Gradient, INDArray> pair =
        getGradientsAndDelta(
            preOutput2d(true, workspaceMgr),
            workspaceMgr); // Returns Gradient and delta^(this), not Gradient and epsilon^(this-1)
    // 150
    long inputShape =
        ((org.deeplearning4j.nn.conf.ocnn.OCNNOutputLayer) this.getConf().getLayer()).getNIn();
    ...
    INDArray epsilonNext =
        workspaceMgr.createUninitialized(
            ...
            input.dataType(),
            new long[] {inputShape, delta.length()},
            'f');
    epsilonNext = epsilonNext.assign(delta.broadcast(epsilonNext.shape())).transpose();

    // Normally we would clear weightNoiseParams here - but we want to reuse them for forward +
    // backward + score
  ...
  private Pair<Gradient, INDArray> getGradientsAndDelta(
    ...
    INDArray delta =
        lossFunction.computeGradient(labels2d, preOut, layerConf().getActivationFn(), maskArray);
    org.deeplearning4j.nn.conf.ocnn.OCNNOutputLayer conf =
        (org.deeplearning4j.nn.conf.ocnn.OCNNOutputLayer) conf().getLayer();
    ...
    if (conf.getLastEpochSinceRUpdated() == 0 && epochCount == 0) {
      ...
      if (window == null) {
        window =
            Nd4j.createUninitializedDetached(preOut.dataType(), conf.getWindowSize()).assign(0.0);
      }

      if (batchWindowSizeIndex < window.length() - currentR.length()) {
        window.put(
            new INDArrayIndex[] {
              NDArrayIndex.interval(batchWindowSizeIndex, batchWindowSizeIndex + currentR.length())
            },
            currentR);
        ...
        window.put(
            new INDArrayIndex[] {
              NDArrayIndex.interval(window.length() - windowIdx, window.length())
            },
            currentR.get(NDArrayIndex.interval(0, windowIdx)));
      }
    ...
    INDArray firstVertDerivV =
        layerConf()
            ...
            .getFirst()
            .muliRowVector(getParam(W_KEY).neg());
    firstVertDerivV =
        firstVertDerivV
            .muliColumnVector(delta)
            .reshape('f', input.size(0), 1, layerConf().getHiddenSize());
    INDArray secondTermDerivV = input.reshape('f', input.size(0), getParam(V_KEY).size(0), 1);
    ...
    INDArray derivV = mulResult.mean(0).muli(oneDivNu).addi(getParam(V_KEY));
  ...
  private INDArray doOutput(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    INDArray output =
        workspaceMgr.createUninitialized(ArrayType.ACTIVATIONS, input.dataType(), input.size(0));
  ...
  public class OCNNLossFunction implements ILossFunction {
    ...
    public INDArray computeScoreArray(
        INDArray labels, INDArray preOutput, IActivation activationFn, INDArray mask) {
    ...
    public INDArray computeGradient(
      ...
      INDArray target =
          relu.backprop(preAct, Nd4j.ones(preOutput.dataType(), preAct.shape())).getFirst();
      return target;
    }
