...
public class MergeVertex extends BaseGraphVertex {
  ...
  public MergeVertex(
      ComputationGraph graph, String name, int vertexIndex, DataType dataType, int mergeAxis) {
      ...
      if (currShape[0] != nExamples) {
        throw new IllegalStateException(
            "Cannot merge activations with different number of examples (activations[0] shape: "
                + Arrays.toString(in[0].shape())
                + ", activations["
                + i
                + "] shape: "
                + Arrays.toString(in[i].shape()));
      }
    }
  ...
  public Pair<Gradient, INDArray[]> doBackward(boolean tbptt, LayerWorkspaceMgr workspaceMgr) {
    ...
    if (forwardPassShapes.length == 1) {
      // No op case
      return new Pair<>(
          null, new INDArray[] {workspaceMgr.leverageTo(ArrayType.ACTIVATION_GRAD, epsilon)});
    }
    ...
    for (int i = 0; i < out.length; i++)
      out[i] =
          workspaceMgr.createUninitialized(
              ArrayType.ACTIVATION_GRAD, epsilon.dataType(), forwardPassShapes[i]);
    ...
    switch (fwdPassRank) {
      case 2:
        ...
        for (int i = 0; i < forwardPassShapes.length; i++) {
          out[i].assign(
              epsilon.get(
                  NDArrayIndex.all(), // All rows
                  NDArrayIndex.interval(
                      cumulative, cumulative + forwardPassShapes[i][1]))); // subset of columns
          ...
          out[i].assign(
              epsilon.get(
                  indices(
                      ...
                      mergeAxis,
                      cumulative,
                      cumulative + forwardPassShapes[i][mergeAxis]))); // All time steps
          ...
          out[i].assign(
              epsilon.get(
                  indices(
                      ...
                      mergeAxis,
                      cumulative,
                      cumulative + forwardPassShapes[i][mergeAxis]))); // height
      ...
      default:
        throw new RuntimeException(
            "Invalid rank during forward pass (not 2, 3, 4)"); // Should never happen
    }
