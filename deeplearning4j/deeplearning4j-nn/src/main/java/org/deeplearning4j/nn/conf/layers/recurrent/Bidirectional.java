...
public class Bidirectional extends Layer {
  ...
  /**
   * This Mode enumeration defines how the activations for the forward and backward networks should
   * be combined.<br>
   ...
   * Where 'forward' is the activations for the forward RNN, and 'backward' is the activations for
   * the backward RNN. In all cases except CONCAT, the output activations size is the same size as
   * the standard RNN that is being wrapped by this layer. In the CONCAT case, the output
   * activations size (dimension 1) is 2x larger than the standard RNN's activations array.
   */
  public enum Mode {
    ADD,
    MUL,
  ...
  public Bidirectional(@NonNull Mode mode, @NonNull Layer layer) {
    if (!(layer instanceof BaseRecurrentLayer
        || layer instanceof LastTimeStep
        || layer instanceof BaseWrapperLayer)) {
      throw new IllegalArgumentException(
          "Cannot wrap a non-recurrent layer: "
  ...
  public org.deeplearning4j.nn.api.Layer instantiate(
    ...
    org.deeplearning4j.nn.api.Layer f =
        fwd.instantiate(c1, trainingListeners, layerIndex, fp, initializeParams, networkDataType);

    org.deeplearning4j.nn.api.Layer b =
        bwd.instantiate(c2, trainingListeners, layerIndex, bp, initializeParams, networkDataType);
