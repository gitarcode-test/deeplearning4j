...
public class AutoEncoder
  ...
  public INDArray encode(INDArray v, boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    INDArray ret =
        workspaceMgr.createUninitialized(ArrayType.ACTIVATIONS, W.dataType(), v.size(0), W.size(1));
  ...
  public INDArray decode(INDArray y, LayerWorkspaceMgr workspaceMgr) {
    ...
    INDArray vBias =
        getParamWithNoise(PretrainParamInitializer.VISIBLE_BIAS_KEY, true, workspaceMgr);
    INDArray preAct = y.mmul(W.transpose()).addiRowVector(vBias);
    return workspaceMgr.leverageTo(
        ArrayType.ACTIVATIONS, layerConf().getActivationFn().getActivation(preAct, true));
  }
  ...
  public void computeGradientAndScore(LayerWorkspaceMgr workspaceMgr) {
    ...
    INDArray hiddenLoss =
        layerConf().getSparsity() == 0
            ? visibleLoss.mmul(W).muli(y).muli(y.rsub(1))
            : visibleLoss.mmul(W).muli(y).muli(y.add(-layerConf().getSparsity()));

    INDArray wGradient =
        corruptedX.transpose().mmul(hiddenLoss).addi(visibleLoss.transpose().mmul(y));
