...
public class Cropping3DLayer
    extends AbstractLayer<org.deeplearning4j.nn.conf.layers.convolutional.Cropping3D> {
  ...
  public Cropping3DLayer(NeuralNetConfiguration conf, DataType dataType) {
    super(conf, dataType);
    this.cropping =
        ((org.deeplearning4j.nn.conf.layers.convolutional.Cropping3D) conf.getLayer())
            .getCropping();
  }
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    INDArray epsNext =
        workspaceMgr.create(ArrayType.ACTIVATION_GRAD, input.dataType(), inShape, 'c');
    INDArray epsNextSubset = inputSubset(epsNext);
    epsNextSubset.assign(epsilon);
    return new Pair<>(
        new DefaultGradient(), workspaceMgr.leverageTo(ArrayType.ACTIVATION_GRAD, epsNext));
  }
