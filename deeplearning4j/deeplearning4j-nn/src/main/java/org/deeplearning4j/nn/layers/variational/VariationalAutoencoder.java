...
public class VariationalAutoencoder implements Layer {
  ...
  protected INDArray getParamWithNoise(
      String param, boolean training, LayerWorkspaceMgr workspaceMgr) {
    INDArray p;
  ...
  public void computeGradientAndScore(LayerWorkspaceMgr workspaceMgr) {
    ...
    INDArray pzxLogStd2W =
        getParamWithNoise(VariationalAutoencoderParamInitializer.PZX_LOGSTD2_W, true, workspaceMgr);
    INDArray pzxLogStd2b =
        getParamWithNoise(VariationalAutoencoderParamInitializer.PZX_LOGSTD2_B, true, workspaceMgr);

    INDArray pzxLogStd2Pre =
        fwd.encoderActivations[fwd.encoderActivations.length - 1]
            .mmul(pzxLogStd2W)
    ...
    INDArray[] encoderActivationDerivs =
        (numSamples > 1 ? new INDArray[encoderLayerSizes.length] : null);
    for (int l = 0; l < numSamples; l++) { // Default (and in most cases) numSamples == 1
      double gemmCConstant =
          (l == 0
              ? 0.0
              : 1.0); // 0 for first one (to get rid of previous buffer data), otherwise 1 (for
                      // adding)
      ...
      INDArray pxzw =
          getParamWithNoise(VariationalAutoencoderParamInitializer.PXZ_W, true, workspaceMgr);
      INDArray pxzb =
          getParamWithNoise(VariationalAutoencoderParamInitializer.PXZ_B, true, workspaceMgr);

      if (l == 0) {
        // Need to add other component of score, in addition to negative log probability
        // Note the negative here vs. the equation in Kingma & Welling: this is because we are
        // minimizing the negative of
        // variational lower bound, rather than maximizing the variational lower bound
        // Unlike log probability (which is averaged over samples) this should be calculated just
        // once
      ...
      double logPTheta =
          reconstructionDistribution.negLogProbability(input, pxzDistributionPreOut, true);
      ...
      if (trainingListeners != null
          && !trainingListeners.isEmpty()
          && l == 0) { // Note: only doing this on the *first* sample
      ...
      for (int i = nDecoderLayers - 1; i >= 0; i--) {
        ...
        INDArray currentDelta =
            afn.backprop(decoderPreOut[i], epsilon)
      ...
      INDArray eZXMeanW =
          getParamWithNoise(VariationalAutoencoderParamInitializer.PZX_MEAN_W, true, workspaceMgr);
      INDArray eZXLogStdev2W =
          getParamWithNoise(
              VariationalAutoencoderParamInitializer.PZX_LOGSTD2_W, true, workspaceMgr);

      INDArray dLdz = epsilon;
      // If we were maximizing the equation in Kinga and Welling, this would be a .sub(meanZ). Here:
      // we are minimizing the negative instead
      ...
      INDArray dLdPreLogSigma2 =
          pzxActivationFn.backprop(pzxLogStd2Pre.dup(), dLdLogSigma2).getFirst();
      ...
      INDArray dLdZXLogStdev2W =
          gradientViews.get(VariationalAutoencoderParamInitializer.PZX_LOGSTD2_W);
      Nd4j.gemm(
          lastEncoderActivation, dLdPreMu, dLdZXMeanW, true, false, scaleFactor, gemmCConstant);
      Nd4j.gemm(
          lastEncoderActivation,
          dLdPreLogSigma2,
          dLdZXLogStdev2W,
          true,
          false,
      ...
      INDArray dLdZXLogStdev2b =
          gradientViews.get(VariationalAutoencoderParamInitializer.PZX_LOGSTD2_B);
      // If we were maximizing the equation in Kinga and Welling, this would be a .sub(meanZ). Here:
      // we are minimizing the negative instead
      if (l == 0) {
        dLdZXMeanb.assign(
            pzxActivationFn
                .backprop(fwd.getPzxMeanPreOut().dup(), dLdz.add(meanZ))
                .getFirst()
                .sum(0));
      ...
      epsilon =
          Nd4j.gemm(
              dLdPreMu, eZXMeanW, false,
              true); // Equivalent to: epsilon = eZXMeanW.mmul(dLdPreMu.transpose()).transpose();
                     // using   (AxB^T)^T = BxA^T
      // Next line: equivalent to
      // epsilon.addi(eZXLogStdev2W.mmul(dLdPreLogSigma2.transpose()).transpose());       using:
      // (AxB^T)^T = BxA^T
      ...
      for (int i = nEncoderLayers - 1; i >= 0; i--) {
        ...
        INDArray currentDelta;
        if (numSamples > 1) {
          // Re-use sigma-prime values for the encoder - these don't change based on multiple
          // samples,
          // only the errors do
          if (l == 0) {
    ...
    // Insert the gradients into the Gradient map in the correct order, in case we need to flatten
    // the gradient later
  ...
  public void setParams(INDArray params) {
    if (params.length() != this.paramsFlattened.length()) {
      throw new IllegalArgumentException(
          "Cannot set parameters: expected parameters vector of length "
              + this.paramsFlattened.length()
              + " but got parameters array of length "
              + params.length()
              + " "
              + layerId());
    }
    ...
    if (this.params != null && gradients.length() != numParams()) {
      throw new IllegalArgumentException(
          "Invalid input: expect gradients array of length "
              + numParams()
              + ", got gradient array of length of length "
              + gradients.length()
              + " "
              + layerId());
    }
  ...
  public int batchSize() {
    if (input.size(0) > Integer.MAX_VALUE) throw new ND4JArraySizeException();
  ...
  public boolean isPretrainParam(String param) {
    return !(param.startsWith("e")
        || param.startsWith(VariationalAutoencoderParamInitializer.PZX_MEAN_PREFIX));
  }
  ...
  public double calcRegularizationScore(boolean backpropParamsOnly) {
    ...
    for (Map.Entry<String, INDArray> e : paramTable().entrySet()) {
      if (backpropParamsOnly && isPretrainParam(e.getKey())) continue;
  ...
  public Pair<Gradient, INDArray> backpropGradient(
    ...
    INDArray meanW =
        getParamWithNoise(VariationalAutoencoderParamInitializer.PZX_MEAN_W, true, workspaceMgr);
    INDArray dLdMeanW =
        gradientViews.get(VariationalAutoencoderParamInitializer.PZX_MEAN_W); // f order
    INDArray lastEncoderActivation = fwd.encoderActivations[fwd.encoderActivations.length - 1];
      ...
      if (i == 0) {
        epsilon =
            workspaceMgr.createUninitialized(
                ...
                currentDelta.dataType(),
                new long[] {weights.size(0), currentDelta.size(0)},
                'f');
  ...
  private VAEFwdHelper doForward(
    ...
    INDArray mW =
        getParamWithNoise(
            VariationalAutoencoderParamInitializer.PZX_MEAN_W, training, workspaceMgr);
    INDArray mB =
        getParamWithNoise(
            VariationalAutoencoderParamInitializer.PZX_MEAN_B, training, workspaceMgr);
    try (MemoryWorkspace closeable = Nd4j.getWorkspaceManager().scopeOutOfWorkspaces()) {
      INDArray pzxMean =
          Nd4j.createUninitialized(mW.dataType(), new long[] {current.size(0), mW.size(1)}, 'f');
  ...
  public void fit() {
    if (input == null) {
      throw new IllegalStateException(
          "Cannot fit layer: layer input is null (not set) " + layerId());
    }

    if (solver == null) {
      try (MemoryWorkspace workspace = Nd4j.getMemoryManager().scopeOutOfWorkspaces()) {
        solver =
            new Solver.Builder().model(this).configure(conf()).listeners(getListeners()).build();
      }
    }
  ...
  /**
   * Calculate the reconstruction probability, as described in An & Cho, 2015 - "Variational
   * Autoencoder based Anomaly Detection using Reconstruction Probability" (Algorithm 4)<br>
   * The authors describe it as follows: "This is essentially the probability of the data being
   * generated from a given latent variable drawn from the approximate posterior distribution."<br>
   * <br>
   * Specifically, for each example x in the input, calculate p(x). Note however that p(x) is a
   * stochastic (Monte-Carlo) estimate of the true p(x), based on the specified number of samples.
   * More samples will produce a more accurate (lower variance) estimate of the true p(x) for the
   * current model parameters.<br>
   * <br>
   * Internally uses {@link #reconstructionLogProbability(INDArray, int)} for the actual
   * implementation. That method may be more numerically stable in some cases.<br>
   * <br>
   * The returned array is a column vector of reconstruction probabilities, for each example. Thus,
   * reconstruction probabilities can (and should, for efficiency) be calculated in a batched
   * manner.
   *
  ...
  public INDArray reconstructionProbability(INDArray data, int numSamples) {
    ...
    return Transforms.exp(
        reconstructionLogProb.castTo(DataType.DOUBLE),
        false); // Cast to double to reduce risk of numerical underflow
  }
   ...
   * @param numSamples Number of samples with which to base the reconstruction probability on.
   * @return Column vector of reconstruction log probabilities for each example (shape:
   *     [numExamples,1])
   */
  public INDArray reconstructionLogProbability(INDArray data, int numSamples) {
    ...
    if (reconstructionDistribution instanceof LossFunctionWrapper) {
      throw new UnsupportedOperationException(
          "Cannot calculate reconstruction log probability when using a LossFunction (via"
              + " LossFunctionWrapper) instead of a ReconstructionDistribution: ILossFunction"
              + " instances are not in general probabilistic, hence it is not possible to calculate"
    ...
    LayerWorkspaceMgr workspaceMgr =
        LayerWorkspaceMgr.noWorkspaces(); // TODO add workspace support to this method
    ...
    INDArray pzxLogStd2W =
        getParamWithNoise(
            VariationalAutoencoderParamInitializer.PZX_LOGSTD2_W, false, workspaceMgr);
    INDArray pzxLogStd2b =
        getParamWithNoise(
            VariationalAutoencoderParamInitializer.PZX_LOGSTD2_B, false, workspaceMgr);
    ...
    INDArray pxzw =
        getParamWithNoise(VariationalAutoencoderParamInitializer.PXZ_W, false, workspaceMgr);
    INDArray pxzb =
        getParamWithNoise(VariationalAutoencoderParamInitializer.PXZ_B, false, workspaceMgr);
    ...
    for (int i = 0; i < numSamples; i++) {
      ...
      for (int j = 0; j < nDecoderLayers; j++) {
        currentActivations =
            currentActivations.mmul(decoderWeights[j]).addiRowVector(decoderBiases[j]);
      ...
      if (i == 0) {
        ...
        sumReconstructionNegLogProbability.addi(
            reconstructionDistribution.exampleNegLogProbability(data, pxzDistributionPreOut));
      }
    }
   ...
   * generate output from P(x|z), where x = E[P(x|z)]<br>
   * i.e., return the mean value for the distribution P(x|z)
   *
   * @param latentSpaceValues Values for the latent space. size(1) must equal nOut configuration
   *     parameter
   * @return Sample of data: E[P(x|z)]
   */
  public INDArray generateAtMeanGivenZ(INDArray latentSpaceValues) {
    INDArray pxzDistributionPreOut =
        decodeGivenLatentSpaceValues(
            latentSpaceValues, LayerWorkspaceMgr.noWorkspaces()); // TODO workspace support
  ...
  private INDArray decodeGivenLatentSpaceValues(
      INDArray latentSpaceValues, LayerWorkspaceMgr workspaceMgr) {
    if (latentSpaceValues.size(1)
        != getParamWithNoise(VariationalAutoencoderParamInitializer.PZX_MEAN_W, false, workspaceMgr)
            .size(1)) {
      throw new IllegalArgumentException(
          "Invalid latent space values: expected size "
              + getParamWithNoise(
                      VariationalAutoencoderParamInitializer.PZX_MEAN_W, false, workspaceMgr)
                  .size(1)
              + ", got size (dimension 1) = "
              + latentSpaceValues.size(1)
              + " "
              + layerId());
    }
  ...
  /**
   * Does the reconstruction distribution have a loss function (such as mean squared error) or is it
   * a standard probabilistic reconstruction distribution?
   */
   ...
   * <b>NOTE (important):</b> This method is used ONLY for VAEs that have a standard neural network
   * loss function (i.e., an {@link ILossFunction} instance such as mean squared error) instead of
   * using a probabilistic reconstruction distribution P(x|z) for the reconstructions (as presented
   * in the VAE architecture by Kingma and Welling).<br>
   * You can check if the VAE has a loss function using {@link #hasLossFunction()}<br>
   * Consequently, the reconstruction error is a simple deterministic function (no Monte-Carlo
   * sampling is required, unlike {@link #reconstructionProbability(INDArray, int)} and {@link
  ...
  public INDArray reconstructionError(INDArray data) {
    if (!hasLossFunction()) {
      throw new IllegalStateException(
          "Cannot use reconstructionError method unless the variational autoencoder is configured"
              + " with a standard loss function (via LossFunctionWrapper). For VAEs utilizing a"
              + " reconstruction distribution, use the reconstructionProbability or"
              + " reconstructionLogProbability methods "
              + layerId());
    }

    INDArray pZXMean = activate(data, false, LayerWorkspaceMgr.noWorkspaces());
    INDArray reconstruction =
        generateAtMeanGivenZ(pZXMean); // Not probabilistic -> "mean" == output

    if (reconstructionDistribution instanceof CompositeReconstructionDistribution) {
      CompositeReconstructionDistribution c =
          (CompositeReconstructionDistribution) reconstructionDistribution;
      ...
      // Re: the activation identity here - the reconstruction array already has the activation
      // function applied,
  ...
  public void assertInputSet(boolean backprop) {
    if (input == null) {
      if (backprop) {
        throw new IllegalStateException(
            "Cannot perform backprop in layer "
                + getClass().getSimpleName()
                + ": layer input field is not set");
      } else {
        throw new IllegalStateException(
            "Cannot perform forward pass in layer "
                + getClass().getSimpleName()
                + ": layer input field is not set");
      }
    }
