...
public class LastTimeStepVertex extends BaseGraphVertex {
  ...
  public INDArray doForward(boolean training, LayerWorkspaceMgr workspaceMgr) {
    ...
    // Then: work out, from the mask array, which time step of activations we want, extract
    // activations
  ...
  public Pair<Gradient, INDArray[]> doBackward(boolean tbptt, LayerWorkspaceMgr workspaceMgr) {
    ...
    INDArray epsilonsOut =
        workspaceMgr.create(ArrayType.ACTIVATION_GRAD, epsilon.dataType(), fwdPassShape, 'f');

    if (fwdPassTimeSteps == null) {
      // Last time step for all examples
      epsilonsOut.put(
          new INDArrayIndex[] {
            NDArrayIndex.all(), NDArrayIndex.all(), NDArrayIndex.point(fwdPassShape[2] - 1)
          },
          epsilon);
    } else {
      ...
      for (int i = 0; i < fwdPassTimeSteps.length; i++) {
        epsilonsOut.put(
            new INDArrayIndex[] {
              NDArrayIndex.point(i), NDArrayIndex.all(), NDArrayIndex.point(fwdPassTimeSteps[i])
            },
            epsilon.getRow(i));
      }
    }
  ...
  public Pair<INDArray, MaskState> feedForwardMaskArrays(
    ...
    // Input: 2d mask array, for masking a time series. After extracting out the last time step, we
    // no longer need the mask array
