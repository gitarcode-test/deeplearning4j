...
public class FailureTestingListener implements TrainingListener, Serializable {
  ...
  protected void call(CallType callType, Model model) {
    ...
    if (triggered) {
      log.error(
          "*** FailureTestingListener was triggered on iteration {}, epoch {} - Failure mode is set"
              + " to {} ***",
          iter,
          epoch,
          failureMode);
      switch (failureMode) {
        ...
        case ILLEGAL_STATE:
          ...
          throw new IllegalStateException(
              "FailureTestListener was triggered with failure mode "
                  + failureMode
                  + " - iteration "
                  + iter
                  + ", epoch "
  ...
  @Data
  public abstract static class FailureTrigger implements Serializable {
     ...
     * @return
     */
    public abstract boolean triggerFailure(
        CallType callType, int iteration, int epoch, Model model);
  ...
  public static class And extends FailureTrigger {
    ...
    public boolean triggerFailure(CallType callType, int iteration, int epoch, Model model) {
      boolean b = true;
      for (FailureTrigger ft : triggers) b &= ft.triggerFailure(callType, iteration, epoch, model);
      return b;
    }
    ...
    public void initialize() {
      super.initialize();
      for (FailureTrigger ft : triggers) ft.initialize();
    }
  }
  ...
  public static class RandomProb extends FailureTrigger {
    ...
    @Override
    public boolean triggerFailure(CallType callType, int iteration, int epoch, Model model) {
      return (this.callType == CallType.ANY || callType == this.callType)
