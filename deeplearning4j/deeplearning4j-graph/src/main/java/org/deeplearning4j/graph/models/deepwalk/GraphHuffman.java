...
public class GraphHuffman implements BinaryTree {
  ...
  /**
   * @param nVertices nVertices number of vertices in the graph that this Huffman tree is being
   *     built for
  ...
  public void buildTree(int[] vertexDegree) {
    ...
    // Now: convert tree into binary codes. Traverse tree (preorder traversal) -> record path
    // (left/right) -> code
  ...
  private int traverse(
      Node node,
      long codeSoFar,
      byte codeLengthSoFar,
      int innerNodeCount,
      int[] innerNodePath,
      int currDepth) {
    if (codeLengthSoFar >= MAX_CODE_LENGTH)
      throw new RuntimeException(
          "Cannot generate code: code length exceeds " + MAX_CODE_LENGTH + " bits");
