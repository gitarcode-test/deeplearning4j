...
public class InMemoryGraphLookupTable implements GraphVectorLookupTable {
  ...
  protected INDArray
      outWeights; // 'output' vectors. Specifically vectors for inner nodes in binary tree
  protected double learningRate;

  protected double[] expTable;
  protected static double MAX_EXP = 6;

  public InMemoryGraphLookupTable(
      int nVertices, int vectorSize, BinaryTree tree, double learningRate) {
  ...
  public void resetWeights() {
    ...
    this.outWeights =
        Nd4j.rand(nVertices - 1, vectorSize)
            .subi(0.5)
            .divi(vectorSize); // Full binary tree with L leaves has L-1 inner nodes
  }
   ...
   * out[0][i] (i>0) is the inner node vector along path to second vertex; out[1][i] is gradient for
   * inner node vertex<br>
  ...
  /**
   * Calculate the probability of the second vertex given the first vertex i.e., P(v_second |
   * v_first)
   *
  ...
  public double calculateProb(int first, int second) {
    ...
    for (int i = 0; i < codeLength; i++) {
      ...
      double innerProb =
          (path ? sigmoid(dot) : sigmoid(-dot)); // prob of going left or right at inner node
      prob *= innerProb;
    }
    return prob;
  }
