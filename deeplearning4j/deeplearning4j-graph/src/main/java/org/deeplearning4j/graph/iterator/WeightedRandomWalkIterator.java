...
public class WeightedRandomWalkIterator<V> implements GraphWalkIterator<V> {
  ...
  /**
   * Construct a RandomWalkIterator for a given graph, with a specified walk length and random
   * number generator seed.<br>
   * Uses {@code NoEdgeHandling.EXCEPTION_ON_DISCONNECTED} - hence exception will be thrown when
   * generating random walks on graphs with vertices containing having no edges, or no outgoing
   * edges (for directed graphs)
   *
   * @see #WeightedRandomWalkIterator(IGraph, int, long, NoEdgeHandling)
   */
  public WeightedRandomWalkIterator(
      IGraph<V, ? extends Number> graph, int walkLength, long rngSeed) {
   ...
   * Order of starting vertices is randomized within this subset
   *
   * @param graph IGraph to conduct walks on
   * @param walkLength length of each walk. Walk of length 0 includes 1 vertex, walk of 1 includes 2
   *     vertices etc
          ...
          case EXCEPTION_ON_DISCONNECTED:
            throw new NoEdgesException(
                "Cannot conduct random walk: vertex "
                    + currVertexIdx
                    + " has no outgoing edges.  Set NoEdgeHandling mode to"
                    + " NoEdgeHandlingMode.SELF_LOOP_ON_DISCONNECTED to self loop instead of"
