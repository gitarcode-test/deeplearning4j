...
public class BertWordPieceTokenizer implements Tokenizer {
        ...
        if (count++ > basicToken.length()) {
          ...
          throw new IllegalStateException(
              "Invalid token encountered: \""
                  + basicToken
                  + "\" likely contains characters that are not present in the vocabulary. Invalid"
                  + " tokens may be cleaned in a preprocessing step using a TokenPreProcessor."
                  + " preTokenizePreProcessor="
                  + preTokenizePreProcessor
                  + ", tokenPreProcess="
                  + tokenPreProcess);
        }
  ...
  protected void checkIfEmpty(Map<String, Integer> m, String candidate) {
    if (m.isEmpty()) {
      throw new IllegalStateException(
          "Invalid token/character encountered: \""
              + candidate
              + "\" likely contains characters that are not present in the vocabulary. Invalid"
              + " tokens may be cleaned in a preprocessing step using a TokenPreProcessor."
              + " preTokenizePreProcessor="
