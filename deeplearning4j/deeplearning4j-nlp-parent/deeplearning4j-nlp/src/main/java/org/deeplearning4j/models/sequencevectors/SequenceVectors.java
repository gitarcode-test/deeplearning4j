...
public class SequenceVectors<T extends SequenceElement> extends WordVectorsImpl<T>
  ...
  @Getter @Setter protected VectorsConfiguration configuration = new VectorsConfiguration();
  ...
  public void buildVocab() {
    ...
    val constructor =
        new VocabConstructor.Builder<T>()
    ...
    if (existingModel != null
        && lookupTable instanceof InMemoryLookupTable
      ...
      */
      ...
      // check for malformed inputs. if numWords/numSentences ratio is huge, then user is passing
      // something weird
      if (vocab.numWords() / constructor.getNumberOfSequences() > 1000) {
        ...
        log.warn(
            "!                                                                                     "
                + "  !");
        log.warn(
            "! Your input looks malformed: number of sentences is too low, model accuracy may"
                + " suffer !");
        log.warn(
            "!                                                                                     "
                + "  !");
  ...
  protected synchronized void initLearners() {
    if (!configured) {
      ...
      if (trainElementsVectors && elementsLearningAlgorithm != null && !trainSequenceVectors) {
        log.info(
            "          building ElementsLearningAlgorithm: ["
                + elementsLearningAlgorithm.getCodeName()
                + "]");
      ...
      if (trainSequenceVectors && sequenceLearningAlgorithm != null) {
        log.info(
            "          building SequenceLearningAlgorithm: ["
                + sequenceLearningAlgorithm.getCodeName()
                + "]");
  ...
  private void initIntersectVectors() {
    if (intersectModel != null && intersectModel.vocab().numWords() > 0) {
      ...
      if (indexes.size() > 0) {
        ...
        Nd4j.scatterUpdate(
            org.nd4j.linalg.api.ops.impl.scatter.ScatterUpdate.UpdateOp.ASSIGN,
  ...
  public void fit() {
    ...
    if (!trainElementsVectors && !trainSequenceVectors)
      throw new IllegalStateException(
          "You should define at least one training goal 'trainElementsRepresentation' or"
              + " 'trainSequenceRepresentation'");
    if (iterator == null)
    ...
    WordVectorSerializer.printOutProjectedMemoryUse(
        vocab.numWords(),
        configuration.getLayersSize(),
        configuration.isUseHierarchicSoftmax() && configuration.getNegative() > 0 ? 3 : 2);

    if (vocab == null || lookupTable == null || vocab.numWords() == 0)
      throw new IllegalStateException(
          "You can't fit() model with empty Vocabulary or WeightLookupTable");

    // if model vocab and lookupTable is built externally we basically should check that lookupTable
        ...
        while (iterator.hasMoreSequences()) {
          ...
          for (T element : sequence.getElements()) {
            ...
            if (realElement != null && !realElement.isInit()) {
              ...
              val randArray =
                  Nd4j.rand(new int[] {1, configuration.getLayersSize()}, rng)
                      .subi(0.5)
    ...
    for (int currentEpoch = 1; currentEpoch <= numEpochs; currentEpoch++) {
      ...
      log.info(
          "Epoch ["
              + currentEpoch
              + "] finished; Elements processed so far: ["
              + wordsCounter.get()
              + "];  Sequences processed: ["
              + linesCounter.get()
              + "]");
  ...
  protected void trainSequence(@NonNull Sequence<T> sequence, AtomicLong nextRandom, double alpha) {
    ...
    */
    ...
    if (trainElementsVectors
        && !(trainSequenceVectors && sequenceLearningAlgorithm instanceof DM)) {
  ...
  public String toString() {
    return "SequenceVectors{"
        ...
        + useUnknown
        + ", variableWindows="
        + Arrays.toString(variableWindows)
        + ", stopWords="
        + stopWords
        + '}';
  }
  ...
  public static class Builder<T extends SequenceElement> {
    ...
    /**
     * This method allows you to use pre-built WordVectors model (e.g. SkipGram) for DBOW sequence
     * learning. Existing model will be transferred into new model before training starts.
     *
     * <p>PLEASE NOTE: This model has no effect for elements learning algorithms. Only sequence
     * learning is affected. PLEASE NOTE: Non-normalized model is recommended to use here.
     *
    ...
    public Builder<T> elementsLearningAlgorithm(String algoName) {
      ...
      this.configuration.setElementsLearningAlgorithm(
          elementsLearningAlgorithm.getClass().getCanonicalName());
    ...
    public Builder<T> elementsLearningAlgorithm(ElementsLearningAlgorithm<T> algorithm) {
      // allow easier to use setup of configuration by allowing null
      // values
      if (elementsLearningAlgorithm == null) return this;
    ...
    /**
     * This method defines how many iterations should be done over whole training corpus during
     * modelling
     *
     * @param numEpochs
     * @return
    ...
    /**
     * Sets number of worker threads to be used in the lower level linear algebra calculations used
     * in calculating hierarchical softmax/sampling
     *
     * @param numWorkers
     * @return
     */
    ...
    /**
     * Sets number of threads running calculations. Note this is different from workers which affect
     * the number of threads used to compute updates. This should be balanced with the number of
     * workers. High number of threads will actually hinder performance.
     *
    ...
    /**
     * This method defines number of dimensions for outcome vectors. Please note: This option has
     * effect only if lookupTable wasn't defined during building process.
     *
     * @param layerSize
     * @return
     */
    ...
    /**
     * This method defines minimal element frequency for elements found in the training corpus. All
     * elements with frequency below this threshold will be removed before training. Please note:
     * this method has effect only if vocabulary is built internally.
     *
     ...
     *
     * <p>Default value: 0. Means no limit
     *
     * @param limit
     * @return
     */
    ...
    /**
     * This method defines, should all model be reset before training. If set to true, vocabulary
     * and WeightLookupTable will be reset before training, and will be built from scratches
     *
     * @param reallyReset
     * @return
     */
    ...
    /**
     * You can provide collection of objects to be ignored, and excluded out of model Please note:
     * Object labels and hashCode will be used for filtering
     *
     * @param stopList
     * @return
     */
    ...
    /**
     * Sets seed for random numbers generator. Please note: this has effect only if vocabulary and
     * WeightLookupTable is built internally
     *
     * @param randomSeed
     * @return
     */
    ...
    /**
     * ModelUtils implementation, that will be used to access model. Methods like: similarity,
     * wordsNearest, accuracy are provided by user-defined ModelUtils
     *
    ...
    /**
     * This method allows you to specify SequenceElement that will be used as UNK element, if UNK is
     * used
     *
     * @param element
     * @return
    ...
    /**
     * This method allows to use variable window size. In this case, every batch gets processed
     * using one of predefined window sizes
     *
     * @param windows
     * @return
     */
    ...
    /**
     * If set to true, initial weights for elements/sequences will be derived from elements
     * themself. However, this implies additional cycle through input iterator.
     *
    ...
    protected void presetTables() {
      ...
      if (StringUtils.isNotEmpty(elementsLearningAlgorithm)) {
        this.elementsLearningAlgorithm =
            DL4JClassLoading.createNewInstance(elementsLearningAlgorithm);
      }
      ...
      if (StringUtils.isNotEmpty(sequenceLearningAlgorithm)) {
        this.sequenceLearningAlgorithm =
            DL4JClassLoading.createNewInstance(sequenceLearningAlgorithm);
      }
      ...
      if (trainElementsVectors && this.elementsLearningAlgorithm == null) {
        ...
        if (this.sequenceLearningAlgorithm != null) {
          this.elementsLearningAlgorithm =
              this.sequenceLearningAlgorithm.getElementsLearningAlgorithm();
        } else {
    ...
    public Builder<T> intersectModel(
        @NonNull SequenceVectors<T> intersectVectors, boolean lockFactor) {
  ...
  /**
   * This class is used to fetch data from iterator in background thread, and convert it to
   * List<VocabularyWord>
   *
   * <p>It becomes very useful if text processing pipeline behind iterator is complex, and we're not
   * loading data from simple text file with whitespaces as separator. Since this method allows you
   * to hide preprocessing latency in background.
   *
   * <p>This mechanics will be change to PrefetchingSentenceIterator wrapper.
   */
  protected class AsyncSequencer extends Thread implements Runnable {
    ...
    public void run() {
      ...
      while (this.iterator.hasMoreSequences()) {
        ...
        // if buffered level is below limitLower, we're going to fetch limitUpper number of strings
        // from fetcher
          ...
          while (linesLoaded.getAndIncrement() < limitUpper && this.iterator.hasMoreSequences()) {
            ...
            */
            ...
            for (T element : document.getElements()) {
              ...
              // please note: this sequence element CAN be absent in vocab, due to minFreq or
              // stopWord or whatever else
              if (realElement != null) {
            ...
            // due to subsampling and null words, new sequence size CAN be 0, so there's no need to
            // insert empty sequence into processing chain
  ...
  /**
   * VectorCalculationsThreads are used for vector calculations, and work together with
   * AsyncIteratorDigitizer. Basically, all they do is just transfer of digitized sentences into
   * math layer.
   *
   * <p>Please note, they do not iterate the sentences over and over, each sentence processed only
   * once. Training corpus iteration is implemented in fit() method.
   */
          ...
          for (int i = 0; i < numIterations; i++) {
            ...
            for (int x = 0; x < sequences.size(); x++) {
              try (val ws = Nd4j.getWorkspaceManager().scopeOutOfWorkspaces()) {
                ...
                log.debug(
                    "LR before: {}; wordsCounter: {}; totalWordsCount: {}",
                    learningRate.get(),
                    this.wordsCounter.get(),
                    ...
                    Math.max(
                        ...
                        learningRate.get()
                            * (1
                                - (1.0
                                    * this.wordsCounter.get()
                                    / ((double) this.totalWordsCount)
                                    / (numIterations * totalEpochs))));
                ...
                if (totalLines.get() % 100000 == 0) {
                  ...
                  double wordsSecTotal =
                      this.wordsCounter.get() / ((double) totalTimeSpent / 1000.0);

                  log.info(
                      "Epoch: [{}]; Words vectorized so far: [{}];  Lines vectorized so far: [{}];"
                          + " Seq/sec: [{}]; Words/sec: [{}]; learningRate: [{}]",
                      this.epochNumber,
                      this.wordsCounter.get(),
                      this.totalLines.get(),
                      String.format("%.2f", seqSec),
                      String.format("%.2f", wordsSecTotal),
                      alpha);
                }
                if (eventListeners != null && !eventListeners.isEmpty()) {
                  for (VectorsListener listener : eventListeners) {
                    if (listener.validateEvent(ListenerEvent.LINE, totalLines.get()))
                      listener.processEvent(
                          ListenerEvent.LINE, SequenceVectors.this, totalLines.get());
                  }
                }
              }
            }
