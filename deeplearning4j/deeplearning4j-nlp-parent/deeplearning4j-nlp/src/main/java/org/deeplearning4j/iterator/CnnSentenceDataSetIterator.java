...
public class CnnSentenceDataSetIterator implements DataSetIterator {
   ...
   * CNN2D: For use with 2d convolution layers: Shape [minibatch, 1, vectorSize, sentenceLength] or
   * [minibatch, 1, sentenceLength, vectorSize], depending on the setting for 'sentencesAlongHeight'
   * configuration.
   */
  public enum Format {
    RNN,
  ...
  public INDArray loadSingleSentence(String sentence) {
    ...
    if (tokens.isEmpty())
      throw new IllegalStateException(
          "No tokens available for input sentence - empty string or no words in vocabulary with"
              + " RemoveWord unknown handling? Sentence = \""
              + sentence
              + "\"");
    if (format == Format.CNN1D || format == Format.RNN) {
      int[] featuresShape =
          new int[] {1, wordVectorSize, Math.min(maxSentenceLength, tokens.size())};
            ...
            if (sentenceLength >= maxLength) {
              ...
              featuresMask
                  .get(
                      ...
                      NDArrayIndex.interval(0, sentenceLength),
                      NDArrayIndex.point(0))
                  .assign(1.0);
            }
          }
  ...
  public static class Builder {
    ...
    /**
     * @deprecated Due to old default, that will be changed in the future. Use {@link
     *     #Builder(Format)} to specify the {@link Format} of the activations
     */
    @Deprecated
    public Builder() {
    ...
    /** Specify how the (labelled) sentences / documents should be provided */
    public Builder sentenceProvider(
