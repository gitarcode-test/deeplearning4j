    ...
    for (int i = 0; i < walkLength; i++) {
      ...
      switch (walkDirection) {
        ...
        case FORWARD_ONLY:
          {
            ...
            int[] nextHops =
                ArrayUtils.removeElements(
                    sourceGraph.getConnectedVertexIndices(currentPosition), lastId);
        ...
        case FORWARD_UNIQUE:
          {
            ...
            int[] nextHops =
                ArrayUtils.removeElements(
                    sourceGraph.getConnectedVertexIndices(currentPosition), visitedHops);
            if (nextHops.length > 0) {
              startPosition = nextHops[rng.nextInt(nextHops.length)];
            } else {
        ...
        case FORWARD_PREFERRED:
          {
            // here we remove all previously visited hops, and if there's no next unique hop
            // available - we fallback to anything, but the last one
            int[] nextHops =
                ArrayUtils.removeElements(
                    sourceGraph.getConnectedVertexIndices(currentPosition), visitedHops);
            if (nextHops.length == 0) {
                  ...
                  case EXCEPTION_ON_DISCONNECTED:
                    {
                      throw new NoEdgesException(
                          "No more edges at vertex [" + currentPosition + "]");
                    }
                  case SELF_LOOP_ON_DISCONNECTED:
                    {
                      startPosition = currentPosition;
