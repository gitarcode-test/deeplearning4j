...
public class InMemoryLookupCache implements VocabCache<VocabWord>, Serializable {
   ...
   * @return 0 if hasn't occurred or the number of times the word occurs
   */
  @Override
  ...
  public synchronized void addWordToIndex(int index, String word) {
    ...
    */
    ...
    if (!wordFrequencies.containsElement(word)) wordFrequencies.incrementCount(word, 1);
  ...
  public synchronized boolean addToken(VocabWord word) {
    if (null == tokens.put(word.getLabel(), word)) return true;
    return false;
  }
