...
public class PrefetchingSentenceIterator implements SentenceIterator {
  ...
  private class AsyncIteratorReader extends Thread implements Runnable {
    ...
    public AsyncIteratorReader(
        @NonNull SentenceIterator iterator, int fetchSize, SentencePreProcessor preProcessor) {
      this.iterator = iterator;
      this.fetchSize = fetchSize;
      this.preProcessor = preProcessor;
    ...
    public void run() {
      while (!shouldTerminate.get()) {
        ...
        else ThreadUtils.uncheckedSleep(50);
          ...
          if (buffer.size() < fetchSize) {
            while (!shouldTerminate.get() && cnt < fetchSize && iterator.hasNext()) {
              try {
                ...
                if (line != null)
                  buffer.add(
                      (this.preProcessor == null) ? line : this.preProcessor.preProcess(line));
              } finally {
                lock.writeLock().unlock();
              }
              cnt++;
            }
            //                            log.info("Lines added: [" + cnt + "], buffer size: [" +
            // buffer.size() + "]");
          } else ThreadUtils.uncheckedSleep(10);
        }
        isRunning.set(false);
      }
    }
