...
/**
 * The ShardedLabelAwareIterator class is an implementation of the LabelAwareIterator interface. It
 * wraps a given LabelAwareIterator (subIterator) and splits its documents into smaller documents
 * (shards) to help manage memory usage. This is particularly useful when dealing with large text
 * documents. The documentSizeLimit field determines the maximum number of tokens per shard. The
 * tokenizerFactory field is used to tokenize the documents before sharding them. The docBatches
 * field stores the shards, and the currentBatch field keeps track of the current shard index.
 *
 * @author Adam Gibson
 */
public class ShardedLabelAwareIterator implements LabelAwareIterator {
  ...
  // Constructor
  public ShardedLabelAwareIterator(
      LabelAwareIterator subIterator, TokenizerFactory tokenizerFactory, int documentSizeLimit) {
  ...
  public LabelledDocument nextDocument() {
    while (docBatches == null
        || currentBatch >= docBatches.size()
        || (docBatches != null && docBatches.isEmpty())) {
