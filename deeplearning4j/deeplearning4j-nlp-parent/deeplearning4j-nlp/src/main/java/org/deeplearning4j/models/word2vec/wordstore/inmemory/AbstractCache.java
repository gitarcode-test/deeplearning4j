...
public class AbstractCache<T extends SequenceElement> implements VocabCache<T> {
  ...
  // we're using <String>for compatibility & failproof reasons: it's easier to store unique labels
  // then abstract objects of unknown size
  ...
  public int wordFrequency(@NonNull String word) {
    ...
    T element = extendedVocabulary.get(word);
    if (element != null) return (int) element.getElementFrequency();
    return 0;
  }
   ...
   * @return >=0 if label exists, -1 if Huffman tree wasn't built yet, -2 if specified label wasn't
   *     found
   */
  @Override
  ...
  /**
   * This method allows to insert specified label to specified Huffman tree position. CAUTION: Never
   * use this, unless you 100% sure what are you doing.
   *
   * @param index
   * @param label
   */
  ...
  public void addWordToIndex(int index, long elementId) {
    if (index >= 0) idxMap.put(index, tokenFor(elementId));
  }
   ...
   *
   * <p>Please note: this method is NOT thread-safe
   *
  ...
  public void importVocabulary(@NonNull VocabCache<T> vocabCache) {
    ...
    for (T element : vocabCache.vocabWords()) {
      if (this.addToken(element)) added.set(true);
    }
    // logger.info("Current state: {}; Adding value: {}", this.documentsCounter.get(),
    // vocabCache.totalNumberOfDocs());
    if (added.get()) this.documentsCounter.addAndGet(vocabCache.totalNumberOfDocs());
  }
  ...
  public String toJson() throws JsonProcessingException {
    ...
    Class clazz = null;
    if (iter.hasNext()) clazz = iter.next().getClass();
    else return retVal.getAsString();
  ...
  public static <T extends SequenceElement> AbstractCache<T> fromJson(String jsonString)
      throws IOException {
    ...
    CollectionType wordsCollectionType =
        mapper.getTypeFactory().constructCollectionType(List.class, VocabWord.class);
    ...
    for (int i = 0; i < jsonArray.size(); ++i) {
      VocabWord item =
          mapper.readValue(
              jsonArray.get(i).getAsJsonObject().get(VOCAB_ITEM_FIELD).getAsString(),
              VocabWord.class);
      items.add((T) item);
    }
  ...
  public boolean equals(Object o) {
    ...
    return getMinWordFrequency() == that.getMinWordFrequency()
        && isHugeModelExpected() == that.isHugeModelExpected()
        && getScavengerThreshold() == that.getScavengerThreshold()
        && getRetentionDelay() == that.getRetentionDelay()
        && Objects.equals(getVocabulary(), that.getVocabulary())
        && Objects.equals(getExtendedVocabulary(), that.getExtendedVocabulary())
        && Objects.equals(getIdxMap(), that.getIdxMap())
        && Objects.equals(getStopWords(), that.getStopWords());
  }
  ...
  public int hashCode() {
    return Objects.hash(
        getVocabulary(),
        getExtendedVocabulary(),
        getIdxMap(),
        getMinWordFrequency(),
        isHugeModelExpected(),
        getStopWords(),
        getScavengerThreshold(),
        getRetentionDelay());
  }
