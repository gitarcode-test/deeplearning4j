...
public class CBOW<T extends SequenceElement> implements ElementsLearningAlgorithm<T> {
  ...
  private Cache<IterationArraysKey, Queue<IterationArrays>> iterationArrays =
      CacheBuilder.newBuilder()
          .maximumSize(
              Integer.parseInt(System.getProperty(DL4JSystemProperties.NLP_CACHE_SIZE, "10000")))
          .build();
  ...
  public double doExec(List<BatchItem<T>> items, INDArray inferenceVector) {
    try (MemoryWorkspace workspace = Nd4j.getWorkspaceManager().scopeOutOfWorkspaces()) {
      ...
      if (items.size() > 1) {
        ...
        for (int i = 0; i < items.size(); ++i) {
          int curr = items.get(i).getWord().getCodeLength();
          if (curr > maxWinWordsCols) maxWinWordsCols = curr;
        }
        ...
        for (int cnt = 0; cnt < items.size(); cnt++) {
          ...
          if (items.get(cnt).getNumLabel() > 0) hasNumLabels = true;

          if (useHS) {
            for (int p = 0; p < currentWord.getCodeLength(); p++) {
              if (currentWord.getPoints().get(p) < 0) continue;
        ...
        CbowRound cbow =
            CbowRound.builder()
                ...
                .numWorkers(workers)
                .iterations(
                    useInference ? configuration.getIterations() * configuration.getEpochs() : 1)
                .build();
        ...
        for (int i = 0; i < windowWords.length; ++i) {
          if (i < windowStatuses.length) inputStatuses[i] = windowStatuses[i] ? 1 : 0;
          else inputStatuses[i] = -1;
        }
  ...
  public void cbow(
    ...
    BatchItem<T> batchItem =
        new BatchItem<>(currentWord, windowWords, statuses, nextRandom.get(), alpha);
    batch.add(batchItem);
