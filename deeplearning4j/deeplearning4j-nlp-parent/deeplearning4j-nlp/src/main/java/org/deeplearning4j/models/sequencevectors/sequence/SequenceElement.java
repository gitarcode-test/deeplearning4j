...
public abstract class SequenceElement implements Comparable<SequenceElement>, Serializable {
  ...
  @JsonProperty protected AtomicDouble elementFrequency = new AtomicDouble(0);
  ...
  // this var is used as state for preciseWeightInit routine, to avoid multiple initializations for
  // the same data
  ...
  */
  ...
  @Getter @Setter protected boolean isLocked;
  ...
  /**
   * This method specifies, whether this element should be treated as label for some
   * sequence/document or not.
   *
   * @param isLabel
   */
  ...
  /**
   * Equals method override should be properly implemented for any extended class, otherwise it will
   * be based on label equality
   *
   * @param object
   * @return
   */
   ...
   *
   * <p>Please note: maximum vocabulary/tree size depends on code length
   *
   * @return
   */
  ...
  public void setCodeLength(short codeLength) {
    ...
    if (codes.size() < codeLength) {
      for (int i = 0; i < codeLength; i++) codes.add((byte) 0);
    }

    if (points.size() < codeLength) {
      for (int i = 0; i < codeLength; i++) points.add(0);
    }
  }
  ...
  /**
   * hashCode method override should be properly implemented for any extended class, otherwise it
   * will be based on label hashCode
   *
  ...
  public String toString() {
    return "SequenceElement: {label: '"
        + this.getLabel()
        + "',"
        + " freq: '"
        + elementFrequency.get()
        + "',"
        + " codes: "
        + codes.toString()
        + " points: "
  ...
  public Long getStorageId() {
    if (storageId == null) storageId = SequenceElement.getLongHash(this.getLabel());
    return storageId;
  }
