...
public class StaticWord2Vec implements WordVectors {
  ...
  protected void init() {
    if (storage.size() != vocabCache.numWords())
      throw new RuntimeException(
          "Number of words in Vocab isn't matching number of stored Vectors. vocab: ["
              + vocabCache.numWords()
              + "]; storage: ["
              + storage.size()
              + "]");
  ...
  public Collection<String> wordsNearest(INDArray words, int top) {
    throw new UnsupportedOperationException(
        "Method isn't implemented. Please use usual Word2Vec implementation");
  }
  ...
  /**
   * Get the top n words most similar to the given word PLEASE NOTE: This method is not available in
   * this implementation.
   *
  ...
  /**
   * Words nearest based on positive and negative words PLEASE NOTE: This method is not available in
   * this implementation.
   *
  ...
  /**
   * Accuracy based on questions which are a space separated list of strings where the first word is
   * the query word, the next 2 words are negative, and the last word is the predicted word to be
   * nearest PLEASE NOTE: This method is not available in this implementation.
   *
  ...
  /**
   * Find all words with a similar characters in the vocab PLEASE NOTE: This method is not available
  ...
  public INDArray getWordVectorMatrix(String word) {
    ...
    int idx = 0;
    if (hasWord(word)) idx = vocabCache.indexOf(word);
    else if (getUNK() != null) idx = vocabCache.indexOf(getUNK());
    else return null;
  ...
  public INDArray getWordVectors(Collection<String> labels) {
    ...
    for (String label : labels) {
      if (hasWord(label) || getUNK() != null) words.add(getWordVectorMatrix(label));
    }
  ...
  /**
   * Specifies ModelUtils to be used to access model PLEASE NOTE: This method has no effect in this
   * implementation.
   *
   * @param utils
   */
  ...
  public void loadWeightsInto(INDArray array) {
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (arr == null) { // TODO is this even possible?
        if (zero == null) zero = Nd4j.create(array.dataType(), 1, array.size(1));
        arr = zero;
      }
      array.putRow(i, arr);
    }
  ...
  public int vectorSize() {
    ...
    if (arr != null) return (int) arr.length();
  ...
  public static class Builder {
    ...
    /**
     * This method lets you to define if decompressed values will be cached, to avoid excessive
     * decompressions. If bytes == 0 - no cache will be used.
     *
     * @param bytes
     * @return
     */
    ...
    /**
     * This method returns Static Word2Vec implementation, which is suitable for tasks like neural
     * nets feeding.
     *
     * @return
     */
