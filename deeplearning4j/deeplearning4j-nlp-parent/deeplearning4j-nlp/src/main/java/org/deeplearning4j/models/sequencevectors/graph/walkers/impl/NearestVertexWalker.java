...
public class NearestVertexWalker<V extends SequenceElement> implements GraphWalker<V> {
  ...
  protected Sequence<V> walk(Vertex<V> node, int cDepth) {
    ...
    if (walkLength == 0) {
      ...
      for (Vertex<V> vertex : vertices) sequence.addElement(vertex.getValue());
    } else {
      ...
      switch (samplingMode) {
        ...
        case MEDIAN_POPULARITY:
          {
            Collections.sort(vertices, new VertexComparator<>(sourceGraph));
            for (int i = (vertices.size() / 2) - (walkLength / 2), e = 0;
                e < walkLength && i < vertices.size();
                i++, e++) {
              sequence.addElement(vertices.get(i).getValue());
        ...
        default:
          throw new ND4JIllegalStateException(
              "Unknown sampling mode was passed in: [" + samplingMode + "]");
      }
    }
  ...
  public static class Builder<V extends SequenceElement> {
     ...
     *
     * <p>PLEASE NOTE: If set to 0 - no limits will be used.
     *
     * <p>Default value: 0
     *
     * @param length
     ...
     *
     * <p>PLEASE NOTE: This option has effect only if walkLength is limited (>0).
     *
     * @param mode
     * @return
     */
  ...
  protected class VertexComparator<V extends SequenceElement, E extends Number>
    ...
    public int compare(Vertex<V> o1, Vertex<V> o2) {
      return Integer.compare(
          graph.getConnectedVertices(o2.vertexID()).size(),
