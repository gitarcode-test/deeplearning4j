...
public class WordVectorsImpl<T extends SequenceElement> implements WordVectors {
  ...
  // number of times the word must occur in the vocab to appear in the calculations, otherwise treat
  // as unknown
  ...
  @Getter @Setter protected int numIterations = 1;
  @Getter @Setter protected int numEpochs = 1;
  @Getter @Setter protected double negative = 0;
  @Getter @Setter protected double sampling = 0;
  protected AtomicDouble learningRate = new AtomicDouble(0.025);
  @Getter @Setter protected double minLearningRate = 0.01;
  @Getter @Setter protected int window = 5;
  @Getter @Setter protected int batchSize;
  @Getter @Setter protected int learningRateDecayWords;
  @Getter @Setter protected boolean resetModel;
  protected boolean useAdeGrad;
  @Getter @Setter protected int workers = 1;

  @Getter @Setter protected int vectorCalcThreads = 1;

  @Getter @Setter protected boolean trainSequenceVectors = false;
  @Getter @Setter protected boolean trainElementsVectors = true;
  @Getter @Setter protected long seed;
  @Getter @Setter protected boolean useUnknown = false;
  @Getter @Setter protected int[] variableWindows;
  ...
  public INDArray getWordVectors(@NonNull Collection<String> labels) {
    ...
    for (String label : labels) {
      ...
      } else indexes[cnt] = useIndexUnknown ? vocab.indexOf(getUNK()) : -1;
      cnt++;
    }
