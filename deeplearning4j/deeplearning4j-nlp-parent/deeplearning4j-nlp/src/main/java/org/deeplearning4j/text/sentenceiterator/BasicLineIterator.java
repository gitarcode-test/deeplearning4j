...
public class BasicLineIterator implements SentenceIterator, Iterable<String> {
  ...
  public BasicLineIterator(@NonNull InputStream stream) {
    ...
    reader =
        new BufferedReader(
            new InputStreamReader(new BufferedInputStream(backendStream, 10 * 1024 * 1024)));
  }
  ...
  public synchronized String nextSentence() {
    try {
      return (preProcessor != null)
          ? this.preProcessor.preProcess(reader.readLine())
          : reader.readLine();
    } catch (Exception e) {
  ...
  public synchronized void reset() {
    try {
      ...
      reader =
          new BufferedReader(
              new InputStreamReader(new BufferedInputStream(backendStream, 10 * 10 * 1024)));
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  }
  ...
  /**
   * Implentation for Iterable interface. Please note: each call for iterator() resets underlying
   * SentenceIterator to the beginning;
   *
   * @return
   */
  @Override
