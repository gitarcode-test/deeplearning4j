...
public class FastText implements WordVectors, Serializable {
  ...
  private static final String METHOD_NOT_AVAILABLE =
      "This method is available for text (.vec) models only - binary (.bin) model currently loaded";
  // Mandatory
  ...
  @Getter @Builder.Default private double learningRate = -1.0;
  @Getter private double learningRateUpdate = -1.0;
  @Getter @Builder.Default private int dim = -1;
  @Getter @Builder.Default private int contextWindowSize = -1;
  @Getter @Builder.Default private int epochs = -1;
  @Getter private String modelName;
  @Getter private String lossName;
  @Getter @Builder.Default private int negativeSamples = -1;
  @Getter @Builder.Default private int numThreads = -1;
  @Getter private boolean saveOutput = false;

  // Optional for quantization
  @Getter @Builder.Default private int cutOff = -1;
  @Getter private boolean retrain;
  @Getter private boolean qnorm;
  @Getter private boolean qout;
  @Getter @Builder.Default private int dsub = -1;
  ...
  public void loadPretrainedVectors(File vectorsFile) {
    ...
    log.info(
        "Loaded vectorized representation from file %s. Functionality will be restricted.",
  ...
  public VocabCache vocab() {
    if (modelVectorsLoaded) {
      ...
      if (!modelLoaded) throw new IllegalStateException("Load model before calling vocab()");
  ...
  public boolean hasWord(String word) {
    ...
    if (words.isEmpty()) words = fastTextImpl.getWords();
