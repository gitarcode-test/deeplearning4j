...
public class CollectionLabeledPairSentenceProvider implements LabeledPairSentenceProvider {
  ...
  /**
   * Lists containing sentences to iterate over with a third for labels Sentences in the same
   * position in the first two lists are considered a pair
   *
   * @param sentenceL
   * @param sentenceR
   * @param labelsForSentences
   */
  public CollectionLabeledPairSentenceProvider(
  ...
  public CollectionLabeledPairSentenceProvider(
    ...
    if (sentenceR.size() != sentenceL.size()) {
      throw new IllegalArgumentException(
          "Sentence lists must be same size (first list size: "
              + sentenceL.size()
              + ", second list size: "
              + sentenceR.size()
              + ")");
    ...
    if (sentenceR.size() != labelsForSentences.size()) {
      throw new IllegalArgumentException(
          "Sentence pairs and labels must be same size (sentence pair size: "
              + sentenceR.size()
              + ", labels size: "
