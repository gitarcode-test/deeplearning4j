...
public class BertIterator implements MultiDataSetIterator {
  ...
  @Getter @Setter protected MultiDataSetPreProcessor preProcessor;
  ...
  protected Map<String, Integer>
      vocabMap; // TODO maybe use Eclipse ObjectIntHashMap or similar for fewer objects?
  ...
  public MultiDataSet next(int num) {
    ...
    if (sentenceProvider != null) {
      ...
      throw new UnsupportedOperationException(
          "Labelled sentence provider is null and no other iterator types have yet been"
              + " implemented");
    }

    Pair<INDArray[], INDArray[]> featuresAndMaskArraysPair =
        convertMiniBatchFeatures(tokensAndLabelList, outLength, segIdOnesFrom);
    INDArray[] featureArray = featuresAndMaskArraysPair.getFirst();
    INDArray[] featureMaskArray = featuresAndMaskArraysPair.getSecond();

    Pair<INDArray[], INDArray[]> labelsAndMaskArraysPair =
        convertMiniBatchLabels(tokensAndLabelList, featureArray, outLength);
    ...
    org.nd4j.linalg.dataset.MultiDataSet mds =
        new org.nd4j.linalg.dataset.MultiDataSet(
            featureArray, labelArray, featureMaskArray, labelMaskArray);
  ...
  /**
   * For use during inference. Will convert a given list of sentences to features and feature masks
   * as appropriate.
   *
  ...
  public Pair<INDArray[], INDArray[]> featurizeSentences(List<String> listOnlySentences) {
    ...
    List<Pair<List<String>, String>> tokensAndLabelList =
        sentenceListProcessed.getTokensAndLabelList();
    int outLength = sentenceListProcessed.getMaxL();

    if (preProcessor != null) {
      Pair<INDArray[], INDArray[]> featureFeatureMasks =
          convertMiniBatchFeatures(tokensAndLabelList, outLength, null);
      MultiDataSet dummyMDS =
          new org.nd4j.linalg.dataset.MultiDataSet(
              featureFeatureMasks.getFirst(), null, featureFeatureMasks.getSecond(), null);
  ...
  /**
   * For use during inference. Will convert a given pair of a list of sentences to features and
   * feature masks as appropriate.
   *
   * @param listOnlySentencePairs
   * @return Pair of INDArrays[], first element is feature arrays and the second is the masks array
   */
  public Pair<INDArray[], INDArray[]> featurizeSentencePairs(
      List<Pair<String, String>> listOnlySentencePairs) {
    Preconditions.checkState(
        sentencePairProvider != null,
        "The featurizeSentencePairs method is meant for inference with sentence pairs. Use only"
            + " when the sentence pair provider is set (i.e not null).");

    List<Triple<String, String, String>> sentencePairsWithNullLabel =
        addDummyLabelForPairs(listOnlySentencePairs);
    SentencePairListProcessed sentencePairListProcessed =
        tokenizePairsMiniBatch(sentencePairsWithNullLabel);
    List<Pair<List<String>, String>> tokensAndLabelList =
        sentencePairListProcessed.getTokensAndLabelList();
    ...
    if (preProcessor != null) {
      ...
      MultiDataSet dummyMDS =
          new org.nd4j.linalg.dataset.MultiDataSet(
              featuresAndMaskArraysPair.getFirst(),
              null,
              featuresAndMaskArraysPair.getSecond(),
              null);
      preProcessor.preProcess(dummyMDS);
      return new Pair<>(dummyMDS.getFeatures(), dummyMDS.getFeaturesMaskArrays());
    }
  ...
  private Pair<INDArray[], INDArray[]> convertMiniBatchFeatures(
      List<Pair<List<String>, String>> tokensAndLabelList, int outLength, long[] segIdOnesFrom) {
    ...
    for (int i = 0; i < tokensAndLabelList.size(); i++) {
      ...
      for (int j = 0; j < outLength && j < t.size(); j++) {
        Preconditions.checkState(
            vocabMap.containsKey(t.get(j)),
            "Unknown token encountered: token \"%s\" is not in vocabulary",
            t.get(j));
        int idx = vocabMap.get(t.get(j));
        outIdxs[i][j] = idx;
        outMask[i][j] = 1;
        if (segIdOnesFrom != null && j >= segIdOnesFrom[i]) outSegmentId[i][j] = 1;
      }
    }
  ...
  private SentencePairListProcessed tokenizePairsMiniBatch(
      List<Triple<String, String, String>> listPairs) {
    SentencePairListProcessed sentencePairListProcessed =
        new SentencePairListProcessed(listPairs.size());
    for (Triple<String, String, String> t : listPairs) {
      ...
      if (prependToken != null) maxLength--;
      if (appendToken != null) maxLength -= 2;
      if (tokensL.size() + tokensR.size() > maxLength) {
        ...
        if (shortSize > maxLength / 2) {
          ...
          tokensL
              .subList(maxLength / 2, tokensL.size())
              .clear(); // if maxsize/2 is odd pop extra on L side to match implementation in TF
      ...
      if (prependToken != null) tokens.add(prependToken);
      tokens.addAll(tokensL);
      if (appendToken != null) tokens.add(appendToken);
  ...
  private Pair<INDArray[], INDArray[]> convertMiniBatchLabels(
      List<Pair<List<String>, String>> tokenizedSentences, INDArray[] featureArray, int outLength) {
    ...
    if (task == Task.SEQ_CLASSIFICATION) {
      ...
      if (sentenceProvider != null) {
        ...
        for (int i = 0; i < mbSize; i++) {
          ...
          Preconditions.checkState(
              classLabels[i] >= 0,
              "Provided label \"%s\" for sentence does not exist in set of classes/categories",
              lbl);
        }
      } else if (sentencePairProvider != null) {
      ...
      if (unsupervisedLabelFormat == UnsupervisedLabelFormat.RANK2_IDX) {
        ...
        throw new IllegalStateException(
            "Unknown unsupervised label format: " + unsupervisedLabelFormat);
      }
  ...
  private List<String> tokenizeSentence(String sentence, boolean ignorePrependAppend) {
    ...
    if (prependToken != null && !ignorePrependAppend) tokens.add(prependToken);

    while (t.hasMoreTokens()) {
      String token = t.nextToken();
      tokens.add(token);
    }
    if (appendToken != null && !ignorePrependAppend) tokens.add(appendToken);
    return tokens;
  }
  ...
  private List<Triple<String, String, String>> addDummyLabelForPairs(
      List<Pair<String, String>> listOnlySentencePairs) {
  ...
  public static class Builder {
    ...
    protected Map<String, Integer>
        vocabMap; // TODO maybe use Eclipse ObjectIntHashMap for fewer objects?
    ...
    /**
     * Specify the {@link Task} the iterator should be set up for. See {@link BertIterator} for more
     * details.
     */
    ...
    /**
     * Specify the TokenizerFactory to use. For BERT, typically {@link
     * org.deeplearning4j.text.tokenization.tokenizerfactory.BertWordPieceTokenizerFactory} is used
     */
    ...
    /**
     * Specifies how the sequence length of the output data should be handled. See {@link
    ...
    /**
     * Minibatch size to use (number of examples to train on for each iteration) See also: {@link
     * #padMinibatches}
     *
     ...
     * If the dataset is not an exact multiple of the minibatch size, should we pad the smaller
     * final minibatch?<br>
     * For example, if we have 100 examples total, and 32 minibatch size, the following number of
     * examples will be returned for subsequent calls of next() in the one epoch:<br>
     * padMinibatches = false (default): 32, 32, 32, 4.<br>
     * padMinibatches = true: 32, 32, 32, 32 (note: the last minibatch will have 4 real examples,
     * and 28 masked out padding examples).<br>
     * Both options should result in exactly the same model. However, some BERT implementations may
     * require exactly an exact number of examples in all minibatches to function.
     */
    ...
    /**
     * Set the preprocessor to be used on the MultiDataSets before returning them. Default: none
     * (null)
     */
    ...
    /**
     * Provide the vocabulary as a map. Keys are the words in the vocabulary, and values are the
     * indices of those words. For indices, they should be in range 0 to vocabMap.size()-1
     * inclusive.<br>
     * If using {@link BertWordPieceTokenizerFactory}, this can be obtained using {@link
     * BertWordPieceTokenizerFactory#getVocab()}
     */
    ...
    /**
     * Used only for unsupervised training (i.e., when task is set to {@link Task#UNSUPERVISED} for
     * learning a masked language model. This can be used to customize how the masking is performed.
     * <br>
    ...
    /**
     * Used only for unsupervised training (i.e., when task is set to {@link Task#UNSUPERVISED} for
     * learning a masked language model. Used to specify the format that the labels should be
     * returned in. See {@link BertIterator} for more details.
     */
    ...
    /**
     * Used only for unsupervised training (i.e., when task is set to {@link Task#UNSUPERVISED} for
     * learning a masked language model. This specifies the token (such as "[MASK]") that should be
     * used when a value is masked out. Note that this is passed to the {@link BertSequenceMasker}
     * defined by {@link #masker(BertSequenceMasker)} hence the exact behaviour will depend on what
     * masker is used.<br>
     ...
     * Some BERT/Transformer models may need this - for example sequences starting with a "[CLS]"
     * token.<br>
     ...
     *
     * @param appendToken Token at end of each sentence for pairs of sentences (null: no token will
     *     be appended)
     * @return
     */
    ...
    public BertIterator build() {
      Preconditions.checkState(
          task != null,
          "No task has been set. Use .task(BertIterator.Task.X) to set the task to be performed");
      Preconditions.checkState(
          tokenizerFactory != null,
          "No tokenizer factory has been set. A tokenizer factory (such as"
              + " BertWordPieceTokenizerFactory) is required");
      Preconditions.checkState(
          vocabMap != null,
          "Cannot create iterator: No vocabMap has been set. Use"
              + " Builder.vocabMap(Map<String,Integer>) to set");
      Preconditions.checkState(
          task != Task.UNSUPERVISED || masker != null,
          "If task is UNSUPERVISED training, a masker must be set via masker(BertSequenceMasker)"
              + " method");
      Preconditions.checkState(
          task != Task.UNSUPERVISED || unsupervisedLabelFormat != null,
          "If task is UNSUPERVISED training, a label format must be set via"
              + " masker(BertSequenceMasker) method");
      Preconditions.checkState(
          task != Task.UNSUPERVISED || maskToken != null,
          "If task is UNSUPERVISED training, the mask token in the vocab (such as \"[MASK]\" must"
              + " be specified");
      if (sentencePairProvider != null) {
        Preconditions.checkState(
            task == Task.SEQ_CLASSIFICATION,
            "Currently only supervised sequence classification is set up with sentence pairs."
                + " \".task(BertIterator.Task.SEQ_CLASSIFICATION)\" is required with a sentence"
                + " pair provider");
        Preconditions.checkState(
            featureArrays == FeatureArrays.INDICES_MASK_SEGMENTID,
            "Currently only supervised sequence classification is set up with sentence pairs."
                + " \".featureArrays(FeatureArrays.INDICES_MASK_SEGMENTID)\" is required with a"
                + " sentence pair provider");
        Preconditions.checkState(
            lengthHandling == LengthHandling.FIXED_LENGTH,
            "Currently only fixed length is supported for sentence pairs."
                + " \".lengthHandling(BertIterator.LengthHandling.FIXED_LENGTH, maxLength)\" is"
                + " required with a sentence pair provider");
        Preconditions.checkState(
            sentencePairProvider != null,
            "Provide either a sentence provider or a sentence pair provider. Both cannot be non"
                + " null");
      }
      if (appendToken != null) {
        Preconditions.checkState(
            sentencePairProvider != null,
            "Tokens are only appended with sentence pairs. Sentence pair provider is not set. Set"
                + " sentence pair provider.");
      }
  ...
  private static class SentencePairListProcessed {
    ...
    @Getter private long[] segIdOnesFrom;
    ...
    private void addProcessedToList(
        long segIdIdx, Pair<List<String>, String> tokenizedSentencePairAndLabel) {
  ...
  private static class SentenceListProcessed {
    ...
    @Getter @Setter private int maxL;

    @Getter private List<Pair<List<String>, String>> tokensAndLabelList;
