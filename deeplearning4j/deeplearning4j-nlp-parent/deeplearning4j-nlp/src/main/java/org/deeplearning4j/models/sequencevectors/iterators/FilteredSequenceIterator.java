...
public class FilteredSequenceIterator<T extends SequenceElement> implements SequenceIterator<T> {
  ...
  /**
   * Creates Filtered SequenceIterator on top of another SequenceIterator and appropriate VocabCache
   * instance
   *
   * @param iterator
   * @param vocabCache
   */
  public FilteredSequenceIterator(
      @NonNull SequenceIterator<T> iterator, @NonNull VocabCache<T> vocabCache) {
  ...
  /**
   * Returns filtered sequence, that contains sequence elements from vocabulary only. Please note:
   * it can return empty sequence, if no elements were found in vocabulary
   *
   * @return
   */
  @Override
