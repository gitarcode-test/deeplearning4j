...
public class ParallelTransformerIterator extends BasicTransformerIterator {
  ...
  public ParallelTransformerIterator(
      @NonNull LabelAwareIterator iterator, @NonNull SentenceTransformer transformer) {
  ...
  public ParallelTransformerIterator(
    ...
    // threads = new TokenizerThread[allowMultithreading ?
    // Math.max(Runtime.getRuntime().availableProcessors(), 2) : 1];
    executorService =
        Executors.newFixedThreadPool(
            allowMultithreading ? Math.max(Runtime.getRuntime().availableProcessors(), 2) : 1);
