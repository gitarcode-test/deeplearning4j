...
@Slf4j
public class BatchedInferenceObservable extends BasicInferenceObservable
  ...
  public void addInput(INDArray[] input, INDArray[] inputMasks) {
    synchronized (locker) {
      ...
      if (isReadLocked.get()) realLocker.readLock().unlock();
    }
  }
  ...
  public List<Pair<INDArray[], INDArray[]>> getInputBatches() {
    ...
    if (counter.get() > 1) {
      ...
      while (pos < inputs.size()) {
        ...
        Pair<INDArray[], INDArray[]> merged =
            DataSetUtil.mergeFeatures(featuresToMerge, fMasksToMerge);
        out.add(merged);
  ...
  private static boolean canBatch(INDArray[] first, INDArray[] candidate) {
    // Check if we can batch these inputs into the one array. This isn't always possible - for
    // example, some fully
    ...
    // Note also we can ignore input masks here - they should have shared dimensions with the input,
    // thus if the
  ...
  public void setOutputBatches(List<INDArray[]> output) {
    ...
    for (int outBatchNum = 0;
        outBatchNum < output.size();
        outBatchNum++) { // Iterate over output batch
      ...
      for (int outputNumber = 0;
          outputNumber < currBatchOutputs.length;
          outputNumber++) { // Iterate over net outputs
        INDArray[] split =
            splitExamples(currBatchOutputs[outputNumber], inputBatchIdxs[0], inputBatchIdxs[1]);

        int currentInputBatch = firstInputBatch;
        // Iterate over input batch (examples) - note that each output batch is made up of 1 or more
        // input batches
  ...
  private INDArray[] splitExamples(
      INDArray netOutput, int firstInputComponent, int lastInputComponent) {
  ...
  public INDArray[] getOutput() {
    // basically we should take care of splits here: each client should get its own part of output,
    // wrt order number
