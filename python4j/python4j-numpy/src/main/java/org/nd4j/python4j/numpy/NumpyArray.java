...
public class NumpyArray extends PythonType<INDArray> {
  ...
  public synchronized void init() {
    ...
    if (Boolean.parseBoolean(System.getProperty(IMPORT_NUMPY_ARRAY, DEFAULT_IMPORT_NUMPY_ARRAY))) {
      ...
      if (Boolean.parseBoolean(
          System.getProperty(ADD_JAVACPP_NUMPY_TO_PATH, DEFAULT_ADD_JAVACPP_NUMPY_TO_PATH))) {
  ...
  public INDArray toJava(PythonObject pythonObject) {
    ...
    if (PyObject_IsInstance(pythonObject.getNativePythonObject(), ndarray) != 1) {
      ...
      throw new PythonException(
          "Object is not a numpy array! Use Python.ndarray() to convert object to a numpy array.");
    }
    Py_DecRef(ndarray);
    Py_DecRef(np);
    ...
    if (shapePtr != null) shapePtr.get(shape, 0, shape.length);
    long[] strides = new long[shape.length];
    SizeTPointer stridesPtr = PyArray_STRIDES(npArr);
    if (stridesPtr != null) stridesPtr.get(strides, 0, strides.length);
  ...
  public PythonObject toPython(INDArray indArray) {
    ...
    if (!PythonConstants.releaseGilAutomatically() || PythonConstants.createNpyViaPython()) {
      try (PythonContextManager.Context context =
          new PythonContextManager.Context("__np_array_converter")) {
        ...
        String code =
            "import ctypes\nimport numpy as np\n"
                ...
                + ")"
                + ".from_address("
                + indArray.data().pointer().address()
                + ")\n"
                + "npArr = np.frombuffer(cArr, dtype="
                + ((numpyType == NPY_HALF) ? "'half'" : "ctypes." + ctype)
                + ").reshape("
  ...
  public INDArray adapt(Object javaObject) {
    ...
    throw new PythonException(
        "Cannot cast object of type " + javaObject.getClass().getName() + " to INDArray");
  }
}
