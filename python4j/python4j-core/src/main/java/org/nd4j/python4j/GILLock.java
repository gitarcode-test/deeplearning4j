 ...
 * Permission under apache license granted here:
...
public class GILLock implements Lock {
  ...
  private static CycleDetectingLockFactory cycleDetectingLockFactory =
      CycleDetectingLockFactory.newInstance(CycleDetectingLockFactory.Policies.DISABLED);
  ;

  private static final ReentrantLock reentrantLock =
      cycleDetectingLockFactory.newReentrantLock("python4j_lock");
  ...
  public void unlock() {
    if (lockedCount.decrementAndGet() == 0) {
      if (pythonGIL != null) pythonGIL.close();
      pythonGIL = null;
