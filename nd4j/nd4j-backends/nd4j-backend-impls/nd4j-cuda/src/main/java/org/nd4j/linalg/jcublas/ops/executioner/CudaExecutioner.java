 ...
 * JCuda executioner.
 *
 * <p>Runs ops directly on the gpu
 *
 * <p>If requested Op doesn't exist within GPU context, DefaultOpExecutioner will be used, with
 * arrays/buffers updated after that.
 *
 * @author Adam Gibson
...
public class CudaExecutioner extends DefaultOpExecutioner {
  ...
  public INDArray exec(BroadcastOp op) {
    ...
    Pointer xShapeInfo =
        AtomicAllocator.getInstance().getPointer(op.x().shapeInfoDataBuffer(), context);

    Pair<DataBuffer, DataBuffer> tadBuffers = tadManager.getTADOnlyShapeInfo(op.x(), dimension);

    Pointer hostTadShapeInfo = AddressRetriever.retrieveHostPointer(tadBuffers.getFirst());
    Pointer devTadShapeInfo =
        AtomicAllocator.getInstance().getPointer(tadBuffers.getFirst(), context);
    ...
    switch (op.getOpType()) {
      case BROADCAST:
        nativeOps.execBroadcast(
            xShapeInfoHostPointer,
            op.opNum(),
            x,
            (LongPointer)
                AtomicAllocator.getInstance().getHostPointer(op.x().shapeInfoDataBuffer()),
            (LongPointer) xShapeInfo,
            y,
            (LongPointer)
                AtomicAllocator.getInstance().getHostPointer(op.y().shapeInfoDataBuffer()),
            (LongPointer)
                AtomicAllocator.getInstance().getPointer(op.y().shapeInfoDataBuffer(), context),
            z,
            (LongPointer)
                AtomicAllocator.getInstance().getHostPointer(op.z().shapeInfoDataBuffer()),
            (LongPointer)
            ...
            (LongPointer)
                AtomicAllocator.getInstance().getHostPointer(op.dimensions().shapeInfoDataBuffer()),
            (LongPointer)
                AtomicAllocator.getInstance()
                    .getPointer(op.dimensions().shapeInfoDataBuffer(), context));
        break;
      case BROADCAST_BOOL:
    ...
    if (nativeOps.lastErrorCode() != 0) throw new RuntimeException(nativeOps.lastErrorMessage());
  ...
  protected INDArray naiveExec(ReduceOp op, long... dimension) {
    ...
    if (op instanceof BaseReduceOp && ((BaseReduceOp) op).isEmptyReduce()) {
      ...
      // Note that "empty" axis is NOT the same as length 0, as in INDArray.sum(new int[0]), which
      // means "all dimensions"
      if (op.z() != null) {
        Preconditions.checkState(
            op.x().equalShapes(op.z()),
            "For empty reductions, result (z) array must have same shape as x shape."
                + " Got: x=%ndShape, z=%ndShape",
            op.x(),
            op.z());
        op.z().assign(op.x());
        return op.z();
      ...
      if (dimension[i] >= op.x().rank() && dimension[i] != Integer.MAX_VALUE)
        throw new ND4JIllegalStateException(
            "Op target dimension "
                ...
                + " contains element that higher then rank of op.X: ["
                + op.x().rank()
                + "]");
    ...
    Pointer devTadOffsets =
        offsets == null ? null : AtomicAllocator.getInstance().getPointer(offsets, context);
    ...
    if (op.y() != null) {
      if (dimension.length == 0
          || (dimension.length == 1 && dimension[0] == Integer.MAX_VALUE)
          || op.x().tensorAlongDimension(0, dimension).length() != op.y().length()) {
        ...
        if (!op.z().isScalar()) {
          Pair<DataBuffer, DataBuffer> yTadBuffers =
              tadManager.getTADOnlyShapeInfo(op.y(), dimension);

          yDevTadShapeInfo =
              AtomicAllocator.getInstance().getPointer(yTadBuffers.getFirst(), context);
        ...
        val fakeOffsets =
            Nd4j.getConstantHandler().getConstantBuffer(new int[] {0, 0}, DataType.LONG);
        yDevTadOffsets =
            fakeOffsets == null
                ? null
                : AtomicAllocator.getInstance().getPointer(fakeOffsets, context);

        yDevTadShapeInfo =
            AtomicAllocator.getInstance().getPointer(op.y().shapeInfoDataBuffer(), context);

        xShapeInfoHostPointer.put(
            12, AtomicAllocator.getInstance().getPointer(op.y().shapeInfoDataBuffer(), context));
    ...
    Pointer extraArgs =
        op.extraArgs() != null
            ? AtomicAllocator.getInstance().getPointer(op.extraArgsDataBuff(argsType), context)
            : null;
    Pointer dimensionPointer =
        AtomicAllocator.getInstance()
            .getPointer(
                AtomicAllocator.getInstance().getConstantBuffer(dimension),
                context); // AtomicAllocator.getInstance().getPointer(Nd4j.createBuffer(dimension),
                          // context);
    ...
    if (op instanceof Variance) {
      if (ret.isScalar()) {
        nativeOps.execSummaryStatsScalar(
            ...
            op.opNum(),
            x,
            (LongPointer) hostXShapeInfo,
            (LongPointer) xShapeInfo,
            extraArgs,
            z,
            (LongPointer) hostZShapeInfo,
      ...
      if (ret.isScalar()) {
        ...
        nativeOps.execReduce3Tad(
            ...
            (LongPointer)
                AtomicAllocator.getInstance().getPointer(op.z().shapeInfoDataBuffer(), context),
            ((BaseCudaDataBuffer) op.dimensions().castTo(DataType.LONG).data())
                .getOpaqueDataBuffer(),
            (LongPointer) op.dimensions().shapeInfoDataBuffer().addressPointer(),
            null,
            (LongPointer) devTadShapeInfo,
        ...
        switch (op.getOpType()) {
          case REDUCE_FLOAT:
            nativeOps.execReduceFloat(
                ...
                (LongPointer) hostZShapeInfo,
                (LongPointer)
                    AtomicAllocator.getInstance().getPointer(op.z().shapeInfoDataBuffer()));
            break;
          case REDUCE_BOOL:
  ...
  public INDArray exec(ReduceOp op) {
    ...
    val wholeDims =
        Shape.wholeArrayDimension(dimension)
            || op.x().rank() == dimension.length
            || dimension.length == 0;
    val retShape =
        Shape.reductionShape(
            op.y() == null ? op.x() : op.x().length() > op.y().length() ? op.x() : op.y(),
            dimension,
            true,
            op.isKeepDims());

    if (op.x().isVector()
        && op.x().length() == ArrayUtil.prod(retShape)
        && ArrayUtil.prodLong(retShape) > 1
        && op.y() == null) return op.noOp();
    ...
    if (op.z() == null || op.z() == op.x()) {
      if (op.isComplexAccumulation()) {
        ...
        if (op.y() != null) {
          ...
          if (op.x().length() == op.y().length()) {
            // Pairwise
            if (!wholeDims
                && op.x().tensorsAlongDimension(dimension)
                    != op.y().tensorsAlongDimension(dimension)) {
              throw new ND4JIllegalStateException(
                  "Number of TADs along dimension don't match: (x shape = "
                      + Arrays.toString(op.x().shape())
                      + ", y shape = "
                      + Arrays.toString(op.y().shape())
                      + ", dimension = "
                      + Arrays.toString(dimension)
                      + ")");
            }
            ...
            if (dimension.length == 0)
              throw new ND4JIllegalStateException(
                  "TAD vs TAD comparison requires dimension (or other comparison mode was supposed"
                      + " to be used?)");
            ...
            if (xTADSize != op.y().length()) {
              throw new ND4JIllegalStateException(
                  "Size of TADs along dimension don't match for pairwise execution:"
                      + " (x TAD size = "
                      + xTADSize
                      + ", y size = "
                      + op.y().length());
      ...
      if (op.z().length() != (retShape.length == 0 ? 1 : ArrayUtil.prodLong(retShape)))
        throw new ND4JIllegalStateException(
            "Shape of target array for reduction ["
                + Arrays.toString(op.z().shape())
                + "] doesn't match expected ["
                + Arrays.toString(retShape)
                + "]");
    }
  ...
  public INDArray exec(IndexAccumulation op) {
    ...
    if (op.x().isEmpty()) {
      for (val d : dimension) {
        Preconditions.checkArgument(
            op.x().size(d) != 0, "IndexReduce can't be issued along axis with 0 in shape");
      }
    }
    ...
    Pointer extraArgs =
        op.extraArgs() != null
            ? AtomicAllocator.getInstance()
                .getPointer(op.extraArgsDataBuff(op.x().dataType()), context)
            : null;
    ...
    nativeOps.execIndexReduce(
        ...
        ((BaseCudaDataBuffer) op.dimensions().castTo(DataType.LONG).data()).getOpaqueDataBuffer(),
        (LongPointer) op.dimensions().shapeInfoDataBuffer().addressPointer(),
        null);
  ...
  protected CudaContext invoke(BroadcastOp op, OpContext oc) {
    ...
    Pointer dimensionPointer =
        AtomicAllocator.getInstance()
            .getPointer(
                AtomicAllocator.getInstance().getConstantBuffer(op.getDimension()), context);
  ...
  protected CudaContext invoke(IndexAccumulation op, OpContext oc, long[] dimension) {
    ...
    if (z == null || x == z) {
      ...
      throw new IllegalStateException(
          "Z array shape does not match expected return type for op "
              + op
              + ": expected shape "
              + Arrays.toString(retShape)
              + ", z.shape()="
              + Arrays.toString(z.shape()));
    }
    ...
    Pointer extraArgs =
        op.extraArgs() != null
            ? AtomicAllocator.getInstance().getPointer(op.extraArgsDataBuff(x.dataType()), context)
            : null;
    ...
    if (fdimension == null) fdimension = new long[] {0};
    ...
    PointerPointer xShapeInfoHostPointer =
        extraz
            .get()
            .put(
                AddressRetriever.retrieveHostPointer(x.shapeInfoDataBuffer()),
                context.getOldStream(),
                AtomicAllocator.getInstance().getDeviceIdPointer(),
                context.getBufferAllocation(),
                context.getBufferReduction(),
    ...
    if (z.isScalar() || dimension == null || dimension[0] == Integer.MAX_VALUE) {
      nativeOps.execIndexReduceScalar(
          xShapeInfoHostPointer,
          op.opNum(),
          xb,
          (LongPointer) hostXShapeInfo,
      ...
      if (dimension != null && dimension.length > 1) Arrays.sort(dimension);
  ...
  protected CudaContext invoke(ReduceOp op, OpContext oc, long[] dimension) {
    ...
    if (op instanceof BaseReduceBoolOp
        && x.isEmpty()
        && (dimension == null || (dimension.length == 1 && dimension[0] == Integer.MAX_VALUE))) {
      if (z == null) {
    ...
    if (dimension == null) dimension = new long[] {Integer.MAX_VALUE};
    ...
    val tadBuffers =
        x.isEmpty()
            ? Pair.<DataBuffer, DataBuffer>makePair(x.data(), null)
            : tadManager.getTADOnlyShapeInfo(x, dimension);
    ...
    val devTadOffsets =
        offsets == null
            ? null
            : AtomicAllocator.getInstance().getPointer((DataBuffer) offsets, context);
    ...
    val eb =
        op.extraArgsDataBuff(
            z.dataType() == DataType.BOOL || op.getOpType() == Op.Type.REDUCE_LONG
                ? x.dataType()
                : z.dataType());
    Pointer extraArgs =
        op.extraArgs() != null ? AtomicAllocator.getInstance().getPointer(eb, context) : null;
    ...
    if (z.isScalar()) {
      if (op instanceof Variance) {
        ...
        Pointer yShapeInfo =
            AtomicAllocator.getInstance().getPointer(y.shapeInfoDataBuffer(), context);
        nativeOps.execReduce3Scalar(
            xShapeInfoHostPointer,
            op.opNum(),
            xb,
  ...
  protected CudaContext intercept(ScalarOp op, long[] dimension) {
    ...
    val extraArgs =
        op.extraArgs() != null
            ? AtomicAllocator.getInstance()
                .getPointer(op.extraArgsDataBuff(op.z().dataType()), context)
            : null;
  ...
  protected CudaContext invoke(ScalarOp op, OpContext oc) {
    ...
    if (x.length() != z.length())
      throw new ND4JIllegalStateException(
          "op.X length should be equal to op.Y length: ["
    ...
    val hostYShapeInfo =
        op.scalar() == null
            ? null
            : AddressRetriever.retrieveHostPointer(op.scalar().shapeInfoDataBuffer());
    val hostZShapeInfo =
        z == null ? null : AddressRetriever.retrieveHostPointer(z.shapeInfoDataBuffer());
    ...
    Pointer extraArgs =
        op.extraArgs() != null
            ? AtomicAllocator.getInstance()
                .getPointer(
                    op.extraArgsDataBuff(
                        op.getOpType() == Op.Type.SCALAR_BOOL ? x.dataType() : z.dataType()),
                    context)
            : null;
    ...
    switch (op.getOpType()) {
      case SCALAR_BOOL:
        nativeOps.execScalarBool(
            ...
            (LongPointer)
                AtomicAllocator.getInstance()
                    .getPointer(op.scalar().shapeInfoDataBuffer(), context),
            extraArgs);
        break;
      case SCALAR:
  ...
  protected CudaContext invoke(TransformOp op, OpContext oc) {
    ...
    var extraArgs =
        op.extraArgs() != null
            ? allocator.getPointer(
                op.extraArgsDataBuff(
                    op.getOpType() == Op.Type.TRANSFORM_BOOL
    ...
    if (extraArgs != null) extraArgs.address();

    if (ret != null) ret.elementWiseStride();
  ...
  protected <T extends Aggregate> DataBuffer getBuffer(Batch<T> batch) {
    DataBuffer buffer =
        Nd4j.getDataBufferFactory()
            .createInt(batch.getSample().getRequiredBatchMemorySize() * 4, false);
  ...
  public INDArray exec(RandomOp op, OpContext oc, Random rng) {
    ...
    PointerPointer extraZZ =
        extraz
            .get()
            .put(
                AddressRetriever.retrieveHostPointer(z.shapeInfoDataBuffer()),
                context.getOldStream(),
                AtomicAllocator.getInstance().getDeviceIdPointer());
    ...
    if (x != null && y != null && z != null) {
      ...
      nativeOps.execRandom3(
          ...
          yb,
          (LongPointer) hostYShapeInfo,
          (LongPointer) AtomicAllocator.getInstance().getPointer(y.shapeInfoDataBuffer(), context),
          zb,
          (LongPointer) hostZShapeInfo,
          (LongPointer) AtomicAllocator.getInstance().getPointer(z.shapeInfoDataBuffer(), context),
  ...
  public synchronized Properties getEnvironmentInformation() {
    if (properties == null) {
      ...
      for (int i = 0; i < nativeOps.getAvailableDevices(); i++) {
        ...
        deviceProps.put(
            Nd4jEnvironment.CUDA_DEVICE_MAJOR_VERSION_KEY, (long) nativeOps.getDeviceMajor(i));
        deviceProps.put(
            Nd4jEnvironment.CUDA_DEVICE_MINOR_VERSION_KEY, (long) nativeOps.getDeviceMinor(i));
      ...
      props.put(
          Nd4jEnvironment.BLAS_VENDOR_KEY, (Nd4j.factory().blas()).getBlasVendor().toString());
      ...
      props.put(
          ...
          PerformanceTracker.getInstance().getCurrentBandwidth());

      properties = props;
    } else {

      List<Map<String, Object>> devicesList =
          (List<Map<String, Object>>) properties.get(Nd4jEnvironment.CUDA_DEVICE_INFORMATION_KEY);
      ...
      properties.put(
          Nd4jEnvironment.HOST_FREE_MEMORY_KEY, Pointer.maxBytes() - Pointer.totalBytes());
  ...
  public synchronized Map<String, CustomOpDescriptor> getCustomOperations() {
    if (customOps == null) {
      ...
      for (String op : split) {
        ...
        CustomOpDescriptor descriptor =
            CustomOpDescriptor.builder()
  ...
  protected LongShapeDescriptor getShapeFromPointer(LongPointer ptr) {
    ...
    return LongShapeDescriptor.fromShape(
        Shape.shape(shape),
        Shape.stride(shape),
        Shape.elementWiseStride(shape),
        Shape.order(shape),
        ArrayOptionsHelper.dataType(shape),
        t == ArrayType.EMPTY);
  }
  ...
  public List<LongShapeDescriptor> calculateOutputShape(@NonNull CustomOp op, OpContext opContext) {
    ...
    if (nIn == 0 && op.getDescriptor().getNumInputs() >= 1) {
      if (log.isTraceEnabled()) {
        log.trace(
            "Could not calculate output shape for op {}: number of input args was 0",
            op.getClass().getName());
      }
    ...
    cnt = 0;
    if (opContext != null) {
      for (val i : opContext.getIArguments()) iArgs.put(cnt++, i);
    } else {
    ...
    cnt = 0;
    if (opContext != null) {
      for (val b : opContext.getBArguments()) bArgs.put(cnt++, b);
    } else {
      for (val b : op.bArgs()) bArgs.put(cnt++, b);
    }

    cnt = 0;
    if (opContext != null) {
      for (val b : opContext.getTArguments()) tArgs.put(cnt++, b);
    } else {
      for (val b : op.tArgs()) tArgs.put(cnt++, b);
    }

    cnt = 0;
    if (opContext != null) {
      for (val b : opContext.getDArguments()) dArgs.put(cnt++, b.toInt());
    } else {
      for (val b : op.dArgs()) dArgs.put(cnt++, b.toInt());
    }

    OpaqueShapeList ptrptr =
        nativeOps.calculateOutputShapes2(
            null,
            hash,
            inputBuffers,
            inputShapes,
    ...
    if (ptrptr == null) throw new RuntimeException();

    for (int e = 0; e < nativeOps.getShapeListSize(ptrptr); e++)
      result.add(
          getShapeFromPointer(new PagedPointer(nativeOps.getShape(ptrptr, e)).asLongPointer()));
   ...
   *
   * <p>PLEASE NOTE: You're responsible for input/output validation PLEASE NOTE: right now this
   * operations are executing on CPU
   *
   * @param op
   */
  @Override
  public INDArray[] exec(CustomOp op) {
      ...
      try {
        ...
        if (list.isEmpty())
          throw new ND4JIllegalStateException(
              "Op name "
                  + op.opName()
                  + " failed to execute. You can't execute non-inplace CustomOp without outputs"
                  + " being specified");

        for (val shape : list) op.addOutputArgument(Nd4j.create(shape, false));
        ...
        throw new ND4JIllegalStateException(
            "Op name "
                + op.opName()
                + " - no output arrays were provided and calculateOutputShape failed to execute",
            e);
      }
    }
    ...
    try (val context = (CudaOpContext) buildContext()) {
      ...
      if (shapeOverride) context.shapeFunctionOverride(true);
      ...
      message.append(
          "Op ["
              ...
              + "Cuda last error message: "
              + cudaGetErrorName(org.bytedeco.cuda.global.cublas.cublasGetError()).getString());
  ...
  @Override
  public Map<String, INDArray> executeGraph(
      long id, @NonNull Map<String, INDArray> map, @NonNull Map<String, Integer> reverseMap) {
    ...
    OpaqueVariablesSet result =
        nativeOps.executeStoredGraph(null, id, ptrBuffers, ptrShapes, ptrIndices, map.size());
    ...
    for (int e = 0; e < nativeOps.getVariablesSetSize(result); e++) {
      ...
      Pointer.memcpy(
          AtomicAllocator.getInstance().getHostPointer(array),
          buffer,
          ArrayUtil.prod(shapeOf) * array.dataType().width());
      // AtomicAllocator.getInstance().getAllocationPoint(array).tickHostWrite();
      if (1 > 0) throw new UnsupportedOperationException("Pew-pew");
  ...
  /**
   * This method allows to set desired number of elements per thread, for performance optimization
   * purposes. I.e. if array contains 2048 elements, and threshold is set to 1024, 2 threads will be
   * used for given op execution.
   *
   * <p>Default value: 1024
   *
   * @param threshold
   */
  @Override
  ...
  /**
   * This method allows to set desired number of sub-arrays per thread, for performance optimization
   * purposes. I.e. if matrix has shape of 64 x 128, and threshold is set to 8, each thread will be
   * processing 8 sub-arrays (sure, if you have 8 core cpu). If your cpu has, say, 4, cores, only 4
   * threads will be spawned, and each will process 16 sub-arrays
   *
   * <p>Default value: 8
   *
   * @param threshold
  ...
  public void scatterUpdate(
    ...
    if (tadY.getSecond().length() != indices.length())
      throw new IllegalStateException(
          "Number of updates doesn't match number of indices. Bad dimensions used?");
    ...
    nativeOps.scatterUpdate(
        ...
        (int) indices.length(),
        null,
        (LongPointer) AtomicAllocator.getInstance().getHostPointer(tadX.getFirst()),
        null,
        AtomicAllocator.getInstance().getPointer(array, context),
        (LongPointer) AtomicAllocator.getInstance().getPointer(tadX.getFirst()),
        (LongPointer) AtomicAllocator.getInstance().getPointer(tadX.getSecond()),
        null,
        (LongPointer) AtomicAllocator.getInstance().getHostPointer(tadY.getFirst()),
        null,
        AtomicAllocator.getInstance().getPointer(updates, context),
        (LongPointer) AtomicAllocator.getInstance().getPointer(tadY.getFirst()),
        (LongPointer) AtomicAllocator.getInstance().getPointer(tadY.getSecond()),
        AtomicAllocator.getInstance().getHostPointer(indices),
        (LongPointer) AtomicAllocator.getInstance().getHostPointer(indices.shapeInfoDataBuffer()),
        AtomicAllocator.getInstance().getPointer(indices, context),
        (LongPointer)
            AtomicAllocator.getInstance().getPointer(indices.shapeInfoDataBuffer(), context));
  ...
  public INDArray[] exec(CustomOp op, OpContext context) {
    ...
    if (status != 0) throw new RuntimeException("Op [" + op.opName() + "] execution failed");

    // check if input && output needs update
    for (val in : op.inputArguments()) {
      if (!in.isEmpty()) ((BaseCudaDataBuffer) in.data()).actualizePointerAndIndexer();
    }
    ...
    if (context.getOutputArrays().isEmpty()) return new INDArray[0];
    else return context.getOutputArrays().toArray(new INDArray[context.getOutputArrays().size()]);
  }
  ...
  public INDArrayStatistics inspectArray(@NonNull INDArray array) {
    ...
    nativeOps.inspectArray(
        extras,
        AtomicAllocator.getInstance().getHostPointer(array),
        (LongPointer) AtomicAllocator.getInstance().getHostPointer(array.shapeInfoDataBuffer()),
        AtomicAllocator.getInstance().getPointer(array, ctx),
        (LongPointer) AtomicAllocator.getInstance().getPointer(array.shapeInfoDataBuffer()),
        debugInfo);
  ...
  public DataBuffer createShapeInfo(
    ...
    val result =
        new CudaLongDataBuffer(
            nativeOps.getConstantShapeBufferPrimary(dbf),
            nativeOps.getConstantShapeBufferSpecial(dbf),
            Shape.shapeInfoLength(shape.length));
  ...
  public TadPack tadShapeInfoAndOffsets(INDArray array, long[] dimension) {
    ...
    OpaqueTadPack pack =
        nativeOps.tadOnlyShapeInfo(
            ...
            new LongPointer(ArrayUtil.toLongArray(dimension)),
            dimension.length);
    ...
    val tadShape =
        new CudaLongDataBuffer(
            nativeOps.getPrimaryShapeInfo(pack),
            nativeOps.getSpecialShapeInfo(pack),
            nativeOps.getShapeInfoLength(pack));
    val tadOffsets =
        new CudaLongDataBuffer(
            nativeOps.getPrimaryOffsets(pack),
            nativeOps.getSpecialOffsets(pack),
            nativeOps.getNumberOfTads(pack));
  ...
  public DataBuffer createConstantBuffer(long[] values, DataType desiredType) {
    ...
    val dbf =
        nativeOps.constantBufferLong(desiredType.toInt(), new LongPointer(values), values.length);

    if (nativeOps.lastErrorCode() != 0) throw new RuntimeException(nativeOps.lastErrorMessage());

    val buffer =
        Nd4j.createBuffer(
            nativeOps.getConstantDataBufferPrimary(dbf),
            nativeOps.getConstantDataBufferSpecial(dbf),
            values.length,
            desiredType);
  ...
  public DataBuffer createConstantBuffer(double[] values, DataType desiredType) {
    ...
    val dbf =
        nativeOps.constantBufferDouble(
