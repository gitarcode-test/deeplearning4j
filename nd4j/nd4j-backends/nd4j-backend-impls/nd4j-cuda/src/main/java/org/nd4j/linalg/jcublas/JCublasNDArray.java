...
public class JCublasNDArray extends BaseNDArray {
  ...
  public JCublasNDArray(
      long[] shape, long[] stride, long offset, char ordering, boolean initialize) {
    super(shape, stride, offset, ordering, initialize);
  }
  ...
  /**
   * Create an JCublasNDArray from the specified slices. This will go through and merge all of the
   * data from each slice in to one JCublasNDArray which will then take the specified shape
   *
  ...
  public JCublasNDArray(
      DataBuffer buffer, long[] shape, long[] stride, char ordering, DataType dataType) {
  ...
  public String toString() {
    if (!isS()) AtomicAllocator.getInstance().synchronizeHostData(this);
  ...
  public LongShapeDescriptor shapeDescriptor() {
    return LongShapeDescriptor.fromShape(
        shape(), stride(), elementWiseStride(), ordering(), dataType(), isEmpty());
  }
  ...
  public INDArray unsafeDuplication(boolean blocking) {
    ...
    DataBuffer rb =
        Nd4j.getMemoryManager().getCurrentWorkspace() == null
            ? Nd4j.getDataBufferFactory().createSame(this.data, false)
            : Nd4j.getDataBufferFactory()
                .createSame(this.data, false, Nd4j.getMemoryManager().getCurrentWorkspace());

    INDArray ret = Nd4j.createArrayFromShapeBuffer(rb, this.shapeInfoDataBuffer());

    if (blocking) Nd4j.getExecutioner().push();
    ...
    if (srcPoint.isActualOnDeviceSide()) {
      ...
      NativeOpsHolder.getInstance()
          ...
          .memcpyAsync(
              ...
              srcPoint.getDevicePointer(),
              this.data.length() * this.data.getElementSize(),
              CudaConstants.cudaMemcpyDeviceToDevice,
              blocking ? context.getOldStream() : context.getSpecialStream());
    ...
    if (blocking) context.syncOldStream();
    else context.syncSpecialStream();

    PerformanceTracker.getInstance()
        .helperRegisterTransaction(
            dstPoint.getDeviceId(), prof, dstPoint.getNumberOfBytes(), direction);
  ...
  public INDArray leverageTo(String id) {
    ...
    if (!this.isView()) {
      ...
      val context =
          AtomicAllocator.getInstance().getFlowController().prepareAction(pointDst, pointSrc);

      MemcpyDirection direction = MemcpyDirection.DEVICE_TO_DEVICE;
      val perfD = PerformanceTracker.getInstance().helperStartTransaction();

      if (pointSrc.isActualOnDeviceSide()) {
        if (NativeOpsHolder.getInstance()
                .getDeviceNativeOps()
                .memcpyAsync(
                    pointDst.getDevicePointer(),
                    pointSrc.getDevicePointer(),
                    this.length() * Nd4j.sizeOfDataType(buffer.dataType()),
                    CudaConstants.cudaMemcpyDeviceToDevice,
      ...
      PerformanceTracker.getInstance()
          .helperRegisterTransaction(
              pointDst.getDeviceId(), perfD, pointSrc.getNumberOfBytes(), direction);
  ...
  public INDArray migrate() {
    ...
    if (!this.isView()) {
      ...
      PerformanceTracker.getInstance()
          .helperRegisterTransaction(
              pointDst.getDeviceId(), perfD, pointDst.getNumberOfBytes(), direction);
  ...
  protected int stringBuffer(FlatBufferBuilder builder, DataBuffer buffer) {
    Preconditions.checkArgument(
        buffer.dataType() == DataType.UTF8, "This method can be called on UTF8 buffers only");
    try {
  ...
  public String getString(long index) {
    if (!isS())
      throw new UnsupportedOperationException(
          "This method is usable only on String dataType, but got [" + this.dataType() + "]");
