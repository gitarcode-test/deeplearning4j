...
public class Configuration implements Serializable {
  ...
  @Getter @Deprecated // Only SEQUENTIAL is supported
  ...
  @Getter private long noGcWindowMs = 100;
  ...
  @Deprecated @Getter private Aggressiveness hostDeallocAggressiveness = Aggressiveness.REASONABLE;

  @Deprecated @Getter private Aggressiveness gpuDeallocAggressiveness = Aggressiveness.REASONABLE;

  /** Allocation aggressiveness */
  @Deprecated @Getter private Aggressiveness gpuAllocAggressiveness = Aggressiveness.REASONABLE;
  ...
  @Getter private long maximumHostCache = 1024 * 1024 * 1024L;

  @Getter private long maximumDeviceCache = 512L * 1024L * 1024L;

  @Getter private boolean usePreallocation = false;

  @Getter private int preallocationCalls = 10;

  @Getter private long maximumHostCacheableLength = 100663296;

  @Getter private long maximumDeviceCacheableLength = 16L * 1024L * 1024L;
  ...
  private void parseEnvironmentVariables() {
    ...
    if (System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_BLOCK_SIZE) != null) {
      try {
        ...
        log.error(
            ...
            System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_BLOCK_SIZE));
      }
    }
    ...
    if (System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_GRID_SIZE) != null) {
      try {
        ...
        log.error(
            ...
            System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_GRID_SIZE));
      }
    }
    ...
    if (System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_CONTEXTS) != null) {
      try {
        ...
        log.error(
            "Can't parse {}: [{}]",
            ND4JEnvironmentVars.ND4J_CUDA_MAX_CONTEXTS,
            System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_CONTEXTS));
      }
    }
    ...
    if (System.getenv(ND4JEnvironmentVars.ND4J_CUDA_FORCE_SINGLE_GPU) != null) {
      try {
        boolean var =
            Boolean.parseBoolean(System.getenv(ND4JEnvironmentVars.ND4J_CUDA_FORCE_SINGLE_GPU));
        allowMultiGPU(!var);
      } catch (Exception e) {
        log.error(
            "Can't parse {}: [{}]",
            ND4JEnvironmentVars.ND4J_CUDA_FORCE_SINGLE_GPU,
            System.getenv(ND4JEnvironmentVars.ND4J_CUDA_FORCE_SINGLE_GPU));
      }
    }
    ...
    if (System.getenv(ND4JEnvironmentVars.ND4J_CUDA_USE_PREALLOCATION) != null) {
      try {
        boolean var =
            Boolean.parseBoolean(System.getenv(ND4JEnvironmentVars.ND4J_CUDA_USE_PREALLOCATION));
        allowPreallocation(var);
      } catch (Exception e) {
        log.error(
            "Can't parse {}: [{}]",
            ND4JEnvironmentVars.ND4J_CUDA_USE_PREALLOCATION,
            System.getenv(ND4JEnvironmentVars.ND4J_CUDA_USE_PREALLOCATION));
      }
    }
    ...
    if (System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_DEVICE_CACHE) != null) {
      try {
        ...
        log.error(
            ...
            System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_DEVICE_CACHE));
      }
    }
    ...
    if (System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_HOST_CACHE) != null) {
      try {
        ...
        log.error(
            ...
            System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_HOST_CACHE));
      }
    }
    ...
    if (System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_DEVICE_ALLOCATION) != null) {
      try {
        long var =
            Long.parseLong(System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_DEVICE_ALLOCATION));
        setMaximumSingleDeviceAllocation(var);
      } catch (Exception e) {
        log.error(
            "Can't parse {}: [{}]",
            ND4JEnvironmentVars.ND4J_CUDA_MAX_DEVICE_ALLOCATION,
            System.getenv(ND4JEnvironmentVars.ND4J_CUDA_MAX_DEVICE_ALLOCATION));
      }
    }
  }
  ...
  public Configuration setPoolSize(int poolSize) {
    if (poolSize < 8) throw new IllegalStateException("poolSize can't be lower then 8");
   ...
   *
   * <p>PLEASE NOTE: Accepted value range is 0.01 > x < 0.95
   *
   * @param percentage
   */
  ...
  void updateDevice() {
    ...
    if (cnt == 0) throw new RuntimeException("No CUDA devices were found in system");
   ...
   *
   * <p>PLEASE NOTE: This method also returns TRUE if system has only one device. This is done to
   * guarantee reallocation avoidance within same device.
   *
   * @return
   */
   ...
   *
   * <p>PLEASE NOTE: This method
   *
   * @param deviceId
   * @return
   */
  public Configuration banDevice(@NonNull Integer deviceId) {
    if (!availableDevices.contains(deviceId)) return this;
  ...
  /**
   * This method forces specific device to be used. All other devices present in system will be
   * ignored.
   *
   * @param deviceId
   * @return
  ...
  /**
   * This method forces specific devices to be used. All other devices present in system will be
   * ignored.
   *
   * @param devices
   * @return
  ...
  public Configuration useDevices(@NonNull int... devices) {
    ...
    for (int device : devices) {
      if (!availableDevices.contains(device)) {
        ...
        if (!usableDevices.contains(device)) usableDevices.add(device);
      }
    }
  ...
  /**
   * This method allows you to set maximum host allocation. However, it's recommended to leave it as
   * default: Xmx + something.
   *
   * @param max amount of memory in bytes
   */
  public Configuration setMaximumZeroAllocation(long max) {
    long xmx = Runtime.getRuntime().maxMemory();
    if (max < xmx) log.warn("Setting maximum memory below -Xmx value can cause problems");

    if (max <= 0) throw new IllegalStateException("You can't set maximum host memory <= 0");
  ...
  /**
   * This method allows you to set maximum device allocation. It's recommended to keep it equal to
   * MaximumZeroAllocation
   *
   * @param max
   */
  public Configuration setMaximumDeviceAllocation(long max) {
    if (max < 0) throw new IllegalStateException("You can't set maximum device memory < 0");
   ...
   *
   * <p>Default value: -1 (that means pick value automatically, device occupancy dependent)
   *
   * @param blockDim
   * @return
   */
  ...
  /**
   * With debug enabled all CUDA launches will become synchronous, with forced stream
   * synchronizations after calls.
   *
   ...
   *
   * <p>SEQUENTIAL: Issue commands in order Java compiler sees them. ASYNCHRONOUS: Issue commands
   * asynchronously, if that's possible. OPTIMIZED: Not implemented yet. Equals to asynchronous for
   * now.
   *
   * <p>Default value: SEQUENTIAL
   *
   ...
   *
   * <p>DIRECT: Do not cache anything, release memory as soon as it's not used. CACHE_HOST: Cache
   * host memory only, Device memory (if any) will use DIRECT mode. CACHE_ALL: All memory will be
   * cached.
   *
   * <p>Defailt value: CACHE_ALL
   *
  ...
  /**
   * This method allows to specify initial memory to be used within system. HOST: all data is
   * located on host memory initially, and gets into DEVICE, if used frequent enough DEVICE: all
   * memory is located on device. DELAYED: memory allocated on HOST first, and on first use gets
   * moved to DEVICE
   *
   * <p>PLEASE NOTE: For device memory all data still retains on host side as well.
   *
   * <p>Default value: DEVICE
   *
   * @param initialMemory
   * @return
   */
  public Configuration setFirstMemory(@NonNull AllocationStatus initialMemory) {
    if (initialMemory != AllocationStatus.DEVICE
        && initialMemory != AllocationStatus.HOST
        && initialMemory != AllocationStatus.DELAYED)
      throw new IllegalStateException(
          "First memory should be either [HOST], [DEVICE] or [DELAYED]");
  ...
  public Configuration setNumberOfGcThreads(int numThreads) {
    if (numThreads <= 0 || numThreads > 20)
      throw new IllegalStateException(
          "Please, use something in range of [1..20] as number of GC threads");

    if (!isInitialized()) this.numberOfGcThreads = numThreads;

    return this;
  }

  /**
   * This method allows to specify maximum length of single memory chunk that's allowed to be
   * cached. Please note: -1 value totally disables limits here.
   *
   * <p>Default value: 96 MB
   *
   * @param maxLen
   * @return
   */
  ...
  /**
   * This method allows to specify number of preallocation calls done by cache subsystem in
   * parallel, to serve later requests.
   *
   * <p>Default value: 25
   *
   * @param numCalls
  ...
  /**
   * This method allows you to specify command queue length, as primary argument for asynchronous
  ...
  public Configuration setCommandQueueLength(int length) {
    if (length <= 0) throw new IllegalStateException("Command queue length can't be <= 0");
  ...
  /**
   * This option specifies minimal time gap between two subsequent System.gc() calls Set to 0 to
   * disable this option.
   *
   * @param windowMs
   * @return
  ...
  public Configuration setNoGcWindowMs(long windowMs) {
    if (windowMs < 1) throw new IllegalStateException("No-GC window should have positive value");
   ...
   * <p>Default value: 4
   *
   * <p>PLEASE NOTE: This parameter has effect only for ASYNCHRONOUS execution model
   *
   * @param length
   * @return
   */
  public Configuration setCommandLanesNumber(int length) {
    if (length < 1) throw new IllegalStateException("Command Lanes number can't be < 1");
   ...
   *
   * <p>PLEASE NOTE: This is NOT magic method, that will automatically scale your application
   * performance.
   *
   * @param reallyAllow
   * @return
