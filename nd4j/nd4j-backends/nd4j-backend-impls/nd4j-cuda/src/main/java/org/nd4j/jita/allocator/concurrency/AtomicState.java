 ...
 *
 * <p>TODO: add more explanations here
 *
...
public class AtomicState {
   ...
   *
   * <p>PLEASE NOTE: this method is blocking, if memory is in Toe state
   */
   ...
   * This method requests to change state to Tick.
   *
   * <p>PLEASE NOTE: this method is blocking, if memory is in Toe state. PLEASE NOTE: if Tick can't
   * be acquired within specified timeframe, exception will be thrown
   *
   * @param time
   * @param timeUnit
   */
   ...
   *
   * <p>PLEASE NOTE: this method is blocking, untill all Tick requests are brought down to Tack
   * state;
   */
    ...
    try {
      ...
      while (getCurrentState() != AccessState.TACK) {
        ...
        if (getCurrentState() == AccessState.TOE
            && toeThread.get() == Thread.currentThread().getId()) {
          break;
        }
        Thread.sleep(20);
      }
   ...
   * This method requests to change state to Toe
   *
   * <p>PLEASE NOTE: this method is non-blocking, if Toe request is impossible atm, it will return
   * false.
   *
   ...
   *
   * <p>PLEASE NOTE: only the thread originally entered Toe state is able to release it.
   */
   ...
   * This methods
   *
   * @return number of WAITING tick requests, if they are really WAITING. If state isn't Toe, return
  ...
  /**
   * This method discards scheduled Toe state entry, but doesn't exits currently entered Toe state,
   * if that's the case.
   */
