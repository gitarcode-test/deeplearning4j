 ...
 * @author Adam Gibson
 */
public class CudaUtf16Buffer extends BaseCudaDataBuffer {
     ...
     */
    public CudaUtf16Buffer(Pointer pointer, Indexer indexer, long length) {
        super(pointer, indexer, length);
    }

    public CudaUtf16Buffer(ByteBuffer buffer, DataType dataType, long length, long offset) {
        super(buffer, dataType, length, offset);
    }

    public CudaUtf16Buffer(long length) {
        super(length);
    }
    ...
    public CudaUtf16Buffer(long length, boolean initialize, MemoryWorkspace workspace) {
        super((length + 1) * 8, 1, initialize, workspace);
        numWords = length;
    }

    public CudaUtf16Buffer(int[] ints, boolean copy, MemoryWorkspace workspace) {
        super(ints, copy, workspace);
    }

    public CudaUtf16Buffer(byte[] data, long numWords) {
    ...
    protected DataBuffer create(long length) {
        return new CudaUtf16Buffer(length);
    }
