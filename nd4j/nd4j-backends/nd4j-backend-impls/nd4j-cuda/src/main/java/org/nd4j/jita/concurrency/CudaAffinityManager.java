...
public class CudaAffinityManager extends BasicAffinityManager {
   ...
   *
   * <p>If no device was assigned to this thread before this call, it'll be assinged here.
   *
   * @return
   */
  @Override
  ...
  protected Integer getNextDevice(long threadId) {
    Integer device = null;
    if (!CudaEnvironment.getInstance().getConfiguration().isForcedSingleGPU()
      ...
      synchronized (this) {
        ...
        logger.debug(
            "Mapping thread [{} - {}] to device [{}], out of [{}] devices...",
            threadId,
            n,
            device,
            CudaEnvironment.getInstance().getConfiguration().getAvailableDevices().size());
   ...
   *
   * <p>Please note: returned value might be different from actual number of used devices.
   *
    ...
    if (numberOfDevices.get() < 0) {
      synchronized (this) {
        if (numberOfDevices.get() < 1) {
          numberOfDevices.set(
              NativeOpsHolder.getInstance().getDeviceNativeOps().getAvailableDevices());
        }
      }
    }
  ...
  public synchronized INDArray replicateToDevice(Integer deviceId, INDArray array) {
    ...
    if (array.isS()) return array.dup(array.ordering());
    ...
    AtomicAllocator.getInstance()
        .getPointer(array, AtomicAllocator.getInstance().getDeviceContext());
    ...
    DataBuffer newShapeBuffer =
        Nd4j.getShapeInfoProvider()
            .createShapeInformation(shape, stride, elementWiseStride, ordering, dtype, empty)
            .getFirst();
  ...
  /**
   * This method marks given DataBuffer as actual in specific location (either host, device, or
   * both)
   *
   * @param buffer
   * @param location
  ...
  public void ensureLocation(INDArray array, Location location) {
    ...
    if (array == null || array.isEmpty() || array.isS()) return;
  ...
  public Location getActiveLocation(INDArray array) {
    if (array.isEmpty()) return Location.EVERYWHERE;
