 ...
 *
 * <p>CUDA implementation for DataBuffer always uses JavaCPP as allocationMode, and device access is
 * masked by appropriate allocator mover implementation.
 *
 * <p>Memory allocation/deallocation is strictly handled by allocator, since JavaCPP alloc/dealloc
 * has nothing to do with CUDA. But besides that, host pointers obtained from CUDA are 100%
 * compatible with CPU
 *
 * @author Adam Gibson
 * @author raver119@gmail.com
 */
public abstract class BaseCudaDataBuffer extends BaseDataBuffer
    implements JCudaBuffer, Deallocatable {

  @Getter protected transient volatile AllocationPoint allocationPoint;
  public static final long BASE_CUDA_DATA_BUFFER_OFFSET = RandomUtils.nextLong();
  ...
  public BaseCudaDataBuffer(
      @NonNull Pointer pointer,
      @NonNull Pointer specialPointer,
      @NonNull Indexer indexer,
      long length) {
    ...
    ptrDataBuffer =
        OpaqueDataBuffer.externalizedDataBuffer(length, this.type, pointer, specialPointer);
  ...
  public BaseCudaDataBuffer(Pointer pointer, Indexer indexer, long length) {
    ...
    PerformanceTracker.getInstance()
        .helperRegisterTransaction(
            allocationPoint.getDeviceId(),
            perfD / 2,
            allocationPoint.getNumberOfBytes(),
  ...
  public void lazyAllocateHostPointer() {
    ...
    } else if (allocationPoint.getHostPointer() != null
        && allocationPoint.getHostPointer().address() != this.pointer.address()) {
      initHostPointerAndIndexer();
    }
  }
  ...
  protected BaseCudaDataBuffer(ByteBuffer buffer, DataType dtype, long length, long offset) {
    ...
    if (offset > 0) temp = new PagedPointer(temp.address() + offset * getElementSize());
    ...
    NativeOpsHolder.getInstance()
        ...
        .memcpyAsync(
            ptr,
            temp,
            length * Nd4j.sizeOfDataType(dtype),
            CudaConstants.cudaMemcpyHostToDevice,
            stream);
    stream.synchronize();
  ...
  protected void initHostPointerAndIndexer() {
    ...
    if (allocationPoint.getHostPointer() == null) {
      ...
      if (parentWorkspace == null) {
        ...
        val ptr =
            parentWorkspace.alloc(
                this.length * this.elementSize, MemoryKind.HOST, this.dataType(), false);
  ...
  protected void initPointers(long length, int elementSize, boolean initialize) {
    ...
    if (initialize) {
      ...
      NativeOpsHolder.getInstance()
          .getDeviceNativeOps()
          .memsetAsync(devicePtr, 0, length * elementSize, 0, ctx.getSpecialStream());
  ...
  public BaseCudaDataBuffer(
      long length, int elementSize, boolean initialize, @NonNull MemoryWorkspace workspace) {
    this.allocationMode = AllocationMode.MIXED_DATA_TYPES;
    initTypeAndSize();
  ...
  public BaseCudaDataBuffer(@NonNull DataBuffer underlyingBuffer, long length, long offset) {
    ...
    this.originalBuffer =
        underlyingBuffer.originalDataBuffer() == null
            ? underlyingBuffer
    ...
    ptrDataBuffer =
        ((BaseCudaDataBuffer) underlyingBuffer)
            .ptrDataBuffer.createView(
                length * underlyingBuffer.getElementSize(),
  ...
  public BaseCudaDataBuffer(long[] data, boolean copy) {
    ...
    if (copy) set(data, data.length, 0, 0);
  }
  ...
  public long address() {
    ...
    if (allocationPoint.getHostPointer() == null) return -1;
  ...
  public void assign(DataBuffer data) {
    ...
    // not clear why. We can see this when Nd4j.createBuffer (which calls getBuffer followed by
    // setData)
    // TODO: it could have something to do with different setData implementations which are fairly
    // new.
  ...
  public void read(DataInputStream s) {
    try {
      ...
      if (savedMode.ordinal() < 3) locLength = s.readInt();
      else locLength = s.readLong();
      ...
      this.allocationPoint =
          AtomicAllocator.getInstance()
              .allocateMemory(this, new AllocationShape(length, elementSize, t), false);
      ...
      switch (type) {
        case DOUBLE:
          {
            this.pointer =
                new CudaPointer(allocationPoint.getHostPointer(), length).asDoublePointer();
            indexer = DoubleIndexer.create((DoublePointer) pointer);
          }
          break;
        case FLOAT:
          {
            this.pointer =
                new CudaPointer(allocationPoint.getHostPointer(), length).asFloatPointer();
            indexer = FloatIndexer.create((FloatPointer) pointer);
          }
          break;
        case HALF:
          {
            this.pointer =
                new CudaPointer(allocationPoint.getHostPointer(), length).asShortPointer();
            indexer = HalfIndexer.create((ShortPointer) pointer);
          }
          break;
        case LONG:
          {
            this.pointer =
                new CudaPointer(allocationPoint.getHostPointer(), length).asLongPointer();
            indexer = LongIndexer.create((LongPointer) pointer);
          }
          break;
        case INT:
        ...
        case UBYTE:
          {
            this.pointer =
                new CudaPointer(allocationPoint.getHostPointer(), length).asBytePointer();
            indexer = UByteIndexer.create((BytePointer) pointer);
          }
          break;
        case BYTE:
        ...
        case BOOL:
          {
            this.pointer =
                new CudaPointer(allocationPoint.getHostPointer(), length).asBooleanPointer();
            indexer = BooleanIndexer.create((BooleanPointer) pointer);
          }
          break;
        default:
  ...
  public DataBuffer dup() {
    ...
    allocator.memcpyBlocking(
        buffer,
        new CudaPointer(allocator.getHostPointer(this).address()),
        this.length * elementSize,
        0);
    return buffer;
  }
  ...
  public void actualizePointerAndIndexer() {
    ...
    if (cptr != null && pointer != null && cptr.address() == pointer.address()) return;
  ...
  public DataBuffer reallocate(long length) {
    ...
    if (isAttached()) {
      ...
      if (oldDevicePointer != null && oldDevicePointer.address() != 0) {
        ...
        NativeOpsHolder.getInstance()
            .getDeviceNativeOps()
            .memcpySync(nPtr, oldDevicePointer, length * getElementSize(), 3, null);
  ...
  public long capacity() {
    if (allocationPoint.getHostPointer() != null) return pointer.capacity();
    else return length;
  }
  ...
  public Deallocator deallocator() {
    if (deallocator != null) return deallocator;
