...
public class CublasPointer implements AutoCloseable {
  ...
  public CublasPointer(JCudaBuffer buffer, CudaContext context) {
    this.buffer = buffer;
    //        this.devicePointer = AtomicAllocator.getInstance().getPointer(new
    // Pointer(buffer.originalDataBuffer() == null ? buffer : buffer.originalDataBuffer()),
    // AllocationUtils.buildAllocationShape(buffer), true);
   ...
   *
   * <p>This wrapper makes sure that the INDArray offset, stride and memory pointers are accurate to
   * the data being copied to and from the device.
   *
   * <p>If the copyToHost function is used in this class, the host buffer offset and data length is
  ...
  /**
   * Whether this is a result pointer or not A result pointer means that this pointer should not
   * automatically be freed but instead wait for results to accumulate so they can be returned from
   * the gpu first
   *
   * @return
   */
  ...
  /**
   * Sets whether this is a result pointer or not A result pointer means that this pointer should
   * not automatically be freed but instead wait for results to accumulate so they can be returned
   * from the gpu first
   *
   * @return
   */
