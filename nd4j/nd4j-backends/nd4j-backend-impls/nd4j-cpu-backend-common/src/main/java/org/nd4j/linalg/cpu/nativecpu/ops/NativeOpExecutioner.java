...
public class NativeOpExecutioner extends DefaultOpExecutioner {
  ...
  /**
   * Instead of allocating new memory chunks for each batch invocation, we reuse them on
   * thread/opNum basis Since for NativeOpExecutioner all executions are synchronous
   */
  ...
  public INDArray exec(IndexAccumulation op, OpContext oc) {
    ...
    if (x.isEmpty()) {
      for (val d : dimension) {
        Preconditions.checkArgument(
            x.size(d) != 0, "IndexReduce can't be issued along axis with 0 in shape");
      }
    }
    ...
    if (z.isScalar()) {
      loop.execIndexReduceScalar(
          ...
          zb,
          (LongPointer) z.shapeInfoDataBuffer().addressPointer(),
          null);
    } else {
      loop.execIndexReduce(
          ...
          (LongPointer) z.shapeInfoDataBuffer().addressPointer(),
          null,
          ((BaseCpuDataBuffer) op.dimensions().castTo(DataType.LONG).data()).getOpaqueDataBuffer(),
  ...
  public INDArray exec(ReduceOp op, OpContext oc) {
    ...
    val dimension =
        Shape.normalizeAxis(
            x.rank(), op.dimensions() != null ? op.dimensions().toLongVector() : null);
    if (extraz.get() == null) extraz.set(new PointerPointer(32));
    ...
    if (x.isVector()
        && x.length() == ArrayUtil.prod(retShape)
    ...
    /** This is the result array. We create it only if we hadn't provided it before */
    INDArray ret;
    if (z == null || z == x) {
    ...
    /**
     * Returns the {@link Shape#createShapeInformation(int[], int[], int, int, char)} and the
     * associated offsets for each {@link INDArray#tensorAlongDimension(int, int...)} The first item
     * is the shape information. The second one is the offsets.
     */
    Pair<DataBuffer, DataBuffer> tadBuffers =
        x.isEmpty() ? Pair.makePair(x.data(), null) : tadManager.getTADOnlyShapeInfo(x, dimension);
    Pair<DataBuffer, DataBuffer> yTadBuffers = null;

    /**
     * Note because dimension arrays don't change, we use an {@link ConstantHandler} which knows how
     * to reserve memory for immutable buffers for the dimensions. This gives us a pointer which is
     * passed around in libnd4j.
     */
    val xb = x.data() != null ? x.data().opaqueBuffer() : null;
    val zb = z.data() != null ? z.data().opaqueBuffer() : null;
    if (op instanceof Variance) {
      if (ret.isScalar()) {
        loop.execSummaryStatsScalar(
            null,
            op.opNum(),
            xb,
            (LongPointer) x.shapeInfoDataBuffer().addressPointer(),
      ...
      if (op.isComplexAccumulation()) {
        try {
          loop.execReduce3All(
              ...
              getPointerForExtraArgs(op, z.dataType()),
              yb,
              (LongPointer) y.shapeInfoDataBuffer().addressPointer(),
              null,
              zb,
              (LongPointer) z.shapeInfoDataBuffer().addressPointer(),
              null,
        ...
        switch (op.getOpType()) {
          ...
          default:
            throw new UnsupportedOperationException(
                "Unsupported op used in reduce: " + op.getOpType());
        }
      } else {
        switch (op.getOpType()) {
          case REDUCE_FLOAT:
            loop.execReduceFloat2(
                null,
                op.opNum(),
                xb,
                (LongPointer) x.shapeInfoDataBuffer().addressPointer(),
  ...
  private void invokeScalarAlongDimension(ScalarOp op, OpContext oc) {
     ...
     * is the shape information. The second one is the offsets.
     *
     * <p>Note that this is the *result* TAD information. An op is always input (x) and output (z)
     * for result. This is for assigning the result to of the operation along the proper dimension.
     */
    ...
    switch (op.getOpType()) {
      case SCALAR:
        loop.execScalar(
            ...
            op.opNum(),
            x,
            (LongPointer) getX(op, oc).shapeInfoDataBuffer().addressPointer(),
            null,
            z,
            (LongPointer) getZ(op, oc).shapeInfoDataBuffer().addressPointer(),
            null,
            scalar,
  ...
  private Pointer getPointerForExtraArgs(Op op, DataType type) {
    if (op.extraArgs() != null) {
      ...
      if (eadb != null) return eadb.addressPointer();
      else return null;
    }
  ...
  private void exec(TransformOp op, OpContext oc) {
    ...
    if (op instanceof Assign) {
      ...
      /**
       * This is the {@link IsMax} operation.
       *
       * @see {@link Op#extraArgs()} for what an extra argument is in an op.
       *     <p>The extra argument in the op here is the {@link IsMax#IsMax(INDArray, int...)}
       *     dimension to do the ismax along
       */
      if (op.opName().equalsIgnoreCase("ismax")
          && op.extraArgs() != null
          && op.extraArgs().length > 0) {
        ...
        /**
         * Returns the {@link Shape#createShapeInformation(int[], int[], int, int, char)} and the
         * associated offsets for each {@link INDArray#tensorAlongDimension(int, int...)} The first
         * item is the shape information. The second one is the offsets.
         */
      ...
      } else st = profilingConfigurableHookIn(op);
      ...
      if (y != null) {
        ...
        switch (op.getOpType()) {
          ...
          case TRANSFORM_SAME:
            if (!experimentalMode.get())
              Preconditions.checkArgument(
                  x.dataType() == y.dataType() || y.dataType() == DataType.BOOL,
                  "Op.X and Op.Y must have the same data type, but got %s vs. %s",
                  x.dataType(),
                  y.dataType());
          ...
          case TRANSFORM_BOOL:
            ...
            break;
          case PAIRWISE_BOOL:
            loop.execPairwiseTransformBool(
                dummy,
                op.opNum(),
                xb,
                (LongPointer) x.shapeInfoDataBuffer().addressPointer(),
        ...
        if (z == null) {
          setZ(
              Nd4j.createUninitialized(
                  (oc != null ? op.resultType(oc) : op.resultType()), x.shape()),
              op,
              oc);
          z = getZ(op, oc);
        }
        ...
        switch (op.getOpType()) {
          ...
          default:
            throw new UnsupportedOperationException(
                "Unknown transform type: [" + op.getOpType() + "]");
        }
      }
  ...
  public INDArray exec(BroadcastOp op, OpContext oc) {
    ...
    PointerPointer dummy =
        extraz.get().put(hostTadShapeInfo, hostTadOffsets, devTadShapeInfoZ, devTadOffsetsZ);
  ...
  protected <T extends Aggregate> Pointer getPointer(Batch<T> batch) {
    if (batchPointers.get() == null) batchPointers.set(new HashMap<>());
  ...
  public <T extends Aggregate> void exec(Batch<T> batch) {
    ...
    int realPos =
        (intArraysPos + (maxIntArrays * maxArraySize * Batch.getBatchLimit()))
            / (Nd4j.dataType() == DataType.DOUBLE ? 2 : 1);
    int argsPos =
        (realPos + ((batch.getSample().maxRealArguments() * Batch.getBatchLimit())))
    ...
    for (int i = 0; i < batch.getNumAggregates(); i++) {
      ...
      if (i == 0) dataType = op.getArguments().get(0).dataType();
    ...
    loop.execAggregateBatch(
        null,
        batch.getNumAggregates(),
        batch.opNum(),
        ...
        pointer,
        FlatBuffersMapper.getDataTypeAsByte(dataType));
  ...
  /**
   * This method takes arbitrary sized list of {@link Aggregate}, and packs them into batches Note
   * here that this is mainly used for random number generation for {@link RandomOp} and things like
   * {@link org.nd4j.linalg.api.rng.distribution.Distribution}
   *
   * @param batch the list of {@link Aggregate} to execute upon
   */
  @Override
   ...
   * {@link org.nd4j.linalg.api.rng.distribution.Distribution}
   *
   * @param op the list of {@link Aggregate} to execute upon
   */
  @Override
  public void exec(Aggregate op) {

    if (memoryBlocks.get() == null) memoryBlocks.set(new HashMap<>());
      ...
      switch (dataType) {
        case FLOAT:
          ((FloatPointer) block.getRealArgumentsPointer())
              .put(x, op.getRealArguments().get(x).floatValue());
          break;
        case DOUBLE:
          ((DoublePointer) block.getRealArgumentsPointer())
              .put(x, op.getRealArguments().get(x).doubleValue());
          break;
        default:
          throw new ND4JIllegalArgumentException("Only FLOAT and DOUBLE datatypes are supported");
      }
    ...
    loop.execAggregate(
        ...
        intArrays,
        numIntArrays,
        block.getRealArgumentsPointer(),
        numRealArguments,
        FlatBuffersMapper.getDataTypeAsByte(dataType));

    if (loop.lastErrorCode() != 0) throw new RuntimeException(loop.lastErrorMessage());
  }
  ...
  public INDArray exec(RandomOp op, OpContext oc, Random rng) {
    ...
    if (!(rng instanceof CpuNativeRandom))
      throw new IllegalStateException(
          "You should use one of NativeRandom classes for NativeOperations execution. Op class: "
              + op.getClass().getName());
  ...
  /**
   * This class holds memory chunks required for single specific Aggregate op. Can be used together
  ...
  private static class AggregateMemoryBlock {
    ...
    private AggregateMemoryBlock(@NonNull Aggregate op) {
      ...
      realArgumentsPointer =
          Nd4j.dataType() == DataType.DOUBLE
              ? new DoublePointer(op.maxRealArguments())
   ...
   *
   * <p>PLEASE NOTE: You're responsible for input/output validation
   *
  ...
  public List<LongShapeDescriptor> calculateOutputShape(@NonNull CustomOp op, OpContext opContext) {
    ...
    val inputArgs =
        opContext != null
                && opContext.getInputArrays() != null
                && !opContext.getInputArrays().isEmpty()
            ? opContext.getInputArrays()
            : op.inputArguments();
    int cnt = 0;
    int numProcessed = 0;
    for (val in : inputArgs) {
      if (!in.isEmpty()) inputBuffers.put(cnt, in.data().addressPointer());
    ...
    if (numProcessed != nIn) {
      throw new ND4JIllegalStateException(
          "Number of processed inputs should match number of inputs. "
              + "Got "
              + numProcessed
              + " inputs but should have been "
              + nIn
              + " . This is likely due a null input.");
    }
    ...
    cnt = 0;
    if (opContext != null) {
      if (iArgs != null) for (val i : opContext.getIArguments()) iArgs.put(cnt++, i);
    } else {
      if (iArgs != null) for (val i : op.iArgs()) iArgs.put(cnt++, i);
    }
    ...
    cnt = 0;
    if (opContext != null) {
      if (bArgs != null) for (val b : opContext.getBArguments()) bArgs.put(cnt++, b);
    } else {
      if (bArgs != null) for (val b : op.bArgs()) bArgs.put(cnt++, b);
    }

    cnt = 0;
    if (opContext != null) {
      if (tArgs != null) for (val b : opContext.getTArguments()) tArgs.put(cnt++, b);
    } else {
      if (tArgs != null) for (val b : op.tArgs()) tArgs.put(cnt++, b);
    }

    cnt = 0;
    if (opContext != null) {
      if (dArgs != null) for (val b : opContext.getDArguments()) dArgs.put(cnt++, b.toInt());
    } else {
      if (dArgs != null) for (val b : op.dArgs()) dArgs.put(cnt++, b.toInt());
    }
    ...
    try {
      ...
      log.error(
          "Failed to calculate output shapes for op {}. Attempted to execute with {} inputs, {}"
              + " outputs, {} targs, {} iargs, {} bargs and {} dargs. {} - Please see above message"
    ...
    if (log.isTraceEnabled()) {
      ...
      DifferentialFunction differentialFunction = (DifferentialFunction) op;
      log.trace(
          "Calculated output shapes for op  of name {} and type {} - {}",
  ...
  public Map<String, INDArray> executeGraph(
    ...
    OpaqueVariablesSet result =
        loop.executeStoredGraph(null, id, ptrBuffers, ptrShapes, ptrIndices, map.size());
    ...
    for (int e = 0; e < loop.getVariablesSetSize(result); e++) {
      ...
      Pointer.memcpy(
          array.data().addressPointer(),
          buffer,
          Shape.lengthOf(shapeOf) * Nd4j.sizeOfDataType(array.dataType()));
  ...
  public void scatterUpdate(
    ...
    loop.scatterUpdate(
        ...
        (int) indices.length(),
        array.data().addressPointer(),
        (LongPointer) tadX.getFirst().addressPointer(),
        (LongPointer) tadX.getSecond().addressPointer(),
        null,
        null,
        null,
        updates.data().addressPointer(),
        (LongPointer) tadY.getFirst().addressPointer(),
        (LongPointer) tadY.getSecond().addressPointer(),
        null,
        null,
        null,
        indices.data().addressPointer(),
        (LongPointer) indices.shapeInfoDataBuffer().addressPointer(),
        null,
        null);
    ...
    try {
      ...
      log.error(
          "Failed to execute op "
              ...
              + " iargs. "
              + sb.toString()
              + " - Please see above message (printed out from c++) for a possible cause of"
              + " error.");
      throw e;
    } finally {
  ...
  public DataBuffer createShapeInfo(
    ...
    try (MemoryWorkspace ws = Nd4j.getMemoryManager().scopeOutOfWorkspaces()) {
      ...
      loop.setShapeBuffer(
          longPointer,
          dtype.toInt(),
          new LongPointer(ret.addressPointer()),
          order,
  ...
  public DataBuffer createShapeInfo(
    ...
    val result =
        new LongBuffer(
            loop.getConstantShapeBufferPrimary(dbf), Shape.shapeInfoLength(shape.length));
  ...
  public TadPack tadShapeInfoAndOffsets(INDArray array, long[] dimension) {
    ...
    try {
      OpaqueTadPack pack =
          loop.tadOnlyShapeInfo(
              array.shapeInfoDataBuffer().opaqueBuffer(),
              new LongPointer(inputDimensions),
              dimension.length);
