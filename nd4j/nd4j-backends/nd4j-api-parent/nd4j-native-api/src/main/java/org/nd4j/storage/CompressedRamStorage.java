...
public class CompressedRamStorage<T extends Object> implements AbstractStorage<T> {
  ...
  public void store(T key, INDArray object) {
    ...
    if (emulateIsAbsent) lock.writeLock().lock();

    compressedEntries.put(key, toStore);

    if (emulateIsAbsent) lock.writeLock().unlock();
  }
  ...
  public INDArray get(T key) {
    try {
      if (emulateIsAbsent) lock.readLock().lock();
      ...
      if (emulateIsAbsent) lock.readLock().unlock();
    }
  }
  ...
  public static class Builder<T> {
    ...
    /**
     * This method defines, which compression algorithm will be used during storage Default value:
     * NoOp();
     *
     * @param compressor
     * @return
    ...
    /**
     * If set to TRUE, all store/update calls will use inplace compression. If set to FALSE,
     * original array won't be modified, and copy will be used.
     *
    ...
    /**
     * If set to TRUE, all Read/Write locks will be used to emulate storeIfAbsent behaviour If set
     * to FALSE, concurrency will be provided by ConcurrentHashMap at Java7 level
     *
     * <p>Default value: FALSE;
     *
     * @param reallyEmulate
