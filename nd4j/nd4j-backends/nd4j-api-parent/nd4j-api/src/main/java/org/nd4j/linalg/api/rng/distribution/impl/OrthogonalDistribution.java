  ...
  public INDArray sample(long[] shape) {
        ...
        Nd4j.getExecutioner()
            .exec(
                new GaussianDistribution(
                    Nd4j.createUninitialized(dtype, flatShape, Nd4j.order()), 0.0, 1.0),
                random);
    ...
    if (gains == null) {
      if (u.rows() >= numRows && u.columns() >= numCols) {
        return u.get(NDArrayIndex.interval(0, numRows), NDArrayIndex.interval(0, numCols))
            .mul(gain)
            .reshape(shape);
      } else {
        return v.get(NDArrayIndex.interval(0, numRows), NDArrayIndex.interval(0, numCols))
            .mul(gain)
            .reshape(shape);
      }
    } else {
