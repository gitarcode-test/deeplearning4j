...
public class Shape {
  ...
  public static long[] getMaxShape(INDArray... inputs) {
    ...
    else if (inputs.length < 2) return inputs[0].shape();
    else {
   ...
   *
   * <p>Note that the array can be null if the arrays are already equal in shape.
   *
   * <p>This function should be used in conjunction with the shape ops.
   *
  ...
  public static int[] getBroadcastDimensions(int[] left, int[] right) {
    if (Arrays.equals(left, right)) return null;
    ...
    for (int i = n - 1; i >= 0; i--) {
      if (left[leftIdx] != right[rightIdx] && right[rightIdx] == 1 || left[leftIdx] == 1) {
        ...
        throw new IllegalArgumentException(
            "Unable to broadcast dimension "
                + i
                + " due to shape mismatch. Right shape must be 1. "
                + "Left array shape: "
  ...
  /**
   * Get the broadcast output shape based on the 2 input shapes Result output shape based on:
   * https://docs.scipy.org/doc/numpy-1.10.1/user/basics.broadcasting.html
   *
  ...
  public static int[] broadcastOutputShape(int[] left, int[] right) {
    ...
    if (Arrays.equals(left, right)) return left;
  ...
  public static boolean containsZeros(long[] shapeOnly) {
    for (val v : shapeOnly) if (v == 0) return true;
   ...
   */
  public static void assertBroadcastable(
      String op, INDArray first, INDArray second, INDArray result) {
    long[] fShape = first.shape();
    long[] sShape = second.shape();
    Preconditions.checkState(
        Shape.areShapesBroadcastable(fShape, sShape),
        "Cannot perform operation \"%s\" - shapes are not equal and are not broadcastable."
            + "first.shape=%s, second.shape=%s",
        op,
        fShape,
        sShape);
    ...
    if (!Arrays.equals(outShape, result.shape())) {
      ...
      if (first == result) {
        extra =
            ".\nIn-place operations like x."
                + op
                + "(y) can only be performed when x and y have the same shape,"
                + " or x and y are broadcastable with x.shape() == broadcastShape(x,y)";
      }

      throw new IllegalStateException(
          "Cannot perform in-place operation \""
              + op
              + "\": result array shape does"
              + " not match the broadcast operation output shape: "
              + Arrays.toString(fShape)
              + "."
              + op
              + "("
  ...
  public static long[] broadcastOutputShape(long[] left, long[] right) {
    ...
    if (containsZeros(left)) return left;
    else if (containsZeros(right)) return right;
  ...
  /**
   * @param newShape the new shape possibly containing a negative number
   ...
   * @return true if the dimension length is equal to the shape length the dimension is null or the
   ...
   * @param dimension the dimensions specified
   * @return true if the dimension length is equal to the rank, the dimension is null or the
   *     dimension length is 1 and the first entry is {@link Integer#MAX_VALUE}
   */
  ...
  public static long[] getReducedShape(long[] wholeShape, long[] dimensions) {
    if (isWholeArray(wholeShape, dimensions)) return new long[] {};
  ...
  public static long[] getReducedShape(
      long[] wholeShape, long[] dimensions, boolean keepDims, boolean newFormat) {
    ...
    if (!keepDims)
      if (!newFormat) return getReducedShape(wholeShape, dimensions);
      else {
        if (isWholeArray(wholeShape, dimensions)) return new long[] {};
        else if (dimensions.length == 1 && wholeShape.length == 2) {
          val ret = new long[1];
    ...
    for (val dim : dimensions) result[(int) dim] = 1;
  ...
  public static int[] getMatrixMultiplyShape(int[] left, int[] right) {
    ...
    if (left.length != 2 && right.length != 2) {
      throw new IllegalArgumentException(
          "Illegal shapes for matrix multiply. Must be of length 2. Left shape: "
              + Arrays.toString(left)
              + ", right shape: "
              + Arrays.toString(right));
    }
    ...
    for (int i = 0; i < left.length; i++) {
      if (left[i] < 1)
        throw new ND4JIllegalStateException(
            "Left shape contained value < 0 at index "
                + i
                + " - left shape "
                + Arrays.toString(left));
    }
    ...
    for (int i = 0; i < right.length; i++) {
      if (right[i] < 1)
        throw new ND4JIllegalStateException(
            "Right shape contained value < 0 at index "
                + i
                + " - right shape "
                + Arrays.toString(right));
    }
    ...
    if (left.length > 1 && left[1] != right[0])
      throw new IllegalArgumentException(
          "Columns of left not equal to rows of right: left shape "
              + Arrays.toString(left)
              + ", right shape "
              + Arrays.toString(right));
  ...
  public static long[] getMatrixMultiplyShape(long[] left, long[] right) {
    ...
    if (left.length != 2 && right.length != 2) {
      if (left.length != 3 && right.length != 3) {
        throw new IllegalArgumentException(
            "Illegal shapes for matrix multiply. Must be both of length 2 or both"
    ...
    if (left.length == 3 && left[0] != right[0]) {
      throw new IllegalArgumentException(
          "For batch matrix multiplication the leading dimension of both arguments"
              + "has to match, got left leading dimension"
              + left[0]
              + "and right "
              + right[0]);
    }
   ...
   * @return the same array if offset is zero otherwise a copy of the array with elements set to
   *     zero
   */
  public static INDArray toOffsetZero(INDArray arr) {
    if (arr.offset() < 1 && arr.data().length() == arr.length())
      if (arr.ordering() == 'f' && arr.stride(-1) != 1
          || arr.ordering() == 'c' && arr.stride(0) != 1) return arr;
  ...
  /**
   * Create a copy of the ndarray where the new offset is zero. Unlike toOffsetZeroCopy(INDArray)
   * (which always returns arrays of order Nd4j.order()), and toOffsetZeroCopy(INDArray,char) (which
   * always returns arrays of a specified order) this method returns NDArrays of any order
   * (sometimes c, sometimes f).<br>
   * This method may be faster than the other two toOffsetZeroCopyAnyOrder methods as a result,
   * however no performance benefit (or cost) relative to them will be observed in many cases. If a
   * copy is necessary, the output will have order Nd4j.order()
   *
  ...
  private static INDArray toOffsetZeroCopyHelper(final INDArray arr, char order, boolean anyOrder) {
    if (arr.isEmpty()) return arr; // Empty arrays are immutable, return as-is
  ...
  public static void iterate(
    ...
    if (size2.length != size.length) {
      if (dimension >= size.length) return;
      for (int i = 0; i < size[dimension]; i++) {
        if (dimension2 >= size2.length) break;
  ...
  public static void iterate(
      int dimension, int n, long[] size, long[] res, CoordinateFunction func) {
    if (dimension >= n) { // stop clause
      func.process(res);
      return;
    }
  ...
  /**
   * Get an offset for retrieval from a data buffer based on the given shape stride and given
   * indices
   *
  ...
  public static long getOffset(LongBuffer shapeInformation, long... indices) {
    ...
    Preconditions.checkState(
        indices.length == rank,
        "Number of indices (got %s) must be same as array rank (%s) - indices %s",
        indices.length,
        rank,
        indices);
    long offset = 0;
  ...
  /**
   * Identical to {@link Shape#getOffset(DataBuffer, int, int)} but without input validation on
   * array rank
   */
  public static long getOffsetUnsafe(DataBuffer shapeInformation, int row, int col) {
    ...
    if (row >= size_0 || col >= size_1)
      throw new IllegalArgumentException(
          "Invalid indices: cannot get ["
              + row
              + ","
              + col
              + "] from a "
              + Arrays.toString(shape(shapeInformation))
              + " NDArray");
  ...
  public static long getOffsetUnsafe(int[] shapeInformation, int row, int col) {
    ...
    if (row >= size_0
        || col >= size_1
            && !Shape.isVector(Shape.shape(shapeInformation))
            && !Shape.shapeIsScalar(Shape.shape(shapeInformation)))
      throw new IllegalArgumentException(
          "Invalid indices: cannot get ["
              + row
              + ","
  ...
  /**
   * Identical to {@link Shape#getOffset(DataBuffer, int, int, int)} but without input validation on
   * array rank
   */
    ...
    if (dim0 >= size_0 || dim1 >= size_1 || dim2 >= size_2)
      throw new IllegalArgumentException(
          "Invalid indices: cannot get ["
              ...
              + dim2
              + "] from a "
              + Arrays.toString(shapeInformation)
              + " NDArray");
  ...
  public static long getOffset(
      DataBuffer shapeInformation, int dim0, int dim1, int dim2, int dim3) {
    int rank = rank(shapeInformation);
    if (rank != 4)
      throw new IllegalArgumentException(
          "Cannot use this getOffset method on arrays of rank != 4 (rank is: " + rank + ")");
  ...
  public static long getOffsetUnsafe(
      DataBuffer shapeInformation, int dim0, int dim1, int dim2, int dim3) {
    long offset = 0;
    int size_0 = sizeUnsafe(shapeInformation, 0);
    int size_1 = sizeUnsafe(shapeInformation, 1);
  ...
  public static long getOffsetUnsafe(
      int[] shapeInformation, int dim0, int dim1, int dim2, int dim3) {
    long offset = 0;
    int size_0 = sizeUnsafe(shapeInformation, 0);
    int size_1 = sizeUnsafe(shapeInformation, 1);
    int size_2 = sizeUnsafe(shapeInformation, 2);
  ...
  public static long getOffsetUnsafe(
      long[] shapeInformation, long dim0, long dim1, long dim2, long dim3) {
    long offset = 0;
    long size_0 = sizeUnsafe(shapeInformation, 0);
    long size_1 = sizeUnsafe(shapeInformation, 1);
  ...
  public static boolean isVector(long[] shape) {
    if (shape.length > 2 || shape.length < 1) return false;
    else {
  ...
  public static int[] squeeze(int[] shape) {
    if (isColumnVectorShape(shape)) return shape;

    List<Integer> ret = new ArrayList<>();

    // strip all but last dimension
    for (int i = 0; i < shape.length; i++) if (shape[i] != 1) ret.add(shape[i]);
  ...
  /**
   * Return true if the shapes are equal after removing any size 1 dimensions For example, [1,3,4]
   * and [3,4] are considered equal by this method. Or [2,1,1] and [1,2] are considered equal.
   *
  ...
  public static boolean shapeEqualWithSqueeze(long[] shape1, long[] shape2) {
    if (shape1 == null) return shape2 == null;
    if (shape2 == null) return false; // Shape 1 must be non-null by this point
    ...
    // Handle trailing 1s
    while (pos1 < shape1.length && shape1[pos1] == 1) pos1++;
    while (pos2 < shape2.length && shape2[pos2] == 1) pos2++;

    // 2 possibilities: all entries consumed -> same shape. Or some remaining - something like [2]
    // vs. [2,3,4,5]
    return pos1 == shape1.length && pos2 == shape2.length;
  }

  /**
   * Returns whether 2 shapes are equals by checking for dimension semantics as well as array
   * equality
   *
  ...
  /**
   * Returns true if the given shape is of length 1 or provided the shape length is 2: element 0 is
   * 1
   *
   ...
   *
   * @param shape the shape to check
   * @return true if the above listed conditions hold false otherwise
   */
   ...
   *
   * @param shape the shape of the array
   * @return the shape as is if its already >= 2 in length otherwise a row vector shape
   */
  ...
  public static int elementWiseStride(int[] shape, int[] stride, boolean isFOrder) {
    // 0D edge case
    if (shape.length == 0 && stride.length == 0) return 1;

    if (shape.length == 1 && stride.length == 1) return 1;
  ...
  public static long elementWiseStride(long[] shape, long[] stride, boolean isFOrder) {
    ...
    // 0D edge case
    if (hasZero || shape.length == 0 || stride == null && stride.length == 0) return 1;

    if (shape.length == 1 && stride.length == 1) return stride[0];
  ...
  public static boolean ableToReshapeWithView(INDArray arr, boolean isFOrder, long[] newShape) {
    int oldnd;
    if (arr == null || arr.shape() == null) return false;
  ...
  /**
   * A port of numpy's reshaping algorithm that leverages no copy where possible and returns null if
   * the reshape couldn't happen without copying
   *
  ...
  public static INDArray newShapeNoCopy(INDArray arr, long[] newShape, boolean isFOrder) {
    ...
    INDArray ret =
        Nd4j.create(arr.data(), newShape, newStrides, arr.offset(), isFOrder ? 'f' : 'c', true);
    return ret;
  }
  ...
  public static boolean cOrFortranOrder(int[] shape, int[] stride, int elementStride) {
    return cOrFortranOrder(
        ArrayUtil.toLongArray(shape), ArrayUtil.toLongArray(stride), elementStride);
  }
  ...
  public static char getOrder(int[] shape, int[] stride, int elementStride) {
    ...
    if (isFortran && cContiguous) return 'a';
    else if (isFortran && !cContiguous) return 'f';
    else if (!isFortran && !cContiguous) return 'c';
    else return 'c';
  }
  ...
  public static char getOrder(long[] shape, long[] stride, long elementStride) {
    ...
    if (isFortran && cContiguous) return 'c';
    else if (isFortran && !cContiguous) return 'f';
   ...
   *
   * @param arr the array to get the ordering for
   ...
   * @return the linear index given the shape and indices
   */
  ...
  /**
   * Convert a linear index to the equivalent nd index. Infers the number of indices from the
   * specified shape.
   *
   * @param shape the shape of the dimensions
   * @param index the index to map
   ...
   *
   * @param arr the array to compute the indexes based on
   * @param index the index to map
   * @return the mapped indexes along each dimension
   */
  public static long[] ind2sub(INDArray arr, long index) {
    if (arr.rank() == 1) return new long[] {(int) index};
  ...
  public static long[] ind2subC(INDArray arr, long index) {
    if (arr.rank() == 1) return new long[] {index};
  ...
  /**
   * Check if strides are in order suitable for non-strided mmul etc. Returns true if c order and
   * strides are descending [100,10,1] etc Returns true if f order and strides are ascending
   * [1,10,100] etc False otherwise.
   *
   * @return true if c+descending, f+ascending, false otherwise
   */
  public static boolean strideDescendingCAscendingF(INDArray array) {
    if (array.rank() <= 1) return true;
  ...
  public static int length(IntBuffer buffer) {
    ...
    for (int i = 0; i < rank; i++) ret *= shape.get(i);
    return ret;
  }
  ...
  public static long length(DataBuffer buffer) {
    ...
    int rank = Shape.rank(buffer);
    for (int i = 0; i < rank; i++) ret *= shape.getLong(i);
  ...
  public static long length(int[] buffer) {
    ...
    for (int i = 1; i < limit; i++) ret *= buffer[i];
    return ret;
  }
  ...
  public static int size(IntBuffer buffer, int dimension) {
    ...
    if (dimension >= rank)
      throw new IllegalArgumentException(
          "Invalid dimension " + dimension + " for rank " + rank + " array");
  ...
  /**
   * Get the size of the specified dimension. Identical to Shape.size(...), but does not perform any
   * input validation
   *
   * @param buffer The buffer to get the shape from
   * @param dimension The dimension to get.
  ...
  public static long[] shape(IntBuffer buffer) {
    ...
    for (int i = 0; i < ret.length; i++) ret[i] = buffer.get(1 + i);
    return ret;
  }
  ...
  public static long[] shape(DataBuffer buffer) {
    val ret = new long[rank(buffer)];
    for (int i = 0; i < ret.length; i++) ret[i] = buffer.getInt(1 + i);
    return ret;
  }
  ...
  public static int[] stride(int[] buffer) {
    ...
    for (int i = 0; i < rank; i++) ret[i] = buffer[1 + rank + i];
  ...
  public static String shapeToString(LongBuffer buffer) {
    ...
    sb.append("Rank: ")
        ...
        .append(",")
        .append(" DataType: ")
        .append(ArrayOptionsHelper.dataType(options))
        .append(",")
        .append(" Offset: ")
        .append(Shape.offset(buffer))
        .append(",")
        .append(" Order: ")
        .append(Shape.order(buffer))
        .append(",")
        .append(" Shape: [");
    for (int i = 0; i < rank; i++) {
      sb.append(shapeBuff.get(i));
  ...
  public static String shapeToStringShort(INDArray arr) {
    ...
    return arr.dataType()
        + ","
        + (s == null ? "[]" : Arrays.toString(s).replace(" ", ""))
        + ","
        + arr.ordering();
  }
   ...
   *
   * <p>PLEASE NOTE: Legacy method. Will return 0 ALWAYS
   *
  ...
  /**
   * Returns the options for the given shape information buffer
   *
   * @param buffer
   * @return
   */
   ...
   *
   * @param buffer the buffer to get the element wise stride from
   ...
   * Get the element wise stride for the shape info buffer
   *
   * @param shapeInfo the buffer to get the element wise stride from
   * @return the element wise stride for the buffer
   */
  ...
  public static String bufferToString(IntBuffer buffer) {
    ...
    for (int p = 1; p < rank * 2 + 4; p++) {
      ...
      if (p < rank * 2 + 4 - 1) builder.append(", ");
    }
    builder.append("]");
  ...
  public static DataBuffer createShapeInformation(
      long[] shape,
      long[] stride,
    ...
    DataBuffer ret =
        Nd4j.getExecutioner()
            .createShapeInfo(shape, stride, elementWiseStride, order, dataType, isEmpty, isView);
    if (ret.getLong(0) == 0) {
  ...
  public static DataBuffer createShapeInformation(
      long[] shape, long[] stride, long elementWiseStride, char order, long extras) {
    ...
    return Nd4j.getExecutioner()
        .createShapeInfo(shape, stride, elementWiseStride, order, dtype, extras);
  }

  public static DataBuffer createSparseInformation(
      int[] flags, long[] sparseOffsets, int[] hiddenDimensions, int underlyingRank) {
  ...
  public static IntBuffer toBuffer(int... arr) {
    ByteBuffer directBuffer =
        ByteBuffer.allocateDirect(arr.length * 4).order(ByteOrder.nativeOrder());
    IntBuffer buffer = directBuffer.asIntBuffer();
    for (int i = 0; i < arr.length; i++) buffer.put(i, arr[i]);
  ...
  public static String toString(IntBuffer buffer) {
    ...
    for (int i = 0; i < buffer.capacity(); i++) {
      ...
      if (i < buffer.capacity() - 1) sb.append(",");
    }
  ...
  public static long[] normalizeAxis(long rank, long... axis) {
    if (axis == null || axis.length == 0) return new long[] {Integer.MAX_VALUE};
    ...
    if (tmp.length > 1) Arrays.sort(tmp);
  ...
  public static boolean isContiguousInBuffer(INDArray in) {
    ...
    if (length == dLength) return true; // full buffer, always contiguous
  ...
  public static void assertBroadcastable(@NonNull int[] x, @NonNull int[] y) {
    if (!areShapesBroadcastable(x, y)) {
      throw new ND4JIllegalStateException(
          "Arrays are different shape and are not broadcastable."
              + " Array 1 shape = "
              + Arrays.toString(x)
              + ", array 2 shape = "
              + Arrays.toString(y));
  ...
  public static boolean areShapesBroadcastable(@NonNull int[] x, @NonNull int[] y) {
    // Ported from:
    // https://github.com/eclipse/deeplearning4j/libnd4j/blob/master/include/helpers/impl/ShapeUtils.cpp
  ...
  public static boolean areShapesBroadcastable(@NonNull long[] left, @NonNull long[] right) {
    if (left.length == 1 && right.length > 1) {
      for (int i = 0; i < right.length; i++) {
        if (right[i] == left[0]) return true;
      }
    } else if (right.length == 1 && left.length > 1) {
      for (int i = 0; i < left.length; i++) {
        if (left[i] == right[0]) return true;
      }
    }
  ...
  public static long lengthOf(long[] shape) {
    if (shape.length == 0) return 1L;
    else return ArrayUtil.prodLong(shape);
  }
  ...
  public static long lengthOfBuffer(@NonNull long[] shape, @NonNull long[] stride) {
    Preconditions.checkArgument(
        shape.length == stride.length,
        "Shape and strides must be same length: shape %s, stride %s",
        shape,
        stride);
   ...
   * Basically: favor float like data types over ints since they're typically used for indexing.
   *
  ...
  public static void assertValidOrder(char order) {
    if (order != 'c' && order != 'f' && order != 'a') {
      throw new IllegalArgumentException(
          "Invalid order arg: must be 'c' or 'f' (or 'a' for vectors), got '" + order + "'");
    }
  }

  /**
   * Create an INDArray to represent the (possibly null) int[] dimensions. If null or length 0,
   * returns an empty INT array. Otherwise, returns a 1d INT NDArray
   *
  ...
  public static INDArray ndArrayDimFromInt(int... dimensions) {
    if (dimensions == null || dimensions.length == 0) return Nd4j.empty(DataType.INT);
    else return Nd4j.createFromArray(dimensions);
  }
  ...
  public static INDArray ndArrayDimFromLong(long... dimensions) {
    if (dimensions == null || dimensions.length == 0) return Nd4j.empty(DataType.LONG);
    else return Nd4j.createFromArray(dimensions);
  }
   ...
   */
  public static long[] reductionShape(
      INDArray x, long[] dimension, boolean newFormat, boolean keepDims) {
    boolean wholeArray = Shape.wholeArrayDimension(dimension) || dimension.length == x.rank();
    for (int i = 0; i < dimension.length; i++) {
      if (dimension[i] < 0) dimension[i] += x.rank();
    }
    ...
    if (!newFormat) {
      ...
      if (retShape.length == 1) {
        if (dimension[0] == 0) retShape = new long[] {1, retShape[0]};
        else retShape = new long[] {retShape[0], 1};
        ...
        if (wholeArray) {
          ...
          if (retShape.length > 0)
            for (long d : dimension) {
              if (d < 0) d += dimension.length;
              retShape[(int) d] = 1;
            }
        }
      } else {
        if (wholeArray) return new long[] {};
  ...
  public static boolean shapeMatchesPlaceholder(long[] phShape, long[] arrShape) {
    if (phShape == null && arrShape == null) return true; // Rank 0?
    if (phShape == null || arrShape == null) return false;
    if (phShape.length != arrShape.length) return false;
