...
public class ROCMultiClass extends BaseEvaluation<ROCMultiClass> {
  ...
  protected ROCMultiClass(
      int axis, int thresholdSteps, boolean rocRemoveRedundantPts, List<String> labels) {
  ...
  /**
   * @param thresholdSteps Number of threshold steps to use for the ROC calculation. Set to 0 for
   *     exact ROC calculation
   */
    ...
    if (underlying != null) {
      for (int i = 0; i < underlying.length; i++) {
        ...
        sb.append("\n")
            .append(
                String.format(
                    pattern, label, auc, getCountActualPositive(i), getCountActualNegative(i)));
      }

      sb.append("Average AUC: ")
          .append(String.format("%-12." + printPrecision + "f", calculateAverageAUC()));
  ...
  public void eval(
    ...
    Preconditions.checkState(
        maskArray == null, "Per-output masking for ROCMultiClass is not supported");
    ...
    if (underlying.length != labels2d.size(1)) {
      throw new IllegalArgumentException(
          "Cannot evaluate data: number of label classes does not match previous call. "
              + "Got "
              + labels2d.size(1)
              + " labels (from array shape "
              + Arrays.toString(labels2d.shape())
              + ")"
              + " vs. expected number of label classes = "
              + underlying.length);
    }
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (prob.rank() == 0) prob = prob.reshape(1, 1);
      if (label.rank() == 0) label = label.reshape(1, 1);
  ...
  /**
   * Calculate the macro-average (one-vs-all) AUCPR (area under precision recall curve) for all
   * classes
   */
  ...
  /**
   * Merge this ROCMultiClass instance with another. This ROCMultiClass instance is modified, by
   * adding the stats from the other instance.
   *
  ...
  public void merge(ROCMultiClass other) {
    ...
    if (underlying.length != other.underlying.length) {
      throw new UnsupportedOperationException(
          "Cannot merge ROCBinary: this expects "
              + underlying.length
              + "outputs, other expects "
              + other.underlying.length
              + " outputs");
    }
    for (int i = 0; i < underlying.length; i++) {
  ...
  private void assertIndex(int classIdx) {
    ...
    if (classIdx < 0 || classIdx >= underlying.length) {
      throw new IllegalArgumentException(
          "Invalid class index ("
              + classIdx
              + "): must be in range 0 to numClasses = "
              + underlying.length);
    }
  }
  ...
  public double getValue(IMetric metric) {
    if (metric instanceof Metric) {
      if (metric == Metric.AUPRC) return calculateAverageAUCPR();
      else if (metric == Metric.AUROC) return calculateAverageAUC();
