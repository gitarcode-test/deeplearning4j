...
public class AsyncDataSetIterator implements DataSetIterator {
   ...
   */
  public AsyncDataSetIterator(
      DataSetIterator iterator, int queueSize, BlockingQueue<DataSet> queue) {
  ...
  public AsyncDataSetIterator(
      DataSetIterator baseIterator, int queueSize, boolean useWorkspace, Integer deviceId) {
    this(
        baseIterator,
        queueSize,
        new LinkedBlockingQueue<DataSet>(queueSize),
        useWorkspace,
        new DefaultCallback(),
        deviceId);
  }
  ...
  public AsyncDataSetIterator(
      DataSetIterator baseIterator, int queueSize, boolean useWorkspace, DataSetCallback callback) {
    this(
        baseIterator,
        queueSize,
        new LinkedBlockingQueue<DataSet>(queueSize),
  ...
  public AsyncDataSetIterator(
      DataSetIterator iterator, int queueSize, BlockingQueue<DataSet> queue, boolean useWorkspace) {
  ...
  public AsyncDataSetIterator(
    ...
    if (iterator.resetSupported() && !iterator.hasNext()) this.backedIterator.reset();
  ...
  /**
   * Is resetting supported by this DataSetIterator? Many DataSetIterators do support resetting, but
   * some don't
   *
  ...
  /**
   * Does this DataSetIterator support asynchronous prefetching of multiple DataSet objects? Most
   * DataSetIterators do, but in some cases it may not make sense to wrap this iterator in an
   * iterator that does asynchronous prefetching. For example, it would not make sense to use
   * asynchronous prefetching for the following types of iterators: (a) Iterators that store their
   * full contents in memory already (b) Iterators that re-use features/labels arrays (as future
   * next() calls will overwrite past contents) (c) Iterators that already implement some level of
   * asynchronous prefetching (d) Iterators that may return different data depending on when the
   * next() method is called
   *
   * @return true if asynchronous prefetching from this iterator is OK; false if asynchronous
   *     prefetching should not be used with this iterator
   */
  @Override
  ...
  public void reset() {
    ...
    try {
      // Shutdown() should be a synchronous operation since the iterator is reset after shutdown()
      // is
   ...
   *
   * <p>PLEASE NOTE: After shutdown() call, this instance can't be used anymore
   */
  ...
  public boolean hasNext() {
    ...
    try {
      if (hasDepleted.get()) return false;

      if (nextElement != null && nextElement != terminator) {
        return true;
      } else if (nextElement == terminator) return false;
  ...
  public DataSet next() {
    ...
    if (hasDepleted.get()) return null;
  ...
  protected class AsyncPrefetchThread extends Thread implements Runnable {
    ...
    protected AsyncPrefetchThread(
        @NonNull BlockingQueue<DataSet> queue,
    ...
    public void run() {
      ...
      try {
        if (useWorkspace)
          workspace =
              Nd4j.getWorkspaceManager().getWorkspaceForCurrentThread(configuration, workspaceId);
        ...
        while (iterator.hasNext() && shouldWork.get()) {
          ...
          if (useWorkspace) {
            try (MemoryWorkspace ws = workspace.notifyScopeEntered()) {
              ...
              if (callback != null) callback.call(smth);
            }
          } else {
            smth = iterator.next();
