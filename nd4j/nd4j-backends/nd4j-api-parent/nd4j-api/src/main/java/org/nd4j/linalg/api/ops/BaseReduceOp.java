...
public abstract class BaseReduceOp extends BaseOp implements ReduceOp {
  @Setter @Getter protected boolean keepDims = false;
  @Setter @Getter protected boolean isComplex = false;
  @Setter @Getter protected boolean isEmptyReduce = false;
  @Setter @Getter protected SDVariable dimensionVariable;
  private String dimensionVariableName;

  public BaseReduceOp(SameDiff sameDiff, SDVariable i_v, long[] dimensions, boolean keepDims) {
    super(sameDiff, null);
    if (i_v != null) {
      if (dimensions == null || dimensions.length < 1) dimensions = new long[] {Integer.MAX_VALUE};
  ...
  public BaseReduceOp(SameDiff sameDiff, SDVariable i_v, long[] dimensions) {
    this(sameDiff, i_v, dimensions, false);
  }

  public BaseReduceOp(SameDiff sameDiff, SDVariable i_v, SDVariable i_v2, long[] dimensions) {
  ...
  public INDArray noOp() {
    if (z != null && x != z) return z().assign(x.reshape(z.shape()));
    else {
  ...
  public void setPropertiesForFunction(Map<String, Object> properties) {
    ...
    if (properties.containsKey("dimensionVariable")
        && properties.get("dimensionVariable") != null) {
