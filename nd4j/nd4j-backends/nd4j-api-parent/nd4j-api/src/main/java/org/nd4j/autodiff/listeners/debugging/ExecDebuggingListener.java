...
public class ExecDebuggingListener extends BaseListener {
  ...
  public void preOpExecution(SameDiff sd, At at, SameDiffOp op, OpContext opContext) {
    ...
    sb.append("op=").append(stepThisIter++).append(logIter ? ") " : " - ");
    ...
    if (printMode == PrintMode.OPS_ONLY) {
        ...
        if (inputs != null) {
          for (int i = 0; i < inputs.size(); i++) {
            sb.append("\n\tInput[")
                .append(i)
                .append("]=")
                .append(inputs.get(i).shapeInfoToString());
          }
        }
        if (outputs != null) {
          for (int i = 0; i < outputs.size(); i++) {
            sb.append("\n\tOutputs[")
                .append(i)
                .append("]=")
                .append(outputs.get(i).shapeInfoToString());
          }
        }
      } else {
        if (lOp.x() != null) {
      ...
      if (co != null) {
        ...
        if (co.iArgs() != null && co.iArgs().length > 0) {
          sb.append("op.addIArgument(")
              .append(Arrays.toString(co.iArgs()).replaceAll("[\\[\\]]", ""))
              .append(");\n");
        }
        if (co.bArgs() != null && co.bArgs().length > 0) {
          sb.append("op.addBArgument(")
              .append(Arrays.toString(co.bArgs()).replaceAll("[\\[\\]]", ""))
              .append(");\n");
        }
        if (co.tArgs() != null && co.tArgs().length > 0) {
          sb.append("op.addTArgument(")
              .append(Arrays.toString(co.tArgs()).replaceAll("[\\[\\]]", ""))
              .append(");\n");
        }
        val inputs = co.inputArguments();
        val outputs = co.outputArguments();
        if (inputs != null) {
          ...
          for (int i = 0; i < inputs.size(); i++) {
            ...
            sb.append(createString(inputs.get(i))).append(";\n");
          }
        ...
        if (outputs != null) {
          ...
          for (int i = 0; i < outputs.size(); i++) {
            ...
            sb.append(createString(outputs.get(i))).append(";\n");
          }
  ...
  private static String createString(INDArray arr) {
    ...
    if (arr.isEmpty()) {
      ...
      sb.append(").reshape(")
          .append(Arrays.toString(arr.shape()).replaceAll("[\\[\\]]", ""))
          .append(")");
