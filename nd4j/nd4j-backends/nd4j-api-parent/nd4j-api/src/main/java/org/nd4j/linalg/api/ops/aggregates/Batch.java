...
public class Batch<T extends Aggregate> {
  ...
  /**
   * This constructor takes List of Aggregates, and builds Batch instance, usable with Nd4j
   * executioner.
   *
   * @param aggregates
   */
  public Batch(List<T> aggregates) {
    ...
    //    throw new RuntimeException("Number of aggregates is higher then " + batchLimit + "
    // elements, multiple batches should be issued.");
  ...
  /**
   * Helper method to create batch from list of aggregates, for cases when list of aggregates is
   * higher then batchLimit
   *
   * @param list
   * @param <U>
   * @return
  ...
  public static <U extends Aggregate> List<Batch<U>> getBatches(List<U> list, int partitionSize) {
    ...
    if (c == null) throw new ND4JIllegalStateException("Can't infer data type from arguments");
