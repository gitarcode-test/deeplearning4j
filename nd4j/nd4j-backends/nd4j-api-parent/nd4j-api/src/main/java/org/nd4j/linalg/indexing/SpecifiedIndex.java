 ...
 * dimension.
 *
 * <p>Note that using this index will always trigger a copy.
 *
 * <p>Pulling random indices from an array wil not allow a view to be computed. Negative indices can
 * also be specified allowing for dynamic resolution of dimensions/coordinates at runtime.
 *
 * @author Adam Gibson
 */
@Data
...
public class SpecifiedIndex implements INDArrayIndex {
   ...
   *
   * @param indexes the coordinates to iterate over. Each element of the array should be of opType
   *     {@link SpecifiedIndex} otherwise it will end up throwing an exception
  ...
  public static Generator<List<List<Long>>> iterateOverSparse(INDArrayIndex... indexes) {
    Generator<List<List<Long>>> gen =
        Itertools.product(new SparseSpecifiedIndexesGenerator(indexes));
    return gen;
  }

  /**
   * A generator for {@link SpecifiedIndex} for {@link Itertools#product(Generator)} to iterate over
   * an array given a set of iterators
   */
