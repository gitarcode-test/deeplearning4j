...
public class LossUtil {
  ...
  public static void applyMask(INDArray to, INDArray mask) {
    ...
    // These cases have different mask shapes. Per example: column vector. Per output: same shape as
    // score array
    if (mask.isColumnVectorOrScalar()) {
      ...
      throw new IllegalStateException(
          "Invalid mask array: per-example masking should be a column vector, per output masking"
              + " arrays should be the same shape as the labels array. Mask shape: "
