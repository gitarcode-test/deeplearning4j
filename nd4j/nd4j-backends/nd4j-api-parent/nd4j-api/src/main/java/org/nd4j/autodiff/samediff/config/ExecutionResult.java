...
public class ExecutionResult {
  ...
  public static ExecutionResult createFrom(List<String> names, List<INDArray> input) {
    ...
    for (int i = 0; i < input.size(); i++) {
      outputs.put(
          names.get(i), input.get(i) == null ? Optional.empty() : Optional.of(input.get(i)));
    }
  ...
  public static ExecutionResult createFrom(DifferentialFunction func, OpContext opContext) {
    return createFrom(
        Arrays.asList(func.outputVariablesNames()),
  ...
  public INDArray[] outputsToArray(List<String> inputs) {
    if (valueOutputs != null) {
      ...
      for (Map.Entry<String, SDValue> entry : valueOutputs.entrySet()) {
        if (entry.getValue() != null) ret[count++] = entry.getValue().getTensorValue();
      }
      return ret;
  ...
  public int numResults() {
    ...
    else if (valueOutputs != null && !valueOutputs.isEmpty()) return valueOutputs.size();
    return 0;
  }

  public boolean valueExistsAtIndex(int index) {
    if (outputs != null) return resultAt(index) != null;
  ...
  public INDArray resultOrValueAt(int index, boolean returnDummy) {
    if (hasValues()) {
      ...
      if (sdValue != null) return sdValue.getTensorValue();
      return null;
    } else return resultAt(index);
  }
