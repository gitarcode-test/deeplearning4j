...
public abstract class BaseNDArrayList<X extends Number> extends AbstractList<X> {
  ...
  private void moveBackward(int index) {
    ...
    INDArrayIndex[] first =
        new INDArrayIndex[] {NDArrayIndex.point(0), NDArrayIndex.interval(index, index + numMoved)};
    INDArrayIndex[] getRange =
        new INDArrayIndex[] {
          NDArrayIndex.point(0), NDArrayIndex.interval(index + 1, index + 1 + numMoved)
        };
  ...
  private void moveForward(int index) {
    ...
    INDArrayIndex[] first =
        new INDArrayIndex[] {
          NDArrayIndex.point(0), NDArrayIndex.interval(index + 1, index + 1 + get.length())
        };
    container.put(first, get);
  }
}
