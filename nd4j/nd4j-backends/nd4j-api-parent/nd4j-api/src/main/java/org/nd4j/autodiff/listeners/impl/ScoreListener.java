...
public class ScoreListener extends BaseListener {
  ...
  public ScoreListener(int frequency, boolean reportEpochs, boolean reportIterPerformance) {
    Preconditions.checkArgument(
        frequency > 0, "ScoreListener frequency must be > 0, got %s", frequency);
  ...
  public ListenerResponse epochEnd(SameDiff sd, At at, LossCurve lossCurve, long epochTimeMillis) {
    if (reportEpochs) {
      ...
      String etl =
          formatDurationMs(etlTotalTimeEpoch)
              + " ETL time"
              + (etlTotalTimeEpoch > 0 ? "(" + format2dp(pcEtl) + " %)" : "");
      log.info(
          "Epoch {} complete on iteration {} - {} batches ({} examples) in {} - {} batches/sec, {}"
              + " examples/sec, {}",
          at.epoch(),
          at.iteration(),
          epochBatchCount,
          epochExampleCount,
          formatDurationMs(epochTimeMillis),
          format2dp(batchesPerSec),
          format2dp(examplesPerSec),
          etl);
    }
  ...
  public void iterationDone(SameDiff sd, At at, MultiDataSet dataSet, Loss loss) {
    ...
    if (at.iteration() > 0 && at.iteration() % frequency == 0) {
      ...
      if (!reportIterPerformance) {
        log.info(
            "Loss at epoch {}, iteration {}: {}{}", at.epoch(), at.iteration(), format5dp(l), etl);
      } else {
        ...
        if (lastReportTime > 0) {
          ...
          log.info(
              "Loss at epoch {}, iteration {}: {}{}, batches/sec: {}, examples/sec: {}",
              at.epoch(),
              at.iteration(),
              format5dp(l),
              etl,
              format5dp(batchPerSec),
              format5dp(exPerSec));
        } else {
          log.info(
