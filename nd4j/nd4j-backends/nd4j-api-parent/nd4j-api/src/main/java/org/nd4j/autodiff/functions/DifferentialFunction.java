...
public abstract class DifferentialFunction {
  ...
  @Getter @Setter @JsonIgnore protected SameDiff sameDiff;

  @Getter @Setter @JsonIgnore protected boolean inPlace;

  @Getter @Setter @JsonIgnore protected INDArray scalarValue;

  @Getter @Setter @JsonIgnore protected long[] dimensions;

  @JsonIgnore protected Object[] extraArgs;

  @Getter @Setter @JsonIgnore protected String ownName;

  @JsonIgnore @Getter @Setter @Builder.Default protected boolean ownNameSetWithDefault = false;

  @Getter protected StackTraceElement creationLocation, creationPointofOrigin;
  @Getter protected StackTraceElement[] sameDiffCalls;
  @Getter protected StackTraceElement[] creationCallStack;
  ...
  public DifferentialFunction(boolean sameDiff) {
    // Only need instance ID if using function in context of SameDiff, not standard ND4J with
    // INDArray args
    if (sameDiff) {
      setInstanceId();
    }
  ...
  /**
   * Initialize the function from the given {@link NodeDef}
   *
   * @param nodeDef
   */
  public DifferentialFunction(
  ...
  /**
   * Initialize the function from the given {@link Onnx.NodeProto}
   *
   * @param node
   */
  public DifferentialFunction(
  ...
  /**
   * Returns the {@link AttributeAdapter} s for each of the possible ops for import (typically
   * tensorflow and onnx)
   *
   * <p>See {@link AttributeAdapter} for more information on what the adapter does.
   *
   * <p>Similar to {@link #mappingsForFunction()}, the returned map contains a {@link
   * AttributeAdapter} for each field name when one is present. (It is optional for one to exist)_
   *
   * @return
   */
  ...
  /**
   * Returns the mappings for a given function ( for tensorflow and onnx import mapping properties
   * of this function). The mapping is indexed by field name. If the function has no properties,
   * this returned map will be empty.
   *
   * <p>Note that some functions have multiple names. This function returns a map indexed by each
   * alias it has for a given name. These names include both onnx and tensorflow names (which might
   * be 1 or more)
   *
   * @return
   */
  ...
  public Map<String, Object> propertiesForFunction() {
    ...
    Preconditions.checkNotNull(
        fields,
        "DifferentialFunctionClassHolder returned null fields for %s - op has not been added to"
            + " ImportClassMapping?",
        getClass());
  ...
  /**
   * Set the value for this function. Note that if value is null an {@link
   * ND4JIllegalStateException} will be thrown.
   *
  ...
  public void setValueFor(Field target, Object value) {
    ...
    if (isConfigProperties()) {
      ...
      if (propertyName == null) propertyName = "config";
      Field f = null;
      ...
      if (f == null) {
        throw new IllegalStateException(
            "Could not find field \"" + propertyName + "\" for class " + getClass().getName());
      }
      ...
      try {
        ...
        if (o == null) {
          ...
          try {
            ...
            throw new RuntimeException(
                "Error creating new instance of configuration object type " + c.getName(), e);
          }
          f.set(this, o);
        }
        target.set(o, value);
        ...
        throw new RuntimeException(
            "Error setting configuration field \""
                + propertyName
                + "\" for config field \""
                + propertyName
                + "\" on class "
      ...
      try {
        ...
        if (target.getType().equals(Boolean.class)
            || target.getType().equals(boolean.class) && value instanceof Number) {
        ...
        if (target.getType().isEnum()
            && (value instanceof Long
                || value instanceof Integer
                    && !target.getType().equals(int.class)
                    && !target.getType().equals(long.class))) {
        ...
        throw new RuntimeException(
            "Error setting property for function " + getClass().getName(), e);
      }
    }
  }
    ...
    if (!firstClass.equals(valueType)) {
      if (firstClass.isEnum()) {
        if (valueType.equals(String.class)) {
          ...
          throw new IllegalStateException(
              "Could not find enum constant value for value \""
                  + value
                  + "\" for enum class "
                  + firstClass.getName());
        }
  ...
  /**
   * Returns the name of the field to be used for looking up field names. This should be used in
   * conjunction with {@link #isConfigProperties()} to facilitate mapping fields for model import.
   *
   * @return
   */
  ...
  /**
   * Return the output variables for this differential function. Note that this op *may* dynamically
  ...
  public String[] outputVariablesNames() {
    ...
    if (outputVars == null) return new String[0];
  ...
  /**
   * Return the variables expecting gradients. This is usually {@link #args()} but may vary
  ...
  public SDVariable arg(int num) {
    ...
    Preconditions.checkArgument(
        num >= 0 && num < args.length,
        "Invalid index: must be 0 to numArgs (0 <= idx < %s), got %s",
        args.length,
        num);
    return args[num];
  }
   ...
   * @return the differentiated output wrt each input variable
   */
  public List<SDVariable> diff(List<SDVariable> i_v1) {
    List<SDVariable> vals = doDiff(i_v1);
    if (vals == null) {
      throw new IllegalStateException(
          "Error executing diff operation: doDiff returned null for op: " + this.opName());
    }
  ...
  /**
   * Note: DO NOT USE THIS METHOD UNLESS YOU KNOW WHAT YOU ARE DOING. This is only for usage in
   * {@link SameDiff#dynamic(String, List, List, List, List, List, List)}
   */
  public void setInstanceId() {
    if (ownName == null) {
      ownNameSetWithDefault = true;
      if (sameDiff == null) this.ownName = UUID.randomUUID().toString();
      else {
        String n = sameDiff.getOpName(opName());
        this.ownName = n;
      }

      if (sameDiff != null) sameDiff.putOpForId(ownName, this);
    }
  }
  ...
  /**
   * The number of the op (mainly for old legacy XYZ ops like {@link Op})
   *
   * @return
   */
  public int opNum() {
   ...
   * @param graph
   */
  public abstract void initFromTensorFlow(
      NodeDef nodeDef, SameDiff initWith, Map<String, AttrValue> attributesForNode, GraphDef graph);

  /**
   * Iniitialize the function from the given {@link Onnx.NodeProto}
   *
   * @param node
   * @param initWith
   * @param attributesForNode
   * @param graph
   */
  public abstract void initFromOnnx(
      Onnx.NodeProto node,
      SameDiff initWith,
      Map<String, Onnx.AttributeProto> attributesForNode,
      Onnx.GraphProto graph);
  ...
  /**
   * The right argument for this function. Note that this assumes that there are 2 args for this
   * function, if 2 are not set, it throws an {@link ND4JIllegalStateException}
   *
   * @return
   */
  public SDVariable rarg() {
    ...
    if (args == null || args.length != 2)
      throw new ND4JIllegalStateException(
          "In order to use this function, the number of arguments for this function must be 2.");
    return args[1];
  }
  ...
  public List<LongShapeDescriptor> calculateOutputShape() {
    throw new ND4JIllegalStateException(
        "Op type of "
            + getClass().getName()
            + "did not override calculateOutputShape() method leaked out for ["
            + this.opName()
            + "]");
  }
  ...
  public List<LongShapeDescriptor> calculateOutputShape(OpContext oc) {
    throw new ND4JIllegalStateException(
        "Op type of "
            + getClass().getName()
            + " did not override calculateOutputShape(OpContext) method leaked out for ["
            + this.opName()
            + "]");
  }

  /**
   * Calculate the data types for the output arrays. Though datatypes can also be inferred from
   * {@link #calculateOutputShape()}, this method differs in that it does not require the input
   * arrays to be populated. This is important as it allows us to do greedy datatype inference for
   * the entire net - even if arrays are not available.
   *
  ...
  public List<DataType> calculateOutputDataTypes(List<DataType> dataTypes) {
    throw new UnsupportedOperationException(
        "Op type of "
            ...
            + " and name "
            + this.toString()
            + " did not override  calculateOutputDataTypes()! This function has not been"
            + " implemented for "
            + getClass().getName());
  }
