...
public class ExistingMiniBatchDataSetIterator implements DataSetIterator {
  ...
  /**
   * Create with the given root directory, using the default filename pattern {@link
   * #DEFAULT_PATTERN}
   *
  ...
  /**
   * @param rootDir The root directory to use
   * @param pattern The filename pattern to use. Used with {@code String.format(pattern,idx)}, where
   *     idx is an integer, starting at 0.
   */
