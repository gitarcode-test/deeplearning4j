...
public abstract class Pooling3D extends DynamicCustomOp {
  ...
  public Pooling3D(
    ...
    Preconditions.checkState(
        pooling3DConfig.getDD() > 0 && pooling3DConfig.getDH() > 0 && pooling3DConfig.getDW() > 0,
        "Dilation values must all be > 0: got dD/H/W = %s/%s/%s",
        pooling3DConfig.getDD(),
        pooling3DConfig.getDH(),
        pooling3DConfig.getDW());
  ...
  public void setPropertiesForFunction(Map<String, Object> properties) {
    if (config == null) {
      ...
      if (isSameMode != null) builder.isSameMode(isSameMode);
  ...
  protected void addArgs() {
    if (this.iArguments == null) this.iArguments = new ArrayList<>();
  ...
  public List<SDVariable> doDiff(List<SDVariable> f1) {
    ...
    Pooling3DDerivative pooling3DDerivative =
        Pooling3DDerivative.derivativeBuilder()
            .inPlace(inPlace)
            .sameDiff(sameDiff)
  ...
  public String getPoolingPrefix() {
    if (config == null) return "pooling3d";
  ...
  public void initFromTensorFlow(
    ...
    for (int i = 0; i < 3; i++) {
      // TF values here have 5 values: minibatch and Channels at positions 0 and 4, which are almost
      // always 1
