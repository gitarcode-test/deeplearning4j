...
public abstract class AbstractMultiDataSetNormalizer<S extends NormalizerStats>
    extends AbstractNormalizer implements MultiDataNormalization {
  ...
  /**
   * Flag to specify if the labels/outputs in the dataset should be also normalized default value is
   * false
   *
   * @param fitLabels
   */
  ...
  /**
   * Whether normalization for the labels is also enabled. Most commonly used for regression, not
   * classification.
   *
  ...
  public void preProcess(@NonNull MultiDataSet toPreProcess) {
    ...
    for (int i = 0; i < numFeatures; i++) {
      strategy.preProcess(
          toPreProcess.getFeatures(i), toPreProcess.getFeaturesMaskArray(i), getFeatureStats(i));
    }
    if (isFitLabel()) {
      for (int i = 0; i < numLabels; i++) {
        strategy.preProcess(
            toPreProcess.getLabels(i), toPreProcess.getLabelsMaskArray(i), getLabelStats(i));
      }
    }
  }
  ...
  /**
   * Undo (revert) the normalization applied by this normalizer to a specific features array. If
   * labels normalization is disabled (i.e., {@link #isFitLabel()} == false) then this is a no-op.
  ...
  /**
   * Undo (revert) the normalization applied by this DataNormalization instance to the specified
   * labels array. If labels normalization is disabled (i.e., {@link #isFitLabel()} == false) then
   * this is a no-op. Can also be used to undo normalization for network output arrays, in the case
   * of regression.
   *
  ...
  /**
   * Undo (revert) the normalization applied by this normalizer to the labels arrays. If labels
   * normalization is disabled (i.e., {@link #isFitLabel()} == false) then this is a no-op. Can also
   * be used to undo normalization for network output arrays, in the case of regression.
   *
  ...
  /**
   * Undo (revert) the normalization applied by this normalizer to a specific labels array. If
   * labels normalization is disabled (i.e., {@link #isFitLabel()} == false) then this is a no-op.
   * Can also be used to undo normalization for network output arrays, in the case of regression.
   *
   * @param labels Labels arrays to revert the normalization on
