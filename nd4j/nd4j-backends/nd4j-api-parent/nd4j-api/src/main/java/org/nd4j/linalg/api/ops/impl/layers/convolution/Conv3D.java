...
public class Conv3D extends DynamicCustomOp {
  ...
  private static final String INVALID_CONFIGURATION =
      "Invalid Conv3D configuration : sW = %s pH = %s dW = %s ";
  ...
  private void initConfig(Conv3DConfig config) {
    ...
    Preconditions.checkState(
        config.getSW() >= 1 && config.getPH() >= 0 && config.getDW() >= 1,
  ...
  private void addArgs() {
    if (getConfig().getPaddingMode() == null) getConfig().setPaddingMode(PaddingMode.VALID);
    addIArgument(
  ...
  public Map<String, Map<String, PropertyMapping>> mappingsForFunction() {
    ...
    val outputPadding =
        PropertyMapping.builder().propertyNames(new String[] {"aD", "aH", "aW"}).build();

    val biasUsed = PropertyMapping.builder().propertyNames(new String[] {"biasUsed"}).build();

    for (val propertyMapping :
        new PropertyMapping[] {
          kernelMapping,
          strideMapping,
          dilationMapping,
  ...
  public List<SDVariable> doDiff(List<SDVariable> f1) {
    ...
    Conv3DDerivative conv3DDerivative =
        Conv3DDerivative.derivativeBuilder()
            .conv3DConfig(config)
