...
public class LongShapeDescriptor {
  ...
  @Getter private long[] shape;

  @Getter private long[] stride;

  @Getter @Setter private long extras;

  public LongShapeDescriptor(
      long[] shape, long[] stride, long offset, long ews, char order, long extras) {
    if (shape != null) {
  ...
  public static LongShapeDescriptor fromShapeDescriptor(@NonNull ShapeDescriptor descriptor) {
    return new LongShapeDescriptor(
        ArrayUtil.toLongArray(descriptor.getShape()),
        ArrayUtil.toLongArray(descriptor.getStride()),
        descriptor.getOffset(),
        descriptor.getEws(),
        descriptor.getOrder(),
        descriptor.getExtras());
  }
  ...
  public static LongShapeDescriptor fromShape(
      @NonNull long[] shape,
    ...
    if (empty) extras = ArrayOptionsHelper.setOptionBit(extras, ArrayType.EMPTY);
  ...
  public static LongShapeDescriptor fromShape(long[] shape, long extras) {
    return new LongShapeDescriptor(
        shape, Nd4j.getStrides(shape, Nd4j.order()), 0, 1, Nd4j.order(), extras);
  }

  /**
   * Return a new LongShapeDescriptor with the same shape, strides, order etc but with the specified
   * datatype instead
   *
