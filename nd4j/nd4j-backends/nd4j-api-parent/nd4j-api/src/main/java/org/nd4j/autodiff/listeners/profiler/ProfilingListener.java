...
public class ProfilingListener extends BaseListener {
  ...
  protected ProfilingListener(
    ...
    try {
      ...
      this.writer.write(
          "["); // JSON array open (array close is optional for Chrome profiler format)
    } catch (IOException e) {
  ...
  public void preOpExecution(SameDiff sd, At at, SameDiffOp op, OpContext opContext) {
    if (logActive) {
      ...
      if (!all && nMs > 0 && firstOpStart == null) firstOpStart = opStartNano;
    }
  }
  ...
  public void opExecution(
    ...
    if (logActive) {
      ...
      if (warmup > 0 && this.nIter > 0) terminationPt += this.warmup;
  ...
  public static class Builder {
    ...
    /**
     * Set a limit on the maximum number of iterations to profile (after warmup, if any). Any ops
     * executed after the specified number of iterations will not be profiled/recorded
     */
    ...
    /**
     * Set a limit on the maximum duration for profiling, in milliseconds. Any ops executed after
     * the specified amount of time since the first (non-warmup) operation start will not be
     * profiled/recorded
     */
    ...
    /**
     * Specify the operations (training, inference, etc) to profile. If not set, all operations are
     * profiled
     */
