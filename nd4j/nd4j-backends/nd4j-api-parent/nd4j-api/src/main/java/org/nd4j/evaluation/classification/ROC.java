...
public class ROC extends BaseEvaluation<ROC> {
  ...
  /**
   * @param thresholdSteps Number of threshold steps to use for the ROC calculation. If set to 0:
   ...
   * @param thresholdSteps Number of threshold steps to use for the ROC calculation. If set to 0:
   *     use exact calculation
   * @param rocRemoveRedundantPts Usually set to true. If true, remove any redundant points from ROC
   *     and P-R curves
   */
   ...
   * @param rocRemoveRedundantPts Usually set to true. If true, remove any redundant points from ROC
   *     and P-R curves
   * @param exactAllocBlockSize if using exact mode, the block size relocation. Users can likely use
   *     the default setting in almost all cases
   */
  ...
  /**
   * Set the axis for evaluation - this should be a size 1 dimension For DL4J, this can be left as
   * the default setting (axis = 1).<br>
  ...
  public double calculateAUC() {
    ...
    Preconditions.checkState(
        exampleCount > 0,
        "Unable to calculate AUC: no evaluation has been performed (no examples)");
  ...
  public RocCurve getRocCurve() {
    ...
    Preconditions.checkState(
        exampleCount > 0,
        "Unable to get ROC curve: no evaluation has been performed (no examples)");

    if (isExact) {
      // Sort ascending. As we decrease threshold, more are predicted positive.
      // if(prob <= threshold> predict 0, otherwise predict 1
      // So, as we iterate from i=0..length, first 0 to i (inclusive) are predicted class 1, all
      // others are predicted class 0
      ...
      fpr.put(
          new INDArrayIndex[] {interval(1, length + 1), all()}, cumSumNeg.div(countActualNegative));

      INDArray tpr = Nd4j.create(DataType.DOUBLE, length + 2, 1);
      tpr.put(
          new INDArrayIndex[] {interval(1, length + 1), all()}, cumSumPos.div(countActualPositive));
  ...
  private static Pair<double[][], int[][]> removeRedundant(
      double[] threshold, double[] x, double[] y, int[] tpCount, int[] fpCount, int[] fnCount) {
    ...
    return new Pair<>(
        new double[][] {t_compacted, x_compacted, y_compacted},
  ...
  public double calculateAUCPR() {
    ...
    Preconditions.checkState(
        exampleCount > 0,
        "Unable to calculate AUPRC: no evaluation has been performed (no examples)");
  ...
  /**
   * Get the precision recall curve as array. return[0] = threshold array<br>
  ...
  public PrecisionRecallCurve getPrecisionRecallCurve() {
    ...
    Preconditions.checkState(
        exampleCount > 0, "Unable to get PR curve: no evaluation has been performed (no examples)");
    ...
    if (isExact) {
      ...
      INDArray precision =
          cumSumPos.castTo(DataType.DOUBLE).div(linspace.reshape(cumSumPos.shape()));
      ...
      tpCountOut[0] = 0;
      fpCountOut[0] =
          0; // (int)(exampleCount - countActualPositive);  //All negatives are predicted positive
      ...
      if (rocRemoveRedundantPts) {
        Pair<double[][], int[][]> pair =
            removeRedundant(
                thresholdOut, precisionOut, recallOut, tpCountOut, fpCountOut, fnCountOut);
  ...
  /**
   * Evaluate (collect statistics for) the given minibatch of data. For time series (3 dimensions)
   * use {@link #evalTimeSeries(INDArray, INDArray)} or {@link #evalTimeSeries(INDArray, INDArray,
   * INDArray)}
   *
  ...
  public void eval(
    ...
    if (labels2d.rank() > 2
        || predictions2d.rank() > 2
        || labels2d.size(1) != predictions2d.size(1)
        || labels2d.size(1) > 2) {
    ...
    Preconditions.checkState(
        count == 0,
        "Cannot perform evaluation with NaN(s) present:"
            + " %s NaN(s) present in predictions INDArray",
        count);
    ...
    if (isExact) {
      ...
      if (probAndLabel == null) {
        ...
        probAndLabel =
            Nd4j.create(
                DataType.DOUBLE,
                new long[] {initialSize, 2},
                'c'); // First col: probability of class 1. Second col: "is class 1"
      }
      ...
      if (exampleCount + labels2d.size(0) >= probAndLabel.size(0)) {
        ...
        if (exampleCount > 0) {
          ...
          newProbAndLabel
              .get(interval(0, exampleCount), all())
              .assign(probAndLabel.get(interval(0, exampleCount), all()));
        }
      ...
      probAndLabel
          .get(interval(exampleCount, exampleCount + currMinibatchSize), NDArrayIndex.point(0))
          .assign(probClass1);

      probAndLabel
          .get(interval(exampleCount, exampleCount + currMinibatchSize), NDArrayIndex.point(1))
          .assign(labelClass1);
      ...
      // Here: calculate true positive rate (TPR) vs. false positive rate (FPR) at different
      // threshold
      ...
      for (int i = 0; i <= thresholdSteps; i++) {
        ...
        // Work out true/false positives - do this by replacing probabilities (predictions) with 1
        // or 0 based on threshold
        ...
        INDArray
            isTruePositive; // = predictedClass1.mul(positiveActualClassColumn); //If predicted == 1
                            // and actual == 1 at this threshold: 1x1 = 1. 0 otherwise
        INDArray
            isFalsePositive; // = predictedClass1.mul(negativeActualClassColumn); //If predicted ==
                             // 1 and actual == 0 at this threshold: 1x1 = 1. 0 otherwise
        if (i == 0) {
          ...
          isTruePositive =
              Nd4j.getExecutioner()
                  .exec(new MulOp(predictedClass1, positiveActualClassColumn, itp))[0];
          isFalsePositive =
              Nd4j.getExecutioner()
                  .exec(new MulOp(predictedClass1, negativeActualClassColumn, ifp))[0];
        }
  ...
  /**
   * Merge this ROC instance with another. This ROC instance is modified, by adding the stats from
   * the other instance.
   *
  ...
  public void merge(ROC other) {
    ...
    if (isExact) {
      ...
      if (this.exampleCount + other.exampleCount > this.probAndLabel.size(0)) {
        // Allocate new array
        val newSize =
            this.probAndLabel.size(0) + Math.max(other.probAndLabel.size(0), exactAllocBlockSize);
        INDArray newProbAndLabel = Nd4j.create(DataType.DOUBLE, newSize, 2);
        newProbAndLabel.put(
            new INDArrayIndex[] {interval(0, exampleCount), all()},
      ...
      probAndLabel.put(
          new INDArrayIndex[] {interval(exampleCount, exampleCount + other.exampleCount), all()},
          toPut);
    } else {
  ...
  public String stats() {
    ...
    if (!isExact) {
      sb.append("\n");
      sb.append("[Note: Thresholded AUC/AUPRC calculation used with ")
  ...
  public double getValue(IMetric metric) {
    ...
    } else throw new IllegalStateException("Can't get value for non-ROC Metric " + metric);
  }
