...
public class MultiDataSet implements org.nd4j.linalg.dataset.api.MultiDataSet {
  ...
  public MultiDataSet(
      INDArray features, INDArray labels, INDArray featuresMask, INDArray labelsMask) {
    this(
   ...
   * @param featuresMaskArrays The mask arrays for the features. May be null. Typically used with
   *     variable-length time series models, etc
   * @param labelsMaskArrays The mask arrays for the labels. May be null. Typically used with
   *     variable-length time series models, etc
   */
  public MultiDataSet(
      INDArray[] features,
  ...
  public MultiDataSet(
    ...
    if (features != null
        ...
        && features.length != featuresMaskArrays.length) {
      throw new IllegalArgumentException(
          "Invalid features / features mask arrays combination: "
    ...
    if (labels != null && labelsMaskArrays != null && labels.length != labelsMaskArrays.length) {
      throw new IllegalArgumentException(
          "Invalid labels / labels mask arrays combination: "
  ...
  private void saveINDArrays(INDArray[] arrays, DataOutputStream dos, boolean isMask)
      throws IOException {
  ...
  private INDArray[] loadINDArrays(int numArrays, DataInputStream dis, boolean isMask)
      throws IOException {
  ...
  public List<org.nd4j.linalg.dataset.api.MultiDataSet> asList() {
    ...
    for (int i = 0; i < nExamples; i++) {
      ...
      INDArray[] thisLabelsMaskArray =
          (labelsMaskArrays != null ? new INDArray[labelsMaskArrays.length] : null);
      ...
      list.add(
          new MultiDataSet(thisFeatures, thisLabels, thisFeaturesMaskArray, thisLabelsMaskArray));
    }
  ...
  private static INDArray getSubsetForExample(INDArray array, int idx) {
    // Note the interval use here: normally .point(idx) would be used, but this collapses the point
    // dimension
    // when used on arrays with rank of 3 or greater
    // So (point,all,all) on a 3d input returns a 2d output. Whereas, we want a 3d [1,x,y] output
    // here
    switch (array.rank()) {
      case 2:
      ...
      case 3:
        return array.get(
            NDArrayIndex.interval(idx, idx, true), NDArrayIndex.all(), NDArrayIndex.all());
      case 4:
        return array.get(
            NDArrayIndex.interval(idx, idx, true),
            NDArrayIndex.all(),
            NDArrayIndex.all(),
            NDArrayIndex.all());
      default:
  ...
  /**
   * Merge a collection of MultiDataSet objects into a single MultiDataSet. Merging is done by
   * concatenating along dimension 0 (example number in batch) Merging operation may introduce mask
   * arrays (when necessary) for time series data that has different lengths; if mask arrays already
   * exist, these will be merged also.
   *
   * @param toMerge Collection of MultiDataSet objects to merge
   * @return a single MultiDataSet object, containing the arrays of
   */
  public static MultiDataSet merge(
      Collection<? extends org.nd4j.linalg.dataset.api.MultiDataSet> toMerge) {
    if (toMerge.size() == 1) {
      ...
      if (features[i] == null || features[i].length != nInArrays) {
        throw new IllegalStateException(
            "Cannot merge MultiDataSets with different number of input arrays: toMerge[0] has "
                + nInArrays
                + " input arrays; toMerge["
                + i
                + "] has "
                + (features[i] != null ? features[i].length : null)
                + " arrays");
      }
      if (labels[i] == null || labels[i].length != nOutArrays) {
        throw new IllegalStateException(
            "Cannot merge MultiDataSets with different number of output arrays: toMerge[0] has "
                + nOutArrays
                + " output arrays; toMerge["
                + i
                + "] has "
                + (labels[i] != null ? labels[i].length : null)
                + " arrays");
      }
    ...
    for (i = 0; i < nInArrays; i++) {
      Pair<INDArray, INDArray> pair =
          DataSetUtil.mergeFeatures(
              features, featuresMasks, i); // merge(features, featuresMasks, i);
  ...
  public String toString() {
    ...
    sb.append("MultiDataSet: ")
        .append(numFeatureArrays())
        .append(" input arrays, ")
        .append(numLabelsArrays())
        .append(" label arrays, ")
        .append(nfMask)
        .append(" input masks, ")
        .append(nlMask)
  ...
  public void migrate() {
    if (Nd4j.getMemoryManager().getCurrentWorkspace() != null) {
      if (features != null)
        for (int e = 0; e < features.length; e++) features[e] = features[e].migrate();

      if (labels != null) for (int e = 0; e < labels.length; e++) labels[e] = labels[e].migrate();
  ...
  public void detach() {
    if (features != null)
      for (int e = 0; e < features.length; e++) features[e] = features[e].detach();

    if (labels != null) for (int e = 0; e < labels.length; e++) labels[e] = labels[e].detach();
