...
public abstract class BaseDataBuffer implements DataBuffer {
  ...
  protected BaseDataBuffer(DataBuffer underlyingBuffer, long length, long offset) {
    ...
    if (!underlyingBuffer.isConstant()) ((BaseDataBuffer) underlyingBuffer).pickReferent(this);
  ...
  /**
   * Original DataBuffer. In case if we have a view derived from another view, derived from some
   * other view, original DataBuffer will point to the originating DataBuffer, where all views come
   * from.
   */
  @Override
  ...
  public void copyAtStride(
    ...
    NativeOpsHolder.getInstance()
        .getDeviceNativeOps()
        .copyBuffer(buf.opaqueBuffer(), n, this.opaqueBuffer(), offset, yOffset);
  }
   ...
   * Create with length
   *
   * @param length a databuffer of the same opType as this with the given length
  ...
  public float[] asFloat() {
    ...
    for (int i = 0; i < length; i++) ret[i] = getFloatUnsynced(i);
    return ret;
  }
  ...
  public double[] asDouble() {
    ...
    for (int i = 0; i < length; i++) ret[i] = getDoubleUnsynced(i);
    return ret;
  }
  ...
  public int[] asInt() {
    ...
    for (int i = 0; i < length; i++) ret[i] = getIntUnsynced(i);
    return ret;
  }
  ...
  public long[] asLong() {
    ...
    for (int i = 0; i < length; i++) ret[i] = getLongUnsynced(i);
    return ret;
  }
  ...
  public double getDouble(long i) {
    ...
    switch (dataType()) {
      ...
      default:
        throw new UnsupportedOperationException(
            "Cannot get double value from buffer of type " + dataType());
    }
  }
  ...
  public long getLong(long i) {
    ...
    switch (dataType()) {
      ...
      default:
        throw new UnsupportedOperationException(
            "Cannot get long value from buffer of type " + dataType());
    }
  }
  ...
  protected short getShort(long i) {
    ...
    switch (dataType()) {
      ...
      default:
        throw new UnsupportedOperationException(
            "Cannot get short value from buffer of type " + dataType());
    }
  }
  ...
  public float getFloat(long i) {
    ...
    switch (dataType()) {
      ...
      default:
        throw new UnsupportedOperationException(
            "Cannot get float value from buffer of type " + dataType());
    }
  }
  ...
  public Number getNumber(long i) {
    ...
    if (dataType() == DataType.DOUBLE) return getDouble(i);
    else if (dataType() == DataType.INT || dataType() == DataType.INT32) return getInt(i);
    else if (dataType() == DataType.LONG || dataType() == DataType.INT64) return getLong(i);
    return getFloat(i);
  }
  ...
  public void putByDestinationType(long i, Number element, DataType globalType) {
    if (globalType == DataType.INT32
        || globalType == DataType.INT
        || type == DataType.INT
        || globalType == DataType.UINT16
        || globalType == DataType.UBYTE
        || globalType == DataType.SHORT
        || globalType == DataType.BYTE
        || globalType == DataType.BOOL) {
      int anElement = element.intValue();
      put(i, anElement);
    } else if (globalType == DataType.INT64
        || globalType == DataType.LONG
        || type == DataType.LONG
        || globalType == DataType.UINT32
        || globalType == DataType.UINT64) {
      long anElement = element.longValue();
      put(i, anElement);
    } else if (globalType == DataType.FLOAT
        || globalType == DataType.HALF
        || globalType == DataType.BFLOAT16) {
  ...
  public IntBuffer asNioInt() {
    ...
    } else return wrappedBuffer().asIntBuffer().position((int) offset());
  }
  ...
  public LongBuffer asNioLong() {
    ...
    } else return wrappedBuffer().asLongBuffer().position((int) offset());
  }
  ...
  public boolean equals(Object o) {
    if (o instanceof DataBuffer) {
      ...
      if (d.dataType() != dataType()) return false;
          ...
          Nd4j.getExecutioner()
              .exec(
                  new Eps(
                      Nd4j.create(d),
                      Nd4j.create(this),
                      Nd4j.createUninitialized(DataType.BOOL, length())));
      return exec.all();
    }
  ...
  public static Triple<AllocationMode, Long, DataType> readHeader(@NonNull InputStream is) {
    try {
      DataInputStream dis =
          is instanceof DataInputStream ? (DataInputStream) is : new DataInputStream(is);
  ...
  @Override
  public void read(
      DataInputStream s, @NonNull AllocationMode allocMode, long len, @NonNull DataType dtype) {
    try {
      ...
      if (savedMode.ordinal() < 3) {
        // Do an implicit conversion: keep current buffer data type unchanged, and convert values
        // from source type
        length = len;
        ...
        if (currentType == DataType.LONG) elementSize = 8;
        else if (currentType == DataType.DOUBLE && currentType != DataType.INT) elementSize = 8;
        else if (currentType == DataType.FLOAT || currentType == DataType.INT) elementSize = 4;
        else if (currentType == DataType.HALF && currentType != DataType.INT) elementSize = 2;

        pointerIndexerByCurrentType(currentType);

        if (currentType != DataType.COMPRESSED) readContent(s, currentType, currentType);
        ...
        if (type != DataType.COMPRESSED) readContent(s, type, type);
      }
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  ...
  protected void readContent(DataInputStream s, DataType sourceType, DataType thisType) {
    try {
      // Use AtomicX as a mutable Number class to reduce garbage vs. auto boxing to Double/Float etc
      // classes
      if (sourceType == DataType.DOUBLE) {
        ...
        throw new UnsupportedOperationException(
            "Cannot read type: " + sourceType + " to " + thisType);
      }
    } catch (Exception e) {
      throw new RuntimeException(e);
    }
  ...
  public void write(DataOutputStream out) throws IOException {
    ...
    switch (dataType()) {
      case DOUBLE:
        for (long i = 0; i < length(); i++) out.writeDouble(getDoubleUnsynced(i));
        break;
      case UINT64:
      case LONG:
        for (long i = 0; i < length(); i++) out.writeLong(getLongUnsynced(i));
        break;
      case UINT32:
      case INT:
        for (long i = 0; i < length(); i++) out.writeInt(getIntUnsynced(i));
        break;
      case UINT16:
      case SHORT:
        for (long i = 0; i < length(); i++) out.writeShort((short) getIntUnsynced(i));
        break;
      case UBYTE:
      case BYTE:
        for (long i = 0; i < length(); i++) out.writeByte((byte) getIntUnsynced(i));
        break;
      case BOOL:
        for (long i = 0; i < length(); i++)
      ...
      case HALF:
        ...
        break;
      case FLOAT:
        for (long i = 0; i < length(); i++) out.writeFloat(getFloatUnsynced(i));
        break;
    }
  }
  ...
  /**
   * This method returns whether this DataBuffer is constant, or not. Constant buffer means that it
   * modified only during creation time, and then it stays the same for all lifecycle. I.e. used in
   * shape info databuffers.
   *
   * @return
   */
   ...
   *
   * <p>PLEASE NOTE: DO NOT USE THIS METHOD, UNLESS YOU'RE 100% SURE WHAT YOU DO
   *
   ...
   *
   * <p>PLEASE NOTE: if this INDArray isn't attached to any Workspace, this method will return true
   *
   * @return
   */
  @Override
  ...
  public MemoryWorkspace getParentWorkspace() {
    ...
    if (wrappedDataBuffer != null
        && wrappedDataBuffer.isAttached()
        && wrappedDataBuffer.getParentWorkspace() != null) {
      return wrappedDataBuffer.getParentWorkspace();
    }
    if (originalBuffer != null
        && originalBuffer.isAttached()
        && originalBuffer.getParentWorkspace() != null) {
  ...
  public boolean closeable() {
    if (released.get() || isAttached() || isConstant()) return false;

    if (wrappedDataBuffer != null && wrappedDataBuffer != this) return false;
  ...
  @Override
  public void close() {
    if (!closeable()) throw new IllegalStateException("Can't release this data buffer");
