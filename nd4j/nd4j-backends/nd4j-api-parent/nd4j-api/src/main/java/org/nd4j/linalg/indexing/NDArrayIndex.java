...
public abstract class NDArrayIndex implements INDArrayIndex {
  ...
  /**
   * Deep copy all elements of this array of {@link INDArrayIndex} invoking {@link
   * INDArrayIndex#dup()} on each entry
   *
   ...
   * @return the point index based on the specified point
   */
  ...
  /**
   * Compute the offset given an array of offsets. The offset is computed(for both fortran an d c
   * ordering) as: sum from i to n - 1 o[i] * s[i] where i is the index o is the offset and s is the
   * stride Notice the -1 at the end.
   *
   ...
   * @return an array of length n containing copies of the given ndarray index
   */
   ...
   *
   * @return an ndarray index meaning collect all elements
   */
  ...
  /**
   * Returns an instance of {@link SpecifiedIndex}. Note that SpecifiedIndex works differently than
   * the other indexing options, in that it always returns a copy of the (subset of) the underlying
   * array, for get operations. This means that INDArray.get(..., indices(x,y,z), ...) will be a
   * copy of the relevant subset of the array.
   *
   ...
   *
   * @return the indexing for adding a new dimension
   */
  ...
  /**
   * Given an all index and the intended indexes, return an index array containing a combination of
   * all elements for slicing and overriding particular indexes where necessary
   *
   ...
   *
   * @param indexes the indexes to count for points
   * @return the number of point indexes in the array
   */
  public static int numPoints(INDArrayIndex... indexes) {
    int ret = 0;
    for (int i = 0; i < indexes.length; i++) if (indexes[i] instanceof PointIndex) ret++;
    return ret;
  }
  ...
  public static INDArrayIndex[] resolveLong(long[] shapeInfo, INDArrayIndex... intendedIndexes) {
    ...
    for (int i = 0; i < intendedIndexes.length; i++) {
      if (intendedIndexes[i] instanceof SpecifiedIndex) numSpecified++;
    }
    ...
    if (numSpecified > 0) {
      ...
      for (int i = 0; i < intendedIndexes.length; i++) {
        if (intendedIndexes[i] instanceof SpecifiedIndex) ret[i] = intendedIndexes[i];
        else {
          if (intendedIndexes[i] instanceof NDArrayIndexAll) {
            ...
            ret[i] =
                new SpecifiedIndex(
                    ArrayUtil.range(
                        intervalIndex.begin, intervalIndex.end(), intervalIndex.stride()));
    ...
    val shape = Shape.shapeOf(shapeInfo);
    if (intendedIndexes.length >= rank
        || Shape.isVector(shapeInfo) && intendedIndexes.length == 1) {
      ...
      if (Shape.isRowVectorShape(shapeInfo) && intendedIndexes.length == 1) {
        ...
        long size;
        if (1 == shape[0] && rank == 2) size = shape[1];
        else size = shape[0];
      ...
      for (int i = 0; i < intendedIndexes.length; i++) {
        if (i < rank) retList.add(validate(shape[i], intendedIndexes[i]));
        else retList.add(intendedIndexes[i]);
      }
    ...
    if (Shape.isMatrix(shape) && intendedIndexes.length == 1) {
      ...
      for (int i = 0; i < intendedIndexes.length; i++) {
        ...
        if (intendedIndexes[i] instanceof NewAxis) numNewAxes++;
      }
    }
  ...
  public static INDArrayIndex[] resolve(long[] shape, INDArrayIndex... intendedIndexes) {
    ...
    if (intendedIndexes.length >= shape.length
        || Shape.isVector(shape) && intendedIndexes.length == 1) {
      if (Shape.isRowVectorShape(shape) && intendedIndexes.length == 1) {
        INDArrayIndex[] ret = new INDArrayIndex[2];
        ret[0] = NDArrayIndex.point(0);
        long size;
        if (1 == shape[0] && shape.length == 2) size = shape[1];
        else size = shape[0];
        ret[1] = validate(size, intendedIndexes[0]);
        return ret;
      }
      List<INDArrayIndex> retList = new ArrayList<>(intendedIndexes.length);
      for (int i = 0; i < intendedIndexes.length; i++) {
        if (i < shape.length) retList.add(validate(shape[i], intendedIndexes[i]));
        else retList.add(intendedIndexes[i]);
      }
      return retList.toArray(new INDArrayIndex[retList.size()]);
    }
  ...
  protected static INDArrayIndex validate(long size, INDArrayIndex index) {
    if ((index instanceof IntervalIndex || index instanceof PointIndex) && size <= index.offset())
      throw new IllegalArgumentException(
          "NDArrayIndex is out of range. Beginning index: "
              + index.offset()
              + " must be less than its size: "
              + size);
    if (index instanceof IntervalIndex && index.end() > size)
      throw new IllegalArgumentException(
          "NDArrayIndex is out of range. End index: "
              + index.end()
              + " must be less than its size: "
              + size);
  ...
  public static INDArrayIndex[] resolve(
    ...
    for (int i = 0; i < allIndex.length; i++) {
      ...
      if (intendedIndexes[i] instanceof NDArrayIndex) {
        ...
        if (idx.indices.length == 1) intendedIndexes[i] = new PointIndex(idx.indices[0]);
      }
   ...
   *
   * @param axes the indexes to get the number of new axes for
   * @return the number of new axis elements in the given array
   */
  public static int numNewAxis(INDArrayIndex... axes) {
    int ret = 0;
    for (INDArrayIndex index : axes) if (index instanceof NewAxis) ret++;
    return ret;
  }
   ...
   * @return an ndarray index array containing of length arr.rank() containing all elements
   */
  public static INDArrayIndex[] allFor(INDArray arr) {
    INDArrayIndex[] ret = new INDArrayIndex[arr.rank()];
    for (int i = 0; i < ret.length; i++) ret[i] = NDArrayIndex.all();
  ...
  /**
   * Create a range based on the given indexes. This is similar to create covering shape in that it
   * approximates the length of each dimension (ignoring elements) and reproduces an index of the
   * same dimension and length.
   *
  ...
  public static INDArrayIndex interval(long begin, long stride, long end) {
    if (end > 0 && Math.abs(begin - end) < 1) end++;
  ...
  public static INDArrayIndex interval(int begin, int stride, int end, boolean inclusive) {
    Preconditions.checkArgument(
        begin <= end || end < 0,
        "Beginning index (%s) in range must be less than or equal to end (%s)",
        begin,
        end);
  ...
  public static INDArrayIndex interval(
      long begin, long stride, long end, long max, boolean inclusive) {
    Preconditions.checkArgument(
        begin <= end || end < 0,
        "Beginning index (%s) in range must be less than or equal to end (%s)",
        begin,
  ...
  @Override
  public long end() {
    if (indices != null && indices.length > 0) return indices[indices.length - 1];
    return 0;
  }
