...
public class UniformDistribution extends BaseDistribution {
   ...
   * {@inheritDoc}
   *
   * <p>For lower bound {@code lower} and upper bound {@code upper}, the mean is {@code 0.5 * (lower
   * + upper)}.
   */
  ...
  public INDArray sample(INDArray ret) {
    if (random.getStatePointer() != null) {
      return Nd4j.getExecutioner()
          .exec(
              new org.nd4j.linalg.api.ops.random.impl.UniformDistribution(ret, lower, upper),
              random);
    } else {
      val idxIter =
          new NdIndexIterator(
