...
public class ShapeFunctionOptimizations extends BaseOptimizerSet {
  ...
  /**
   * Fuse [permute1 -> permute2 -> ... -> permuteN] into a single permute op, as long as the
   * intermediate permute outputs aren't needed for another op
   */
  public static class FuseChainedPermutes implements Optimizer {
    ...
    public boolean checkAndApply(
      ...
      while (currInput != null) {
        ...
        if (v.getInputsForOp().size() > 1) break;
      }
  ...
  /**
   * Fuse [reshape1 -> reshape2 -> ... -> reshapeN] into a single reshape op, as long as the
   * intermediate reshape ops aren't needed for another op
   */
  ...
  /**
   * Fuse [concat(concat(concat(x,y,dim=D), z, dim=D), a, dim=D)] into a single concat op,
   * concat(x,y,z,a, dim=D) As long as the intermediate outputs aren't needed elsewhere
   */
