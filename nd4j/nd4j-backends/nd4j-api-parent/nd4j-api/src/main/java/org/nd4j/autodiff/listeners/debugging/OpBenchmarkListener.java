...
public class OpBenchmarkListener extends BaseListener {
   ...
   * @param minRuntime Minimum runtime - only applies to Mode.SINGLE_ITER_PRINT. If op runtime below
   *     this: don't print
   */
  ...
  public void operationStart(SameDiff sd, Operation op) {
    if (printDone) return;
    if (this.operation == null || this.operation == op) printActive = true;
  }
  ...
  public void opExecution(
    ...
    if (mode == Mode.SINGLE_ITER_PRINT && printActive && (now - start) > this.minRuntime) {
      ...
      if (aggregateModeMap == null) aggregateModeMap = new LinkedHashMap<>();

      if (!aggregateModeMap.containsKey(op.getName())) {
        String s = getOpString(op, null);
        OpExec oe =
            new OpExec(
                op.getName(), op.getOp().opName(), op.getOp().getClass(), new ArrayList<Long>(), s);
  ...
  private String getOpString(SameDiffOp op, Long now) {
    ...
    sb.append(op.getName())
        .append(" - ")
        .append(op.getOp().getClass().getSimpleName())
        .append("(")
        .append(op.getOp().opName())
        .append(") - ");
    if (now != null) {
    ...
    if (op.getOp() instanceof DynamicCustomOp) {
      ...
      Op o = (Op) op.getOp();
      if (o.x() != null) sb.append("  x: ").append(o.x().shapeInfoToString());
      if (o.y() != null) sb.append("  y: ").append(o.y().shapeInfoToString());
      if (o.z() != null) sb.append("  z: ").append(o.z().shapeInfoToString());
    }
    return sb.toString();
  }
