...
public abstract class DefaultOpExecutioner implements OpExecutioner {
  ...
  private static final String SCOPE_PANIC_MSG =
      "For more details, see the ND4J User Guide:"
          + " https://deeplearning4j.konduit.ai/nd4j/reference#workspaces-scope-panic";
  ...
  /**
   * Inject an op context created using {@link #buildContext()} and return a reference to the
   * context.
   *
   * @return
   */
  ...
  /** Clears the context injected with {@link #injectNewContext()} ()} */
  @Override
  ...
  /**
   * Setting an {@link OpContext} will cause {@link #buildContext()} to consume the specified op
   * context in place of creating a new one.
   *
   * @param context
   */
  @Override
  ...
  /**
   * Execute a redirected {@link org.nd4j.linalg.api.ops.impl.transforms.custom.Assign} op from the
   * old {@link TransformOp} based {@link Assign} based Assign op
   *
  ...
  public static void execAssign(TransformOp op, OpContext oc, OpExecutioner executioner) {
    org.nd4j.linalg.api.ops.impl.transforms.custom.Assign op2 =
        new org.nd4j.linalg.api.ops.impl.transforms.custom.Assign();
    ...
    if (oc == null) {
      if (Nd4j.getEnvironment().isDebugAndVerbose() && op.x().isView()) {
        log.warn(
            "Assign op running on a view. This may cause issues with the underlying buffer being"
                + " modified and the view not seeing these changes");
      }
      op2.addBArgument(op.x().isView());
      op2.addInputArgument(op.x());
      if (op.y() != null) op2.addInputArgument(op.y());
  ...
  public static void initOpContext(CustomOp op, boolean shapeOverride, OpContext context) {
    ...
    if (!op.isInplaceCall()) context.setOutputArrays(op.outputArguments());
  ...
  protected void checkForCompression(Op op) {
    if (op.x() != null && op.x().isCompressed()) Nd4j.getCompressor().decompressi(op.x());

    if (op.y() != null && op.y().isCompressed()) Nd4j.getCompressor().decompressi(op.y());

    if (op.z() != null && op.z().isCompressed()) Nd4j.getCompressor().decompressi(op.z());
  }
  ...
  public void exec(MetaOp op) {
    throw new UnsupportedOperationException(
        "MetaOp execution isn't supported for this OpExecutioner yet");
  }

  @Override
  public void exec(GridOp op) {
    throw new UnsupportedOperationException(
        "GridOp execution isn't supported for this OpExecutioner yet");
  }
      ...
      if (ws.getWorkspaceType() != MemoryWorkspace.Type.CIRCULAR) {
        ...
        if (!ws.isScopeActive()) {
          throw new ND4JIllegalStateException(
              "Op ["
                  + opName
                  + "] X argument uses leaked workspace pointer from workspace ["
                  + ws.getId()
  ...
  protected void checkForWorkspaces(Op op, OpContext oc) {
    ...
    if (x != null) checkWorkspace(op.opName(), x);

    val y = oc != null && oc.getInputArrays().size() > 1 ? oc.getInputArray(1) : op.y();
    if (y != null) checkWorkspace(op.opName(), y);

    val z = oc != null ? oc.getOutputArray(0) : op.z();
    if (z != null) checkWorkspace(op.opName(), z);
  }
  ...
  public void profilingHookOut(Op op, OpContext oc, long timeStart) {
    if (Nd4j.getEnvironment().isLogNDArrayEvents()) {
      ...
      INDArray y =
          op.y() != null ? op.y() : oc.getInputArrays().size() > 1 ? oc.getInputArray(1) : null;
  ...
  public static List<INDArray> inputArrsFromOp(Op op, OpContext opContext) {
    if (opContext != null && !opContext.getInputArrays().isEmpty()) {
      ...
      if (op.x() != null && op.y() != null) return Arrays.asList(op.x(), op.y());
      else if (op.x() != null) return Collections.singletonList(op.x());
      else if (op.y() != null) return Collections.singletonList(op.y());
  ...
  public static List<INDArray> outputArrsFromOp(Op op, OpContext opContext) {
    if (opContext != null && !opContext.getOutputArrays().isEmpty()) {
      ...
      if (op.z() != null) return Collections.singletonList(op.z());
      else if (op.y() != null) return Collections.singletonList(op.y());
      else if (op.x() != null) return Collections.singletonList(op.x());
      else return Collections.emptyList();
    }
  ...
  public long profilingConfigurableHookIn(Op op, OpContext oc) {
    ...
    logOpArrayEventsIfNeccessary(
        op, inArgs, outArgs, NDArrayEventType.BEFORE_OP_INPUT, NDArrayEventType.BEFORE_OP_OUTPUT);
    logOpArrayEventsIfNeccessary(
        op, inArgs, outArgs, NDArrayEventType.OP_INPUT, NDArrayEventType.OP_OUTPUT);
  ...
  public long profilingConfigurableHookIn(CustomOp op, OpContext oc) {
    ...
    if (isDebug() && isVerbose()) {
      ...
      log.info(
          "About to execute op {} of type {} with inputs {} and outputs {}",
          differentialFunction.getOwnName(),
          op.opName(),
          Arrays.toString(arg),
          Arrays.toString(differentialFunction.outputVariablesNames()));
    }

    logCustomOpArrayEventIfNeccessary(
        inArgs, outArgs, NDArrayEventType.BEFORE_OP_INPUT, NDArrayEventType.BEFORE_OP_OUTPUT);
    logCustomOpArrayEventIfNeccessary(
        inArgs, outArgs, NDArrayEventType.OP_INPUT, NDArrayEventType.OP_OUTPUT);
  ...
  public long profilingConfigurableHookIn(Op op, DataBuffer... tadBuffers) {
    ...
    logOpArrayEventsIfNeccessary(
        op, inputs, outputs, NDArrayEventType.BEFORE_OP_INPUT, NDArrayEventType.BEFORE_OP_OUTPUT);
  ...
  private void logOpArrayEventsIfNeccessary(
      Op op,
      List<INDArray> inArgs,
      List<INDArray> outArgs,
  ...
  private void logCustomOpArrayEventIfNeccessary(
      List<INDArray> inArgs,
      List<INDArray> outArgs,
      NDArrayEventType inputEvenType,
      NDArrayEventType outputEventType) {
    logArrays(inArgs, outArgs, inputEvenType, outputEventType);
  }

  private static void logArrays(
      List<INDArray> inArgs,
      List<INDArray> outArgs,
      NDArrayEventType eventType,
      NDArrayEventType outputEventType) {
  ...
  public static void validateDataType(DataType expectedType, Op op) {
    if (op.x() != null
        && !Shape.isEmpty(op.x().shapeInfoJava())
        && op.x().data().dataType() == DataType.COMPRESSED) {
      Nd4j.getCompressor().decompressi(op.x());
    }

    if (op.y() != null
        && !Shape.isEmpty(op.y().shapeInfoJava())
        && op.y().data().dataType() == DataType.COMPRESSED) {
      Nd4j.getCompressor().decompressi(op.y());
    }

    if (op.z() != null
        && !Shape.isEmpty(op.z().shapeInfoJava())
  ...
  protected static String firstX(INDArray array, int x) {
    ...
    for (int e = 0; e < limit; e++) {
      if (array.isS()) builder.append(array.getString(e));
      else builder.append(array.getDouble(e));

      if (e < limit - 1) builder.append(", ");
    }
    builder.append("]");
  ...
  public static void validateDataType(DataType expectedType, Object op, INDArray... operands) {
    if (operands == null || operands.length == 0) return;
      ...
      if (operand.data().dataType() != expectedType) {
        throw new ND4JIllegalStateException(
            "INDArray ["
                + cnt
                + "] dataType is ["
                + operand.data().dataType()
                + "] instead of expected ["
                + expectedType
                + "]"
                + (op != null ? " op: " + op.getClass().getName() : ""));
      }
      cnt++;
    }
  }
  ...
  private long _length(long[] shape) {
    ...
    else if (shape.length == 1) return shape[0];
    else {
      long length = 1;
      for (int e = 0; e < shape.length; e++) length *= shape[e];
  ...
  public Map<String, INDArray> executeGraph(
      long id, Map<String, INDArray> map, Map<String, Integer> reverseMap) {
    throw new UnsupportedOperationException("Not yet implemented");
  }
  ...
  public static List<INDArray> getIntermediateResults(
      PointerPointer<OpaqueDataBuffer> pointerPointer,
      PointerPointer<LongPointer> opaqueConstantShapeBufferPointerPointer) {
    List<INDArray> results = new ArrayList<>();
    if (pointerPointer == null) return results;
    OpaqueDataBuffer[] buffers = new OpaqueDataBuffer[(int) pointerPointer.capacity()];
    LongPointer[] shapes =
        new LongPointer[(int) opaqueConstantShapeBufferPointerPointer.capacity()];
    for (int e = 0; e < pointerPointer.capacity(); e++) {
      if (buffers[e] == null) continue;

      DataBuffer buffer = Nd4j.createBuffer(shapes[e], null, shapes[e].capacity(), DataType.LONG);
      DataBuffer originalBuffer =
          Nd4j.createBuffer(
              buffers[e].primaryBuffer(),
              buffers[e].specialBuffer(),
              Shape.length(buffer),
              Shape.dataType(buffer));
  ...
  public void scatterUpdate(
      ScatterUpdate.UpdateOp op, INDArray array, INDArray indices, INDArray updates, long[] axis) {
    throw new UnsupportedOperationException();
  }

  /**
   * Get the information about the op in a String representation, for throwing more useful
   * exceptions (mainly for debugging)
   *
   * @param op
   * @param dimensions Use optional here for 3 states: null = "not an exec(Op, int... dim) call".
   *     empty = "exec(Op, null)". Otherwise present = "exec(Op,int[])" call
   * @return
   */
  public String opInfoString(Op op, Optional<long[]> dimensions) {
    ...
    if (x == y && x != null) sb.append("(x == y)");
    if (x == z && x != null) sb.append("(x == z)");
    if (y == z && y != null) sb.append("(y == z)");
    sb.append("\n");
  ...
  public String arrayInfo(INDArray arr) {
    ...
    if (arr.isEmpty()) return "(empty NDArray)";
