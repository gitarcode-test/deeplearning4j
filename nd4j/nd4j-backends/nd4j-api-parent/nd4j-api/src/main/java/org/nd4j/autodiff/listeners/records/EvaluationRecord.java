...
public class EvaluationRecord {
  ...
  public EvaluationRecord(Map<String, List<IEvaluation>> evaluations) {
    ...
    for (List<IEvaluation> le : evaluations.values()) {
      for (IEvaluation e : le) {
        isEmpty = false;
        if (classEvaluations.containsKey(e.getClass())) classEvaluations.remove(e.getClass());
        else classEvaluations.put(e.getClass(), e);
      }
    }
  }
  ...
  public List<IEvaluation> evaluations(String param) {
    Preconditions.checkArgument(evaluations.containsKey(param), "No evaluations for %s.", param);
   ...
   *
   * <p>Will throw an exception if there are more than one or no evaluations for the param
   *
   * @param param The target param/variable
   */
  public <T extends IEvaluation> T evaluation(String param) {
    ...
    Preconditions.checkArgument(
        evaluations.get(param).size() == 1,
        "Multiple evaluations for %s.  Use evaluations().",
        param);
   ...
   *
   * <p>Will throw an exception if there are more than one or no evaluations of that type
   *
   * @param evalClass The type of evaluation to look for
   */
  public <T extends IEvaluation<T>> T evaluation(Class<T> evalClass) {
    Preconditions.checkArgument(
        classEvaluations.containsKey(evalClass),
        "Can't get evaluation for %s.  Either no evaluations with that class are present, or more"
            + " than one are.",
        evalClass);
   ...
   *
   * <p>Will throw an exception if there are more than one or no evaluations of that type for the
   * given param
   *
   * @param param The target param/variable
   * @param evalClass The type of evaluation to look for
  ...
  public <T extends IEvaluation<T>> T evaluation(String param, Class<T> evalClass) {
    Collection<IEvaluation> evals =
        Collections2.filter(evaluations(param), Predicates.instanceOf(evalClass));

    Preconditions.checkArgument(
        evals.size() == 1, "Multiple or no evaluations of type %s for param %s.", evalClass, param);
   ...
   *
   * <p>Will throw an exception if the target evaluation doesn't support the given metric
   *
   * @param param The target param/variable
