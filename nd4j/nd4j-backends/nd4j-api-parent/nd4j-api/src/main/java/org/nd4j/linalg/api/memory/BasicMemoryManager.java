...
public abstract class BasicMemoryManager implements MemoryManager {
  ...
  /**
   * This method detaches off-heap memory from passed INDArray instances, and optionally stores them
   * in cache for future reuse PLEASE NOTE: Cache options depend on specific implementations
   *
   * @param arrays
   */
  @Override
  ...
  public void memcpy(DataBuffer dstBuffer, DataBuffer srcBuffer) {
    ...
    Pointer.memcpy(
        dstBuffer.addressPointer(),
        srcBuffer.addressPointer(),
  ...
  public void invokeGcOccasionally() {
    ...
    if (averagingEnabled.get()) intervals.add((int) (currentTime - lastGcTime.get()));
    ...
    if (averagingEnabled.get()) if (intervals.size() > intervalTail) intervals.remove();
  }
  ...
  public MemoryWorkspace scopeOutOfWorkspaces() {
    ...
    if (workspace == null) return new DummyWorkspace();
    else {
