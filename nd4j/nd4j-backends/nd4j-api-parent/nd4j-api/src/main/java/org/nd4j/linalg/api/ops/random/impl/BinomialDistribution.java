...
public class BinomialDistribution extends BaseRandomOp {
  ...
  public BinomialDistribution(
      SameDiff sd, int trials, double probability, DataType dataType, long[] shape) {
  ...
  /**
   * This op fills Z with binomial distribution over given trials with single given probability for
   * all trials
   *
   * @param z
   * @param trials
  ...
  /**
   * This op fills Z with binomial distribution over given trials with probability for each trial
   * given as probabilities INDArray
   *
   * @param z
   * @param trials
  ...
  public BinomialDistribution(@NonNull INDArray z, int trials, @NonNull INDArray probabilities) {
    ...
    if (trials > probabilities.length())
      throw new IllegalStateException(
          "Number of trials is > then amount of probabilities provided");

    if (probabilities.elementWiseStride() < 1)
      throw new IllegalStateException(
          "Probabilities array shouldn't have negative elementWiseStride");

    Preconditions.checkArgument(
        probabilities.dataType() == z.dataType(),
        "Probabilities and Z operand should have same data type");
  ...
  public List<DataType> calculateOutputDataTypes(List<DataType> inputDataTypes) {
    Preconditions.checkState(
        ...
        "Expected no input datatypes (no args) for %s, got %s",
        getClass(),
        inputDataTypes);
