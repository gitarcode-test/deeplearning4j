...
public class NDArrayMetaData implements Serializable {
  ...
  /**
   * Create an array of {@link NDArrayMetaData} from the given list
   *
  ...
  /**
   * Creates a singular array of {@link NDArrayMetaData} from the given array
   *
   * @param arr the array to create the metadata from
  ...
  /**
   * Create an {@link NDArrayMetaData} from an {@link INDArray} note that when creating this data
   * all data will be stored on heap. This logging is very expensive and is mainly for use to track
   * down subtle issues like underlying views changing.
   *
   * @param arr the array to create the metadata from
   * @return
   */
  public static NDArrayMetaData from(INDArray arr) {
    return NDArrayMetaData.builder()
        ...
        .data(
            arr.isEmpty()
                ? "[]"
                : Nd4j.getEnvironment().isTruncateNDArrayLogStrings()
                    ? arr.toString()
                    : arr.toStringFull())
