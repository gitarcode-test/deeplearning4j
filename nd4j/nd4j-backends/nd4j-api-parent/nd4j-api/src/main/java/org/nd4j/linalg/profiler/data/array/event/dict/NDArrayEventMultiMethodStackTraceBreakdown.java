...
/**
 * A breakdown of {@link NDArrayEvent} by stack trace element. This is used for comparing the
 * breakdown of events by stack trace element and comparing them.
 */
public class NDArrayEventMultiMethodStackTraceBreakdown
    extends ConcurrentHashMap<String, NDArrayEventStackTraceBreakDown> {

  public Map<String, Set<NDArrayEvent>> eventsWithParentInvocation(
      StackTraceQuery stackTraceQuery, StackTraceQuery targetOrigin) {
    ...
    for (Map.Entry<String, NDArrayEventStackTraceBreakDown> breakdown : entrySet()) {
      ...
      for (Entry<StackTraceElement, Table<StackTraceElement, StackTraceElement, List<NDArrayEvent>>>
          table : breakdown.getValue().entrySet()) {
  ...
  /**
   * Get all the breakdowns mapped by method name
   *
   ...
   */
  public Map<String, Set<BreakDownComparison>> comparisonsForStackFrame(
      String className,
      String[] methodName,
      int lineNumber,
    ...
    for (String method : methodName) {
      ...
      StackTraceElement stackTraceElement =
          StackTraceLookupKey.stackTraceElementOf(
              StackTraceLookupKey.of(className, method, lineNumber));
      ...
      Map<String, Set<BreakDownComparison>> ret2 =
          entries.stream()
              .collect(
                                  ...
                                  input3 ->
                                      !StackTraceQueryFilters.shouldFilter(
                                          new StackTraceElement[] {
                                            input3.pointsOfOrigin().getFirst(),
                                            input3.pointsOfOrigin().getSecond()
                                          },
                                          pointOfOriginFilters))
  ...
  public Map<String, Set<BreakDownComparison>> allBreakDowns(MultiMethodFilter filter) {
    ...
    Map<String, Set<StackTraceElement>> possibleParentPointsOfInvocation =
        possibleParentPointsOfInvocation();
    for (String s : keySet()) {
      Set<StackTraceElement> possiblePointsOfOriginForMethod = possiblePointsOfOrigin.get(s);
      Set<StackTraceElement> possiblePointsOfInvocationForMethod =
          possiblePointsOfInvocation.get(s);
      Set<StackTraceElement> possibleParentPointsOfInvocationForMethod =
          possibleParentPointsOfInvocation.get(s);
      possiblePointsOfOriginForMethod.stream()
          .forEach(
              origin -> {
                possiblePointsOfOriginForMethod.stream()
                    .forEach(
                        compPointOfOrigin -> {
                          possiblePointsOfInvocationForMethod.stream()
                              .forEach(
                                  invocation -> {
                                    possibleParentPointsOfInvocationForMethod.stream()
                                        .forEach(
                                            parentInvocation -> {
                                              // check for filters where appropriate to make results
                                              // easier to work with
                                              if (!MultiMethodFilter.isEmpty(filter)) {
                                                if (filter.getPointOfOriginFilters() != null
                                                    && !filter
                                                        .getPointOfOriginFilters()
                                                        .isEmpty()) {
                                                  if (filter.isInclusionFilter()) {
                                                    if (StackTraceQuery
                                                        .stackTraceElementMatchesCriteria(
                                                            filter.getPointOfOriginFilters(),
                                                            origin,
                                                            -1)) {
                                                      return;
                                                    }
                                                ...
                                                if (filter.getPointOfInvocationFilters() != null
                                                    && !filter
                                                        .getPointOfInvocationFilters()
                                                        .isEmpty()) {
                                                  if (filter.isInclusionFilter()) {
                                                    if (StackTraceQuery
                                                        .stackTraceElementMatchesCriteria(
                                                            filter.getPointOfInvocationFilters(),
                                                            invocation,
                                                            -1)) {
                                                      return;
                                                    }
                                                ...
                                                if (filter.getParentPointOfInvocationFilters()
                                                        != null
                                                    && !filter
                                                        .getParentPointOfInvocationFilters()
                                                        .isEmpty()) {
                                                  if (filter.isInclusionFilter()) {
                                                    if (StackTraceQuery
                                                        .stackTraceElementMatchesCriteria(
                                              ...
                                              BreakdownArgs breakdownArgs =
                                                  BreakdownArgs.builder()
                                                      .commonParentOfInvocation(
                                                          StackTraceLookupKey.of(parentInvocation))
                                                      .compPointOfOrigin(
                                                          StackTraceLookupKey.of(compPointOfOrigin))
                                                      .pointOfOrigin(StackTraceLookupKey.of(origin))
                                                      .commonPointOfInvocation(
                                                          StackTraceLookupKey.of(invocation))
                                                      .build();
                                              BreakDownComparison breakDownComparison =
                                                  get(s).compareBreakDown(breakdownArgs);
                                              // avoid extra noise with empty results
                                              if (breakDownComparison.anyEmpty()) {
                                                return;
                                              }
                                              // don't add things that are only the same
                                              if (filter.isOnlyIncludeDifferences()
                                                  && breakDownComparison.firstIndexDifference()
                                                      < 0) {
                                                return;
                                              }
