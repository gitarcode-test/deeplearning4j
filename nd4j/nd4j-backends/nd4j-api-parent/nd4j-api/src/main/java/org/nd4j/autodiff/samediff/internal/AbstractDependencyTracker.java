...
public abstract class AbstractDependencyTracker<T, D> {
  @Getter
  private final IDependencyMap<T, D>
      dependencies; // Key: the dependent. Value: all things that the key depends on

  @Getter
  private final IDependencyMap<T, Pair<D, D>>
      orDependencies; // Key: the dependent. Value: the set of OR dependencies

  @Getter
  private final Map<D, Set<T>> reverseDependencies =
      new LinkedHashMap<>(); // Key: the dependee. Value: The set of

  // all dependents that depend on this
  // value
  @Getter private final Map<D, Set<T>> reverseOrDependencies = new HashMap<>();

  @Getter
  private final Set<D> satisfiedDependencies =
      new LinkedHashSet<>(); // Mark the dependency as satisfied. If not in

  // set: assumed to not be satisfied
  @Getter
  private final Set<T>
      allSatisfied; // Set of all dependent values (Ys) that have all dependencies satisfied

  @Getter
  private final Queue<T> allSatisfiedQueue =
      new LinkedList<>(); // Queue for *new* "all satisfied" values. Values are
  ...
  public boolean isEmpty() {
    return dependencies.isEmpty() && orDependencies.isEmpty() && allSatisfiedQueue.isEmpty();
  }

  /**
   * @return True if the dependency has been marked as satisfied using {@link #markSatisfied(Object,
   *     boolean)}
   */
  ...
  /**
   * Mark the specified value as satisfied. For example, if two dependencies have been previously
   * added (X -> Y) and (X -> A) then after the markSatisfied(X, true) call, both of these
  ...
  /**
   * Check whether any dependencies x -> y exist, for y (i.e., anything previously added by {@link
   * #addDependency(Object, Object)} or {@link #addOrDependency(Object, Object, Object)}
   *
  ...
  public void addDependency(@NonNull T y, @NonNull D x) {
    ...
    if (!reverseDependencies.containsKey(x)) reverseDependencies.put(x, newTSet());
  ...
  protected void checkAndUpdateIfAllSatisfied(@NonNull T y) {
    ...
    if (allSat) {
      ...
      if (!allSatisfiedQueue.contains(y)) {
        ...
        sb.append("Dependent object \"")
            .append(toStringT(y))
            .append("\" was previously processed after all dependencies")
            .append(
                " were marked satisfied, but is now additional dependencies have been added.\n");
        ...
        if (dl != null) {
          ...
          for (D d : dl) {
            sb.append(d)
                .append(" - ")
                .append(isSatisfied(d) ? "Satisfied" : "Not satisfied")
                .append("\n");
          }
        }
        ...
        if (dlOr != null) {
          ...
          for (Pair<D, D> p : dlOr) {
            sb.append(p)
                .append(" - satisfied=(")
                .append(isSatisfied(p.getFirst()))
                .append(",")
                .append(isSatisfied(p.getSecond()))
                .append(")");
          }
        }
  ...
  public void removeDependency(@NonNull T y, @NonNull D x) {
    ...
    if (s2 != null) {
      s2.remove(y);
      if (s2.isEmpty()) reverseDependencies.remove(x);
    }

    Iterable<Pair<D, D>> s3 =
        orDependencies.removeGroupReturn(
            y,
            t -> {
    ...
    if (s3 != null) {
      ...
      for (Pair<D, D> p : s3) {
        if (!removedReverse) {
          ...
          if (set1.isEmpty()) reverseOrDependencies.remove(p.getFirst());
          if (set2.isEmpty()) reverseOrDependencies.remove(p.getSecond());
   ...
   * If either x1 or x2 (or both) are marked satisfied via {@link #markSatisfied(Object, boolean)}
   * then the dependency is considered satisfied
   *
   * @param y Dependent
   * @param x1 Dependee 1
   * @param x2 Dependee 2
  ...
  public void addOrDependency(@NonNull T y, @NonNull D x1, @NonNull D x2) {
    ...
    if (!reverseOrDependencies.containsKey(x1)) reverseOrDependencies.put(x1, newTSet());
    if (!reverseOrDependencies.containsKey(x2)) reverseOrDependencies.put(x2, newTSet());
   ...
   * {@link #markSatisfied(Object, boolean)} Throws an exception if {@link #hasNewAllSatisfied()}
   * returns false.<br>
   * Note that once a value has been retrieved from here, no new dependencies of the form (X -> Y)
   * can be added for this value; the value is considered "processed" at this point.
   *
   * @return The next new "all satisfied dependent"
   */
  public T getNewAllSatisfied() {
    Preconditions.checkState(
        hasNewAllSatisfied(), "No new/unprocessed dependents that are all satisfied");
  ...
  /**
   * As per {@link #getNewAllSatisfied()} but instead of returning the first dependee, it returns
   * the first that matches the provided predicate. If no value matches the predicate, null is
   * returned
   *
