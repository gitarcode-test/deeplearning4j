...
public class Evaluation extends BaseEvaluation<Evaluation> {
  ...
  @EqualsAndHashCode
      .Exclude // Exclude axis: otherwise 2 Evaluation instances could contain identical stats and
               // fail equality
  protected int axis = 1;
  protected Integer binaryPositiveClass =
      1; // Used *only* for binary classification; default value here to 1 for legacy JSON loading
  protected final int topN;
  protected int topNCorrectCount = 0;
  protected int topNTotalCount =
      0; // Could use topNCountCorrect / (double)getNumRowCounter() - except for eval(int,int),
         // hence separate counters
  ...
  @Getter @Setter protected List<String> labelsList = new ArrayList<>();
  ...
  protected Map<Pair<Integer, Integer>, List<Object>>
      confusionMatrixMetaData; // Pair: (Actual,Predicted)

  /**
   * For stats(): When classes are excluded from precision/recall, what is the maximum number we
   * should print? If this is set to a high value, the output (potentially thousands of classes) can
   * become unreadable.
   */
  @Getter @Setter protected int maxWarningClassesToPrint = 16;
  ...
  /**
   * Constructor for specifying the number of classes, and optionally the positive class for binary
   * classification. See Evaluation javadoc for more details on evaluation in the binary case
   *
   * @param numClasses The number of classes for the evaluation. Must be 2, if binaryPositiveClass
   *     is non-null
  ...
  public Evaluation(int numClasses, Integer binaryPositiveClass) {
    ...
    if (binaryPositiveClass != null) {
      Preconditions.checkArgument(
          binaryPositiveClass == 0 || binaryPositiveClass == 1,
          "Only 0 and 1 are valid inputs for binaryPositiveClass; got " + binaryPositiveClass);
      Preconditions.checkArgument(
          numClasses == 2,
          "Cannot set binaryPositiveClass argument "
              + "when number of classes is not equal to 2 (got: numClasses="
              + numClasses
              + ")");
    }
    this.binaryPositiveClass = binaryPositiveClass;
  ...
  /**
   * The labels to include with the evaluation. This constructor can be used for generating labeled
   * output rather than just numbers for the labels
   *
   * @param labels the labels to use for the output
   */
  ...
  /**
   * Use a map to generate labels Pass in a label index with the actual label you want to use for
   * output
   *
   ...
   * @param topN Value to use for top N accuracy calculation (<=1: standard accuracy). Note that
   *     with top N accuracy, an example is considered 'correct' if the probability for the true
   *     class is one of the highest N values
   */
  ...
  /**
   * Create an evaluation instance with a custom binary decision threshold. Note that binary
   * decision thresholds can only be used with binary classifiers.<br>
   * Defaults to class 1 for the positive class - see class javadoc, and use {@link
   * #Evaluation(double, Integer)} to change this.
   *
   ...
   * Create an evaluation instance with a custom binary decision threshold. Note that binary
   * decision thresholds can only be used with binary classifiers.<br>
   * This constructor also allows the user to specify the positive class for binary classification.
  ...
  /**
   * Created evaluation instance with the specified cost array. A cost array can be used to bias the
   * multi class predictions towards or away from certain classes. The predicted class is determined
   * using argMax(cost * probability) instead of argMax(probability) when no cost array is present.
   *
  ...
  public Evaluation(List<String> labels, INDArray costArray) {
    if (costArray != null && !costArray.isRowVectorOrScalar()) {
      throw new IllegalArgumentException(
          "Invalid cost array: must be a row vector (got shape: "
  ...
  private static List<String> createLabelsFromMap(Map<Integer, String> labels) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      if (str == null)
        throw new IllegalArgumentException(
            "Invalid labels map: missing key for class "
                + i
                + " (expect integers 0 to "
                + (size - 1)
                + ")");
      labelsList.add(str);
    }
  ...
  /**
   * Set the axis for evaluation - this is the dimension along which the probability (and label
   * classes) are present.<br>
   ...
   * For 3D, RNNs/CNN1D (DL4J RnnOutputLayer), NCW format, shape [minibatch, numClasses,
   * sequenceLength] - axis = 1<br>
   * For 3D, RNNs/CNN1D (DL4J RnnOutputLayer), NWC format, shape [minibatch, sequenceLength,
   * numClasses] - axis = 2<br>
   * For 4D, CNN2D (DL4J CnnLossLayer), NCHW format, shape [minibatch, channels, height, width] -
   * axis = 1<br>
   ...
   *
   * <p>Note that an IllegalArgumentException is thrown if the two passed in matrices aren't the
   * same length.
   *
   ...
   * @param recordMetaData Optional; may be null. If not null, should have size equal to the number
   *     of outcomes/guesses
   */
  @Override
  public void eval(
    ...
    Triple<INDArray, INDArray, INDArray> p =
        BaseEvaluation.reshapeAndExtractNotMasked(labels, predictions, mask, axis);
    if (p == null) {
    ...
    Preconditions.checkState(
        maskArray == null, "Per-output masking for Evaluation is not supported");

    // Check for NaNs in predictions - without this, evaulation could silently be intepreted as
    // class 0 prediction due to argmax
    long count =
        Nd4j.getExecutioner()
            .execAndReturn(new MatchCondition(predictions2d, Conditions.isNan()))
            .getFinalResult()
            .longValue();
    Preconditions.checkState(
        count == 0,
        "Cannot perform evaluation with NaNs present in predictions:"
            + " %s NaNs present in predictions INDArray",
        count);
    ...
    // If confusion is null, then Evaluation was instantiated without providing the classes -> infer
    // # classes from
    if (confusion == null) {
    ...
    if (!Arrays.equals(labels2d.shape(), predictions2d.shape())) {
      throw new IllegalArgumentException(
          "Unable to evaluate. Predictions and labels arrays are not same shape."
              + " Predictions shape: "
              + Arrays.toString(predictions2d.shape())
              + ", Labels shape: "
    ...
    if (nCols == 1) {
      INDArray binaryGuesses =
          predictions2d
              .gt(binaryDecisionThreshold == null ? 0.5 : binaryDecisionThreshold)
              .castTo(predictions.dataType());
      ...
      if (binaryDecisionThreshold != null) {
        if (nCols != 2) {
          throw new IllegalStateException(
              "Binary decision threshold is set, but number of columns for "
                  + "predictions is "
                  + nCols
                  + ". Binary decision threshold can only be used for binary "
                  + "prediction cases");
        }
        ...
        guessIndex =
            Nd4j.argMax(predictions2d.mulRowVector(costArray.castTo(predictions2d.dataType())), 1);
      } else {
      ...
      for (int i = 0; i < nExamples; i++) {
        ...
        // if actual == predicted, then it's a true positive, assign true negative to every other
        // label
        if (actual == predicted) {
  ...
  public void eval(int predictedIdx, int actualIdx) {
    ...
    if (predictedIdx == actualIdx) {
      ...
      for (Integer clazz : confusion().getClasses()) {
        if (clazz != predictedIdx) trueNegatives.incrementCount(clazz, 1.0f);
      }
    } else {
      ...
      for (Integer clazz : confusion().getClasses()) {
        if (clazz != predictedIdx && clazz != actualIdx) trueNegatives.incrementCount(clazz, 1.0f);
      }
    }
  }
   ...
   *
   * @param suppressWarnings whether or not to output warnings related to the evaluation results
   * @param includeConfusion whether the confusion matrix should be included it the returned stats
   *     or not
   * @return A (multi-line) String with accuracy, precision, recall, f1 score etc
   */
  ...
  private String stats(
      boolean suppressWarnings, boolean includeConfusion, boolean logConfusionSizeWarning) {
    if (numRowCounter == 0) {
    ...
    double acc = accuracy();
    double precision =
        precision(); // Macro precision for N>2, or binary class 1 (only) precision by default
    ...
    if (nClasses > 2 || binaryPositiveClass == null) {
      builder
          .append("\nPrecision, recall & F1: macro-averaged (equally weighted avg. of ")
          .append(nClasses)
          .append(" classes)");
    }
    if (nClasses == 2 && binaryPositiveClass != null) {
      builder
          .append("\nPrecision, recall & F1: reported for positive class (class ")
          .append(binaryPositiveClass);
      if (labelsList != null) {
    ...
    // "Note that for “micro-averaging in a multiclass setting with all labels included will produce
    // equal precision, recall and F,"
  ...
  public String confusionMatrix() {
    ...
    maxCount =
        Math.max(maxCount, nClasses); // Include this as header might be bigger than actual values
  ...
  private void warningHelper(StringBuilder warnings, List<Integer> list, String metric) {
    ...
    warnings
        .append(" never predicted by the model and ")
        .append(wasWere)
        .append(" excluded from average ")
        .append(metric);
    if (list.size() <= maxWarningClassesToPrint) {
      warnings
          .append("\nClasses excluded from average ")
          .append(metric)
          .append(": ")
          .append(list)
          .append("\n");
  ...
  public double precision(Integer classLabel, double edgeCase) {
    Preconditions.checkState(
        numRowCounter > 0, "Cannot get precision: no evaluation has been performed");
   ...
   * 1. For binary classification, if the positive class is set (via default value of 1, via
   * constructor, or via {@link #setBinaryPositiveClass(Integer)}), the returned value will be for
   * the specified positive class only.<br>
   * 2. For the multi-class case, or when {@link #getBinaryPositiveClass()} is null, the returned
   * value is macro-averaged across all classes. i.e., is macro-averaged precision, equivalent to
   * {@code precision(EvaluationAveraging.Macro)}<br>
   *
  ...
  /**
   * Calculate the average precision for all classes. Can specify whether macro or micro averaging
   * should be used NOTE: if any classes have tp=0 and fp=0, (precision=0/0) these are excluded from
   * the average
   *
  ...
  /**
   * When calculating the (macro) average precision, how many classes are excluded from the average
   * due to no predictions - i.e., precision would be the edge case of 0/0
   *
  ...
  /**
   * When calculating the (macro) average Recall, how many classes are excluded from the average due
   * to no predictions - i.e., recall would be the edge case of 0/0
   *
  ...
  /**
   * When calculating the (macro) average FBeta, how many classes are excluded from the average due
   * to no predictions - i.e., FBeta would be calculated from a precision or recall of 0/0
   *
  ...
  public double recall(int classLabel, double edgeCase) {
    Preconditions.checkState(
        numRowCounter > 0, "Cannot get recall: no evaluation has been performed");
    double tpCount = truePositives.getCount(classLabel);
   ...
   * the specified positive class only.<br>
   * 2. For the multi-class case, or when {@link #getBinaryPositiveClass()} is null, the returned
   * value is macro-averaged across all classes. i.e., is macro-averaged recall, equivalent to
   * {@code recall(EvaluationAveraging.Macro)}<br>
   *
  ...
  /**
   * Calculate the average recall for all classes - can specify whether macro or micro averaging
   * should be used NOTE: if any classes have tp=0 and fn=0, (recall=0/0) these are excluded from
   * the average
   *
   * @param averaging Averaging method - macro or micro
  ...
  public double falsePositiveRate(int classLabel, double edgeCase) {
    Preconditions.checkState(
        numRowCounter > 0, "Cannot get false positive rate: no evaluation has been performed");
    double fpCount = falsePositives.getCount(classLabel);
   ...
   * the specified positive class only.<br>
   * 2. For the multi-class case, or when {@link #getBinaryPositiveClass()} is null, the returned
   * value is macro-averaged across all classes. i.e., is macro-averaged false positive rate,
   * equivalent to {@code falsePositiveRate(EvaluationAveraging.Macro)}<br>
   *
  ...
  /**
   * Calculate the average false positive rate across all classes. Can specify whether macro or
   * micro averaging should be used
   *
   * @param averaging Averaging method - macro or micro
  ...
  public double falseNegativeRate(Integer classLabel, double edgeCase) {
    Preconditions.checkState(
        numRowCounter > 0, "Cannot get false negative rate: no evaluation has been performed");
    double fnCount = falseNegatives.getCount(classLabel);
    double tpCount = truePositives.getCount(classLabel);
  ...
  /**
   * False negative rate based on guesses so far Note: value returned will differ depending on
   * number of classes and settings.<br>
   * 1. For binary classification, if the positive class is set (via default value of 1, via
   * constructor, or via {@link #setBinaryPositiveClass(Integer)}), the returned value will be for
   * the specified positive class only.<br>
   * 2. For the multi-class case, or when {@link #getBinaryPositiveClass()} is null, the returned
   * value is macro-averaged across all classes. i.e., is macro-averaged false negative rate,
   * equivalent to {@code falseNegativeRate(EvaluationAveraging.Macro)}<br>
   *
  ...
  /**
   * False Alarm Rate (FAR) reflects rate of misclassified to classified records <a
   * href="http://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1058&context=isw">http://ro.ecu.edu.au/cgi/viewcontent.cgi?article=1058&context=isw</a>
   * <br>
   * Note: value returned will differ depending on number of classes and settings.<br>
   * 1. For binary classification, if the positive class is set (via default value of 1, via
   * constructor, or via {@link #setBinaryPositiveClass(Integer)}), the returned value will be for
   * the specified positive class only.<br>
   * 2. For the multi-class case, or when {@link #getBinaryPositiveClass()} is null, the returned
   * value is macro-averaged across all classes. i.e., is macro-averaged false alarm rate)
   *
   * @return the fpr for the outcomes
   */
  public double falseAlarmRate() {
    if (binaryPositiveClass != null && numClasses() == 2) {
      return (falsePositiveRate(binaryPositiveClass) + falseNegativeRate(binaryPositiveClass))
          / 2.0;
    }
   ...
   * @param beta Beta value to use
   * @param classLabel Class label
   * @param defaultValue Default value to use when precision or recall is undefined (0/0 for prec.
   *     or recall)
   * @return F_beta
   */
  public double fBeta(double beta, int classLabel, double defaultValue) {
    Preconditions.checkState(
        numRowCounter > 0, "Cannot get fBeta score: no evaluation has been performed");
   ...
   * the specified positive class only.<br>
   * 2. For the multi-class case, or when {@link #getBinaryPositiveClass()} is null, the returned
   * value is macro-averaged across all classes. i.e., is macro-averaged f1, equivalent to {@code
   * f1(EvaluationAveraging.Macro)}<br>
   *
  ...
  public double fBeta(double beta, EvaluationAveraging averaging) {
    ...
    if (nClasses == 2) {
      return EvaluationUtils.fBeta(
          beta,
          (long) truePositives.getCount(1),
          (long) falsePositives.getCount(1),
  ...
  public double gMeasure(int output) {
    Preconditions.checkState(
        numRowCounter > 0, "Cannot get gMeasure: no evaluation has been performed");
  ...
  public double accuracy() {
    Preconditions.checkState(
        numRowCounter > 0, "Cannot get accuracy: no evaluation has been performed");
  ...
  /**
   * Top N accuracy of the predictions so far. For top N = 1 (default), equivalent to {@link
   * #accuracy()}
   *
  ...
  public double matthewsCorrelation(int classIdx) {
    Preconditions.checkState(
        numRowCounter > 0, "Cannot get Matthews correlation: no evaluation has been performed");
  ...
  private Map<Integer, Integer> addMapsByKey(
   ...
   * @return the number of times the label actually occurred
   */
  ...
  /**
   * Return the number of correct predictions according to top N value. For top N = 1 (default) this
   * is equivalent to the number of correct predictions
   *
  ...
  /**
   * Return the total number of top N evaluations. Most of the time, this is exactly equal to {@link
   * #getNumRowCounter()}, but may differ in the case of using {@link #eval(int, int)} as top N
   * accuracy cannot be calculated in that case (i.e., requires the full probability distribution,
   * not just predicted/actual indices)
   *
  ...
  /**
   * Merge the other evaluation object into this one. The result is that this Evaluation instance
  ...
  public void merge(Evaluation other) {
    ...
    if (topN != other.topN) {
      log.warn(
          "Different topN values ({} vs {}) detected during Evaluation merging. Top N accuracy may"
              + " not be accurate.",
          topN,
          other.topN);
    }
   ...
   *
   * <p><b>Note</b>: Prediction errors are ONLY available if the "evaluate with metadata" method is
   * used: {@link #eval(INDArray, INDArray, List)} Otherwise (if the metadata hasn't been recorded
   * via that previously mentioned eval method), there is no value in splitting each prediction out
   * into a separate Prediction object - instead, use the confusion matrix to get the counts, via
   * {@link #getConfusionMatrix()}
   *
  ...
  public List<Prediction> getPredictionErrors() {
    ...
    Collections.sort(
        sorted,
        new Comparator<Map.Entry<Pair<Integer, Integer>, List<Object>>>() {
          @Override
          public int compare(
              Map.Entry<Pair<Integer, Integer>, List<Object>> o1,
  ...
  /**
   * Get a list of predictions, for all data with the specified <i>actual</i> class, regardless of
   * the predicted class.
   *
   * <p><b>Note</b>: Prediction errors are ONLY available if the "evaluate with metadata" method is
   * used: {@link #eval(INDArray, INDArray, List)} Otherwise (if the metadata hasn't been recorded
  ...
  public List<Prediction> getPredictionsByActualClass(int actualClass) {
    ...
    for (Map.Entry<Pair<Integer, Integer>, List<Object>> entry :
        confusionMatrixMetaData.entrySet()) { // Entry Pair: (Actual,Predicted)
  ...
  /**
   * Get a list of predictions, for all data with the specified <i>predicted</i> class, regardless
   * of the actual data class.
   *
   * <p><b>Note</b>: Prediction errors are ONLY available if the "evaluate with metadata" method is
   * used: {@link #eval(INDArray, INDArray, List)} Otherwise (if the metadata hasn't been recorded
   * via that previously mentioned eval method), there is no value in splitting each prediction out
  ...
  /**
   * Get a list of predictions in the specified confusion matrix entry (i.e., for the given
   * actua/predicted class pair)
   *
   * @param actualClass Actual class
   * @param predictedClass Predicted class
   * @return List of predictions that match the specified actual/predicted classes, or null if the
   *     "evaluate with metadata" method was not used
   */
  ...
  public double getValue(IMetric metric) {
    ...
    } else throw new IllegalStateException("Can't get value for non-evaluation Metric " + metric);
  }
