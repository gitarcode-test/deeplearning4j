...
public class RegressionEvaluation extends BaseEvaluation<RegressionEvaluation> {
  ...
  /**
   * Create a regression evaluation object with the specified number of columns, and default
   * precision for the stats() method.
   *
  ...
  /**
   * Create a regression evaluation object with the specified number of columns, and specified
   * precision for the stats() method.
   *
   * @param nColumns Number of columns
   */
  ...
  public RegressionEvaluation(String... columnNames) {
    this(
        columnNames == null || columnNames.length == 0 ? null : Arrays.asList(columnNames),
        DEFAULT_PRECISION);
  }
  ...
  public void eval(INDArray labelsArr, INDArray predictionsArr, INDArray maskArr) {
    ...
    sumSquaredPredicted.addi(
        predictions.mul(predictions).sum(0).castTo(labelsSumPerColumn.dataType()));
    ...
    if (maskArray == null) {
      ...
      newExampleCountPerColumn =
          exampleCountPerColumn.add(maskArray.sum(0).castTo(labelsSumPerColumn.dataType()));
    }
    currentMean
        .muliRowVector(exampleCountPerColumn)
        .addi(labels.sum(0).castTo(labelsSumPerColumn.dataType()))
        .diviRowVector(newExampleCountPerColumn);
    currentPredictionMean
        .muliRowVector(exampleCountPerColumn)
        .addi(predictions.sum(0).castTo(labelsSumPerColumn.dataType()))
  ...
  public String stats() {
    if (!initialized) {
      ...
      if (columnNames == null) columnNames = createDefaultColumnNames(numColumns());
  ...
  public double rootMeanSquaredError(int column) {
    ...
    return Math.sqrt(
        sumSquaredErrorsPerColumn.getDouble(column) / exampleCountPerColumn.getDouble(column));
  }
   ...
   * @deprecated Use {@link #pearsonCorrelation(int)} instead. For the R2 score use {@link
   *     #rSquared(int)}.
   */
  @Deprecated
   ...
   * @see <a
   *     href="https://en.wikipedia.org/wiki/Pearson_correlation_coefficient#For_a_sample">Wikipedia</a>
   */
  public double pearsonCorrelation(int column) {
    ...
    r /=
        Math.sqrt(sumSquaredLabels - exampleCount * labelMean * labelMean)
  ...
  public double relativeSquaredError(int column) {
    ...
    // (sum(predicted^2) - 2 * sum(predicted * actual) + sum(actual ^ 2)) / (sum(actual ^ 2) - n *
    // actualMean)
    double numerator =
        sumSquaredPredicted.getDouble(column)
            - 2 * sumOfProducts.getDouble(column)
            + sumSquaredLabels.getDouble(column);
    double denominator =
        sumSquaredLabels.getDouble(column)
            - exampleCountPerColumn.getDouble(column)
                * currentMean.getDouble(column)
                * currentMean.getDouble(column);
   ...
   * @deprecated Use {@link #averagePearsonCorrelation()} instead. For the R2 score use {@link
   *     #averageRSquared()}.
   */
  @Deprecated
