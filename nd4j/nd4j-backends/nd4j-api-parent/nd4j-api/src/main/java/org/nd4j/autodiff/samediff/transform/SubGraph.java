...
public class SubGraph {
  ...
  public List<SDVariable> outputs() {
    // Outputs: the SDVariables of the root OR child nodes that are not consumed *ONLY* by another
    // op within the subgraph
    ...
    if (childNodes != null && !childNodes.isEmpty()) {
      ...
      if (rootNode.args() != null) Collections.addAll(seenAsInput, rootNode.args());

      for (DifferentialFunction df : childNodes) {
        if (df.args() != null) Collections.addAll(seenAsInput, df.args());
        if (df.outputVariables() != null) Collections.addAll(allOutputs, df.outputVariables());
      }
    }

    // Now: filter all output variables that are consumed *only* by
    // Example subgraph: x -> y -> z... then Y is not an output
    // But suppose same subgraph, but connection y -> a exists; then Y must be an output, because
  ...
  public List<SDVariable> inputs() {
    // Inputs: the SDVariables that are inputs to this subgraph are those used by any of the
