...
public class GradCheckUtil {
  ...
  public static boolean checkGradients(TestCase t) {
    return checkGradients(
        ...
        t.placeholderValues(),
        t.gradCheckEpsilon(),
        t.gradCheckMaxRelativeError(),
        t.gradCheckMinAbsError(),
        t.gradCheckPrint(),
        t.gradCheckDefaultExitFirstFailure(),
        false,
        t.gradCheckDebugMode(),
        t.gradCheckSkipVariables(),
        t.gradCheckMask());
  }
  ...
  public static boolean checkGradients(
    ...
    for (Variable v : sd.getVariables().values()) {
      if (v.getVariable().getVariableType() == VariableType.ARRAY
          || v.getVariable().getVariableType() == VariableType.PLACEHOLDER) {
      ...
      if (v.getVariable().getArr(true) == null) {
        throw new IllegalStateException(
            "Variable \"" + v.getName() + "\" does not have array associated with it");
      }
    }
    ...
    Preconditions.checkState(
        lossFnVariables != null && !lossFnVariables.isEmpty(),
        "Expected 1 or more loss function variables for gradient check, got %s",
        lossFnVariables);

    // TODO also check that all inputs are non-zero (otherwise: consider out = sum(x * y) with all x
    // and y being 0
    ...
    for (Variable v : sd.getVariables().values()) {
      if (v.getVariable().dataType().isFPType()
        ...
        Preconditions.checkNotNull(
            g, "No gradient variable found for variable %s", v.getVariable());
    ...
    for (SDVariable v : sd.variables()) {
      ...
      if (ga == null) {
        throw new IllegalStateException(
            "Null gradient array encountered for variable: " + v.name());
      }
      if (!Arrays.equals(v.getArr().shape(), ga.shape())) {
        throw new IllegalStateException(
            "Gradient shape does not match variable shape for variable \""
                + v.name()
                + "\": shape "
                + Arrays.toString(v.getArr().shape())
                + " vs. gradient shape "
    ...
    for (SDVariable s : sd.variables()) {
      if (fnOutputs.contains(s.name()) || !s.dataType().isFPType()) {
        // This is not an input to the graph, or is not a floating point input (so can't be gradient
        // checked)
        continue;
      }
      ...
      if (s.dataType() != DataType.DOUBLE) {
        log.warn(
            "DataType for variable {} is not double (is: {}) may cause precision issues in gradient"
                + " checks",
            s.name(),
            s.dataType());
      }
      ...
      if (varMask != null) {
        Preconditions.checkState(
            a.equalShapes(varMask),
            "Variable \"%s\": Gradient check mask and array shapes must be equal: got %s vs. mask"
                + " shape %s",
            s.name(),
            a.shape(),
            varMask.shape());
        Preconditions.checkState(
            varMask.dataType() == DataType.BOOL,
            "Variable \"%s\": Gradient check mask must be BOOLEAN datatype, got %s",
            s.name(),
            varMask.dataType());
      }
      ...
      while (iter.hasNext()) {
        ...
        Map<String, INDArray> m =
            sd.output(
                placeholderValues, lossFnVariables); // .get(outName).sumNumber().doubleValue();
        ...
        if (aGrad == null) {
          log.warn(
              "No gradient array for variable \"{}\" was found, skipping variable...", s.name());
          continue;
        }
        ...
        if (Double.isInfinite(analyticGrad) || Double.isNaN(analyticGrad)) {
          throw new IllegalStateException(
              "Analytic (SameDiff) gradient was "
                  + analyticGrad
                  + " for variable \""
                  + name
                  + "\", parameter "
        ...
        if (numericalGrad == 0.0 || analyticGrad == 0.0) {
          ...
          relError =
              Math.abs(analyticGrad - numericalGrad)
                  / (Math.abs(Math.abs(analyticGrad) + Math.abs(numericalGrad)));
        }
          ...
          if (absError < minAbsError) {
            ...
            log.info(
                "Param "
                    ...
                    + ", scoreMinus= "
                    + scoreMinus);
            if (exitOnFirstFailure) return false;
            totalNFailures++;
          }
        } else if (print) {
          log.info(
  ...
  public static boolean checkActivationGradients(ActGradConfig config) {
    ...
    Preconditions.checkState(
        actGrads != null && !actGrads.isEmpty(),
        "No activation gradients were specified to gradient check");
    ...
    for (String s : actGrads) {
      ...
      Preconditions.checkState(
          ...
          "Only variables with type ARRAY may be "
              + "gradient checked using this method. Variable \"%s\" has type %s",
          s,
          v.getVariableType());
      Preconditions.checkState(
          v.dataType().isFPType(),
          "Cannot gradient check activation variable \"%s\": must be floating point type. Is type:"
              + " %s",
          s,
          v.dataType());
      if (v.dataType() != DataType.DOUBLE) {
        log.warn(
            "Floating point variable {} is not double precision - this may result in spurious"
                + " failures due to limited precision. Variable is type: {}",
            s,
            v.dataType());
      }
    }
    ...
    // Collect names of variables to get gradients for - i.e., the names of the GRADIENT variables
    // for the specified activations
    ...
    for (String s : actGrads) {
      ...
      Preconditions.checkState(
          grad != null,
          "Could not get gradient for activation \"%s\": gradient variable is null",
          s);
    ...
    Map<String, INDArray> grads =
        sd.calculateGradients(config.getPlaceholderValues(), new ArrayList<>(varsRequiringGrads));
    ...
    for (String s : actGrads) {
      ...
      INDArray varMask =
          (config.getGradCheckMask() == null ? null : config.getGradCheckMask().get(s));
            ...
            if (config.isPrint())
              log.info(
                  "Param "
                      ...
                      + ", scoreMinus= "
                      + scoreMinus);
            if (config.isExitOnFirstFailure()) return false;
            totalNFailures++;
          }
