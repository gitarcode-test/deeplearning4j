...
public class Reshape extends DynamicCustomOp {
  ...
  public static final int C_ORDER = -99;
  public static final int F_ORDER = -102;
  ...
  public Reshape(SameDiff sameDiff, SDVariable i_v, long[] shape) {
    ...
    // and a dimension.
    if (iArguments.isEmpty()) addIArgument(C_ORDER);
    addIArgument(shape);
    this.reshapeWithViewPossible =
        org.nd4j.linalg.api.shape.Shape.ableToReshapeWithView(
            i_v.getArr(),
            iArguments.get(0) == F_ORDER,
            Longs.toArray(iArguments.subList(1, iArguments.size())));
  }
  ...
  public Reshape(INDArray in, char order, long... shape) {
    ...
    Preconditions.checkState(
        order == 'c' || order == 'f', "Invalid order: must be 'c' or 'f', got %s", order);
    this.shape = shape;
    // c ordering: see (char) 99 for c ordering and (char) 'f' is 102
    // note it has to be negative for the long array case only
    // to flag the difference between an ordering being specified
  ...
  public Reshape(@NonNull INDArray in, @NonNull INDArray shape, INDArray out) {
    ...
    this.reshapeWithViewPossible =
        org.nd4j.linalg.api.shape.Shape.ableToReshapeWithView(
            in, iArguments.get(0) == F_ORDER, shape.toLongVector());
  }
