...
public class NDValidation {
   ...
   * Some operations (such as sum, norm2, add(Number) etc) don't make sense when applied to
   * boolean/utf8 arrays
   *
    ...
    if (v.dataType() == DataType.BOOL || v.dataType() == DataType.UTF8)
      throw new IllegalStateException(
          "Cannot apply operation \""
              + opName
              + "\" to array with non-numerical data type "
              + v.dataType());
  }

  /**
   * Validate that the operation is being applied on numerical INDArrays (not boolean or utf8). Some
    ...
    if (v.dataType() == DataType.BOOL || v.dataType() == DataType.UTF8)
      throw new IllegalStateException(
          "Input \""
              ...
              + "\" for operation \""
              + opName
              + "\" must be an numerical type type;"
              + " got array with non-integer data type "
              + v.dataType());
  }
  ...
  public static void validateNumerical(String opName, INDArray v1, INDArray v2) {
    if (v1.dataType() == DataType.BOOL
        || v1.dataType() == DataType.UTF8
        || v2.dataType() == DataType.BOOL
        || v2.dataType() == DataType.UTF8)
      throw new IllegalStateException(
          "Cannot perform operation \""
              + opName
              + "\" on arrays if one or both variables"
              + " are non-numerical: got "
              + v1.dataType()
              + " and "
              + v2.dataType());
  }
  ...
  public static void validateInteger(String opName, INDArray v) {
    ...
    if (!v.dataType().isIntType())
      throw new IllegalStateException(
          "Cannot apply operation \""
              + opName
              + "\" to array with non-integer data type "
              + v.dataType());
  }
    ...
    if (!v.dataType().isIntType())
      throw new IllegalStateException(
          "Input \""
              ...
              + opName
              + "\" must be an integer"
              + " type; got array with non-integer data type "
              + v.dataType());
  }
      ...
      if (!v[i].dataType().isIntType())
        throw new IllegalStateException(
            "Input \""
                ...
                + opName
                + "\" must be an integer"
                + " type; got array with non-integer data type member"
                + v[i].dataType());
    }
  }
  ...
  public static void validateFloatingPoint(String opName, INDArray v) {
    ...
    if (!v.dataType().isFPType())
      throw new IllegalStateException(
          "Cannot apply operation \""
              + opName
              + "\" to array with non-floating point data type "
              + v.dataType());
  }
    ...
    if (!v.dataType().isFPType())
      throw new IllegalStateException(
          "Input \""
              ...
              + "\" for operation \""
              + opName
              + "\" must be an floating point type; got array with non-floating point data type "
              + v.dataType());
  }
  ...
  public static void validateBool(String opName, INDArray v) {
    ...
    if (v.dataType() != DataType.BOOL)
      throw new IllegalStateException(
          "Cannot apply operation \""
              + opName
              + "\" to array with non-boolean point data type "
              + v.dataType());
  }
    ...
    if (v.dataType() != DataType.BOOL)
      throw new IllegalStateException(
          "Input \""
              ...
              + "\" for operation \""
              + opName
              + "\" must be an boolean variable; got array with non-boolean data type "
              + v.dataType());
  }
  ...
  public static void validateBool(String opName, INDArray v1, INDArray v2) {
    if (v1.dataType() != DataType.BOOL || v2.dataType() != DataType.BOOL)
      throw new IllegalStateException(
          "Cannot perform operation \""
              + opName
              + "\" on array if one or both variables are non-boolean: "
              + v1.dataType()
              + " and "
              + v2.dataType());
  }
  ...
  /**
   * Validate that the operation is being applied on array with the exact same datatypes (which may
   * optionally be restricted to numerical INDArrays only (not boolean or utf8))
   *
   * @param opName Operation name to print in the exception
   * @param numericalOnly If true, the variables must all be the same type, and must be numerical
   *     (not boolean/utf8)
  ...
  public static void validateSameType(String opName, boolean numericalOnly, INDArray... vars) {
    ...
    if (vars.length == 1) {
      ...
      if (numericalOnly) validateNumerical(opName, vars[0]);
      for (int i = 1; i < vars.length; i++) {
        if (first != vars[i].dataType()) {
          ...
          throw new IllegalStateException(
              "Cannot perform operation \""
                  + opName
                  + "\" to arrays with different datatypes:"
                  + " Got arrays with datatypes "
                  + Arrays.toString(dtypes));
        }
      }
    }
  }
