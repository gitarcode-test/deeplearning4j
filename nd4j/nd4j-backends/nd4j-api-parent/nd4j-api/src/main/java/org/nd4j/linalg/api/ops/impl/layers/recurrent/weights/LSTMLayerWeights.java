...
public class LSTMLayerWeights extends RNNWeights {
   ...
   *
   * <p>Input to hidden and hidden to hidden are concatenated in dimension 0, so the input to hidden
   * weights are [:inSize, :] and the hidden to hidden weights are [inSize:, :].
   */
  ...
  public SDVariable[] argsWithInputs(SDVariable... inputs) {
    Preconditions.checkArgument(
        inputs.length == 4,
        "Expected 4 inputs, got %s",
        inputs.length); // Order: x, seqLen, yLast, cLast
    // lstmLayer c++ op expects: x, Wx, Wr, Wp, b, seqLen, yLast, cLast
    return ArrayUtil.filterNull(
        inputs[0], weights, rWeights, bias, inputs[1], inputs[2], inputs[3], peepholeWeights);
  }
  ...
  public INDArray[] argsWithInputs(INDArray... inputs) {
    ...
    // lstmLayer c++ op expects: x, Wx, Wr, Wp, b, seqLen, yLast, cLast
    return ArrayUtil.filterNull(
        inputs[0], iWeights, irWeights, iBias, inputs[1], inputs[2], inputs[3], iPeepholeWeights);
  }
