...
public class LogFileWriter {
  ...
  public long writeGraphStructure(SameDiff sd) throws IOException {
    Preconditions.checkState(
        endStaticInfoOffset < 0,
        "Cannot write graph structure - already wrote end of static info marker");
  ...
  public long writeFinishStaticMarker() throws IOException {
    Preconditions.checkState(
        endStaticInfoOffset < 0, "Wrote final static already information already");
  ...
  public List<Pair<UIEvent, Table>> readEvents() throws IOException {
    // TODO eventually we'll support working out the offset for files that were not written in this
    // session
    Preconditions.checkState(
        endStaticInfoOffset >= 0,
        "Cannot read events - have not written end of static info marker");
    ...
    try (RandomAccessFile f = new RandomAccessFile(file, "r");
      ...
      while (f.getFilePointer() < f.length()) {
        ...
        switch (infoType) {
          ...
          default:
            throw new RuntimeException(
                "Unknown or not yet implemented event type: " + e.eventType());
        }
  ...
  public long registerEventNameQuiet(String name) {
    Preconditions.checkState(
        !registeredEventName(name), "Event name \"%s\" has already been registered", name);
    try {
  ...
  public long registerEventName(String name) throws IOException {
    Preconditions.checkState(
        endStaticInfoOffset >= 0, "Cannot write name - have not written end of static info marker");

    FlatBufferBuilder fbb = new FlatBufferBuilder(0);
    long time = System.currentTimeMillis();
    int offset =
        UIEvent.createUIEvent(
            fbb, UIEventType.ADD_NAME, UIEventSubtype.NONE, -1, time, 0, 0, (short) -1, 0, 0);
    fbb.finish(offset);
   ...
   * @return Number of bytes written
   */
  public long writeScalarEvent(
      String name, EventSubtype subtype, long time, int iteration, int epoch, Number scalar)
      throws IOException {
    // TODO add support for plugin, variable and frame/iter
    Preconditions.checkState(
        indexNameMap.containsKey(name), "Name \"%s\" not yet registered", name);
  ...
  public long writeHistogramEventDiscrete(
      @NonNull String name,
      ...
      throws IOException {
    Preconditions.checkState(
        binLabels == null || binLabels.size() == y.length(),
        "Number of bin labels (if present) must "
            + "be same as Y array length - got %s bins, array shape %ndShape",
        (binLabels == null ? 0L : binLabels.size()),
        y.length());
    Preconditions.checkState(
        y.rank() == 1, "Y array must be rank 1, got Y array with shape %ndShape", y);
    ...
    int offset2 =
        UIHistogram.createUIHistogram(
            fbb2, UIHistogramType.DISCRETE, y.length(), 0, yOffset, binLabelsOffset);
    fbb2.finish(offset2);

    return append(fbb, fbb2);
  }

  public long writeHistogramEventEqualSpacing(
      String name,
      EventSubtype subtype,
      long time,
      int iteration,
    ...
    Preconditions.checkState(
        max > min,
        "Maximum histogram value must be greater than minimum - got max=%s, min=%s",
        max,
        min);
    ...
    int offset2 =
        UIHistogram.createUIHistogram(
            fbb2, UIHistogramType.EQUAL_SPACING, y.length(), binRangesOffset, yOffset, 0);
    fbb2.finish(offset2);

    return append(fbb, fbb2);
  }

  public long writeHistogramEventCustomBins(
      String name,
      EventSubtype subtype,
      long time,
      int iteration,
    ...
    Preconditions.checkState(
        bins.rank() == 2,
        "Bins array must have shape [2,numBins], got bins array with shape %ndShape",
        bins);
    Preconditions.checkState(
        y.length() == bins.size(1),
        "Bins array must have shape [2,numBins], where numBins must match y.length()=%s, got bins"
    ...
    int offset2 =
        UIHistogram.createUIHistogram(
            fbb2, UIHistogramType.CUSTOM, y.length(), binRangesOffset, yOffset, 0);
    fbb2.finish(offset2);
  ...
  protected int encodeGraphStructure(FlatBufferBuilder fbb, SameDiff sd) {
    ...
    for (Map.Entry<String, Variable> e : varMap.entrySet()) {
      ...
      int outputOfOpIdx = 0;
      if (outputOfOp != null) outputOfOpIdx = fbb.createString(outputOfOp);
      ...
      int uiExtraLabelOffset =
          0; // String value - "extra" information to be shown in label. Currently unused
    ...
    for (Map.Entry<String, SameDiffOp> e : opMap.entrySet()) {
      ...
      if (df instanceof Enter
          || df instanceof Exit
          || df instanceof NextIteration) { // Enter, Exit, NextIteration
      ...
      opListOffsets[count++] =
          UIOp.createUIOp(
              fbb, nameIdx, opNameIdx, inputsIdx, outputsIdx, controlDepIdxs, extraLabelOffset);
    }
    int opsListOffset = UIGraphStructure.createOpsVector(fbb, opListOffsets);

    return UIGraphStructure.createUIGraphStructure(
        fbb, inputsOffset, inputPairOffset, outputsOffset, outputsListOffset, opsListOffset);
  }
