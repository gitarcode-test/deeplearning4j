...
public abstract class BaseScalarOp extends BaseOp implements ScalarOp {
  ...
  public BaseScalarOp(INDArray x, INDArray y, INDArray z, Number num) {
    super(x, y, z);
    if (x.isCompressed()) Nd4j.getCompressor().decompressi(x);
  ...
  public List<LongShapeDescriptor> calculateOutputShape(OpContext oc) {
    ...
    LongShapeDescriptor desc =
        x.isEmpty()
            ? LongShapeDescriptor.fromShape(x.shape(), Shape.pickPairwiseDataType(aT, sT))
  ...
  public List<DataType> calculateOutputDataTypes(List<DataType> dataTypes) {
    ...
    Preconditions.checkState(
        ...
        "Expected 1 or more input datatype %s, got input %s",
        getClass(),
        dataTypes);
