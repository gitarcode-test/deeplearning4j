...
public class SDVariable implements Serializable {
  ...
  @Getter protected String varName;
  @Getter @Setter protected VariableType variableType;
  ...
  public SDVariable(
    ...
    if (varType != VariableType.PLACEHOLDER)
      Preconditions.checkState(
          ...
          "Unknown datatype is not allowed for SDVariables (variable name: %s)",
          varName);
    if (varName == null) varName = sameDiff.generateNewVarName(varName, 0, true);
   ...
   *
   * <p>This getter will lazy initialize an array if one is not found based on the associated shape
   * and {@link WeightInitScheme} - if this is possible. If this is not possible (due to shapes
   * being unknown, etc) null is returned
   *
   ...
   *
   * <p>This getter will lazy initialize an array if one is not found based on the associated shape
   * and {@link WeightInitScheme} - if this is possible.<br>
  ...
  public INDArray getArr(boolean enforceExistence) {
    ...
    if (variableType == VariableType.ARRAY && enforceExistence) {
      throw new UnsupportedOperationException(
          "Cannot get array for ARRAY type SDVariable - use SDVariable.exec or SameDiff.output"
              + " instead");
  ...
  /**
   * Alias for the gradient variable - same as {@link #getGradient()}. The gradient variable is the
   * variable that represents the derivative of the loss function with respect to the output of this
   * variable. I.e., if this variable is X and loss function is L, then gradient() returns the
  ...
  public long[] getShape() {
    if (variableType == VariableType.PLACEHOLDER || shape != null) {
      ...
      if (getArr() != null) return getArr().shape();
    }
  ...
  public long[] placeholderShape() {
    if (variableType != VariableType.PLACEHOLDER) {
      throw new IllegalStateException(
          "placeholderShape() can only be used for placeholder variables: variable \""
              + getVarName()
   ...
   * Returns an array with the same shape/size as the input, with values 1 where condition is
   * satisfied, or value 0 otherwise
   *
   * @param name Name of the output variable
   * @param value value argument to use in operation
   * @return Output SDVariable with values 0 (not satisfied) and 1 (where the condition is
   *     satisfied)
   */
   ...
   * Supports broadcasting: if x and y have different shapes and are broadcastable, the output shape
   * is broadcast.<br>
  ...
  public SDVariable mmul(String name, SDVariable other, @NonNull MMulTranspose mMulTranspose) {
    return sameDiff.mmul(
        name,
        this,
        other,
        mMulTranspose.isTransposeA(),
        mMulTranspose.isTransposeB(),
        mMulTranspose.isTransposeResult());
  }
   ...
   * Supports broadcasting: if this and x have different shapes and are broadcastable, the output
   * shape is broadcast.
   *
   * @param name Name of the output variable
   ...
   * Note that if keepDims = true, the output variable has the same rank as the input variable, with
   * the reduced dimensions having size 1. This can be useful for later broadcast operations (such
   * as subtracting the mean along a dimension).<br>
   * Example: if input has shape [a,b,c] and dimensions=[1] then output has shape: keepDims = true:
   * [a,1,c]<br>
   * keepDims = false: [a,c]
   *
   * @param name Output variable name
   * @param keepDims If true: keep the dimensions that are reduced on (as length 1). False: remove
   *     the reduction dimensions
   * @param dimensions Dimensions to reduce over. If dimensions are not specified, full array
   *     reduction is performed
   * @return Output variable: reduced array of rank (input rank - num dimensions) if keepDims =
   *     false, or of rank (input rank) if keepdims = true
   */
   ...
   *
   * @param name Output variable name
   * @param keepDims If true: keep the dimensions that are reduced on (as size 1). False: remove the
   ...
   * keepDims = false: [a,c]
   *
   * @param biasCorrected If true: divide by (N-1) (i.e., sample stdev). If false: divide by N
   *     (population stdev)
   * @param keepDims If true: keep the dimensions that are reduced on (as size 1). False: remove the
   *     reduction dimensions
   * @param dimensions Dimensions to reduce over. If dimensions are not specified, full array
  ...
  /**
   * Norm1 (L1 norm) reduction operation: The output contains the L1 norm for each tensor/subset
  ...
  /**
   * Norm2 (L2 norm) reduction operation: The output contains the L2 norm for each tensor/subset
   * along the specified dimensions:<br>
  ...
  /**
   * Max norm (infinity norm) reduction operation: The output contains the max norm for each
  ...
  /**
   * Reshape the current variable to the specified (dynamic) shape. The output variable will have
   * the same values as the input, but with the specified shape.<br>
  ...
  /**
   * Reshape the current variable to the specified shape. The output variable will have the same
   * values as the input, but with the specified shape.<br>
   * Note that prod(shape) must match length(input) == prod(input.shape)
   *
   * @param newShape New shape for variable
   * @return Output variable
   ...
   * <br>
   * Example: if the current variable has shape [a,b,c] and dimensions = [2,0,1] the output has
   * shape [c,a,b]
   *
  ...
  public String toString() {
    return "SDVariable(name=\""
        ...
        + ",dtype="
        + dataType
        + (variableType == VariableType.PLACEHOLDER && shape != null
   ...
   * Control dependencies can be used to enforce the execution order. For example, if a control
   * dependency X->Y exists, then Y will only be executed after X is executed - even if Y wouldn't
   * normally depend on the result/values of X.
   *
  ...
  public void addControlDependency(SDVariable controlDependency) {
    ...
    if (vThis.getOutputOfOp() != null && vCD.getOutputOfOp() != null) {
      ...
      if (oThis.getControlDeps() == null) oThis.setControlDeps(new ArrayList<>());
      if (!oThis.getControlDeps().contains(oCD.getName()))
        oThis.getControlDeps().add(oCD.getName());

      if (oCD.getControlDepFor() == null) oCD.setControlDepFor(new ArrayList<>());
  ...
  public SDVariable getView(SDIndex... indices) {
    ...
    for (int i = 0; i < indices.length; i++) {
      ...
      switch (indices[i].getIndexType()) {
        case INTERVAL:
          indicesVars[i] =
              CreateView.createInterval(
                  sameDiff,
                  indices[i].getIntervalBegin(),
                  indices[i].getIntervalEnd(),
                  indices[i].getIntervalStrides(),
                  indices[i].isInclusive() ? 1 : 0);
          break;
        case POINT:
        ...
        case INTERVAL_INPUT:
          indicesVars[i] =
              CreateView.createInterval(
                  sameDiff,
                  indices[i].getIntervalInputBegin(),
                  indices[i].getIntervalInputEnd(),
                  indices[i].getIntervalStrideInput(),
                  indices[i].getInclusiveInput());
          break;
        case ALL:
  ...
  public SDVariable get(SDIndex... indices) {
    ...
    for (int i = 0; i < indices.length; i++) {
      if (indices[i].getIndexType() == SDIndex.IndexType.POINT_INPUT
          || indices[i].getIndexType() == SDIndex.IndexType.INTERVAL_INPUT) {
        variableIndices = true;
      }

      // convert indices to SDVariable based indices
      if (variableIndices
          && (indices[i].getIndexType() == SDIndex.IndexType.INTERVAL
              || indices[i].getIndexType() == SDIndex.IndexType.POINT)) {
        switch (indices[i].getIndexType()) {
          case INTERVAL:
            indices[i] =
                SDIndex.interval(
                    sameDiff.constant(indices[i].getIntervalBegin()),
                    sameDiff.constant(indices[i].getIntervalEnd()),
                    sameDiff.constant(indices[i].getIntervalEnd()));
            break;
          case POINT:
            indices[i] =
                SDIndex.point(
                    sameDiff.constant(indices[i].getPointIndex()), indices[i].isPointKeepDim());
            break;
        }
      }
    }
    ...
    for (int i = 0; i < ndims; i++) {
      ...
      if (indexType == SDIndex.IndexType.ALL) {
        ...
        end_mask_arr[i] = 1;
          ...
          || indexType == SDIndex.IndexType.POINT_INPUT) {
        ...
        if (!index.isPointKeepDim()) {
          ...
          || indexType == SDIndex.IndexType.INTERVAL_INPUT) {
    ...
    if (variableIndices) {
      ...
      return this.sameDiff.stridedSlice(
          this, beginVar, endVar, stridesVar, begin_mask, end_mask, 0, 0, shrink_axis);
    } else {
      return this.sameDiff.stridedSlice(
          this, begin, end, strides, begin_mask, end_mask, 0, 0, shrink_axis);
    }
  }
  ...
  public static SDVariable sliceEnd(SDVariable input, SDVariable sliceIndexInput) {
    ...
    SDVariable range =
        sameDiff.range(sameDiff.constant(0), input.rank(), sameDiff.constant(1), DataType.INT64);
    // 0 1 1
   ...
   * Can be used (for example) to get rows, columns, sub-matrices, etc.
   *
   * <p>This will loop over the indices (think of it as a list) and concatenate each slice of the
   * input array to the final result.
   *
   * <p>Expected input for indices would be a vector with indices such as 0,1,2,3,4. For each
   * element in the index we then concatenate the result to the previous iteration.
   *
   * <p>Note that this is slow and should only be used in very specific circumstances. Otherwise
   * {@link org.nd4j.linalg.api.ops.impl.shape.StridedSlice} will be more performant for creating
   * views. Many times {@link org.nd4j.linalg.api.ops.impl.shape.StridedSlice} avoids this slower
   * approach by directly calculating the strides of a view.
   *
   * @param indices Indices to get
   * @return Sub-array variable
   */
  public SDVariable get(SDVariable indices) {
    ...
    SDVariable startResult =
        sameDiff.slice(
            this,
            initialSize.castTo(DataType.INT64),
            sliceEnd(this, sameDiff.onesLike(shape()).castTo(DataType.INT64)));
    // start at 1 because we start with the initial output (basically the item at the first element
    // in the indices)
    SDVariable currIteration = sameDiff.var(Nd4j.ones(1).castTo(DataType.INT32));
    // this condition is normally used when you want to toss in an extra condition to terminate
    // early
    ...
    return this.sameDiff
        .loopWithConditions(
            ControlFlow.LoopParams.builder()
                .functionBody(loop)
                .loopVars(
                    new SDVariable[] {
                      currIteration, indicesLength, cond, startResult, this, indices
                    })
                .functionBodyInputs(
                    new String[] {
                      // note here all inputs are the same as the outputs, and we return the
                      // original
                      ...
                      "index", "max", "cond", "input", "pullFrom", "indices"
                    })
                .functionBodyOutputs(
                    new String[] {"index", "max", "cond", "output", "pullFrom", "indices"})
                .functionName("slices")
                .loopName("outputs")
                // note the ordering here is important. Output is the accumulated output of each
                // iteration appending
                // a result to the previous iteration. We start with the initial input and add more
                // overtime.
                .build())[3];
  }
   ...
   * Can be used (for example) to get rows, columns, sub-matrices, etc.
   *
   * <p>This will loop over the indices (think of it as a list) and add each slice specified by the
   * indices from the source to the new array.
   *
   * <p>The end result will be this variable but with the new updated results.
   *
   * <p>Note that this is slow and should only be used in very specific circumstances. Otherwise
   * {@link org.nd4j.linalg.api.ops.impl.shape.StridedSlice} will be more performant for creating
   * views. Many times {@link org.nd4j.linalg.api.ops.impl.shape.StridedSlice} avoids this slower
    ...
    return this.sameDiff
        .loopWithConditions(
            ControlFlow.LoopParams.builder()
                ...
                .loopVars(
                    new SDVariable[] {
                      currIteration, indicesLength, cond, this, toPut, indices, putIndices
                    })
                .functionBodyInputs(
                    new String[] {
                      // note here all inputs are the same as the outputs, and we return the
                      // original
                      // the default  3 values (current iteration, max index to loop to and optional
                      // condition)
                      // index,max,cond,assignTo,putIn,indices,indicesPut
                      "index", "max", "cond", "assignTo", "toPut", "indices", "indicesPut"
                    })
                .functionBodyOutputs(
                    new String[] {
                      "index", "max", "cond", "assignOutput", "toPut", "indices", "indicesPut"
                    })
                .functionName("sliceputs")
                .loopName("outputs")
                // note the ordering here is important. Output is the original array where we
                // assigned values.
                .build())[3];
  }

  /**
   * Create a graph that takes in the indices as a placeholder, loops over each element in the index
   * vector and appends the slice to the end result. This graph is equivalent to something like:
   * INDArray input = ....; INDArray indices = ...; INDArray result =
   * input.get(NDArrayIndex.point(indices.getInt(0)); for(int i = i; i < maxIndex &&
   * customInputResult; i++) { result = Nd4j.concat(0,input.get(NDArrayIndex.point(i))); } return
   * result
   *
   * <p>Note this is similar to {@link INDArray#get(INDArray)}
   *
   * @param relative the expected target input variable. We use this to pull expected return data
   *     type for the result
  ...
  public static SameDiff createLoopPut(SDVariable relative, SDVariable indices) {
    ...
    SDVariable indexToRetrieve =
        indicesLoop
            .getView(SDIndex.point(index))
            .reshape(1)
            .castTo("indexToReceive", DataType.INT64);
    SDVariable indexToPut =
        indicesPut.getView(SDIndex.point(index)).reshape(1).castTo("indexToPut", DataType.INT64);
    ...
    // ensure the output depends on the final assign so it gets executed, return the final output as
    // a view
  ...
  public static SameDiff createLoopConcat(SDVariable relative, SDVariable indices) {
    ...
    // the current index to retrieve
    SDVariable indexToRetrieve =
        indicesLoop.get(SDIndex.point(index)).reshape(1).castTo("indexToReceive", DataType.INT64);

    // the final concatenated output
    SDVariable sliceOutput =
        loop.expandDims("outputSlice", pullFrom.get(SDIndex.point(indexToRetrieve)), 0);
    SDVariable output = loop.concat("output", 0, input, sliceOutput);
    return loop;
  }

  /**
   * Convert this variable to a constant. This is equivalent to "freezing" a variable so that it's
   * value won't be changed by further training.<br>
  ...
  /**
   * Rename this variable to a new name. Equivalent to {@link SameDiff#renameVariable(String,
   * String)}
   *
  ...
  /**
   * Mark this variable as a loss function variable. This means that this variable will be minimized
   * via backprop during training.<br>
  ...
  /**
   * Determine if this variable has a gradient with respect to the current loss. Note that: (a)
   * Non-floating-point variables (integer, string, etc) will never have gradients<br>
   ...
   * (c) Floating point variables may not have any gradient if the current loss does not depend on
   * the variable at all<br>
   *
  ...
  public boolean equals(Object o) {
    ...
    if (!(o instanceof SDVariable)) return false;

    SDVariable s = (SDVariable) o;
    if (!varName.equals(s.varName)) return false;
    if (variableType != s.variableType) return false;
    if (dataType != s.dataType) return false;
