...
public class NormalizerSerializer {
   ...
   * @throws IOException
   */
  public void write(@NonNull Normalizer normalizer, @NonNull OutputStream stream)
      throws IOException {
  ...
  /**
   * Get the default serializer configured with strategies for the built-in normalizer
   * implementations
   *
  ...
  private NormalizerSerializerStrategy getStrategy(Normalizer normalizer) {
    ...
    throw new RuntimeException(
        String.format(
            "No serializer strategy found for normalizer of class %s. If this is a custom"
                + " normalizer, you probably forgot to register a corresponding custom serializer"
                + " strategy with this serializer.",
            normalizer.getClass()));
  }
  ...
  /**
   * Check if a serializer strategy supports a normalizer. If the normalizer is a custom opType, it
   * checks if the supported normalizer class matches.
   *
   * @param strategy
   ...
   */
  private boolean strategySupportsNormalizer(
      NormalizerSerializerStrategy strategy,
  ...
  private Header parseHeader(InputStream stream) throws IOException {
    ...
    if (!header.equals(HEADER)) {
      throw new IllegalArgumentException(
          "Could not restore normalizer: invalid header. If this normalizer was saved with a"
              + " opType-specific strategy like StandardizeSerializerStrategy, use that class to"
              + " restore it as well.");
    }
    ...
    if (version != 1) {
      throw new IllegalArgumentException(
          "Could not restore normalizer: invalid version (" + version + ")");
    }
    ...
    if (type.equals(NormalizerType.CUSTOM)) {
      ...
      Class<? extends NormalizerSerializerStrategy> strategyClass =
          ND4JClassLoading.loadClassByName(strategyClassName);
