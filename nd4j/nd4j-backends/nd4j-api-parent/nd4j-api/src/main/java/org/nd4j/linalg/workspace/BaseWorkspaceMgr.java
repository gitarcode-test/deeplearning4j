...
public abstract class BaseWorkspaceMgr<T extends Enum<T>> implements WorkspaceMgr<T> {
  private static final boolean DISABLE_LEVERAGE =
      false; // Mainly for debugging/optimization purposes
  ...
  protected void recordWorkspaceEvent(
      WorkspaceUseMetaData.EventTypes eventType, MemoryWorkspace workspace, T arrayType) {
    if (workspace == null) return;
    WorkspaceUseMetaData workspaceUseMetaData =
        WorkspaceUseMetaData.builder()
  ...
  public void setWorkspace(
      @NonNull T forEnum, @NonNull String wsName, @NonNull WorkspaceConfiguration configuration) {
  ...
  public void assertOpen(T arrayType, String msg) throws ND4JWorkspaceException {
    if (!scopeOutOfWs.contains(arrayType) && !isWorkspaceOpen(arrayType)) {
      throw new ND4JWorkspaceException(
          "Assertion failed: expected workspace for array type "
              + arrayType
              + " to be open: "
              + msg);
    }
  ...
  public void setCurrentWorkspace(T arrayType) {
    ...
    if (scopeOutOfWs.contains(arrayType)) {
      ...
      MemoryWorkspace workspaceForCurrentThread =
          workspaceManager.getWorkspaceForCurrentThread(workspaceName);
  ...
  public void assertCurrentWorkspace(@NonNull T arrayType, String msg) {
    ...
    if (!scopeOutOfWs.contains(arrayType)
        && (curr == null || !getWorkspaceName(arrayType).equals(curr.getId()))) {
      throw new ND4JWorkspaceException(
          "Assertion failed: expected current workspace to be \""
              + getWorkspaceName(arrayType)
              + "\" (for array type "
              + arrayType
              + ") - actual current workspace is "
              + (curr == null ? null : curr.getId())
              + (msg == null ? "" : ": " + msg));
    }
    ;
  }
        ...
        if (!array.data().getParentWorkspace().getId().equals(getWorkspaceName(arrayType))) {
          throw new IllegalStateException(
              "Array of type "
                  + arrayType
                  + " is leveraged from "
                  + array.data().getParentWorkspace().getId()
                  + " to "
                  + getWorkspaceName(arrayType)
                  + " but WorkspaceMgn.leverageTo() is currently disabled");
        }
      }
      return array;
    }
  ...
  @Override
  public INDArray validateArrayLocation(
      @NonNull T arrayType,
    ...
    if (scopeOutOfWs.contains(arrayType)) {
      ...
      if (!ok) {
        if (migrateIfInvalid) {
          ...
          throw new ND4JWorkspaceException(
              "Array workspace validation failed: Array of type "
                  + arrayType
                  + " should be detached (no workspace) but is in workspace: "
                  + array.data().getParentWorkspace().getId());
        }
      } else {
  ...
  /**
   * This method creates INDArray of specified dataType and shape, and puts it into Workspace, if
   * any. has with respect to the java deallocator service.
   *
   * <p>All crashes now seem to be induced by java side free calls.
   *
   * <p>We need to figure out where these free calls are coming from and why they crash everything.
   * The guess is the way deallocation + workspace closing works. This is the most recent set of
   * changes made.
   *
  ...
  @Override
  public INDArray createUninitialized(
      @NonNull T arrayType, @NonNull DataType dataType, @NonNull long[] shape, char order) {
    enforceExistsAndActive(arrayType);
    try (MemoryWorkspace ws = notifyScopeBorrowed(arrayType)) {
  ...
  public INDArray castTo(
    ...
    if (toCast.dataType() == dataType) {
      if (!dupIfCorrectType) {
        ...
        if (!toCast.isAttached()
            || toCast.data().getParentWorkspace().getId().equals(workspaceNames.get(arrayType))) {
          return toCast;
        }
      }
  ...
  private void validateConfig(@NonNull T arrayType) {
    ...
    if (!configMap.containsKey(arrayType)) {
      throw new ND4JWorkspaceException(
          "No workspace configuration has been provided for arrayType: " + arrayType);
    }
    if (!workspaceNames.containsKey(arrayType)) {
      throw new ND4JWorkspaceException(
          "No workspace name has been provided for arrayType: " + arrayType);
    }
  }
  ...
  private void enforceExistsAndActive(@NonNull T arrayType) {
    ...
    if (!Nd4j.getWorkspaceManager().checkIfWorkspaceExistsAndActive(name)) {
      ...
      stringBuilder
          .append("Workspace \"")
          .append(name)
          .append("\" for array type ")
          .append(arrayType)
          .append(" is not open. Workspaces open: ")
          .append(allOpenWorkspaces());

      MemoryWorkspaceManager workspaceManager = Nd4j.getWorkspaceManager();
      List<MemoryWorkspace> allWorkspacesForCurrentThread =
          workspaceManager.getAllWorkspacesForCurrentThread();
