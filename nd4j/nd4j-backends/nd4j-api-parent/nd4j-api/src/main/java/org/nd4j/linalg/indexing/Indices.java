...
public class Indices {
   ...
   *
   * <p>For c ordering this is just the index itself. For fortran ordering, the following algorithm
   * is used.
   *
   * <p>Assuming an ndarray is a list of vectors. The index of the vector relative to the given
   * index is calculated.
   *
   * <p>vectorAlongDimension is then used along the last dimension using the computed index.
   *
   * <p>The offset + the computed column wrt the index: (index % the size of the last dimension)
  ...
  public static int rowNumber(int index, INDArray arr) {
    ...
    if (arr.vectorsAlongDimension(-1) > Integer.MAX_VALUE) throw new ND4JArraySizeException();
  ...
  public static long[] offsets(long[] shape, INDArrayIndex... indices) {
    ...
    if (indices.length == shape.length) {
      ...
      if (numPoints > 0) {
        ...
        for (int i = 0; i < indices.length; i++)
          if (indices[i].offset() > 0) nonZeros.add(indices[i].offset());
        if (nonZeros.size() > shape.length)
          ...
          for (int i = 0; i < nonZeros.size(); i++) ret[i] = nonZeros.get(i);
        }
      } else {
        int shapeIndex = 0;
        for (int i = 0; i < indices.length; i++) {
   ...
   *
   * <p>Think of this as what fills in the indices for numpy or matlab: Given a which is (4,3,2) in
   * numpy:
   *
   * <p>a[1:3] is filled in by the rest to give back the full slice
   *
   * <p>This algorithm fills in that delta
   *
  ...
  public static INDArrayIndex[] fillIn(int[] shape, INDArrayIndex... indexes) {
    if (shape.length == indexes.length) return indexes;
   ...
   *
   * <p>The shape is defined as (for each dimension) the difference between the end index + 1 and
   * the begin index
   *
  ...
  /**
   * Create an n dimensional index based on the given interval indices. Start and end represent the
   * begin and end of each interval
   *
   * @param start the start indexes
   * @param end the end indexes
   * @return the interval index relative to the given start and end indices
   */
    ...
    if (start.length() != end.length())
      ...
      if (start.length() > Integer.MAX_VALUE)
        throw new ND4JIllegalStateException(
            "Can't proceed with INDArray with length > Integer.MAX_VALUE");
   ...
   * @param inclusive whether the last index should be included
   * @return the ndarray indexes covering each dimension
   */
  public static INDArrayIndex[] createFromStartAndEnd(
      INDArray start, INDArray end, boolean inclusive) {
    if (start.length() != end.length())
      throw new IllegalArgumentException("Start length must be equal to end length");
    else {
      if (start.length() > Integer.MAX_VALUE)
   ...
   * the begin index
   *
   * <p>If specified, this will check for whether any of the indices are >= to end - 1 and if so,
   * prune it down
   *
   * @param shape the original shape
   * @param indices the indices to calculate the shape for
  ...
  public static long[] shape(long[] shape, INDArrayIndex... indices) {
    ...
    if (newAxesPrepend > 0) {
      for (int i = 0; i < newAxesPrepend; i++) accumShape.add(0, 1L);
    }

    /**
     * For each dimension where we want to prepend a dimension we need to add it at the index such
     * that we account for the offset of the number of indexes added up to that point.
     *
     * <p>We do this by doing an offset for each item added "so far"
     *
     * <p>Note that we also have an offset of - 1 because we want to prepend to the given index.
     *
     * <p>When prepend new axes for in the middle is triggered i is already > 0
     */
   ...
   * @return true if the given indexes are searching for a scalar false otherwise
   */
  public static boolean isScalar(INDArray indexOver, INDArrayIndex... indexes) {
    ...
    if (allOneLength && numNewAxes == 0 && indexes.length == indexOver.rank()) return true;
