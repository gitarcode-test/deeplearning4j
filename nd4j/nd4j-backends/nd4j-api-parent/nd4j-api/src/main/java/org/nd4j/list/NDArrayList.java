...
public class NDArrayList extends BaseNDArrayList<Double> {
  ...
  /**
   * Initialize with the desired size. This will set the list.size() to be equal to the passed in
   * size
   *
   ...
   * @param size the initial size of the array. This will set list.size() to be equal to the passed
   *     in size.
   */
  public NDArrayList(@NonNull INDArray container, int size) {
    Preconditions.checkState(
        container == null || container.rank() == 1,
        "Container must be rank 1: is rank %s",
  ...
  /**
   * Get a view of the underlying array relative to the size of the actual array. (Sometimes there
   * are overflows in the internals but you want to use the internal INDArray for computing
   * something directly, this gives you the relevant subset that reflects the content of the list)
   *
  ...
  public boolean add(Double aDouble) {
    if (container == null) {
      ...
      newContainer.put(
          new INDArrayIndex[] {NDArrayIndex.interval(0, container.length())}, container);
  ...
  public boolean remove(Object o) {
    ...
    container.put(
        new INDArrayIndex[] {NDArrayIndex.interval(idx, container.length())},
        container.get(NDArrayIndex.interval(idx + 1, container.length())));
  ...
  public boolean addAll(Collection<? extends Double> collection) {
    if (collection instanceof NDArrayList) {
      ...
      container.put(
          new INDArrayIndex[] {NDArrayIndex.interval(size, size + collection.size())},
          ndArrayList.container.get(NDArrayIndex.interval(0, ndArrayList.size())));
  ...
  private void moveBackward(int index) {
    ...
    INDArrayIndex[] getRange =
        new INDArrayIndex[] {NDArrayIndex.interval(index + 1, index + 1 + numMoved)};
  ...
  private void moveForward(int index) {
    ...
    INDArrayIndex[] first =
        new INDArrayIndex[] {NDArrayIndex.interval(index + 1, index + 1 + get.length())};
