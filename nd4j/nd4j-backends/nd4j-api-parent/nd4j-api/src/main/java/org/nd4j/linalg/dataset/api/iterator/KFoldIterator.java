...
public class KFoldIterator implements DataSetIterator {
  ...
  /**
   * Create a k-fold cross-validation iterator given the dataset and k=10 train-test splits. N
   * number of samples are split into k batches. The first (N%k) batches contain (N/k)+1 samples,
   * while the remaining batches contain (N/k) samples. In case the number of samples (N) in the
   * dataset is a multiple of k, all batches will contain (N/k) samples.
   *
  ...
  /**
   * Create an iterator given the dataset with given k train-test splits N number of samples are
   * split into k batches. The first (N%k) batches contain (N/k)+1 samples, while the remaining
   * batches contain (N/k) samples. In case the number of samples (N) in the dataset is a multiple
   * of k, all batches will contain (N/k) samples.
   *
  ...
  /**
   * The number of examples in every fold is (N / k), except when (N % k) > 0, when the first (N %
   * k) folds contain (N / k) + 1 examples
   *
