...
public class EvaluationBinary extends BaseEvaluation<EvaluationBinary> {
  ...
  // Because we want evaluation to work for large numbers of examples - and with low precision
  // (FP16), we won't
  ...
  protected EvaluationBinary(
      int axis, ROCBinary rocBinary, List<String> labels, INDArray decisionThreshold) {
    this.axis = axis;
   ...
   *
   * @param decisionThreshold Decision threshold for each output; may be null. Should be a row
   *     vector with length equal to the number of outputs, with values in range 0 to 1. An array of
   *     0.5 values is equivalent to the default (no manually specified decision threshold).
   */
  public EvaluationBinary(INDArray decisionThreshold) {
    if (decisionThreshold != null) {
      ...
      if (decisionThreshold.minNumber().doubleValue() < 0.0) {
        throw new IllegalArgumentException(
            "Invalid decision threshold array: minimum value is less than 0");
      }
  ...
  /**
   * This constructor allows for ROC to be calculated in addition to the standard evaluation
   * metrics, when the rocBinarySteps arg is non-null. See {@link ROCBinary} for more details
   *
  ...
  public void eval(INDArray labelsArr, INDArray predictionsArr, INDArray maskArr) {
    ...
    long count =
        Nd4j.getExecutioner()
            .execAndReturn(new MatchCondition(predictionsArr, Conditions.isNan()))
            .getFinalResult()
            .longValue();
    ...
    if (countTruePositive != null && countTruePositive.length != labelsArr.size(axis)) {
      throw new IllegalStateException(
          "Labels array does not match stored state size. Expected labels array with "
              + "size "
              + countTruePositive.length
              + ", got labels array with size "
              + labelsArr.size(axis)
              + " for axis "
              + axis);
    }
    ...
    Triple<INDArray, INDArray, INDArray> p =
        BaseEvaluation.reshapeAndExtractNotMasked(labelsArr, predictionsArr, maskArr, axis);
    INDArray labels = p.getFirst();
    INDArray predictions = p.getSecond();
    INDArray maskArray = p.getThird();

    if (labels.dataType() != predictions.dataType()) labels = labels.castTo(predictions.dataType());
    ...
    // First: binarize the network prediction probabilities, threshold 0.5 unless otherwise
    // specified
    ...
    INDArray truePositives =
        classPredictions.mul(labels); // 1s where predictions are 1, and labels are 1. 0s elsewhere
    INDArray trueNegatives =
        notClassPredictions.mul(
            notLabels); // 1s where predictions are 0, and labels are 0. 0s elsewhere
    INDArray falsePositives =
        classPredictions.mul(notLabels); // 1s where predictions are 1, labels are 0
    INDArray falseNegatives =
        notClassPredictions.mul(labels); // 1s where predictions are 0, labels are 1
  ...
  /**
   * Merge the other evaluation object into this one. The result is that this {@link
   * #EvaluationBinary} instance contains the counts etc from both
   *
  ...
  public void merge(EvaluationBinary other) {
    ...
    if (countTruePositive == null) {
      ...
      if (this.countTruePositive.length != other.countTruePositive.length) {
        throw new IllegalStateException(
            "Cannot merge EvaluationBinary instances with different sizes. This "
                + "size: "
                + this.countTruePositive.length
                + ", other size: "
  ...
  /**
   * Returns the number of labels - (i.e., size of the prediction/labels arrays) - if known. Returns
   * -1 otherwise
   */
  ...
  public int totalCount(int outputNum) {
    ...
    return countTruePositive[outputNum]
        + countTrueNegative[outputNum]
        + countFalseNegative[outputNum]
  ...
  public double accuracy(int outputNum) {
    assertIndex(outputNum);
    return (countTruePositive[outputNum] + countTrueNegative[outputNum])
        / (double) totalCount(outputNum);
  }
  ...
  public double precision(int outputNum) {
    ...
    return countTruePositive[outputNum]
        / (double) (countTruePositive[outputNum] + countFalsePositive[outputNum]);
  }
  ...
  public double recall(int outputNum) {
    assertIndex(outputNum);
    return countTruePositive[outputNum]
        / (double) (countTruePositive[outputNum] + countFalseNegative[outputNum]);
  }
  ...
  /**
   * Macro average of the Matthews correlation coefficient (MCC) (see {@link
  ...
  private void assertIndex(int outputNum) {
    ...
    if (outputNum < 0 || outputNum >= countTruePositive.length) {
      throw new IllegalArgumentException(
          "Invalid input: output number must be between 0 and "
              + (outputNum - 1)
              + ". Got index: "
              + outputNum);
    }
  ...
  public String stats(int printPrecision) {
    ...
    String pattern =
        "%-"
            ...
            + subPattern
            + subPattern
            + subPattern // Accuracy, f1, precision, recall
            + "%-8d%-7d%-7d%-7d%-7d"; // Total count, TP, TN, FP, FN

    String patternHeader =
        "%-" + (maxLabelsLength + 5) + "s%-12s%-12s%-12s%-12s%-8s%-7s%-7s%-7s%-7s";

    List<String> headerNames =
        Arrays.asList(
            "Label", "Accuracy", "F1", "Precision", "Recall", "Total", "TP", "TN", "FP", "FN");
  ...
  public double getValue(IMetric metric) {
    if (metric instanceof Metric) {
      switch ((Metric) metric) {
        ...
        default:
          throw new IllegalStateException(
              "Can't get value for non-binary evaluation Metric " + metric);
      }
    } else
