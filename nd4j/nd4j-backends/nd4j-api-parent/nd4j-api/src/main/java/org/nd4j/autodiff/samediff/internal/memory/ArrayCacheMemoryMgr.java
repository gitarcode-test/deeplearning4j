...
public class ArrayCacheMemoryMgr extends AbstractMemoryMgr {
  ...
  public static final double DEFAULT_MAX_MEM_FRACTION = 0.25;
  public static final long DEFAULT_SMALL_ARRAY_THRESHOLD = 1024;
  public static final double DEFAULT_LARGE_ARRAY_MAX_MULTIPLE = 2.0;
  ...
  @Getter @Setter private static AtomicDouble maxMemFrac;
  ...
  private static boolean enableCache =
      Boolean.parseBoolean(
          System.getProperty(ND4JSystemProperties.SAMEDIFF_MEMORY_CACHE_ENABLE, "true"));
  ...
  private static Set<Long> getLruCacheForThread() {
    if (lruCache.get() != null) return lruCache.get();
    else {
      lruCache.set(new ConcurrentSkipListSet<>());
      return lruCache.get();
    }
  ...
  private static Table<DataType, String, List<INDArray>> getArraysForThread() {
    if (arrays.get() != null) return arrays.get();
    else {
      arrays.set(HashBasedTable.create());
      return arrays.get();
    }
  ...
  private static Map<Long, INDArray> getLruCachedValuesForThread() {
    if (lruCacheValues.get() != null) return lruCacheValues.get();
    else {
      lruCacheValues.set(new ConcurrentHashMap<>());
  ...
  public static void setCacheDefaults() {
    maxMemFrac =
        new AtomicDouble(
            Double.parseDouble(
                System.getProperty(
                    ND4JSystemProperties.CACHE_MEM_FRACTION,
                    String.valueOf(DEFAULT_MAX_MEM_FRACTION))));
    smallArrayThreshold =
        new AtomicLong(
            Long.parseLong(
                System.getProperty(
                    ND4JSystemProperties.SMALL_ARRAY_THRESHOLD,
                    String.valueOf(DEFAULT_SMALL_ARRAY_THRESHOLD))));
    largerArrayMaxMultiple =
        new AtomicDouble(
            Double.parseDouble(
                System.getProperty(
                    ND4JSystemProperties.LARGE_ARRAY_MAX_MULTIPLE,
                    String.valueOf(DEFAULT_LARGE_ARRAY_MAX_MULTIPLE))));
  ...
  @Getter @Setter private static AtomicLong smallArrayThreshold;
  ...
  public synchronized INDArray allocate(boolean detached, DataType dataType, long... shape) {
    ...
    if (arraysForThread.contains(dataType, arrayShapeString) && enableCache) {
      ...
      while (!arrFound) {
        arr =
            !arraysForThread.get(dataType, arrayShapeString).isEmpty()
                ? arraysForThread.get(dataType, arrayShapeString).remove(0)
                : null;
        if (arr != null && (!arr.closeable() || arr.wasClosed() || arr.isView())) {
          log.trace(
              "Found array closeable, not returning from cache. Only closeable arrays are"
                  + " returnable from the cache.");
          if (arr.isView()) arr.setCloseable(false);
          log.trace(
              "Found view array with id "
                  + arr.getId()
                  + " in cache. Avoiding return. Allocating new array.");

          continue;
        } else if (!arraysForThread.contains(dataType, arrayShapeString)
            || getArraysForThread().get(dataType, arrayShapeString).isEmpty()) {
          break;
        }
    ...
    INDArray ret =
        detached ? Nd4j.createUninitializedDetached(dataType, shape) : Nd4j.create(dataType, shape);
    return ret;
  }
  ...
  public INDArray allocate(boolean detached, LongShapeDescriptor descriptor) {
    ...
    Table<DataType, String, List<INDArray>> arraysForThread = getArraysForThread();
    if (arraysForThread.contains(dataType, arrayShape)
        && enableCache
        && shape.length > 0
        && !Longs.contains(shape, 0)) {
      INDArray arr = null;
  ...
  public void release(@NonNull INDArray array) {
    ...
    Preconditions.checkState(
        !lruCacheForThread.contains(id),
        "Array was released multiple times: id=%s, shape=%ndShape",
        id,
        array);
    ...
    if (array.dataType() == DataType.UTF8) {
      ...
      if (thisBytes > maxCacheBytes.get()) {
        ...
        if (array.closeable()) array.close();
        return;
      }
      ...
      while (currentCacheSize.get() + thisBytes > maxCacheBytes.get() && iter.hasNext()) {
        ...
        if (listx != null) listx.remove(nextOldest);
  ...
  @Override
  public void close() {
    getArraysForThread().values().stream()
        .forEach(
            input ->
                input.stream()
                    .forEach(
