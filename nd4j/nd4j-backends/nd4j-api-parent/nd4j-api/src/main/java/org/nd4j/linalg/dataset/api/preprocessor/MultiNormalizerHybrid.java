...
@Setter
public class MultiNormalizerHybrid extends AbstractNormalizer
    implements MultiDataNormalization, Serializable {
  ...
  private void fitPartial(
      ...
      Map<Integer, NormalizerStats.Builder> inputStatsBuilders,
      Map<Integer, NormalizerStats.Builder> outputStatsBuilders) {
    ensureStatsBuilders(
        inputStatsBuilders, globalInputStrategy, perInputStrategies, dataSet.numFeatureArrays());
    ensureStatsBuilders(
        outputStatsBuilders, globalOutputStrategy, perOutputStrategies, dataSet.numLabelsArrays());

    for (int index : inputStatsBuilders.keySet()) {
      inputStatsBuilders
          .get(index)
          .add(dataSet.getFeatures(index), dataSet.getFeaturesMaskArray(index));
    }
    for (int index : outputStatsBuilders.keySet()) {
      outputStatsBuilders
          .get(index)
          .add(dataSet.getLabels(index), dataSet.getLabelsMaskArray(index));
    }
  }

  private void ensureStatsBuilders(
      Map<Integer, NormalizerStats.Builder> builders,
      NormalizerStrategy globalStrategy,
      Map<Integer, NormalizerStrategy> perArrayStrategies,
      int numArrays) {
    if (builders.isEmpty()) {
  ...
  private Map<Integer, NormalizerStats> buildAllStats(
      @NonNull Map<Integer, NormalizerStats.Builder> builders) {
  ...
  private void preProcess(
      ...
      INDArray[] masks,
      NormalizerStrategy globalStrategy,
      Map<Integer, NormalizerStrategy> perArrayStrategy,
      Map<Integer, NormalizerStats> stats) {
  ...
  /**
   * Undo (revert) the normalization applied by this DataNormalization instance (arrays are modified
   * in-place)
   *
  ...
  /**
   * Undo (revert) the normalization applied by this DataNormalization instance to the entire inputs
   * array
   *
  ...
  /**
   * Undo (revert) the normalization applied by this DataNormalization instance to the features of a
   * particular input
   *
   * @param features The normalized array of inputs
   * @param maskArrays Optional mask arrays belonging to the inputs
  ...
  /**
   * Undo (revert) the normalization applied by this DataNormalization instance to the entire
   * outputs array
   *
  ...
  /**
   * Undo (revert) the normalization applied by this DataNormalization instance to the labels of a
   * particular output
   *
   * @param labels The normalized array of outputs
   * @param maskArrays Optional mask arrays belonging to the outputs
  ...
  private NormalizerStrategy getStrategy(
      NormalizerStrategy globalStrategy,
      Map<Integer, NormalizerStrategy> perArrayStrategy,
      int index) {
