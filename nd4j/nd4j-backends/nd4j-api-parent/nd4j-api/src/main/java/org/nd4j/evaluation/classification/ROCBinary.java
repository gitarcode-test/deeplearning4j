...
public class ROCBinary extends BaseEvaluation<ROCBinary> {
   ...
   * independent binary classes) are present.<br>
  ...
  public void eval(
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (maskArray != null) {
        ...
        if (prob.isView()) prob = prob.dup();
        if (label.isView()) label = label.dup();
  ...
  private void assertIndex(int outputNum) {
    if (underlying == null) {
      throw new UnsupportedOperationException(
          "ROCBinary does not have any stats: eval must be called first");
    }
    if (outputNum < 0 || outputNum >= underlying.length) {
      throw new IllegalArgumentException(
          "Invalid input: output number must be between 0 and " + (outputNum - 1));
    }
  }
  ...
  public double getValue(IMetric metric) {
    if (metric instanceof Metric) {
      ...
      else throw new IllegalStateException("Can't get value for non-binary ROC Metric " + metric);
    } else throw new IllegalStateException("Can't get value for non-binary ROC Metric " + metric);
  }
