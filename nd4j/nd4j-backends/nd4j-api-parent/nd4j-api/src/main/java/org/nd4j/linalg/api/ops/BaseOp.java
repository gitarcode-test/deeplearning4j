...
public abstract class BaseOp extends DifferentialFunction implements Op {
  ...
  @Getter @Setter protected String xVertexId, yVertexId, zVertexId;
  ...
  public static Type getOpType(Op op) {
    ...
    if (op instanceof CustomOp) {
      ...
      if (op.y() == null) type = ((ReduceOp) op).getOpType();
      else type = Type.REDUCE3;
  ...
  public DataBuffer extraArgsDataBuff(DataType dtype) {
    if (extraArgz != null) return extraArgz;
    ...
    if (extraArgs != null) {
      if (Shape.isZ(dtype) || Shape.isB(dtype)) {
        ...
        for (int i = 0; i < extraArgs.length; i++) {
          if (!(extraArgs[i] instanceof Number)) continue;
          Number arg = (Number) extraArgs[i];
          if (arg == null) arg = 0.0;
  ...
  public INDArray getInputArgument(int index) {
    Preconditions.checkState(
        index >= 0 && index < 2, "Input argument index must be 0 or 1, got %s", index);
    return index == 0 ? x : y;
  }
  ...
  public SDVariable[] outputVariables(String baseName) {
    if (zVertexId == null) {
      ...
      if (isInPlace()) {
        ...
        z = inputArr;
        if (sameDiff.getOutputsForOp(this) == null) sameDiff.addOutgoingFor(newVars, this);
        computeVariables(newVars);
    ...
    if (sameDiff.isEagerMode()) {
      ...
      if (args.length == 1) {
        ...
        if (this.opType() == Type.REDUCE3
            ...
            || this.opType() == Type.TRANSFORM_SAME) y = args[1].getArr();
        else if ((opType() == Type.REDUCE_FLOAT
                || opType() == Type.REDUCE_LONG
                || opType() == Type.REDUCE_BOOL
                || opType() == Type.REDUCE_BOOL
                || opType() == Type.REDUCE_SAME)
            && args.length > 1) {
          this.dimensionz = args[1].getArr();
          if (!args[1].getArr().isEmpty()) this.dimensions = args[1].getArr().toLongVector();
          else this.dimensions = new long[0];
        }
      }

      if (x == null) {
        throw new IllegalArgumentException(
            "No variable found for the given input variables of "
                + args[0].name()
      ...
      if (z == null) {
        if (!(this instanceof ReduceOp)) {
          ...
          if (this instanceof BaseReduceOp) {
            ...
            setZ(
                Nd4j.create(Shape.reductionShape(x, dimensions, true, baseReduceOp.keepDims))
                    .castTo(newVars[0].dataType())
                    .detach());
          } else {
            setZ(
                Nd4j.create(Shape.reductionShape(x, dimensions, true, false))
                    .castTo(newVars[0].dataType())
                    .detach());
          }
        }
      ...
      try (OpContext ctx = Nd4j.getExecutioner().buildContext()) {
        ...
        for (Listener l : sameDiff.getListeners()) {
          l.preUpdate(
              sameDiff, At.defaultAt(), sameDiff.getVariables().get(outputVariable().name()), z);
        }
  ...
  public CustomOp toCustomOp() {
    ...
    if (y() != null) customOpBuilder.addInputs(x(), y());
    else customOpBuilder.addInputs(x());
  ...
  protected void defineDimensions(long... dimensions) {
    ...
    if (dimensions == null || dimensions.length == 0) dimensions = new long[] {Integer.MAX_VALUE};
  ...
  public Number getFinalResult() {
    ...
    if (z.isEmpty()) throw new ND4JIllegalStateException("Can't get number from empty array");
    ...
    else if (z.isZ()) return new Long(z.getInt(0));
    else if (z.isB()) return new Integer(z.getInt(0));
