...
public abstract class BaseTransformFloatOp extends BaseTransformOp implements TransformFloatOp {
  ...
  public DataType resultType() {
    if (this.x() != null && this.x().isR()) return this.x().dataType();
  ...
  public List<LongShapeDescriptor> calculateOutputShape(OpContext oc) {
    ...
    if (x.isEmpty()) {
      ...
      LongShapeDescriptor longShapeDescriptor =
          LongShapeDescriptor.emptyWithShape(x.shape(), x.dataType());
      ret.add(longShapeDescriptor);
      return ret;
    }
    return Collections.singletonList(
        LongShapeDescriptor.fromShape(
            x.shape(), x.isR() ? x.dataType() : Nd4j.defaultFloatingPointType()));
  }
  ...
  public List<DataType> calculateOutputDataTypes(List<DataType> dataTypes) {
    Preconditions.checkState(
        ...
        getClass(),
        dataTypes);
    if (dataTypes.get(0).isFPType()) return Collections.singletonList(dataTypes.get(0));
