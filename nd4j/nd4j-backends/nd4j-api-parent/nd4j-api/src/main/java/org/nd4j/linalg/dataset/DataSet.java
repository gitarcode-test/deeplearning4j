...
public class DataSet implements org.nd4j.linalg.dataset.api.DataSet {
   ...
   * @param second the labels (these should be binarized label matrices such that the specified
   *     label has a value of 1 in the desired column with the label)
   */
  ...
  /**
   * Create a dataset with the specified input INDArray and labels (output) INDArray, plus
   * (optionally) mask arrays for the features and labels
   *
  ...
  public static DataSet merge(List<? extends org.nd4j.linalg.dataset.api.DataSet> data) {
    ...
    for (org.nd4j.linalg.dataset.api.DataSet ds : data) {
      ...
      if (anyFeaturesPreset && ds.getFeatures() == null
          || (!first && !anyFeaturesPreset && ds.getFeatures() != null)) {
        throw new IllegalStateException(
            "Cannot merge features: encountered null features in one or more DataSets");
      }
      if (anyLabelsPreset && ds.getLabels() == null
          || (!first && !anyLabelsPreset && ds.getLabels() != null)) {
        throw new IllegalStateException(
            "Cannot merge labels: enountered null labels in one or more DataSets");
      }
  ...
  public org.nd4j.linalg.dataset.api.DataSet getRange(int from, int to) {
    if (hasMaskArrays()) {
      ...
      return new DataSet(
          features.get(interval(from, to)),
          labels.get(interval(from, to)),
          featureMaskHere,
          labelMaskHere);
    }
  ...
  public void load(InputStream from) {
    try {
      ...
      DataInputStream dis =
          from instanceof BufferedInputStream
              ? new DataInputStream(from)
  ...
  public void save(OutputStream to) {
    ...
    if (exampleMetaData != null && exampleMetaData.size() > 0) included |= BITMASK_METADATA_PRESET;
  ...
  public Map<Integer, Double> labelCounts() {
    ...
    for (int i = 0; i < nTensors; i++) {
      ...
      if (maxIdx < 0)
        throw new IllegalStateException(
            "Please check the iamax implementation for "
                + Nd4j.getBlasWrapper().getClass().getName());
      if (ret.get(maxIdx) == null) ret.put(maxIdx, 1.0);
      else ret.put(maxIdx, ret.get(maxIdx) + 1.0);
    }
    return ret;
  }
  ...
  public void shuffle(long seed) {
    ...
    // As per CpuNDArrayFactory.shuffle(List<INDArray> arrays, Random rnd, List<int[]> dimensions)
    // and libnd4j transforms.h shuffleKernelGeneric
  ...
  public void squishToRange(double min, double max) {
    for (int i = 0; i < getFeatures().length(); i++) {
      ...
      if (curr < min) getFeatures().put(i, Nd4j.scalar(min));
      else if (curr > max) getFeatures().put(i, Nd4j.scalar(max));
    }
  }
  ...
  /**
   * @Deprecated Subtract by the column means and divide by the standard deviation
   */
  @Deprecated
  @Override
   ...
   *
   * @param labels the number of labels/columns in the outcome matrix Note that this clears the
  ...
  public DataSet get(int i) {
    if (i >= numExamples() || i < 0)
      throw new IllegalArgumentException(
          "invalid example number: must be 0 to " + (numExamples() - 1) + ", got " + i);
    if (i == 0 && numExamples() == 1) return this;
    return new DataSet(
        getHelper(features, i),
        getHelper(labels, i),
        getHelper(featuresMask, i),
        getHelper(labelsMask, i));
  }
  ...
  /**
   * Partitions a dataset in to mini batches where each dataset in each list is of the specified
   * number of examples
   *
  ...
  public List<DataSet> dataSetBatches(int num) {
    ...
    for (List<DataSet> l : list) ret.add(DataSet.merge(l));
    return ret;
  }

  /**
   * Sorts the dataset by label: Splits the data transform such that examples are sorted by their
   * labels. A ten label dataset would produce lists with batches like the following: x1 y = 1 x2 y
   * = 2 ... x10 y = 10
   *
  ...
  /**
   * Splits a dataset in to test and train randomly. This will modify the dataset in place to
   * shuffle it before splitting into test/train!
   *
  ...
  public String getLabelName(int idx) {
    if (!labelNames.isEmpty()) {
      ...
      throw new IllegalStateException(
          "Label names are not defined on this dataset. Add label names in order to use"
              + " getLabelName with an id.");
  }
  ...
  public DataSet sample(
      int numSamples, org.nd4j.linalg.api.rng.Random rng, boolean withReplacement) {
  ...
  public int numExamples() {
    if (getFeatures() != null) return (int) getFeatures().size(0);
    else if (getLabels() != null) return (int) getLabels().size(0);
    return 0;
  }
  ...
  /**
   * Sets the label names, will throw an exception if the passed in label names doesn't equal the
   * number of outcomes
   *
  ...
  /**
   * Optional column names of the data transform, this is mainly used for interpreting what columns
   * are in the dataset
   *
   * @return
   */
  ...
  /**
   * Sets the column names, will throw an exception if the column names don't match the number of
   * columns
   *
   * @param columnNames
   */
  ...
  public SplitTestAndTrain splitTestAndTrain(double fractionTrain) {
    Preconditions.checkArgument(
        fractionTrain > 0.0 && fractionTrain < 1.0,
        "Train fraction must be > 0.0 and < 1.0 - got %s",
        fractionTrain);
    int numTrain = (int) (fractionTrain * numExamples());
    if (numTrain <= 0) numTrain = 1;
  ...
  public long getMemoryFootprint() {
    ...
    reqMem +=
        featuresMask == null
            ? 0
            : featuresMask.length() * Nd4j.sizeOfDataType(featuresMask.dataType());
    reqMem +=
        labelsMask == null ? 0 : labelsMask.length() * Nd4j.sizeOfDataType(labelsMask.dataType());
  ...
  public void migrate() {
    if (Nd4j.getMemoryManager().getCurrentWorkspace() != null) {
      if (features != null) features = features.migrate();

      if (labels != null) labels = labels.migrate();

      if (featuresMask != null) featuresMask = featuresMask.migrate();

      if (labelsMask != null) labelsMask = labelsMask.migrate();
    }
  }

  @Override
  public void detach() {
    if (features != null) features = features.detach();

    if (labels != null) labels = labels.detach();

    if (featuresMask != null) featuresMask = featuresMask.detach();

    if (labelsMask != null) labelsMask = labelsMask.detach();
  }
  ...
  public MultiDataSet toMultiDataSet() {
    ...
    return new org.nd4j.linalg.dataset.MultiDataSet(
        fNew, lNew, fMaskNew, lMaskNew, exampleMetaData);
  }

  @Override
  public void setCloseable(boolean closeable) {
    if (features != null) features.setCloseable(closeable);
    if (featuresMask != null) featuresMask.setCloseable(closeable);
    if (labels != null) labels.setCloseable(closeable);
    if (labelsMask != null) labelsMask.setCloseable(closeable);
  }
}
