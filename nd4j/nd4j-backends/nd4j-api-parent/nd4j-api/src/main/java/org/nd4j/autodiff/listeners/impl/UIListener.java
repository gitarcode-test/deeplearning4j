...
public class UIListener extends BaseListener {
   ...
   * CREATE: Only allow new file creation. An exception will be thrown if the log file already
   * exists.<br>
   * APPEND: Only allow appending to an existing file. An exception will be thrown if: (a) no file
   * exists, or (b) the network configuration in the existing log file does not match the current
   * log file.<br>
   * CREATE_OR_APPEND: As per APPEND, but create a new file if none already exists.<br>
   * CREATE_APPEND_NOCHECK: As per CREATE_OR_APPEND, but no exception will be thrown if the existing
   * model does not match the current model structure. This mode is not recommended.<br>
   */
  ...
  /**
   * Used to specify how the Update:Parameter ratios are computed. Only relevant when the update
   * ratio calculation is enabled via {@link Builder#updateRatios(int, UpdateRatio)}; update ratio
   * collection is disabled by default<br>
  ...
  /**
   * Used to specify which histograms should be collected. Histogram collection is disabled by
   * default, but can be enabled via {@link Builder#histograms(int, HistogramType...)}. Note that
   * multiple histogram types may be collected simultaneously.<br>
   ...
   * PARAMETER_UPDATES: All trainable parameter updates, before they are applied during training
   * (updates are gradients after applying updater and learning rate etc)<br>
   * ACTIVATIONS: Activations - ARRAY type SDVariables - those that are not constants, variables or
   * placeholders<br>
    ...
    switch (fileMode) {
      case CREATE:
        Preconditions.checkState(
            !logFile.exists(),
            "Log file already exists and fileMode is set to CREATE: %s\n"
                + "Either delete the existing file, specify a path that doesn't exist, or set the"
                + " UIListener to another mode such as CREATE_OR_APPEND",
            logFile);
        break;
      case APPEND:
        Preconditions.checkState(
            logFile.exists(),
            "Log file does not exist and fileMode is set to APPEND: %s\n"
                + "Either specify a path to an existing log file for this model, or set the"
                + " UIListener to another mode such as CREATE_OR_APPEND",
            logFile);
        break;
    }

    if (logFile.exists()) restoreLogFile();
  }

  protected void restoreLogFile() {
    if (logFile.length() == 0 && fileMode == FileMode.CREATE_OR_APPEND
    ...
    try {
      ...
      throw new RuntimeException(
          "Error restoring existing log file at path: " + logFile.getAbsolutePath(), e);
    }

    if (fileMode == FileMode.APPEND || fileMode == FileMode.CREATE_OR_APPEND) {
      // Check the graph structure, if it exists.
      // This is to avoid users creating UI log file with one network configuration, then
      // unintentionally appending data
      ...
      try {
        ...
        throw new RuntimeException(
            "Error restoring existing log file, static info at path: " + logFile.getAbsolutePath(),
            e);
      }
      ...
      if (si != null) {
        for (int i = 0; i < staticList.size(); i++) {
          ...
          if (r.infoType() == UIInfoType.GRAPH_STRUCTURE) {
            // We can't check structure now (we haven't got SameDiff instance yet) but we can flag
            // it to check on first iteration
      ...
      if (structure != null) {
        ...
        if (actPhs.size() != phs.size() || !actPhs.containsAll(phs)) {
          throw new IllegalStateException(
              "Error continuing collection of UI stats in existing model file "
                  + logFile.getAbsolutePath()
                  + ": Model structure differs. Existing (file) model placeholders: "
                  + phs
                  + " vs. current model placeholders: "
                  + actPhs
                  + ". To disable this check, use FileMode.CREATE_APPEND_NOCHECK though this may"
                  + " result issues when rendering data via UI");
        }
        ...
        if (varNames.size() != vars.size() || !varNames.containsAll(vars)) {
          ...
          msg.append("Error continuing collection of UI stats in existing model file ")
              ...
              .append(": Current model structure differs vs. model structure in file - ")
              .append(countDifferent)
              .append(" variable names differ.");
          if (different.size() == countDifferent) {
            msg.append("\nVariables in new model not present in existing (file) model: ")
                .append(different);
          } else {
            msg.append("\nFirst 10 variables in new model not present in existing (file) model: ")
                .append(different);
          }
          msg.append(
              "\n"
    ...
    if (epochTrainEval != null) {
      ...
      for (Map.Entry<Pair<String, Integer>, Evaluation> e : epochTrainEval.entrySet()) {
        String n =
            "evaluation/"
                + e.getKey()
                    .getFirst(); // TODO what if user does same eval with multiple labels? Doesn't
                                 // make sense... add validation to ensure this?
        ...
        for (Evaluation.Metric m : l) {
          ...
          if (!wroteEvalNames) {
            if (!writer.registeredEventName(
                mName)) { // Might have been registered if continuing training
          ...
          try {
            writer.writeScalarEvent(
                mName,
                LogFileWriter.EventSubtype.EVALUATION,
                time,
                at.iteration(),
                at.epoch(),
                score);
  ...
  public void iterationStart(SameDiff sd, At at, MultiDataSet data, long etlMs) {
    if (writer == null) initalizeWriter(sd);
    if (checkStructureForRestore) checkStructureForRestore(sd);
  ...
  public void iterationDone(SameDiff sd, At at, MultiDataSet dataSet, Loss loss) {
    ...
    for (int i = 0; i < lossVals.length; i++) {
      try {
        ...
        writer.writeScalarEvent(
            eventName,
            LogFileWriter.EventSubtype.LOSS,
            time,
            at.iteration(),
            at.epoch(),
            lossVals[i]);
    ...
    if (lossVals.length > 1) {
      ...
      try {
        ...
        writer.writeScalarEvent(
            eventName, LogFileWriter.EventSubtype.LOSS, time, at.iteration(), at.epoch(), total);
      } catch (IOException e) {
        throw new RuntimeException("Error writing to log file", e);
      }
    }
      ...
      if (at.iteration() % learningRateFrequency == 0) {
        ...
        if (u.hasLearningRate()) {
          ...
          try {
            writer.writeScalarEvent(
                "learningRate",
                LogFileWriter.EventSubtype.LEARNING_RATE,
                time,
                at.iteration(),
                at.epoch(),
                lr);
  ...
  public void opExecution(
    ...
    // Note we'll do it in opExecution not iterationDone because we can't be sure arrays will be
    // stil be around in the future
    // i.e., we'll eventually add workspaces and clear activation arrays once they have been
    // consumed
    if (at.operation() == Operation.TRAINING
        && trainEvalMetrics != null
        && trainEvalMetrics.size() > 0) {
      long time = System.currentTimeMillis();
      ...
      if (relevantOpsForEval == null) {
        ...
        for (Pair<String, Integer> p : trainEvalMetrics.keySet()) {
          ...
          Preconditions.checkState(
              opName != null,
              "Cannot evaluate on variable of type %s - variable name: \"%s\"",
              v.getVariable().getVariableType(),
              opName);
      ...
      for (Pair<String, Integer> p : trainEvalMetrics.keySet()) {
        ...
        if (trainEvalFrequency > 0
            && at.iteration() > 0
            && at.iteration() % trainEvalFrequency == 0) {
          for (Evaluation.Metric m : trainEvalMetrics.get(p)) {
            ...
            if (!wroteEvalNamesIter) {
              if (!writer.registeredEventName(
                  n)) { // Might have been written previously if continuing training
                writer.registerEventNameQuiet(n);
              }
              wrote = true;
            }
