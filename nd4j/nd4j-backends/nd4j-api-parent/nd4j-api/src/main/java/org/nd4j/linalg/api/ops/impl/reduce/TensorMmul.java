...
public class TensorMmul extends DynamicCustomOp {
  ...
  public List<SDVariable> doDiff(List<SDVariable> gradients) {
    return Arrays.asList(
        new TensorMmulBp(sameDiff, larg(), rarg(), outputVariable(), gradients.get(0), axes)
            .outputVariables());
  }
  ...
  public List<DataType> calculateOutputDataTypes(List<DataType> inputDataTypes) {
    Preconditions.checkState(
        inputDataTypes != null && inputDataTypes.size() == 2,
        "Expected exactly 2 input data types for %s, got %s",
        getClass(),
        inputDataTypes);
