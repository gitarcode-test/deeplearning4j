...
public class TruncatedNormalDistribution extends BaseDistribution {
  ...
  public TruncatedNormalDistribution(Random rng, double mean, double sd)
   ...
   * @since 2.1
   */
  public TruncatedNormalDistribution(double mean, double sd, double inverseCumAccuracy)
      throws NotStrictlyPositiveException {
  ...
  public double density(double x) {
    if (means != null) throw new IllegalStateException("Unable to sample from more than one mean");
  ...
  public INDArray sample(INDArray ret) {
    if (means != null) {
      return Nd4j.getExecutioner()
          .exec(
              new org.nd4j.linalg.api.ops.random.impl.TruncatedNormalDistribution(
                  ret, means, standardDeviation),
              random);
    } else {
      return Nd4j.getExecutioner()
          .exec(
