...
public class ImagePreProcessingScaler implements DataNormalization {
  ...
  public ImagePreProcessingScaler(double a, double b, int maxBits) {
    ...
    // some images are 16-bit, some 32-bit, integer, or float, and those BTW already come with
    // values in [0..1]...
  ...
  public void preProcess(INDArray features) {
    ...
    if (this.minRange != 0) features.addi(this.minRange); // Offset by minRange
  }
  ...
  public void transformLabel(INDArray label) {
    Preconditions.checkState(
        label != null && label.rank() == 4,
        "Labels can only be transformed for segmentation use"
            + " cases using this preprocesser - i.e., labels must be rank 4. Got: %ndShape",
        label);
    transform(label);
  }
