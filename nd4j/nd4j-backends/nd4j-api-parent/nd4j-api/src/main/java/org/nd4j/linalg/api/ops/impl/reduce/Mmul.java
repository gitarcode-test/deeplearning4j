...
public class Mmul extends DynamicCustomOp {
  ...
  public Mmul(SameDiff sameDiff, SDVariable i_v1, SDVariable i_v2, MMulTranspose mt) {
    ...
    this.mt = mt;
    addIArgument(
        ArrayUtil.fromBoolean(mt.isTransposeA()),
  ...
  public Mmul(
    ...
    addInputArgument(x, y);
    addIArgument(
        ArrayUtil.fromBoolean(transposeX),
  ...
  public Mmul(
    ...
    addIArgument(
        ArrayUtil.fromBoolean(transA), ArrayUtil.fromBoolean(transB), ArrayUtil.fromBoolean(false));
    this.alpha = alpha;
    this.beta = beta;
    addTArgument(alpha, beta);
    mt =
  ...
  public Map<String, Object> propertiesForFunction() {
    if (mt == null) return Collections.emptyMap();
  ...
  public void setPropertiesForFunction(Map<String, Object> properties) {
    if (mt == null) mt = MMulTranspose.builder().build();
    mt.setProperties(properties);
  }

  /**
   * For a 2D matrix of shape (M, N) we return (N, M). For a 3D matrix with leading mini-batch
   * dimension (mb, M, N) we return (mb, N, M)
   *
  ...
  public long[] transposeShapeArray(long[] shape) {
    if (shape.length == 2) {
      ...
      throw new IllegalArgumentException(
          "Matrix input has to be of length 2 or 3, got: " + shape.length);
    }
  }
  ...
  public void initFromTensorFlow(
    ...
    if (nodeDef.getOp().equalsIgnoreCase("MatMul")) {
      ...
      // In practice, BatchMatMul seems to use "adj_x" and "adj_y" instead of "transpose_a" and
      // "transpose_b"
    ...
    MMulTranspose mMulTranspose =
        MMulTranspose.builder().transposeA(isTransposeA).transposeB(isTransposeB).build();
    this.mt = mMulTranspose;
    iArguments.clear();
    addIArgument(
        ArrayUtil.fromBoolean(mt.isTransposeA()), ArrayUtil.fromBoolean(mt.isTransposeB()));
  }
  ...
  public void initFromOnnx(
    ...
    val isTransposeA =
        !attributesForNode.containsKey("transA")
            ? false
            : attributesForNode.get("transA").getI() > 0;
    val isTransposeB =
        !attributesForNode.containsKey("transB")
            ? false
            : attributesForNode.get("transB").getI() > 0;
    MMulTranspose mMulTranspose =
        MMulTranspose.builder().transposeA(isTransposeA).transposeB(isTransposeB).build();
    this.mt = mMulTranspose;
  }
  ...
  public List<SDVariable> doDiff(List<SDVariable> gradients) {
    return Arrays.asList(
        new MmulBp(sameDiff, arg(0), arg(1), gradients.get(0), mt).outputVariables());
  }
  ...
  public List<DataType> calculateOutputDataTypes(List<DataType> dataTypes) {
    if (!dArguments.isEmpty()) return Collections.singletonList(dArguments.get(0));
    Preconditions.checkState(
        dataTypes != null && dataTypes.size() >= 2,
        "Expected at least 2 inputs to mmul op, got %s",
        dataTypes);
    Preconditions.checkState(
        dataTypes.get(0).isFPType() && dataTypes.get(1).isFPType(),
        "Inputs to mmul op must both be a floating" + "point type: got %s",
        dataTypes);
