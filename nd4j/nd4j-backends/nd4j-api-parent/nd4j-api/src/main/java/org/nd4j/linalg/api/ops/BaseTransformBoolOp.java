...
public abstract class BaseTransformBoolOp extends BaseTransformOp implements TransformSameOp {
  ...
  public List<LongShapeDescriptor> calculateOutputShape(OpContext oc) {
    ...
    LongShapeDescriptor desc =
        x.isEmpty()
            ? LongShapeDescriptor.emptyWithShape(x.shape(), DataType.BOOL)
  ...
  public List<DataType> calculateOutputDataTypes(List<DataType> dataTypes) {
    ...
    Preconditions.checkState(
        dataTypes != null && dataTypes.size() == args.length,
        "Expected exactly %s input datatype(s) for %s, got input %s",
        args.length,
        getClass(),
        dataTypes);
