...
public abstract class BasicWorkspaceManager implements MemoryWorkspaceManager {
  ...
  /**
   * This method allows to specify "Default" configuration, that will be used in signatures which do
   * not have WorkspaceConfiguration argument
   *
   * @param configuration
   */
  @Override
  ...
  public void setDebugMode(DebugMode mode) {
    if (mode == null) mode = DebugMode.DISABLED;
  ...
  public void destroyWorkspace(MemoryWorkspace workspace) {
    if (workspace == null || workspace instanceof DummyWorkspace) return;
  ...
  protected void ensureThreadExistense() {
    if (backingMap.get() == null) backingMap.set(new HashMap<>());
  }
   ...
   *
   * <p>PLEASE NOTE: Do not forget to close this block.
   *
   * @return
   */
  @Override
