...
public class CheckpointListener extends BaseListener implements Serializable {
    ...
    if (this.checkpointRecordFile.exists() && this.checkpointRecordFile.length() > 0) {
      ...
      if (deleteExisting) {
        ...
        if (files != null && files.length > 0) {
          for (File f : files) {
            ...
            if (name.startsWith("checkpoint_")
                && (name.endsWith("MultiLayerNetwork.zip")
                    || name.endsWith("ComputationGraph.zip"))) {
              f.delete();
            }
          }
        }
        ...
        throw new IllegalStateException(
            "Detected existing checkpoint files at directory "
  ...
  @Override
  public ListenerResponse epochEnd(
      SameDiff sameDiff, At at, LossCurve lossCurve, long epochTimeMillis) {
  ...
  public void iterationDone(SameDiff sd, At at, MultiDataSet dataSet, Loss loss) {
    ...
    if (saveEveryUnit != null) {
      if (saveEverySinceLast) {
        ...
        long lastSave =
            (lastSaveEveryMsNoSinceLast != null ? lastSaveEveryMsNoSinceLast : startTime);
  ...
  private void saveCheckpointHelper(SameDiff model, At at) throws Exception {
    ...
    Checkpoint c =
        new Checkpoint(
            ++lastCheckpointNum, System.currentTimeMillis(), at.iteration(), at.epoch(), null);
    ...
    if (logSaving) {
      log.info(
          "Model checkpoint saved: epoch {}, iteration {}, path: {}",
          c.getEpoch(),
          c.getIteration(),
    ...
    if (keepMode == null || keepMode == KeepMode.ALL) {
      ...
      for (Checkpoint cp : availableCheckpoints()) {
        if (cp.getCheckpointNum() > 0 && (cp.getCheckpointNum() + 1) % keepEvery == 0) {
          ...
          continue;
        } else if (cp.getCheckpointNum()
            > lastCheckpointNum
                - keepLast) { // Example: latest is 5, keep last 2 -> keep checkpoints 4 and 5
  ...
  /**
   * List all available checkpoints. A checkpoint is 'available' if the file can be loaded. Any
   * checkpoint files that have been automatically deleted (given the configuration) will not be
   * returned here.
   *
   ...
   * List all available checkpoints. A checkpoint is 'available' if the file can be loaded. Any
   * checkpoint files that have been automatically deleted (given the configuration) will not be
   * returned here. Note that the checkpointInfo.txt file must exist, as this stores checkpoint
   * information
   *
  ...
  public static List<Checkpoint> availableCheckpoints(File directory) {
    ...
    Preconditions.checkState(
        checkpointRecordFile.exists(),
        "Could not find checkpoint record file at expected path %s",
  ...
  public static File getFileForCheckpoint(File rootDir, int checkpointNum) {
    ...
    throw new IllegalStateException(
        "Model file for checkpoint " + checkpointNum + " does not exist");
  }

  /**
   * Load a given checkpoint number
   *
   * @param loadUpdaterState If true: load the updater state. See {@link SameDiff#load(File,
  ...
  public static class Builder {
    ...
    /**
     * Save a model periodically (if sinceLast == false), or if the specified amount of time has
     * elapsed since the last model was saved (if sinceLast == true)
     *
     * @param amount Quantity of the specified time unit
     * @param timeUnit Time unit
     */
    ...
    /**
     * Keep only the last N most recent model checkpoint files. Older checkpoints will automatically
     * be deleted.
     *
    ...
    public Builder keepLast(int n) {
      if (n <= 0) {
        throw new IllegalArgumentException(
            "Number of model files to keep should be > 0 (got: " + n + ")");
      }
     ...
     * For example: suppose you save every 100 iterations, for 2050 iteration, and use
     * keepLastAndEvery(3,5). This means after 2050 iterations you would have saved 20 checkpoints -
     * some of which will be deleted. Those remaining in this example: iterations 500, 1000, 1500,
     * 1800, 1900, 2000.
     *
    ...
    public Builder keepLastAndEvery(int nLast, int everyN) {
      if (nLast <= 0) {
        throw new IllegalArgumentException(
            "Most recent number of model files to keep should be > 0 (got: " + nLast + ")");
      }
      if (everyN <= 0) {
        throw new IllegalArgumentException(
            "Every n model files to keep should be > 0 (got: " + everyN + ")");
      }
    ...
    /**
     * Whether the updater state (history/state for Adam, Nesterov momentum, etc) should be saved
     * with each checkpoint.<br>
     * Updater state is saved by default. If you expect to continue training on any of the
     * checkpoints, this should be set to true. However, it will increase the file size.
     *
     * @param saveUpdaterState If true: updater state will be saved with checkpoints. False: not
     *     saved.
     */
    ...
    /**
     * If the checkpoint listener is set to save to a non-empty directory, should the
     * CheckpointListener-related content be deleted?<br>
     * This is disabled by default (and instead, an exception will be thrown if existing data is
     * found)<br>
     * WARNING: Be careful when enabling this, as it deletes all saved checkpoint models in the
     * specified directory!
     */
    ...
    public CheckpointListener build() {
      if (saveEveryNEpochs == null && saveEveryAmount == null && saveEveryNIterations == null) {
        throw new IllegalStateException(
            "Cannot construct listener: no models will be saved (must use at least"
