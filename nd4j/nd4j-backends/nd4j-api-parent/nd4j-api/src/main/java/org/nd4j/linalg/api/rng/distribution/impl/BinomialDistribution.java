...
public class BinomialDistribution extends BaseDistribution {
      ...
      ret =
          1.0D
              - Beta.regularizedBeta(
                  this.probabilityOfSuccess, x + 1.0D, (this.numberOfTrials - x));
    }
   ...
   * {@inheritDoc}
   *
   * <p>For {@code n} trials and probability parameter {@code p}, the variance is {@code n * p * (1
   * - p)}.
   */
   ...
   * {@inheritDoc}
   *
   * <p>The upper bound of the support is the number of trials except for the probability parameter
   * {@code p = 0}.
   *
    ...
    if (random.getStatePointer() != null) {
      if (p != null) {
        return Nd4j.getExecutioner()
            .exec(
                new org.nd4j.linalg.api.ops.random.impl.BinomialDistributionEx(
        ...
        return Nd4j.getExecutioner()
            .exec(
                new org.nd4j.linalg.api.ops.random.impl.BinomialDistributionEx(
                    ret, numberOfTrials, probabilityOfSuccess),
                random);
      }
    } else {
        ...
        for (int i = 0; i < len; i++) {
          ...
          org.apache.commons.math3.distribution.BinomialDistribution binomialDistribution =
              new org.apache.commons.math3.distribution.BinomialDistribution(
                  (RandomGenerator) Nd4j.getRandom(), numberOfTrials, p.getDouble(idx));
          ret.putScalar(idx, binomialDistribution.sample());
        }
      } else {
        org.apache.commons.math3.distribution.BinomialDistribution binomialDistribution =
            new org.apache.commons.math3.distribution.BinomialDistribution(
                (RandomGenerator) Nd4j.getRandom(), numberOfTrials, probabilityOfSuccess);
        for (int i = 0; i < len; i++) {
