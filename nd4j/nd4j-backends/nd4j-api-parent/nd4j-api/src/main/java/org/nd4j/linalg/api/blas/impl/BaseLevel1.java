...
public abstract class BaseLevel1 extends BaseLevel implements Level1 {
  ...
  @Override
  public double dot(
      long n, DataBuffer x, int offsetX, int incrX, DataBuffer y, int offsetY, int incrY) {
  ...
  public void axpy(long n, double alpha, INDArray x, INDArray y) {
    if (x.data().dataType() == DataType.DOUBLE) {
      ...
      DefaultOpExecutioner.validateDataType(DataType.FLOAT, x, y);
      saxpy(
          n, (float) alpha, x, BlasBufferUtil.getBlasStride(x), y, BlasBufferUtil.getBlasStride(y));
    } else {
      DefaultOpExecutioner.validateDataType(DataType.HALF, x, y);
      haxpy(
          n, (float) alpha, x, BlasBufferUtil.getBlasStride(x), y, BlasBufferUtil.getBlasStride(y));
  ...
  public void scal(long N, double alpha, INDArray X) {
    if (X.data().dataType() == DataType.DOUBLE) dscal(N, alpha, X, BlasBufferUtil.getBlasStride(X));
    else if (X.data().dataType() == DataType.FLOAT)
  ...
  protected abstract float hdot(
      long N, DataBuffer X, int offsetX, int incX, DataBuffer Y, int offsetY, int incY);

  protected abstract float sdot(long N, INDArray X, int incX, INDArray Y, int incY);

  protected abstract float sdot(
      long N, DataBuffer X, int offsetX, int incX, DataBuffer Y, int offsetY, int incY);

  protected abstract double ddot(long N, INDArray X, int incX, INDArray Y, int incY);

  protected abstract double ddot(
      long N, DataBuffer X, int offsetX, int incX, DataBuffer Y, int offsetY, int incY);
  ...
  protected abstract void srot(
      long N, INDArray X, int incX, INDArray Y, int incY, float c, float s);
  ...
  protected abstract void drot(
      long N, INDArray X, int incX, INDArray Y, int incY, double c, double s);
