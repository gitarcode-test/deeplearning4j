...
public class Conv2D extends DynamicCustomOp {
  ...
  private static final String INVALID_CONFIGURATION =
      "Invalid Conv2D configuration : sW = %s pH = %s dW = %s ";
  ...
  public void setPropertiesForFunction(Map<String, Object> properties) {
    if (config == null) {
      ...
      if (paddingMode != null) builder.paddingMode(PaddingMode.valueOf(paddingMode));
    ...
    config =
        Conv2DConfig.builder()
            ...
            .paddingMode(
                iArguments.size() < 9
                    ? PaddingMode.VALID
                    : PaddingMode.fromNumber(iArguments.get(8).intValue()))
            .dataFormat(iArguments.size() < 10 ? "NCHW" : iArguments.get(9) > 0 ? "NHWC" : "NCHW")
            .weightsFormat(
                iArguments.size() < 11
                    ? YXIO
                    : WeightsFormat.values()[iArguments.get(10).intValue()])
            .build();
  }
