...
public class Pooling2D extends DynamicCustomOp {
  ...
  /**
   * Divisor mode for average pooling only. 3 modes are supported: MODE_0: EXCLUDE_PADDING:
  ...
  public Pooling2D(
      @NonNull INDArray[] inputs, INDArray[] outputs, @NonNull Pooling2DConfig config) {
  ...
  private void addArgs() {
    ...
    addIArgument(
        (t == Pooling2DType.AVG) ? config.getDivisor().ordinal() : (int) config.getExtra());
  ...
  public List<SDVariable> doDiff(List<SDVariable> f1) {
    ...
    Pooling2DDerivative pooling2DDerivative =
        Pooling2DDerivative.derivativeBuilder()
  ...
  public void initFromTensorFlow(
    ...
    if (!isSameMode) log.debug("Mode: {}", paddingMode);
  ...
  public String getPoolingPrefix() {
    if (config == null) return "somepooling";
