...
public interface CustomOp {
  ...
  /**
   * This allows a custom op to configure relevant fields from its arguments. This is needed when
   * ops are created via reflection for things like model import.
   */
  ...
  /**
   * Asserts a valid state for execution, otherwise throws an {@link
   ...
   *
   * @return True if the output arrays were initialized (and hence should be calculated), false
   *     otherwise
   */
