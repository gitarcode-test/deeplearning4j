...
public class DynamicCustomOp extends DifferentialFunction implements CustomOp {
  ...
  @Builder.Default protected List<INDArray> inputArguments = new ArrayList<>();
  @Builder.Default protected List<INDArray> outputArguments = new ArrayList<>();

  protected double[] tArgumentsArr;

  @Builder.Default protected List<Double> tArguments = new ArrayList<>();

  protected long[] iArgumentsArr;

  @Builder.Default protected List<Long> iArguments = new ArrayList<>();

  protected boolean[] bArgumentsArr;

  @Builder.Default protected List<Boolean> bArguments = new ArrayList<>();

  protected DataType[] dArgumentsArr;

  @Builder.Default protected List<DataType> dArguments = new ArrayList<>();

  protected String[] sArgumentsArr;
  @Builder.Default protected List<String> sArguments = new ArrayList<>();

  @Builder.Default protected List<Integer> axis = new ArrayList<>();

  @Getter @Setter protected boolean inplaceCall;
  @Getter private long hash;
  @Getter @Setter protected SDVariable[] outputVariables;
  ...
  public DynamicCustomOp(
      String opName, INDArray input, INDArray output, List<Double> tArguments, long[] iArguments) {
    this(
        opName,
        (input == null ? null : new INDArray[] {input}),
  ...
  /**
   * Initialize this custom op with all of the inputs, outputs, and respective arguments for
   * execution
   *
  ...
  public DynamicCustomOp(
      ...
      INDArray[] outputs,
      List<Double> tArguments,
      List<Long> iArguments) {
    if (inputs != null) {
      for (int i = 0; i < inputs.length; i++) {
        if (inputs[i] == null) {
          throw new IllegalArgumentException(
              "Input " + i + " for op " + this.getClass() + " was null!");
        }
      }

      inputArguments = new ArrayList<>(Arrays.asList(inputs));
    }
    if (outputs != null) outputArguments = new ArrayList<>(Arrays.asList(outputs));
    this.opName = opName;
    if (tArguments == null) {
    ...
    if (iArguments != null) {
      for (val a : iArguments) this.iArguments.add(a.longValue());
    }
    bArguments = new ArrayList<>();
    dArguments = new ArrayList<>();
  }
  ...
  /**
   * Initialize this for {@link SameDiff} execution Any extra int or float arguments for operations
   * must be added to the respective TArguments or IArguments lists upon construction
   *
  ...
  public SDVariable[] outputVariables(String baseName) {
    ...
    if (this.sameDiff == null) return new SDVariable[0];
    if (this.outputVariables == null) {
      ...
      val newVars =
          sameDiff.generateOutputVariableForOp(this, baseName, false); // Also adds outgoing
  ...
  protected void addOutputsToOp() {
    computeArrays();
    if (sameDiff.getOutputsForOp(this) == null) sameDiff.addOutgoingFor(outputVariables, this);
  }

  /**
   * Generate fake data for {@link #computeArrays()} of the given shape with the data type {@link
  ...
  /**
   * Generate fake data for {@link #computeArrays()} of the the given shape with the given data type
   *
   * @param shape the shape to use
    ...
    if (sameDiff.isEagerMode()) {
      ...
      if (inputArguments.isEmpty()) {
        for (SDVariable arg : args) {
          if (arg.getArr() != null && !arg.isPlaceHolder()) addInputArgument(arg.getArr());
          else if (arg.isPlaceHolder() && arg.getShape() != null) {
            if (arg.getShape() != null && !sameDiff.getEagerArrays().hasArray(arg.name())) {
              // if we have a shape, ensure we create a proper 1 mini batch size input of the
              // relevant shape
              ...
              log.warn(
                  "Variable name "
                      ...
                      + " with unique name of "
                      + getOwnName()
                      + " was not able to resolve an array for eager computation, inserting dummy"
                      + " array. This can happen with control flow ops. Please validate this if in"
                      + " error.");
            } else {
      ...
      if (outputVariables.length > 0 && outputArguments().isEmpty()) {
        // override output variables to ensure data types, shapes and output arrays are properly
        // computed
        List<LongShapeDescriptor> longShapeDescriptors =
            Nd4j.getExecutioner().calculateOutputShape(this);
        if (!longShapeDescriptors.isEmpty())
          for (int i = 0; i < longShapeDescriptors.size(); i++) {
            if (outputVariables[i].getArr() != null) {
              ...
              INDArray arr =
                  longShapeDescriptors.get(i).isEmpty()
                      ? Nd4j.create(longShapeDescriptors.get(i))
                      : Nd4j.create(defaultType, shape);
        ...
        try (OpContext ctx = Nd4j.getExecutioner().buildContext()) {
          ...
          for (Listener l : sameDiff.getListeners()) {
            for (int i = 0; i < outputVariables.length; i++) {
              l.preUpdate(
                  sameDiff,
                  At.defaultAt(),
                  sameDiff.getVariables().get(outputVariables[i].name()),
                  exec[i]);
            }
          }
          ...
          if (outputVariables.length != exec.length) {
            log.warn(
                "During eager execution of op "
                    + getOwnName()
                    + " of type "
                    + opName()
                    + " the output variables had length "
                    + outputVariables.length
                    + " while execution output was "
                    + exec.length
                    + " stub scalar variables will be used.");
          }
          for (int i = 0; i < outputVariables.length; i++) {
            if (i >= exec.length) {
  ...
  public void addIArgument(int... arg) {
    for (long a : arg) iArguments.add(a);
  }
  ...
  private void addIArgument(Integer... arg) {
    for (val a : arg) addIArgument(a.longValue());
  }
  ...
  public void addTArgument(double... arg) {
    if (arg != null) addTArgument(Doubles.asList(arg).toArray(new Double[arg.length]));
  }

  @Override
  public void addDArgument(DataType... arg) {
    if (dArguments == null) dArguments = new ArrayList<>();

    if (arg != null) dArguments.addAll(Arrays.asList(arg));
  }
  ...
  public void addInputArgument(INDArray... arg) {
    for (int i = 0; i < arg.length; i++) {
      if (arg[i] == null) throw new ND4JIllegalStateException("Input " + i + " was null!");
    }
  ...
  public INDArray getInputArgument(int index) {
    if (inputArguments == null || index >= inputArguments.size()) return null;
  ...
  public void setInputArgument(int index, INDArray input) {
    if (index >= inputArguments.size()) {
      ...
      while (inputArguments.size() <= index) inputArguments.add(null);
    }
  ...
  public void addOutputArgument(INDArray... arg) {
    for (int i = 0; i < arg.length; i++) {
      if (arg[i] == null) throw new ND4JIllegalStateException("Output " + i + " was null!");
    }
  ...
  public INDArray getOutputArgument(int index) {
    if (outputArguments == null || index >= outputArguments.size()) return null;
  ...
  public static DynamicCustomOpsBuilder builder(String opName) {
    ...
    return new DynamicCustomOpsBuilder(
        lcName,
        desc.getHash(),
        desc.getNumInputs(),
        desc.getNumOutputs(),
        desc.isAllowsInplace(),
        desc.getNumTArgs(),
        desc.getNumIArgs());
  }
  ...
  public List<LongShapeDescriptor> calculateOutputShape(OpContext oc) {
    ...
    if (outputShapes != null && !outputShapes.isEmpty()) return outputShapes;

    if (descriptor == null) {
      throw new IllegalStateException(
          "Could not find descriptor for op: "
              + opName()
              + (DynamicCustomOp.class == this.getClass()
                  ? ""
    ...
    if (descriptor.getNumIArgs() >= 0 && nI < descriptor.getNumIArgs()) {
      throw new IllegalStateException(
          String.format(
              "Could not calculate output shape for op %s: not fully initialized (%d IArgs"
                  + " specified, %d required)",
              getClass().getName(), nI, descriptor.getNumIArgs()));
    }
    ...
    if (descriptor.getNumTArgs() >= 0 && nT < descriptor.getNumTArgs()) {
      throw new IllegalStateException(
          String.format(
              "Could not calculate output shape for op %s: not fully initialized (%d TArgs"
                  + " specified, %d required)",
              getClass().getName(), nT, descriptor.getNumTArgs()));
    }
    ...
    if (descriptor.getNumInputs() >= 0 && nIn < descriptor.getNumInputs()) {
      throw new IllegalStateException(
          String.format(
              "Could not calculate output shape for op %s: not fully initialized (%d input"
                  + " (INDArray) args specified, {} required)",
              getClass().getName(), nIn, descriptor.getNumInputs()));
    }

    List<LongShapeDescriptor> ret;
    if (oc == null) ret = Nd4j.getExecutioner().calculateOutputShape(this);
    else ret = Nd4j.getExecutioner().calculateOutputShape(this, oc);
    return ret;
  }
      ...
      if (sameDiff == null) {
        throw new ND4JIllegalStateException(
            "Op ["
                ...
                + "] failure for ["
                + this.getOwnName()
                + "]: Number of inputs is invalid for execution. "
                + numInputArguments()
                + " were provided but "
                + descriptor.getNumInputs()
        ...
        throw new ND4JIllegalStateException(
            "Op ["
                ...
                + " were provided but "
                + descriptor.getNumInputs()
                + " are required for execution. Input variable names: "
                + Arrays.toString(inputNames)
                + ". Input variable array shapes: "
                + Arrays.toString(arrayShapes));
      }
    }
    ...
    if (descriptor.getNumOutputs() > 0 && numOutputArguments() < descriptor.getNumOutputs())
      throw new ND4JIllegalStateException(
          "Op ["
              ...
              + "] failure for ["
              + this.getOwnName()
              + "]: Number of outputs is invalid for execution. Specified ["
              + numOutputArguments()
              + "] but should be ["
              + descriptor.getNumOutputs()
              + "]");
    ...
    if (descriptor.getNumIArgs() >= 0 && numIArguments() < descriptor.getNumIArgs())
      throw new ND4JIllegalStateException(
          "Op ["
              ...
              + "] failure for ["
              + this.getOwnName()
              + "]: Number of integer arguments is invalid for execution. Specified ["
              + numIArguments()
              + "] but should be ["
              + descriptor.getNumIArgs()
              + "]");
    ...
    if (descriptor.getNumTArgs() >= 0 && numTArguments() < descriptor.getNumTArgs())
      throw new ND4JIllegalStateException(
          "Op ["
              ...
              + "] failure for ["
              + this.getOwnName()
              + "]: Number of inputs is invalid for execution. Specified ["
              + numTArguments()
              + "] but should be ["
              + descriptor.getNumTArgs()
              + "]");
  }
  ...
  public List<SDVariable> doDiff(List<SDVariable> f1) {
    throw new UnsupportedOperationException(
        "Please extend DynamicCustomOp.doDiff to support SameDiff backprop "
            + "operations. Op: "
            + getClass().getName());
  }
  ...
  public boolean[] bArgs() {
    ...
    for (int e = 0; e < result.length; e++) result[e] = bArguments.get(e);
  ...
  public void addBArgument(boolean... arg) {
    if (arg != null) {
      for (val b : arg) bArguments.add(b);
    }
  }
  ...
  public static class DynamicCustomOpsBuilder {
    ...
    protected DynamicCustomOpsBuilder(
        String opName,
        long hash,
        int numInputs,
        int numOutputs,
    ...
    /**
     * This method takes arbitrary number of input INDArrays in, as Op input Note that this
     * ACCUMULATES arguments. You are able to call this method multiple times and it will add
     * arguments to a list. PLEASE NOTE: this method does NOT validate lengths/shapes.
     *
     * @param inputs
     * @return
     */
    public DynamicCustomOpsBuilder addInputs(INDArray... inputs) {
        ...
        if (inputs == null)
          throw new ND4JIllegalStateException(
              "CustomOp ["
                  ...
                  + "] expects at least "
                  + numInputs
                  + " arguments. Null was passed instead.");
      ...
      for (val in : inputs) inputArguments.add(in);

      return this;
    }

    /**
     * This method takes arbitrary number of output INDArrays in, to store operation result Note
     * that this ACCUMULATES arguments. You are able to call this method multiple times and it will
     * add arguments to a list. PLEASE NOTE: this method does NOT validate lengths/shapes.
     *
     * @param outputs
     * @return
     */
    public DynamicCustomOpsBuilder addOutputs(INDArray... outputs) {
      ...
      for (val in : outputs) outputArguments.add(in);
    ...
    /**
     * This method takes arbitrary number of Integer arguments for op, Note that this ACCUMULATES
     * arguments. You are able to call this method multiple times and it will add arguments to a
     * list. PLEASE NOTE: this method does NOT validate values.
     *
     * @param iargs
     * @return
     */
    public DynamicCustomOpsBuilder addIntegerArguments(List<Long> iargs) {
        ...
        if (iargs == null)
          throw new ND4JIllegalStateException(
              "CustomOp ["
                  ...
                  + "] expects "
                  + numIArguments
                  + " integer arguments. Null was passed instead.");
      ...
      for (val in : iargs) iArguments.add(in.longValue());

      return this;
    }

    /**
     * This method takes arbitrary number of String arguments for op, Note that this ACCUMULATES
     * arguments. You are able to call this method multiple times and it will add arguments to a
     * list. PLEASE NOTE: this method does NOT validate values.
     *
     * @param sArgs
        ...
        if (sArgs == null)
          throw new ND4JIllegalStateException(
              "CustomOp ["
                  ...
                  + "] expects "
                  + numSArguments
                  + " string arguments. Null was passed instead.");
        ...
        if (numSArguments > sArgs.size())
          throw new ND4JIllegalStateException(
              "CustomOp ["
                  ...
                  + "] expects at least "
                  + numSArguments
                  + " string arguments, but "
                  + sArgs.size()
                  + " was passed to constructor");
      }

      for (val in : sArgs) sArguments.add(in);
      ...
      if (numSArguments != 1 && numSArguments > 0)
        throw new ND4JIllegalStateException(
            "CustomOp ["
                ...
                + "] expects "
                + numSArguments
                + " string arguments. One arg was passed instead.");
      ...
      if (numIArguments != 1 && numIArguments > 0)
        throw new ND4JIllegalStateException(
            "CustomOp ["
                ...
                + "] expects "
                + numIArguments
                + " integer arguments. One arg was passed instead.");
    ...
    public DynamicCustomOpsBuilder addIntegerArguments(long... iargs) {
      ...
      for (val in : iargs) iArguments.add(Long.valueOf((long) in));
    ...
    public DynamicCustomOpsBuilder addIntegerArguments(int... iargs) {
      ...
      for (val in : iargs) iArguments.add((long) in);
    ...
    public DynamicCustomOpsBuilder addBooleanArguments(boolean... bargs) {
      for (val in : bargs) bArguments.add(in);
    ...
    /**
     * This method takes arbitrary number of Double arguments for op, Note that this ACCUMULATES
     * arguments. You are able to call this method multiple times and it will add arguments to a
     * list. PLEASE NOTE: this method does NOT validate values.
     *
     * @return
    ...
    public DynamicCustomOpsBuilder addFloatingPointArguments(Double... targs) {
      ...
      for (val in : targs) tArguments.add(in);
