...
public class Variance extends BaseReduceOp {
  ...
  public Variance(
      SameDiff sameDiff, SDVariable i_v, long[] dimensions, boolean keepDims, double mean) {
  ...
  public Variance(
      SameDiff sameDiff, SDVariable i_v, SDVariable i_v2, long[] dimensions, double mean) {
  ...
  public Variance(
      SameDiff sameDiff, SDVariable i_v, SDVariable dimensions, boolean keepDims, double mean) {
    super(sameDiff, i_v, dimensions, keepDims);
    this.mean = mean;
  }
  ...
  public Variance(
      INDArray x, INDArray y, INDArray z, boolean keepDims, long[] dimensions, double mean) {
  ...
  public Variance(
      SameDiff sameDiff, SDVariable i_v, SDVariable i_v2, SDVariable dimensions, double mean) {
    super(sameDiff, i_v, i_v2, dimensions);
    this.mean = mean;
  }
  ...
  public Variance(
      INDArray x, INDArray y, INDArray z, double mean, double bias, long... dimensions) {
    super(x, y, z, dimensions);
    this.mean = mean;
    this.bias = bias;
  }
  ...
  public Variance(
      SameDiff sameDiff, SDVariable i_v, double mean, double bias, boolean biasCorrected) {
    super(sameDiff, i_v);
    this.mean = mean;
    this.bias = bias;
    this.biasCorrected = biasCorrected;
  ...
  public Variance(
      INDArray x, INDArray y, double mean, double bias, boolean biasCorrected, long... dimensions) {
    super(x, y, dimensions);
    this.mean = mean;
    this.bias = bias;
    this.biasCorrected = biasCorrected;
  ...
  public Variance(
      INDArray x, INDArray z, boolean biasCorrected, boolean keepDims, long... dimensions) {
  ...
  public List<SDVariable> doDiff(List<SDVariable> grad) {
    ...
    return new VarianceBp(sameDiff, arg(), grad.get(0), biasCorrected, keepDims, dimensions)
        .outputs();
  }
  ...
  public DataType resultType(OpContext oc) {
    ...
    if (x != null && x.isR()) return x.dataType();
  ...
  public List<LongShapeDescriptor> calculateOutputShape(OpContext oc) {
    ...
    long[] inputShape =
        (argShape == null || Shape.isPlaceholderShape(argShape) ? x.shape() : argShape);
  ...
  public List<DataType> calculateOutputDataTypes(List<DataType> dataTypes) {
    Preconditions.checkState(
        ...
        getClass(),
        dataTypes);
    // Variance and stdev reduction: Always FP out, but if FP in is float/double/half then it's
    // float/double/half out
    // If not FP in, then return default FP type out
    if (dataTypes.get(0).isFPType()) return dataTypes;
