...
public abstract class AbstractSession<T, O> {
  ...
  /**
   * All execution in Samediff happens in a frame... this is the name of the main/outer frame -
   * i.e., the "default" frame Other frames (such as for loops) may be nested within this frame
   */
  ...
  @Getter
  protected final Map<VarId, SDValue> nodeValueOutputs =
      new LinkedHashMap<>(); // Key: variable (at a given frame +
  ...
  /**
   * Contains variables we *might* need to execute in process of getting outputs we want. Variables
   * not in this set are definitely not needed to get the requested output variables, but variables
   * that are in this set may not be executed depending on the graph structure - i.e., switch ops,
   * etc
   */
  ...
  /**
   * Contains the names of ops that don't have any inputs. Kept because normally ops are triggered
   * for execution when their all their inputs have been calculated; we'll trigger that step
   * manually during execution initialization
   */
  ...
  public SDValue get(
    ...
    if (enforceExistence) {
      Preconditions.checkNotNull(
          out,
          "No output found for variable %s (frame %s, iteration %s)",
          variable,
          frame,
          iteration);
    }
  ...
  /**
   * Get the output of the session - i.e., perform inference/forward pass and return the outputs for
   ...
   * @param requiredActivations Additional activations that are required. Won't be output, but
   *     opExecution will be called. May be null.
   * @return The specified variable values, optionally in the specified workspace
   */
  public Map<String, T> output(
  ...
  public ExecutionResult output(
    ...
    Preconditions.checkState(
        !variables.isEmpty() || !requiredActivations.isEmpty(),
      ...
      for (Map.Entry<String, T> placeHolderValue : placeholderValues.entrySet()) {
        if (otherPlaceHolderValues.containsKey(placeHolderValue.getKey())) {
          throw new IllegalArgumentException(
              "Unable to determine which placeholder to use. Please ensure all names across both"
                  + " placeholders are unique");
        }

        otherPlaceHolderValues.put(
            placeHolderValue.getKey(), SDValue.create((INDArray) placeHolderValue.getValue()));
      }
    }

    if (requiredActivations == null) requiredActivations = Collections.emptySet();
    ...
    for (String s : variables) {
      Preconditions.checkState(
          sameDiff.variableMap().containsKey(s),
          "Requested output variable %s does not exist in SameDiff instance",
          s);
    }
        ...
        if (required && (presentPlaceholders.isEmpty() || !presentPlaceholders.contains(s))) {
          throw new IllegalStateException(
              "An input placeholder \""
                  + s
                  + "\" is required to calculate the requested outputs,"
    ...
    ExecStep start =
        new ExecStep(
            ExecType.EXEC_START, "", null); // Dummy dependency to trigger the variables and
    // constants
    for (SDVariable v : sameDiff.variables()) {
      ...
      if (vt == VariableType.VARIABLE || vt == VariableType.CONSTANT) {
        ...
        if (var.getControlDeps() != null) {
          addVarControlDeps(
              es, var); // Before this variable can be considered available for use, we need
    ...
    while (allExecuted.size() < allRequired.size()) {
      ...
      boolean skipDepUpdate =
          false; // Only used for Switch ops, which have slightly different handling...
      ...
      if (es.getType() == ExecType.CONSTANT || es.getType() == ExecType.VARIABLE) {
        ...
        } else if (otherPlaceHolderValues != null
        ...
        if (allRequired.contains(es.getName())) {
          Preconditions.checkState(
              placeholderValues != null && !placeholderValues.containsKey(es.getName())
                  || otherPlaceHolderValues != null
                      && otherPlaceHolderValues.containsKey(es.getName()),
              "No array was provided for the placeholder variable \"%s\" that is required for"
                  + " execution",
              es.getName());
          // User requested placeholder value as one of the outputs
          if (placeholderValues.containsKey(es.getName()))
            outValues.put(
                es.getName(), SDValue.create((INDArray) placeholderValues.get(es.getName())));
        ...
        O parameterizedOp =
            getAndParameterizeOp(
                ...
                constAndPhInputs,
                placeholderValues,
                reqOutputVariablesSet,
                otherPlaceHolderValues);
        ExecutionResult opOutputValues =
            getOutputs(
                parameterizedOp,
                outFrameIter,
                inputs,
        ...
        if (!opOutVarNames.isEmpty() && opOutputValues.hasSingle()) {
          Preconditions.checkState(
              lengthToCheck == opOutVarNames.size(),
              "Unexpected number of outputs from executed op %s:"
                  + " got %s outputs when %s outputs were expected (%s)",
              parameterizedOp.getClass().getSimpleName(),
              opOutputValues.numResults(),
              opOutVarNames.size(),
              opOutVarNames);
        }
        ...
        for (int i = 0; i < lengthToCheck; i++) {
          if (opOutputValues.hasSingle() && opOutputValues.resultAt(i) == null
              || opOutputValues.hasValues()
                  && !opOutputValues.valueExistsAtIndex(i)
          ...
          VarId vid =
              new VarId(
                  n,
                  outFrameIter.getFrame(),
                  outFrameIter.getIteration(),
        ...
        if (o instanceof Switch) {
          ...
          if (argNames[0].equals(argNames[1])) {
            ...
            Preconditions.checkState(
                nullCount == 1,
                "Expected exactly one output to be present for switch ops, got %s",
                nullCount);
            ...
            int nullCount =
                (opOutputValues.valueExistsAtIndex(0) ? 1 : 0)
    ...
    return ExecutionResult.builder().valueOutputs(outValues).build();
  }
  ...
  protected void addVarControlDeps(ExecStep es, Variable v) {
    ...
    if (cds != null) {
      for (String s : cds) {
        ...
        dt.addDependency(
            es, controlES); // Before this variable can be considered available for use, we need
        // specified op to be executed
      }
    }
  }
  ...
  /**
   * Execution failed - can't calculate all requested outputs, and there's nothing left to
   * calculate. Throws an exception with a useful message
   *
   * @param userRequestedUnique All outputs that the user requested
   * @param out Current outputs
   * @param step Execution step
   */
  protected void execFailed(
      Set<String> userRequestedUnique,
    ...
    sb.append("No variable are available for execution at step ")
        ...
        .append(": ")
        .append(missingCount)
        .append(" requested output values remaining, ")
        .append(allExecuted.size() - allRequired.size())
        .append(" variables required to be executed remaining");
    ...
    log.warn(
        "Not all required variables were executed. This may be due to conditionals. Missing"
            + " variables include: "
            + sb.toString());
  }

  /**
   * Update the descendant dependencies So if the graph structure is X -> A, then add all
   * (X,Y,Z,...) -> A to the dependency tracker This is for a specific frame and iteration, for both
   * sides of the dependency (in and out)
   *
  ...
  protected void updateDescendantDeps(ExecStep justExecuted, FrameIter outFrameIter) {
    ...
    if (justExecuted.getType() == ExecType.OP) {
    ...
    } else if (justExecuted.getType() == ExecType.SWITCH_L
        || justExecuted.getType() == ExecType.SWITCH_R) {
      SameDiffOp op = sameDiff.getOps().get(n);
      List<String> outNames = op.getOutputsOfOp();
      String branchVarName =
          (justExecuted.getType() == ExecType.SWITCH_L ? outNames.get(0) : outNames.get(1));
      ...
      throw new UnsupportedOperationException(
          "Unknown or not yet implemented exec type: " + justExecuted);
    }
  }

  /**
   * Suppose operation X has just been executed. For X -> someOp, add all dependencies for someOp,
   * i.e., all Z -> someOp (which includes X, but may not only be X)
   *
  ...
  /**
   * Get the ExecStep for the given variable, given execution is happening at the specified
   * frame/iteration
   */
  protected ExecStep getExecStepForVar(String varName, FrameIter frameIter) {
    ...
    if (vt == VariableType.VARIABLE) {
      return new ExecStep(
          ExecType.VARIABLE, v.getVariable().name(), new FrameIter(OUTER_FRAME, 0, null));
    } else if (vt == VariableType.PLACEHOLDER) {
      return new ExecStep(
          ExecType.PLACEHOLDER, v.getVariable().name(), new FrameIter(OUTER_FRAME, 0, null));
    } else if (vt == VariableType.CONSTANT) {
      return new ExecStep(
          ExecType.CONSTANT, v.getVariable().name(), new FrameIter(OUTER_FRAME, 0, null));
    } else {
      ...
      if (sdo == null) {
        throw new IllegalStateException(
            "Samediff output op named "
                + v.getName()
                + " did not have any ops associated with it.");
      }
      ...
      if (sdo.getOp() instanceof Switch) {
        ...
        if (idx == 0) {
          ...
          throw new IllegalStateException(
              "Expected variable \""
                  + v.getName()
                  + "\" to be an output of operation \""
                  + outOfOp
                  + "\", but op output variables are: "
                  + opOutputs);
        }
  ...
  /**
   * Initialize the subgraph - the subgraph and subgraphOps sets This works our what ops and
   * variables we might need to execute to get the requested outputs. In general, this is a subset
   * of the graph.
   *
  ...
  protected void initSubgraph(Set<String> variables) {
    ...
    while (!processingQueue.isEmpty()) {
      ...
      String opName =
          stripVarSuffix(
              sameDiff.getVariableOutputOp(varName) == null
                  ? null
                  : sameDiff.getVariableOutputOp(varName).getOwnName());

      if (!subgraph.contains(varName)) {
        String[] opInputs =
            opName == null ? null : sameDiff.getInputsForOp(sameDiff.getOpById(opName));
  ...
  /**
   * Preprocess the placeholder values, if required. Mainly reserved for casting in the case of
   * InferenceSession
   *
   * @param placeholders Placeholders to preprocess.
   * @return Preprocessed placeholders
   */
  protected Map<String, SDValue> preprocessValuePlaceholders(
      Map<String, SDValue> placeholders, At at) {
    return placeholders;
  }
  ...
  /**
   * Get the constant or variable output - for example, constant array or constant shape. Note that
   * both constants and variables (i.e., VariableType.CONSTANT and VariableType.VARIABLE) are the
   * same for all frames and iterations.
   *
  ...
  /**
   * Get the parameterized op to execute - for example, the op/DifferentialFunction with all inputs
   * set
   *
   * @param opName Name of the op
   * @param frameIter The frame and iteration of the op outputs
   * @param inputs The inputs to the op (excluding constants/placeholders) - for the specific frame
   *     + iteration
   * @param allIterInputs The inputs - those that are not iteration-specific (mainly Enter op vars,
   *     which might be used in all iterations but are only executed once on iter 0)
   * @param constAndPhInputs The constant and placeholder inputs - used for all frames/iterations
   * @param allReqVariables All required variables requested for the current session execution (not
   *     just the current op outputs)
   * @param otherPlaceholders
   * @return The parameterized op
   */
  public abstract O getAndParameterizeOp(
      String opName,
      FrameIter frameIter,
      Set<VarId> inputs,
   ...
   */
  public abstract ExecutionResult getOutputs(
      O op,
  ...
  /**
   * Get the VarId from the specified name. The VarId should be in one or the other of the
   * collections, and only one VarId with that name should exist
   */
  protected static VarId lookup(
      String name,
      Collection<VarId> varIds,
      Collection<VarId> varIds2,
      boolean exceptionOnNotFound) {
    VarId vid = varIds == null ? null : lookup(name, varIds, false);
    if (vid == null && varIds2 != null) vid = lookup(name, varIds2, false);
  ...
  /**
   * Get the {@link INDArray} associated with the given variable name
   *
   * @param name the variable name
   * @return the list of {@link INDArray}
   */
  public List<INDArray> getTensorArraysInSession(
      String name, String frame, int iteration, FrameIter parentFrame) {
    ...
    varIds.addAll(
        nodeValueOutputs.entrySet().stream()
            .filter(
                input ->
                    input.getValue() != null
                        && input.getValue().getSdValueType() == SDValueType.LIST)
    ...
    if (lookup == null && op.args().length > 0) {
      ...
      if (lookup != null) {
        List<INDArray> ret =
            nodeValueOutputs.containsKey(lookup)
                ? nodeValueOutputs.get(lookup).getListValue()
                : null;
        if (ret == null && parentFrame != null) return getTensorArraysInSession(name);
      }
      return null;
    }
  ...
  /**
   * Get the VarId from the specified name. The VarId should be in the collection, and only one
   * VarId with that name should exist
   */
  protected static VarId lookup(
      String name, Collection<VarId> varIds, boolean exceptionOnNotFound) {
   ...
   * SWITCH_L and SWITCH_R: This is a bit of a hack to account for the fact that only one of the
   * switch branches (left or right) will ever be available; without this, once the switch op is
   * executed, we'll (incorrectly) conclude that *both* branches can be executed<br>
   * EXEC_START: Start of execution<br>
   * CONTROL_DEP: Control dependency for op. Used for TF import, due to its odd "constant depends on
   * op in a frame" behaviour
   */
  ...
  /**
   * ExecStep represents a single execution step, for a single op (or variable/constant etc) at a
   * specific frame/iteration
   */
  @Getter
  @EqualsAndHashCode
  protected static class ExecStep {
    ...
    protected VarId toVarId() {
      return new VarId(
          name, frameIter.getFrame(), frameIter.getIteration(), frameIter.getParentFrame());
    }
