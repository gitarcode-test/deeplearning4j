...
public class NativeOpsHolder {
  ...
  @Getter @Setter private NativeOps deviceNativeOps;
  ...
  public static int getCores(int totals) {
    ...
    if (ht_off > 24) {
      int rounds = 0;
      while (ht_off
          > 24) { // we loop until final value gets below 24 cores, since that's reasonable
      ...
      if (ht_off == 20 && rounds < 2) ht_off /= 2;
      ...
      if (ht_off <= 6) {
        ...
        // value
        if (ht_off == 20 || ht_off == 16) ht_off /= 2;
      }
    }
    return ht_off;
  }
    ...
    try {
      ...
      if (getDeviceNativeOps() == null
          && Boolean.parseBoolean(
              System.getProperty(ND4JSystemProperties.INIT_NATIVEOPS_HOLDER, "true"))) {
        ...
        if (!System.getProperties().containsKey(Nd4j.NATIVE_OPS) && nativeOpsDefault == null) {
          throw new IllegalStateException(
              "No native operations class found. Please either define native.ops as a system"
                  + " property or fix your relevant backend properties file.");
        }
        String name = System.getProperty(Nd4j.NATIVE_OPS, nativeOpsDefault.toString());
        Class<? extends NativeOps> nativeOpsClass =
            ND4JClassLoading.loadClassByName(name).asSubclass(NativeOps.class);
      ...
      throw new RuntimeException(
          "ND4J is probably missing dependencies. For more information, please refer to:"
  ...
  public void initOps() {
    ...
    if (numThreadsString != null && !numThreadsString.isEmpty()) {
      ...
      } else deviceNativeOps.setOmpNumThreads(getCores(Runtime.getRuntime().availableProcessors()));
    }
    ...
    try {
      ...
      String vednnUrl =
          "org/nd4j/linalg/cpu/nativecpu/bindings/linux-x86_64-vednn-avx2/libnd4jcpu_device.vso";
      String vednnUrlGraal = "linux-x86_64-vednn-avx2/libnd4jcpu_device.vso";

      String vednnUrlStatic =
          "org/nd4j/linalg/cpu/nativecpu/bindings/linux-x86_64-vednn-avx2/libnd4jcpu_device.vsa";
      String vednnUrlGraalStatic = "linux-x86_64-vednn-avx2/libnd4jcpu_device.vsa";

      for (String url :
          new String[] {vednnUrl, vednnUrlGraal, vednnUrlStatic, vednnUrlGraalStatic}) {
