...
/**
 * This is a mirror of the C++ ArrayOptionsHelper class. It contains the bit mask options for the
 * shape information buffer. These options are used to determine the data type, array type, and
 * other properties of the array.
 *
 * <p>The ArrayOptions implementation and header for c++ can be found at this link:
...
public class ArrayOptionsHelper {
  ...
  public static final long DTYPE_BYTE_BIT =
      32768; // Also used for UBYTE in conjunction with sign bit
  ...
  /**
   * Returns true if the given shape info has the {@link #hasBitSet(long, long)} with the property
   * {@link #IS_VIEW}
   *
  ...
  public static ArrayType arrayType(long opt) {
    if (hasBitSet(opt, ATYPE_SPARSE_BIT)) return ArrayType.SPARSE;
    else if (hasBitSet(opt, ATYPE_COMPRESSED_BIT)) return ArrayType.COMPRESSED;
    else if (hasBitSet(opt, ATYPE_EMPTY_BIT)) return ArrayType.EMPTY;
    else return ArrayType.DENSE;
  }
  ...
  public static DataType dataType(long opt) {
    if (hasBitSet(opt, DTYPE_COMPRESSED_BIT)) return DataType.COMPRESSED;
    else if (hasBitSet(opt, DTYPE_HALF_BIT)) return DataType.FLOAT16;
    else if (hasBitSet(opt, DTYPE_BFLOAT16_BIT)) return DataType.BFLOAT16;
    else if (hasBitSet(opt, DTYPE_FLOAT_BIT)) return DataType.FLOAT;
    else if (hasBitSet(opt, DTYPE_DOUBLE_BIT)) return DataType.DOUBLE;
    ...
    else if (hasBitSet(opt, DTYPE_BOOL_BIT)) return DataType.BOOL;
    else if (hasBitSet(opt, DTYPE_BYTE_BIT)) {
      return hasBitSet(opt, DTYPE_UNSIGNED_BIT)
          ? DataType.UINT8
          : DataType.INT8; // Byte bit set for both UBYTE and BYTE
    } else if (hasBitSet(opt, DTYPE_SHORT_BIT))
      return hasBitSet(opt, DTYPE_UNSIGNED_BIT) ? DataType.UINT16 : DataType.INT16;
    else if (hasBitSet(opt, DTYPE_UTF8_BIT)) return DataType.UTF8;
    else throw new ND4JUnknownDataTypeException("Unknown extras set: [" + opt + "]");
  }
