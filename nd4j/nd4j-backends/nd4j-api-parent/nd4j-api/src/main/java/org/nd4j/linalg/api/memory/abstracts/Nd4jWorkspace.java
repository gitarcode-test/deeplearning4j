...
public abstract class Nd4jWorkspace implements MemoryWorkspace {
  ...
  // types
  @Getter @Setter protected Enum associatedEnumType;
  ...
  @Setter protected MemoryWorkspace previousWorkspace;
  ...
  public static final int alignmentBase = 32;
  ...
  public Nd4jWorkspace(@NonNull WorkspaceConfiguration configuration, @NonNull String workspaceId) {
    ...
    else workspaceType = Type.SCOPED;
    ...
    if (configuration.getPolicyLocation() == LocationPolicy.MMAP) {
      ...
      if (configuration.getTempFilePath() != null) {
        ...
        throw new ND4JIllegalStateException(
            "MMAP target file path should be non-null or workspace initialSize should be >0 for"
                + " temp file");
    }
   ...
   *
   * <p>PLEASE NOTE: It shows only amount of HOST memory. If current backend assumes DEVICE/HOST
   * memory pair, DEVICE memory will probably have the same size, but won't be accounted in this
   * value.
   *
   * @return
   */
  ...
  protected void init() {
    ...
    if (workspaceConfiguration.getPolicyLocation() == LocationPolicy.MMAP
        && workspaceConfiguration.getPolicyLearning() != LearningPolicy.NONE)
      throw new IllegalArgumentException(
          "Workspace backed by memory-mapped file can't have LearningPolicy defined");

    // we don't want overallocation in case of MMAP
    if (currentSize.get() > 0
        && workspaceConfiguration.getPolicyLocation() != LocationPolicy.MMAP) {
      if (!isOver.get()) {
      ...
      if (workspaceConfiguration.getMaxSize() > 0
          && currentSize.get() > workspaceConfiguration.getMaxSize())
  ...
  public static long alignMemory(long requiredMemory) {
    ...
    if (div != 0) requiredMemory += (alignmentBase - div);
  ...
  public PagedPointer alloc(
    ...
    */
    ...
    if (!isUsed.get()) {
      ...
      AllocationsTracker.getInstance()
          .getTracker(id)
          .allocateExternal(type, kind, numElements, requiredMemory);
      return pointer;
    }
    ...
    */
    ...
    if (hostOffset.get() + requiredMemory <= currentSize.get()
      ...
      if (isDebug.get())
        log.info(
            "Workspace [{}]: Allocating array of {} bytes, capacity of {} elements, prevOffset: {};"
      ...
      // if current workspace isn't enough - we allocate it separately as spilled (or pinned, in
      // case of circular mode)

      // in case of circular mode - we just reset offsets, and start from the beginning of the
      // workspace
      if (workspaceConfiguration.getPolicyReset() == ResetPolicy.ENDOFBUFFER_REACHED
          && currentSize.get() > 0
          && !trimmer
      ...
      if (isDebug.get())
        log.info(
            "Workspace [{}]: step: {}, spilled  {} bytes, capacity of {} elements",
            id,
            stepsCount.get(),
            requiredMemory,
            numElements);
        ...
        case EXTERNAL:
          ...
          if (!trimmer) {
            ...
            pinnedAllocations.add(
                new PointersPair(stepsCount.get(), requiredMemory, pointer, null));
  ...
  public void initializeWorkspace() {
    ...
    // if we're in cyclic mode, we do reallocations only after 2 full cycles, to avoid race
    // conditions
    ...
    if (!isInit.get())
      if (workspaceConfiguration.getPolicyLearning() != LearningPolicy.NONE) {
        ...
        else currentSize.set(maxCycle.get());
        ...
        // handling optional overallocation here, however it's usually good idea to use it
        // everywhere, to avoid frequent realloc calls
        if (!isOver.get()) {
          if (workspaceConfiguration.getPolicyAllocation() == AllocationPolicy.OVERALLOCATE
              && workspaceConfiguration.getOverallocationLimit() > 0
              && currentSize.get() > 0) {
        ...
        if (workspaceConfiguration.getMinSize() > 0
            && currentSize.get() < workspaceConfiguration.getMinSize())
          currentSize.set(workspaceConfiguration.getMinSize());

        // purge spilled allocations
        if (externalCount.get() > 0
            && (workspaceConfiguration.getPolicyReset() == ResetPolicy.BLOCK_LEFT
                || resetPlanned.get())) {
          clearExternalAllocations();
          resetPlanned.set(false);
        }

        // calling for implementation-specific workspace initialization. basically allocation
        // happens there
        init();
      }
  }
   ...
   *
   * <p>PLEASE NOTE: This method can return non-zero calues only for circular workspace mode
   *
   * @return
   */
  ...
  public void destroyWorkspace(boolean extended) {
    if (workspace.getHostPointer() != null
        && workspace.getHostPointer().getOriginalPointer() != null
  ...
  public MemoryWorkspace notifyScopeBorrowed() {
    if (isBorrowed.get())
      throw new ND4JIllegalStateException(
          "Workspace [" + id + "]: Can't borrow from borrowed workspace");
  ...
  public void close() {
    ...
    // next we check, if the same workspace was opened multiple times sequentially. then we just
    // decrement counter, without reset
    if (tagScope.get() > 0) {
      if (tagScope.decrementAndGet() == 0) {
        Nd4j.getMemoryManager().setCurrentWorkspace(this);
      }
    ...
    // Furthermore, need to commit before marking workspace as closed, to avoid (incorrectly)
    // hitting scope panic
    ...
    */
    ...
    // if during this cycle we've used more memory then before - increase max count. we'll use it in
    // future for optional reallocation
    if (cycleAllocations.get() > maxCycle.get()) {
      if (isDebug.get())
        log.info(
            "Workspace [{}] device_{}, current cycle: {}; max cycle: {}",
            id,
            Nd4j.getAffinityManager().getDeviceForCurrentThread(),
            cycleAllocations.get(),
            maxCycle.get());
    ...
    if (workspaceConfiguration.getPolicyLearning() != LearningPolicy.NONE && maxCycle.get() > 0) {
      ...
      if ((workspaceConfiguration.getPolicyLearning() == LearningPolicy.OVER_TIME
        ...
        if (Nd4j.getWorkspaceManager().getDebugMode() != DebugMode.SPILL_EVERYTHING)
          initializeWorkspace();
      } else if (currentSize.get() > 0
          && cycleAllocations.get() > 0
          && workspaceConfiguration.getPolicySpill() == SpillPolicy.REALLOCATE
          && workspaceConfiguration.getPolicyReset() != ResetPolicy.ENDOFBUFFER_REACHED) {
    ...
    if (pinnedCount.get() > 0) clearPinnedAllocations(false);

    // if we're in trimmed mode (preparing for reallocation of circular buffer) - we can do it 2
    // generations after
    if (trimmedMode.get() && trimmedStep.get() + 2 < stepsCount.get()) {
    ...
    if (workspaceConfiguration.getPolicyReset() == ResetPolicy.BLOCK_LEFT) {
      ...
      // for variable input we want to ensure alignment to max block, to avoid accidental buffer
      // overruns
      long diff = initialBlockSize.get() - cycleAllocations.get();

      // we don't care about offsets if that's trimmed mode, offsets will be reset anyway upon
      // reallocation
      if (diff > 0 && !trimmedMode.get() && deviceOffset.get() > 0) {

        if (isDebug.get())
          log.info(
              "Workspace [{}]: Align to [{}]; diff: [{}]; block size: [{}]; currentOffset: [{}];"
                  + " workspaceSize: [{}]; trimmedMode: {}",
              id,
   ...
   *
   * <p>PLEASE NOTE: Never call this method unless you realize all consequences
   */
  public void reset() {
    hostOffset.set(0);
    deviceOffset.set(0);
