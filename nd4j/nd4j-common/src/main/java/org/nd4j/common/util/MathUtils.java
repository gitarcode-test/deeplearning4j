...
public class MathUtils {
  ...
  /**
   * See: <a
   * href="https://stackoverflow.com/questions/466204/rounding-off-to-nearest-power-of-2">https://stackoverflow.com/questions/466204/rounding-off-to-nearest-power-of-2</a>
   *
  ...
  /**
   * Calculate string similarity with tfidf weights relative to each character frequency and how
   * many times a character appears in a given string
   *
  ...
  /**
   * Inverse document frequency: the total docs divided by the number of times the word appeared in
   * a document
   *
  ...
  /**
   * This returns the minimized loss values for a given vector. It is assumed that the x, y pairs
   * are at vector[i], vector[i+1]
   *
  ...
  public static double w_1(double[] x, double[] y, int n) {
    return (n * sumOfProducts(x, y) - sum(x) * sum(y))
        / (n * sumOfSquares(x) - Math.pow(sum(x), 2));
  }
   ...
   * @return the product of all numbers in the array, or 0 if the length is or or nums i null
   */
   ...
   * @return a double array containing all of the numbers in that column for all of the arrays.
  ...
  /**
   * This returns the coordinate split in a list of coordinates such that the values for ret[0] are
   * the x values and ret[1] are the y values
   *
   * @param vector the vector to split with x and y values/
   * @return a coordinate split for the given vector of values. if null, is passed in null is
   *     returned
   */
  ...
  /**
   * This will partition the given whole variable data applyTransformToDestination in to the
   * specified chunk number.
   *
   ...
   * the x values and ret[1] are the y values
   *
   * @param vector the vector to split with x and y values Note that the list will be more stable
   *     due to the size operator. The array version will have extraneous values if not monitored
   *     properly.
   * @return a coordinate split for the given vector of values. if null, is passed in null is
   *     returned
   */
  ...
  /**
   * This returns the x values of the given vector. These are assumed to be the even values of the
   * vector.
   *
  ...
  /**
   * This calculates the adjusted r^2 including degrees of freedom. Also known as calculating
  ...
  public static double min(double[] doubles) {
    ...
    for (double d : doubles) if (d < ret) ret = d;
    return ret;
  }
  ...
  /**
   * Converts an array containing the natural logarithms of probabilities stored in a vector back
   * into probabilities. The probabilities are assumed to sum to one.
   *
  ...
  /**
   * Rounds a double to the next nearest integer value in a probabilistic fashion (e.g. 0.8 has a
   * 20% chance of being rounded down to 0 and a 80% chance of being rounded up to 1). In the limit,
   * the average of the rounded numbers generated by this procedure should converge to the original
   * double.
   *
   * @param value the double value
  ...
  /**
   * This will return the bernoulli trial for the given event. A bernoulli trial is a mechanism for
   * detecting the probability of a given event occurring k times in n independent trials
   *
   ...
   * @return a z value such that the value is the interleaved lsd to msd for each double in the
   *     vector
   */
  ...
  /**
   * This will calculate the Manhattan distance between two sets of points. The Manhattan distance
   * is equivalent to: 1_sum_n |p_i - q_i|
   *
  ...
  /**
   * Shuffle the array elements using the specified RNG seed. Uses Fisher Yates shuffle internally:
  ...
  /**
   * Shuffle the array elements using the specified Random instance Uses Fisher Yates shuffle
   * internally: <a
   * href="https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle#The_modern_algorithm">
   * https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle#The_modern_algorithm</a>
   *
   * @param array Array to shuffle
   ...
   * If shuffleFirst < array.length, only the elements 0 to shuffleFirst-1 are modified; values at
   * indices shuffleFirst to array.length-1 are not changed. Uses Fisher Yates shuffle internally:
   * <a href="https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle#The_modern_algorithm">
   * https://en.wikipedia.org/wiki/Fisher%E2%80%93Yates_shuffle#The_modern_algorithm</a>
   *
