...
public class ClassPathResource extends AbstractFileResolvingResource {
  ...
  /**
   * Get the File. If the file cannot be accessed directly (for example, it is in a JAR file), we
   * will attempt to extract it from the JAR and copy it to the temporary directory, using {@link
  ...
  public File getFile() throws IOException {
    try {
      ...
      // java.io.FileNotFoundException: class path resource [iris.txt] cannot be resolved to
      // absolute file path because
      // it does not reside in the file system:
      // jar:file:/.../dl4j-test-resources-0.9.2-SNAPSHOT.jar!/iris.txt
   ...
   * This is for resources where a file is needed and the classpath resource is in a jar file. The
   * file is copied to the default temporary directory, using {@link Files#createTempFile(String,
   * String, FileAttribute[])}. Consequently, the extracted file will have a different filename to
   * the extracted one.
   *
   ...
   * Note that:<br>
   * - If the directory is not specified, the file is copied to the default temporary directory,
   * using {@link Files#createTempFile(String, String, FileAttribute[])}. Consequently, the
   * extracted file will have a different filename to the extracted one.<br>
   * - If the directory *is* specified, the file is copied directly - and the original filename is
   * maintained
   *
  ...
  /**
   * Extract the directory recursively to the specified location. Current ClassPathResource must
   * point to a directory.<br>
   * For example, if classpathresource points to "some/dir/", then the contents - not including the
   * parent directory "dir" - will be extracted or copied to the specified destination.<br>
   *
  ...
  public void copyDirectory(File destination) throws IOException {
    Preconditions.checkState(
        ...
        "Destination directory must exist and be a directory: %s",
        destination);
    ...
    if (isJarURL(url)) {
      ...
      */
      ...
      try {
        ...
        Preconditions.checkState(
            entry.isDirectory(), "Source must be a directory: %s", entry.getName());
        ...
        while (entries.hasMoreElements()) {
          ...
          if (name.startsWith(pathNoSlash)
              && name.length() > pathNoSlash.length()
              && (name.charAt(pathNoSlash.length()) == '/'
                  || name.charAt(pathNoSlash.length())
                      == '\\')) { // second condition: to avoid "/dir/a/" and "/dir/abc/" both
                                  // matching startsWith
            ...
            if (e.isDirectory()) {
              ...
              try (BufferedOutputStream bos =
                  new BufferedOutputStream(new FileOutputStream(extractTo))) {
      ...
      Preconditions.checkState(
          destination.exists() && destination.isDirectory(),
          "Destination must be a directory and must exist: %s",
          destination);
  ...
  private static InputStream getInputStream(String path, Class<?> clazz, ClassLoader classLoader)
      throws IOException {
    InputStream is;
    if (clazz != null) {
    ...
    if (is == null) {
      ...
      if (is instanceof BufferedInputStream) return is;
