...
public class CompactHeapStringList implements List<String> {
  ...
  public static final int DEFAULT_INTEGER_REALLOCATION_BLOCK_SIZE_BYTES =
      1024 * 1024; // 1MB - 262144 ints, 131k entries
  ...
  /**
   * @param reallocationBlockSizeBytes Number of bytes by which to increase the char[], when
   *     allocating a new storage array
   * @param intReallocationBlockSizeBytes Number of bytes by which to increase the int[], when
   *     allocating a new storage array
   */
    ...
    if (2 * (usedCount + 1) >= offsetAndLength.length) {
      ...
      int newLength =
          offsetAndLength.length
              + Math.min(
                  reallocationIntegerBlockSizeBytes / 4,
  ...
  public String get(int index) {
    if (index >= usedCount) {
      throw new IllegalArgumentException(
          "Invalid index: " + index + " >= size(). Size = " + usedCount);
    }
