...
public abstract class VfsUtils {
  ...
  static long getSize(Object vfsResource) throws IOException {
    return ((Long) invokeVfsMethod(VIRTUAL_FILE_METHOD_GET_SIZE, vfsResource, new Object[0]))
        .longValue();
  }

  static long getLastModified(Object vfsResource) throws IOException {
    return ((Long)
            invokeVfsMethod(VIRTUAL_FILE_METHOD_GET_LAST_MODIFIED, vfsResource, new Object[0]))
        .longValue();
  }

  static InputStream getInputStream(Object vfsResource) throws IOException {
    return (InputStream)
        invokeVfsMethod(VIRTUAL_FILE_METHOD_GET_INPUT_STREAM, vfsResource, new Object[0]);
  }
  ...
  static File getFile(Object vfsResource) throws IOException {
    if (VfsUtils.VFS_VER.V2.equals(version)) {
      if (((Boolean)
              invokeVfsMethod(VFS_UTILS_METHOD_IS_NESTED_FILE, null, new Object[] {vfsResource}))
          .booleanValue()) {
        ...
        try {
          return new File(
              (URI)
                  invokeVfsMethod(
                      VFS_UTILS_METHOD_GET_COMPATIBLE_URI, null, new Object[] {vfsResource}));
  ...
  static {
    ...
    try {
      ...
      try {
        ...
        logger.error(
            "JBoss VFS packages (for both JBoss AS 5 and 6) were not found - JBoss VFS support"
                + " disabled");
    ...
    try {
      ...
      VIRTUAL_FILE_METHOD_GET_CHILD =
          ReflectionUtils.findMethod(virtualFile, ex, new Class[] {String.class});
      ...
      VIRTUAL_FILE_METHOD_VISIT =
          ReflectionUtils.findMethod(
              virtualFile, "visit", new Class[] {VIRTUAL_FILE_VISITOR_INTERFACE});
