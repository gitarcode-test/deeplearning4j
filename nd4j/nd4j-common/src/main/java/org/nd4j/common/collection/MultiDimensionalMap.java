...
public class MultiDimensionalMap<K, T, V> implements Serializable {
   ...
   * of this call is equivalent to that of calling {@link Map<>#put(k, v)} on this map once for each
   * mapping from key <tt>k</tt> to value <tt>v</tt> in the specified map. The behavior of this
   ...
   * @throws NullPointerException if the specified map is null, or if this map does not permit null
  ...
  /**
   * Returns a {@link Set} view of the keys contained in this map. The applyTransformToDestination
   * is backed by the map, so changes to the map are reflected in the applyTransformToDestination,
   * and vice-versa. If the map is modified while an iteration over the applyTransformToDestination
   * is in progress (except through the iterator's own <tt>remove</tt> operation), the results of
   * the iteration are undefined. The applyTransformToDestination supports element removal, which
   ...
   * applyTransformToDestination is backed by the map, so changes to the map are reflected in the
   * applyTransformToDestination, and vice-versa. If the map is modified while an iteration over the
   * applyTransformToDestination is in progress (except through the iterator's own <tt>remove</tt>
  ...
  public static class Entry<K, T, V> implements Map.Entry<Pair<K, T>, V> {
     ...
     * (Writes through to the map.) The behavior of this call is undefined if the mapping has
     * already been removed from the map (by the iterator's <tt>remove</tt> operation).
     *
     ...
     * @throws IllegalArgumentException if some property of this value prevents it from being stored
     *     in the backing map
