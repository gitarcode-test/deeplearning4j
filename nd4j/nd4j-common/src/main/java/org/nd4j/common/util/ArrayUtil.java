...
public class ArrayUtil {
   ...
   * @return a boolean array with true values for nonzero integers and false values for zero
   *     integers
   */
   ...
   * @return whether the given target array is contained in the list
   */
  public static boolean listOfIntsContains(List<int[]> list, int[] target) {
    for (int[] arr : list) if (Arrays.equals(target, arr)) return true;
    return false;
  }
   ...
   * @return an array of length n filled with the given value
   */
  ...
  public static long[] nTimes(long n, long toReplicate) {
    if (n > Integer.MAX_VALUE) throw new RuntimeException("Index overflow in nTimes");
  ...
  public static <T> T[] nTimes(int n, T toReplicate, Class<T> tClass) {
    Preconditions.checkState(
        n >= 0, "Invalid number of times to replicate: must be >= 0, got %s", n);
  ...
  /**
   * Returns true if all the elements in the given int array are unique
   *
   * @param toTest the array to test
   * @return true if all the items are unique false otherwise
   */
   ...
   *
   * <p>Create a random permutation of the numbers 0, ..., size - 1. see Algorithm P, D.E. Knuth:
   * The Art of Computer Programming, Vol. 2, p. 145
   */
  ...
  public static int[] toInts(boolean[] data) {
    ...
    for (int i = 0; i < ret.length; i++) ret[i] = (int) (data[i] ? 1 : 0);
    return ret;
  }
   ...
   * @return the offset for the given stride
   */
   ...
   * Sum of an int array
   *
   * @param add the elements to calculate the sum for
  ...
  public static int sum(int[] add) {
    ...
    int ret = 0;
    for (int i = 0; i < add.length; i++) ret += add[i];
    return ret;
  }
   ...
   *
   * @param mult the elements to calculate the sum for
  ...
  public static int prod(long... mult) {
    ...
    int ret = 1;
    for (int i = 0; i < mult.length; i++) ret *= mult[i];
    return ret;
  }
  ...
  public static long prodLong(List<? extends Number> mult) {
    if (mult.isEmpty()) return 0;
    long ret = 1;
    for (int i = 0; i < mult.size(); i++) ret *= mult.get(i).longValue();
    return ret;
  }
  ...
  public static boolean anyMore(int[] target, int[] test) {
    Preconditions.checkArgument(
        target.length == test.length,
        "Unable to compare: different sizes: length %s vs. %s",
        target.length,
        test.length);
   ...
   * @return the offset for the given shape,offset,and strides
   */
  public static long calcOffsetLong(
      List<Integer> shape, List<Integer> offsets, List<Integer> strides) {
    if (shape.size() != offsets.size() || shape.size() != strides.size())
      throw new IllegalArgumentException("Shapes,strides, and offsets must be the same size");
    long ret = 0;
    for (int i = 0; i < offsets.size(); i++) {
  ...
  public static float[] floatCopyOf(double[] data) {
    if (data.length == 0) return new float[1];
    float[] ret = new float[data.length];
    for (int i = 0; i < ret.length; i++) ret[i] = (float) data[i];
    return ret;
  }
  ...
  /**
   * Generate an int array ranging from "from" to "to". The total number of elements is
   * (from-to)/increment - i.e., range(0,2,1) returns [0,1] If from is > to this method will count
   * backwards
   *
   * @param from the from
   * @param to the end point of the data
  ...
  /**
   * Generate an int array ranging from "from" to "to". The total number of elements is (from-to) -
   * i.e., range(0,2) returns [0,1] If from is > to this method will count backwards
   *
   * @param from the from
   * @param to the end point of the data
   * @return the int array with a length equal to absoluteValue(from - to)
  ...
  public static double[] toDoubles(int[] ints) {
    ...
    for (int i = 0; i < ints.length; i++) ret[i] = (double) ints[i];
    return ret;
  }
  ...
  public static float[] toFloats(boolean[] ints) {
    ...
    for (int i = 0; i < ints.length; i++) ret[i] = (float) (ints[i] ? 1 : 0);
    return ret;
  }
  ...
  public static int[] cutBelowZero(int[] data) {
    ...
    for (int i = 0; i < data.length; i++) ret[i] = data[i] < 0 ? 0 : data[i];
    return ret;
  }
   ...
   * @return the new array with the omitted item
   */
  ...
  public static int[] keep(int[] data, int... index) {
    ...
    int count = 0;
    for (int i = 0; i < data.length; i++) if (Ints.contains(index, i)) ret[count++] = data[i];
  ...
  public static long[] keep(long[] data, long... index) {
    ...
    int count = 0;
    for (int i = 0; i < data.length; i++) if (Longs.contains(index, i)) ret[count++] = data[i];
   ...
   * Return a copy of this array with the given index omitted
   *
   * <p>PLEASE NOTE: index to be omitted must exist in source array.
   *
   * @param data the data to copy
   * @param index the index of the item to remove
   * @return the new array with the omitted item
  ...
  public static long[] removeIndex(long[] data, long... index) {
    ...
    if (index.length >= data.length) {
      throw new IllegalStateException(
          "Illegal remove: indexes.length > data.length (index.length="
              + index.length
              + ", data.length="
              + data.length
              + ")");
  ...
  public static long[] getTensorMmulShape(long[] aShape, long[] bShape, int[][] axes) {
    ...
    for (int i = 0; i < validationLength; i++) {
      ...
      if (axes[0][i] < 0) axes[0][i] += aShape.length;
      if (axes[1][i] < 0) axes[1][i] += bShape.length;
    }
    ...
    if (listA.size() == 0) {
      ...
      for (int i = 0; i < oldShapeA.length; i++) oldShapeA[i] = aShape[(int) oldShapeA[i]];
    }
    ...
    if (listB.isEmpty()) {
      ...
      for (int i = 0; i < oldShapeB.length; i++) oldShapeB[i] = bShape[(int) oldShapeB[i]];
    }
  ...
  /**
   * Permute the given input switching the dimensions of the input shape array with in the order of
  ...
  /**
   * Original credit:
   * https://github.com/alberts/array4j/blob/master/src/main/java/net/lunglet/util/ArrayUtils.java
   *
   * @param a
   * @return
   */
  ...
  public static int[] argsort(final int[] a, final boolean ascending) {
    ...
    for (int i = 0; i < ret.length; i++) ret[i] = indexes[i];

    return ret;
  }

  /**
   * Convert all dimensions in the specified axes array to be positive based on the specified range
   * of values
   *
   * @param range
   * @param axes
  ...
  /**
   * Note this byte array conversion is a simple cast and not a true cast. Use {@link
   * #toByteArraySimple(long[])} for a true cast.
   *
   * @param longArray
   * @return
   */
   ...
   *
   * <p>The intent here is for striding.
   *
   * <p>For example in slicing, you want the major stride to be first. You achieve this by taking
   * the last index of the matrix's stride and putting this as the first stride of the new ndarray
   * for slicing.
   *
   * <p>All of the elements except the copied elements are initialized as the given value
   *
   ...
   */
  public static int[] valueStartingAt(
      int valueStarting, int[] copy, int idxFrom, int idxAt, int length) {
    int[] ret = new int[length];
    Arrays.fill(ret, valueStarting);
    for (int i = 0; i < length; i++) {
      if (i + idxFrom >= copy.length || i + idxAt >= ret.length) break;
  ...
  /**
   * Returns the array with the item in index removed, if the array is empty it will return the
   * array itself
   *
   * @param data the data to remove data from
   * @param index the index of the item to remove
   * @return a copy of the array with the removed item, or the array itself if empty
   */
  ...
  public static boolean isInverse(int[] first, int[] second) {
    ...
    for (int i = 0; i < first.length; i++) {
      if (first[i] != second[backWardCount--]) return false;
    }
    return true;
  }
  ...
  public static int[] plus(int[] ints, int mult) {
    ...
    for (int i = 0; i < ints.length; i++) ret[i] = ints[i] + mult;
    return ret;
  }
  ...
  public static int[] plus(int[] ints, int[] mult) {
    ...
    int[] ret = new int[ints.length];
    for (int i = 0; i < ints.length; i++) ret[i] = ints[i] + mult[i];
    return ret;
  }
  ...
  public static int[] times(int[] ints, int[] mult) {
    Preconditions.checkArgument(
        ints.length == mult.length, "Ints and mult must be the same length");
    int[] ret = new int[ints.length];
    for (int i = 0; i < ints.length; i++) ret[i] = ints[i] * mult[i];
    return ret;
  }
  ...
  public static int nonOneStride(int[] arr) {
    for (int i = 0; i < arr.length; i++) if (arr[i] != 1) return arr[i];
    return 1;
  }
  ...
  public static double[] read(int length, DataInputStream dis) throws IOException {
    ...
    for (int i = 0; i < length; i++) ret[i] = dis.readDouble();
    return ret;
  }
  ...
  public static float[] readFloat(int length, DataInputStream dis) throws IOException {
    ...
    for (int i = 0; i < length; i++) ret[i] = dis.readFloat();
    return ret;
  }
  ...
  public static boolean[] flatten(boolean[][] arr) {
    if (arr.length == 0 || arr[0].length == 0) return new boolean[0];
  ...
  public static String[] flatten(String[][] arr) {
    if (arr.length == 0 || arr[0].length == 0) return new String[0];
  ...
  public static String[] flatten(String[][][] arr) {
    if (arr.length == 0 || arr[0].length == 0 || arr[0][0].length == 0) return new String[0];
  ...
  public static boolean[] flatten(boolean[][][] arr) {
    if (arr.length == 0 || arr[0].length == 0 || arr[0][0].length == 0) return new boolean[0];
  ...
  public static float[] flatten(float[][] arr) {
    if (arr.length == 0 || arr[0].length == 0) return new float[0];
  ...
  public static float[] flatten(float[][][] arr) {
    if (arr.length == 0 || arr[0].length == 0 || arr[0][0].length == 0) return new float[0];
  ...
  public static double[] flatten(double[][][] arr) {
    if (arr.length == 0 || arr[0].length == 0 || arr[0][0].length == 0) return new double[0];
  ...
  public static int[] flatten(int[][][] arr) {
    if (arr.length == 0 || arr[0].length == 0 || arr[0][0].length == 0) return new int[0];
  ...
  public static short[] flatten(short[][][] arr) {
    if (arr.length == 0 || arr[0].length == 0 || arr[0][0].length == 0) return new short[0];
  ...
  public static byte[] flatten(byte[][][] arr) {
    if (arr.length == 0 || arr[0].length == 0 || arr[0][0].length == 0) return new byte[0];
  ...
  public static short[] flatten(short[][] arr) {
    if (arr.length == 0 || arr[0].length == 0) return new short[0];
  ...
  public static byte[] flatten(byte[][] arr) {
    if (arr.length == 0 || arr[0].length == 0) return new byte[0];
  ...
  public static long[] flatten(long[][] arr) {
    if (arr.length == 0 || arr[0].length == 0) return new long[0];
  ...
  public static long[] flatten(long[][][] arr) {
    if (arr.length == 0 || arr[0].length == 0 || arr[0][0].length == 0) return new long[0];
   ...
   */
  public static double[] flatten(double[][] arr) {
    if (arr.length == 0 || arr[0].length == 0) return new double[0];
  ...
  public static double[] flattenF(double[][] arr) {
    ...
    int count = 0;
    for (int j = 0; j < arr[0].length; j++)
      for (int i = 0; i < arr.length; i++) ret[count++] = arr[i][j];
    return ret;
  }
   ...
   * @return the elements of this array cast as an int
   */
  public static double[][] toDouble(int[][] arr) {
    double[][] ret = new double[arr.length][arr[0].length];
    for (int i = 0; i < arr.length; i++) {
      for (int j = 0; j < arr[i].length; j++) ret[i][j] = arr[i][j];
    }
    return ret;
  }
  ...
  public static double[] combine(float[]... ints) {
    int length = 0;
    for (int i = 0; i < ints.length; i++) length += ints[i].length;
    double[] ret = new double[length];
    int count = 0;
    for (float[] i : ints) {
      for (int j = 0; j < i.length; j++) {
  ...
  public static <E> E[] combine(E[]... arrs) {
    int length = 0;
    for (int i = 0; i < arrs.length; i++) length += arrs[i].length;
   ...
   * Note that the method assumes the array is rectangular (not ragged) and first elements (i.e.,
   * array[0][0][0]...) are non-null <br>
   * Note also that if allowSize0Dims is true, any elements are length 0, all subsequent dimensions
   * will be reported as 0. i.e., a double[3][0][2] would be reported as shape [3,0,0]. If
   * allowSize0Dims is false, an exception will be thrown for this case instead. Can pass any Java
   * array opType: double[], Object[][][], float[][], etc.<br>
   * Length of returned array is number of dimensions; returned[i] is size of ith dimension.
   */
  public static int[] arrayShape(Object array, boolean allowSize0Dims) {
    ...
    for (int i = 0; i < shape.length - 1; i++) {
      ...
      if (shape[i] == 0) {
        ...
        throw new IllegalStateException(
            "Cannot calculate array shape: Array has size 0 for dimension " + i);
      }
  ...
  /**
   * Returns the index of the maximum value in the array. If two entries have same maximum value,
  ...
  /**
   * Returns the index of the minimum value in the array. If two entries have same minimum value,
   * index of the first one is returned.
   */
  ...
  public static long[][][] toPrimitives(Long[][][] array) {
    ...
    for (int i = 0; i < array.length; i++)
      for (int j = 0; j < array[0].length; j++)
        for (int k = 0; j < array[0][0].length; k++) res[i][j][k] = array[i][j][k];
   ...
   * Can be used to check Object arrays with any number of dimensions (up to rank 4), or primitive
   * arrays with rank 2 or higher<br>
  ...
  public static <T> void assertNotRagged(T[] array) {
    ...
    if (rank >= 2) {
      for (int i = 1; i < arrayShape[0]; i++) {
        ...
        Preconditions.checkState(
            arrayShape[1] == len,
            "Ragged array detected: array[0].length=%s does not match array[%s].length=%s",
            arrayShape[1],
            i,
            len);
      }
    ...
    if (rank >= 3) {
      ...
      for (int i = 0; i < arrayShape[0]; i++) {
        for (int j = 0; j < arrayShape[1]; j++) {
          ...
          Preconditions.checkState(
              arrayShape[2] == len,
              "Ragged array detected: array[0][0].length=%s does not match array[%s][%s].length=%s",
              arrayShape[2],
              i,
              j,
              len);
      ...
      for (int i = 0; i < arrayShape[0]; i++) {
        for (int j = 0; j < arrayShape[1]; j++) {
          for (int k = 0; k < arrayShape[2]; k++) {
            ...
            Preconditions.checkState(
                arrayShape[3] == len,
                "Ragged array detected: array[0][0][0].length=%s does not match"
                    + " array[%s][%s][%s].length=%s",
                arrayShape[3],
                i,
                j,
                k,
  ...
  public static int arrayLength(Object current) {
    if (current instanceof Object[]) {
      ...
      throw new IllegalStateException(
          "Unknown array type (or not an array): " + current.getClass()); // Should never happen
  }
  ...
  /**
   * Is this shape an empty shape? Shape is considered to be an empty shape if it contains any
   * zeros. Note: a length 0 shape is NOT considered empty (it's rank 0 scalar)
   *
