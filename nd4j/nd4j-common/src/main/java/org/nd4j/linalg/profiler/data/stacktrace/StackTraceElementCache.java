...
/**
 * Utility class for easier usage of stack trace elements. Allows caching and direct lookup of stack
 * trace elements by class name, method name, and line number.
 */
public class StackTraceElementCache {
  ...
  /**
   * Lookup a stack trace element by key. Note you can also directly use {@link #lookup(String,
   * String, int)} This method is mainly for use cases where you have to deal with multiple stack
   * traces and it could be verbose. Note that when looking up stack traces sometimes user space
   * code can be missing. If a cache entry is missing, we'll attempt to cache the current thread's
   * stack trace elements. Since user input is not guaranteed to be valid, we don't just dynamically
   * create stack trace entries.
   *
   * <p>If your stack trace is missing, ensure you call {@link
   * #storeStackTrace(StackTraceElement[])} on your calling thread.
   *
   * <p>Usually this should be a transparent process included in certain constructors related to the
   * Environment's NDArray logging being set to true.
   *
  ...
  public static void storeStackTrace(StackTraceElement[] stackTrace) {
    ...
    for (StackTraceElement stackTraceElement : stackTrace) {
      if (stackTrace != null) storeStackTraceElement(stackTraceElement);
    }
  }
  ...
  public static void storeStackTraceElement(StackTraceElement stackTraceElement) {
    ...
    StackTraceLookupKey key =
        StackTraceLookupKey.builder()
