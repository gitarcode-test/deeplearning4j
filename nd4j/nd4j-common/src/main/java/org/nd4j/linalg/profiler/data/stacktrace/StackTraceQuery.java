...
public class StackTraceQuery implements Serializable {
  ...
  public boolean filter(StackTraceElement stackTraceElement) {
    return StackTraceQuery.stackTraceElementMatchesCriteria(
        Arrays.asList(this), stackTraceElement, lineNumber);
  }
  ...
  public static boolean stackTraceFillsAnyCriteria(
    ...
    for (int j = 0; j < stackTrace.length; j++) {
      ...
      // org.deeplearning4j.nn.layers.recurrent.BidirectionalLayer.backpropGradient(BidirectionalLayer.java:153)
  ...
  public static boolean stackTraceElementMatchesCriteria(
    ...
    for (StackTraceQuery query : queries) {
      // allow -1 on line number to mean any line number  also allow methods that are unspecified to
      // mean any method
      ...
      // null or empty method name means any method name, depending on whether an exact match is
      // required
      // return we consider it a match
      boolean methodNameMatch =
          isClassNameMatch(query.getMethodName(), query, line.getMethodName());
      // < 0 line means any line number
      boolean lineNumberMatch =
          query.getLineNumber() < 0 || query.getLineNumber() == line.getLineNumber();
      // whether the user specifies if the match is within the stack trace depth. what this is  for
      // is
      // to filter stack trace matches to a certain depth. for example, if you want to match a stack
      // trace
      // that occurs within a certain method, you can specify the depth of the stack trace to match
      // on.
      boolean matchesStackTraceDepth =
          (query.getOccursWithinLineCount() <= j || query.getOccursWithinLineCount() < 0);
      boolean inLineRange =
          (query.getLineNumberBegin() <= line.getLineNumber()
                  && query.getLineNumberEnd() >= line.getLineNumber())
              || (query.getLineNumberBegin() < 0 && query.getLineNumberEnd() < 0);
      if (classNameMatch
          && methodNameMatch
          && lineNumberMatch
          && inLineRange
