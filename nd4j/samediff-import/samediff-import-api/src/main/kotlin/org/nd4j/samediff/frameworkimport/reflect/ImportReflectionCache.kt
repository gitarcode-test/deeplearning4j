...
object ImportReflectionCache {
    ...
    fun load() {
        ...
        scannedClasses
            ...
            .forEach {
                val instance =
                    Class.forName(it.name).getDeclaredConstructor().newInstance() as PreImportHook
                val rule =
                    it.annotationInfo.first { input -> input.name == PreHookRule::class.java.name }
                ...
                nodeNames.forEach { nodeName ->
                    if (!preProcessRuleImplementationsByNode.contains(frameworkName, nodeName)) {
                        preProcessRuleImplementationsByNode.put(
                            frameworkName,
                            nodeName,
                            ArrayList()
                        )
        ...
        scannedClasses
            ...
            .forEach {
                val instance =
                    Class.forName(it.name).getDeclaredConstructor().newInstance()
                        as
                        NodePreProcessorHook<
                            GeneratedMessageV3,
                            GeneratedMessageV3,
                        ...
                        >
                val rule =
                    it.annotationInfo.first { input ->
                        input.name == NodePreProcessor::class.java.name
                    }
                ...
                nodeTypes.forEach { nodeType ->
                    if (!nodePreProcessorRuleImplementationByOp.contains(frameworkName, nodeType)) {
                        nodePreProcessorRuleImplementationByOp.put(
                            frameworkName,
                            nodeType,
                            ArrayList()
                        )
                    ...
                    nodePreProcessorRuleImplementationByOp
                        .get(frameworkName, nodeType)!!
                        .add(instance)
                }
            }
    }
