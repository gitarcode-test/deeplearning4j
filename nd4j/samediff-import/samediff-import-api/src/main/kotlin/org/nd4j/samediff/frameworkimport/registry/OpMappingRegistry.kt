...
>(inputFrameworkName: String, nd4jOpDescriptors: OpNamespace.OpDescriptorList) {
    ...
    fun registerOpDefs(opDefList: Map<String, OP_DEF_TYPE>) {
        opDefList.forEach { (name, inputOpDef) -> registerInputFrameworkOpDef(name, inputOpDef) }
    }
    ...
    fun lookupInputFrameworkOpDef(name: String): OP_DEF_TYPE {
        if (opDefList.isEmpty()) {
            ...
            opList.forEach { (name, opDefType) -> opDefList[name] = opDefType }
        }
    ...
    > {
        ...
        if (!registeredOps.containsKey(inputFrameworkOpName)) {
            ...
            val noRules =
                allRules
                    .cellSet()
                    .filter { input -> input.rowKey == inputFrameworkOpName }
                    .isEmpty()
            if (noRules)
                throw IllegalArgumentException(
                    "No import process defined for $inputFrameworkOpName"
                )
            else {
                println()
            }
        ...
        if (!registeredOps.containsKey(inputFrameworkOpName)) {
            throw IllegalArgumentException(
                "No input framework op name with name $inputFrameworkOpName found!"
            )
        }
    ...
    /**
     * TODO: Make loading op mapping rules (both tensor and attribute), input framework op
     *   definitions casted as OP_DEF_TYPE and op descriptors file.
     * TODO: Get rid of static global constants (onnxops,tensorflow ops)
     * TODO: See if possible to genericize lists of ops
     */
    fun loadFromFile(
        mapperDeclarationsFile: String,
        mappingProcessLoader:
            MappingProcessLoader<
                GRAPH_TYPE,
                OP_DEF_TYPE,
                NODE_TYPE,
    ...
    fun loadFromDefinitions(
        mapperDeclarations: MapperNamespace.MappingDefinitionSet,
        mappingProcessLoader:
            MappingProcessLoader<
                GRAPH_TYPE,
                OP_DEF_TYPE,
    ...
    fun saveProcessesAndRuleSet() {
        ...
        registeredOps.asMap().forEach { (_, listOfMappingProcesses) ->
            ...
            mapperDeclarations
                ...
                .forEach { processString -> bufferToWrite.append(processString + "\n") }
        }
        ...
        FileUtils.write(
            File("$inputFrameworkName-processes.pbtxt"),
            finalSet.toString(),
