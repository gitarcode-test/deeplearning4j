...
/** Change a boolean to an int or an int or double to a boolean */
                                ...
                                when (argDescriptorType) {
                                    OpNamespace.ArgDescriptor.ArgType.DOUBLE -> {
                                        ...
                                        descriptorBuilder.doubleValue =
                                            if (irAttribute.boolValue()) 1.0 else 0.0
                                        descriptorBuilder.argIndex = targetIdx
                                    }
                                    OpNamespace.ArgDescriptor.ArgType.INT64 -> {
                                        descriptorBuilder.argType = argDescriptorType
                                        descriptorBuilder.int64Value =
                                            if (irAttribute.boolValue()) 1 else 0
                                        descriptorBuilder.argIndex = targetIdx
                                    }
                                    else -> {
                                        throw IllegalArgumentException(
                                            "Illegal type passed in $argDescriptorType"
                                        )
                                    }
                                }
