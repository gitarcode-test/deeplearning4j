...
>(
    ...
    abstract fun createIRAttribute(
        name: String,
        attrDef: ATTR_DEF,
        attributeValueType: ATTR_VALUE_TYPE
    ): IRAttribute<ATTR_DEF, ATTR_VALUE_TYPE, TENSOR_TYPE, DATA_TYPE>
            ...
            v.forEach { descriptor ->
                when (descriptor.argType) {
                    OpNamespace.ArgDescriptor.ArgType.STRING ->
                        builder.addInputStringAttrName(descriptor.name)
                    OpNamespace.ArgDescriptor.ArgType.BOOL ->
                        builder.addInputBooleanName(descriptor.name)
                    OpNamespace.ArgDescriptor.ArgType.DOUBLE,
                    OpNamespace.ArgDescriptor.ArgType.FLOAT ->
                        builder.addInputFloatName(descriptor.name)
                    OpNamespace.ArgDescriptor.ArgType.INT32,
                    OpNamespace.ArgDescriptor.ArgType.INT64 ->
                        builder.addInputIntName(descriptor.name)
                    OpNamespace.ArgDescriptor.ArgType.INPUT_TENSOR ->
                        builder.addInputTensorName(descriptor.name)
                    else -> {
                        throw IllegalArgumentException("Illegal arg type ${descriptor.argType}")
                    }
                }

                builder.addTransformerArgs(
                    MapperNamespace.TransformerArgs.newBuilder().setKey(k).addAllTransformerArgs(v)
                )
            }
        }

        /**
         * TODO: metadata (perhaps looking up from each framework for each attribute) what each
         *   named type is.
         */
        mappingNamesToPerform.forEach { outputName, inputName ->
            val descriptorForName =
                opDescriptor!!.argDescriptorList.first { descriptor ->
                    descriptor.name == outputName
                }
            builder.putInputToOutput(outputName, inputName)
            when (descriptorForName.argType) {
                OpNamespace.ArgDescriptor.ArgType.BOOL -> {
                    builder.addOutputBooleanName(outputName)
                }
                OpNamespace.ArgDescriptor.ArgType.INT64 -> {
                    builder.addOutputIntName(outputName)
                }
                OpNamespace.ArgDescriptor.ArgType.DOUBLE -> {
                    builder.addOutputDoubleName(outputName)
                }
                OpNamespace.ArgDescriptor.ArgType.DATA_TYPE ->
                    builder.addOutputDataTypeName(outputName)
                OpNamespace.ArgDescriptor.ArgType.OUTPUT_TENSOR ->
                    builder.addOutputTensorName(outputName)
                OpNamespace.ArgDescriptor.ArgType.STRING ->
                    builder.addOutputStringAttrName(outputName)
                OpNamespace.ArgDescriptor.ArgType.INPUT_TENSOR ->
                    builder.addInputTensorName(outputName)
                else -> {
                    throw IllegalArgumentException("Illegal type ${descriptorForName.argType}")
                }
            }
            ...
            if (inputOpDefTypes!!.containsKey(inputName)) {
                when (inputOpDefTypes!![inputName]!!) {
                    ...
                    AttributeValueType.LIST_FLOAT -> builder.addInputFloatName(inputName)
                    AttributeValueType.INT,
                    AttributeValueType.LIST_INT -> builder.addInputIntName(inputName)
                    AttributeValueType.BOOL,
                    AttributeValueType.LIST_BOOL -> builder.addInputBooleanName(inputName)
                    AttributeValueType.STRING,
                    AttributeValueType.LIST_STRING -> builder.addInputStringAttrName(inputName)
                    ...
                    AttributeValueType.TENSOR -> builder.addInputTensorName(inputName)
                        ...
                        throw IllegalArgumentException(
                            "Illegal type ${inputOpDefTypes!![inputName]!!}"
                        )
                    }
                }
            }
    ...
    override fun argDescriptorTypesForOutputName(
        ...
        if (!names.contains(name)) {
            throw java.lang.IllegalArgumentException(
                "Unable to find name $name for op $nd4jOpDescriptor.name"
            )
        }

        return nd4jOpDescriptor.argDescriptorList
            .filter { argDescriptor -> argDescriptor.name == name }
