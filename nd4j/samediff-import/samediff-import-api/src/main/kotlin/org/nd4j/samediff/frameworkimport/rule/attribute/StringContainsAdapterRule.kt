            ...
            argDescriptorTypeList.forEach { argDescriptorType ->
                ...
                when (argDescriptorType) {
                    OpNamespace.ArgDescriptor.ArgType.BOOL -> {
                        descriptorBuilder.boolValue =
                            testValue.isNotEmpty() && compString.contains(testValue)
                    }
                    OpNamespace.ArgDescriptor.ArgType.INT64 -> {
                        descriptorBuilder.int64Value =
                            if (testValue.isNotEmpty() && compString.contains(testValue)) 1 else 0
                    }
                    OpNamespace.ArgDescriptor.ArgType.FLOAT ->
                        descriptorBuilder.floatValue =
                            if (testValue.isNotEmpty() && compString.contains(testValue)) 1.0f
                            else 0.0f
                    OpNamespace.ArgDescriptor.ArgType.DOUBLE ->
                        descriptorBuilder.doubleValue =
                            if (testValue.isNotEmpty() && compString.contains(testValue)) 1.0
                            else 0.0
                    OpNamespace.ArgDescriptor.ArgType.INT32 ->
                        descriptorBuilder.int32Value =
                            if (testValue.isNotEmpty() && compString.contains(testValue)) 1 else 0
                    OpNamespace.ArgDescriptor.ArgType.INPUT_TENSOR -> {
                        descriptorBuilder.inputValue =
                            if (testValue.isNotEmpty() && compString.contains(testValue))
                                nameSpaceTensorFromNDarray(Nd4j.scalar(true))
                            else nameSpaceTensorFromNDarray(Nd4j.scalar(false))
                    }
                    OpNamespace.ArgDescriptor.ArgType.OUTPUT_TENSOR -> {
                        descriptorBuilder.outputValue =
                            if (testValue.isNotEmpty() && compString.contains(testValue))
                                nameSpaceTensorFromNDarray(Nd4j.scalar(true))
                            else nameSpaceTensorFromNDarray(Nd4j.scalar(false))
                    }
                    OpNamespace.ArgDescriptor.ArgType.STRING ->
                        descriptorBuilder.stringValue =
                            if (testValue.isNotEmpty() && compString.contains(testValue)) "true"
                            else "false"
