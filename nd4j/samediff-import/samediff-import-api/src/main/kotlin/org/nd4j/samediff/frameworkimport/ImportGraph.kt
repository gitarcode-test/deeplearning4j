...
/**
 * Core import class for running model import for any framework. This should be paired with an
 * [OpMappingRegistry] and a set of classes implemented in protobuf that extend [GeneratedMessageV3]
 * and [ProtocolMessageEnum] respectively.
 *
 * The end result with these abstractions is direct interop with a file format's schema convertable
 * to primitives like Nd4j's [INDArray] and [SameDiff]
 *
 * @author Adam Gibson
 */
open class ImportGraph<
...
> {
    ...
    > {
        ...
        val opMappingRegistry =
            OpRegistryHolder.opMappingRegistryForName<
                GRAPH_TYPE,
                NODE_TYPE,
                OP_DEF_TYPE,
                TENSOR_TYPE,
        ...
        graph.nodeList().forEach { node ->
            ...
            val opMappingProcess =
                OpRegistryHolder.lookupOpMappingProcess<
                    ...
                    ATTRIBUTE_VALUE_TYPE
                >(
                    inputFrameworkOpName = node.opName(),
                    inputFrameworkName = graph.frameworkName()
                )
            val opDefLookup = opMappingRegistry.lookupInputFrameworkOpDef(node.opName())
            val mappingContext =
                graph.createMappingContext(
                    opDef = opDefLookup,
    ...
    /**
     * @return The specified name without the leading "^" character (if any) that appears for
    ...
    inner class FuncContextResult<
        ...
        DATA_TYPE : ProtocolMessageEnum
    >(
        dfInstance: DifferentialFunction,
        mappingContext:
            MappingContext<
                GRAPH_TYPE,
                NODE_TYPE,
    ...
    > {
        ...
        val dfInstance =
            if (DifferentialFunctionClassHolder.getInstance().hasName(nd4jOpName))
                DifferentialFunctionClassHolder.getInstance(nd4jOpName)
            else DynamicCustomOp.builder(nd4jOpName).build()
        Preconditions.checkState(
            dfInstance != null,
            "Could not find class for input framework Ops: %s",
            opName
        )
        var df: DifferentialFunction =
            try {
                ...
                // Should never happen because function was already created via no-arg constructor
                // earlier
        ...
        /**
         * Note that ndarrays actually need to be reordered here when input indices aren't equal to
         * what's in the original framework. We should potentially run the import process sooner and
         * compute the input name ordering from that instead.
         */
        val opDefLookup = opMappingRegistry.lookupInputFrameworkOpDef(opName)
        val mappingContext =
            irGraph.createMappingContext(
                opDef = opDefLookup,
    ...
    /**
     * Import a Graph based on a {@link IRGraph} model from a GraphDef, with optional import
     * overrides
     *
    ...
    fun importGraph(
        ...
        val availableToAdd:
            Queue<IRNode<NODE_TYPE, TENSOR_TYPE, ATTR_DEF_TYPE, ATTR_VALUE_TYPE, DATA_TYPE>> =
            LinkedList()
        val remainingNodes:
            MutableMap<
                String,
                IRNode<NODE_TYPE, TENSOR_TYPE, ATTR_DEF_TYPE, ATTR_VALUE_TYPE, DATA_TYPE>
            > =
            HashMap() // All other nodes, not in availableToAdd
        val nodeInputTo: MutableMap<String, ListOrderedSet<String>> =
            HashMap() // For op x -> y, x is key, y is value. Note that these are OP names not
                      // VARIABLE names
        var nNodes: Int
        ...
        if (dynamicVariables != null) {
            ...
            dynamicVariables.forEach { (name, ndarray) ->
                ...
                /**
                 * TODO: convert placeholders to proper data types with checking. It appears not all
                 *   dyanmicVariables will match expected data type.
                 */
                if (!sd.hasVariable(name)) sd.`var`(name, converted)
        ...
        /**
         * Now the nodes in the graph may change after running an import process. Run an import
         * process first before proceeding to process all the nodes in the graph
         */
        val originalNodeList = irGraph.nodeList()
        val nodeNameToFuncContext =
            HashMap<
                String,
                FuncContextResult<
                    GRAPH_TYPE,
        ...
        for (i in 0 until nNodes) {
            ...
            if (irGraph.isConstantOpName(op) || numInputs == 0) {
                ...
                for (inputIdx in 0 until numInputs) {
                    ...
                    if (!nodeInputTo[inOpName]!!.contains(name)) nodeInputTo[inOpName]!!.add(name)
                }
                ...
                if (irGraph.addGraphOutputsAsProcessingNodes()) {
                    ...
                    for (outputIdx in 0 until numOutputs) {
                        ...
                        if (irGraph.hasNode(outOpName) && !irGraph.isConstant(outOpName)) {
                            ...
                            // no node for output name, avoid duplicates being added to the
                            // processing
                            // queue
        ...
        val constControlDeps: MutableMap<String, List<String>> =
            HashMap() // Key: constant name. Value: control dependencies
        ...
        while (!availableToAdd.isEmpty()) {
            ...
            if (
                opFilter != null &&
                    opFilter.skipOp(nd.internalValue(), sd, rawAttrMap, irGraph.internalValue())
            ) {
                ...
                if (importOverride == null || !importOverride.containsKey(name)) {
                    ...
                    if (irGraph.nodeIsPlaceHolder(nd.nodeName())) {
                        ...
                        if (!sd.hasVariable(nd.nodeName())) {
                            ...
                            if (shape != null) sd.placeHolder(name, dt, *shape)
                            else sd.placeHolder(name, dt)
                        } else {
                            ...
                            if (sdVar.arr == null && dynamicVariables.containsKey(nd.nodeName())) {
                                ...
                                val castedArr =
                                    irGraph
                                        .convertToNDArray(dynamicVariables[nd.nodeName()]!!)
                                        .castTo(dt)
                                sd.associateArrayWithVariable(castedArr, sdVar)
                                dynamicVariables[nd.nodeName()] =
                                    irGraph.convertToTensor(castedArr, nd.nodeName())
                        ...
                        if (!sd.hasVariable(nd.nodeName())) {
                            ...
                            if (
                                nd.numOutputs() < 1 ||
                                    irGraph.frameworkName().contains("tensorflow")
                            )
                                sd.constant(name, arr)
                            else // onnx case
                             sd.constant(nd.outputAt(0), arr)
                            logger.debug {
                                "Added constant for node name ${nd.nodeName()} with shape ${arr.shapeInfoToString()}"
                            }
                            val inputCount = nd.numInputs()
                            if (inputCount > 0) {
                                // Very likely control dependency. i.e., "we must execute op X
                                // before the constant is really available to be used"
                                val l: MutableList<String> = ArrayList(inputCount)
                                for (i in 0 until inputCount) {
                                    val n = nd.inputAt(i)
                                    check(isControlDep(n)) {
                                        "Found non-control dependency input \"$n\" for constant \"$name\""
                                    }
                        ...
                        irGraph.isVariable(nd.nodeName()) && !sd.hasVariable(nd.nodeName())
                    ) {
                        var shape = irGraph.shapeOfInput(nd.nodeName())
                        val dt = irGraph.dataTypeForVariable(nd.nodeName()).nd4jDataType()
                        if (shape != null) sd.`var`(name, dt, *shape) else sd.`var`(name, dt, -1)
                        ...
                        for (i in 0 until numInputs) {
                            // use input name if it exists and matches, otherwise if the input names
                            // do not map 1 to 1 for import
                            // use samediff to generate a unique name
                            val origInName = nd.inputAt(i)
                            var inName = stripControl(origInName)
                            if (inName.endsWith(":0")) {
                                // Strip ":0" suffix. Some ops can depend on placeholders, like
                                // "image_tensor:0" but in SameDiff this is a variable called
                                // "image_tensor"
                            ...
                            // Such variables must have already been created, given we process in
                            // order
                            // declare empty variable for anything that's an input > 0
                            if (
                                !sd.hasVariable(inName) &&
                                    irGraph.frameworkName().contains("tensorflow") &&
                                    inName.contains(':')
                            ) {
                                ...
                                if (!sd.hasVariable(knownBaseName)) {
                                    throw IllegalArgumentException(
                                        "No variable name found for $knownBaseName"
                                    )
                                } else {
                            ...
                            // auto declare variables if they don't exist, avoid constants. Pull
                            // constants out
                            // from the graph and initialize them if an input name appears before a
                            // mention of a constant.
                            // This can happen in certain frameworks. Sometimes frameworks will have
                            // auto sorted
                            // DAGS, this may not be true for all situations though.
                            // note, we only want variables being auto declared if they are actually
                            // inputs or outputs not only nodes
                            if (
                                !isControlDep &&
                                    !sd.hasVariable(inName) &&
                                    !irGraph.hasConstantInitializer(inName) &&
                                    irGraph.isInputOrOutput(inName)
                            ) {
                                val otherInputs =
                                    nd.inputs().filter { input -> sd.hasVariable(input) }
                                var dataType = DataType.FLOAT
                                // guess input from other data types
                                if (otherInputs.isNotEmpty()) {
                                    dataType = sd.getVariable(otherInputs[0]).dataType()
                                }
                                sd.`var`(SDVariable(inName, VariableType.ARRAY, sd, null, dataType))
                            } else if (
                                !isControlDep &&
                                    !sd.hasVariable(inName) &&
                                    irGraph.hasConstantInitializer(inName)
                                ...
                                throw IllegalStateException(
                                    "Input variable at index $i named $inName of node $name was not assigned to any variable"
                                )
                            }

                            val v = sd.variables[inName]
                            if (v == null && df is Merge) {
                                // Edge case for import - we allow merge ops to be added before both
                                // inputs are available
                                // This is to break the cycles in loops, otherwise we can't process
                                // anything in order
                            ...
                            if (
                                v != null &&
                                    !isControlDep &&
                                    (v!!.inputsForOp == null || !v.inputsForOp.contains(name))
                            ) {
                        ...
                        val op =
                            SameDiffOp.builder().name(name).op(df).controlDeps(controlDeps).build()
                        // take only up to the inputs that are specified in the node/
                        // this is for cases where node inputs is > intended number for ops
                        // a common example is when ops convert input ndarrays to integers or float
                        // inputs
                        val resolvedArgInputs =
                            importInfo[name]!!
                                .second
                                .argDescriptorList
                                .filter { input ->
                            ...
                            when (opMappingProcess.arrayResolutionType()) {
                                ...
                                MapperNamespace.VariableResolutionType.OVERRIDE -> {
                                    if (numInputsToTake < inNames.size)
                                        ...
                                        resolvedArgInputs
                                            .subList(inputsAfterOriginal, resolvedArgInputs.size)
                                            .forEach { arg ->
                                                val newName =
                                                    sd.generateNewVarName(
                                                        "${op.name}_${arg.name}",
                                                        0
                                                    )
                                                ...
                                                if (!sd.hasVariable(op.inputsToOp[arg.argIndex])) {
                                                    if (arg.inputValue != null) {
                                                        sd.`var`(
                                                            op.inputsToOp[arg.argIndex],
                                                            ndarrayFromNameSpaceTensor(
                                                                arg.inputValue
                                                            )
                                                        )
                                                    } else {
                                                        throw java.lang.IllegalArgumentException(
                                                            "No argument value found for op ${op.name} for value arg with name ${arg.name}"
                                                        )
                                                    }
                                                }
                                            }
                                    } else op.inputsToOp = inNames

                                    // clear out inputs for variables as well to reflect the actual
                                    // graph structure
                                    // NO OP NOTE: we make an exception for no op mappings. No op
                                    // mappings are a potential
                                    // signal that we are using  pre hook rules as substitutions for
                                    // operations
                                    // without a mapping process. This can happen when we don't have
                                    // an exact mapping
                                    // for an op and need a way of substituting the op with an
                                    // equivalent set of samediff
                                    // op calls. Specifying no nop as a way of handling mapping
                                    // processes allows a special
                                    // sentinel value  that , in combination with pre hook rules,
                                    // can be used
                                    // to substitute ops when they may not otherwise be supported.
                                    // The reason we can't take away the inputs is the user may
                                    // specify the inputs
                                    // to the op and the hook rule may need those inputs to use as a
                                ...
                                MapperNamespace.VariableResolutionType.ERROR_ON_NOT_EQUAL -> {
                                    throw java.lang.IllegalStateException(
                                        "Number of variable names for node ${mappingContext!!.nodeName()} not exact equal to number of inputs resolved from nd4j op descriptor which was ${resolvedArgInputs.size}"
                                    )
                                }
                                MapperNamespace.VariableResolutionType.UNRECOGNIZED -> {
                                    throw java.lang.IllegalArgumentException(
                                        "Illegal type ${opMappingProcess.arrayResolutionType()}"
                                    )
                                }
                            }
                        ...
                        // cache attributes just in case we have any rules so we don't create the
                        ...
                        if (proceedWithInit && !sd.ops.containsKey(name)) sd.ops[name] = op
                        ...
                        if (mappingContext.relevantPrehookRules().isEmpty()) {
                            val newInNames =
                                sd.ops[name]!!
                                    .inputsToOp // Just in case import has modified this, like for
                                                // concat case
                            val newInDtypes: MutableList<DataType> = ArrayList(newInNames.size)
                            if (df is Merge) {
                                // Merge op: as noted elsewhere, we allow merge to be processed when
                                // only one of the inputs is available
                                // to break cycles for loops
                                // We know that Merge op has the restriction of the same datatype
                                // for both inputs, so we'll
                            ...
                            // note we validate the op definition here to ensure that all ops have
                            // at least 1 output unless otherwise specified.
                            ...
                            if (numInputs < 1 && nd4jOpName != "noop") {
                                throw IllegalStateException(
                                    "Op $nd4jOpName does not have any outputs!"
                                )
                            }

                            // logger.debug {"Out dtypes size ${outDTypes.size} and numOutputs
                            // $numOutputs")
                            ...
                            for (i in 0 until numOutputs) {
                                ...
                                outSDVars[i] =
                                    if (sd.hasVariable(varName)) sd.getVariable(varName)
                                    else sd.`var`(varName, VariableType.ARRAY, null, dt)
                                ...
                                if (sd.variables.containsKey(varName)) {
                                    ...
                                    outVars[i] =
                                        Variable.builder()
                                            ...
                                            .inputsForOp(
                                                null
                                            ) // This is updated incrementally as other ops are
                                              // added
                                            .controlDepsForOp(
                                                null
                                            ) // Control deps are handled later
                                ...
                                logger.debug {
                                    "Added variable to graph: $varName (output of op $name)"
                                }
                            ...
                            // don't run computeArrays if graph contains control flow, too many edge
                            // cases
                        ...
                        logger.debug {
                            "Node ${nd.nodeName()} not found in import context, skipping!"
                        }
                    }
                } else {
                    ...
                    val dfInstance =
                        if (DifferentialFunctionClassHolder.getInstance().hasName(opName))
                            DifferentialFunctionClassHolder.getInstance(opName)
                    ...
                    df =
                        try {
                            ...
                            // Should never happen because function was already created via no-arg
                            // constructor earlier
                            throw RuntimeException(t)
                        }
                    ...
                    val opInputs =
                        opDescriptor.argDescriptorList.filter { argDescriptor ->
            ...
            // Now that we have just added an op (or variable) - check what this feeds into, and see
            // what we can now process
            // as a result
            if (nodeInputTo.containsKey(name)) {
                ...
                for (nextOp in set!!) {
                    ...
                    if (nextOpDef == null) {
                        ...
                        if (
                            sd.ops.containsKey(nextOp) ||
                                opSet.contains(importInfoForNode!!.first.nd4jOpName())
                        ) {
                            // Already processed this.
                            // Almost certainly the close of a loop - like NextIteration -> Merge
                            // case
                            continue
                        }
                        throw IllegalStateException(
                            "Could not find op definition for op to import: $nextOp"
                        )
                    }
                    ...
                    // Merge ops are an edge case. We'll allow these to be executed with just ONE
                    // input, to break
                    // the cycle in loops. In loops, generally we have (Enter, NextIteration) ->
                    // Merge, which
                    ...
                    if (allAlreadyInGraph || mergeCase) {
                        ...
                        if (!availableToAddSet.contains(nextOp)) {
                            // Avoid processing same op multiple times, for repeated inputs to one
                            // op, etc
                            availableToAdd.add(nextOpDef)
                            logger.debug { "Added ${nextOpDef.nodeName()}" }
                            availableToAddSet.add(nextOp)
                            logger.debug {
                                "Added to processing queue: ${nextOpDef.opName()} (name=$nextOp)"
                            }
                        }
                    }
                }
        ...
        // Post process the control dependencies, if any (done after because dependencies may not
        // exist when imported)
        ...
        // Post process the merge ops - all we are missing is a
        // Variable.getInputsForOp().add(mergeOpName);
    ...
    private fun renameOp(secondOp: SameDiffOp, firstOp: SameDiffOp, sd: SameDiff) {
