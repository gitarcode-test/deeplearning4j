...
>(
    ...
    override fun convertAttributes(
        ...
        for ((k, v) in mappingNamesToPerform()) {
            ...
            val stringVal =
                mappingCtx.irAttributeValueForNode(v).listStringValue()[indexOfValue.toInt()]
            val activationInt =
                (transformerArgs[k]
                        ?: error(
                            "Unable to map value $v to a type string for op name ${mappingCtx.nd4jOpName()} and input op name ${mappingCtx.opName()}"
                        ))
                    .filter { argDescriptor -> argDescriptor.name == stringVal }
                    .map { argDescriptor -> argDescriptor.int64Value }
                    .first()
