...
class OnnxIRGraph(
    ...
    init {
        // sometimes onnx nodes will have empty names, ensure that each node has a deterministically
        // generated name
        ...
        cachedNodeList.forEachIndexed { index, node ->
            if (node.nodeName().isEmpty()) {
                ...
                if (node.numOutputs() > 1) {
                    println(
                        "Found node with no name and > 1 input.  Node was $node. Using first output as name."
                    )
                }
        ...
        val initializers =
            this.graphDef.initializerList.map { input -> input.name.replace(":0", "") }
        println(initializers)
        val inputList =
            this.graphDef.inputList
                .filter { input ->
                    !opTypes.containsKey(input.name.replace(":0", "")) &&
                        !initializers.contains(input.name.replace(":0", ""))
                }
                .map { input -> input.name.replace(":0", "") }
        val varList =
            this.graphDef.inputList
                .filter { input -> initializers.contains(input.name.replace(":0", "")) }
                .map { input -> input.name.replace(":0", "") }
        ...
        outputList.addAll(
            this.graphDef.outputList
                .filter { valueInfo -> !valueInfo.name.contains(valueInfo.name) }
                .map { input -> input.name.replace(":0", "") }
        )
    }

    /**
     * Handle zero suffixes such that the suffixes are removed. This is for when you import a
     * tensorflow model or import a model from tf onnx and need to handle the :0 edge case which is
     * pretty common when interacting with anything that came from tensorflow.
     */
    ...
    override fun nodeList():
        ...
        // for model import purposes, add identity ops as dummies similar to how tensorflow does
        // placeholders/constants
        val initializerListNames =
            graphDef.initializerList.map { input -> input.name.replace(":0", "") }
        graphDef.inputList
            .filter { input -> !initializerListNames.contains(input.name.replace(":0", "")) }
            .forEach { input ->
                // note: this is not a real op name in onnx, this is purely for flagging for import
                // to grab the node from the initializer
                ...
                input.type.tensorType.shape.dimList.forEach { tensorBuilder.addDims(it.dimValue) }
        ...
        inputList.addAll(
            graphDef.inputList
                .filter { input -> !initializerListNames.contains(input.name) }
                .map { input -> input.name }
        )
        outputList.addAll(
            graphDef.outputList
                .filter { valueInfo -> !outputList.contains(valueInfo.name) }
                .map { input -> input.name }
        )
        ...
        graphDef.nodeList.forEach {
            val opDefOrNull =
                if (!frameworkList.containsKey(it.opType)) {
                    // use Constant as a placeholder for any op that resolves to noop, this is
                    // probably an op handled by the custom implementation
    ...
    override fun shapeOfInput(varName: String): LongArray? {
        val firstOrNull =
            graphDef.initializerList.firstOrNull { inputNode -> inputNode.name == varName }
        if (firstOrNull != null) return firstOrNull.dimsList.toLongArray()
            ...
            for (i in ret.indices) {
                // missing dimension, probably dynamic, infer as -1 to match dynamic shape behavior
                // in samediff
                if (ret[i] == 0L) {
                    ret[i] = -1
                }
    ...
    override fun dataTypeForVariable(varName: String): IRDataType<Onnx.TensorProto.DataType> {
        ...
        val firstOrNull =
            graphDef.initializerList.firstOrNull { inputNode -> inputNode.name == varNameStripped }
        val input = graphDef.inputList.firstOrNull { input2 -> input2.name == varNameStripped }
        if (firstOrNull != null)
            ...
            if (input != null && input.type.hasTensorType()) {
                return OnnxIRDataType(
                    Onnx.TensorProto.DataType.forNumber(input.type.tensorType.elemType)
                )
            } else if (input != null && input.type.hasSequenceType()) {
                return OnnxIRDataType(
                    Onnx.TensorProto.DataType.forNumber(
                        input.type.sequenceType.elemType.tensorType.elemType
                    )
                )
            }
        ...
        else return OnnxIRDataType(Onnx.TensorProto.DataType.UNDEFINED)
    }

    override fun importInfoForEachNode(
        dynamicVariables: MutableMap<String, Onnx.TensorProto>
    ): Map<
        String,
        Pair<
    ...
    override fun opMappingRegistry():
        OpMappingRegistry<
            Onnx.GraphProto,
            Onnx.NodeProto,
            Onnx.NodeProto,
    ...
    override fun addConstantNode(name: String, value: INDArray) {
        ...
        val tensorShapeInfo = TensorTypeProto {
            shape = OnnxShapeProto { OnnxShape(value.shape().toList()) }
        }

        val valueType = TypeProto { tensorType = tensorShapeInfo }

        val newValueInfo = ValueInfoProto { Type(valueType) }
    ...
    override fun getConstantArrayForName(name: String): INDArray {
        ...
        return OnnxIRTensor(graphDef.initializerList.first { input -> input.name == name })
            .toNd4jNDArray()
    }
    ...
    > {
        ...
        return OnnxIRNode(
            node,
            opMappingRegistry.lookupInputFrameworkOpDef(node.opType),
            opMappingRegistry
        )
    }
    ...
    override fun updateNodeCacheWith(
        nodeList:
            List<
                IRNode<
                    Onnx.NodeProto,
        ...
        nodeList.forEach { graphDefBuilder.addNode(it.internalValue()) }
