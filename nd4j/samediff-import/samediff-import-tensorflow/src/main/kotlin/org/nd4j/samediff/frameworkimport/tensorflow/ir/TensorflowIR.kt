...
fun loadTensorflowOps(): OpList {
    val string =
        IOUtils.toString(ClassPathResource("ops.proto").inputStream, Charset.defaultCharset())
...
fun tensorflowAttributeValueTypeFor(attributeName: String, opDef: OpDef): AttributeValueType {
    ...
    if (!names.contains(attributeName) && !isTensorflowTensorName(attributeName, opDef)) {
        throw java.lang.IllegalArgumentException(
            "Tensorflow op ${opDef.name} does not have attribute name $attributeName"
        )
    } else if (isTensorflowTensorName(attributeName, opDef)) {
        // note we allows tensors here since sometimes input tensors in tensorflow become attributes
        // in nd4j
 ...
 * GeneratedMessageV3, ATTR_DEF_TYPE : GeneratedMessageV3, ATTR_VALUE_TYPE : GeneratedMessageV3,
 * DATA_TYPE: ProtocolMessageEnum > initAttributes( df: DifferentialFunction, applied:
 * Pair<MappingContext<NODE_TYPE, OP_DEF_TYPE, TENSOR_TYPE, ATTR_DEF_TYPE, ATTR_VALUE_TYPE,
 * DATA_TYPE>, OpNamespace.OpDescriptor>, mappingContext: MappingContext<NODE_TYPE, OP_DEF_TYPE,
 * TENSOR_TYPE, ATTR_DEF_TYPE, ATTR_VALUE_TYPE, DATA_TYPE>, sd: SameDiff )
 */
...
fun mapTensorProto(tfTensor: TensorProto): INDArray {
    val m =
        TFTensorMappers.newMapper(tfTensor)
            ?: throw RuntimeException("Not implemented datatype: " + tfTensor.dtype)
    return m.toNDArray()
}
